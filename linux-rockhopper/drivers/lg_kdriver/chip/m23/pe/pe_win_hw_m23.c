...
----------------------------------------------------------------------------------------*/
#define PE_WIN_HW_M23_UHD_H_MAX			3840
#define PE_WIN_HW_M23_UHD_V_MAX			2160
#define PE_WIN_HW_M23_FHD_H_MAX			1920
#define PE_WIN_HW_M23_FHD_V_MAX			1080
#define PE_WIN_HW_M23_SHD_H_MAX			1366
#define PE_WIN_HW_M23_SHD_V_MAX			768
#define PE_WIN_HW_M23_SHP_WIN			0
#define PE_WIN_HW_M23_CEN_WIN			1
#define PE_WIN_HW_M23_WIN_EN_OFF		0
#define PE_WIN_HW_M23_WIN_EN__ON		1
#define PE_WIN_HW_M23_P1SHP_WIN_OFST	64

#define PE_WIN_HW_M23_WIN_OFFSET_MAX	(4)

/*----------------------------------------------------------------------------------------
	Macro Definitions
----------------------------------------------------------------------------------------*/
#define PE_WIN_HW_M23_CHECK_CODE(_checker,_action,fmt,args...)	\
...
----------------------------------------------------------------------------------------*/
static void PE_WIN_HW_M23_SetVerWinReverse(UINT32 *y0,UINT32 *y1,UINT32 v_max);
static void PE_WIN_HW_M23_GetWinParams(PE_TSK_M23_CFG_T *i_pstCfg, LX_PE_WIN_PARAMS_T *o_pstWinPrms);
static void PE_WIN_HW_M23_GetActNWinCrdsForExtFRC(PE_TSK_M23_CFG_T 			*i_pstCfg,
												LX_PE_WIN_PARAMS_T 			*i_pstWinPrms,
												LX_PE_ACT_WIN_COORDINATES_T *o_pstActWinCrds);
static void PE_WIN_HW_M23_GetShpCenWinEnForExtFRC(PE_TSK_M23_CFG_T 		*i_pstCfg,
												BOOLEAN					i_bIsResizeSet,
												LX_PE_SHP_CEN_WIN_EN_T	*o_pstShpCenWinEn);
static void PE_WIN_HW_M23_ValidateWinCoords(LX_PE_WIN_PARAMS_T 			*i_pstWinPrms,
											LX_PE_ACT_WIN_COORDINATES_T *io_pstActWinCrds);
static void PE_WIN_HW_M23_SetCen(PE_TSK_M23_CFG_T 				*i_pstCfg,
								 LX_PE_WIN_PARAMS_T 			*i_pstWinPrms,
								 LX_PE_ACT_WIN_COORDINATES_T 	*i_pstActWinCrds);
static void PE_WIN_HW_M23_SetShp(PE_TSK_M23_CFG_T 				*i_pstCfg,
								 LX_PE_WIN_PARAMS_T 			*i_pstWinPrms,
								 LX_PE_ACT_WIN_COORDINATES_T 		*i_pstActWinCrds);
static void PE_WIN_HW_M23_SetShpWinEn(PE_TSK_M23_CFG_T 				*i_pstCfg,
									 LX_PE_WIN_PARAMS_T 			*i_pstWinPrms,
									 LX_PE_SHP_CEN_WIN_EN_T 		*i_pstShpCenWinEn);
static void PE_WIN_HW_M23_SetCenWinEn(PE_TSK_M23_CFG_T 				*i_pstCfg,
									 LX_PE_WIN_PARAMS_T 			*i_pstWinPrms,
									 LX_PE_SHP_CEN_WIN_EN_T 		*i_pstShpCenWinEn);
static void PE_WIN_HW_M23_SetWinResolution(PE_TSK_M23_CFG_T 			*i_pstCfg,
										   LX_PE_WIN_PARAMS_T 			*i_pstWinPrms);
...
/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
static UINT32 _g_win_hw_m23_trace=0x0;	//default should be off.
static PE_WIN_HW_M23_SETTINGS_T _g_pe_win_hw_m23_info;
 ...
 * @author
 */
int PE_WIN_HW_M23_Init(PE_CFG_CTRL_T *pstParams)
{
	int ret = RET_OK;
	#ifdef PE_HW_M23_BRINGUP
	PE_REG_M23_HW_OPT_T hw_opt;
	do{
		CHECK_KNULL(pstParams);
		if(PE_KDRV_VER_M23)
		{
			memset(&_g_pe_win_hw_m23_info,-1,sizeof(PE_WIN_HW_M23_SETTINGS_T));
			PE_DBG_WIN("pe1 win ctrl init, inside.\n");
			hw_opt = PE_REG_M23_GetDispOpt();
			_g_pe_win_hw_m23_info.is_external_chip = hw_opt.external_chip;
			/*esf*/
			#if 0
			PE_SHP_M23_QWr05(shp_esf_win_ctrl_00, 		reg_win_esf_win0_en,0x1, \
 ...
 * @author
 */
int PE_WIN_HW_M23_SetDebugSettings(LX_PE_DBG_SETTINGS_T *pstParams)
{
	int ret = RET_OK;
	do{
		CHECK_KNULL(pstParams);
		...
		{
			_g_win_hw_m23_trace = (pstParams->print_lvl & (0x1<<PE_ITEM_PKTMODL(WIN)))? 0x1:0x0;
		}
		PE_PRINT_NOTI("wid:%d,type:0x%x,[0x%x]print:0x%x(%d),bypass:0x%x\n",\
			pstParams->win_id,pstParams->type,(0x1<<PE_ITEM_PKTMODL(WIN)),\
			pstParams->print_lvl,_g_win_hw_m23_trace,pstParams->bypass);
	}while(0);
	return ret;
}
/**
 ...
 * @author
 */
int PE_WIN_HW_M23_SetEnhancingWindow(PE_TSK_M23_CFG_T *pCfg)
{
	int ret 						= RET_OK;
	#ifdef PE_HW_M23_BRINGUP
	...
	do {
		...
		if(PE_KDRV_VER_M23) {
			...
			PE_WIN_HW_M23_GetWinParams(pCfg, &stWinPrms);

			PE_WIN_HW_M23_GetActNWinCrdsForExtFRC(pCfg, &stWinPrms, &stActWinCrds);

			/* reverse mode */
			if(disp0_info->mode.is_reverse)	{
				PE_WIN_HW_M23_SetVerWinReverse(&stActWinCrds.act0_y0, &stActWinCrds.act0_y1, stWinPrms.v_max);
				PE_WIN_HW_M23_SetVerWinReverse(&stActWinCrds.act1_y0, &stActWinCrds.act1_y1, stWinPrms.v_max);
				PE_WIN_HW_M23_SetVerWinReverse(&stActWinCrds.win0_y0, &stActWinCrds.win0_y1, stWinPrms.v_max);
				PE_WIN_HW_M23_SetVerWinReverse(&stActWinCrds.win1_y0, &stActWinCrds.win1_y1, stWinPrms.v_max);
			}
			...
			PE_WIN_HW_M23_GetShpCenWinEnForExtFRC(pCfg, is_resize_case, &stShpCenWinEn);

			/* 20130221, sj.youm, check valid window coordinate */
			PE_WIN_HW_M23_ValidateWinCoords(&stWinPrms, &stActWinCrds);

			/* set act(cen) */
			PE_WIN_HW_M23_SetCen(pCfg, &stWinPrms, &stActWinCrds);

			/* set win(shp) */
			PE_WIN_HW_M23_SetShp(pCfg, &stWinPrms, &stActWinCrds);

			/* shp win en */
			PE_WIN_HW_M23_SetShpWinEn(pCfg, &stWinPrms, &stShpCenWinEn);

			/* cen win en */
			PE_WIN_HW_M23_SetCenWinEn(pCfg, &stWinPrms, &stShpCenWinEn);

			/* etc */
			PE_WIN_HW_M23_SetWinResolution(pCfg, &stWinPrms);
...
int PE_WIN_HW_M23_GetCurWinSettings(PE_WIN_HW_M23_SETTINGS_T *pstParams)
{
	int ret = RET_OK;
	#ifdef PE_HW_M23_BRINGUP
	PE_WIN_HW_M23_SETTINGS_T *pInfo = &_g_pe_win_hw_m23_info;
	PE_WIN_HW_M23_CTRL_T *win_info = &_g_pe_win_hw_m23_info.win_inf;
	__attribute__((unused)) PE_WIN_HW_M23_LRCR_CTRL_T *lrcr_info = &_g_pe_win_hw_m23_info.lrcr_inf;
	PE_WIN_HW_M23_TNR_CTRL_T *tnr_info = &_g_pe_win_hw_m23_info.tnr_inf;
	do{
		CHECK_KNULL(pstParams);
		if(PE_KDRV_VER_M23)
		{
			memcpy(pstParams,pInfo,sizeof(PE_WIN_HW_M23_SETTINGS_T));
			PE_DBG_WIN("act0:%d,%d,%d,%d, act1:%d,%d,%d,%d\n",\
			...
			PE_DBG_WIN("win0:%d,%d,%d,%d, win1:%d,%d,%d,%d\n",\
			...
			PE_DBG_WIN("h,v_max:%d,%d, w0,w1,w01_en:[%d,%d],[%d,%d],[%d,%d]\n",\
			...
			PE_DBG_WIN("histo_l,r, apply_r : %d,%d,%d,%d, %d,%d,%d,%d, %d,%d,%d,%d\n",\
			...
			PE_DBG_WIN("tnr win h,v size0:%d,%d, size1:%d,%d\n",\
 ...
 * Gets window params
 *
 * @param   *i_pstCfg		[in] 	PE_TSK_M23_CFG_T
...
static void PE_WIN_HW_M23_GetWinParams(PE_TSK_M23_CFG_T *i_pstCfg, LX_PE_WIN_PARAMS_T *o_pstWinPrms)
{
	...
	// __attribute__((unused)) PE_INF_M23_CTRL_T *ctrl0_info 	= &i_pstCfg->inf.ctrl_info[LX_PE_WIN_0];
	// __attribute__((unused)) PE_INF_M23_CTRL_T *ctrl1_info 	= &i_pstCfg->inf.ctrl_info[LX_PE_WIN_1];
	PE_REG_M23_HW_OPT_T hw_opt = PE_REG_M23_GetDispOpt();

	is_orbit = (disp0_info->mode.is_orbit== 0x1)?0x1:0x0;
	PE_VCP_DISP_M23_QRd02(vcp_u0_win_o_pos,		cdc_win_o_pos_x, x_ofst,\
											 	cdc_win_o_pos_y, y_ofst);
	
	PE_VCP_DISP_M23_QRd02(vcp_u0_win_o_size,	cdc_win_o_size_x, x_size,\
												cdc_win_o_size_y, y_size);

	PE_VCP_DISP_M23_QRd02(vcp_u1_win_o_pos,		cdc_win_o_pos_x, x1_ofst,\
											 	cdc_win_o_pos_y, y1_ofst);
	PE_VCP_DISP_M23_QRd02(vcp_u1_win_o_size,	cdc_win_o_size_x, x1_size,\
												cdc_win_o_size_y, y1_size);
	if(hw_opt.panel_resolution== 1) // fulL-hd 1920,1080
	{
		o_pstWinPrms->h_max=PE_WIN_HW_M23_FHD_H_MAX;
		o_pstWinPrms->v_max=PE_WIN_HW_M23_FHD_V_MAX;
	}
	else if(hw_opt.panel_resolution== 2) // hd 1366, 768
	{
		o_pstWinPrms->h_max=PE_WIN_HW_M23_SHD_H_MAX;
		o_pstWinPrms->v_max=PE_WIN_HW_M23_SHD_V_MAX;
	}
	else // 4k 3840x2160
	{
		o_pstWinPrms->h_max=PE_WIN_HW_M23_UHD_H_MAX;
		o_pstWinPrms->v_max=PE_WIN_HW_M23_UHD_V_MAX;
	}
	/* 2012xxxx, sj.youm, inner pattern for 2d,3d and so on. */
	o_pstWinPrms->is_invalid_size[LX_PE_WIN_0] = \
		(x_size <= (PE_WIN_HW_M23_WIN_OFFSET_MAX<<1) || \
		y_size <= (PE_WIN_HW_M23_WIN_OFFSET_MAX<<1))? 1:0;
	o_pstWinPrms->is_invalid_size[LX_PE_WIN_1] = \
		(x1_size <= (PE_WIN_HW_M23_WIN_OFFSET_MAX<<1) || \
		y1_size <= (PE_WIN_HW_M23_WIN_OFFSET_MAX<<1))? 1:0;
 ...
 * @author
 */
static void PE_WIN_HW_M23_GetActNWinCrdsForExtFRC_PIP(PE_TSK_M23_CFG_T 			*i_pstCfg,
													LX_PE_WIN_PARAMS_T 			*i_pstWinPrms,
													LX_PE_ACT_WIN_COORDINATES_T *o_pstActWinCrds)
{
	__attribute__((unused)) LX_PE_INF_DISPLAY_T *disp0_info = &i_pstCfg->inf.disp_info[LX_PE_WIN_0];
												...
												LX_PE_ACT_WIN_COORDINATES_T *o_pstActWinCrds)
{
	...
	if(_g_pe_win_hw_m23_info.is_external_chip==0x0 &&\
		disp0_info->sub_mode > LX_DE_SUB_MODE_NONE && disp0_info->sub_mode <LX_DE_SUB_MODE_MAXN)
	{
		PE_WIN_HW_M23_GetActNWinCrdsForExtFRC_PIP( i_pstCfg, i_pstWinPrms, o_pstActWinCrds );
	}
	else
	{
		/* L : window 0 */
												...
												LX_PE_SHP_CEN_WIN_EN_T	*o_pstShpCenWinEn)
{
	...
	{
				o_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_SHP_WIN]=PE_WIN_HW_M23_WIN_EN__ON;
				o_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_SHP_WIN]=PE_WIN_HW_M23_WIN_EN_OFF;
				o_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_SHP_WIN]=PE_WIN_HW_M23_WIN_EN_OFF;
				o_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_CEN_WIN]=PE_WIN_HW_M23_WIN_EN__ON;
				o_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_CEN_WIN]=PE_WIN_HW_M23_WIN_EN_OFF;
				o_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_CEN_WIN]=PE_WIN_HW_M23_WIN_EN_OFF;
	}
	else if(disp0_info->sub_mode == LX_DE_SUB_MODE_PBP)
	{
		o_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_SHP_WIN]=PE_WIN_HW_M23_WIN_EN__ON;
		o_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_SHP_WIN]=(ctrl0_info->vcp_mute_on)? \
			PE_WIN_HW_M23_WIN_EN_OFF:PE_WIN_HW_M23_WIN_EN__ON;
		o_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_SHP_WIN]=PE_WIN_HW_M23_WIN_EN_OFF;
		o_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_CEN_WIN]=PE_WIN_HW_M23_WIN_EN__ON;
		o_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_CEN_WIN]=PE_WIN_HW_M23_WIN_EN__ON;
		o_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_CEN_WIN]=PE_WIN_HW_M23_WIN_EN_OFF;
	}
	else if(disp0_info->sub_mode == LX_DE_SUB_MODE_MAGZOOM || disp0_info->sub_mode == LX_DE_SUB_MODE_LIVEZOOM) //mirror
	{
		o_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_SHP_WIN]=PE_WIN_HW_M23_WIN_EN__ON;
		o_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_SHP_WIN]=PE_WIN_HW_M23_WIN_EN__ON;
		o_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_SHP_WIN]=PE_WIN_HW_M23_WIN_EN__ON;
		o_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_CEN_WIN]=PE_WIN_HW_M23_WIN_EN__ON;
		o_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_CEN_WIN]=PE_WIN_HW_M23_WIN_EN__ON;
		o_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_CEN_WIN]=PE_WIN_HW_M23_WIN_EN__ON;
	}
	else
	{
		o_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_SHP_WIN]=(i_bIsResizeSet)? \
			PE_WIN_HW_M23_WIN_EN_OFF:PE_WIN_HW_M23_WIN_EN__ON;
		o_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_SHP_WIN]=PE_WIN_HW_M23_WIN_EN_OFF;
		o_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_SHP_WIN]=PE_WIN_HW_M23_WIN_EN_OFF;
		o_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_CEN_WIN]=PE_WIN_HW_M23_WIN_EN__ON;
								 ...
								 LX_PE_ACT_WIN_COORDINATES_T 		*i_pstActWinCrds)
{
	...
	if((win_info->act0_x0 != i_pstActWinCrds->act0_x0)|| (win_info->act0_y0 != i_pstActWinCrds->act0_y0)|| \
		...
		PE_DBG_WIN("[inf]rev:%d,submode:%d,ofs0,1:%d,%d,%d,%d, %d,%d,%d,%d\n",\
		...
		PE_DBG_WIN("[inf]o_win0:%d,%d,%d,%d,o_win1:%d,%d,%d,%d\n",\
		...
		PE_DBG_WIN("[set]act0 : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
		...
		PE_DBG_WIN("[set]act1 : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
		...
		if(PE_KDRV_VER_M23)	{
            if (1)//i_pstCfg->opr.pause_task == PE_TSK_M23_RUN_STOP_WIN2)
			{
	            PE_CC_PE1_M23_QWr02(pe1_win2_ctrl_01,	win_w0_x0_0,(i_pstActWinCrds->act0_x0),\
	                                                  	win_w0_y0_0,i_pstActWinCrds->act0_y0);
	            PE_CC_PE1_M23_QWr02(pe1_win2_ctrl_02,	win_w0_x1_0,(i_pstActWinCrds->act0_x1),\
	                                                  	win_w0_y1_0,i_pstActWinCrds->act0_y1);
			}
			else
			{
				PE_DBG_WIN("[inf]pause_task:%d, w0 SKIP.\n",i_pstCfg->opr.pause_task);
			}
            PE_CC_PE1_M23_QWr02(pe1_win2_ctrl_03,	win_w1_x0_0,(i_pstActWinCrds->act1_x0),\
                                                  	win_w1_y0_0,i_pstActWinCrds->act1_y0);
            PE_CC_PE1_M23_QWr02(pe1_win2_ctrl_04,	win_w1_x1_0,(i_pstActWinCrds->act1_x1),\
                                                  	win_w1_y1_0,i_pstActWinCrds->act1_y1);
			#if 0
			PE_CC_PE1_M23_QWr02(pe1_apl_ctrl_00,apl_win_ctrl_x0,(i_pstActWinCrds->act0_x0>>1),\
			 										apl_win_ctrl_y0,i_pstActWinCrds->act0_y0);
			if (disp0_info->out_type==LX_PE_OUT_PIP && ctrl0_info->hv_same)	//double window
			{
				PE_CC_PE1_M23_QWr02(pe1_apl_ctrl_01,apl_win_ctrl_x1,(i_pstActWinCrds->act1_x1>>1),
													apl_win_ctrl_y1,i_pstActWinCrds->act1_y1);
			}
			else
			{
				PE_CC_PE1_M23_QWr02(pe1_apl_ctrl_01,apl_win_ctrl_x1,(i_pstActWinCrds->act0_x1>>1),
													apl_win_ctrl_y1,i_pstActWinCrds->act0_y1);
			}
			#else
			if(_g_pe_win_hw_m23_info.is_external_chip==0 && \
				disp0_info->sub_mode==LX_DE_SUB_MODE_PIP && !i_pstWinPrms->is_invalid_size[LX_PE_WIN_1])//miracast & sub video on
			{
				PE_CC_PE1_M23_QWr02(pe1_apl_ctrl_00,	apl_win_ctrl_x0_0,(i_pstActWinCrds->act1_x0),\
			 											apl_win_ctrl_y0_0,i_pstActWinCrds->act1_y0);
				PE_CC_PE1_M23_QWr02(pe1_apl_ctrl_01,	apl_win_ctrl_x1_0,(i_pstActWinCrds->act1_x1),
														apl_win_ctrl_y1_0,i_pstActWinCrds->act1_y1);
			}
			else if(_g_pe_win_hw_m23_info.is_external_chip==0 && \
				disp0_info->sub_mode==LX_DE_SUB_MODE_PBP && !i_pstWinPrms->is_invalid_size[LX_PE_WIN_1])//multi view & sub video on
			{
				PE_CC_PE1_M23_QWr02(pe1_apl_ctrl_00,	apl_win_ctrl_x0_0,(i_pstActWinCrds->act0_x0),\
			...
			else
			{
				PE_CC_PE1_M23_QWr02(pe1_apl_ctrl_00,	apl_win_ctrl_x0_0,(i_pstActWinCrds->act0_x0),\
			 											apl_win_ctrl_y0_0,i_pstActWinCrds->act0_y0);
				PE_CC_PE1_M23_QWr02(pe1_apl_ctrl_01,	apl_win_ctrl_x1_0,(i_pstActWinCrds->act0_x1),
								 ...
								 LX_PE_ACT_WIN_COORDINATES_T 		*i_pstActWinCrds)
{
	...
	if((win_info->win0_x0 != i_pstActWinCrds->win0_x0)|| (win_info->win0_y0 != i_pstActWinCrds->win0_y0)|| \
		...
		PE_DBG_WIN("[set]win0 : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
		...
		PE_DBG_WIN("[set]win1 : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
		...
		PE_DBG_WIN("[inf]hmax, vmax : cur:%d,%d\n",\
		...
		if(PE_KDRV_VER_M23) {
			...
			/* esf */
			#if 0
            PE_SHP_M23_QWr02(shp_esf_win_ctrl_01,	reg_win_esf_win_w0_x0,	shp_l_w0_x0,\
													reg_win_esf_win_w0_y0,	i_pstActWinCrds->win0_y0);
            PE_SHP_M23_QWr02(shp_esf_win_ctrl_02,	reg_win_esf_win_w0_x1,	shp_l_w0_x1,\
													reg_win_esf_win_w0_y1,	i_pstActWinCrds->win0_y1);
            PE_SHP_M23_QWr02(shp_esf_win_ctrl_03,	reg_win_esf_win_w1_x0,	shp_l_w1_x0,\
													reg_win_esf_win_w1_y0,	i_pstActWinCrds->win1_y0);
            PE_SHP_M23_QWr02(shp_esf_win_ctrl_04,	reg_win_esf_win_w1_x1,	shp_l_w1_x1,\
													reg_win_esf_win_w1_y1,	i_pstActWinCrds->win1_y1);
			#endif
			/* dp*/
			PE_SHP_M23_QWr02(shp_dp_win_ctrl_01,	reg_win_dp_win_w0_x0,	shp_l_w0_x0,\
													reg_win_dp_win_w0_y0,	i_pstActWinCrds->win0_y0);
            PE_SHP_M23_QWr02(shp_dp_win_ctrl_02,	reg_win_dp_win_w0_x1,	shp_l_w0_x1,\
													reg_win_dp_win_w0_y1,	i_pstActWinCrds->win0_y1);
            PE_SHP_M23_QWr02(shp_dp_win_ctrl_03,	reg_win_dp_win_w1_x0,	shp_l_w1_x0,\
													reg_win_dp_win_w1_y0,	i_pstActWinCrds->win1_y0);
            PE_SHP_M23_QWr02(shp_dp_win_ctrl_04,	reg_win_dp_win_w1_x1,	shp_l_w1_x1,\
													reg_win_dp_win_w1_y1,	i_pstActWinCrds->win1_y1);
			/* dj*/
			PE_SHP_M23_QWr02(shp_dj_win_ctrl_01,	reg_win_dj_win_w0_x0,	shp_l_w0_x0,\
													reg_win_dj_win_w0_y0,	i_pstActWinCrds->win0_y0);
            PE_SHP_M23_QWr02(shp_dj_win_ctrl_02,	reg_win_dj_win_w0_x1,	shp_l_w0_x1,\
													reg_win_dj_win_w0_y1,	i_pstActWinCrds->win0_y1);
            PE_SHP_M23_QWr02(shp_dj_win_ctrl_03,	reg_win_dj_win_w1_x0,	shp_l_w1_x0,\
													reg_win_dj_win_w1_y0,	i_pstActWinCrds->win1_y0);
            PE_SHP_M23_QWr02(shp_dj_win_ctrl_04,	reg_win_dj_win_w1_x1,	shp_l_w1_x1,\
									 ...
									 LX_PE_SHP_CEN_WIN_EN_T 		*i_pstShpCenWinEn)
{
	...
	if((win_info->win0_en[PE_WIN_HW_M23_SHP_WIN] != i_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_SHP_WIN]) || \
		(win_info->win1_en[PE_WIN_HW_M23_SHP_WIN] != i_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_SHP_WIN]) || \
		(win_info->win01_en[PE_WIN_HW_M23_SHP_WIN] != i_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_SHP_WIN]))
	{
		PE_DBG_WIN("[inf]rev:%d,submode:%d,ofs0,1:%d,%d,%d,%d, %d,%d,%d,%d\n",\
			disp0_info->mode.is_reverse,disp0_info->sub_mode,\
			i_pstWinPrms->x0_ofst[LX_PE_WIN_0],i_pstWinPrms->y0_ofst[LX_PE_WIN_0],\
		...
		PE_DBG_WIN("[set]shp_win_en(0,1,01):%d,%d,%d \n",\
			win_info->win0_en[PE_WIN_HW_M23_SHP_WIN], win_info->win1_en[PE_WIN_HW_M23_SHP_WIN],\
			win_info->win01_en[PE_WIN_HW_M23_SHP_WIN]);
		if(PE_KDRV_VER_M23) {
			/*esf*/
			#if 0
			PE_SHP_M23_QWr03(shp_esf_win_ctrl_00, reg_win_esf_win0_en, i_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_SHP_WIN], \
                                                  reg_win_esf_win1_en, i_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_SHP_WIN], \
                                                  reg_win_esf_win01_en,i_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_SHP_WIN]);
			#endif
			/*dp*/
			PE_SHP_M23_QWr03(shp_dp_win_ctrl_00, reg_win_dp_win0_en, i_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_SHP_WIN] , \
                                                 reg_win_dp_win1_en, i_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_SHP_WIN] , \
                                                 reg_win_dp_win01_en,i_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_SHP_WIN]);
			/*dj*/
			PE_SHP_M23_QWr03(shp_dj_win_ctrl_00, reg_win_dj_win0_en, i_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_SHP_WIN] , \
                                                 reg_win_dj_win1_en, i_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_SHP_WIN] , \
                                                 reg_win_dj_win01_en,i_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_SHP_WIN]);
		}

		win_info->win0_en[PE_WIN_HW_M23_SHP_WIN] = i_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_SHP_WIN];
		win_info->win1_en[PE_WIN_HW_M23_SHP_WIN] = i_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_SHP_WIN];
		win_info->win01_en[PE_WIN_HW_M23_SHP_WIN] = i_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_SHP_WIN];
	}
	#endif /*PE_HW_M23_BRINGUP*/
									 ...
									 LX_PE_SHP_CEN_WIN_EN_T 		*i_pstShpCenWinEn)
{
	...
	if((win_info->win0_en[PE_WIN_HW_M23_CEN_WIN] != i_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_CEN_WIN]) || \
	   (win_info->win1_en[PE_WIN_HW_M23_CEN_WIN] != i_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_CEN_WIN]) || \
	   (win_info->win01_en[PE_WIN_HW_M23_CEN_WIN] != i_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_CEN_WIN]))
	{
		PE_DBG_WIN("[inf]rev:%d,submode:%d,ofs0,1:%d,%d,%d,%d, %d,%d,%d,%d\n",\
			disp0_info->mode.is_reverse,disp0_info->sub_mode,\
			i_pstWinPrms->x0_ofst[LX_PE_WIN_0],i_pstWinPrms->y0_ofst[LX_PE_WIN_0],\
		...
		PE_DBG_WIN("[set]cen_win_en(0,1,01):%d,%d,%d \n",\
			win_info->win0_en[PE_WIN_HW_M23_CEN_WIN], win_info->win1_en[PE_WIN_HW_M23_CEN_WIN],\
			win_info->win01_en[PE_WIN_HW_M23_CEN_WIN]);

		if(PE_KDRV_VER_M23) {

			PE_CC_PE1_M23_QWr03(pe1_win2_ctrl_00,win0_en,i_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_CEN_WIN], \
                                                    win1_en,i_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_CEN_WIN], \
                                                    win01_en,i_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_CEN_WIN]);
		}

		win_info->win0_en[PE_WIN_HW_M23_CEN_WIN] = i_pstShpCenWinEn->win0_en[PE_WIN_HW_M23_CEN_WIN];
		win_info->win1_en[PE_WIN_HW_M23_CEN_WIN] = i_pstShpCenWinEn->win1_en[PE_WIN_HW_M23_CEN_WIN];
		win_info->win01_en[PE_WIN_HW_M23_CEN_WIN] = i_pstShpCenWinEn->win01_en[PE_WIN_HW_M23_CEN_WIN];
	}
	#endif /*PE_HW_M23_BRINGUP*/
										   ...
										   LX_PE_WIN_PARAMS_T 			*i_pstWinPrms)
{
	...
	if((win_info->h_max != i_pstWinPrms->h_max)|| (win_info->v_max != i_pstWinPrms->v_max))	{
		...
		PE_DBG_WIN("[inf]rev:%d,sub_mode:%d,ofs0,1:%d,%d,%d,%d, %d,%d,%d,%d\n",\
			disp0_info->mode.is_reverse,disp0_info->sub_mode,\
			i_pstWinPrms->x0_ofst[LX_PE_WIN_0],i_pstWinPrms->y0_ofst[LX_PE_WIN_0],\
			i_pstWinPrms->x1_ofst[LX_PE_WIN_0],i_pstWinPrms->y1_ofst[LX_PE_WIN_0],\
			i_pstWinPrms->x0_ofst[LX_PE_WIN_1],i_pstWinPrms->y0_ofst[LX_PE_WIN_1],\
		...
		PE_DBG_WIN("[set]h,v_max : prev:%d,%d, cur:%d,%d\n",\
		...
		if(PE_KDRV_VER_M23) {
			...
			PE_SHP_M23_QWr02(shp_core_ctrl_01,reg_top_width,shp_top_w,reg_top_height,i_pstWinPrms->v_max);
			PE_CC_PE1_M23_QWr02(pe1_top_ctrl_01,width,(i_pstWinPrms->h_max),height,i_pstWinPrms->v_max);
            //PE_CC_PE1_M23_QWr02(pe1_apl_ctrl_00,apl_win_ctrl_x0,0,apl_win_ctrl_y0,0);
            //PE_CC_PE1_M23_QWr02(pe1_apl_ctrl_01,apl_win_ctrl_x1,(i_pstWinPrms->h_max>>1),apl_win_ctrl_y1,i_pstWinPrms->v_max);
		}
