...
static void __iomem *ipc_disable = NULL;
...
unsigned int get_micom_disable(void)
{
	unsigned int reg;

	if(ipc_disable == NULL)
		ipc_disable = ioremap(0xc3030028, 4);

	reg = readl(ipc_disable);

	return reg;
}
EXPORT_SYMBOL(get_micom_disable) ;

void set_micom_disable(unsigned int disable)
{
	if(ipc_disable == NULL)
		ipc_disable = ioremap(0xc3030028, 4);

	writel(disable, ipc_disable);
}
EXPORT_SYMBOL(set_micom_disable) ;
#if 0
...
static int e60_ucom_task(void *pParam)
{
	unsigned int i,ret;
	...
	KDRV_UCOM_NOTI("e60 e60_ucom_task is created\n");

	do {
		/* Check stop condition when device is closed. */
		if (kthread_should_stop()) {
			KDRV_UCOM_NOTI("e60 e60_ucom_task - exit!\n");
			break;
		}
...
void e60_ucom_task_init(void)
{
	...
	if (micom_emul_task == NULL) {
		...
		micom_emul_task = kthread_create_on_node(e60_ucom_task,
					NULL,
					cpu,
...
void e60_ucom_FuncInit(void)
{
	...
	KDRV_UCOM_NOTI("m2a_ipc[%p]m2a_irq[%p]a2m_ipc[%p]a2m_irq[%p]dis[%d]\n",
		m2a_ipc_base,m2a_irq_base,a2m_ipc_base,a2m_irq_base,get_micom_disable());
	#else
...
void e60_ucom_FuncChipReset(void)
{
	/* param for chip reset */
	writel(0x10000 , micom_base + 0x8c );
	/* intr set for chip reset */
	writel(0x1 , a2m_irq_base + 0x4 );
}
...
int e60_ucom_FuncWrite(LX_UCOM_PARAM_T * param)
{
	int ret;
...
int e60_ucom_FuncRead(LX_UCOM_PARAM_T * param)
{
...
int e60_ucom_FuncGetStatus(void )
{
...
static ucom_e60_func_t e60_ucom_func =
{
	.init = e60_ucom_FuncInit,
	.chipreset = e60_ucom_FuncChipReset,
	.write = e60_ucom_FuncWrite,
	.read = e60_ucom_FuncRead,
	.getstatus= e60_ucom_FuncGetStatus,

};

ucom_e60_func_t* get_e60_ucom_func(void)
{
	return &e60_ucom_func;
}
