...
#ifndef __OSD_REG_M23_H__
#define __OSD_REG_M23_H__
...
/*-----------------------------------------------------------------------------
	0x018c ctrl_bosd1_mixer_miracast_ctrl ''
------------------------------------------------------------------------------*/
typedef struct {
	UINT32
...
/*-----------------------------------------------------------------------------
	0x01e4 ctrl_bosd1_mixer_obj_alpha ''
------------------------------------------------------------------------------*/
typedef struct {
	UINT32
	reg_obj_map_en                  : 1,	//     0
...
typedef struct {
	...
	reserved                        ;   	// 31: 0
#else
	// 원래 reserved 이나, 코드 호환성 유지 차원에서 OSD4 것을 사용함.
	UINT32
	osd_ctrl_direc                  : 1,	//     0
	                                : 3,	//  1: 3 reserved
...
typedef struct {
	UINT32
	reg_yc2rgb_csc_en_de_clk        : 1;	//     0
} OSD0_CSC_EN;

/*-----------------------------------------------------------------------------
	0x005c osd_pre_mul_alpha ''
------------------------------------------------------------------------------*/
typedef struct {
	UINT32
...
/*-----------------------------------------------------------------------------
	0x0080 osd0_scaler_in_size ''
------------------------------------------------------------------------------*/
typedef struct {
	UINT32
	reg_scaler_in_height            :13,	//  0:12
	                                : 3,	// 13:15 reserved
	reg_scaler_in_width             :13;	// 16:28
} OSD0_SCALER_IN_SIZE;

/*-----------------------------------------------------------------------------
	0x0084 osd0_scaler_out_size ''
------------------------------------------------------------------------------*/
typedef struct {
	UINT32
	reg_scaler_out_height           :13,	//  0:12
	                                : 3,	// 13:15 reserved
	reg_scaler_out_width            :13;	// 16:28
} OSD0_SCALER_OUT_SIZE;
...
typedef struct
{
	...
	CTRL_BOSD1_MIXER_MIRACAST_CTRL  	ctrl_bosd_mixer_miracast_ctrl  ;	// 0x018c : ''
	UINT32                          	                 __rsvd_00[0]  ;	// 0x0190 ~ 0x0198
	...
	CTRL_BOSD1_MIXER_LED_ALPHA      	ctrl_bosd_mixer_led_alpha      ;	// 0x01e0 : ''
	CTRL_BOSD1_MIXER_OBJ_ALPHA      	ctrl_bosd_mixer_obj_alpha      ;	// 0x01e4 : ''

#if 1
	UINT32								__rsvd_01[9]; // add 32 byte sized RSVD regs
#endif
}
...
typedef union
{
	...
	struct
	{
		...
		OSD0_CSC_EN                     	osd0_csc_en                     ;	// 0x0058 : ''
		OSD_PRE_MUL_ALPHA               	osd_pre_mul_alpha               ;	// 0x005c : ''
	};
	...
	struct
	{
		...
		UINT32                          	                 __rsvd_00[   1];	// 0x007c
		OSD0_SCALER_IN_SIZE             	osd0_scaler_in_size             ;	// 0x0080 : ''
		OSD0_SCALER_OUT_SIZE            	osd0_scaler_out_size            ;	// 0x0084 : ''
		UINT32                          	                 __rsvd_01[   1];	// 0x0088
		...
		UINT32                          	                 __rsvd_02[   7];	// 0x009c ~ 0x00b4
...
typedef struct
{
...
#if 1
	OSD0_COMMON_DISP_SIZE           	osdx_common_disp_size[8]        ;	// 0x0050 : ''
#else
	...
	UINT32                          	                 __rsvd_00[  67];	// 0x0074 ~ 0x017c

	OSDX_BLEND_OSD_MIXER				osdx_blend_osd_mixer[1];

	// M23 dfines only 2 valid layer
	// OSD[0]: UI
	// OSDi1]: N/A
	// OSD[2]: N/A
	// OSD[3]: cursor
	//
	// osd_x_ctrl 의 시작 offset 은 0x200 이어야 함
	//
	// (note) osdx_ctrl has 0x200 bytes offset from the osd register base
	OSDX_CTRL							osdx_ctrl[4];						// 0x0200 ~
}
OSD_M23_REG_T;

/////////////////////////////////////////////////////////////////////////////////////////////////
// OSDX SHADOW ERG MACRO
/////////////////////////////////////////////////////////////////////////////////////////////////
#define OSDX_M23_Off(_g,_r)             ((UINT32)&(g_osd_m23_reg_shadow->_g._r) - (UINT32)(g_osd_m23_reg_shadow))

#define OSDX_M23_RdFL(_g,_r)            ((g_osd_m23_reg_shadow->_g._r)=(g_osd_m23_reg->_g._r))
#define OSDX_M23_WrFL(_g,_r)            ((g_osd_m23_reg->_g._r)=(g_osd_m23_reg_shadow->_g._r))

#define OSDX_M23_Rd(_g,_r)              *((UINT32*)(&(g_osd_m23_reg_shadow->_g._r)))
#define OSDX_M23_Wr(_g,_r,_v)           ((OSDX_M23_Rd(_g,_r))=((UINT32)(_v)))

#define OSDX_M23_Rd00(_g,_r,_f)         (g_osd_m23_reg_shadow->_g._r._f)
#define OSDX_M23_Rd01(_g,_r,_f01,_v01)  do {                                                \
                                            (_v01) = (g_osd_m23_reg_shadow->_g._r._f01);    \
                                        } while(0)
#define OSDX_M23_Wr01(_g,_r,_f01,_v01)  do {                                                \
                                            (g_osd_m23_reg_shadow->_g._r._f01) = (_v01);    \
                                        } while(0)
#define OSDX_M23_Wr02(_g,_r,_f01,_v01,_f02,_v02)    do {                                    \
                                            (g_osd_m23_reg_shadow->_g._r._f01) = (_v01);    \
                                            (g_osd_m23_reg_shadow->_g._r._f02) = (_v02);    \
                                        } while(0)


/////////////////////////////////////////////////////////////////////////////////////////////////
// OSD SHADOW ERG MACRO
/////////////////////////////////////////////////////////////////////////////////////////////////
#define OSD_M23_RdFL(_r)			((g_osd_m23_reg_shadow->_r)=(g_osd_m23_reg->_r))
#define OSD_M23_WrFL(_r)			((g_osd_m23_reg->_r)=(g_osd_m23_reg_shadow->_r))

#define OSD_M23_Rd(_r)				*((UINT32*)(&(g_osd_m23_reg_shadow->_r)))
#define OSD_M23_Wr(_r,_v)			((OSD_M23_Rd(_r))=((UINT32)(_v)))

#define OSD_M23_Rd00(_r,_f)         (g_osd_m23_reg_shadow->_r._f)


#define OSD_M23_Rd01(_r,_f01,_v01)													\
								do { 											\
									(_v01) = (g_osd_m23_reg_shadow->_r._f01);				\
								} while(0)

#define OSD_M23_Rd02(_r,_f01,_v01,_f02,_v02)										\
								do { 											\
									(_v01) = (g_osd_m23_reg_shadow->_r._f01);				\
									(_v02) = (g_osd_m23_reg_shadow->_r._f02);				\
								} while(0)

#define OSD_M23_Rd03(_r,_f01,_v01,_f02,_v02,_f03,_v03)								\
								do { 											\
									(_v01) = (g_osd_m23_reg_shadow->_r._f01);				\
									(_v02) = (g_osd_m23_reg_shadow->_r._f02);				\
									(_v03) = (g_osd_m23_reg_shadow->_r._f03);				\
								} while(0)

#define OSD_M23_Rd04(_r,_f01,_v01,_f02,_v02,_f03,_v03,_f04,_v04)					\
								do { 											\
									(_v01) = (g_osd_m23_reg_shadow->_r._f01);				\
									(_v02) = (g_osd_m23_reg_shadow->_r._f02);				\
									(_v03) = (g_osd_m23_reg_shadow->_r._f03);				\
									(_v04) = (g_osd_m23_reg_shadow->_r._f04);				\
								} while(0)

#define OSD_M23_Wr01(_r,_f01,_v01)													\
								do { 											\
									(g_osd_m23_reg_shadow->_r._f01) = (_v01);				\
								} while(0)

#define OSD_M23_Wr02(_r,_f01,_v01,_f02,_v02)										\
								do { 											\
									(g_osd_m23_reg_shadow->_r._f01) = (_v01);				\
									(g_osd_m23_reg_shadow->_r._f02) = (_v02);				\
								} while(0)

#define OSD_M23_Wr03(_r,_f01,_v01,_f02,_v02,_f03,_v03)								\
								do { 											\
									(g_osd_m23_reg_shadow->_r._f01) = (_v01);				\
									(g_osd_m23_reg_shadow->_r._f02) = (_v02);				\
									(g_osd_m23_reg_shadow->_r._f03) = (_v03);				\
								} while(0)

#define OSD_M23_Wr04(_r,_f01,_v01,_f02,_v02,_f03,_v03,_f04,_v04)					\
								do { 											\
									(g_osd_m23_reg_shadow->_r._f01) = (_v01);				\
									(g_osd_m23_reg_shadow->_r._f02) = (_v02);				\
									(g_osd_m23_reg_shadow->_r._f03) = (_v03);				\
									(g_osd_m23_reg_shadow->_r._f04) = (_v04);				\
								} while(0)

extern volatile OSD_M23_REG_T*	g_osd_m23_reg;
extern OSD_M23_REG_T*			g_osd_m23_reg_shadow;


/* 134 regs, 134 types in Total*/

#ifdef __cplusplus
}
#endif

#endif	/* __OSD_REG_M23_H__ */

/* from 'M23-A0_PQE_OSD_reg_man.csv' 20220128 11:59:31 KST by getregs v2.9 */
