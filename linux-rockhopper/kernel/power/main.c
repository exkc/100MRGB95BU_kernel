...
static int __init hib_state_set_snapshot(char *p)
{
	...
	snapshot_enable = 1;
#if defined(CONFIG_CMA) && !defined(CONFIG_CMA_ALLOW_HIGHUSER_MOVABLE_ALLOC)
	/*
...
static const struct proc_ops dep_parts_fops = {
	.proc_open = open_dep_parts,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = write_dep_parts
};
			      ...
			      const char *buf, size_t n)
{
	...
	if (pm_userresume_enabled) {
		 ...
		 * user-resume is done.
		 *
		 * In order to avoid the conflict, locking system_transition_mutex is required to indicate
