...
void security_ekp_ro_cred_free(struct cred *cred)
{
#ifdef CONFIG_EKP_CRED_PROTECTION
	/*
	 * There is a failure case in prepare_creds() that may
	 * result in a call here with cred->security being NULL.
	 */
	if (unlikely(cred->security == NULL))
		return;
	...
	call_void_hook(cred_free, cred);
	kfree(cred->security);
	ekp_romemset_zero(__pa((&cred->security)), sizeof(void *));
#endif
}
