...
#define MAX_SRC_NAME DEV_NAME_MAX
#define MAX_OP_NAME	DEV_NAME_MAX
typedef enum {
...
typedef enum {
	...
	DISP_DEV_VTH,        // to memory for hdr
	DISP_DEV_MAX
} DISP_DEV_TYPE;
...
typedef struct {
	...
	UINT32                   disp_type; // M16+ model's display type 60/120Hz, 2k/4k
...
typedef struct {
	...
	LX_DE_SUB_SHAPE_T        sub_shape;
	LX_DE_SOURCE_SIZE_WIN_T  src_size_win[DISP_DEV_MAX];
	LX_DE_WIN_INFO_T         win_info[DISP_DEV_MAX];
	LX_DE_ORBIT_T            orbit_win[DISP_DEV_MAX];
	LX_DE_ORBIT_T            orbit_mov[DISP_DEV_MAX];
} WINDOW_INFO_T;
