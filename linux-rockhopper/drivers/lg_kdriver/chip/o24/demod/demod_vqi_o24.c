...
int DEMOD_O24_ATSC_SW_Init (void)
{
...
int _DEMOD_O24_VSB_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int _DEMOD_O24_VSB_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_O24_VSB_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	...
	new_tuneMode = _DEMOD_O24_VSB_CheckTuneMode(pDemodInfo);

	if(RET_OK != DEMOD_O24_Get_BW(pDemodInfo, &ter_bandwidth_KHz))
	{
...
************************************************************************************/
/**
 * DEMOD_O24_VSB_Obtaining_Signal_Lock.
...
int DEMOD_O24_VSB_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_TUNE_START:
			{
				...
				if(RET_OK != DEMOD_O24_VSB_SetDemod(pDemodInfo))
				{
					DEMOD_ERROR("Fail, DEMOD_O24_VSB_SetDemod()\n");
					return RET_ERROR;
				}

				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.vsb.tuneMode)
				{
					if(RET_OK != DEMOD_O24_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_ON, &scanProcessing, &scanLock))
					{
						DEMOD_ERROR("Fail, DEMOD_O24_NeverlockScan()\n");
						return RET_ERROR;
					}
				...
				else
				{
					...
					{
						DEMOD_ERROR("Fail, DEMOD_O24_TPOutEnable()\n");
						return RET_ERROR;
					}
		...
		case LX_DEMOD_TUNE_SCAN:
			{
				...
				if(RET_OK != DEMOD_O24_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_OFF, &scanProcessing, &scanLock))
				{
					DEMOD_ERROR("Fail, DEMOD_O24_NeverlockScan()\n");
					return RET_ERROR;
				}
		...
		case LX_DEMOD_SYNC_CHECKING :
			{
				...
				{
					DEMOD_ERROR("Fail, DEMOD_O24_Get_Lock()\n");
					return RET_ERROR;
				}
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE:
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.vsb.tuneMode)
					{
						...
						if(RET_OK != DEMOD_O24_Get_Packet_Error(&packetError))
						{
							DEMOD_ERROR("Fail, DEMOD_O24_Get_Packet_Error()\n");
							return RET_ERROR;
						}
						else
						{
							...
							if (RET_OK != _DEMOD_O24_VSB_CheckSignalState(pDemodInfo))
							{
								DEMOD_ERROR("Fail, _DEMOD_O24_VSB_CheckSignalState()\n");
								return RET_ERROR;
							}
						}
					}
					else
					{
						...
						if (RET_OK != DEMOD_O24_Get_CarrierFreqOffset(&freqOffset))
						{
							DEMOD_ERROR("Fail, DEMOD_O24_Get_CarrierFreqOffset()\n");
							return RET_ERROR;
						}
...
************************************************************************************/
/**
 * DEMOD_O24_VSB_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O24_VSB_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
...
#if 1
	if(RET_OK != DEMOD_O24_VSB_Monitor(pDemodInfo))
	{
		DEMOD_ERROR("Fail, DEMOD_O24_VSB_Monitor()\n");
		return RET_ERROR;
	}
#endif
...
int _DEMOD_O24_QAM_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int _DEMOD_O24_QAM_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_O24_QAM_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context * 	pFrontend;
	UINT8			new_tuneMode;
	UINT32 			freq_Khz = 0;
	...
	pFrontend = DEMOD_Get_LGDBB_Context();
	new_tuneMode = _DEMOD_O24_QAM_CheckTuneMode(pDemodInfo);
...
************************************************************************************/
/**
 * DEMOD_O24_QAM_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O24_QAM_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	...
	switch (pDemodInfo->controlState)
	{
		...
		case LX_DEMOD_TUNE_START:
			{
				if(RET_OK != DEMOD_O24_QAM_SetDemod(pDemodInfo))
				{
					DEMOD_ERROR("Fail, DEMOD_O24_QAM_SetDemod()\n");
					return RET_ERROR;
				}
		...
		case LX_DEMOD_SYNC_CHECKING :
			{
				...
				if (RET_OK != _DEMOD_O24_QAM_CheckSignalState(pDemodInfo))
				{
					DEMOD_ERROR("Fail, _DEMOD_O24_QAM_CheckSignalState()\n");
					return RET_ERROR;
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if(RET_OK != DEMOD_O24_QAM_EQ_Signal_Detector(FALSE))
					{
						DEMOD_ERROR("Fail, DEMOD_O24_QAM_EQ_Signal_Detector()\n");
						return RET_ERROR;
					}
				...
				else
				{
					if(LX_DEMOD_TUNE_NORMAL == realTuneMode)
					{
						...
						{
							if(RET_OK != DEMOD_O24_QAM_ModeAutoDetection(TRUE))
							{
								DEMOD_ERROR("Fail, DEMOD_O24_QAM_ModeAutoDetection()\n");
								return RET_ERROR;
							}
							#if 0
							if(RET_OK != DEMOD_O24_SoftwareReset())
							{
								DEMOD_ERROR("Fail, DEMOD_O24_SoftwareReset() !!!\n");
								return RET_ERROR;
							}
							#endif
...
************************************************************************************/
/**
 * DEMOD_O24_QAM_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O24_QAM_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_LOCK_STATE_T			lockState;
...
int _DEMOD_O24_ISDB_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int _DEMOD_O24_ISDB_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_O24_ISDB_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	...
	new_tuneMode = _DEMOD_O24_ISDB_CheckTuneMode(pDemodInfo);
...
int DEMOD_O24_ISDBT_Set_MemSconfig (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int DEMOD_O24_ISDBT_GetEmergencyAlertFlagStatus (BOOLEAN *pEalarm)
{
	LX_DEMOD_L2_Context *	pFrontend;
	UINT8 			balarm;
...
************************************************************************************/
/**
 * DEMOD_O24_ISDBT_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O24_ISDBT_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	static LX_DEMOD_FLAG_T 		scanProcessing = LX_DEMOD_FLAG_START;
	static LX_DEMOD_LOCK_STATE_T 	scanLock = LX_DEMOD_UNLOCKED;
	static unsigned int 		sigStablestartTime = 0;
	...
	switch (pDemodInfo->controlState)
	{
		...
		case LX_DEMOD_TUNE_START:
			{
				if(RET_OK != DEMOD_O24_ISDB_SetDemod(pDemodInfo))
				{
					DEMOD_ERROR("Fail, DEMOD_O24_ISDB_SetDemod()\n");
					return RET_ERROR;
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.isdbt.tuneMode)
					{
						...
						else
						{
							...
							{
#if 0
								if(RET_OK != DEMOD_O24_ISDBT_Set_PhaseNoiseEnhancement(TRUE))
								{
									DEMOD_ERROR("Fail, DEMOD_O24_ISDBT_Set_PhaseNoiseEnhancement()\n");
									return RET_ERROR;
								}
#endif
								if(packetError < LX_DEMOD_PACKET_ERROR_THD)
...
************************************************************************************/
/**
 * DEMOD_O24_ISDBT_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O24_ISDBT_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
...
//			pauseTimeDemod = LX_DEMOD_LOCKED_PAUSE_TIME;
		...
		if (RET_OK != _DEMOD_O24_ISDB_CheckSignalState(pDemodInfo)) {
			DEMOD_ERROR("Fail, _DEMOD_O24_ISDB_CheckSignalState()\n");
			return RET_ERROR;
		}
		...
		if(pDemodInfo->signalStatus.packetError >= _ISDBT_Full_Pkerr_Threshold) {
			...
			if(gLGDBB_PowerOnInit == 2)
			{
				...
				if(powerOnElapsedTime >= 15000)
				{
					...
					if (RET_OK != DEMOD_O24_ISDBT_Set_CrGearshEn(TRUE ))
					{
						DEMOD_ERROR("Fail, DEMOD_O24_ISDBT_Set_CrGearshEn()\n");
						return RET_ERROR;
					}
				}
			}
...
************************************************************************************/
/**
 * DEMOD_O24_QAM_SoftwareResetFEC.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_QAM_SoftwareResetFEC (void)
{
	return RET_OK;
}
...
int DEMOD_O24_QAM_Monitor (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_QAM_ModeAutoDetection.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_QAM_ModeAutoDetection (BOOLEAN bEnable)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_QAM_256Mode (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_QAM_Set_DefaultRegisterValue.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_QAM_Set_DefaultRegisterValue (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_QAM_Set_NeverLockWaitTime
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_QAM_Set_NeverLockWaitTime (UINT32 waitMs)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_QAM_Get_TotalInfo (void)
{
	return RET_OK;
}

/**
 * DEMOD_O24_QAM_EQ_Signal_Detector.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_QAM_EQ_Signal_Detector (BOOLEAN OnOff)
{
	return RET_OK;
}

/**
 * DEMOD_O24_ISDBT_SetDefaultRegisterValue
...
int DEMOD_O24_ISDBT_Set_DefaultRegisterValue (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_ISDBT_Set_Config_auto
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ISDBT_Set_Config_auto (BOOLEAN bAutoDetect)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_ISDBT_Set_PartialConfig
 * Set hierachical signal path and bandwidth.
 * This function is used when DEMOD_O24_ISDBT_Set_Config_auto is enabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ISDBT_Set_PartialConfig (LX_DEMOD_ISDBT_CONFIG_T *pIsdbtConfigParam)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_ISDBT_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when DEMOD_O24_ISDBT_Set_Config_auto is disabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ISDBT_Set_FullConfig (LX_DEMOD_ISDBT_CONFIG_T *pIsdbtConfigParam)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_ISDBT_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ISDBT_Set_NeverLockWaitTime (UINT32 waitMs)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_ISDBT_Set_CrGearshEn
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ISDBT_Set_CrGearshEn (BOOLEAN bEnable)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_ISDBT_Set_PhaseNoiseEnhancement
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ISDBT_Set_PhaseNoiseEnhancement (BOOLEAN bEnhanced)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_ISDBT_Get_DelaySpreadStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ISDBT_Get_DelaySpreadStatus (UINT16 *pDelaySpread)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ISDBT_Get_FFTMode (LX_DEMOD_LOCK_STATE_T *pTxModeLock, LX_DEMOD_FFTMODE_T *pFftMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O24_ISDBT_Get_GuradIntervalMode
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ISDBT_Get_GuradIntervalMode (LX_DEMOD_GIMODE_T *pGIMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_TMCC_INFO	tmcc;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ISDBT_Get_TMCCInfo (LX_DEMOD_ISDBT_TMCC_CONFIG_T *pTmccParams)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_TMCC_INFO	tmcc;
...
int DEMOD_O24_ISDBT_Get_TotalInfo (void)
{
	return RET_OK;
}
...
int DEMOD_O24_ISDBT_Get_IFO_LOCK (BOOLEAN *pIfoLock)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_ISDBT_Get_CochanDetIndicator
...
int DEMOD_O24_ISDBT_Get_CochanDetIndicator (BOOLEAN *pDetStatus)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_ISDBT_MemoryBusCheck.
...
int DEMOD_O24_ISDBT_MemoryBusCheck (void)
{
	UINT32 m1_prevW = 0x0;
	UINT32 m1_prevR = 0x0;
	UINT32 m1_currW = 0x0;
...
int DEMOD_O24_ISDBT_Set_MemBus_flushing (void)
{
	UINT32 bus_gating;
	UINT32 bflush;
...
************************************************************************************/
/**
 * DEMOD_O24_VSB_Set_DefaultRegisterValue.
...
int DEMOD_O24_VSB_Set_DefaultRegisterValue (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_VSB_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O24_VSB_Set_NeverLockWaitTime (UINT32 waitMs)
{
	return RET_OK;
}
...
int DEMOD_O24_VSB_CochannelExist (BOOLEAN *pCochannel)
{
	return RET_OK;
}
...
int DEMOD_O24_VSB_PreMonitor (void)
{
	DEMOD_TRACE("start\n");
...
int DEMOD_O24_VSB_Monitor (DEMOD_CTX_T *pDemodInfo)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O24_VSB_Get_MSEdynStatus.
...
int DEMOD_O24_VSB_Get_MSEdynStatus (BOOLEAN *pbEnable)
{
	return RET_OK;
}
...
int DEMOD_O24_VSB_Get_TotalInfo (void)
{
	return RET_OK;
}
...
int DEMOD_O24_VSB_SetDemod_Simple (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O24_ATSC3_Get_GuradIntervalMode
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ATSC3_Get_GuradIntervalMode (LX_DEMOD_ATSC3_GIMODE_T *pGIMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int DEMOD_O24_ATSC3_Get_FFTMode (LX_DEMOD_FFTMODE_T *pFftMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int DEMOD_O24_ATSC3_Get_CodeRate (LX_DEMOD_ATSC3_CODERATE_T *pCodeRate)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int DEMOD_O24_ATSC3_Get_Constellation (LX_DEMOD_ATSC3_CONSTELLATION_T *pConstellation)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int _DEMOD_O24_ATSC3_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int _DEMOD_O24_ATSC3_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_O24_ATSC3_SetDemodExpand (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context * 	pFrontend;
	...
	tuneMode = _DEMOD_O24_ATSC3_CheckTuneMode(pDemodInfo);
...
************************************************************************************/
/**
 * DEMOD_O24_ATSC3_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O24_ATSC3_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_TUNE_START:
			{
				...
				if(RET_OK != DEMOD_O24_ATSC3_SetDemodExpand(pDemodInfo))
				{
					DEMOD_ERROR("Fail, DEMOD_O24_VSB_SetDemod()\n");
					return RET_ERROR;
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.atsc3.tuneMode)
					{
						...
						else
						{
							...
							if (RET_OK != _DEMOD_O24_ATSC3_CheckSignalState(pDemodInfo))
							{
								DEMOD_ERROR("Fail, _DEMOD_O24_VSB_CheckSignalState()\n");
								return RET_ERROR;
							}
...
/**
 * DEMOD_O24_ATSC3_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O24_ATSC3_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_LOCK_STATE_T			lockState;
...
//			pauseTimeDemod = LX_DEMOD_LOCKED_PAUSE_TIME;
		...
		if (RET_OK != _DEMOD_O24_ATSC3_CheckSignalState(pDemodInfo))
		{
			DEMOD_ERROR("Fail, _DEMOD_O24_ATSC3_CheckSignalState()\n");
			return RET_ERROR;
		}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ATSC3_Set_PLP (LX_DEMOD_ATSC3_CONFIG_T *pAtsc3ConfigParam)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
/**
 * DEMOD_O24_ATSC3_Change_PLP is deprecated
 */


int DEMOD_O24_ATSC3_Get_MPLP_Info (LX_DEMOD_ATSC3_MULTI_PLP_ID_T *pAtsc3mPLPInfo, LX_DEMOD_ATSC3_MULTI_PLP_ID_SEL_T plpSel)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O24_ATSC3_Set_OIFType (LX_DEMOD_ATSC3_OIF_TYPE_T *pAtsc3OifType) // htlim
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int DEMOD_O24_ATSC3_Set_MemSconfig (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
