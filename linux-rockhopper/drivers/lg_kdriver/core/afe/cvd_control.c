...
//extern CVD_SET_SYSTEM_3CS_T (*pSetColorSystem_3CS)[CVD_STATE_VideoNotReady];
...
/*----------------------------------------------------------------------------------------
	global Variables
----------------------------------------------------------------------------------------*/
UINT32 g_CVD_CONTROL_DRV_REV = 0x19111400;
...
int CVD_AGC_Peak_En_Control(int force_peak_enable, int avg_noise, int avg_agc_gain)
{
	int agc_peak_en_value;

	agc_peak_en_value = CVD_Get_AGC_Peak_En_Value();
	...
	if (force_peak_enable == 2) {
		CVD_Set_AGC_Peak_En(1);
	}
	else if (force_peak_enable == 0) {
		CVD_Set_AGC_Peak_En(0);
	}
...
// Not Used
#if 0
int CVD_NoBurst_CKill_Control(CVD_STATUS_3CS_T *pCVD_Status, int clear_noburst_ckill)
{
...
int CVD_No_Color_Cstate_Control(CVD_STATUS_3CS_T *pCVD_Status, int cstate_reset)
{
...
//			g_CVD_Status_3CS.no_color_detected = 0;
		...
		if(no_color_count > 3) {
			...
			CVD_DEBUG("No Burst Detected !!!\n");
			CVD_DEBUG("Color[%d],chroma[%d],secam[%d],pal[%d],cvd_fc[%d],cs0_fc[%d],cs1_fc[%d],burst_mag[0x%x],noise[0x%x],vnon[%d],noburst[%d]\n",\
				pCVD_Status->cvd_color_system, pCVD_Status->cvd_chromalock_flag, pCVD_Status->cvd_secam_flag, pCVD_Status->cvd_pal_flag, 
				pCVD_Status->cvd_fc_flag, pCVD_Status->cs0_fc_flag, pCVD_Status->cs1_fc_flag, pCVD_Status->cvd_burst_mag, pCVD_Status->status_noise, pCVD_Status->vnon_standard_flag, g_CVD_Status_3CS.no_burst_flag);
		...
		color_lock = 0;
			...
			&& !( (pCVD_Status->cvd_color_system == CVD_STATE_PALi) \
				&& g_CVD_Status_3CS.no_burst_flag && g_CVD_No_Burst_SECAM_WA) )	//191113 : R-12 no burst signal ,not to transit to SECAM
	{
		if(color_lock == 0 ) {
			...
			if(color_count > 5) {
				...
				CVD_DEBUG("Burst Detected !\n");
				CVD_DEBUG("Color[%d],chroma[%d],h[%d],v[%d],secam[%d],pal[%d],cvd_fc[%d],cs0_fc[%d],cs1_fc[%d],burst_mag[0x%x],noise, [0x%x],vnon[%d],noburst[%d]\n",\
						...
						pCVD_Status->cs1_fc_flag, pCVD_Status->cvd_burst_mag, pCVD_Status->status_noise, pCVD_Status->vnon_standard_flag, g_CVD_Status_3CS.no_burst_flag);
...
// Not Used
#if 0
int CVD_Burst_Gate_Control_for_NoBurst(CVD_STATUS_3CS_T *pCVD_Status, int clear_burst_gate)
{
	...
	if(clear_burst_gate) {
		...
		if(no_color_count > 5) {
			...
			CVD_Burst_Gate_Control(g_SetColorSystem_3CS[g_CVD_Status_3CS.cvd_color_system].reg_burst_gate_start_new, 0xC0);
		...
		if(g_CVD_Status_3CS.no_color_detected == 1) {
			...
			CVD_Burst_Gate_Control(g_SetColorSystem_3CS[g_CVD_Status_3CS.cvd_color_system].reg_burst_gate_start_new, g_SetColorSystem_3CS[g_CVD_Status_3CS.cvd_color_system].reg_burst_gate_end_new);
		}
...
// Not Used
int CVD_Cstate_Control_for_SC_PullInRange(CVD_STATUS_3CS_T *pCVD_Status, int init_cstate) 
{
...
int CVD_Pattern_Detection(CVD_PATTERN_DETECTION_T *pCVD_Pattern_Detection_t )
{
	...
	CVD_Get_PE0_Motion_Value(&pe0_motion_value_t.tnr_x_avg_t[0], &pe0_motion_value_t.tnr_x_avg_s[0], &pe0_motion_value_t.tpd_s_status[0] );

	CVD_ColorBar_Detector(&colorbar_diff_sum, &colorbar_75_100);
...
//	CVD_DEBUG("PE0 : avg_t[0x%x], avg_s[0x%x], s_status[0x%x] \n", pe0_motion_value_t.avg_tnr_x_avg_t, pe0_motion_value_t.avg_tnr_x_avg_s, pe0_motion_value_t.avg_tpd_s_status);
	...
	pCVD_Pattern_Detection_t->global_motion_value = CVD_Get_Global_Motion_Value();
...
int CVD_AGC_Bypass_Function(UINT32 noise_status, UINT32 hlock_flag, UINT32 no_signal_flag )
{
	...
	if( ( noise_status == CVD_NOISE_MAX) && ( hlock_flag == 0 )&& ( no_signal_flag == 0)) {
		if(bypass_enable == 0) {
			bypass_enable = 1;
			CVD_AGC_Bypass(bypass_enable);
			//CVD_Set_dcrestore_accum_width(0x1b);
			//111109 by kim.min for weak cable RF signals.
			CVD_Set_dcrestore_accum_width(DCRESTORE_ACCUM_WIDTH_FOR_WEAK_RF);
		...
		if(g_CVD_Status_3CS.cvd_prev_color_system < CVD_STATE_VideoNotReady)
			CVD_Set_dcrestore_accum_width(pSetColorSystem_3CS->Color_System_Params[g_CVD_Status_3CS.cvd_prev_color_system].reg_dcrestore_accum_width);
		CVD_DEBUG("## Disable AGC Bypass\n");
	}
	/*
	   if(noise_status == CVD_NOISE_MAX)
	   CVD_Set_dcrestore_accum_width(0x28);
	   else if(noise_status < 0x100)
	   CVD_Set_dcrestore_accum_width(0x22);
	 */
	CVD_UnlockDevice();
	return ret;
}
...
int CVD_Set_for_Tunning(BOOLEAN bEnable)
{
	...
	if(bEnable == TRUE)
	{
		...
		CVD_Set_dcrestore_accum_width(DCRESTORE_ACCUM_WIDTH_DEFAULT);
		CVD_Set_Noise_Threshold(NOISE_THRESHOLD_DEFAULT);

		// restore default value of reg_hnon_std_threshold
#ifdef L9_FAST_3DCOMB_WORKAROUND
		CVD_Set_HNon_Standard_Threshold(pSetColorSystem_3CS->Color_System_Params[CVD_STATE_NTSC].reg_hnon_std_threshold);
#endif
#ifdef M16_FAST_3DCOMB_WORKAROUND
		//Enable vf_nstd_en when auto tuning
		CVD_vf_nstd_control(1);	
		// restore default value of reg_hnon_std_threshold
		CVD_Set_HNon_Standard_Threshold(pSetColorSystem_3CS->Color_System_Params[CVD_STATE_NTSC].reg_hnon_std_threshold);
#endif
		...
		//Auto Programming
		CVD_Enable_Stable_Sync_Mode(0);
#endif
#ifdef CVD_NTSC_NOISY_STABLE_VSYNC_WORKAROUND
			CVD_Enable_LPF_en(0);
#endif
#ifdef M16_ENABLE_PALM_STABLE_SYNC_MODE // not used
	...
	else
	{
		...
		if(g_CVD_Status_3CS.cvd_prev_color_system < CVD_STATE_VideoNotReady)
			CVD_Set_dcrestore_accum_width(pSetColorSystem_3CS->Color_System_Params[g_CVD_Status_3CS.cvd_prev_color_system].reg_dcrestore_accum_width);
		CVD_Set_Noise_Threshold(NOISE_THRESHOLD_WEAK_RF);
	}
	CVD_UnlockDevice();
	return ret;
}
...
int CVD_Get_Reg_States_Detail(LX_AFE_CVD_STATES_DETAIL_T	*pcvd_states_detail_t)
{
	...
	pcvd_states_detail_t->No_Signal_Flag = CVD_M16Ax_Get_No_Signal_Flag();
	pcvd_states_detail_t->HLock_Flag = CVD_M16Ax_Get_HLock_Flag();
	pcvd_states_detail_t->VLock_Flag = CVD_M16Ax_Get_VLock_Flag();
	pcvd_states_detail_t->VLine_625_Flag = CVD_M16Ax_Get_Vline_625_Flag();
	pcvd_states_detail_t->Pal_Flag = CVD_M16Ax_Get_PAL_Flag();
	pcvd_states_detail_t->Secam_Flag = CVD_M16Ax_Get_SECAM_Flag();
	pcvd_states_detail_t->Chromalock_Flag = CVD_M16Ax_Get_Chromalock_Flag();
	pcvd_states_detail_t->NoBurst_Flag = CVD_M16Ax_Get_NoBurst_Flag();
	pcvd_states_detail_t->FC_Flag = CVD_M16Ax_Get_FC_Flag(LX_CVD_MAIN);
	pcvd_states_detail_t->cvd_cordic_freq = CVD_M16Ax_Get_Cordic_Freq(LX_CVD_MAIN);
	pcvd_states_detail_t->status_noise = CVD_M16Ax_Get_Noise_Status();

	pcvd_states_detail_t->Pal_Flag_CS0 = CVD_M16Ax_Get_PAL_Flag_CS0();
	pcvd_states_detail_t->Secam_Flag_CS0 = CVD_M16Ax_Get_SECAM_Flag_CS0();
	pcvd_states_detail_t->Chromalock_Flag_CS0 = CVD_M16Ax_Get_Chromalock_Flag_CS0();
	pcvd_states_detail_t->FC_Flag_CS0 = CVD_M16Ax_Get_FC_Flag_CS0();
	pcvd_states_detail_t->cvd_cordic_freq_CS0 = CVD_M16Ax_Get_Cordic_Freq_CS0();

	pcvd_states_detail_t->Pal_Flag_CS1 = CVD_M16Ax_Get_PAL_Flag_CS1();
	pcvd_states_detail_t->Secam_Flag_CS1 = CVD_M16Ax_Get_SECAM_Flag_CS1();
	pcvd_states_detail_t->Chromalock_Flag_CS1 = CVD_M16Ax_Get_Chromalock_Flag_CS1();
	pcvd_states_detail_t->FC_Flag_CS1 = CVD_M16Ax_Get_FC_Flag_CS1();
	pcvd_states_detail_t->cvd_cordic_freq_CS1 = CVD_M16Ax_Get_Cordic_Freq_CS1();
	CVD_M16Ax_Get_PE0_Motion_Value(&pcvd_states_detail_t->tnr_x, &pcvd_states_detail_t->tnr_s, &pcvd_states_detail_t->s_status );
	pcvd_states_detail_t->motion_value = CVD_M16Ax_Get_Global_Motion_Value();
	pcvd_states_detail_t->agc_gain = CVD_M16Ax_Get_Status_AGC_Gain();
	pcvd_states_detail_t->cagc = CVD_M16Ax_Get_CAGC_Value();
	//pcvd_states_detail_t->saturation = CVD_M16Ax_Get_Saturation_Value();
	pcvd_states_detail_t->saturation = CVD_M16Ax_Get_oadj_c_coeff_value();
	pcvd_states_detail_t->hnon_standard = CVD_M16Ax_Get_HNon_Standard_Flag();
	pcvd_states_detail_t->vnon_standard = CVD_M16Ax_Get_VNon_Standard_Flag();
	pcvd_states_detail_t->burst_mag = CVD_M16Ax_Get_CVD_Burst_Mag_Value();
	pcvd_states_detail_t->vdetect_vcount = CVD_M16Ax_Get_Vdetect_Vcount_Value();
	pcvd_states_detail_t->agc_peak_en = CVD_M16Ax_Get_AGC_Peak_En_Value();
	pcvd_states_detail_t->agc_bypass = CVD_M16Ax_Get_AGC_Bypass_Value();
	...
	pcvd_states_detail_t->status_clamp_updn = CVD_M16Ax_Get_status_clamp_updn();

	CVD_ColorBar_Detector(&pcvd_states_detail_t->colorbar_diff, &pcvd_states_detail_t->colorbar_75_100);

	CVD_M16Ax_Get_AFE_CVBS_status(&pcvd_states_detail_t->cvbs_icon, &pcvd_states_detail_t->cvbs_pdb, &pcvd_states_detail_t->cvbs_cp, &pcvd_states_detail_t->cvbs_insel);
	CVD_M16Ax_Get_AFE_VBUF_status(&pcvd_states_detail_t->buf_ycm, &pcvd_states_detail_t->buf_sel1, &pcvd_states_detail_t->buf_pdb1, &pcvd_states_detail_t->vdac_pdb);

	AFE_PRINT("M16 CVD Driver Version : CVD_HW[0x%x], CVD_CONTROL[0x%x], CVD_TASK[0x%x] \n", g_M16_CVD_HW_DRV_REV, g_M16_CVD_CONTROL_DRV_REV, g_M16_CVD_TASK_DRV_REV);
	AFE_PRINT("Color Bar Diff Sum[%d], 75/100[%d]\n", pcvd_states_detail_t->colorbar_diff, pcvd_states_detail_t->colorbar_75_100);
	AFE_PRINT("no_color [%d]\n", g_CVD_Status_3CS.no_color_detected);
	return 0;
}
#endif

int CVD_ColorBar_Detector(UINT32 *pdiff_sum, UINT8 *pcolorbar_75_100)
{
	...
	CVD_Get_ColorBar_Level(composite_level);
...
int CVD_Comb2d_Only_Test(int set_params, int enable, int set_on_time, int set_off_time, int set_md_on_time, int set_md_off_time)
{
#if 0
