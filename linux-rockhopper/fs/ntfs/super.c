		...
		const int silent)
{
	...
	if (!is_boot_sector_ntfs(sb, (NTFS_BOOT_SECTOR*)boot_sector,
				 silent)) {
		if (!silent)
...
static bool parse_ntfs_boot_sector(ntfs_volume *vol, const NTFS_BOOT_SECTOR *b)
{
	...
	if (vol->mft_record_size < vol->sector_size) {
		ntfs_warning(vol->sb, "Mft record size (%i) is smaller than the "
				"sector size (%i).", vol->mft_record_size,
				vol->sector_size);
	}
 ...
 */
static int load_and_check_logfile(ntfs_volume *vol,
...
static void ntfs_volume_free(ntfs_volume *vol)
{
 ...
 */
s64 ntfs_available_clusters_count(ntfs_volume *vol, s64 nr_clusters)
{
	s64 free_clusters;
...
{
	ntfs_volume *vol = container_of(work, ntfs_volume, precalc_work);
	s64 nr_free;
...
#endif
{
	...
	*vol = (ntfs_volume) {
		.sb = sb,
		.fmask = current->fs->umask,
		.dmask = current->fs->umask,
	};
	...
	if (!(boot = read_ntfs_boot_sector(sb, silent))) {
		if (!silent)
	 ...
	 * using it.
	 */
	result = parse_ntfs_boot_sector(vol, (NTFS_BOOT_SECTOR*)boot);
	kfree(boot);
	if (!result) {
		if (!silent)
	...
	if ((sb->s_root = d_make_root(vol->root_ino))) {
		...
		nr_records = __get_nr_free_mft_records(vol,
				...
				((((NTFS_I(vol->mft_ino)->initialized_size >> vol->mft_record_size_bits) +
...
static int ntfs_init_fs_context(struct fs_context *fc)
{
	...
	/* Allocate a new ntfs_volume and place it in sb->s_fs_info. */
	vol = kmalloc(sizeof(ntfs_volume), GFP_NOFS);
	if (!vol)
		return -ENOMEM;
...
static int __init init_ntfs_fs(void)
{
	...
	ntfs_name_cache = kmem_cache_create(ntfs_name_cache_name,
			(NTFS_MAX_NAME_LEN+2) * sizeof(ntfschar), 0,
	...
	ntfs_inode_cache = kmem_cache_create(ntfs_inode_cache_name,
			sizeof(ntfs_inode), 0, SLAB_RECLAIM_ACCOUNT, NULL);
#else
	ntfs_inode_cache = kmem_cache_create(ntfs_inode_cache_name,
			sizeof(ntfs_inode), 0,
