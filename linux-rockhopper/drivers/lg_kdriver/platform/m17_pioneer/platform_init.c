...
static int	hfr_te_shared_mpool = 1;
module_param_named(hfr_te_shared_mpool, hfr_te_shared_mpool, int, 0644 );

#ifdef USE_HEVC_4K_HFR_MEM_MAP
static int	VDEC_HFR_MEMSZ = UHD_VDEC_4K_HFR_EXT_HMA_SZ;	/* SIC internal use only */
#else
...
#ifdef USE_HEVC_4K_HFR_MEM_MAP
	DEFINE_MEM_DESC(VDEC_HFR,MEMCFG_DESC_FLAG_CMA),
#else
...
#endif
	LX_MEMCFG_ENTRY_EX(gMemCfgAud[0], MEM_DESC(AUDIO)),		/* AUDIO - MS12 V1.3 */
...
#ifdef INCLUDE_KDRV_DE
	LX_MEMCFG_ENTRY_EX(gMemCfgDeFW[0], MEM_DESC(DE_FW)),	/* DE FW(2MB) */ // 20170705, to solve capture fail problem...
...
//	LX_MEMCFG_ENTRY_EX(gMemCfgDeFW[0], MEM_DESC(DE_FW)),	/* DE FW(2MB) */
	...
	LX_MEMCFG_ENTRY_EX(gMemCfgDe[0], MEM_DESC(DE)),		/* DE (FW+FRAME) for 60Hz Display */
	LX_MEMCFG_ENTRY_EX(gMemCfgBT_CMA[0], MEM_DESC(BT)),		/* BT speaker */
...
//	LX_MEMCFG_ENTRY_EX(gMemCfgDeFW[0], MEM_DESC(DE_FW)),	/* DE FW(2MB) */
	...
	LX_MEMCFG_ENTRY_EX(gMemCfgDe[0], MEM_DESC(DE)),		/* DE (FW+FRAME) for 120Hz Display*/
	LX_MEMCFG_ENTRY_EX(gMemCfgBT_CMA[0], MEM_DESC(BT)),		/* BT speaker */
	LX_MEMCFG_ENTRY_EX(gMemCfgVT_CMA[1], MEM_DESC(VT)),		/* 4k frame buffer for VT */
#endif
...
static void BASE_PLT_SetupCommon(void)
{
	...
	/* setup HFR EXT memory */
#ifdef INCLUDE_KDRV_VDEC
	#ifdef USE_HEVC_4K_HFR_MEM_MAP
 	if(g_model_ctx.ddr_size == MODEL_OPT_DDR_1_5_GB_STD)
	{
		vdec_hfr_hma.size = 0;
	}
	else
    ...
    switch(g_model_ctx.ddr_size)
    {
		...
		default:					   vdec0_hma.size = (VDEC_HFR_MEMSZ)? UHD_VDEC_4K_2K_HFR_HMA_SZ:UHD_VDEC_4K_2K_HMA_SZ; break;
	}
#endif
...
static void BASE_PLT_Setup_1_5_GB_STD(void)
{
	...
	printk("configure M16+(%2X) kdriver memory (1.5GB/STD)\n",
			lx_chip_rev() & 0xff);
...
{
	printk("configure M16+(%2X) kdriver memory (2.0GB/STD)\n",
			lx_chip_rev() & 0xff);
...
{
	/* determine memory layout */
	printk("configure M16+(%2X) kdriver memory (2.0GB/ADV)\n",
			lx_chip_rev() & 0xff);
...
{
	printk("configure M16+(%2X) kdriver memory (3.0GB/STD)\n",
			lx_chip_rev() & 0xff);
...
{
	printk("configure M16+(%2X) kdriver memory (3.0GB/ADV)\n",
			lx_chip_rev() & 0xff);
...
static void BASE_PLT_DoPostMemoryLayout(void)
{
	...
	switch(g_model_ctx.ddr_size)
	{
		...
		case MODEL_OPT_DDR_2_0_GB_STD:
		case MODEL_OPT_DDR_2_0_GB_ADV:
		default:						ddr0_section[0].size = (VDEC_HFR_MEMSZ)? UHD_VDEC_4K_2K_HFR_DPB_SZ:UHD_VDEC_4K_2K_DPB_SZ; break;
	}
	...
	/* alloc another DPB for HFR
	 *
	 * raxis.lim (2017/09/22)
	 * select backup memory pool for "vdec_hfr_hma" from "dpb" to "te_shared" (SICDTV-4375)
	 */
	if (vdec_hfr_hma.size>0)
	{
		ret = hma_pool_register_type (vdec_hfr_hma.name, vdec_hfr_hma.base, vdec_hfr_hma.size, BIGCMA_POOL);
		if (ret < 0)
		{
			printk (KERN_ERR "cannot register \"%s\" pool\n", vdec_hfr_hma.name);
		}
		...
		{
			hma_pool_fallback (vdec_hfr_hma.name, (hfr_te_shared_mpool)?"te_shared":"dpb");
		}
	}

#ifdef USE_HEVC_4K_HFR_MEM_MAP
	/* raxis.lim (2017/09/xx)
	 * if 4K FHR is supported, DPB doesn't need any backup memory pool because it has enough memory (?)
	 * ask justine.jeong to check surface region as valid DPB area before using fallback.
	 */
#else
	hma_pool_fallback ("dpb", "surface");
#endif

	/* make dpb pool alias for human readability */
	hma_pool_alias("dpb0", "dpb");
	hma_pool_alias("dpb1", "vdec_hfr_hma");
