 ...
 */
int DE_IPC_O26_Init(UINT32 mcu_id,           \
					void **pVideoBuffToMCU, \
					void **pVideoBuffFrMCU)
{
	int ret = RET_OK;
   ...
   *pVideoBuffFrMCU     = (UINT32)OS_KMalloc(DTVSOC_IPC_FROM_MCU_SIZE);
   ...
   {
	   case 0:
		   *pVideoBuffToMCU = (void *)ioremap( \
				   DTVSOC_IPC_FROM_CPU_O26_BASE,\
				   DTVSOC_IPC_FROM_CPU_SIZE);
		   *pVideoBuffFrMCU = (void *)ioremap(\
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_IPC_O26_Free(void **pVideoBuffToMCU, void **pVideoBuffFrMCU)
{
#ifdef USE_QEMU_SYSTEM
