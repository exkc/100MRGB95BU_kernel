...
/*----------------------------------------------------------------------------------------
	global Variables
----------------------------------------------------------------------------------------*/
//static ucom_l18_gov_t* ucom_l18_gov = NULL;
...
ucom_l18_func_t * ucom_func;
ucom_driver_t* ucom_drv;
...
static OS_PROC_DESC_TABLE_T	_g_ucom_l18_device_proc_table[] =
{
...
int	l18_read_proc(UINT32 procId, char* buffer )
{
	int	status=0,ret = 0 ;
...
int l18_write_proc( UINT32 procId, char* command )
{
	int ret = 0;
	switch( procId ) {
 ...
 * @see UCOM_Init
*/
void l18_ucom_proc_init (void)
{
	ucom_func	= get_l18_ucom_func();
	ucom_drv =  get_l18_ucom_driver();


	OS_PROC_CreateEntryEx ( UCOM_MODULE, _g_ucom_l18_device_proc_table,
										l18_read_proc,
										l18_write_proc );
 ...
 * @see UCOM_Cleanup
*/
void l18_ucom_proc_cleanup (void)
{
	OS_PROC_RemoveEntry( UCOM_MODULE );
}

static ucom_proc_t l18_proc_func =
{
	.init = l18_ucom_proc_init,
	.cleanup = l18_ucom_proc_cleanup,
};

ucom_proc_t* get_l18_ucom_proc(void)
{
	return &l18_proc_func;
}
