		...
		const int uname_len, ntfs_name **res)
{
	...
	for (;; ie = (INDEX_ENTRY*)((u8*)ie + le16_to_cpu(ie->length))) {
		...
		if ((u8*)ie < (u8*)ctx->mrec ||
		    ...
		    (u8*)ie + sizeof(INDEX_ENTRY_HEADER) + le16_to_cpu(ie->key_length) >
...
descend_into_child_node:
	...
	post_read_mst_fixup((NTFS_RECORD*)kaddr, PAGE_SIZE);
	folio_unlock(folio);
	folio_put(folio);
#else
...
static int ntfs_ia_blocks_readahead(ntfs_inode *ia_ni, loff_t pos)
{
...
filldir:
		...
		if (err) {
			...
			if (private->key)
				kfree(private->key);
			private->key = (void *)kmalloc(le16_to_cpu(next->key_length), GFP_KERNEL);
			if (!private->key) {
				err = -ENOMEM;
				goto out;
			}
...
out:
	while (!RB_EMPTY_ROOT(&ra_root)) {
		...
		cnir = rb_entry(node, struct ntfs_index_ra, rb_node);
		page_cache_sync_readahead(vol->mft_ino->i_mapping,
                                        ra, NULL,
...
int ntfs_check_empty_dir(ntfs_inode *ni, MFT_RECORD *ni_mrec)
{
	ntfs_attr_search_ctx *ctx;
	int ret = 0;
