...
#define GFX_PM_DEBUG(format, args...)   DBG_PRINT( g_gfx_debug_fd, 30,  format, ##args)
...
----------------------------------------------------------------------------------------*/
static inline int _get_header_size_fbdc(int w, int h)
{
    int number_block;

    number_block = (ALIGN(w, 64) * h) / 64;

    return ALIGN(number_block * 4, 16);
}

static inline int _get_alloc_size_fbdc(int w, int h)
{
    int alloc_size;

    alloc_size = ALIGN(w, 64) * h * 4;
    //alloc_size = (int)(alloc_size*4/5);   // +80% for compact mode
    //alloc_size = (int)(alloc_size*13/20); // +65% for compact mode
    alloc_size = (int)(alloc_size / 2);     // +50% for compact mode

    alloc_size += _get_header_size_fbdc(w, h);

    return LX_CALC_ALIGNED_VALUE(alloc_size, 12 /* 4K */);
}
...
extern GFX_SURFACE_OBJ_T*   GFX_GetSurfaceObject(UINT32 fd);
...
extern void GFX_FreeProcessSurface(UINT32 pid);
extern void GFX_DetatchProcessSurface(UINT32 surface_fd);
...
extern void GFX_ExecuteUserCommand(char* command);
extern void GFX_ExecutePMTest(char* command);
