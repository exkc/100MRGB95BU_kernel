...
#if defined(CONFIG_EKP) && defined(CONFIG_KEXEC_FIRMWARE)
static __always_inline void ekp_kexec_prepare(struct kimage *kimage)
{
	if (likely(ekp_enabled())) {
		u64 func = FUNC_ISE_CALL;
		u64 cmd = EKP_KEXEC_PREPARE;
		u64 val = (u64)(~(unsigned long)kimage ^ kimage->start);

		asm volatile(
		"	mov	x0, %0\n"
		"	mov	x1, %1\n"
		"	mov	x2, %x2\n"
		"	.globl	__ekp_machine_kexec\n"
		"__ekp_machine_kexec:\n"
		"	hvc	#0"
		: : "r" (func), "r" (cmd), "rZ" (val) : "x0", "x1", "x2");
	}
}
#else
static inline void ekp_kexec_prepare(struct kimage *kimage) {}
#endif
