...
/*----------------------------------------------------------------------------------------
  global Variables
  ----------------------------------------------------------------------------------------*/
const char *g_logm_clrStrings[] = {	/* color    : index: Value      */
...
static int LOGM_GetTailHeader(struct logm_header **h, unsigned char **data,
			      struct logm_buffer_ctrl *bc);
static int _LOGM_PutLogPrintk(LX_LOGM_LOGOBJ_T * obj, LX_LOGM_LOGLEVEL_T level,
			      const char *str, int len);
static void _LOGM_BufLogCnt(struct logm_buffer_ctrl *bc,
			    LX_LOGM_LOGLEVEL_T level);
static void _LOGM_ObjLogCnt(LX_LOGM_LOGOBJ_T * obj, LX_LOGM_LOGLEVEL_T level);
...
int LOGM_ObjRegister(const char *name)
{
	...
	LOGM_CHECK_ERROR(name == NULL, return RET_ERROR, "param null\n");
...
/**
 * add mask of logm obj
 * if bit '1' then enable print of the bit
 * @param fd
 * @return int
 */
int LOGM_ObjMaskAdd(SINT32 fd, UINT32 mask)
{
	unsigned long flag;
...
/**
 * set hidden_mask of logm obj
 * if bit '1' then do not print the log
 * just put the log at buffer
 * @param fd
 * @return int
 */
int LOGM_ObjHiddenMaskSet(SINT32 fd, UINT32 mask)
{
	unsigned long flag;
	...
	spin_lock_irqsave(&g_logm_obj_map_lock, flag);
	g_logm_obj_map[fd].hidden_mask = mask;
#if LOGM_FORCE_ERR_WARN
	g_logm_obj_map[fd].hidden_mask &=
	    ~((1 << LX_LOGM_LEVEL_ERROR) | (1 << LX_LOGM_LEVEL_WARNING));
#endif
	spin_unlock_irqrestore(&g_logm_obj_map_lock, flag);
...
EXPORT_SYMBOL(LOGM_ObjHiddenMaskSet);

/**
 * get hidden_mask of logm obj
 * @param fd
 * @return int
 */
int LOGM_ObjHiddenMaskGet(SINT32 fd, UINT32 * mask)
{
	unsigned long flag;
	...
	spin_lock_irqsave(&g_logm_obj_map_lock, flag);
	*mask = g_logm_obj_map[fd].hidden_mask;
	spin_unlock_irqrestore(&g_logm_obj_map_lock, flag);

	return RET_OK;
}

EXPORT_SYMBOL(LOGM_ObjHiddenMaskGet);

/**
 * enable a bit hidden_mask of logm obj
 * if '1' then don't print log
 * , but put the log at buffer
 * @param fd
 * @param bit
 */
int LOGM_ObjBitHiddenMaskEnable(SINT32 fd, SINT32 bit)
{
	unsigned long flag;
	...
	spin_lock_irqsave(&g_logm_obj_map_lock, flag);
	g_logm_obj_map[fd].hidden_mask |= (1 << bit);
#if LOGM_FORCE_ERR_WARN
	g_logm_obj_map[fd].hidden_mask &=
	    ~((1 << LX_LOGM_LEVEL_ERROR) | (1 << LX_LOGM_LEVEL_WARNING));
#endif
	...
	debug("enable hidden bit %d, %s\n", bit, g_logm_obj_map[fd].name);
	return RET_OK;
}

EXPORT_SYMBOL(LOGM_ObjBitHiddenMaskEnable);

/**
 * disable a bit hidden_mask of logm obj
 * if bit '0' then do print log
 * @param fd
 * @param bit
 */
int LOGM_ObjBitHiddenMaskDisable(SINT32 fd, SINT32 bit)
{
	unsigned long flag;
	...
	debug("disable hidden bit %d, %s\n", bit, g_logm_obj_map[fd].name);
	return RET_OK;
}

EXPORT_SYMBOL(LOGM_ObjBitHiddenMaskDisable);
		  ...
		  const char *fn, int fl, const char *fmt, ...)
{
	...
	LOGM_CHECK_ERROR((*fd < 0 || *fd >= LOGM_LOGOBJ_MAX)
			 , return RET_ERROR, "invalid fd = %d [called %s:%d]\n",
			 *fd, fn, fl);
...
int LOGM_PutLine(LX_LOGM_LOGOBJ_T * obj,
		 LX_LOGM_LOGLEVEL_T level, LX_LOGM_LOGTYPE_T type,
		 const void *bin_data, int bin_size, const char *str, int len)
{
	unsigned long flag;
	int ret;
	void *data = NULL;
	...
	//check hidden log
	if (obj && (obj->hidden_mask & (1 << level)))
		type = LX_LOGM_LOGTYPE_1;

	//if true, all logs be printed through "printk"
	//not null, not \n
	if (g_logm_path_ctrl && (type != LX_LOGM_LOGTYPE_1))
		_LOGM_PutLogPrintk(obj, level, str, len);

	//put log at logm buffer
	spin_lock_irqsave(&(bc->buffer_lock), flag);
	ret = LOGM_PutBuffer(obj, level, type, data, bin_size, str, len, bc);

	if (g_logm_global_ctrl_buffer && !g_logm_path_ctrl) {
		if (type != LX_LOGM_LOGTYPE_1)
...
static int log_to_kernel_level(LX_LOGM_LOGLEVEL_T level)
{
	switch (level) {
...
static int _LOGM_PutLogPrintk(LX_LOGM_LOGOBJ_T * obj,
			      LX_LOGM_LOGLEVEL_T level, const char *str,
			      int len)
{
	...
	LOGM_CHECK_ERROR(pk_buf == NULL, return RET_ERROR, "fail kmalloc\n");
	...
	if (!g_logm_short_format) {
		printk(KERN_SOH "%d" LOGM_NAME_LEVEL_FORMAT " (%05d:%05d) %s\n",
		       log_to_kernel_level(level), obj ? obj->name : "___",
		       LOGM_LevelName(level),
		       //current->comm,
		       task_tgid_nr(current), task_pid_nr(current), pk_buf);
	} else {
		printk(KERN_SOH "%d" LOGM_NAME_LEVEL_FORMAT " %s\n",
		       log_to_kernel_level(level), obj ? obj->name : "___",
		       LOGM_LevelName(level), pk_buf);
	}
			  ...
			  const char *str, int len, struct logm_buffer_ctrl *bc)
{
 ...
 case1:
		if (LOGM_BUFFER_SIZE <
		    ALIGN8(bc->buffer_head + sizeof(struct logm_header) * 2 +
			   len)) {
			header =
			    (struct logm_header *)(bc->logm_buf +
						   bc->buffer_head);
			header->size = -1;
	...
	if (bc->buffer_head < bc->buffer_tail) {
		while (bc->buffer_tail - bc->buffer_head <=
		       ALIGN8(sizeof(struct logm_header) + len)) {
			debug("head %4d\n", bc->buffer_head);
			LOGM_DropLine(bc);
	...
	header->ptid =
	    LOGM_MAKE_PTID(task_tgid_nr(current), task_pid_nr(current));
	header->count = ++(bc->buffer_write_counter);
	memcpy(bc->logm_buf + bc->buffer_head + sizeof(struct logm_header), str,
	       len);
			      ...
			      struct logm_buffer_ctrl *bc)
{
	...
	if (data)
		*data =
		    bc->logm_buf + bc->buffer_tail + sizeof(struct logm_header);
 ...
 *
 */
int LOGM_ReadLog(struct logm_buffer_ctrl *bc, char *data, int readed, int size)
{
	int ret;
	...
	if (h->type == LX_LOGM_LOGTYPE_0) {
		...
		if (ret < 0)
			goto func_exit;
	} else if (h->type == LX_LOGM_LOGTYPE_1) {
		/* hidden log, don't print */
		ret = 0;
	} else {
		error("Oops\n");
		ret = 0;
...
int LOGM_ParseLog(struct logm_header *h, char *dest, int32_t dest_len)
{
	...
	if (!g_logm_short_format) {
		hlen = snprintf(dest, dest_len,
				...
				nanosec_rem / 1000,
				h->obj ? h->obj->name : "___",
				LOGM_LevelName(h->level),
				(h->ptid >> 16) & 0xffff, h->ptid & 0xffff);
	} else {
		hlen = snprintf(dest, dest_len,
				"[%5lu.%06u] " LOGM_NAME_LEVEL_FORMAT " ",
	...
	if (col_str) {
		...
		memcpy((void *)(&dest[hlen + col_len + length]),
		       (void *)nocol_str, nocol_len);
	}
