...
// KyuYeunKim: removed #include <mali_kbase_gpu_memory_debugfs.h> due to build conflict
...
struct kbase_device {
	...
	// KyuYeunKim: adds gpu_util_dentry to xreclaimable_work
	/* debugfs entry for utilization */
        struct dentry *gpu_util_dentry;
	/* Root dirctory for gpu memory */
	struct proc_dir_entry *webos_gpu_directory;
	/* Root dirctory for um */
	struct proc_dir_entry *webos_um_directory;
	/* gpu memory information for /proc/meminfo */
	struct gpu_info_driver gpu_info;

	/* gpu xreclaimable memory */
	struct xreclaimable *gpu_xr;
	struct hrtimer time_xreclaimable;
	bool time_xreclaimable_running;
	spinlock_t time_xreclaimable_lock;
	struct workqueue_struct *xreclaimable_wq;
	struct work_struct xreclaimable_work;
