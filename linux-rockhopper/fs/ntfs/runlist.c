 ...
 * @s_rl_count: the count of runlist element (including ENOENT)
 *
 ...
 */
runlist_element *ntfs_runlists_merge(runlist *d_runlist,
				     ...
				     size_t *new_rl_count)
{
	...
	int marker = 0;
	VCN marker_vcn = 0;
	runlist_element *drl = d_runlist->rl, *rl;
		...
		size_t *new_rl_count)
{
	...
	runlist_element *rl, *new_rl;	/* The output runlist. */
	...
	for (rlc = rl; rlc->length; rlc++)
		if (rlc->lcn < 0) {
			if (rlc->lcn != LCN_HOLE && rlc->lcn != LCN_DELALLOC) {
				pr_err("%s: bad runlist", __FUNCTION__);
				return -EINVAL;
			}
			return 1;
		}
...
s64 ntfs_rl_get_compressed_size(ntfs_volume *vol, runlist_element *rl)
{
	...
	if (!rl) {
		ntfs_error(vol->sb, "%s: ", __FUNCTION__);
		return -EINVAL;
	}
	...
	for (rlc = rl; rlc->length; rlc++) {
		if (rlc->lcn < 0) {
			if (rlc->lcn != LCN_HOLE && rlc->lcn != LCN_DELALLOC) {
				ntfs_error(vol->sb, "%s: bad runlist", __FUNCTION__);
				return -EINVAL;
			}
		} else
			ret += rlc->length;
...
static inline bool ntfs_rle_lcn_contiguous(runlist_element *left_rle,
					   runlist_element *right_rle)
{
...
/**
 * @dst_rl: fully mapped runlist.
 * @dst_cnt: including LCN_ENOENT
 * @src_cnt: excluding LCN_ENOENT
 */
runlist_element *ntfs_rl_insert_range(runlist_element *dst_rl, int dst_cnt,
				      runlist_element *src_rl, int src_cnt,
				      size_t *new_rl_cnt)
{
	runlist_element *i_rl, *new_rl, *src_rl_origin = src_rl;
	runlist_element dst_rl_split;
	VCN start_vcn = src_rl[0].vcn, src_vcn_cnt;
	int new_1st_cnt, new_2nd_cnt, new_3rd_cnt, new_cnt;
	int i;

	BUG_ON(dst_rl[dst_cnt - 1].length != 0 ||
	       src_rl[src_cnt - 1].lcn < LCN_HOLE);

	start_vcn = src_rl[0].vcn;
	src_vcn_cnt = 0;
	for (i = 0; i < src_cnt; i++)
		src_vcn_cnt += src_rl[i].length;
	...
	if (i_rl[0].vcn != start_vcn) {
		...
		runlist_element *dst_rle, *src_rle;
merge_src_rle:
				    ...
				    size_t *new_rl_cnt)
{
	...
	end_vcn = min_t(VCN, start_vcn + len - 1,
	...
	*punch_rl = ntfs_malloc_nofs((punch_cnt + 1) * sizeof(runlist_element));
	if (!*punch_rl)
		return ERR_PTR(-ENOMEM);

	new_cnt = dst_cnt - (int)(e_rl - s_rl + 1) + 3;
	new_rl = ntfs_malloc_nofs(new_cnt * sizeof(runlist_element));
	if (!new_rl) {
		ntfs_free(*punch_rl);
		*punch_rl = NULL;
		return ERR_PTR(-ENOMEM);
...
runlist_element *ntfs_rl_collapse_range(runlist_element *dst_rl, int dst_cnt,
					VCN start_vcn, VCN len,
					runlist_element **punch_rl,
					size_t *new_rl_cnt)
{
	runlist_element *s_rl, *e_rl, *new_rl, *dst_3rd_rl;
	VCN end_vcn;
