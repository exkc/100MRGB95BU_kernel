...
----------------------------------------------------------------------------------------*/
/* O22A0 */
static void __PHY_Reg_O22A0_RdFL(UINT8 port, UINT32 reg, UINT32 *pData);
static void __PHY_Reg_O22A0_WrFL(UINT8 port, UINT32 reg, UINT32 data);

static void __PHY_Reg_B1_O22A0_RdFL(UINT8 port, UINT32 reg, UINT32 *pData);
static void __PHY_Reg_B1_O22A0_WrFL(UINT8 port, UINT32 reg, UINT32 data);

static void __PHY_Reg_B2_O22A0_RdFL(UINT8 port, UINT32 reg, UINT32 *pData);
static void __PHY_Reg_B2_O22A0_WrFL(UINT8 port, UINT32 reg, UINT32 data);

/* TCS State Hist */
static void __HDMI21_PHY_O22A0_IncreaseTcsStateHistIdx(UINT8 port);
static int __HDMI21_PHY_O22A0_CheckTcsStateHistMatchVal(UINT8 port, UINT32 val);

/* DFE Center */
static void __HDMI21_PHY_O22A0_SetDfeResultCenter(UINT8 port);
/* DR_RESETB */
static int __HDMI21_PHY_O22A0_ToggleDrResetB(int port, int isSet);
static int __HDMI21_PHY_O22A0_ToggleCrModeSelResetB(int port, int isSet);

/* FLT_START */
static int __HDMI21_PHY_O22A0_ToggleFltStart(int port, int isHigh, int isSet, UINT32 __callerLine);

static int HDMI_PHY_Set_Reg_Table(UINT8 port, HDMI_PHY_REG_APB_O22A0_T reg, HDMI_PHY_INIT_MODE_T mode);
...
static void _SET_PHY (UINT8 port, HDMI_PHY_REG_APB_O22A0_T index, HDMI_PHY_INIT_MODE_T mode);

//static int __HDMI21_PHY_O22A0_CheckEqCalDone(int port);
/*----------------------------------------------------------------------------------------
	External Variables
----------------------------------------------------------------------------------------*/
...
extern UINT32 o22a0_phy_table[PHY_REG_MAX_NUM_O22A0][HDMI_PHY_INITIATED_MAXNUM];
extern volatile PHY_REG_TABLE_O22A0 o22a0_phy_reg_table[PHY_REG_MAX_NUM_O22A0];
...
----------------------------------------------------------------------------------------*/
/* O22A0 */
HDMI21_PHY_REG_CTRL_O22A0_T 	phyReg_o22a0[HDMI21_NUM_OF_PORTS_O22];
HDMI21_PHY_REG_CTRL_B1_O22A0_T 	phyReg_B1_o22a0[HDMI21_NUM_OF_PORTS_O22];
HDMI21_PHY_REG_CTRL_B2_O22A0_T 	phyReg_B2_o22a0[HDMI21_NUM_OF_PORTS_O22];

static UINT32 gPhyTcsStateHist[HDMI21_NUM_OF_PORTS_O22][HDMI21_PHY_TCS_HIST_HISTORY_DEPTH] = {0,};
static int    gPhyTcsStateHistIdx[HDMI21_NUM_OF_PORTS_O22] = {0, };
static int    gPhyFrlFrlStartManualIdx[HDMI21_NUM_OF_PORTS_O22] = {0, };

//TODO: O22A0
//HDMI21_PHY_O22A0_SAFF_OFFSET_DATA_T gPhySaffOffsetResult_O22[HDMI21_NUM_OF_PORTS_O22][HDMI21_HAL_CNT_OF_CHANNEL_IN_ONE_PHY];

//TODO: O22A0
//int gHDMI21PhyFakeTcsDone_O22[HDMI21_NUM_OF_PORTS_O22] = {0, };
//int gHDMI21PhyJinWooNewO22Check[HDMI21_NUM_OF_PORTS_O22] = {0, };
...
----------------------------------------------------------------------------------------*/
/* O22A0 */
static void __HDMI21_PHY_O22A0_CheckPhyExceptionCase(UINT8 port, int resetClear);
static void __HDMI21_PHY_O22A0_SetPhyExceptionCase(UINT8 port, HDMI21_HAL_PHY_EXCEPTION_CASE_T phyCase, int isReverse);
static int 	__HDMI21_PHY_O22A0_Check_CRDone(UINT8 port);
static int  __HDMI21_PHY_O22A0_Check_RSDone(UINT8 port);
static int  __HDMI21_PHY_O22A0_IsOnboardPort(UINT8 port);

static int __HDMI21_PHY_O22A0_Check_EqCalDone(UINT8 port);
/* Common */
...
int HDMI21_PHY_O22A0_CheckDfeDone(UINT8 port)
{
	int ret = 0;
	...
	PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_5f);
	PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_5f, dfe_done_ch0, ch0DfeDone);
	PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_5f, dfe_done_ch1, ch1DfeDone);
	PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_5f, dfe_done_ch2, ch2DfeDone);
	PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_5f, dfe_done_ch3, ch3DfeDone);
...
void HDMI21_PHY_O22A0_InitiateDfeCenter(UINT8 port)
{
	int ch0DfeDone = 0;
	int ch1DfeDone = 0;
	int ch2DfeDone = 0;
	...
	if(gHDMI21RxDevPhyControler[HDMI21__PHY_DFE_CENTER_SWWA].value > 0) {
		...
		HDMI21_PHY_O22A0_Update_TMDS_TCS_Value(port, 0);
		__HDMI21_PHY_O22A0_SetDfeResultCenter(port);
	}
...
/* IOCTL */
int HDMI21_PHY_O22A0_Rx_Get_PHYStatus(LX_HDMI_PHY_INFORM_T *pData, int size, int updatePhyI2C, int isInternalCall)
{
	int ret = RET_ERROR;
	LX_HDMI_PHY_INFORM_T userdata;
	UINT32 tempData;
	UINT32 valuefor5V[HDMI21_NUM_OF_PORTS_O22] = {0,};
	int i;
	...
	do{
		...
		for(i=0; i<HDMI21_NUM_OF_PORTS_O22; i++){
		...
		if(updatePhyI2C > 0){
			for(i=0; i<HDMI21_NUM_OF_PORTS_O22; i++){
				if(userdata.hdmi5v[i] > 0){
					/* Read Errors */
					tcsError = HDMI21_PHY_O22A0_Check_TMDSError(i);
					SyncRecovery = HDMI21_PHY_O22A0_Check_SyncRecovery(i);
					isCrDone = __HDMI21_PHY_O22A0_Check_CRDone(i);
					isRsDone = __HDMI21_PHY_O22A0_Check_RSDone(i);
					cedError = HDMI21_PHY_O22A0_Check_CEDError(i);
					deltaError = HDMI21_PHY_O22A0_Check_DeltaError(i);
					deltaEye = HDMI21_PHY_O22A0_Check_DeltaEye(i);
					if(gHDMI21RxDevVideoControler[HDMI21__VIDEO_UPDATE_PHY_DFE_RESULT].value > 0){
						dfeResult = HDMI21_PHY_O22A0_Check_DfeResult(i);
					}

					updateTcsValue = HDMI21_PHY_O22A0_Update_TMDS_TCS_Value(i, 0);
				}
			}
		}
...
int HDMI21_PHY_O22A0_InitiatePhySystemCtrl(UINT8 port) 
{ 
	int ret = RET_ERROR; 
	...
	do{ 
		...
		if(port >= HDMI21_NUM_OF_PORTS_O22){
			HDMI20_ERROR("Error! Port exceeded max port size(%d)]n", port);
			break;
		}

		/* Mem allocate Phy Block #0 SHDW space */
		if((phyReg_o22a0[port].shdw.addr == NULL)){
			phyReg_o22a0[port].shdw.addr = (UINT32 *)OS_KMalloc((size_t)sizeof(HDMI21_PHY_REG_O22A0_T));

		}
		else {
			OS_Free((void *)phyReg_o22a0[port].shdw.addr);
			phyReg_o22a0[port].shdw.addr = (UINT32 *)OS_KMalloc((size_t)sizeof(HDMI21_PHY_REG_O22A0_T));
		}

		/* Mem allocate Phy Block #1 SHDW space */
		if((phyReg_B1_o22a0[port].shdw.addr == NULL)){
			phyReg_B1_o22a0[port].shdw.addr = (UINT32 *)OS_KMalloc((size_t)sizeof(HDMI21_PHY_REG_B1_O22A0_T));

		}
		else {
			OS_Free((void *)phyReg_B1_o22a0[port].shdw.addr);
			phyReg_B1_o22a0[port].shdw.addr = (UINT32 *)OS_KMalloc((size_t)sizeof(HDMI21_PHY_REG_B1_O22A0_T));
		}

		/* Mem allocate Phy Block #1 SHDW space */
		if((phyReg_B2_o22a0[port].shdw.addr == NULL)){
			phyReg_B2_o22a0[port].shdw.addr = (UINT32 *)OS_KMalloc((size_t)sizeof(HDMI21_PHY_REG_B2_O22A0_T));

		}
		else {
			OS_Free((void *)phyReg_B2_o22a0[port].shdw.addr);
			phyReg_B2_o22a0[port].shdw.addr = (UINT32 *)OS_KMalloc((size_t)sizeof(HDMI21_PHY_REG_B2_O22A0_T));
		}
...
void HDMI21_PHY_O22A0_UpdatePhyLockFlag(UINT8 port)
{
	int tcsDone = 0;
	int updateTcsValue = 0;
	int isTcsStable = 0;
	...
	do{
		...
		//need to check
		/* 2020/06/01 */
		if(__HDMI21_PHY_O22A0_ToggleFltStart(port, 0, 0, __L__ ) > 0) {
			__HDMI21_PHY_O22A0_ToggleFltStart(port, 0, 1, __L__);
		}

		/* Update Data */
		tcsDone = HDMI21_PHY_O22A0_Check_TCSDone(port);
		
		updateTcsValue = HDMI21_PHY_O22A0_Update_TMDS_TCS_Value(port, 0);
		isCrDone = __HDMI21_PHY_O22A0_Check_CRDone(port);
		isRsDone = __HDMI21_PHY_O22A0_Check_RSDone(port);

		tcsError = HDMI21_PHY_O22A0_Check_TMDSError(port);
		SyncRecovery = HDMI21_PHY_O22A0_Check_SyncRecovery(port);
		cedError = HDMI21_PHY_O22A0_Check_CEDError(port);
		deltaError = HDMI21_PHY_O22A0_Check_DeltaError(port);
		deltaEye = HDMI21_PHY_O22A0_Check_DeltaEye(port);
		if(gHDMI21RxDevVideoControler[HDMI21__VIDEO_UPDATE_PHY_DFE_RESULT].value > 0){
			dfeResult = HDMI21_PHY_O22A0_Check_DfeResult(port);
		}
		...
		if(tcsDone >= 0){
			...
			if(gHDMI21RxDevPhyControler[HDMI21__PHY_ENABLE_PHY_EXCEPTION_CASE].value > 0){
				__HDMI21_PHY_O22A0_CheckPhyExceptionCase(port, 0);
			}
		}
	}while(0);
	...
	if(port < HDMI21_NUM_OF_PORTS_O22){
		...
		__HDMI21_PHY_O22A0_IncreaseTcsStateHistIdx(port);
				...
				&& (gHDMI21RxHandler[port].phyInitiatedMode >= HDMI21_HAL_PHY_INITIATED_FRL_3G_3L))
		{
			...
			HDMI21_PHY_O22A0_CheckDfeDone(port);
		}
		else if((gHDMI21RxHandler[port].fsm_phy_lock > 0) && (isPhyLocked == true)  \
				&& (gHDMI21RxHandler[port].phyFrlSwWaState == 2) \
				...
				&& (gHDMI21RxHandler[port].phyInitiatedMode >= HDMI21_HAL_PHY_INITIATED_FRL_3G_3L))
		{
			...
			if((gHDMI21RxDevPhyControler[HDMI21__PHY_FRL_EQ_SWWA_OPTION_3].value & 0x1) != 0) { 
				...
				PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_47);
				PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_47, ch0_sr_ssb, ch0SrSsb);
				PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_48);
				PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_48, ch1_sr_ssb, ch1SrSsb);
				PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_49);
				PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_49, ch2_sr_ssb, ch2SrSsb);
				PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_4a);
				PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_4a, ch3_sr_ssb, ch3SrSsb);
			}
			else {
				ch0SrSsb = 0x10;
			}
		...
		else if(isPhyLocked == false)
		{
			...
			if(gHDMI21RxHandler[port].phyInitiatedMode >= HDMI21_HAL_PHY_INITIATED_FRL_3G_3L) {
				...
				tcsStateMatchVal = __HDMI21_PHY_O22A0_CheckTcsStateHistMatchVal(port, gHDMI21RxDevPhyControler[HDMI21__PHY_TCS_STATE_HIST_MATCH].value);
				...
				if((tcsStateMatchVal == gHDMI21RxDevPhyControler[HDMI21__PHY_TCS_STATE_HIST_DEPTH].value)&&(_r0.SCDC_FLT_UPDATE == 0)){
					PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_59);
					PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_59, flt_state, flt_state);
					...
					if(flt_state == gHDMI21RxDevPhyControler[HDMI21__PHY_FLT_STATE_MATCH].value) {
						if((gHDMI21RxDevPhyControler[HDMI21__PHY_FRL_EQ_SWWA_OPTION_1].value & 0x1) != 0x0) {
							PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_59);
							PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_5a, eq_success_sel, 0x8);
							PHY_REG_B2_O22A0_WrFL(port, hdmi_phy_addr_5a);
							HDMI21_LINE_DBG(595, "PHY[%d]  : FLT State[%d] in TCS State stuck case. Execute #1. EQ Sucess Sel [8]->[7]\n", port, flt_state);

							PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_59);
							PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_5a, eq_success_sel, 0x7);
							PHY_REG_B2_O22A0_WrFL(port, hdmi_phy_addr_5a);
						}
						...
						if((gHDMI21RxDevPhyControler[HDMI21__PHY_FRL_EQ_SWWA_OPTION_1].value & 0x2) != 0x0) {
							...
							__HDMI21_PHY_O22A0_ToggleDrResetB(port, 1);
							__HDMI21_PHY_O22A0_ToggleDrResetB(port, 0);
						}
						...
						if((gHDMI21RxDevPhyControler[HDMI21__PHY_FRL_EQ_SWWA_OPTION_1].value & 0x4) != 0x0) {
							...
							__HDMI21_PHY_O22A0_ToggleCrModeSelResetB(port, 1);
							__HDMI21_PHY_O22A0_ToggleCrModeSelResetB(port, 0);
						}
					}
					else {
						if(_r0.FRL_START_STS == 0) {
							...
							if(_r0.SCDC_FRL_START == 0) {
								...
								if(gPhyFrlFrlStartManualIdx[port] >= gHDMI21RxDevPhyControler[HDMI21__PHY_FRL_EQ_SWWA_OPTION_2].value) {
									...
									PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_99);
									PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_99, eq_auto_pll3, 0);
									PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_99);

									PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_66);
									PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_66,  flt_restart, 0x1);
									PHY_REG_B2_O22A0_WrFL(port, hdmi_phy_addr_66);

									HDMI21_LINE_DBG(595, "PHY[%d] : Send FLT Restart!\n", port);

									PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_66);
									PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_66,  flt_restart, 0x0);
									PHY_REG_B2_O22A0_WrFL(port, hdmi_phy_addr_66);

									HDMI21_PHY_O22A0_ToggleTCSEn(port, 0);
									HDMI21_PHY_O22A0_ToggleTCSEn(port, 1);
	...
	if(gHDMI21RxDevPhyControler[HDMI21__PHY_ENABLE_PHY_EXCEPTION_CASE].value > 0){
		if(port < HDMI21_NUM_OF_PORTS_O22){
			if( gHDMI21RxHandler[port].isPhyLocked != true ){
				__HDMI21_PHY_O22A0_CheckPhyExceptionCase(port, 1);
			}
		}
	}
...
/* IOCTL */
int HDMI21_PHY_O22A0_Rx_AccessPhy(UINT8 port, LX_HDMI20_PHY_ACCESS_T *pPhyCtrl, int size)
{
	int ret = RET_ERROR;
...
int HDMI21_PHY_O22A0_Set_3G6G_Mode(UINT8 port, UINT8 isAuto, HDMI21_HAL_PHY_INITIATED_MODE_T mode, UINT8 isScrambling, UINT8 lowfreqGain)
{
	int ret = RET_ERROR;
	...
	do{
		...
		/* Read FRL No Retrain : 0x58C on O22A0, 0x590 on O22A0 */
		...
		isOnboard = __HDMI21_PHY_O22A0_IsOnboardPort(port);
		//O22A0 BringUp
		isOnboard = 0;
		...
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_81);
		PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_81, i2c_dfe_level_a_11_mode, 0x0);
		PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_81, i2c_dfe_level_a_01_mode, 0x0);
		PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_81);
		...
		//Hard Coding (audio)
		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_4b);
		if(mode ==  HDMI21_HAL_PHY_INITIATED_6G){
			PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_4b, pll_aud_man_val, 0x3);
			PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_4b, pll_aud_man_mode, 0x1);	
		}
		else{
			PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_4b, pll_aud_man_val, 0x0);
			PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_4b, pll_aud_man_mode, 0x0);	
		}
		PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_4b);

		/* 2020/08/19 */
		//Hard Coding
		if((((mode >= HDMI21_HAL_PHY_INITIATED_FRL_8G_4L) && (port == 3)))\
				|| ((mode >= HDMI21_HAL_PHY_INITIATED_FRL_10G_4L) && (port == 2))){
			PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_34);
			PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_34, cr_man_aud_ref_div, 1);
			PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_34, cr_i2c_aud_ref_div_mode, 1);
			PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_34);

			PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_4b);
			PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_4b, pll_aud_man_mode, 1);
			PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_4b, pll_aud_man_val, 3);
			PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_4b);
		}
		else{
			PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_34);
			PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_34, cr_man_aud_ref_div, 0);
			PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_34, cr_i2c_aud_ref_div_mode, 0);
			PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_34);
		}

		//Hard Coding
		if((gHDMI21RxHandler[port].top_read(0xB0) & 0x20000000) != 0) {
			PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_07);
			PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_07, mode_4ch_val, 0x1);
			PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_07, mode_4ch_man, 0x1);
			PHY_REG_B2_O22A0_WrFL(port, hdmi_phy_addr_07);
		}

		/* 0x39 : EQ_CAL_MODE_VAL 0->1 : Redo EQ offset calibration when lowfreqGain(RS) value changes */
		if(wasEQmodeChanged){
			HDMI21_PHY_O22A0_Reset_EqOffsetCalibration(port, 1, __L__);
			HDMI21_PHY_O22A0_Reset_EqOffsetCalibration(port, 0, __L__);
		}
		...
		if(mode >= HDMI21_HAL_PHY_INITIATED_FRL_3G_3L) {
			...
			PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_5a);
			PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_5a, eq_success_sel, 0x7);
			PHY_REG_B2_O22A0_WrFL(port, hdmi_phy_addr_5a);
			...
			if(mode >= HDMI21_HAL_PHY_INITIATED_FRL_8G_4L) {
				...
				if(gHDMI21RxDevPhyControler[HDMI21__PHY_RS_SCAN_RETRY_DISABLE].value == 0){
					PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_5a);
					PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_5a, eq_fail_sel, 0x4);
					PHY_REG_B2_O22A0_WrFL(port, hdmi_phy_addr_5a);
				}
			...
			__HDMI21_PHY_O22A0_ToggleFltStart(port, 1, 1, __L__);
		}else{
			if(gHDMI21RxHandler[port].isFLT >= 1){
				gHDMI21RxHandler[port].isFLT = 0;
		...
		//RESETB_ALL/////////////////////////////////////////////////////////////////////////////////////////////
		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_00);
		PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_00, resetb_all, 0x1);
		PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_00);
		//ODT_PDB////////////////////////////////////////////////////////////////////////////////////////////////
		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_48);
		PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_48, eq_i2c_odt_pdb_mode, 0x1);
		PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_48, eq_man_odt_pdb, 0x1);
		PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_48);
...
int HDMI21_PHY_O22A0_IsPhyPdbPowerOn(UINT8 port)
{
	UINT32 data;
	int ret = RET_ERROR;
...
int HDMI21_PHY_O22A0_ControlPhyPDB(UINT8 port, BOOLEAN bOnOff)
{
	UINT32 data, __devid;
	int ret = RET_ERROR;
...
int HDMI21_PHY_O22A0_Reset_EqOffsetCalibration(UINT8 port, UINT8 isReset, UINT32 callerId)
{
	int ret = RET_ERROR;
	...
	do{
		...
		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_47);
		if(isReset)	{PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_47, eq_cal_mode_val, 0x0);}
		else 		{PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_47, eq_cal_mode_val, 0x1);}
		PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_47);
...
int HDMI21_PHY_O22A0_PowerControl_PLLPDB(UINT8 port, UINT8 isDown, UINT32 callerId)
{
	int ret = RET_ERROR;
	...
	do{
		...
		/* hdmi_phy_addr_03 */
		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_02);
		if(isDown)	{PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_02, cr_pll_pdb, 0x0);}
		else 		{PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_02, cr_pll_pdb, 0x1);}
		PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_02);
...
int HDMI21_PHY_O22A0_Reset_PLLPDB(UINT8 port, UINT8 isReset, UINT32 callerId)
{
	int ret = RET_ERROR;
	...
	do{
		...
		/* CR_PLL_RESETB_PDB */
		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_02);
		if(isReset)	{PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_02, cr_pll_resetb, 0x0);}
		else 		{PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_02, cr_pll_resetb, 0x1);}
		PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_02);
...
static int __HDMI21_PHY_O22A0_Check_CRDone(UINT8 port)
{
	...
	do{
		...
		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_3e);
		PHY_REG_O22A0_Rd01(port, hdmi_phy_addr_3e, cr_done, isCrDone);
...
static int __HDMI21_PHY_O22A0_Check_RSDone(UINT8 port)
{
	...
	do{
		...
		/* Step1. Check RS Done */
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_2b);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_2b, rs_scan_done_ch3, rs_scan_done_ch[3]);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_2b, rs_scan_done_ch2, rs_scan_done_ch[2]);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_2b, rs_scan_done_ch1, rs_scan_done_ch[1]);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_2b, rs_scan_done_ch0, rs_scan_done_ch[0]);
		...
		/* Update RS Result */
		PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_1a);
		PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_1a, eq_rs_ch0, __rs_scan_ch[0]);
		PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_1b);
		PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_1b, eq_rs_ch1, __rs_scan_ch[1]);
		PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_1c);
		PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_1c, eq_rs_ch2, __rs_scan_ch[2]);
		PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_1d);
		PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_1d, eq_rs_ch3, __rs_scan_ch[3]);
...
int HDMI21_PHY_O22A0_Check_SyncRecovery(UINT8 port)
{
	...
	do{
		...
		if(gHDMI21RxHandler[port].phyInitiatedMode >= HDMI21_HAL_PHY_INITIATED_FRL_3G_3L){
			...
			/* Step 1. Check TCS Done */
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_14);
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_14, tot_sync_recover_ch0, phySyncRecovery[0]);

			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_15);
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_15, tot_sync_recover_ch1, phySyncRecovery[1]);

			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_16);
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_16, tot_sync_recover_ch2, phySyncRecovery[2]);

			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_17);
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_17, tot_sync_recover_ch3, phySyncRecovery[3]);
		}
		/* Step 2. Put in data */
...
int HDMI21_PHY_O22A0_Check_TMDSError(UINT8 port)
{
	...
	do{
		...
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_13);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_13, err_update, phyErrorCounter);


		/* Step 1. Check TCS Done */
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_18);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_18, tot_tmds_error_ch0, phyTMDSError[0]);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_19);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_19, tot_tmds_error_ch1, phyTMDSError[1]);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_1a);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_1a, tot_tmds_error_ch2, phyTMDSError[2]);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_1b);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_1b, tot_tmds_error_ch3, phyTMDSError[3]);
...
int HDMI21_PHY_O22A0_Check_DeltaEye(UINT8 port)
{
	...
	do{
		...
		/* Step 1. Check TCS Done */
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_94);
		PHY_REG_B1_O22A0_Rd02(port, hdmi_phy_addr_94, delta_plus_ch0, phyDeltaPlus[0], delta_minus_ch0, phyDeltaMinus[0]);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_95);
		PHY_REG_B1_O22A0_Rd02(port, hdmi_phy_addr_95, delta_plus_ch1, phyDeltaPlus[1], delta_minus_ch1, phyDeltaMinus[1]);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_96);
		PHY_REG_B1_O22A0_Rd02(port, hdmi_phy_addr_96, delta_plus_ch2, phyDeltaPlus[2], delta_minus_ch2, phyDeltaMinus[2]);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_97);
		PHY_REG_B1_O22A0_Rd02(port, hdmi_phy_addr_97, delta_plus_ch3, phyDeltaPlus[3], delta_minus_ch3, phyDeltaMinus[3]);
...
int HDMI21_PHY_O22A0_Check_DeltaError(UINT8 port)
{
	...
	do{
		...
		/* Step 1. Check TCS Done */
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_14);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_14, tot_delta_error_ch0, phyDeltaError[0]);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_15);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_15, tot_delta_error_ch1, phyDeltaError[1]);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_16);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_16, tot_delta_error_ch2, phyDeltaError[2]);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_17);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_17, tot_delta_error_ch3, phyDeltaError[3]);
...
int HDMI21_PHY_O22A0_Check_CEDError(UINT8 port)
{
	...
	do{
		...
		/* Step 1. Check TCS Done */
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_18);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_18, tot_ced_error_ch0, phyCEDError[0]);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_19);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_19, tot_ced_error_ch1, phyCEDError[1]);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_1a);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_1a, tot_ced_error_ch2, phyCEDError[2]);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_1b);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_1b, tot_ced_error_ch3, phyCEDError[3]);
...
int HDMI21_PHY_O22A0_Check_DfeResult(UINT8 port)
{
	int ret = RET_ERROR;
	int i;
	...
	do{
		...
		for(i=0;i<HDMI21_AMOUNT_OF_TMDS_CHANNEL_IN_ONE_PHY;i++){
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_3b);
			PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_3b, dfe_result_ch_sel, i);
			PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_3b);

			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_35);
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_35, dfe_level_a_01, phyDfeA01[i]);
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_36);
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_36, dfe_level_a_11, phyDfeA11[i]);
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_37);
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_37, dfe_level_b1, phyDfeB1[i]);
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_38);
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_38, dfe_level_b2, phyDfeB2[i]);
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_39);
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_39, dfe_level_b3, phyDfeB3[i]);
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_3a);
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_3a, dfe_level_b4, phyDfeB4[i]);
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_3a);
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_3a, dfe_level_b5, phyDfeB5[i]);
...
int HDMI21_PHY_O22A0_Check_TCSDone(UINT8 port)
{
	...
	do{
		...
		/* Step 1. Check TCS Done */
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_2c);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_2c, tcs_done, isTcsDone);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_2c, tcs_fail_ch0, fail_ch0);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_2c, tcs_fail_ch1, fail_ch1);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_2c, tcs_fail_ch2, fail_ch2);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_2c, tcs_fail_ch3, fail_ch3);
		...
		/*
		//TODO: O22A0
		if((gHDMI21PhyFakeTcsDone_O22[port] > 0) && (gHDMI21RxHandler[port].phyInitiatedMode >= HDMI21_HAL_PHY_INITIATED_FRL_10G_4L)) {
			ret = RET_OK;
		}
		*/
...
UINT8 HDMI21_PHY_O22A0_Get_TMDS_FREQ(UINT8 port, int isMSB)
{
	UINT8 tmds_value = 0;
	...
	do{
		...
		if(isMSB){
			PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_3a); 
			PHY_REG_O22A0_Rd01(port, hdmi_phy_addr_3a, tmds_freq_2, tmds_value);
		}
		else{
			PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_3b); 
			PHY_REG_O22A0_Rd01(port, hdmi_phy_addr_3b, tmds_freq_1, tmds_value);
		}
...
void HDMI21_PHY_O22A0_UpdatePhyTMDSclock(UINT8 port)
{
	UINT32 tmdsVal;
	int isManualEQ = 0;
	HDMI_PHY_INIT_MODE_T local_mode = HDMI_PHY_INITIATED_3G_M3;
	...
	/* Calculate */
	tmdsVal = ((((UINT32)HDMI21_PHY_O22A0_Get_TMDS_FREQ(port, 1) & 0x000000FF)<<8) | ((UINT32)HDMI21_PHY_O22A0_Get_TMDS_FREQ(port, 0) & 0x000000FF));
		...
		if(gHDMI21RxHandler[port].phyInitiatedMode < HDMI21_HAL_PHY_INITIATED_6G) {
			if(gHDMI21RxHandler[port].phyCurrentTMDSExceptionRange != (gHDMI21RxHandler[port].phyPrevTMDSExceptionRange)){
				...
				do { 
					...
					PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_5a);
					PHY_REG_B1_O22A0_Wr02(port, hdmi_phy_addr_5a,\
							ana_cdr_sel,	o22a0_phy_table[ANA_CDR_SEL_O22A0][local_mode],\
							cdr_fltr_ctrl,	o22a0_phy_table[CDR_FLTR_CTRL_O22A0][local_mode] );
					PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_5a);

					PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_9c);
					PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_9c, delta_scan2_mode, o22a0_phy_table[DELTA_SCAN2_MODE_O22A0][local_mode]);
					PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_9c);

					PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_9b);
					PHY_REG_B1_O22A0_Wr02(port, hdmi_phy_addr_9b, i2c_delta_sample_mode, 0x0, i2c_delta_sample, 0x0);
					PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_9b);

					PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_b8);
					PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_b8, tcs_err_sel, o22a0_phy_table[TCS_ERR_SEL_O22A0][local_mode]);
					PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_b8);

					PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_2f);
					PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_2f, dr_n2, o22a0_phy_table[DR_N2_O22A0][local_mode]);
					PHY_REG_B2_O22A0_WrFL(port, hdmi_phy_addr_2f);

					PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_8c);
					PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_8c, i2c_delta_cdr_val, o22a0_phy_table[I2C_DELTA_CDR_VAL_O22A0][local_mode]);
					PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_8c, i2c_delta_cdr_mode, o22a0_phy_table[I2C_DELTA_CDR_MODE_O22A0][local_mode]);
					PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_8c);

					PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_04);
					PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_04, dr_clk_inv_ch3, o22a0_phy_table[DR_CLK_INV_CH3_O22A0][local_mode]);
					PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_04, dr_clk_inv_ch2, o22a0_phy_table[DR_CLK_INV_CH2_O22A0][local_mode]);
					PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_04, dr_clk_inv_ch1, o22a0_phy_table[DR_CLK_INV_CH1_O22A0][local_mode]);
					PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_04, dr_clk_inv_ch0, o22a0_phy_table[DR_CLK_INV_CH0_O22A0][local_mode]);
					PHY_REG_B2_O22A0_WrFL(port, hdmi_phy_addr_04);

					//O22A0 Add
					PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_b9);
					PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_b9, tcs_divide_method, o22a0_phy_table[TCS_DIVIDE_METHOD_O22A0][local_mode]);
					PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_b9);
...
int HDMI21_PHY_O22A0_Update_TMDS_TCS_Value(UINT8 port, int isClear)
{
	...
	do{
		...
		if(isClear){
			...
			if(gHDMI21RxHandler[port].phyCurrentTMDSExceptionRange >= 0){
				...
				PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_20); PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_20, cs_min_ch0, tcs_min[0]);
				PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_22); PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_22, cs_min_ch1, tcs_min[1]);
				PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_24); PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_24, cs_min_ch2, tcs_min[2]);
				PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_26); PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_26, cs_min_ch3, tcs_min[3]);

				/* Step 2. Check TCS Max for every TMDS channel */
				PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_21); PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_21, cs_max_ch0, tcs_max[0]);
				PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_23); PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_23, cs_max_ch1, tcs_max[1]);
				PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_25); PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_25, cs_max_ch2, tcs_max[2]);
				PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_27); PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_27, cs_max_ch3, tcs_max[3]);
			}
			else {
				/* Step 1. Check TCS min for every TMDS channel */
				PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_f4); PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_f4, delta_cs_min_ch0, tcs_min[0]);
				PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_f6); PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_f6, delta_cs_min_ch1, tcs_min[1]);
				PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_f8); PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_f8, delta_cs_min_ch2, tcs_min[2]);
				PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_fa); PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_fa, delta_cs_min_ch3, tcs_min[3]);

				/* Step 2. Check TCS Max for every TMDS channel */
				PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_f5); PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_f5, delta_cs_max_ch0, tcs_max[0]);
				PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_f7); PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_f7, delta_cs_max_ch1, tcs_max[1]);
				PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_f9); PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_f9, delta_cs_max_ch2, tcs_max[2]);
				PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_fb); PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_fb, delta_cs_max_ch3, tcs_max[3]);
			}

			/* Step 3. Check TCS Result */
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_1c); PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_1c, cs_out_ch0, tcs_result[0]);
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_1d); PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_1d, cs_out_ch1, tcs_result[1]);
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_1e); PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_1e, cs_out_ch2, tcs_result[2]);
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_1f); PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_1f, cs_out_ch3, tcs_result[3]);

			/* Step 4. Check TCS State */
			//O22A0 TODO
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_31); 
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_31, tcs_state_ch0, __tcs_state_ch0);
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_32); 
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_32, tcs_state_ch1, __tcs_state_ch1);
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_33); 
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_33, tcs_state_ch2, __tcs_state_ch2);
			PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_34); 
			PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_34, tcs_state_ch3, __tcs_state_ch3);
...
int HDMI21_PHY_O22A0_ToggleTCSEn(int port, int isSet)
{
	HDMI20_PHY("PHY[%d] : TCS [%s]\n", port, isSet?"Enable":"Disable");

	if(isSet) {
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_03);
		PHY_REG_B1_O22A0_Wr02(port, hdmi_phy_addr_03, tcs_en_man_mode, 0x1, tcs_en_man_val, 0x1);
		PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_03);
		...
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_03);
		PHY_REG_B1_O22A0_Wr02(port, hdmi_phy_addr_03, tcs_en_man_mode, 0x1, tcs_en_man_val, 0x0);
		PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_03);
	}
...
int HDMI21_PHY_O22A0_Rx_Initiate_Phy(int port, HDMI21_HAL_PHY_INITIATED_MODE_T mode)
{
	int ret = RET_ERROR;
	UINT8 lowfreqGain = 0;
	int isManualEQ = 0;
	...
	do{
		...
		if(HDMI21_PHY_O22A0_IsPhyPdbPowerOn(port) < 0){
			HDMI20_PHY("PHY[%d] : Phy PDB was Low but 5V High. Set PHY PDB to HIGH\n", port);
			HDMI21_PHY_O22A0_ControlPhyPDB(port, 1);
		}
		...
		if((gHDMI21RxHandler[port].top_read(0xb0) & 0x20000000) != 0) {
			PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_11);
			PHY_REG_O22A0_Wr02(port, hdmi_phy_addr_11, eq_pdb_ch0_man, 0x1, eq_pdb_ch0_val, 0x1);
			PHY_REG_O22A0_Wr02(port, hdmi_phy_addr_11, eq_pdb_ch1_man, 0x1, eq_pdb_ch1_val, 0x1);
			PHY_REG_O22A0_Wr02(port, hdmi_phy_addr_11, eq_pdb_ch2_man, 0x1, eq_pdb_ch2_val, 0x1);
			PHY_REG_O22A0_Wr02(port, hdmi_phy_addr_11, eq_pdb_ch3_man, 0x1, eq_pdb_ch3_val, 0x1);
			PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_11);
		}
		else {
			PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_11);
			PHY_REG_O22A0_Wr02(port, hdmi_phy_addr_11, eq_pdb_ch0_man, 0x0, eq_pdb_ch0_val, 0x0);
			PHY_REG_O22A0_Wr02(port, hdmi_phy_addr_11, eq_pdb_ch1_man, 0x0, eq_pdb_ch1_val, 0x0);
			PHY_REG_O22A0_Wr02(port, hdmi_phy_addr_11, eq_pdb_ch2_man, 0x0, eq_pdb_ch2_val, 0x0);
			PHY_REG_O22A0_Wr02(port, hdmi_phy_addr_11, eq_pdb_ch3_man, 0x0, eq_pdb_ch3_val, 0x0);
			PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_11);
		}

		PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_02);
		PHY_REG_B2_O22A0_Wr02(port, hdmi_phy_addr_02, pdb_ch0_man, 0x0, pdb_ch0_val, 0x0);
		PHY_REG_B2_O22A0_Wr02(port, hdmi_phy_addr_02, pdb_ch1_man, 0x0, pdb_ch1_val, 0x0);
		PHY_REG_B2_O22A0_Wr02(port, hdmi_phy_addr_02, pdb_ch2_man, 0x0, pdb_ch2_val, 0x0);
		PHY_REG_B2_O22A0_Wr02(port, hdmi_phy_addr_02, pdb_ch3_man, 0x0, pdb_ch3_val, 0x0);
		PHY_REG_B2_O22A0_WrFL(port, hdmi_phy_addr_02);

		/* 2021/11/16 */
		PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_8b);
		PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_8b, idx_max_lock, 0x0);
		PHY_REG_B2_O22A0_WrFL(port, hdmi_phy_addr_8b);



		/* 2019/11/06 */
		HDMI21_PHY_O22A0_PowerControl_PLLPDB(port, 1, __L__);

		/* 2019/10/22 */
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_03);
		PHY_REG_B1_O22A0_Wr02(port, hdmi_phy_addr_03, tcs_en_man_mode, 0x0, tcs_en_man_val, 0x0);
		PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_03);

		/* 0x20 : 0x02 (MANUAL HDMI20_MODE = 0) / 0xAD : 0xA0 (DISABLE_DECORDER/SCR_MAN_OFF) / 0x5D : 0x06(EQ_RS_MAN=6)... */
		HDMI21_PHY_O22A0_Set_3G6G_Mode(port, 0, mode, mode, lowfreqGain);

		
		/* 2019/11/04 : DK Kim said to have this set here on top of PDB RESET_ALL */
		HDMI21_PHY_O22A0_PowerControl_PLLPDB(port, 0, __L__);
...
int HDMI21_PHY_O22A0_CheckEqCalDone(int port)
{
	int ret = 0;
	UINT32 __d = 0;

	PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_46);
	PHY_REG_O22A0_Rd01(port, hdmi_phy_addr_46, eq_cal_done, __d);
...
{
	UINT32 apb_addr;
	if(port >= HDMI21_NUM_OF_PORTS_O22) return;
...
static int __HDMI21_PHY_O22A0_CheckTcsStateHistMatchVal(UINT8 port, UINT32 val)
{
	...
	do {
		/* Port Defence */
		if(port >= HDMI21_NUM_OF_PORTS_O22) break;
...
static void __HDMI21_PHY_O22A0_SetDfeResultCenter(UINT8 port)
{
	...
	if((gHDMI21RxHandler[port].isPhyFrlCtsMode > 0)&&(gHDMI21RxDevPhyControler[HDMI21__PHY_DFE_ADD_VAL_ON_CTS].value > 0)){
		...
		/* Write Manual Value */
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_a2);
		PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_a2, eq_cs_man_ch0, deltaResult[0]);
		PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_a2);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_a3);
		PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_a3, eq_cs_man_ch1, deltaResult[1]);
		PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_a3);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_a4);
		PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_a4, eq_cs_man_ch2, deltaResult[2]);
		PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_a4);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_a5);
		PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_a5, eq_cs_man_ch3, deltaResult[3]);
		PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_a5);

		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_99);
		PHY_REG_B1_O22A0_Wr01(port, hdmi_phy_addr_99, eq_auto_pll3, 1);
		PHY_REG_B1_O22A0_WrFL(port, hdmi_phy_addr_99);
	}
...
static int __HDMI21_PHY_O22A0_ToggleCrModeSelResetB(int port, int isSet)
{
	...
	if(isSet) {
		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_04);
		PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_04, cr_mode_sel_resetb, 0x0);
		PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_04);
	}
	else{
		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_04);
		PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_04, cr_mode_sel_resetb, 0x1);
		PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_04);
	}
...
static int __HDMI21_PHY_O22A0_ToggleDrResetB(int port, int isSet)
{
	...
	if(isSet) {
		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_01);
		PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_01, resetb_sel, 0x1);
		PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_01);

		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_06);
		PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_06, dr_resetb, 0x0);
		PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_06);
	}
	else{
		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_06);
		PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_06, dr_resetb, 0x1);
		PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_06);

		PHY_REG_O22A0_RdFL(port, hdmi_phy_addr_01);
		PHY_REG_O22A0_Wr01(port, hdmi_phy_addr_01, resetb_sel, 0x0);
		PHY_REG_O22A0_WrFL(port, hdmi_phy_addr_01);
	}
...
static int __HDMI21_PHY_O22A0_ToggleFltStart(int port, int isHigh, int isSet, UINT32 __callerLine)
{
	...
	/* Flush Shadow */
	PHY_REG_B2_O22A0_RdFL(port, hdmi_phy_addr_6a);
	
	if(isSet) {
		HDMI21_LINE_DBG(502, "PHY[%d] : Internal FLT_START signal[%s]. Caller[%d]\n", port, isHigh?"High":"Low", __callerLine);

		if(isHigh) {
			PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_6a, flt_start, 1);
		}
		else{
			PHY_REG_B2_O22A0_Wr01(port, hdmi_phy_addr_6a, flt_start, 0);
		}

		PHY_REG_B2_O22A0_WrFL(port, hdmi_phy_addr_6a);
	}

	PHY_REG_B2_O22A0_Rd01(port, hdmi_phy_addr_6a, flt_start, ret);
...
void HDMI21_PHY_O22A0_GetSaffValue(int port)
{
	//O22A0: TODO
	//unused function
...
void HDMI21_PHY_O22A0_SetSaffManualValue(int port)
{
	//O22A0 : TODO
	//unused function
...
static int __HDMI21_PHY_O22A0_Check_EqCalDone(UINT8 port)
{
	...
	do{
		...
		/* Step 1. Check TCS Done */
		PHY_REG_B1_O22A0_RdFL(port, hdmi_phy_addr_f0);
		PHY_REG_B1_O22A0_Rd01(port, hdmi_phy_addr_f0, eq_cal_done_cr, eqCalDone);
...
static void _SET_PHY (UINT8 port,HDMI_PHY_REG_APB_O22A0_T index, HDMI_PHY_INIT_MODE_T mode)
{
	UINT32 addr = o22a0_phy_reg_table[index].addr;
	HDMI_PHY_BLOCK_O22A0 block = o22a0_phy_reg_table[index].phy_block;
	volatile UINT32 data = 0x0;
	volatile UINT32 clear = MSK(o22a0_phy_reg_table[index].length, \
			o22a0_phy_reg_table[index].offset);
	volatile UINT32 mask = ((1 << (o22a0_phy_reg_table[index].length))-1);

	switch(block)
	{
		case PHY_PDC_O22A0:
			__PHY_Reg_O22A0_RdFL(port, addr, &data);
			data &= (~clear);
			mask &= (o22a0_phy_reg_table[index].reg_val[mode]);
			mask <<= o22a0_phy_reg_table[index].offset;
			data |= mask;
			__PHY_Reg_O22A0_WrFL(port, addr, data);
			break;

		case PHY_DR1_O22A0:
			__PHY_Reg_B1_O22A0_RdFL(port, addr, &data);
			data &= (~clear);
			mask &= (o22a0_phy_reg_table[index].reg_val[mode]);
			mask <<= o22a0_phy_reg_table[index].offset;
			data |= mask;
			__PHY_Reg_B1_O22A0_WrFL(port, addr, data);
			break;

		case PHY_DR2_O22A0:
			__PHY_Reg_B2_O22A0_RdFL(port, addr, &data);
			data &= (~clear);
			mask &= (o22a0_phy_reg_table[index].reg_val[mode]);
			mask <<= o22a0_phy_reg_table[index].offset;
			data |= mask;
			__PHY_Reg_B2_O22A0_WrFL(port, addr, data);
			break;
...
{
	int ret = 0;
	HDMI_PHY_REG_APB_O22A0_T index = -1;

	for(index=I2C_CON_SEL_MODE_O22A0; index<PHY_REG_MAX_NUM_O22A0; ++index)
	{
