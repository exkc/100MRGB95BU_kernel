...
#define pr_fmt(fmt) "low_mem_notify: " fmt
...
#define DEFAULT_SWAP_CONST		2

#define EFS_SWAP_NAME   "eswap"
#define F2IO_SWAP_NAME	"f2io-0"
#define ZRAM_SWAP_NAME	"zram0"
...
#define thresholds_refcount_init(tp)	atomic_set(&(tp)->refcount, 0)
#define thresholds_refcount_inc(tp)	atomic_inc(&(tp)->refcount)
#define thresholds_refcount_dec(tp)	atomic_dec(&(tp)->refcount)
#define wait_thresholds_used(tp)		\
do {	while (atomic_read(&(tp)->refcount))	\
		msleep_interruptible(1);	\
} while (0)
#define thresholds_refcount_use		thresholds_refcount_inc
#define thresholds_refcount_free	thresholds_refcount_dec
...
static inline int is_swap_dev_on(void)
{
	return (swap_dev_mode != SWAP_DEV_NONE);
}
...
void low_mem_notify_get_zram_stats(unsigned long *stats)
{
	...
	list_for_each_entry_rcu(pos, &zram_list, list) {
		...
		stats[ZRAM_STATS_ZERO_PAGES] +=
			atomic64_read(&pos->zram->stats.same_pages);
	}
...
static inline unsigned long _free_pages(void)
{
	unsigned long free = global_zone_page_state(NR_FREE_PAGES) -
				totalreserve_pages;
	unsigned long file = global_node_page_state(NR_FILE_PAGES) -
...
static inline unsigned long _usable_pages(void)
{
	...
	free += efs_shrink ?
		swap : swap / swap_const;
	free -= total_swapcache_pages();
#endif
	free += global_node_page_state(NR_SLAB_RECLAIMABLE_B);
...
void get_usable_info(struct usable_info * ui)
{
	unsigned long free = global_zone_page_state(NR_FREE_PAGES) -
		totalreserve_pages;
	ui->free = free;
	ui->file = global_node_page_state(NR_FILE_PAGES) -
		global_node_page_state(NR_SHMEM);
...
static void low_mem_notify_threshold(int force)
{
	...
	thresholds_refcount_use(&thresholds);

	free = _usable_pages(); /* _free_pages(); */

	i = t->current_threshold;

	if (force && thresholds.primary)
		eventfd_signal(t->entries[i].eventfd);

	for (; thresholds.primary && i >= 0 && unlikely(t->entries[i].threshold > free); i--)
		eventfd_signal(t->entries[i].eventfd);

	i++;

	for (; thresholds.primary && i < t->size && unlikely(t->entries[i].threshold <= free); i++)
		eventfd_signal(t->entries[i].eventfd);

	t->current_threshold = i - 1;

	thresholds_refcount_free(&thresholds);
unlock:
	rcu_read_unlock();
}
...
static int low_mem_reset_events(void)
{
	...
	mutex_lock(&thresholds_lock);
	dprintk("reset thresholds");
	wait_thresholds_used(&thresholds);
	if (thresholds.primary) {
		for (i = 0; i < thresholds.primary->size; i++) {
			eventfd_ctx_put(thresholds.primary->entries[i].eventfd);
		}
	}
	...
	thresholds_refcount_init(&thresholds);
	mutex_unlock(&thresholds_lock);
	return 0;
}
...
static ssize_t swap_show(struct kobject *kobj, struct kobj_attribute *attr,
			 char *buf)
{
	return sprintf(buf, "%d\n", swap_dev_mode);
}
	...
	if (t > 0) {
		...
		for (type = 0; type < MAX_SWAPFILES; type++) {
			...
			if (!p || !(p->flags & SWP_USED))
				continue;
			if (!strcmp(p->swap_file->f_path.dentry->d_name.name,
				    EFS_SWAP_NAME)) {
				swap_dev_mode = SWAP_DEV_EFS;
				swap_filenum = type;
				break;
			}
...
static ssize_t swap_const_show(struct kobject *kobj,
			       struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", swap_const);
}

static ssize_t swap_const_store(struct kobject *kobj,
				struct kobj_attribute *attr,
				const char *buf, size_t count)
{
	int t;
				...
				struct kobj_attribute *attr, char *buf)
{
	...
	unsigned long file = global_node_page_state(NR_FILE_PAGES)
				- global_node_page_state(NR_SHMEM);
	unsigned long slab_reclaimable = global_node_page_state(NR_SLAB_RECLAIMABLE_B);
	...
	unsigned long bufferuse = swap;

	bufferuse = efs_shrink ?
		bufferuse : bufferuse / swap_const;
#else
	unsigned long swap = 0, swapcache = 0, bufferuse = 0;
#endif

	usable = free + swap + file - swapcache + slab_reclaimable +
		xreclaimable - workingset - bufferuse;

	count  = sprintf(&buf[count], "usable pages :         %8lu\n", usable);
	count += sprintf(&buf[count], "(+) free :             %8ld\n", (long)free);
	count += sprintf(&buf[count], "(+) swap :             %8lu\n", swap);
	count += sprintf(&buf[count], "(-) bufferuse :        %8lu(%lu)\n",
		bufferuse, swap);
...
static ssize_t usable_raw_show(struct kobject *kobj,
			       struct kobj_attribute *attr, char *buf)
{
	unsigned long free = global_zone_page_state(NR_FREE_PAGES) - totalreserve_pages;
	unsigned long file = global_node_page_state(NR_FILE_PAGES)
						- global_node_page_state(NR_SHMEM);
	unsigned long slab_reclaimable = global_node_page_state(NR_SLAB_RECLAIMABLE_B);
	unsigned long workingset = atomic_long_read(&workingset_pages);
	...
	return sprintf(buf, "%ld %ld %ld %ld %ld %ld %ld %ld %ld\n",
			usable, free, swap, bufferuse, file, swapcache,
			slab_reclaimable, xreclaimable, workingset);
}
LOW_MEM_ATTR_RO(usable_raw);

/* For backward compatibility */
static struct kobj_attribute low_mem_efs_swap_attr =
	__ATTR(efs_swap, 0644, swap_show, swap_store);

static struct kobj_attribute low_mem_zramswap_attr =
	__ATTR(zramswap, 0644, swap_show, swap_store);
...
static int __init low_mem_init(void)
{
	...
	swap_dev_mode = SWAP_DEV_NONE;
	swap_const = DEFAULT_SWAP_CONST;
