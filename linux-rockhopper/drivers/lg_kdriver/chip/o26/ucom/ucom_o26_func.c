...
ucom_o26_data_t o26_udata;
static struct completion  o26_micom_completion;
#endif
...
#define O26_UCOM_STATUS_PRINTNUM	3
...
static void __iomem *o26_ipc_disable = NULL;
static void __iomem *o26_chipreset_base = NULL;
static void __iomem *o26_chipreset1_base = NULL;
static void __iomem *o26_chipreset2_base = NULL;
static void __iomem *o26_micom_req_base = NULL;
void __iomem *o26_micom_eeprom_base = NULL;

UINT16 o26_ucomkey =0 ;
UINT32 o26_ucom_abnormal = 0;

void o26_ucom_FuncInit(void);
void o26_ucom_FuncChipReset(void);
int o26_ucom_FuncWrite(LX_UCOM_PARAM_T * param);
int o26_ucom_FuncRead(LX_UCOM_PARAM_T * param);;
int __o26_spi_request(unsigned int cmd);
int __o26_ucom_spi_write( UINT8 * addr,  UINT8 * data , UINT8  num );
int __o26_ucom_spi_read( UINT8 * addr,  UINT8 * data , UINT8  num );
int o26_ucom_spi_write32( UINT32 addr, UINT32 val);
UINT32 o26_ucom_spi_read32(UINT32 addr);
int o26_ucom_spi_read( UINT8 * addr,  UINT8 * data , UINT8  num );
int o26_ucom_spi_write( UINT8 * addr,  UINT8 * data , UINT8  num );
int o26_ucom_FuncGetStatus(void );

unsigned int o26_get_micom_disable(void)
{
	unsigned int reg = 0;
	if(o26_ipc_disable == NULL)
		o26_ipc_disable = ioremap(0xF33A717c, 4);

	reg = readl(o26_ipc_disable);

	return reg;
}
EXPORT_SYMBOL(o26_get_micom_disable) ;

void o26_set_micom_disable(unsigned int disable)
{
	if(o26_ipc_disable == NULL)
		o26_ipc_disable = ioremap(0xF33A717c, 4);

	writel(disable, o26_ipc_disable);
}
EXPORT_SYMBOL(o26_set_micom_disable) ;
#if 0
...
void o26_ucom_FuncInit(void)
{
	...
	o26_micom_req_base = ioremap(0xFD3004C8,4);

	o26_micom_eeprom_base = ioremap( 0xca3a310c,4 );

	o26_chipreset_base = ioremap(0xCA3A0014,4);
	o26_chipreset1_base = ioremap(0xCA3A7010,4);
	o26_chipreset2_base = ioremap(0xCA222000,4);
	//
	/* to make mainsoc access eeprom */
	//PAD_WOC_65 (0xca3a310c)
	//reg_pad_micom_06_mux_sel[15:12] : 2 : micom_sclin2  (micom i2c)  / 3: scl7_out (main i2c)
	//reg_pad_micom_08_mux_sel[07:04]:  2 : micom_sdaout2 (micom i2c)  / 3: sda7_out (main i2c)
...
#ifdef USE_O26_IPC_INTR
	o26_udata.irq = 92 ;//32 + 60(O26_CPU_interrupt_list.xlsx)

	init_completion(&o26_micom_completion);

	if(request_irq( o26_udata.irq, _ucom_irqhandler, (unsigned long)0, "ucom0", &o26_udata))
	{
...
{
	/* recovery */
	if(o26_chipreset_base != NULL)
		writel(0x0 , o26_chipreset_base );
	if(o26_chipreset1_base != NULL)
		writel(0x1 , o26_chipreset1_base );
	if(o26_chipreset2_base != NULL)
		writel(0x1 , o26_chipreset2_base );
}
static u32 _ucom_key(void)
{
	/* should be called with ucom_mutex locked */
	if(o26_ucomkey >= 0x7fff)
		o26_ucomkey = 0;
	return 0xface0000 | o26_ucomkey++;
}

static int _ucom_request(u32 key, u32 flag)
{
#ifndef USE_O26_IPC_INTR
	unsigned long timeout = O26_UCOM_TIMEOUT * NSEC_PER_MSEC;
#endif
	...
	if(flag != UCOM_FAKE_CMD )
	{
		...
		if( wait_for_completion_timeout(&o26_micom_completion, msecs_to_jiffies(O26_UCOM_TIMEOUT)) < 0)
		{
			...
			goto quit;
			if(o26_ucom_abnormal > MAX_UCOM_ABNORMAL ) o26_ucom_abnormal = 0;
			else 	o26_ucom_abnormal++;
			/* clear done: requested by micom fw */
			writel(0,	m2a_irq_base);

			cur = sched_clock();
			if(o26_ucom_abnormal < O26_UCOM_STATUS_PRINTNUM){
				KDRV_UCOM_NOTI("ukdrv %2x:[%llu]\n", readl(a2m_ipc_base + MAILBOX(0) + REG(2)) & 0xff,
						cur - start);
			}
			err = -ETIME;
			goto quit;
		...
		while (!(readl(m2a_irq_base) & M2A_IPC_DONE)) {  // if m2a_irq_base == 0 , waiting
			...
			if (cur - start > timeout) {
				...
				if(o26_ucom_abnormal < O26_UCOM_STATUS_PRINTNUM)
				{
					KDRV_UCOM_NOTI("ukdrv [%2x:%2x][%llu] [%x:%x]\n", readl(a2m_ipc_base + MAILBOX(0) + REG(2)) & 0xff, readl(m2a_ipc_base + MAILBOX(0) + REG(2)) & 0xff,
							cur - start,key, readl(m2a_ipc_base + MAILBOX(0) + REG(1)));
				}
				err = -ETIME;
				goto quit;
			}
...
int o26_ucom_FuncWrite(LX_UCOM_PARAM_T * param)
{
	...
	while(retrycnt > 0)
	{
		...
		if(ret < 0 && retrycnt == 0)
		{
			KDRV_UCOM_ERROR("check ukdrv retry[%d] WCMD[%2x]size[%x] [%x:%x] ret[%x]\n",retrycnt ,param->Cmd,param->bufSize,key,readl(m2a_ipc_base + MAILBOX(0) + REG(1)),ret);
			return ret;
		}
	}
	...
	if(param->bufSize > 0)
	{
		...
		{
			ret = sprintf(str + i*3," %2x", pbuf[i]);
		}
		if(ret)
			KDRV_UCOM_DEBUG("[%2x:%2x] [%s] [%x:%x]\n",param->Cmd, readl(m2a_ipc_base + MAILBOX(0) + REG(2))&0xff,str,key,readl(m2a_ipc_base + MAILBOX(0) + REG(1)));
	}
	else
		KDRV_UCOM_DEBUG("[%2x] only\n",param->Cmd);
...
int o26_ucom_FuncRead(LX_UCOM_PARAM_T * param)
{
	...
	while(retrycnt > 0)
	{
		...
		if(ret < 0 && retrycnt == 0)
		{
			KDRV_UCOM_ERROR("check ukdrv retry [%d] RCMD[%2x]size[%x] [%x:%x]ret[%x]\n",retrycnt,param->Cmd,param->bufSize,key,readl(m2a_ipc_base + MAILBOX(0) + REG(1)),ret);
			return ret;
		}
	}
	...
	{
		pbuf = (UINT8 *)(param->ubuf);
		KDRV_UCOM_DEBUG("[%2x] [%2x] [%2x]\n",param->Cmd ,pbuf[0],pbuf[1]);
		return 0;
	}
...
int __o26_spi_request(unsigned int cmd)
{
	...
	cmd &= 0xFF00FFFF;
	cmd |= 0x00FE0000;
	writel(cmd, m2a_ipc_base + MAILBOX(2) + REG(1));

	timeout = jiffies + msecs_to_jiffies(O26_UCOM_TIMEOUT) ;
...
int __o26_ucom_spi_write( UINT8 * addr,  UINT8 * data , UINT8  num )
{
	...
	/* ucom spi serialization request */
	while(retrycnt > 0)
	{
		...
		if( retrycnt == 1 )
		{
			writel(0, m2a_ipc_base + MAILBOX(2) + REG(1));
			KDRV_UCOM_ERROR("spi busy clear\n");
			break;
		}
	}
	retrycnt = UCOM_RETRY_CNT;
...
int o26_ucom_spi_write32( UINT32 addr, UINT32 val)
{
	...
	ret = o26_ucom_spi_write(addrbuf,buf,1);
	return ret;
}
...
UINT32 o26_ucom_spi_read32(UINT32 addr)
{
	...
	o26_ucom_spi_read(addrbuf,buf,1);
...
{
	int ret = 0;
	if(o26_get_micom_disable() == 0)
	{
		O26_KDRV_UCOM_LOCK();
		ret = __o26_ucom_spi_read(addr, data, num);
...
int o26_ucom_spi_write( UINT8 * addr,  UINT8 * data , UINT8  num )
{
	...
	{
		O26_KDRV_UCOM_LOCK();
		ret = __o26_ucom_spi_write(addr,data, num);
		O26_KDRV_UCOM_UNLOCK();
	}
	return ret;
}
...
static ucom_o26_func_t o26_ucom_func =
{
	.init = o26_ucom_FuncInit,
	.chipreset = o26_ucom_FuncChipReset,
	.write = o26_ucom_FuncWrite,
	.read = o26_ucom_FuncRead,
	.getstatus= o26_ucom_FuncGetStatus,

};

ucom_o26_func_t* get_o26_ucom_func(void)
{
	return &o26_ucom_func;
}
