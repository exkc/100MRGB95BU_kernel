...
static UINT16	gO20GBB_FullDTMBPkerrCount 		= 12;
static UINT16 	gO20GBB_FullDTMBPkerrThreshold 		= 255;
static UINT16	gO20GBB_DTMB_UnlockCount_full_packeterr	= 0;

/***********************************************************************************
	Static Variable & Function Prototypes Declarations
 ************************************************************************************/
int _DEMOD_O20_DTMB_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
...
int _DEMOD_O20_LGDBB_DTMB_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	...
	tuneMode = _DEMOD_O20_DTMB_CheckTuneMode(pDemodInfo);

	if(RET_OK != DEMOD_O20_Get_BW(pDemodInfo, &dtmb_bandwidth_Khz))
	{
...
int _DEMOD_O20_LGDBB_DTMB_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int DEMOD_O20_DTMB_SW_Init (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
//	if(DEMOD_L2_DTMB_SW_Init(pFrontend, 0x26) != NO_DEMOD_ERROR)   // CPBOX
	...
	{
		DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] Fail, DEMOD_O20_DTMB_SW_Init()\n");
		DEMOD_RETURN_FAIL(__F__);
	}
...
************************************************************************************/
/**
 * DEMOD_O20_DTMB_Get_CarrierMode
...
*/
int DEMOD_O20_DTMB_Get_CarrierMode (LX_DEMOD_CARRIERMODE_T *pCarrierMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int DEMOD_O20_DTMB_Get_Time_Interval (BOOLEAN *pbM720)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int DEMOD_O20_DTMB_Get_Pn_Mode (LX_DEMOD_PNMODE_T *pPnMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int DEMOD_O20_DTMB_Get_CodeRate (LX_DEMOD_CODERATE_T *pCodeRate)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int DEMOD_O20_DTMB_Get_Constellation (LX_DEMOD_RF_MODE_T *pConstellation)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
************************************************************************************/
/**
 * DEMOD_O20_DTMB_Obtaining_Signal_Lock.
...
int DEMOD_O20_DTMB_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
...
//	static unsigned int 		sigStablestartTime 	= 0;
	...
	switch (pDemodInfo->controlState)
	{
		...
		case LX_DEMOD_TUNE_START:	/***************************************************/
			{
				if(RET_OK != _DEMOD_O20_LGDBB_DTMB_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, _DEMOD_O20_LGDBB_DTMB_SetDemod() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.dtmb.tuneMode)
				{
					...
					gO20GBB_FullDTMBPkerrThreshold 	= 255;
					gO20GBB_FullDTMBPkerrCount 		= 9;
		...
		case LX_DEMOD_SYNC_CHECKING : 	/********************************************/
			{
				...
				{
					DEMOD_PRINT("[DEMOD_CHIP] FAIL, DEMOD_O20_Get_OperMode()\n");
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				{
					DEMOD_PRINT("[DEMOD_CHIP] FAIL, DEMOD_O20_Get_Lock()\n");
					DEMOD_RETURN_FAIL(__F__);
				}
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE: 	/********************************************/
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if (RET_OK != _DEMOD_O20_LGDBB_DTMB_CheckSignalState(pDemodInfo))
					{
						DEMOD_PRINT( "[DEMOD_CHIP] FAIL, _DEMOD_O20_LGDBB_DTMB_CheckSignalState()\n");
						DEMOD_RETURN_FAIL(__F__);
					}
...
************************************************************************************/
/**
 * DEMOD_O20_DTMB_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O20_DTMB_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if(pDemodInfo->signalStatus.packetError >= gO20GBB_FullDTMBPkerrThreshold)
		{
			gO20GBB_DTMB_UnlockCount_full_packeterr++;
			DEMOD_PRINT( "[DEMOD_CHIP] gO20GBB_DTMB_UnlockCount_full_packeterr = %d \n",gO20GBB_DTMB_UnlockCount_full_packeterr);

			if(gO20GBB_DTMB_UnlockCount_full_packeterr == gO20GBB_FullDTMBPkerrCount)
			{
		...
		else
		{
			...
			if(pDemodInfo->bIsTPoutEn == TRUE)
			{
				...
				{
					DEMOD_PRINT( "[DEMOD_CHIP] FAIL, DEMOD_O20_TPOutEnable(TRUE)\n");
					DEMOD_RETURN_FAIL(__F__);
				}
			}
...
************************************************************************************/
/**
 * DEMOD_O20_DTMB_Set_Config_auto
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DTMB_Set_Config_auto (BOOLEAN bAutoDetect)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 DEMOD_O20_DTMB_Set_PartialConfig
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DTMB_Set_PartialConfig (LX_DEMOD_DTMB_CONFIG_T *pDtmbConfigParam)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int DEMOD_O20_DTMB_Set_MemSconfig (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
	...
	if(base_st_mem == 0)
	{
		DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] ERROR, O20 DEMOD DTMB Wrong Memory Base Address \n");
		return RET_ERROR;
	}
	else
	{
		DEMOD_PRINT("[DEMOD_CHIP] DEMOD_O20_DTMB_Set_MemSconfig Base_addr[0x%x] \n", base_st_mem );
