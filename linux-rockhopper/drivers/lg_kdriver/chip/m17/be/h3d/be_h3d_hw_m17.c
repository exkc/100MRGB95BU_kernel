...
/*----------------------------------------------------------------------------------------
  Static Variables
  ----------------------------------------------------------------------------------------*/
static BE_H3D_HW_M17_INFO_T _gBeH3dHwM17Info = {
	.bT3CMode = FALSE,
};
...
/*========================================================================================
  ...
  ========================================================================================*/
int BE_H3D_HW_M17_Set_Mode(LX_BE_H3D_MODE_T *pstParams)
{
	int ret = RET_OK;
	...
	do{
		...
		{
			_gBeH3dHwM17Info.bT3CMode = (BOOLEAN)stHwParams.uT3CMode;
		}
	} while(0);
	return ret;
}

int BE_H3D_HW_M17_Get_Mode(LX_BE_H3D_MODE_T *pstParams)
{
	int ret = RET_OK;

	do{
		pstParams->bA3CMode = FALSE;
		pstParams->bT3CMode = _gBeH3dHwM17Info.bT3CMode;
	} while(0);
	return ret;
}

int BE_H3D_HW_M17_T3C_Depth_Level(LX_BE_H3D_T3C_DEPTH_LEVEL_T *pstParams)
{
	int ret = RET_OK;
	unsigned int status = 0;
...
int BE_H3D_HW_M17_Set_TridFmt( LX_BE_TRIDTV_FMT_CTRL_T *pstParams)
{
	int ret = RET_OK;
	unsigned int status = 0;
	...
	do{
		...
		switch (pstParams->lx_mode)
		{
			...
			default:
				BE_ERROR("lx_mode is invalied param for[M17]\n");
				BREAK_WRONG(-1);
				break;
		}
		...
		_gBeH3dHwM17Info.bT3CMode = (BOOLEAN)stHwParams1.uT3CMode;	
	} while(0);
	return ret;
}

int BE_H3D_HW_M17_ProcHandler(BE_H3D_PROC_ID_T proc_id, struct seq_file* m, void* data)
{
	switch(proc_id)
	{
