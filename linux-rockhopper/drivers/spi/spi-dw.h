...
struct dw_spi {
	...
	void __iomem		*regs;
	void __iomem		*reg_strength[2];  /* drive strength base*/
	u32			strength[4];	/* driver strength num */
	u32			rx_dly;
	int			nstrength;
	unsigned long		paddr;
	int			irq;
...
static inline void dw_spi_set_rx_dly(struct dw_spi *dws)
{
	dw_spi_enable_chip(dws, 0);
	dw_writel(dws, DW_SPI_RX_SAMPLE_DLY, dws->rx_dly );
...
static inline void   dw_spi_recovery_clk(struct dw_spi *dws)
{
	dw_spi_enable_chip(dws, 0);
	dw_spi_set_clk(dws, dws->clk_div);
	dw_spi_enable_chip(dws, 1);
}
