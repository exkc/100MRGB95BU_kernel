...
/*----------------------------------------------------------------------------------------
  Type Definitions
  ----------------------------------------------------------------------------------------*/
void o26_irb_funcInit(void);
int o26_irb_tx(LX_IRB_PARAM_T * param);
void o26_irb_funcReInit(void);
void o26_irb_exit(void);
...
int o26_irb_tx(LX_IRB_PARAM_T * param)
{
	...
	if( param == NULL || m2a_irq_base == NULL || a2m_ipc_base == NULL || a2m_irq_base == NULL || irb_key_base == NULL )
		return -1;

	writel(0, m2a_irq_base);

	/*  busy status is same in o26  checked by micom engineer 210428*/
	...
	if( param->numMakeTimes)
	{
		...
		for(i = 0; i < index ; i++)
		{
			...
			else if( (i > 1) && (midx > 0) && (i+1 < param->numMakeTimes) && pbuf[i] == 0 && pbuf[i+1] != 0 )
			{
	...
	/*  no done clear  in o26  checked by micom engineer 210428*/

	KDRV_IRB_DEBUG("o26_irb tx makenum[%x] pbuf[%x]\n",
					midx,pbuf[0]);
	if(mbuf)
		kfree(mbuf);
	return 0;
...
void o26_irb_funcReInit(void)
{
    ...
    KDRV_IRB_NOTI("o26_irb reinit\n");
}
void o26_irb_exit(void)
{
    KDRV_IRB_DEBUG("o26_irb end\n");
}


static irbs_o26_func_t o26_irb_func =
{
    .init = o26_irb_funcInit,
    .reinit = o26_irb_funcReInit,
    .tx = o26_irb_tx,
    .exit = o26_irb_exit,
};

irbs_o26_func_t* get_o26_irb_func(void)
{
    return &o26_irb_func;
}
