...
void FBDEV_O22_InitHWCtx(FBDEV_HW_CTX_T* hal_obj);

/*----------------------------------------------------------------------------------------
    External Variables
----------------------------------------------------------------------------------------*/
extern FBDEV_O22_HW_FUNC_T      g_fbdev_o22_cmn_hw_func;    /* O20 HW common func */

/*----------------------------------------------------------------------------------------
    global Functions
----------------------------------------------------------------------------------------*/
void FBDEV_O22_InitOSDZOrder(void);
LX_FBDEV_CFG_T* FBDEV_O22_GetCfg(void);

/*----------------------------------------------------------------------------------------
    global Variables
----------------------------------------------------------------------------------------*/
volatile OSD_O22_REG_T*         g_osd_o22_reg = NULL;
OSD_O22_REG_T*         g_osd_o22_reg_shadow = NULL;

volatile OSD_AFBC_O22_REG_T*    g_osd_afbc_o22_reg[2] = {NULL, NULL};
OSD_AFBC_O22_REG_T*    g_osd_afbc_o22_reg_shadow[2] = {NULL, NULL};

volatile OSD_SHP_O22_REG_T*     g_osd_shp_o22_reg[2] = {NULL, NULL};
OSD_SHP_O22_REG_T*     g_osd_shp_o22_reg_shadow[2] = {NULL, NULL};

volatile OSD_TOP_O22_REG_T*     g_osd_top_o22_reg = NULL;
OSD_TOP_O22_REG_T*     g_osd_top_o22_reg_shadow = NULL;

static FBDEV_O22_HW_CTX_T       g_fbdev_o22_hw_ctx;         /* O20 OSD H/W ctrl */

/*----------------------------------------------------------------------------------------
    Static Function Prototypes Declarations
----------------------------------------------------------------------------------------*/
static void FBDEV_O22_AutoDetectOSDHWValue(void);
...
{
    .GetCfg             = FBDEV_O22_GetCfg,
    .RunSuspend         = FBDEV_O22_CMN_RunSuspend,
    .RunResume          = FBDEV_O22_CMN_RunResume,

    .InitHW             = FBDEV_O22_InitHW,
    .ShutdownHW         = FBDEV_O22_ShutdownHW,
    .ExecHWCommand      = FBDEV_O22_CMN_ExecHWCommand,

    .SetOSDEnable       = FBDEV_O22_CMN_SetOSDEnable,
    .GetOSDEnable       = FBDEV_O22_CMN_GetOSDEnable,

    .InitOSDLayer       = FBDEV_O22_CMN_InitOSDLayer,
    .WriteOSDHeader     = FBDEV_O22_CMN_WriteOSDHeader,

    .GetZList           = FBDEV_O22_GetZList,
    .SetZList           = FBDEV_O22_SetZList,

    .WriteOSDEndian     = FBDEV_O22_WriteOSDEndian,
    .WriteOSDCSC        = FBDEV_O22_WriteOSDCSC,

    .GetInterruptStatus = FBDEV_O22_CMN_GetInterruptStatus,
    .SetInterruptClear  = FBDEV_O22_CMN_SetInterruptClear,
    .SetInterruptEnable = FBDEV_O22_CMN_SetInterruptEnable,

    .SetViewCtrl        = FBDEV_O22_CMN_SetViewCtrl,
    .SetConnCtrl        = FBDEV_O22_CMN_SetConnCtrl,

    .CreateProc         = FBDEV_O22_CreateProc,
    .ProcHandler        = FBDEV_O22_CMN_ProcHandler,
};
...
// this function is called before at O22 configure stage NOT at HW initialization stage.
void    FBDEV_O22_InitHWCtx(FBDEV_HW_CTX_T* hal_obj)
{
    int i;
    ...
    memcpy(&g_fbdev_o22_hw_ctx.func, &g_fbdev_o22_cmn_hw_func, sizeof(FBDEV_O22_HW_FUNC_T));

    for (i = 0; i < LX_FBDEV_ID_MAX; i++)
    {
        g_fbdev_o22_hw_ctx.conn_ctrl[i].conn_type   = g_fbdev_o22_hw_ctx.hw_conn_type;
        g_fbdev_o22_hw_ctx.fbc_ctrl[i].status       = FBDEV_O22_FBC_STATUS_NONE;
        g_fbdev_o22_hw_ctx.sr_ctrl[i].status        = FBDEV_O22_SR_STATUS_OFF;
        g_fbdev_o22_hw_ctx.sr_ctrl[i].coeff[0]      = 0x3FD;
        g_fbdev_o22_hw_ctx.sr_ctrl[i].coeff[1]      = 0x83;
        g_fbdev_o22_hw_ctx.sr_ctrl[i].coeff[2]      = 0x83;
        g_fbdev_o22_hw_ctx.sr_ctrl[i].coeff[3]      = 0x3FD;

        g_fbdev_o22_hw_ctx.sr_ctrl[i].ti_gain       = -1;
    }

    g_fbdev_o22_hw_ctx.mirror_ctrl.status       = FBDEV_O22_MIRROR_NONE;

    memset(&g_fbdev_o22_hw_ctx.ext_render_ctrl, 0x0, sizeof(FBDEV_O22_EXT_RENDER_CTRL_T));

    /* auto detect the default OSD connection and display size etc */
    FBDEV_O22_AutoDetectOSDHWValue();

    /* initialize hal object
     * after this function, all fbdev kdriver can get g_fbdev_o22_hw_ctx via g_fbdev_hw_ctx->chip_priv_ctx
     */
    memcpy(hal_obj, &_g_fbdev_hw_ctx, sizeof(FBDEV_HW_CTX_T));
    hal_obj->chip_priv_ctx = &g_fbdev_o22_hw_ctx;
}
...
static void FBDEV_O22_AutoDetectOSDHWValue(void)
{
    ...
    UINT32  osd_panel_val       = LX_MAKE_RES(3840, 2160); /* default 4K resolution */
    UINT32  osd_conn_val        = LX_FBDEV_CONN_OSD_VIDEO_BLEND;/* default path */
    UINT32  osd_pulse_bosd_val  =
        0xA000;                       /* default pulse timer delay. default 0xA000. see SICDTV-7971 */
    ...
    FBDEV_O22_HW_CTX_T* hw_ctx = &g_fbdev_o22_hw_ctx;
    ...
    /* read HW configuration by parsing some kernel parameters */
    {
        ...
        {
            hw_ctx->b_support_ext_frc = 0;
#ifdef FBDEV_O22_SUPPORT_4K_ESTREAMER
            hw_ctx->b_support_fb1 = 1;
#else
        ...
        if (lx_chip_plt() & LX_CHIP_PLT_QUHD)
        {
            ...
            osd_panel_val     = LX_MAKE_RES(7680, 4320);
        }
        ...
        else if (lx_chip_plt() & LX_CHIP_PLT_HD)
        {
            ...
            osd_panel_val     = LX_MAKE_RES(1368, 768);
        }
        else
        {
    ...
    hw_ctx->hw_panel_width      = LX_RES_GET_WIDTH(osd_panel_val);
    hw_ctx->hw_panel_height     = LX_RES_GET_HEIGHT(osd_panel_val);
    ...
    hw_ctx->hw_wa_flag              |= FBDEV_O22_HW_WA_1X_CSR_DISP_ON_BLEND_PATH;
...
int FBDEV_O22_InitHW(void)
{
    ...
    /* ioremap TOP regs */
    if (NULL == g_osd_top_o22_reg)
    {
        g_osd_top_o22_reg = (OSD_TOP_O22_REG_T*)ioremap(O22_CCO_TOP_CTRL_BASE, sizeof(OSD_TOP_O22_REG_T));
    }

    FBDEV_CHECK_CODE(NULL == g_osd_top_o22_reg, goto func_exit, "out of mem\n");

    if (NULL == g_osd_top_o22_reg_shadow)
    {
        g_osd_top_o22_reg_shadow = (OSD_TOP_O22_REG_T*)OS_Malloc(sizeof(OSD_TOP_O22_REG_T));
    }

    FBDEV_CHECK_CODE(NULL == g_osd_top_o22_reg_shadow, goto func_exit, "out of mem\n");

    /* ioremap OSD regs */
    if (NULL == g_osd_o22_reg)
    {
        g_osd_o22_reg = (OSD_O22_REG_T*)ioremap(O22_CCO_BASE, sizeof(OSD_O22_REG_T));
    }

    FBDEV_CHECK_CODE(NULL == g_osd_o22_reg, goto func_exit, "out of mem\n");

    if (NULL == g_osd_o22_reg_shadow)
    {
        g_osd_o22_reg_shadow = (OSD_O22_REG_T*)OS_Malloc(sizeof(OSD_O22_REG_T));
    }

    FBDEV_CHECK_CODE(NULL == g_osd_o22_reg_shadow, return RET_ERROR, "out of mem\n");

    /* ioremap OSD_SHP(0) regs */
    if (NULL == g_osd_shp_o22_reg[0])
    {
        g_osd_shp_o22_reg[0] = (OSD_SHP_O22_REG_T*)ioremap(O22_CCO_SHP0_BASE, sizeof(OSD_SHP_O22_REG_T));
    }

    FBDEV_CHECK_CODE(NULL == g_osd_shp_o22_reg[0], goto func_exit, "out of mem\n");

    if (NULL == g_osd_shp_o22_reg_shadow[0])
    {
        g_osd_shp_o22_reg_shadow[0] = (OSD_SHP_O22_REG_T*)OS_Malloc(sizeof(OSD_SHP_O22_REG_T));
    }

    FBDEV_CHECK_CODE(NULL == g_osd_shp_o22_reg_shadow[0], return RET_ERROR, "out of mem\n");

    /* ioremap OSD_SHP(1) regs */
    if (NULL == g_osd_shp_o22_reg[1])
    {
        g_osd_shp_o22_reg[1] = (OSD_SHP_O22_REG_T*)ioremap(O22_CCO_SHP1_BASE, sizeof(OSD_SHP_O22_REG_T));
    }

    FBDEV_CHECK_CODE(NULL == g_osd_shp_o22_reg[1], goto func_exit, "out of mem\n");

    if (NULL == g_osd_shp_o22_reg_shadow[1])
    {
        g_osd_shp_o22_reg_shadow[1] = (OSD_SHP_O22_REG_T*)OS_Malloc(sizeof(OSD_SHP_O22_REG_T));
    }

    FBDEV_CHECK_CODE(NULL == g_osd_shp_o22_reg_shadow[1], return RET_ERROR, "out of mem\n");

    /* ioremap OSD_AFBC(0) regs */
    if (NULL == g_osd_afbc_o22_reg[0])
    {
        g_osd_afbc_o22_reg[0] = (OSD_AFBC_O22_REG_T*)ioremap(O22_CCO_AFBC0_BASE, sizeof(OSD_AFBC_O22_REG_T));
    }

    FBDEV_CHECK_CODE(NULL == g_osd_afbc_o22_reg[0], goto func_exit, "out of mem\n");

    if (NULL == g_osd_afbc_o22_reg_shadow[0])
    {
        g_osd_afbc_o22_reg_shadow[0] = (OSD_AFBC_O22_REG_T*)OS_Malloc(sizeof(OSD_AFBC_O22_REG_T));
    }

    FBDEV_CHECK_CODE(NULL == g_osd_afbc_o22_reg_shadow[0], return RET_ERROR, "out of mem\n");

    /* ioremap OSD_AFBC(1) regs */
    if (NULL == g_osd_afbc_o22_reg[1])
    {
        g_osd_afbc_o22_reg[1] = (OSD_AFBC_O22_REG_T*)ioremap(O22_CCO_AFBC1_BASE, sizeof(OSD_AFBC_O22_REG_T));
    }

    FBDEV_CHECK_CODE(NULL == g_osd_afbc_o22_reg[1], goto func_exit, "out of mem\n");

    if (NULL == g_osd_afbc_o22_reg_shadow[1])
    {
        g_osd_afbc_o22_reg_shadow[1] = (OSD_AFBC_O22_REG_T*)OS_Malloc(sizeof(OSD_AFBC_O22_REG_T));
    }

    FBDEV_CHECK_CODE(NULL == g_osd_afbc_o22_reg_shadow[1], return RET_ERROR, "out of mem\n");

    OSD_TOP_O22_RdFL(ctrl_spath);
    ...
    {
        OSD_TOP_O22_Wr(ctrl_auto_init, 0x55555555);
        OSD_TOP_O22_Wr(ctrl_intr_pulse, 0xCA876420);

        OSD_TOP_O22_Wr(ctrl_spath, 0x00000D80);
    }
    else /* SEP */
    {
        OSD_TOP_O22_Wr(ctrl_auto_init, 0x55555555);

        OSD_TOP_O22_Wr(ctrl_intr_pulse, 0xCAC76420);

        /* (2021/05/27)
         * O22 S.OSD sync is fetched from VSD PIP.
         ...
         *
         * O20 대비 O22 의 큰 차이점이므로 주의할것
         */
        OSD_TOP_O22_Wr(ctrl_spath, 0x00000D80);
#if 0
        FBDEV_WARN("++ 2k s.osd sync_gen\n");
        OSD_TOP_O22_Wr(osd_sync_gen_ctrl0, 0x00040005);
        OSD_TOP_O22_Wr(osd_sync_gen_ctrl1, 0x00240438);
        OSD_TOP_O22_Wr(osd_sync_gen_ctrl2, 0x0058002C);
        OSD_TOP_O22_Wr(osd_sync_gen_ctrl3, 0x00940780);
        OSD_TOP_O22_Wr(osd_sync_gen_ctrl4, 0x00040005);
        OSD_TOP_O22_Wr(osd_sync_gen_ctrl5, 0x00240438);
        OSD_TOP_O22_Wr(osd_sync_gen_ctrl6, 0x0001006F);
        OSD_TOP_O22_Wr(osd_sync_gen_ctrl7, 0x00080058);
#endif

        OSD_TOP_O22_Wr01(ctrl_dpath, sel_osd_src_sosd, 0x3);
    }
    ...
    if (FBDEV_O22_CMN_GetHWStatus(FBDEV_O22_HW_STATUS_AFBC, LX_FBDEV_ID_OSD0))
    {
    ...
    if (FBDEV_O22_CMN_GetHWStatus(FBDEV_O22_HW_STATUS_AFBC, LX_FBDEV_ID_OSD1) &&
    ...
    OSD_TOP_O22_WrFL(ctrl_auto_init);
    OSD_TOP_O22_WrFL(ctrl_intr_pulse);

    OSD_TOP_O22_WrFL(ctrl_dpath);
    OSD_TOP_O22_WrFL(ctrl_spath);
    ...
    {
        OSD_TOP_O22_WrFL(osd_sync_gen_ctrl0);
        OSD_TOP_O22_WrFL(osd_sync_gen_ctrl1);
        OSD_TOP_O22_WrFL(osd_sync_gen_ctrl2);
        OSD_TOP_O22_WrFL(osd_sync_gen_ctrl3);
        OSD_TOP_O22_WrFL(osd_sync_gen_ctrl4);
        OSD_TOP_O22_WrFL(osd_sync_gen_ctrl5);
        OSD_TOP_O22_WrFL(osd_sync_gen_ctrl6);
        OSD_TOP_O22_WrFL(osd_sync_gen_ctrl7);
    }
    ...
    if (FBDEV_O22_CMN_IsUHDDispMode())
    {
         ...
         */
        OSD_TOP_O22_Wr(pic_init_g0_timer, 0x70000000);
        OSD_TOP_O22_Wr(pic_init_g1_timer, 0x70000000);
        OSD_TOP_O22_Wr(pic_init_g2_timer, 0x70000000);
        OSD_TOP_O22_Wr(pic_init_g3_timer, 0x70000000);
        OSD_TOP_O22_Wr(pic_init_g4_timer, 0x70000000);
        OSD_TOP_O22_Wr(pic_init_g5_timer, 0x70000000);
        OSD_TOP_O22_Wr(pic_init_g6_timer, 0x70000000);
        OSD_TOP_O22_Wr(pic_init_g7_timer, 0x70000000);

        OSD_TOP_O22_WrFL(pic_init_g0_timer);
        OSD_TOP_O22_WrFL(pic_init_g1_timer);
        OSD_TOP_O22_WrFL(pic_init_g2_timer);
        OSD_TOP_O22_WrFL(pic_init_g3_timer);
        OSD_TOP_O22_WrFL(pic_init_g4_timer);
        OSD_TOP_O22_WrFL(pic_init_g5_timer);
        OSD_TOP_O22_WrFL(pic_init_g6_timer);
        OSD_TOP_O22_WrFL(pic_init_g7_timer);
    }
    else
    {
        FBDEV_WARN("++ non-UHD mode : pic_init_timer config (not tested yet)\n");
        OSD_TOP_O22_Wr(pic_init_g0_timer, 0x00000000);
        OSD_TOP_O22_Wr(pic_init_g1_timer, 0x00000000);
        OSD_TOP_O22_Wr(pic_init_g2_timer, 0x00000000);
        OSD_TOP_O22_Wr(pic_init_g3_timer, 0x00000000);
        OSD_TOP_O22_Wr(pic_init_g4_timer, 0x00000000);
        OSD_TOP_O22_Wr(pic_init_g5_timer, 0x00000000);
        OSD_TOP_O22_Wr(pic_init_g6_timer, 0x00000000);
        OSD_TOP_O22_Wr(pic_init_g7_timer, 0x00000000);
     ...
     */
    OS_WrReg(O22_CCO_MIF0_BASE + 0x20, 0x400000FB);
    OS_WrReg(O22_CCO_MIF0_BASE + 0x60, 0x400000FA);
    OS_WrReg(O22_CCO_MIF1_BASE + 0x20, 0x400000FB);

    // (2021/11/01) O22-to-F22 OSD change flag configuration
    //
    ...
    //
    OS_WrReg(O22_CCO_TOP_CTRL1_BASE + 0x18, 0xFF0101);

    /* HW revision specific initialization */
    FBDEV_O22_InitOSDZOrder();

    FBDEV_O22_CMN_InitHW();
...
int FBDEV_O22_SetZList(LX_FBDEV_ZLIST_T* zList)
{

    return RET_OK;
}

int FBDEV_O22_GetZList(LX_FBDEV_ZLIST_T* zList)
{
    zList->order[0] = 0;
    zList->order[1] = 1;
    zList->order[2] = 2;
...
int FBDEV_O22_WriteOSDEndian(int fb_dev_id, ENDIAN_INFORM_T* endian)
{
    return RET_OK;
}

int FBDEV_O22_WriteOSDCSC(int layer, CSC_INFORM_T* csc)
{
    return RET_OK;
}
