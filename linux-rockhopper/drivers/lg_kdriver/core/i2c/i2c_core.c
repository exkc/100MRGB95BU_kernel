...
#define I2C_CHECK_INITED(h)												\
	do{																	\
		...
		if (!h->inited )												\
		{																\
			I2C_PRINT_ERROR("%s is not inited \n", h->name);			\
			return -1;													\
		}																\
	} while(0)
...
#define I2C_CHECK_PARAM(subAddrSize, bufSize) 											\
	do{																					\
		...
		{																				\
			I2C_PRINT_ERROR("Invalid parameter : SubAddrLen[%d], BufferLen[%d]\n",		\
				subAddrSize, bufSize );													\
			return -1;																	\
		}																				\
	} while(0)
...
#define I2C_MUTEX_LOCK(h)							\
	do{												\
		if(h->func.lock(h) < 0)						\
		{											\
			I2C_PRINT_ERROR("Mutex Lock Fail\n");	\
			return -1;								\
		}											\
	}while(0)
#define I2C_MUTEX_UNLOCK(h)							\
	do{ 											\
		...
		{											\
			I2C_PRINT_ERROR("Mutex Unlock Fail\n");	\
		}											\
	}while(0)


#define I2C_REG_WRITE(h, reg, value)		(writel(value, reg(h->reg_base)))
#define I2C_REG_READ(h, reg)				(readl(reg(h->reg_base)))
...
{
	int rc;
	rc = wait_for_completion_timeout( &h->irq_completion, msecs_to_jiffies(timeout));
...
int I2C_DevSuspendAll(void)
{
	...
	{
		void * reg_addr = _pI2cHw[i16Count].reg_base;
...
{
	h->reg_base = (void *)ioremap(_pI2cConfig->hw_map[h->ch].reg_addr, I2C_REG_SIZE);
	_I2C_Reset(h);
	...
	if(h->use_irq)
	{
		...
		if(lx_i2c_cfg() & LX_I2C_CFG_LEGACY_MODE)
		{
			...
			if(request_irq( h->irq_num, _I2C_IrqHandler, (unsigned long)0, h->irq_name, h))
		#else
...
static int _I2C_HwWrite(I2C_HW_HANDLE h, LX_I2C_RW_DATA_T *data)
{
	...
	else /* KERNEL_API */
	{
		...
		adap = i2c_get_adapter(h->ch);

		if (!adap)
		return -ENODEV;

		sub_addr_size = data->subAddrSize ;
		data_size =	data->bufSize ;

		msg	[0].flags = 0;
		msg[0].addr = (data->slaveAddr >> 1) & 0x7f;    /* I2C address of chip */
		msg[0].len = sub_addr_size + data_size;

		if(sub_addr_size + data_size > SBUF_SIZE)
		{
			mbuf = (UINT8*)OS_Malloc(sub_addr_size + data_size);
			if(mbuf == NULL) return -ENOMEM;
			msg[0].buf = (void __force *)mbuf;
		}
		else
			msg[0].buf = (void __force *)sbuf;


		memcpy((__u8 *)msg[0].buf,data->subAddr,sub_addr_size);
		memcpy((__u8 *)(msg[0].buf + sub_addr_size) ,data->buf,data_size);

		res = i2c_transfer(adap, msg, 1);

		i2c_put_adapter(adap);

		res = (res >= 0) ? data->bufSize :res;
	...
	if(res != data->bufSize)
		I2C_DEBUG("ch[%d] slave[0x%x] check res[%d] \n",h->ch,data->slaveAddr,res);
	HW_END_XMIT(h,flag);
...
static int _I2C_HwRead(I2C_HW_HANDLE h, LX_I2C_RW_DATA_T *data)
{
	...
	else
	{
		...
		if(data_size > SBUF_SIZE)
		{
			mbuf = (UINT8*)OS_Malloc(data_size);
			if(mbuf == NULL) return -ENOMEM;
			msg[msg_num_index].buf = (void __force *)mbuf;
		}
		else
			msg[msg_num_index].buf = (void __force *)databuf;

		res = i2c_transfer(adap, msg, msg_num_index+1);

		i2c_put_adapter(adap);

		if (res >= 0)
		{
		memcpy(data->buf ,(__u8 *)(msg[msg_num_index].buf),data_size);
		res = data_size;
		}
	}
#else
...
int I2C_DevTransfer(LX_I2C_DEV_HANDLE dev, LX_I2C_RW_DATA_T *data, UINT8 mode)
{
	...
	dbgch = (debug_ch_devno >> 8) & 0xff;
    dbgdev = debug_ch_devno & 0xff;

	if(dbgch != 0 && dbgch == (h->ch +1))
		I2C_PRINT("[i2c:%d][%2x][%s]\n",dbgch,dbgdev,(mode)?"RD":"WR");
