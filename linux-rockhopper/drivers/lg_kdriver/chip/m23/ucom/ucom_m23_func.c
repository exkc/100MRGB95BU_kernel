...
#ifdef USE_IPC_INTR
ucom_m23_data_t gm23_udata;
static struct completion  m23_micom_completion;
#endif
...
#define M23_UCOM_STATUS_PRINTNUM	3
...
static void __iomem *m23_ipc_disable = NULL;
void __iomem *m23_micom_eeprom_base = NULL;
static void __iomem *m23_chipreset_base = NULL;
...
void m23_ucom_FuncInit(void);
void m23_ucom_FuncChipReset(void);
int m23_ucom_FuncWrite(LX_UCOM_PARAM_T * param);
int m23_ucom_FuncRead(LX_UCOM_PARAM_T * param);
int m23_ucom_FuncGetStatus(void );
void m23_set_micom_disable(unsigned int disable);

unsigned int m23_get_micom_disable(void)
{
	unsigned int reg = 0;

	if(m23_ipc_disable == NULL)
		m23_ipc_disable = ioremap(0xF3514108, 4);

	reg = readl(m23_ipc_disable);

	return reg;
	return 0;
}
EXPORT_SYMBOL(m23_get_micom_disable) ;
...
void m23_set_micom_disable(unsigned int disable)
{
	...
	writel(disable, m23_ipc_disable);
}
EXPORT_SYMBOL(m23_set_micom_disable) ;
#if 0
...
void m23_ucom_FuncInit(void)
{
	...
	m23_chipreset_base = ioremap( 0xFD3004C8,4 );

	#if 0
	m23_micom_eeprom_base = ioremap( 0xF3082104,4 );
	...
	#ifdef USE_IPC_INTR
	gm23_udata.irq = 135 ;//103 +32 M23_CPU_interrupt_list_v0.1.xlsx

	init_completion(&m23_micom_completion);

	if(request_irq( gm23_udata.irq, _ucom_irqhandler, (unsigned long)0, "ucom0", &gm23_udata))
	{
...
void m23_ucom_FuncChipReset(void)
{
	...
	if(m23_chipreset_base != NULL)
		writel(0x1 , m23_chipreset_base );
}
...
static int _ucom_request(u32 key, u32 flag)
{
	...
	if(flag != UCOM_FAKE_CMD )
	{
		...
		if( wait_for_completion_timeout(&m23_micom_completion, msecs_to_jiffies(15)) < 0)
		{
			...
			cur = sched_clock();
			if(ucom_abnormal < M23_UCOM_STATUS_PRINTNUM){
		...
		#else
		/* clear possible unhandled pending interrupts: requested by micom fw */
		writel(0, m2a_irq_base);
		/* wake M to handle the request */
		writel(0x1, a2m_irq_base);
		start = sched_clock();
...
int m23_ucom_FuncWrite(LX_UCOM_PARAM_T * param)
{
	...
	if(ret < 0)
	{
		KDRV_UCOM_DEBUG("ukdrv WCMD[%s:%x]size[%x]key[%x] ret[%x]\n",get_cmd_string(param->Cmd) ,param->Cmd,param->bufSize,key,ret);
		return ret;
	}
...
int m23_ucom_FuncRead(LX_UCOM_PARAM_T * param)
{
	...
	if(ret < 0)
	{
		KDRV_UCOM_DEBUG("ukdrv RCMD[%s:%x]size[%x]key[%x]ret[%x]\n",get_cmd_string(param->Cmd) ,param->Cmd,param->bufSize,key,ret);
		return ret;
	}
...
static ucom_m23_func_t m23_ucom_func =
{
	.init = m23_ucom_FuncInit,
	.chipreset = m23_ucom_FuncChipReset,
	.write = m23_ucom_FuncWrite,
	.read = m23_ucom_FuncRead,
	.getstatus= m23_ucom_FuncGetStatus,

};

ucom_m23_func_t* get_m23_ucom_func(void)
{
	return &m23_ucom_func;
}
