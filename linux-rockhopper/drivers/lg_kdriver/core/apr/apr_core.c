...
{
	E_APR_FMT_DECIDED = 0,
	E_APR_FMT_NOT_DECIDED = 1
} E_APR_FMT_DECISION_RET;
...
static E_APR_FMT_DECISION_RET APR_HistDiffMeasure(UINT32 lr_dif,UINT32 tb_dif,UINT32 lr_prev_dif,UINT32 tb_prev_dif);
static void APR_DeltaMeasure(UINT32 lr_dif,UINT32 tb_dif);
static void APR_GetCapbility(APR_CFG_T *io_pstAprCfg);
static void APR_GetThrForSrcType(APR_THR_T  **p_default_threshold);
...
#endif
static int APR_CaptureFrame3dMode(APR_OV_INFO_T *ov_info);
...
/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
LX_APR_CMD _g_apr_trace = LX_APR_CMD_OFF;	//default should be off.

#if 0
static	UINT32  thr_adj = 0;
UINT32  tmp_thr3 = 0, tmp_thr4 = 0, tmp_thr6 = 0, tmp_thr7 = 0;
UINT32  tmp_thr_const0 = 0, tmp_thr_const1 = 0, tmp_thr_const2 = 0, tmp_thr_const3 = 0;
#endif

APR_THR_TMP_T _gsTmpThrValues = {0,};
...
static E_APR_FMT_DECISION_RET APR_HistDiffMeasure(UINT32 lr_dif,UINT32 tb_dif,UINT32 lr_prev_dif,UINT32 tb_prev_dif)
{
	E_APR_FMT_DECISION_RET eRet= E_APR_FMT_NOT_DECIDED;
	UINT32 thr3, thr4, thr6, thr7;
	UINT32 thr_const0, thr_const1, thr_const2, thr_const3;
	APR_CFG_T *pCfg = &apr_info.cfg;
	
	thr3   = _gsTmpThrValues.tmp_thr3;
	thr4   = _gsTmpThrValues.tmp_thr4;
	thr6   = _gsTmpThrValues.tmp_thr6;
	thr7   = _gsTmpThrValues.tmp_thr7;
	thr_const0 = _gsTmpThrValues.tmp_thr_const0;
	thr_const1 = _gsTmpThrValues.tmp_thr_const1;
	thr_const2 = _gsTmpThrValues.tmp_thr_const2;
	thr_const3 = _gsTmpThrValues.tmp_thr_const3;
	...
	{
		if((lr_prev_dif >= thr6) || (tb_dif > APR_MAX_HISTOGRAM_ON_CASE3))
		{
			pCfg->tmp_fmt = LX_APR_FMT_2D_2D;
		}
		if(pCfg->tmp_fmt == LX_APR_FMT_2D_2D)
	...
	{
		if((tb_prev_dif >= thr6) || (lr_dif > APR_MAX_HISTOGRAM_ON_CASE3))
		{
			pCfg->tmp_fmt = LX_APR_FMT_2D_2D;
		}
		if(pCfg->tmp_fmt == LX_APR_FMT_2D_2D)
	...
	do {
		...
		switch(pstParams->cmd)
		{
			...
			case LX_APR_HCMD_GET_INTERNAL_STATE :
				APR_DumpCurrentConfig(&apr_info);
				break;
...
static int APR_CheckConfig(void)
{
	...
	do {
		if(cur_src_type != pCfg->src_type)
		{
			...
			APR_GetThrForSrcType(&p_default_threshold);
...
static int APR_CaptureFrame(void)
{
	...
	do {
		...
		if(pCfg->cap.enb == LX_APR_CAP_DRW3) // for 3D image capure
		{
		...
		switch(pCfg->out_type)
		{
			...
			case LX_PE_OUT_2D_3D :
				if(pCfg->fd3.enb)
				{
					APR_CheckTime("Capture line", &apr_info, APR_GET_TIME_CAP, APR_GET_START);
					...
					APR_CheckTime("Capture line",&apr_info, APR_GET_TIME_CAP, APR_GET_END);
				}
				else
				{
					APR_CheckTime("Capture line",&apr_info, APR_GET_TIME_CAP, APR_GET_START);
					...
					APR_CheckTime("Capture line",&apr_info, APR_GET_TIME_CAP, APR_GET_END);
					APR_CheckTime("Measure difference",&apr_info, APR_GET_TIME_MAT, APR_GET_START);
					ret = pCb_ldf->d3d(pCapture);
					APR_CheckTime("Measure difference",&apr_info, APR_GET_TIME_MAT, APR_GET_END);
				}
				break;
			case LX_PE_OUT_3D_2D :
			case LX_PE_OUT_FS :
			...
			case LX_PE_OUT_DUAL_TB :
			case LX_PE_OUT_DUAL_SS :
				ret=APR_CaptureFrame3dMode(&ov_info);
				break;
			default:
				break;
		}
...
static int APR_ProcessFD3(void)
{
	...
	do {
		switch(pCfg->out_type)
		{
			...
			case LX_PE_OUT_PIP :
			default :
				APR_CheckTime("Measure fd3",&apr_info, APR_GET_TIME_FD3, APR_GET_START);
				ret = pCb_fd3->fd3();
				APR_CheckTime("Measure fd3",&apr_info, APR_GET_TIME_FD3, APR_GET_END);
				break;
		}
	} while(0);
#endif
...
*
*/
static int APR_CaptureFrame3dMode(APR_OV_INFO_T *i_pstOvInfo)
{
	int ret = RET_OK;
	APR_CFG_T *pCfg = &apr_info.cfg;
	//APR_OV_INFO_T ov_info;
	...
	APR_CheckTime("Capture line",&apr_info, APR_GET_TIME_CAP, APR_GET_START);
	ret = APR_GetOVInfo(APR_CAP_SRC_L_OVR, i_pstOvInfo);
	if (ret) goto func_exit;
	...
	ret = APR_GetOVInfo(APR_CAP_SRC_R_OVR, i_pstOvInfo);
	if (ret)  goto func_exit;
...
static int APR_DecideFormatByLRCHST(void)
{
	...
	do {
		...
		_gsTmpThrValues.thr_adj = (pData->hist_lrc.tb_ratio << 10)/(1036800);	//1920x540=1036800

		if(pStm->win_pos == APR_WIN_LRTB_FULL)
		{
			_gsTmpThrValues.tmp_thr3   = (pThr->fmt_th3 * _gsTmpThrValues.thr_adj)>>10;
			_gsTmpThrValues.tmp_thr4   = (pThr->fmt_th4 * _gsTmpThrValues.thr_adj)>>10;
			_gsTmpThrValues.tmp_thr6   = (pThr->fmt_th6 * _gsTmpThrValues.thr_adj)>>10;
			_gsTmpThrValues.tmp_thr7   = (pThr->fmt_th7 * _gsTmpThrValues.thr_adj)>>10;
			_gsTmpThrValues.tmp_thr_const0 = (2000 * _gsTmpThrValues.thr_adj)>>10;
			_gsTmpThrValues.tmp_thr_const1 = (5000 * _gsTmpThrValues.thr_adj)>>10;
			_gsTmpThrValues.tmp_thr_const2 = (200 * _gsTmpThrValues.thr_adj)>>10;
			_gsTmpThrValues.tmp_thr_const3 = (70 * _gsTmpThrValues.thr_adj)>>10;
		}
		else if(pStm->win_pos == APR_WIN_LRTB_HAF1 || \
				pStm->win_pos == APR_WIN_LRTB_HAF2 || \
				pStm->win_pos == APR_WIN_LRTB_MAX)
		{
			_gsTmpThrValues.tmp_thr3   = ( (pThr->fmt_th3 * _gsTmpThrValues.thr_adj) >>1)>>10;
			_gsTmpThrValues.tmp_thr4   = ( (pThr->fmt_th4 * _gsTmpThrValues.thr_adj) >>1)>>10;
			_gsTmpThrValues.tmp_thr6   = ( (pThr->fmt_th6 * _gsTmpThrValues.thr_adj) >>1)>>10;
			_gsTmpThrValues.tmp_thr7   = ( (pThr->fmt_th7 * _gsTmpThrValues.thr_adj) >>1)>>10;
			_gsTmpThrValues.tmp_thr_const0 = ( (2000 * _gsTmpThrValues.thr_adj) >>1)>>10;
			_gsTmpThrValues.tmp_thr_const1 = ( (5000 * _gsTmpThrValues.thr_adj) >>1)>>10;
			_gsTmpThrValues.tmp_thr_const2 = ( (200 * _gsTmpThrValues.thr_adj) >>1)>>10;
			_gsTmpThrValues.tmp_thr_const3 = ( (70 * _gsTmpThrValues.thr_adj) >>1)>>10;
		}
		...
		if((pCfg->out_type == LX_PE_OUT_2D) || (pCfg->out_type == LX_PE_OUT_2D_3D))
		{
			...
			if(APR_HistDiffMeasure(lr_dif,tb_dif,lr_prev_dif,tb_prev_dif) == E_APR_FMT_DECIDED)
			{
				goto LABEL_DECIDE_FMT;
			}

			// Delta measure
			APR_DeltaMeasure(lr_dif,tb_dif);
		}
...
static int APR_DetectHandler(void)
{
	...
	do {
		...
		if (ret) break;
		ret = APR_DisplayResult(&apr_info,&_gsTmpThrValues);
	} while(0);
