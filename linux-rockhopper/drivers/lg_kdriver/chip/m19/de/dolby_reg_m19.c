...
UINT32 gForceMDPath_M19 = 0;

/*----------------------------------------------------------------------------------------
 *	 Static Function Prototypes Declarations
 *---------------------------------------------------------------------------------------*/
static int DE_DHDR_IP_REG_M19_InitDevHandler(DOLBY_DEVICE_DATA_T *o_PtrDolbyInit);
static int DE_DHDR_IP_REG_M19_Phy2Virt(DOLBY_DEVICE_DATA_T *o_PtrDolbyInit);
...
static int DE_DHDR_IP_REG_M19_InitDevHandler(DOLBY_DEVICE_DATA_T *o_PtrDolbyInit)
{
	...
	do {
		...
		o_PtrDolbyInit->sCompDMRegMap.unHwAddr			= DOLBY_HAL_M19_CTRL_ADDR;
		o_PtrDolbyInit->sCompDMRegMap.unSize			= DOLBY_HAL_M19_CTRL_SIZE;
		o_PtrDolbyInit->sCompDMRegMap.unPtrVirtAddr		= NULL;
		if(gForceMDPath_M19 == 0){
			o_PtrDolbyInit->sMDRegMap.unHwAddr		= DOLBY_HAL_M19A0_HDMI_MD_ADDR;
		}
		o_PtrDolbyInit->sMDRegMap.unSize				= DOLBY_HAL_M19_HDMI_TOTMD_SIZE;
		o_PtrDolbyInit->sMDRegMap.unPtrVirtAddr			= NULL;

		o_PtrDolbyInit->sOTTMDRegMap.unHwAddr			= DOLBY_HAL_M19_OTTMD_DDR_ADDR;
		o_PtrDolbyInit->sOTTMDRegMap.unSize				= DOLBY_HAL_M19_OTTMD_DDR_SIZE;
		o_PtrDolbyInit->sOTTMDRegMap.unPtrVirtAddr		= NULL;

		o_PtrDolbyInit->sLUTDDRRegMap.unHwAddr			= DOLBY_HAL_M19_LUT_DDR_ADDR;
		o_PtrDolbyInit->sLUTDDRRegMap.unSize			= DOLBY_HAL_M19_LUT_DDR_SIZE;
		o_PtrDolbyInit->sLUTDDRRegMap.unPtrVirtAddr		= NULL;

		o_PtrDolbyInit->sCompDMDDRRegMap.unHwAddr		= DOLBY_HAL_M19_IP_CONFIG_DDR_ADDR;
		o_PtrDolbyInit->sCompDMDDRRegMap.unSize			= DOLBY_HAL_M19_IP_CONFIG_DDR_SIZE;
		o_PtrDolbyInit->sCompDMDDRRegMap.unPtrVirtAddr	= NULL;

		o_PtrDolbyInit->sFrameIdxRegMap.unHwAddr		= DOLBY_HAL_M19_FRAME_INDEX_BASE;
		o_PtrDolbyInit->sFrameIdxRegMap.unSize			= DOLBY_HAL_M19_FRAME_INDEX_SIZE;
		o_PtrDolbyInit->sFrameIdxRegMap.unPtrVirtAddr	= NULL;

		o_PtrDolbyInit->sDoviDDRMap.unHwAddr			= HDR_BASE_ADDRESS;
		if(lx_chip_rev() >= LX_CHIP_REV(M19,C0))
			o_PtrDolbyInit->sDoviDDRMap.unSize			= VIDEO_M19_HDR_IPC_MEM_SIZE_C0;
		else
			o_PtrDolbyInit->sDoviDDRMap.unSize			= VIDEO_M19_HDR_IPC_MEM_SIZE;

		o_PtrDolbyInit->sDoviDDRMap.unPtrVirtAddr		= NULL;

		o_PtrDolbyInit->eDolbyDrvState 					= DOLBY_UNINTIALIZED;
		o_PtrDolbyInit->unFrameNum						= 0;
		o_PtrDolbyInit->unHDRIntrIrqNum					= DOLBY_HAL_M19_HDRINTRIRQ_NUMBER;
		o_PtrDolbyInit->unMCUIntrIrqNum					= DOLBY_HAL_M19_MCUIRQ_NUMBER;
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_M19_Init(DOLBY_DEVICE_DATA_T *io_PtrDolbyInit)
{
	int ret = RET_ERROR;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
	...
	do{
		...
		ret = DE_DHDR_IP_REG_M19_InitDevHandler(io_PtrDolbyInit);
		if(ret != RET_OK) {
			DE_ERROR("Error: Init Dev Handler Failed\n");
			break;
		}

		ret = DE_DHDR_IP_REG_M19_Phy2Virt(io_PtrDolbyInit);
	}while(0);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_M19_GetIrqStatus(DOLBY_DEVICE_DATA_T *i_PtrDolby,UINT32 *o_PtrIrqStatus,int i_unFlag)
{
	int ret = RET_OK;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_M19_SetIrqStatus(DOLBY_DEVICE_DATA_T *i_PtrDolby,UINT32 *o_PtrIrqStatus,int i_unFlag)
{
	int ret = RET_OK;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_M19_GetHDMIMetaData(DOLBY_DEVICE_DATA_T *i_sptrDolby,LX_DOLBY_DM_MD_T *o_sptrMetaData)
{
	int ret = RET_ERROR;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
	...
	do{
		...
		if(i_sptrDolby->sMDRegMap.unPtrVirtAddr != NULL) {
			...
			memcpy((void *)o_sptrMetaData->ucDMMetaData,(void *)i_sptrDolby->sMDRegMap.unPtrVirtAddr,DOLBY_HAL_M19_HDMI_TOTMD_SIZE);
			o_sptrMetaData->usSize = DOLBY_HAL_M19_HDMI_TOTMD_SIZE;
		}
		else {
...
int DE_DHDR_IP_REG_M19_GetOTTMetaData(DOLBY_DEVICE_DATA_T *i_PtrDolby,LX_DOLBY_DM_MD_T *o_ptrOTTDMMD,UINT32 *io_nptrFramePTS,UINT32 *io_sptrFrames,UINT32 *o_sptrProfileId)
{
	int ret = RET_OK;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_M19_SetConfig(DOLBY_DEVICE_DATA_T *i_PtrDolby,LX_DOLBY_CONFIG_REG_T *i_PtrConfig)
{
	int ret = RET_OK;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_M19_GetConfig(DOLBY_DEVICE_DATA_T *i_PtrDolby,LX_DOLBY_CONFIG_REG_T *i_PtrConfig)
{
	int ret = RET_ERROR;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
	...
	do{
		...
		if(i_PtrDolby->eCurMode== LX_DOLBY_OTT_MODE){
			ret = DE_DHDR_IP_REG_M19_GetCompConfig(i_PtrDolby,&i_PtrConfig->sCompRegMap);
			ret = DE_DHDR_IP_REG_M19_GetDMConfig(i_PtrDolby,&i_PtrConfig->sDMRegMap);
		}
		else{
			ret = DE_DHDR_IP_REG_M19_GetDMConfig(i_PtrDolby,&i_PtrConfig->sDMRegMap);
		}
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_M19_SetCompConfig(DOLBY_DEVICE_DATA_T *i_PtrDolby,LX_DOLBY_COMP_REG_T *i_PtrCompConfig)
{
	int ret = RET_OK;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_M19_GetCompConfig(DOLBY_DEVICE_DATA_T *i_PtrDolby,LX_DOLBY_COMP_REG_T *i_PtrCompConfig)
{
	int ret = RET_ERROR;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_M19_SetDMConfig(DOLBY_DEVICE_DATA_T *i_PtrDolby,LX_DOLBY_DM_REG_T *i_PtrDMConfig)
{
	int ret = RET_OK;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_M19_GetDMConfig(DOLBY_DEVICE_DATA_T *i_PtrDolby,LX_DOLBY_DM_REG_T *i_PtrDMConfig)
{
	int ret = RET_ERROR;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
...
int DE_DHDR_IP_REG_M19_SetLut(DOLBY_DEVICE_DATA_T *i_PtrDolby,UINT32 i_PtrFrameNum)
{
	int ret = RET_OK;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_M19_Start(DOLBY_DEVICE_DATA_T *i_PtrDolby)
{
	int ret = RET_OK;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_M19_UnInit(DOLBY_DEVICE_DATA_T *o_PtrDolbyUninit)
{
	int ret = RET_ERROR;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
 ...
 */
int DE_DHDR_IP_REG_M19_SetMdPath(DOLBY_DEVICE_DATA_T *io_PtrDolby,LX_HDR_HDMI_MD_PATH_T e_md_path)
{
	int ret = RET_OK;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
	...
	do{
		if(e_md_path == E_MD_IPP_M0)
			io_PtrDolby->sMDRegMap.unHwAddr		= DOLBY_HAL_M19A0_HDMI_MD_ADDR;
		else if(e_md_path == E_MD_HDR_HDMI_HW)
			io_PtrDolby->sMDRegMap.unHwAddr		= DOLBY_HAL_M19A0_HDMI_MD_ADDR;
		else {
			gForceMDPath_M19 = 0;
			DE_ERROR("Invalid Md path specified. Not changing HDMI MD address Mapping and turn off force mode gForceMDPath_M19 %d\n",gForceMDPath_M19);			
			ret = RET_ERROR;
			break;
		}
		io_PtrDolby->sMDRegMap.unSize	= DOLBY_HAL_M19_HDMI_TOTMD_SIZE;
		gForceMDPath_M19 = 1;
		...
		DE_ERROR("Info: MD H/W & Virtual Addr 0x%x %p force mode gForceMDPath_M19 %d\n",io_PtrDolby->sMDRegMap.unHwAddr,io_PtrDolby->sMDRegMap.unPtrVirtAddr,gForceMDPath_M19);
	}while(0);
