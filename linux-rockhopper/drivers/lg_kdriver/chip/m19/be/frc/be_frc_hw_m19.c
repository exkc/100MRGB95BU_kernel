...
#undef BE_FRC_HW_M19_TEMP_USE_TYPE_BY_CTRL_SPATH
...
/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
//static BE_FRC_HW_M19_INFO_T _beFrcHwM19Info= {0,};
static BE_FRC_HW_M19_INFO_T _gBeFrcHwM19Info= {0,};
...
int BE_FRC_HW_M19_Initialize(void)
{
	...
	do {
		memset(&_gBeFrcHwM19Info, 0, sizeof(BE_FRC_HW_M19_INFO_T));
...
int BE_FRC_HW_M19_ControlMemc(LX_BE_FRC_MEMC_MODE_T *pstParams)
{
	...
	do {
		...
		BE_TRACE1("%d, %d, %d -> %d, %d, %d\n", pstParams->u8MemcType, pstParams->u8Blur, pstParams->u8Judder,\
			_gBeFrcHwM19Info.u32MemcType, _gBeFrcHwM19Info.u32Blur, _gBeFrcHwM19Info.u32Judder);
		...
		stHwParams.uFRCUserLevel	= pstParams->u8MemcType == 0xFF ? 0xFFFFFFFF : pstParams->u8MemcType;
		 ...
		 if (stHwParams.uFRCBlur != 0xFFFFFFFF)
			_gBeFrcHwM19Info.u32Blur  = stHwParams.uFRCBlur;

   	        if (stHwParams.uFRCJudder != 0xFFFFFFFF)
   	        	_gBeFrcHwM19Info.u32Judder        = stHwParams.uFRCJudder;

               if (stHwParams.uFRCUserLevel != 0xFFFFFFFF)
		   	_gBeFrcHwM19Info.u32MemcType  = stHwParams.uFRCUserLevel;

		BE_FRC_HW_M19_SetMemcMode(&_gBeFrcHwM19Info);
	} while (0);
...
int BE_FRC_HW_M19_SetMemcLowDelay(LX_BE_FRC_LOW_DELAY_T *pstParams)
{
	...
	do {
		BE_TRACE1("%d -> %d\n", pstParams->u32Lowdelay, _gBeFrcHwM19Info.u32LowDelay);

		 if (pstParams->u32Lowdelay != 0xFFFFFFFF)
			_gBeFrcHwM19Info.u32LowDelay = pstParams->u32Lowdelay;
...
int BE_FRC_HW_M19_SetMemcMotionPro(LX_BE_FRC_MOTION_PRO_T *pstParams)
{
	...
	do {
		BE_TRACE1("%d -> %d\n", pstParams->u32MotionPro, _gBeFrcHwM19Info.u32MotionPro);

		 if (pstParams->u32MotionPro != 0xFFFFFFFF)
			_gBeFrcHwM19Info.u32MotionPro= pstParams->u32MotionPro;
...
int BE_FRC_HW_M19_SetMemcMode(BE_FRC_HW_M19_INFO_T *_gBeFrcHwM19Info)
{
	int ret = RET_OK;
	...
	do {
		...
		if (!_gBeFrcHwM19Info)
		{
			BE_ERROR("_gBeFrcHwM19Info is null.\n");
			ret=RET_ERROR;	break;
		}

		#if 1	//add uFRCMotionProEn
		if (_gBeFrcHwM19Info->u32LowDelay == 1)
		{
			stHwParams.uFRCUserLevel= LX_BE_FRC_MEMC_DISABLE;
		}
		else
		{
			stHwParams.uFRCUserLevel= _gBeFrcHwM19Info->u32MemcType;
		}
		stHwParams.uFRCBlur	= _gBeFrcHwM19Info->u32Blur;
		stHwParams.uFRCJudder = _gBeFrcHwM19Info->u32Judder;
		stHwParams.uFRCMotionProEn = _gBeFrcHwM19Info->u32MotionPro;
		BE_TRACE1("%d,%d,%d/%d/%d -> MemcType(%d), Blur(%d), Judder(%d), MPro(%d)\n",\
			_gBeFrcHwM19Info->u32MemcType, _gBeFrcHwM19Info->u32Blur,_gBeFrcHwM19Info->u32Judder,\
			_gBeFrcHwM19Info->u32LowDelay, _gBeFrcHwM19Info->u32MotionPro, stHwParams.uFRCUserLevel,\
		...
		else if (_gBeFrcHwM19Info->u32MotionPro ==1)
		{
			stHwParams.uFRCBlur	= _gBeFrcHwM19Info->u32Blur;
			stHwParams.uFRCJudder= _gBeFrcHwM19Info->u32Judder;
...
int BE_FRC_HW_M19_SetConfig(LX_BE_FRC_CFG_T *pstParams)
{
	...
	do {
		...
		{
			#ifdef BE_FRC_HW_M19_TEMP_USE_TYPE_BY_CTRL_SPATH
