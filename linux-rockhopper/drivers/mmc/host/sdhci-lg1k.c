...
static void sdhci_lg1k_get_errorinfo(struct sdhci_host *host)
{
	pr_err("emmc error: status:0x%x, cmd:0x%x, arg:0x%x, blk:0x%x, "
			"state:0x%x, cntr:0x%x, clk:0x%x\n",
			readl(host->ioaddr + SDHCI_INT_STATUS),
			readl(host->ioaddr + SDHCI_TRANSFER_MODE),
			readl(host->ioaddr + SDHCI_ARGUMENT),
			readl(host->ioaddr + SDHCI_BLOCK_SIZE),
