...
void __iomem *o20_cpu_clk_base = NULL;
extern struct regulator *o20_regulator;
...
/*----------------------------------------------------------------------------------------
	Macro Definitions
----------------------------------------------------------------------------------------*/
static DEFINE_SPINLOCK(g_o20_pm_lock);
static DEFINE_SPINLOCK(g_o20_regul_lock);
static DEFINE_SPINLOCK(g_o20_cpuregul_lock);

static unsigned int o20_regul = 0;

module_param(o20_regul, uint, S_IRUGO);
MODULE_PARM_DESC(o20_gpuNP, "o20_regul");
...
static	pms_o20_regulator_t  _stRegulator[] =
{
	/* vbase== 883 mV */
...
typedef struct
{
	UINT32 vout_ref;
	UINT32 vout_set;
}pms_o20_regulatorB_t;

/* TPS549B22 table */
static  pms_o20_regulatorB_t  _stRegulatorB[] =
{
	/*mv, set*/
	{1150,540},
	{1140,535},
...
/* MP8876 table */
static  pms_o20_regulatorB_t  _stRegulatorMPS[] =
{
	/*mv, set*/
	{1150,73},
	{1140,72},
...
static int g_o20_pm_task_disable = 0; // 이 변수는 CM3 RECOVERY용 변수임
static pms_o20_work_t g_o20_pms;
static pms_o20_work_t g_o20_regul;
static pms_o20_work_t g_o20_cpuregul;

static unsigned int g_o20_sampling_rate = 20; // 20ms
static unsigned int g_o20_core_vol = 0;

static unsigned int g_o20_err = 0;
...
#define O20_CORE_REGULATOR_IDX			2

#define O20_CORE_REGULATOR_DEVICE_ID_TPS549A20	0x3E
#define O20_CORE_REGULATOR_DEVICE_ID_TPS549B22	0x3A
#define O20_CORE_REGULATOR_DEVICE_ID_MPS	0xc0


static LX_I2C_DEV_HANDLE _i2c_core_regulator_handle;

int O20_CORE_REGUL_WRI2C( UINT8 subaddr,UINT8* data, UINT8 nbyte)
{
    int rc = -1;

	/* detect regulator type (ti/mps) */
	if (o20_regul == REGULATOR_TPS549A20)
	{
		rc = REG_WriteI2CL( O20_CORE_REGULATOR_IDX , O20_CORE_REGULATOR_DEVICE_ID_TPS549A20,subaddr, data, nbyte);
	}
	else if (o20_regul == REGULATOR_TPS549B22)
	{
		rc = REG_WriteI2CL( O20_CORE_REGULATOR_IDX , O20_CORE_REGULATOR_DEVICE_ID_TPS549B22,subaddr, data,nbyte);
	}
	else if (o20_regul == REGULATOR_MPS)
	{
		rc = REG_WriteI2CL( O20_CORE_REGULATOR_IDX , O20_CORE_REGULATOR_DEVICE_ID_MPS,subaddr, data,nbyte);
	}
	else
	{
...
int O20_CORE_REGUL_RDI2C(UINT8 subaddr, UINT8* data, UINT8 nbyte)
{
    int rc = -1;
	...
	if (o20_regul == REGULATOR_TPS549A20)
	{
		rc = REG_ReadI2CL( O20_CORE_REGULATOR_IDX , O20_CORE_REGULATOR_DEVICE_ID_TPS549A20,subaddr, data,nbyte);

	}
	else if (o20_regul == REGULATOR_TPS549B22)
	{
		rc = REG_ReadI2CL( O20_CORE_REGULATOR_IDX , O20_CORE_REGULATOR_DEVICE_ID_TPS549B22,subaddr, data,nbyte);
	}
	else if (o20_regul == REGULATOR_MPS)
	{
		rc = REG_ReadI2CL( O20_CORE_REGULATOR_IDX , O20_CORE_REGULATOR_DEVICE_ID_MPS,subaddr, data,nbyte);
	}
	else
	{
		PM_NOTI("check DCDC vendor[%d]\n",o20_regul);
...
unsigned int o20_get_error_state(void)
{
	return g_o20_err;
}

int o20_set_error_state(unsigned int value)
{
	g_o20_err = value;
...
static void o20_set_work(pms_o20_work_t o20_work)
{
	unsigned long flags;

	spin_lock_irqsave(&g_o20_pm_lock, flags);
	g_o20_pms = o20_work;
	spin_unlock_irqrestore(&g_o20_pm_lock, flags);
}

static void o20_set_regul_work(pms_o20_work_t o20_work)
{
	unsigned long flags;

	spin_lock_irqsave(&g_o20_regul_lock, flags);
	g_o20_regul = o20_work;
	spin_unlock_irqrestore(&g_o20_regul_lock, flags);
}

static void o20_set_cpuregul_work(pms_o20_work_t o20_work)
{
	unsigned long flags;

	spin_lock_irqsave(&g_o20_cpuregul_lock, flags);
	g_o20_cpuregul = o20_work;
	spin_unlock_irqrestore(&g_o20_cpuregul_lock, flags);
}

static int o20_GetCPUType(void)
{
	int ret = 0;
	pms_o20_avsinto_t info;

	pm_o20_avs = get_o20_pm_avs();
	ret = pm_o20_avs->get_cpu_avs_type(&info);

	return ret;
}

unsigned int o20_GetCPUOn(void)
{
...
unsigned int o20_GetCPUFreq(void)
{
...
int o20_SetCPUFreq(unsigned int freq)
{
...
int o20_SetCoreSwitchingFreq(unsigned int freq)
{
	int ret = 0;
	...
	if (o20_regul == REGULATOR_TPS549A20)
	{
		...
		O20_CORE_REGUL_WRI2C(FREQUENCY_CONFIG, (UINT8 *)data ,1);
	...
	else if(o20_regul == REGULATOR_TPS549B22)
	{
		...
		O20_CORE_REGUL_WRI2C(MFR_SPECIFIC_03, (UINT8 *)data ,1);
		PM_NOTI("freq[%d]khz set[%d]\n",freq,freq_set);
	}
	else if(o20_regul == REGULATOR_MPS)
	{
		...
		O20_CORE_REGUL_WRI2C(SYSCTRL2, (UINT8 *)data ,1);
		PM_NOTI("freq[%d]khz set[%d]\n",freq,freq_set);
	}
	else
	{
...
int __TPS549A20_set_volt(unsigned int volt)
{
	int ret = 0, i = 0;
	pms_o20_regulator_t * regul = NULL ;
	...
	for(i=0; i < sizeof(_stRegulator)/sizeof(pms_o20_regulator_t); i++)
	{
	...
	ret = O20_CORE_REGUL_WRI2C(OPERATION, (UINT8 *)data ,1);
	if(ret < 0 )
	{
		PM_NOTI("check tpsA\n");
		return -1;
	...
	O20_CORE_REGUL_WRI2C(ON_OFF_CONFIG, (UINT8 *)data ,1);
	...
	O20_CORE_REGUL_WRI2C(VOUT_MARGIN,(UINT8 *)data,1);
	//printf("vout_margin_high[%x] vout_margin_low[%x]\n",vout_margin_high,vout_margin_low);

	data[0]= vout_set;
	O20_CORE_REGUL_WRI2C(VOUT_ADJUSTMENT,(UINT8 *)data,1);
	PM_NOTI("tiA volt[%d]mV set[0x%x:%d]\n",volt,vout_set,vout_set);

	return ret;

}

int __TPS549B22_set_volt(unsigned int volt)
{
	int ret = 0;
	int i = 0;
	pms_o20_regulatorB_t * regul = NULL ;
	UINT32 vout_set  = 0;
	u8 data[5];
	...
	for(i=0; i < sizeof(_stRegulatorB)/sizeof(pms_o20_regulatorB_t); i++)
	{
	...
	O20_CORE_REGUL_WRI2C(VOUT_COMMAND,(UINT8 *)data,2);
	PM_NOTI("tiB volt[%d]mV set[0x%x:%d]\n",volt,vout_set,vout_set);
	return ret;

}

int __MPS_set_volt(unsigned int volt)
{
	int ret = 0;
	int i = 0;
	UINT8 data[5];
	...
	for(i=0; i < sizeof(_stRegulatorMPS)/sizeof(pms_o20_regulatorB_t); i++)
	{
	...
	ret = O20_CORE_REGUL_WRI2C(SYSCTRL2, (UINT8 *)data,1);
...
int o20_SetCoreVal(unsigned int volt)
{
	int ret = 0;
	...
	if (o20_regul == REGULATOR_TPS549A20)
	{
		ret =__TPS549A20_set_volt(volt);
	}
	else if (o20_regul == REGULATOR_TPS549B22)
	{
		ret = __TPS549B22_set_volt(volt);
	}
	else if (o20_regul == REGULATOR_MPS)
	{
		ret = __MPS_set_volt(volt);
	}
	else
	{
		PM_NOTI("check DCDC vendor[%d]\n",o20_regul);
	...
	else if (o20_regul == REGULATOR_MPS)
	{
		...
		ret = O20_CORE_REGUL_WRI2C(VSEL, (UINT8 *)data ,1);
		if(ret < 0 )
		{
	...
	#endif
	#endif
	PM_DEBUG("o20_regul[%x] ret[%d]\n",o20_regul ,ret);


	return ret;
}
unsigned int o20_GetCoreVal(void)
{
	unsigned int mV = g_o20_core_vol;
	return mV;
}


static unsigned int o20_get_sampling_rate(void)
{
	return g_o20_sampling_rate;
}

static int o20_set_sampling_rate(unsigned int ms)
{
	g_o20_sampling_rate = ms;

	return 0;
}

static int o20_pm_task_dvfs(pms_o20_work_t *pms_o20_work)
{
	int ret = 0;
#ifdef O20_DVFS_ENABLE
	struct cpufreq_policy *policy;
	if ((pms_o20_work->do_dvfs == 1) || (pms_o20_work->do_dvfs == 2))
	{
		pms_o20_work->do_dvfs = 0;
		policy = cpufreq_cpu_get(0);
		if (policy != NULL)
			ret = cpufreq_driver_target(policy, pms_o20_work->target_freq*1000, CPUFREQ_RELATION_H);
		else
			ret = 1;

		if(ret != 0)
			o20_set_error_state(0xF << 28 | pms_o20_work->target_freq << 16 | (ERROR_MASK & ret));
	}
	else if (pms_o20_work->do_dvfs == 3)
	{
		pms_o20_work->do_dvfs = 0;
		PM_DEBUG_PRINT("FREQ %ukHz\n",pms_o20_work->target_freq);
		ret = o20_SetCPUFreq(pms_o20_work->target_freq);
		if(ret != 0)
			o20_set_error_state(0xF << 28 | pms_o20_work->target_freq << 16 | (ERROR_MASK & ret));
	}
#endif
...
static int o20_pm_task_hotplug(pms_o20_work_t *pms_o20_work)
{
	struct device * pCpudev = NULL;
	int ret = 0;

	if (pms_o20_work->do_hotplug && (o20_get_error_state() == 0))
	{
		pms_o20_work->do_hotplug = 0;

		PM_DEBUG_PRINT("HOTPLUG DO cpu%u:%u\n",pms_o20_work->core_n,pms_o20_work->core_onoff);

		if (pms_o20_work->core_onoff == 0)
		{
			if (cpu_online(pms_o20_work->core_n))
			{
				ret = cpu_down(pms_o20_work->core_n);
				if(ret != 0)
				{
					if (ret != -EBUSY)
						o20_set_error_state(0xD << 28 | pms_o20_work->core_n << 24 | pms_o20_work->core_onoff << 16 | (ERROR_MASK & ret));
				}
				else
				{
					pCpudev = get_cpu_device(pms_o20_work->core_n);
					device_lock(pCpudev);
					pCpudev->offline = 1;
					device_unlock(pCpudev);

					if (o20_GetCPUOn() & (1 << pms_o20_work->core_n))
					{
		...
		else
		{
			if (!cpu_online(pms_o20_work->core_n))
			{
				ret = cpu_up(pms_o20_work->core_n);
				if(ret != 0)
				{
					if (ret != -EBUSY)
						o20_set_error_state(0xD << 28 | pms_o20_work->core_n << 24 | pms_o20_work->core_onoff << 16 | (ERROR_MASK & ret));
...
static int o20_pm_task_avs(pms_o20_work_t *pms_o20_work)
{
	int ret = 0;
	int prevMV = 0;
	unsigned int type = 0;
	...
	if (pms_o20_work->do_avs == 1)
	{
		pms_o20_work->do_avs = 0;
		o20_SetCoreVal(pms_o20_work->target_corevol);
	}
	else if (pms_o20_work->do_avs == 2)
	{
		pms_o20_work->do_avs = 0;
		type = pms_o20_work->target_vol;
		...
		prevMV = regulator_get_voltage(o20_regulator) ;

		switch(type){
		case CPU_AVS_S:
			//HV
			ret = regulator_set_voltage(o20_regulator, CPU_AVS_S_UV,CPU_AVS_S_UV);
			break;
		case CPU_AVS_N:
			//MV
			ret = regulator_set_voltage(o20_regulator, CPU_AVS_N_UV,CPU_AVS_N_UV);
			break;
		case CPU_AVS_F:
			//LV
			ret = regulator_set_voltage(o20_regulator, CPU_AVS_F_UV,CPU_AVS_F_UV);
			break;
		case CPU_AVS_FF:
			//VLV
			ret = regulator_set_voltage(o20_regulator, CPU_AVS_FF_UV,CPU_AVS_FF_UV);
		...
		PM_NOTI("CPU ret[%d] mV[%d] => mV[%d]\n",ret,prevMV,regulator_get_voltage(o20_regulator) );
		regulator_put(o20_regulator);
	}
	return ret;
}

static void o20_cpuregul_task_call(pms_o20_work_t o20work)
{
	o20_set_cpuregul_work(o20work);
	complete(&cpu_regulator_completion);
}
void o20_taskdisable_reset(void)
{
	g_o20_pm_task_disable = 0;
}

static void o20_lgdtv_pm_task_reset(void)
{
	reinit_completion(&cpu_hotplug_completion);
}

static void o20_lgdtv_pm_task_call(pms_o20_work_t o20work)
{
	o20_set_work(o20work);
	complete(&cpu_hotplug_completion);
}

static void o20_lgdtv_regul_task_reset(void)
{
	reinit_completion(&regulator_completion);
}

static void o20_lgdtv_regul_task_call(pms_o20_work_t o20work)
{
	o20_set_regul_work(o20work);
	complete(&regulator_completion);
}


static int o20_lgdtv_pm_task(void *pParam)
{
	unsigned long flags;

	pms_o20_work_t pms_o20_work;

	PM_NOTI("o20 lgdtv_pm_task is created\n");

	do {
		/* Check stop condition when device is closed. */
		if (kthread_should_stop()) {
			PM_NOTI("o20 lgdtv_pm_task - exit!\n");
			break;
		}
		...
		if (g_o20_pm_task_disable == 1)
			continue;

		if (o20_get_error_state()== 0)
		{
			spin_lock_irqsave(&g_o20_pm_lock, flags);
			memcpy((void *)&pms_o20_work, (void *)&g_o20_pms, sizeof(pms_o20_work_t));
			spin_unlock_irqrestore(&g_o20_pm_lock, flags);

			o20_pm_task_dvfs(&pms_o20_work);
			o20_pm_task_hotplug(&pms_o20_work);
...
static int o20_cpu_regulator_task(void *pParam)
{
	unsigned long flags;

	pms_o20_work_t pms_o20_work;

	PM_NOTI("o20_cpuregul_task is created\n");

	do {
		/* Check stop condition when device is closed. */
		if (kthread_should_stop()) {
			PM_NOTI("o20_cpuregul_task - exit!\n");
			break;
		}
		...
		spin_lock_irqsave(&g_o20_cpuregul_lock, flags);
		memcpy((void *)&pms_o20_work, (void *)&g_o20_cpuregul, sizeof(pms_o20_work_t));
		spin_unlock_irqrestore(&g_o20_cpuregul_lock, flags);
		o20_pm_task_avs(&pms_o20_work);
...
static int o20_regulator_pm_task(void *pParam)
{
	unsigned long flags;

	pms_o20_work_t pms_o20_work;

	PM_NOTI("o20_regulator_pm_task is created\n");

	do {
		/* Check stop condition when device is closed. */
		if (kthread_should_stop()) {
			PM_NOTI("o20_regulator_pm_task - exit!\n");
			break;
		}
		...
		spin_lock_irqsave(&g_o20_regul_lock, flags);
		memcpy((void *)&pms_o20_work, (void *)&g_o20_regul, sizeof(pms_o20_work_t));
		spin_unlock_irqrestore(&g_o20_regul_lock, flags);
		o20_pm_task_avs(&pms_o20_work);
...
void o20_pmtask_init(void)
{
	...
	if (cpu_hotplug_pm_task == NULL) {
		...
		cpu_hotplug_pm_task = kthread_create_on_node(o20_lgdtv_pm_task,
					NULL,
					cpu,
	...
	if (regulator_pm_task == NULL) {
		...
		regulator_pm_task = kthread_create_on_node(o20_regulator_pm_task,
					NULL,
					cpu,
	...
	if (cpu_regulator_task == NULL) {
		...
		cpu_regulator_task = kthread_create_on_node(o20_cpu_regulator_task,
					NULL,
					cpu,
...
static void o20_AllCoreOn(void)
{
...
#define CPU_ON				1
	...
	for(i=1; i<4; i++)
	{
		if (cpu_online(i) == 0)
		{
			...
			pms_work.do_dvfs = 0;
			o20_lgdtv_pm_task_call(pms_work);
			msleep(30);		/* max cpu on time */
		}
	}
}

static void o20_SetMaxPerformance(void)
{
	pms_o20_work_t pms_work;
	...
	memset(&pms_work, 0x0, sizeof(pms_o20_work_t));
	pms_work.target_freq = O20_CPU_FREQ_ND;
...
void o20_FuncInit(void)
{

	/* o20 DCDC regulater vendor is fixed as TI
		detailed info: http://clm.lge.com/issue/browse/SCDCR-1798 */

	//o20_regul = REGULATOR_TPS539A20;
...
static pms_o20_func_t o20_pm_func =
{
	.init = o20_FuncInit,
	.getfreq = o20_GetCPUFreq,
	.setfreq = o20_SetCPUFreq,
	.setcorevolt = o20_SetCoreVal,
	.setcore_switching_freq = o20_SetCoreSwitchingFreq,
	.getcorevolt = o20_GetCoreVal,
	.get_cpu_on = o20_GetCPUOn,
	.get_cpu_type = o20_GetCPUType,
	.set_max_performance = o20_SetMaxPerformance,
	.run = o20_lgdtv_pm_task_call,
	.run_regul = o20_lgdtv_regul_task_call,
	.run_cpuregul = o20_cpuregul_task_call,
	.reset = o20_lgdtv_pm_task_reset,
	.reset_regul = o20_lgdtv_regul_task_reset,
	.taskdisable_reset = o20_taskdisable_reset,
	.get_sampling_rate = o20_get_sampling_rate,
	.set_sampling_rate = o20_set_sampling_rate,
	.get_error_state = o20_get_error_state,
	.set_error_state = o20_set_error_state,
};

pms_o20_func_t* get_o20_pm_func(void)
{
	return &o20_pm_func;
}
