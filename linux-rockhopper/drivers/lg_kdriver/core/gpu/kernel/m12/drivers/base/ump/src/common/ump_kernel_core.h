...
typedef struct umpp_allocation
{
	...
	pid_t client; /**< the process ID using the memory */
...
/**
 * Sum total ump internal memory
 * It called from kbase_mali.ko for calulating ump internal memory
 * @param ump_memtotal The ump internal memory
 */
int ump_debug_mem_for_meminfo(unsigned int *ump_memtotal);

int ump_debug_mem_for_webos_mem(int client_pid);
