	...
	if (unlikely(emmc_log == 1)) {
		if ((cmd->opcode == MMC_READ_SINGLE_BLOCK) ||
			...
			if (cmd->opcode == MMC_SWITCH) {
				...
				if ((size & 0xffff0000) == 0x3220000)
					pr_warn(":[EMMC PON]: 0x%x\n", size);
			}
		}
	} else if (unlikely(emmc_log == 2)) {
		if ((cmd->opcode == MMC_READ_SINGLE_BLOCK) ||
			(cmd->opcode == MMC_READ_MULTIPLE_BLOCK) ||
			(cmd->opcode == MMC_WRITE_BLOCK) ||
			(cmd->opcode == MMC_WRITE_MULTIPLE_BLOCK) ||
			(cmd->opcode == MMC_SEND_EXT_CSD))
			pr_warn(":[EMMC CMD%02u]: 0x%04x 0x%04x, 0x%08x, 0x%04x 0x%04x\n",
				cmd->opcode, SDHCI_MAKE_CMD(cmd->opcode, flags),
				sdhci_readw(host, SDHCI_TRANSFER_MODE),
				sdhci_readl(host, SDHCI_ARGUMENT),
				sdhci_readw(host, SDHCI_BLOCK_COUNT),
				sdhci_readw(host, SDHCI_BLOCK_SIZE));
		else
			pr_warn(":[EMMC CMD%02u]: 0x%04x 0x%04x, 0x%08x\n", cmd->opcode,
