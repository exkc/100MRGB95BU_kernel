...
int umpp_phys_commit(umpp_allocation * alloc)
{
	...
	{
		alloc->block_array = kmalloc(sizeof(ump_dd_physical_block_64) * alloc->blocksCount, __GFP_HARDWALL | GFP_KERNEL);
	}
	else
	{
		alloc->block_array = vmalloc(sizeof(ump_dd_physical_block_64) * alloc->blocksCount);
	}
#endif
	...
	for (i = 0; i < alloc->blocksCount; i++)
	{
		void * mp;
#if defined(ORIG_CODE)
		struct page * page = alloc_page(GFP_USER | __GFP_NORETRY | __GFP_NOWARN | __GFP_COLD);
#else
		struct page * page = alloc_page(GFP_USER | __GFP_COLD);
#endif
		if (NULL == page)
		{
			break;
		...
		/* cache invalidation for data from GPU */
...
int umpp_linux_mmap(struct file * filp, struct vm_area_struct * vma)
{
	...
	if (UMP_DD_INVALID_MEMORY_HANDLE != h)
	{
		...
		vma->vm_flags |= VM_DONTCOPY | VM_DONTEXPAND | VM_IO | VM_DONTDUMP;
#else
