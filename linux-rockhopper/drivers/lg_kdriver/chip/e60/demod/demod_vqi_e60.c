...
unsigned int gE60GBB_PowerOnInitStartTimeISDBT = 0;

/***********************************************************************************
	Static Variable & Function Prototypes Declarations
 ************************************************************************************/
UINT16	_gE60GBB_VQI_UnlockCount_full_packeterr = 0;
UINT16 	_gE60GBB_FullVQIPkerrCount = 7;
UINT16 	_gE60GBB_FullVQPkerrThreshold = 300;
UINT16 	_gE60GBB_FullISDBTPkerrThreshold = 200;

BOOLEAN _gE60GBB_ISDBT_Mem_set = FALSE;
...
int DEMOD_E60_ATSC_SW_Init (void)
{
...
int _DEMOD_E60_LGDBB_VSB_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int _DEMOD_E60_VSB_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_E60_LGDBB_VSB_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	...
	new_tuneMode = _DEMOD_E60_VSB_CheckTuneMode(pDemodInfo);

	if(RET_OK != DEMOD_E60_Get_BW(pDemodInfo, &ter_bandwidth_KHz))
	{
...
************************************************************************************/
/**
 * DEMOD_E60_VSB_Obtaining_Signal_Lock.
...
int DEMOD_E60_VSB_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_TUNE_START:
			{
				...
				if(RET_OK != DEMOD_E60_LGDBB_VSB_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_E60_LGDBB_VSB_SetDemod() \n");
					DEMOD_RETURN_FAIL(__F__);
				}

				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.vsb.tuneMode)
				{
					if(RET_OK != DEMOD_E60_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_ON, &scanProcessing, &scanLock))
					{
				...
				else
				{
					...
					{
						DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_E60_TPOutEnable() \n");
						DEMOD_RETURN_FAIL(__F__);
					}
		...
		case LX_DEMOD_TUNE_SCAN:
			{
				...
				if(RET_OK != DEMOD_E60_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_OFF, &scanProcessing, &scanLock))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_E60_NeverlockScan() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE:
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.vsb.tuneMode)
					{
						...
						if(RET_OK != DEMOD_E60_Get_Packet_Error(&packetError))
						{
							DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_E60_Get_Packet_Error() \n");
							DEMOD_RETURN_FAIL(__F__);
						}
						else
						{
							...
							if (RET_OK != _DEMOD_E60_LGDBB_VSB_CheckSignalState(pDemodInfo))
							{
								DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, _DEMOD_E60_LGDBB_VSB_CheckSignalState() \n");
								DEMOD_RETURN_FAIL(__F__);
							}
						}
					}
...
************************************************************************************/
/**
 * DEMOD_E60_VSB_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_E60_VSB_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
...
#if 1
	if(RET_OK != DEMOD_E60_VSB_Monitor(pDemodInfo))
	{
		DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_E60_VSB_Monitor() \n");
		DEMOD_RETURN_FAIL(__F__);
	}
#endif
...
//			pauseTimeDemod = LX_DEMOD_LOCKED_PAUSE_TIME;
		...
		{
			DEMOD_PRINT( "[DEMOD E60 GBB] FAIL DEMOD_Get_signal_status() \n");
			DEMOD_RETURN_FAIL(__F__);
		}
		#endif
...
int _DEMOD_E60_LGDBB_QAM_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int _DEMOD_E60_QAM_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_E60_LGDBB_QAM_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context * 	pFrontend;
	UINT8			new_tuneMode;
	UINT32 			freq_Khz = 0;
	...
	pFrontend = DEMOD_E60_GetContext();
	new_tuneMode = _DEMOD_E60_QAM_CheckTuneMode(pDemodInfo);
	...
	// printk("[E60 LGDBB] ################### chungiii SetDemod QAM check ter_bandwidth_KHz[%d] \n", ter_bandwidth_KHz);
...
************************************************************************************/
/**
 * DEMOD_E60_QAM_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_E60_QAM_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	...
	switch (pDemodInfo->controlState)
	{
		...
		case LX_DEMOD_TUNE_START:
			{
				if(RET_OK != DEMOD_E60_LGDBB_QAM_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_E60_LGDBB_QAM_SetDemod() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
		...
		case LX_DEMOD_SYNC_CHECKING :
			{
				...
				if (RET_OK != _DEMOD_E60_LGDBB_QAM_CheckSignalState(pDemodInfo))
				{
						DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, _DEMOD_E60_LGDBB_QAM_CheckSignalState() \n");
						DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if(RET_OK != DEMOD_E60_QAM_EQ_Signal_Detector(FALSE))
					{
						DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_E60_QAM_EQ_Signal_Detector() \n");
						DEMOD_RETURN_FAIL(__F__);
					}
				...
				else
				{
					if(LX_DEMOD_TUNE_NORMAL == realTuneMode)
					{
						...
						{
							if(RET_OK != DEMOD_E60_QAM_ModeAutoDetection(TRUE))
							{
								DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_E60_QAM_ModeAutoDetection() \n");
								DEMOD_RETURN_FAIL(__F__);
							}
							#if 0
							if(RET_OK != DEMOD_E60_SoftwareReset())
							{
								DEMOD_PRINT("[DEMOD E60 GBB] FAIL (%s:%d) DEMOD_E60_SoftwareReset() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							#endif
...
************************************************************************************/
/**
 * DEMOD_E60_QAM_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_E60_QAM_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_LOCK_STATE_T			lockState;
...
//			pauseTimeDemod = LX_DEMOD_LOCKED_PAUSE_TIME;
	...
	{
		pDemodInfo->lockCount = 0;
		_gE60GBB_VQI_UnlockCount_full_packeterr = 0;
...
int _DEMOD_E60_LGDBB_ISDB_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int _DEMOD_E60_ISDB_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_E60_LGDBB_ISDB_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	...
	new_tuneMode = _DEMOD_E60_ISDB_CheckTuneMode(pDemodInfo);
...
int DEMOD_E60_ISDBT_TDI_Enable (BOOLEAN bEnable)
{
	LX_DEMOD_L2_Context *	pFrontend;
	...
	//printk("[DEMOD E60p]   DEMOD_E60_TDI_Enable  ###### chungiii  check TDI data[0x%x] \n", vqData);
...
int DEMOD_E60_ISDBT_Set_MemSconfig (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int DEMOD_E60_ISDBT_GetEmergencyAlertFlagStatus (BOOLEAN *pEalarm)
{
	LX_DEMOD_L2_Context *	pFrontend;
	UINT8 			balarm;
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_E60_ISDBT_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	static LX_DEMOD_FLAG_T 		scanProcessing = LX_DEMOD_FLAG_START;
	static LX_DEMOD_LOCK_STATE_T 	scanLock = LX_DEMOD_UNLOCKED;
	static unsigned int 		sigStablestartTime = 0;
	...
	switch (pDemodInfo->controlState)
	{
		...
		case LX_DEMOD_TUNE_START:
			{
				if(RET_OK != DEMOD_E60_LGDBB_ISDB_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_E60_LGDBB_ISDB_SetDemod() \n");
					DEMOD_RETURN_FAIL(__F__);
				}

				#if 1   // FALSE(0x88) = only when using FPGA board , TRUE(0x08) = only when using SOC
				if(RET_OK != DEMOD_E60_ISDBT_TDI_Enable(TRUE))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_E60_ISDBT_TDI_Enable() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
				#endif
		...
		case LX_DEMOD_SYNC_CHECKING :
			{
				...
				else
				{
					...
					if(pDemodInfo->bNotifiedUnlock == TRUE)
					{
						...
						{
							if(RET_OK != DEMOD_E60_ISDBT_SafetyReset())
							{
								DEMOD_PRINT("[DEMOD E60] FAIL (%s:%d) DEMOD_SoftwareReset() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							DEMOD_PRINT("[DEMOD E60](%s) DEMOD_SoftwareReset() !!!\n", __F__);
						}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.isdbt.tuneMode)
					{
						...
						else
						{
							...
							{
#if 0
								if(RET_OK != DEMOD_E60_ISDBT_Set_PhaseNoiseEnhancement(TRUE))
								{
									DEMOD_PRINT("[DEMOD_CHIP] FAIL, DEMOD_E60_ISDBT_Set_PhaseNoiseEnhancement() \n");
									DEMOD_RETURN_FAIL(__F__);
								}
#endif
								if(packetError < LX_DEMOD_PACKET_ERROR_THD)
								{
									...
									DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP][20200728] LX_DEMOD_SIGNAL_STABLE : MSG_ISDBT_FRONT_END_LOCKED..... ^^^0^^^\n");
									DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^^0^^^\n", elapsedTime);
									DEMOD_PRINT( "[DEMOD_CHIP] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^^^0^^^\n", freqOffset);
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_E60_ISDBT_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
...
//			pauseTimeDemod = LX_DEMOD_LOCKED_PAUSE_TIME;
		...
		if (RET_OK != _DEMOD_E60_LGDBB_ISDB_CheckSignalState(pDemodInfo)) {
			DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, _DEMOD_E60_LGDBB_ISDB_CheckSignalState() \n");
			DEMOD_RETURN_FAIL(__F__);
		}

		if(pDemodInfo->signalStatus.packetError >= _gE60GBB_FullISDBTPkerrThreshold) {
			_gE60GBB_VQI_UnlockCount_full_packeterr++;
			DEMOD_PRINT( "^r^[DEMOD E60](%s) _gE60GBB_VQI_UnlockCount_full_packeterr = %d !!!\n", __F__,_gE60GBB_VQI_UnlockCount_full_packeterr);

			if(_gE60GBB_VQI_UnlockCount_full_packeterr == _gE60GBB_FullVQIPkerrCount)
			{
				...
				if(RET_OK != DEMOD_E60_ISDBT_SafetyReset())
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_E60_ISDBT_SafetyReset() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
				DEMOD_PRINT("[DEMOD_CHIP] SUCCESS, DEMOD_E60_ISDBT_SafetyReset() \n");
			...
			if(gE60GBB_PowerOnInit == 2)
			{
				powerOnElapsedTime = jiffies_to_msecs(jiffies) - gE60GBB_PowerOnInitStartTimeISDBT;

				DEMOD_PRINT("[DEMOD_CHIP] powerOnElapsedTime = %d \n",powerOnElapsedTime);

				if(powerOnElapsedTime >= 15000)
				{
					gE60GBB_PowerOnInit = 0;

					if (RET_OK != DEMOD_E60_ISDBT_Set_CrGearshEn(TRUE ))
					{
						DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_E60_ISDBT_Set_CrGearshEn() \n");
						DEMOD_RETURN_FAIL(__F__);
					}
				}
			}
...
************************************************************************************/
/**
 * DEMOD_E60_QAM_SoftwareResetFEC.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_QAM_SoftwareResetFEC (void)
{
	return RET_OK;
}
...
int DEMOD_E60_QAM_Monitor (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_QAM_ModeAutoDetection.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_QAM_ModeAutoDetection (BOOLEAN bEnable)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_QAM_256Mode (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_QAM_Set_DefaultRegisterValue.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_QAM_Set_DefaultRegisterValue (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_QAM_Set_NeverLockWaitTime
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_QAM_Set_NeverLockWaitTime (UINT32 waitMs)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_QAM_Get_TotalInfo (void)
{
	return RET_OK;
}

/**
 * DEMOD_E60_QAM_EQ_Signal_Detector.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_QAM_EQ_Signal_Detector (BOOLEAN OnOff)
{
	return RET_OK;
}

/**
 * DEMOD_E60_QAM_Set_Frame_resynchronizaiton.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_QAM_Set_Frame_resynchronizaiton (BOOLEAN OnOff)
{
	return RET_OK;
}

/**
 * DEMOD_E60_ISDBT_SetDefaultRegisterValue
...
int DEMOD_E60_ISDBT_Set_DefaultRegisterValue (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_Set_Config_auto
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_ISDBT_Set_Config_auto (BOOLEAN bAutoDetect)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_Set_PartialConfig
 * Set hierachical signal path and bandwidth.
 * This function is used when DEMOD_E60_ISDBT_Set_Config_auto is enabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_ISDBT_Set_PartialConfig (LX_DEMOD_ISDBT_CONFIG_T *pIsdbtConfigParam)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when DEMOD_E60_ISDBT_Set_Config_auto is disabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_ISDBT_Set_FullConfig (LX_DEMOD_ISDBT_CONFIG_T *pIsdbtConfigParam)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_ISDBT_Set_NeverLockWaitTime (UINT32 waitMs)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_Set_CrGearshEn
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_ISDBT_Set_CrGearshEn (BOOLEAN bEnable)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_Set_PhaseNoiseEnhancement
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_ISDBT_Set_PhaseNoiseEnhancement (BOOLEAN bEnhanced)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_Get_DelaySpreadStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_ISDBT_Get_DelaySpreadStatus (UINT16 *pDelaySpread)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_ISDBT_Get_FFTMode (LX_DEMOD_LOCK_STATE_T *pTxModeLock, LX_DEMOD_FFTMODE_T *pFftMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
//	UINT8	fftmoder_di_1_0;
	...
	*pFftMode = LX_DEMOD_DVBT_ISDBT_FFT_UNKNOWN;
	...
	if(RET_OK != DEMOD_E60_Get_Lock(LX_DEMOD_PSYNC_LOCK, &lockState))
	{
		DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_E60_Get_Lock() !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}
#endif
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_Get_GuradIntervalMode
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_ISDBT_Get_GuradIntervalMode (LX_DEMOD_GIMODE_T *pGIMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_TMCC_INFO	tmcc;
	...
	*pGIMode = LX_DEMOD_DVBT_ISDBT_GIMODE_UNKNOWN;
	...
	if(RET_OK != DEMOD_E60_Get_Lock(LX_DEMOD_PSYNC_LOCK, &lockState))
	{
		DEMOD_PRINT("[DEMOD] FAIL, DEMOD_E60_Get_Lock() !!!\n");
		DEMOD_RETURN_FAIL(__F__);
	}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_E60_ISDBT_Get_TMCCInfo (LX_DEMOD_ISDBT_TMCC_CONFIG_T *pTmccParams)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_TMCC_INFO	tmcc;
...
int DEMOD_E60_ISDBT_Get_TotalInfo (void)
{
	return RET_OK;
}
...
int DEMOD_E60_ISDBT_Get_IFO_LOCK (BOOLEAN *pIfoLock)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_Get_CochanDetIndicator
...
int DEMOD_E60_ISDBT_Get_CochanDetIndicator (BOOLEAN *pDetStatus)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_EqualizereReset.
...
int DEMOD_E60_ISDBT_EqualizereReset (void)
{
	return RET_OK;
}
...
int DEMOD_E60_ISDBT_SafetyReset (void)
{
#if 0
	DEMOD_E60_FecReset(TRUE);
	OS_MsecSleep(5);
...
************************************************************************************/
/**
 * DEMOD_E60_ISDBT_MemoryBusCheck.
...
int DEMOD_E60_ISDBT_MemoryBusCheck (void)
{
	UINT32 m1_prevW = 0x0;
	UINT32 m1_prevR = 0x0;
	UINT32 m1_currW = 0x0;
...
int DEMOD_E60_ISDBT_Set_MemBus_flushing (void)
{
	UINT32 bus_gating;
	UINT32 bflush;
...
************************************************************************************/
/**
 * DEMOD_E60_VSB_Set_DefaultRegisterValue.
...
int DEMOD_E60_VSB_Set_DefaultRegisterValue (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_VSB_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_E60_VSB_Set_NeverLockWaitTime (UINT32 waitMs)
{
	return RET_OK;
}
...
int DEMOD_E60_VSB_CochannelExist (BOOLEAN *pCochannel)
{
	return RET_OK;
}
...
int DEMOD_E60_VSB_PreMonitor (void)
{
	DEMOD_TRACE("[DEMOD E60 DTV] @ %s =====\n",__F__);
...
int DEMOD_E60_VSB_Monitor (DEMOD_CTX_T *pDemodInfo)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_E60_VSB_Get_MSEdynStatus.
...
int DEMOD_E60_VSB_Get_MSEdynStatus (BOOLEAN *pbEnable)
{
	return RET_OK;
}
...
int DEMOD_E60_VSB_Get_TotalInfo (void)
{
	return RET_OK;
}
...
int DEMOD_E60_LGDBB_VSB_SetDemod_Simple (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
