...
static int slt_open( void )
{
	...
	SLT_REG = (volatile O22_SLT_REG_T*)ioremap(O22_SLT_BASE, sizeof(O22_SLT_REG_T));
	if( SLT_REG == NULL )
	{
		SLT_ERROR("can't ioremap slt addr = 0x%08x\n", O22_SLT_BASE);
		return RET_ERROR;
	}
	
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
	ret = request_irq(O22_IRQ_SLT, (irq_handler_t)slt_isr, NULL, "SLT", NULL);
#else
	ret = request_irq(O22_IRQ_SLT, (irq_handler_t)slt_isr, IRQF_DISABLED, "SLT", NULL);
#endif
	if( ret != RET_OK )
	{
...
static int slt_close(void)
{
	...
	/* CRC Enable */
	SLT_RdFL( crc_enable );
	SLT_Wr( crc_enable, 0x0 );
	SLT_WrFL( crc_enable );

	iounmap( SLT_REG );

    free_irq(O22_IRQ_SLT, NULL );
