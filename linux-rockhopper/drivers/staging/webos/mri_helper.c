...
#define NR_VMSTAT_ITEMS		(NR_VM_ZONE_STAT_ITEMS + \
				 ...
				 NR_VM_STAT_ITEMS)
#define NR_VMEVENT_ITEMS	(IS_ENABLED(CONFIG_VM_EVENT_COUNTERS) ? \
				 NR_VM_EVENT_ITEMS : 0)
struct mri_stats_item {
	...
	unsigned long dma32_usable;
	unsigned long vmstat[NR_VMSTAT_ITEMS];
#ifdef CONFIG_VM_EVENT_COUNTERS
	unsigned long _vmstat[NR_VMEVENT_ITEMS];
#endif
	unsigned long nr_free[MAX_NR_ZONES][MAX_PAGE_ORDER];
	unsigned long migrate_types[MAX_NR_ZONES][MAX_PAGE_ORDER];
	unsigned long nr_reserved_highatomic[MAX_NR_ZONES];
#ifdef CONFIG_MEMORY_ISOLATION
	unsigned long nr_isolate_pageblock[MAX_NR_ZONES];
#endif
	int node_dirty_ok[MAX_NR_ZONES];
...
enum swap_free_level {
	MRI_SWAP_FREE_CRITICAL = 0,
	MRI_SWAP_FREE_LOW,
	MRI_SWAP_FREE_NORMAL,
	MRI_NUM_SWAP_FREE_LEVELS,
};
...
{
	enum mri_levels level;

	for (level = MRI_CRITICAL; level > MRI_LOW; level--) {
...
static enum mri_lowmem_level mri_calc_lowmem_level(void)
{
	...
	enum mri_lowmem_level wmark_level = MRI_NUM_LOWMEM_LEVELS;
	enum mri_lowmem_level wmark_idx;
	enum mri_lowmem_level vm_lvl = MRI_LOWMEM_CRITICAL;
	...
	for_each_populated_zone(z) {
		...
		for (i = 0; i < MAX_NR_ZONES; i++) {
			...
			for (wmark_idx = MRI_LOWMEM_CRITICAL; wmark_idx < MRI_NUM_LOWMEM_LEVELS; wmark_idx++) {
...
static void mri_event_swap_free(unsigned long swap)
{
	struct mri_event_list *ev_lst = &mri_event_lists[MRI_EVENT_SWAP_FREE];
	struct mri_event *ev;

	if (!swap) {
		struct usable_info u_swap;
		get_usable_info(&u_swap);
		swap = u_swap.swap;
	}

	if (list_empty(&ev_lst->list) || ev_lst->curr == swap)
		return;

	pr_debug("mri_helper swap: %d -> %d\n", ev_lst->curr, swap);

	mutex_lock(&ev_lst->lock);
	list_for_each_entry(ev, &ev_lst->list, list) {
		/* level up */
		if (ev->level > ev_lst->curr && ev->level <= swap) {
			eventfd_signal(ev->efd);
			continue;
		}
		/* level down */
...
static void mri_event_nr_unevictable(int nr_unevictable)
{
	...
	if (!nr_unevictable)
		nr_unevictable = global_node_page_state(NR_UNEVICTABLE) -
			global_zone_page_state(NR_MLOCK);
...
static void mri_event_nr_zspages(int nr_zspages)
{
	...
	if (!nr_zspages)
		nr_zspages = global_zone_page_state(NR_ZSPAGES);
...
static unsigned long get_current_dma32_usable(void)
{
	struct zone *zone = &NODE_DATA(0)->node_zones[ZONE_MOVABLE];
	struct usable_info ui;
	get_usable_info(&ui);
	return (unsigned long)(ui.usable - zone_page_state(zone, NR_FREE_PAGES));
}

static void mri_event_dma32_usable(unsigned long dma32_usable)
{
	struct mri_event_list *ev_lst = &mri_event_lists[MRI_EVENT_DMA32_USABLE];
	struct mri_event *ev;
	int level = (int)dma32_usable;

	if (!dma32_usable) {
		level = get_current_dma32_usable();
	}

	if (list_empty(&ev_lst->list) || ev_lst->curr == level)
		return;

	pr_debug("mri_helper: %d -> %d\n", ev_lst->curr, level);
...
static void mri_event_work(struct mri_stats_item *item)
{
	if (item) {
		...
		mri_event_nr_unevictable(global_node_page_state(NR_UNEVICTABLE) -
				global_zone_page_state(NR_MLOCK));
		mri_event_lowmem_level();
		mri_event_psi();
		mri_event_dma32_usable(item->dma32_usable);
		mri_event_swap_free(item->usable.swap);
	} else {
		mri_event_level(0);
		...
		mri_event_lowmem_level();
		mri_event_psi();
		mri_event_dma32_usable(0);
		mri_event_swap_free(0);
	}
}
...
static int _event_cmp(void *priv, const struct list_head *a, const struct list_head *b)
{
...
static int mri_register_event(struct eventfd_ctx *efd, int type, char *buf)
{
	...
	case MRI_EVENT_DMA32_USABLE:
	case MRI_EVENT_SWAP_FREE:
...
static void __mri_do_profile(struct mri_stats_item *item, long long delays)
{
	...
	for_each_populated_zone(zone) {
		...
		item->dma32_usable = ui->usable;
		if (zi == ZONE_MOVABLE) {
			item->dma32_usable -= zone_page_state(zone, NR_FREE_PAGES);
		}
		...
		#endif
		item->node_dirty_ok[zi] = (int)node_dirty_ok(zone->zone_pgdat);
	}

	/* vm zone stat */
	for (i = NR_FREE_PAGES; i < NR_VM_ZONE_STAT_ITEMS; i++)
		vmstat[i] = global_zone_page_state(i);
	vmstat += NR_VM_ZONE_STAT_ITEMS;

#ifdef CONFIG_NUMA
	for (i = 0; i < NR_VM_NUMA_EVENT_ITEMS; i++)
		vmstat[i] = global_numa_event_state(i);
	vmstat += NR_VM_NUMA_EVENT_ITEMS;
#endif

	/* vm node stat */
	for (i = 0; i < NR_VM_NODE_STAT_ITEMS; i++) {
		vmstat[i] = global_node_page_state_pages(i);
		if (vmstat_item_print_in_thp(i))
			vmstat[i] /= HPAGE_PMD_NR;
	}
	vmstat += NR_VM_NODE_STAT_ITEMS;

	/* dirty limits */
	global_dirty_limits(vmstat + NR_DIRTY_BG_THRESHOLD,
			    vmstat + NR_DIRTY_THRESHOLD);
	vmstat[NR_MEMMAP_PAGES] = get_nr_memmap_pages();
	vmstat[NR_MEMMAP_BOOT_PAGES] = get_nr_memmap_boot_pages();
	vmstat += NR_VM_STAT_ITEMS;
...
static const struct proc_ops mri_high_order_alloc_proc_fops = {
	.proc_write		= mri_high_order_alloc_proc_write,
};
...
static const struct proc_ops mri_delays_proc_fops = {
	.proc_open		= mri_delays_proc_open,
	.proc_read		= seq_read,
...
static int mri_stats_show(struct seq_file *m, void *arg)
{
	...
	mri_printf(m, "dma32_usable %lu\n", item->dma32_usable);

	/* vmstat */
	stat_items_size = NR_VMSTAT_ITEMS;
#ifdef CONFIG_VM_EVENT_COUNTERS
	...
	for_each_populated_zone(z) {
		...
		mri_printf(m, "node_dirty_ok %s\n",
			   item->node_dirty_ok ? "ok" : "nok");
	}
...
static const struct proc_ops mri_stats_fops = {
	.proc_open	= mri_stats_open,
	.proc_read	= seq_read,
	.proc_lseek	= seq_lseek,
				      ...
				      loff_t *pos)
{
	...
	eventfd = eventfd_ctx_fileget(fd_file(efile));
	if (IS_ERR(eventfd)) {
		ret = PTR_ERR(eventfd);
		goto out;
	}
...
static const struct proc_ops mri_control_proc_fops = {
	.proc_write		= mri_control_proc_write,
};
...
static const struct proc_ops mri_snap_proc_fops = {
	.proc_open	= mri_snap_proc_open,
	.proc_read	= seq_read,
	.proc_lseek	= seq_lseek,
	.proc_release	= single_release,
};
...
static const struct proc_ops mri_ratio_proc_fops = {
	.proc_read		= mri_ratio_read,
	.proc_write		= mri_ratio_write,
...
static const struct proc_ops mri_reset_proc_fops = {
	.proc_write		= mri_reset_proc_write,
};
...
static const struct proc_ops mri_freeze_proc_fops = {
	.proc_open		= mri_freeze_open,
	.proc_read		= seq_read,
	.proc_lseek		= seq_lseek,
	.proc_write		= mri_freeze_write,
	.proc_release	= single_release,
};
#endif /* CONFIG_PROC_FS */

#ifdef CONFIG_MAGIC_SYSRQ
static void sysrq_handle_show_mri_buffer(u8 key)
{
	mutex_lock(&mri_lock);
