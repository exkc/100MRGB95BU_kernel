...
)
{
	...
	retc = DEMOD_L1_Set_Property(frontend->demod, DEMOD_PROP_DVBS_OPMSEL_CODE);
	if(retc != NO_DEMOD_ERROR)
	{
		DEMOD_ERROR("FAIL, DEMOD_L1_Set_Property(DEMOD_PROP_DVBS_OPMSEL_CODE) 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
		return retc;
	}
		...
		case DEMOD_MODULATION_DVBS2:
			{
				...
				if(frontend->demod->prop->dd_dvbs_opm_sel.opmSel == DEMOD_DD_DVBS_OPM_SEL_PROP_NORMAL_MODE)
					frontend->demod->cmd->dvbs_sym_rate.srd_en_s       = DEMOD_CMD_DVBC_SRD_AUTO_EN_DISABLE;
				else if(frontend->demod->prop->dd_dvbs_opm_sel.opmSel == DEMOD_DD_DVBS_OPM_SEL_PROP_BLIND_LOCK_MODE)
					frontend->demod->cmd->dvbs_sym_rate.srd_en_s       = DEMOD_CMD_DVBC_SRD_AUTO_EN_DISABLE;// = DEMOD_CMD_DVBC_SRD_AUTO_EN_ENABLE;
				else
					frontend->demod->cmd->dvbs_sym_rate.srd_en_s       = DEMOD_CMD_DVBC_SRD_AUTO_EN_ENABLE;

				frontend->demod->cmd->dvbs_sym_rate.sym_rate =	symbol_rate_Kbps ;

				if (frontend->auto_detect_SAT)
				{
					DEMOD_NOTI("DVB-S/S2 auto detect Enable\n");
					frontend->demod->prop->dd_auto_mode.auto_detect = DEMOD_PROP_AUTO_MODE_DET_DVBS_DVBS2_ENABLE;
				...
				{
					DEMOD_NOTI("DVB-S/S2 auto detect Disable\n");
					if (standard == DEMOD_MODULATION_DVBS )
					{
						frontend->demod->prop->dd_auto_mode.auto_detect = DEMOD_PROP_AUTO_MODE_DET_DVBS_ONLY_ENABLE;
					}
					else if(standard == DEMOD_MODULATION_DVBS2)
					{
						frontend->demod->prop->dd_auto_mode.auto_detect = DEMOD_PROP_AUTO_MODE_DET_DVBS2_ONLY_ENABLE;
					}
					retc = DEMOD_L1_Set_Property(frontend->demod, DEMOD_PROP_AUTO_MODE_CODE);
					if(retc != NO_DEMOD_ERROR)
					{
...
int DEMOD_L2_SAT_send_diseqc_sequence(LX_DEMOD_L2_Context * frontend, UINT8 sequence_length, UINT8 *sequenceBuffer, UINT8 cont_tone, UINT8 tone_burst, UINT8 burst_sel, UINT8 repeatEn)
{
	...
	{
		frontend->demod->cmd->dd_diseqc_send.diseqc_en  = DEMOD_CMD_DISEQC_SEND_DISEQC_ENABLE;
		frontend->demod->cmd->dd_diseqc_send.msg_length = sequence_length - 3;
	}
	else
	{
		frontend->demod->cmd->dd_diseqc_send.diseqc_en  = DEMOD_CMD_DISEQC_SEND_DISEQC_DISABLE;
	...
	frontend->demod->cmd->dd_diseqc_send.cont_tone  = (cont_tone == 1) ? DEMOD_CMD_DISEQC_SEND_CONT_TONE_ENABLE : DEMOD_CMD_DISEQC_SEND_CONT_TONE_DISABLE ;
	frontend->demod->cmd->dd_diseqc_send.tone_burst =  (tone_burst == 1) ? DEMOD_CMD_DISEQC_SEND_TONE_BURST_ENABLE : DEMOD_CMD_DISEQC_SEND_TONE_BURST_DISABLE;
	frontend->demod->cmd->dd_diseqc_send.burst_sel  =  (burst_sel == 1) ? DEMOD_CMD_DISEQC_SEND_BURST_SEL_MODULATED : DEMOD_CMD_DISEQC_SEND_BURST_SEL_UNMODUALTED;
/*
...
int DEMOD_L2_SAT_Blind_Info (LX_DEMOD_L2_Context * frontend)
{
	...
	if(retc != NO_DEMOD_ERROR)
	{
		DEMOD_ERROR("FAIL, DEMOD_L1_GET_DD_SIGSRCH_INFO() 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
		return retc;
	}
...
int DEMOD_L2_SAT_Blind_Init (LX_DEMOD_L2_Context * frontend, UINT32 startFreq, UINT32 endFreq)
{
	...
	startFreq = 970000;
	DEMOD_NOTI("StartFreq : %d, EndFreq : %d\n", startFreq, endFreq);
	retc = DEMOD_L1_GET_DD_BLIND_Init(frontend->demod, startFreq, endFreq);
	if(retc != NO_DEMOD_ERROR)
	{
		DEMOD_ERROR("FAIL, DEMOD_L1_GET_DD_BLIND_Init() 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
		return retc;
	}
	return retc;
}
...
int DEMOD_L2_SAT_Blind_End (LX_DEMOD_L2_Context * frontend)
{
	...
	if(retc != NO_DEMOD_ERROR)
	{
		DEMOD_ERROR("FAIL, DEMOD_L1_GET_DD_BLIND_End() 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
		return retc;
	}
	return retc;
}
...
int DEMOD_L2_SAT_Blind_FoundInfo (LX_DEMOD_L2_Context * frontend)
{
	...
	if(retc != NO_DEMOD_ERROR)
	{
		DEMOD_ERROR("FAIL, DEMOD_L1_GET_DD_BLIND_FoundInfo() 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
		return retc;
	}
	return retc;
}
...
int DEMOD_L2_SAT_Blind_SymbolRate_Detect (LX_DEMOD_L2_Context * frontend)
{
	...
	retc = DEMOD_L1_SET_DD_SymbolRate_Detect(frontend->demod, DEMOD_CMD_DVBC_SRD_AUTO_EN_ENABLE, frontend->demod->rsp->dd_sigsrch_info.bw_idx);
	if(retc != NO_DEMOD_ERROR)
	{
		DEMOD_ERROR("FAIL, DEMOD_L1_SET_DD_SymbolRate_Detect() 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
		return retc;
