...
extern int BE_PWM_Ioctl(unsigned int cmd, unsigned long arg);

extern void BE_PROC_Init(void);
extern void BE_PROC_Cleanup(void);
...
static int		BE_Ioctl	(struct inode *inode, struct file *filp, UINT32 cmd, ULONG arg);
#else
static long		BE_Ioctl	(struct file *filp, unsigned int cmd, unsigned long arg);
#ifdef CONFIG_COMPAT
static long		BE_CompatIoctl	(struct file *filp, unsigned int cmd, unsigned long arg);
#endif
#endif
...
static int BE_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (lx_chip() >= LX_CHIP_O20)
	{
		...
		if(BE_LED_HAL_Suspend())
		{
			printk("BE - suspend FAIL\n");
			return -1;
		}
	}
...
static int BE_resume(struct platform_device *pdev)
{
	...
	if (BE_TOP_HAL_Resume())
	{
		printk("BE - resume FAIL\n");
	}

	if (BE_FRC_HAL_Resume())
	{
		printk("BE - resume FAIL\n");
	}
...
static void BE_remove(struct platform_device *pdev)
#else
 ...
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
static int BE_Ioctl ( struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg )
#else
static long BE_Ioctl ( struct file *filp, unsigned int cmd, unsigned long arg )
#endif
{
	...
	else if((_IOC_NR(cmd)) < BE_IOC_LED_MAX)	{ret = OS_LockMutex(&gs_be_led_mtx); ret = BE_LED_Ioctl(cmd,arg); OS_UnlockMutex(&gs_be_led_mtx);}
	else if((_IOC_NR(cmd)) < BE_IOC_PWM_MAX)	{ret = OS_LockMutex(&gs_be_pwm_mtx); ret = BE_PWM_Ioctl(cmd,arg); OS_UnlockMutex(&gs_be_pwm_mtx);}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
#ifdef CONFIG_COMPAT
static long BE_CompatIoctl(struct file *filp, unsigned int cmd,
						   unsigned long arg)
{
	long ret;
	...
	{
		BE_FW_DWLD_T arg_buf;

		ret = copy_from_user(&arg_buf, (void *)arg, sizeof(BE_FW_DWLD_T));
		if (ret)
		{
			return -EINVAL;
		}

		arg_buf.pData = (CHAR *)(uintptr_t)arg_buf.compat_pData;

		ret = copy_to_user((void *)arg, &arg_buf, sizeof(BE_FW_DWLD_T));
		if (ret)
		{
			return -EINVAL;
		}
	...
	{
		LX_BE_WCG_CALC_FW_LUT_SIZE_T arg_buf;

		ret = copy_from_user(&arg_buf, (void *)arg,
							 sizeof(LX_BE_WCG_CALC_FW_LUT_SIZE_T));
		if (ret)
		{
			return -EINVAL;
		}

		arg_buf.wcgLut.pu16SectionData =
			(UINT16 *)(uintptr_t)arg_buf.wcgLut.compat_pu16SectionData;
		arg_buf.wcgLut.regData.pu32Data =
			(UINT32 *)(uintptr_t)arg_buf.wcgLut.regData.compat_pu32Data;
	...
	{
		LX_BE_WCG_FW_LUT_T arg_buf;

		ret = copy_from_user(&arg_buf, (void *)arg, sizeof(LX_BE_WCG_FW_LUT_T));
		if (ret)
		{
			return -EINVAL;
		}

		arg_buf.fwWcgLutData = (CHAR *)(uintptr_t)arg_buf.compat_fwWcgLutData;
		arg_buf.wcgLut.pu16SectionData =
			(UINT16 *)(uintptr_t)arg_buf.wcgLut.compat_pu16SectionData;
		arg_buf.wcgLut.regData.pu32Data =
			(UINT32 *)(uintptr_t)arg_buf.wcgLut.regData.compat_pu32Data;

		ret = copy_to_user((void *)arg, &arg_buf, sizeof(LX_BE_WCG_FW_LUT_T));
		if (ret)
		{
			return -EINVAL;
		}
	...
	{
		LX_BE_PQ_DPG_T arg_buf;

		ret = copy_from_user(&arg_buf, (void *)arg, sizeof(LX_BE_PQ_DPG_T));
		if (ret)
		{
			return -EINVAL;
		}

		arg_buf.rDpgLut.pData =
			(UINT32 *)(uintptr_t)arg_buf.rDpgLut.compat_pData;
		arg_buf.gDpgLut.pData =
			(UINT32 *)(uintptr_t)arg_buf.gDpgLut.compat_pData;
		arg_buf.bDpgLut.pData =
			(UINT32 *)(uintptr_t)arg_buf.bDpgLut.compat_pData;

		ret = copy_to_user((void *)arg, &arg_buf, sizeof(LX_BE_PQ_DPG_T));
		if (ret)
		{
			return -EINVAL;
		}
	...
	case BE_IOW_SET_LSR_PARAM:
	{
		...
		ret = copy_from_user(&arg_buf, (void *)arg, sizeof(LX_BE_LSR_T));
		if (ret)
		{
			return -EINVAL;
		}

		arg_buf.lsrTbl.pu32Data =
			(UINT32 *)(uintptr_t)arg_buf.lsrTbl.compat_pu32Data;

		ret = copy_to_user((void *)arg, &arg_buf, sizeof(LX_BE_LSR_T));
		if (ret)
		{
			return -EINVAL;
		}
	...
	case BE_IOW_SET_GSR:
	{
		...
		ret = copy_from_user(&arg_buf, (void *)arg, sizeof(LX_BE_GSR_T));
		if (ret)
		{
			return -EINVAL;
		}

		arg_buf.gsrTbl.pu32Data =
			(UINT32 *)(uintptr_t)arg_buf.gsrTbl.compat_pu32Data;

		ret = copy_to_user((void *)arg, &arg_buf, sizeof(LX_BE_GSR_T));
		if (ret)
		{
			return -EINVAL;
		}
	...
	case BE_IOR_GET_OSD_ALPHA_APL:
	{
		...
		ret = copy_from_user(&arg_buf, (void *)arg, sizeof(LX_BE_LUT_T));
		if (ret)
		{
			return -EINVAL;
		}
		...
		arg_buf.pu32Data =
			(UINT32 *)(uintptr_t)arg_buf.compat_pu32Data;

		ret = copy_to_user((void *)arg, &arg_buf, sizeof(LX_BE_LUT_T));
		if (ret)
		{
			return -EINVAL;
		}
	...
	{
		LX_BE_OSD_RGB_LEVEL_T arg_buf;

		ret = copy_from_user(&arg_buf, (void *)arg, sizeof(LX_BE_OSD_RGB_LEVEL_T));
		if (ret)
		{
			return -EINVAL;
		}

		arg_buf.levelTbl.pu32Data =
			(UINT32 *)(uintptr_t)arg_buf.levelTbl.compat_pu32Data;

		ret = copy_to_user((void *)arg, &arg_buf, sizeof(LX_BE_OSD_RGB_LEVEL_T));
		if (ret)
		{
			return -EINVAL;
		}
	...
	{
		LX_BE_IRR_READ_T arg_buf;

		ret = copy_from_user(&arg_buf, (void *)arg, sizeof(LX_BE_IRR_READ_T));
		if (ret)
		{
			return -EINVAL;
		}

		arg_buf.irrData.pu32Data =
			(UINT32 *)(uintptr_t)arg_buf.irrData.compat_pu32Data;

		ret = copy_to_user((void *)arg, &arg_buf, sizeof(LX_BE_IRR_READ_T));
		if (ret)
		{
			return -EINVAL;
		}
	...
	{
		LX_BE_PQ_LUT_T arg_buf;

		ret = copy_from_user(&arg_buf, (void *)arg, sizeof(LX_BE_PQ_LUT_T));
		if (ret)
		{
			return -EINVAL;
		}
		...
		arg_buf.pData =
			(UINT32 *)(uintptr_t)arg_buf.compat_pData;

		ret = copy_to_user((void *)arg, &arg_buf, sizeof(LX_BE_PQ_LUT_T));
		if (ret)
		{
			return -EINVAL;
		}
...
int BE_Init(void)
{
	...
	OS_DEBUG_EnableModuleByIndex ( g_be_debug_fd, BE_DBG_PRT_IDX_NOTI, DBG_COLOR_NONE );	// BE_NOTI
	...
	else
	{
		...
		else
		{
			...
			be_device.dev.parent = &de_get_platform_device()->dev;
#endif
	...
	OS_InitMutex(&gs_be_pwm_mtx, OS_SEM_ATTR_DEFAULT);
