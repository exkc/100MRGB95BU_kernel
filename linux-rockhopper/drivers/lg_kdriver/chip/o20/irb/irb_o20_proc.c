...
static OS_PROC_DESC_TABLE_T	_g_irb_o20_device_proc_table[] =
{
...
static int o20_write_proc( UINT32 procId, char* command )
{
	...
	switch( procId )
	{
		case PROC_ID_TX:
			...
			irb_o20_func->tx(vendor,opt,key);
			#endif
			break;
		default:
			ret = -1;
 ...
 * @see IRB_Init
*/
void o20_irb_proc_init (void)
{
	irb_o20_func = get_o20_irb_func();


	OS_PROC_CreateEntryEx ( IRB_MODULE, _g_irb_o20_device_proc_table,
										o20_read_proc,
										o20_write_proc );
}
 ...
 * @see IRB_Cleanup
*/
void o20_irb_proc_cleanup (void)
{
	OS_PROC_RemoveEntry( IRB_MODULE );
}

static irbs_proc_t o20_irb_proc_func =
{
	.init = o20_irb_proc_init,
	.cleanup = o20_irb_proc_cleanup,
};

irbs_proc_t* get_o20_irb_proc(void)
{
	return &o20_irb_proc_func;
}
