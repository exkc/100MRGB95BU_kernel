...
int _DEMOD_O20_DVBS_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int _DEMOD_O20_DVBS2_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int _DEMOD_O20_DVBS_OperatingMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  operatingMode;
...
int _DEMOD_O20_DVBS2_OperatingMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  operatingMode;
...
int _DEMOD_O20_LGDBB_DVBS_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	...
	new_tuneMode 		= _DEMOD_O20_DVBS_CheckTuneMode(pDemodInfo);
	operatingMode 		= _DEMOD_O20_DVBS_OperatingMode(pDemodInfo);
...
int _DEMOD_O20_LGDBB_DVBS2_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
//	UINT16 			dvb_s2_bandwidth_Khz;
	...
	new_tuneMode 		= _DEMOD_O20_DVBS2_CheckTuneMode(pDemodInfo);
	operatingMode 		= _DEMOD_O20_DVBS2_OperatingMode(pDemodInfo);
...
int _DEMOD_O20_LGDBB_SAT_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O20_DVBS_Set_Config_auto
...
*/
int DEMOD_O20_DVBS_Set_Config_auto (BOOLEAN bAutoDetect)
{
/*
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O20_DVBS_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when LX_DEMOD_O20_DVBS_Set_Config_auto is disabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DVBS_Set_FullConfig (LX_DEMOD_DVBS_CONFIG_T *pDvbsConfigParam)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DVBS_Get_CoderRate (LX_DEMOD_CODERATE_T *pCodeRate)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DVBS_Get_SymbolRate (UINT32 *pSymbolRate )
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DVBS_Get_TotalInfo (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
	...
	if (RET_OK != DEMOD_O20_Get_NeverLockStatus(LX_DEMOD_DVBS, &neverLockStatus))
	{
...
************************************************************************************/
/**
 * DEMOD_O20_DVBS_Set_DiseqcToneMessageCtrl
 * Get the information of symbol rate on DVB-S.
 *
 * @param *pLpCodeRate[OUT] LX_DEMOD_CODERATE_T - code rate : 1/2, 2/3, 3/4, 5/6 and 7/8.
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DVBS_Set_DiseqcToneMessageCtrl (LX_DEMOD_DVBS_DiSEqC_MSG_T *pDiseqcMsg)
{
	LX_DEMOD_L2_Context *	pFrontend;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DVBS_Get_BlindInfo(UINT32 centerFreq, LX_DEMOD_DVBS_BLIND_PARAM_T *pBlindParam)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_DVBS_Obtaining_Signal_Lock.
...
int DEMOD_O20_DVBS_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_TUNE_START:
			{
				...
				if(RET_OK != _DEMOD_O20_LGDBB_DVBS_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, _DEMOD_O20_LGDBB_DVBS_SetDemod() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
		...
		case LX_DEMOD_SYNC_CHECKING :
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					DEMOD_PRINT( "^R^[DEMOD O20] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_SIGNAL_STABLE..... ^0^\n");
				}
				else
				{
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE:
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if (RET_OK != _DEMOD_O20_LGDBB_SAT_CheckSignalState(pDemodInfo))
					{
						DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, _DEMOD_O20_LGDBB_SAT_CheckSignalState() \n");
						DEMOD_RETURN_FAIL(__F__);
					}
				...
				{
//					pDemodInfo->controlState	= LX_DEMOD_SYNC_CHECKING;
//					DEMOD_PRINT( "^R^[DEMOD O20] LX_DEMOD_SYNC_CHECKING : Going back to LX_DEMOD_SYNC_CHECKING..... **;\n");
				}
	   		}
			break;
...
************************************************************************************/
/**
 * DEMOD_O20_DVBS_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O20_DVBS_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
...
************************************************************************************/
/**
 * DEMOD_O20_DVBS2_Set_Config_auto
 * Control automatic setting of DVB-T config.
 *
 * @param bAutoDetect 	[IN] BOOLEAN - automatic setting (TRUE) / manual setting(FALSE).
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DVBS2_Set_Config_auto (BOOLEAN bAutoDetect)
{
...
************************************************************************************/
/**
 * DEMOD_O20_DVBS2_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when LX_DEMOD_O20_DVBS_Set_Config_auto is disabled.
 *
 * @param *pDvbtConfigParam 	[IN] LX_DEMOD_DVBS_CONFIG_T - DVB-T config. structure
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DVBS2_Set_FullConfig (LX_DEMOD_DVBS2_CONFIG_T *pDvbs2ConfigParam)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DVBS2_Get_CoderRate (LX_DEMOD_CODERATE_T *pCodeRate)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DVBS2_Get_Pilot (BOOLEAN *pPilot)
{
	*pPilot = FALSE;
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_DVBS2_Get_SymbolRate
 * Get the information of symbol rate on DVB-S.
 *
 * @param *pLpCodeRate[OUT] LX_DEMOD_CODERATE_T - code rate : 1/2, 2/3, 3/4, 5/6 and 7/8.
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DVBS2_Get_SymbolRate (UINT32 *pSymbolRate)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_DVBS2_Get_TotalInfo (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
	...
	if (RET_OK != DEMOD_O20_Get_NeverLockStatus(LX_DEMOD_DVBS2, &neverLockStatus))
	{
		DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_Get_NeverLockStatus() \n");
		neverLockStatus = LX_DEMOD_UNKOWN_STATE;
	}
...
//	DEMOD_PRINT("[DEMOD O20] AGC LOCK is %s \n",(agcLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD_CHIP] IF AGC value = %d(0x%x)\n",ifagcValue,ifagcValue);
	DEMOD_PRINT("[DEMOD_CHIP] Neverlock is %s\n",(neverLockStatus == LX_DEMOD_DETECTED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD_CHIP] Spectrum auto = %s, Inversion  = %s\n",(spectrumAuto == LX_DEMOD_AUTO) ? " Auto" : " Manual",
																					(spectrumInv == LX_DEMOD_INVERSION) ? " Inverted" : " Normal" );
...
************************************************************************************/
/**
 * DEMOD_O20_DVBS2_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O20_DVBS2_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	static LX_DEMOD_FLAG_T 		scanProcessing = LX_DEMOD_FLAG_START;
	static LX_DEMOD_LOCK_STATE_T 	lockState = LX_DEMOD_UNLOCKED;
	static unsigned int 		sigStablestartTime = 0;
	...
	switch (pDemodInfo->controlState)
	{
		...
		case LX_DEMOD_TUNE_START:
			{
				if(RET_OK != _DEMOD_O20_LGDBB_DVBS2_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, _DEMOD_O20_LGDBB_DVBS2_SetDemod() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.dvbs2.tuneMode)
					{
						...
						else
						{
							...
							if(RET_OK != DEMOD_O20_Get_CarrierFreqOffset(&freqOffset))
							{
								DEMOD_ERROR("[DEMOD_CHIP] FAIL, DEMOD_O20_Get_CarrierFreqOffset() \n");
								return RET_ERROR;
							}
...
************************************************************************************/
/**
 * DEMOD_O20_DVBS2_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O20_DVBS2_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_LOCK_STATE_T			lockState;
	LX_DEMOD_OPER_MODE_T 			operMode;
...
************************************************************************************/
/**
 * DEMOD_O20_DVBSX_BlindScan_Init
...
*/
int DEMOD_O20_DVBSX_BlindScan_Init (LX_DEMOD_BLIND_SCAN_INIT_FREQ_PARAM_T *initFreq)
{
	LX_DEMOD_L2_Context *	pFrontend;
	...
	DEMOD_PRINT("[DEMOD_CHIP] DEMOD_O20_DVBSX_BlindScan_Init startFreqKhz : %d, endFreqKhz : %d\n", initFreq->startFreqKhz, initFreq->endFreqKhz);
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_O20_DVBSX_BlindScan_End (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_O20_DVBSX_Get_TunerFreq (LX_DEMOD_BLIND_SCAN_FREQ_PARAM_T *freqInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O20_DVBSX_Get_OffsetFreq
 * Get Tuner Freq infomration on DVB-S/S2.
 *
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_O20_DVBSX_Get_OffsetFreq (LX_DEMOD_BLIND_SCAN_OFFSET_PARAM_T *pOffsetInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O20_DVBSX_SymbolRate_Detec
 * Notification Blind Scan Init on DVB-S/S2.
 *
 * @param None
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_O20_DVBSX_SymbolRate_Detec (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O20_DVBSX_BlindLock_Loop
 * Notification Blind Scan Init on DVB-S/S2.
 *
 * @param None
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_O20_DVBSX_BlindLock_Loop (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O20_DVBSX_Get_Nextfreq_Status
 * Get Tuner Freq infomration on DVB-S/S2.
 *
 * @param SINT32* freqOff
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_O20_DVBSX_Get_Nextfreq_Status (BOOLEAN* pBlindScanEnd)
{
	LX_DEMOD_L2_Context *	pFrontend;
