...
static u32 internal_pullup;
static u32 reset_reg;
...
static int lg1k_dwc_phy_init(void)
{
	if (internal_pullup == 0) {
		...
		writel(0x00880000, emmc_base + EMMC_VENDOR2_BASE);
		writel(0x04490449, emmc_base + EMMC_VENDOR2_BASE + 0x4);
		writel(0x04510441, emmc_base + EMMC_VENDOR2_BASE + 0x8);
		writel(0x00000449, emmc_base + EMMC_VENDOR2_BASE + 0xC);
		writel(0x00880001, emmc_base + EMMC_VENDOR2_BASE);
		writel(0x00000200, emmc_base + EMMC_VENDOR2_BASE + 0x1C);
		writel(0x0000001A, emmc_base + EMMC_VENDOR2_BASE + 0x20);
		writel(0x00000000, emmc_base + EMMC_VENDOR1_BASE + 0x40);
...
void lg1k_emmc_ip_reset(void)
{
	...
	offset = reset_reg & 0xFFF;
	base = reset_reg & 0xFFFFF000;
	dwc_syntop = ioremap(base, 4096);

	reg = readl(dwc_syntop + offset) | reset_value;
...
static int __init lg1k_ll_mmc_init(void)
{
	...
	if (of_property_read_u32(np, "internal-pullup", &internal_pullup) < 0)
		internal_pullup = 0x0;

	ret = of_property_read_u32(np, "reset-reg", &reset_reg);
	if ((ret < 0) || reset_reg == 0) {
#define O22_EMMC_CNTROLLER_SYNTOP	0xc805000c
#define M23_EMMC_CNTROLLER_SYNTOP	0xc475000c
#define O24_EMMC_CNTROLLER_SYNTOP	0xC424000c
#define O26_EMMC_CNTROLLER_SYNTOP	0xC621000c
		if (of_machine_is_compatible("lge,lg1213"))
			reset_reg = O22_EMMC_CNTROLLER_SYNTOP;

		if (of_machine_is_compatible("lge,lg1315"))
			reset_reg = M23_EMMC_CNTROLLER_SYNTOP;

		if (of_machine_is_compatible("lge,lg1214"))
			reset_reg = O24_EMMC_CNTROLLER_SYNTOP;

		if (of_machine_is_compatible("lge,lg1215"))
			reset_reg = O26_EMMC_CNTROLLER_SYNTOP;
	}

#define EMMC_DEFAULT_RESET_VALUE	0xf
	if (of_property_read_u32(np, "reset-value", &reset_value) < 0) {
		reset_value = EMMC_DEFAULT_RESET_VALUE;
	}
