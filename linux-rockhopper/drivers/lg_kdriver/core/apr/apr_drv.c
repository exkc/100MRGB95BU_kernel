...
#ifdef CONFIG_COMPAT
static long	APR_CompatIoctl(struct file *, unsigned int, unsigned long );
#endif
#endif
...
#ifdef CONFIG_COMPAT
static long
APR_CompatIoctl ( struct file *filp, unsigned int cmd, unsigned long arg )
{
	long ret;

	ret = APR_Ioctl(filp, cmd, arg);
...
//	OS_DEBUG_EnableModuleByIndex ( g_apr_debug_fd, LX_LOGM_LEVEL_TRACE, DBG_COLOR_GRAY );
	...
	else
	{
		...
		else
		{
			...
			apr_device.dev.parent = &de_get_platform_device()->dev;
