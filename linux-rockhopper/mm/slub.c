...
static inline void set_freepointer(struct kmem_cache *s, void *object, void *fp)
{
	...
	if (is_slab_readonly(s->flags)) {
		freeptr_t ptr = freelist_ptr_encode(s, fp, freeptr_addr);
#ifdef CONFIG_SLAB_FREELIST_HARDENED
		ekp_slab_set_fp(__pa(freeptr_addr), fp, (void *)ptr.v,
				s->random, swab(freeptr_addr));
#else
		ekp_slab_set_fp(__pa(freeptr_addr), fp, (void *)ptr.v, 0, 0);
#endif
	} else {
 ...
 */
#ifdef CONFIG_EKP
static inline struct slab *alloc_ro_slab_page(gfp_t flags, int node,
		struct kmem_cache_order_objects oo)
{
	...
	if (WARN_ON(node != NUMA_NO_NODE)) {
		pr_err("%s: EKP is not support NUMA yet!\n", __func__);
		return NULL;
	}

	folio = (struct folio *)ekp_alloc_ro_pages(flags, order);
	if (!folio)
		return NULL;
...
static struct slab *allocate_slab(struct kmem_cache *s, gfp_t flags, int node)
{
	...
	if (is_slab_readonly(s->flags))
		slab = alloc_ro_slab_page(alloc_gfp, node, oo);
	else
...
static void __free_slab(struct kmem_cache *s, struct slab *slab)
{
	...
	if (is_slab_readonly(s->flags)) {
		__ekp_free_ro_pages(&folio->page, order);
		return;
	}
			  ...
			  unsigned int orig_size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (p[i] && init && (!kasan_init ||
			...
			if (is_slab_readonly(s->flags))
				ekp_romemset_zero(__pa(p[i]), zero_size);
			else
