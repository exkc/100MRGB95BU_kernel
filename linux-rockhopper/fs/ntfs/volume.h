...
static inline void ntfs_inc_free_clusters(ntfs_volume *vol, s64 nr)
{
...
static inline void ntfs_dec_free_clusters(ntfs_volume *vol, s64 nr)
{
	if (!NVolFreeClusterKnown(vol))
		wait_event(vol->free_waitq, NVolFreeClusterKnown(vol));
...
static inline void ntfs_inc_free_mft_records(ntfs_volume *vol, s64 nr)
{
	if (!NVolFreeClusterKnown(vol))
		return;
...
static inline void ntfs_dec_free_mft_records(ntfs_volume *vol, s64 nr)
{
	if (!NVolFreeClusterKnown(vol))
		return;
...
static inline void ntfs_set_lcn_empty_bits(ntfs_volume *vol, unsigned long index,
...
static __always_inline void ntfs_hold_dirty_clusters(ntfs_volume *vol, s64 nr_clusters)
{
	atomic64_add(nr_clusters, &vol->dirty_clusters);
}

static __always_inline void ntfs_release_dirty_clusters(ntfs_volume *vol, s64 nr_clusters)
{
