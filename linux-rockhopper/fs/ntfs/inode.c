 ...
 * Author(s): Namjae Jeon <linkinjeon@kernel.org>, Hyunchul Lee <hyc.lee@gmail.com>.
 */
...
ntfschar R[3] = { cpu_to_le16('$'), cpu_to_le16('R'), 0 };
...
view_index_meta:
		...
		if (a->non_resident) {
			...
			vi->i_size = ni->data_size = sle64_to_cpu(
...
{
	ntfs_volume *vol = NTFS_SB(vi->i_sb);
	ntfs_inode *ni = NTFS_I(vi), *base_ni = NTFS_I(base_vi);
	MFT_RECORD *m;
	ATTR_RECORD *a;
	ntfs_attr_search_ctx *ctx;
	int err = 0;
	...
	if (!a->non_resident) {
		...
		vi->i_size = ni->data_size = sle64_to_cpu(a->data.non_resident.data_size);
		ni->initialized_size = sle64_to_cpu(
				a->data.non_resident.initialized_size);
		ni->allocated_size = sle64_to_cpu(
				a->data.non_resident.allocated_size);
...
static int ntfs_read_locked_index_inode(struct inode *base_vi, struct inode *vi)
{
	loff_t bvi_size;
	ntfs_volume *vol = NTFS_SB(vi->i_sb);
	ntfs_inode *ni = NTFS_I(vi), *base_ni = NTFS_I(base_vi), *bni;
	struct inode *bvi;
	MFT_RECORD *m;
	ATTR_RECORD *a;
	ntfs_attr_search_ctx *ctx;
 ...
 */
static int load_attribute_list_mount(ntfs_volume *vol, runlist_element *rl, u8 *al_start,
...
int ntfs_read_inode_mount(struct inode *vi)
{
	...
	mft_idx = (vol->mft_lcn << vol->cluster_size_bits) >> PAGE_SHIFT;
	...
	if (err) {
		...
		if (a->non_resident) {
			...
			err = load_attribute_list_mount(vol,
							rl,
							ni->attr_list,
							ni->attr_list_size,
							sle64_to_cpu(
...
static int ntfs_delete_base_inode(ntfs_inode *ni)
{
	...
	/* Deallocate clusters which belong to Non-resident attributes */
	while ((err = ntfs_attrs_walk(actx)) == 0) {
		if (actx->attr->non_resident) {
			...
			if (IS_ERR(rl)) {
				err = PTR_ERR(rl);
				ntfs_error(sb, "Failed to decompress runlist.  "
			...
			if (err) {
				ntfs_error(sb, "Failed to free clusters.  "
						"Leaving inconsistent metadata.\n");
			}
			ntfs_free(rl);
		}
	...
	if (err != -ENOENT) {
		ntfs_error(sb, "Attribute enumeration failed.  "
	...
	if (err) {
		ntfs_error(sb, "Failed to free base MFT record.  "
				"Leaving inconsistent metadata.\n");
	}
	return err;
}
 ...
 * Return 0 on success or -errno on error.
 */
static int ntfs_inode_sync_standard_information(struct inode *vi, MFT_RECORD *m)
{
	ntfs_inode *ni = NTFS_I(vi);
	ntfs_attr_search_ctx *ctx;
	STANDARD_INFORMATION *si;
	...
	if (si->creation_time != nt) {
		ntfs_debug("Updating creation time for inode 0x%lx: old = 0x%llx, "
 ...
 */
int ntfs_inode_sync_filename(ntfs_inode *ni)
{
	...
	while (!(err = ntfs_attr_lookup(AT_FILE_NAME, NULL, 0, 0, 0, NULL, 0, ctx))) {
		...
		err = ntfs_index_lookup(fn, sizeof(FILE_NAME_ATTR), ictx);
		if (err) {
		...
		if (ctx->mrec->flags & MFT_RECORD_IS_DIRECTORY)
			...
			read_lock_irqsave(&ni->size_lock, flags);
			if (NInoSparse(ni))
				fnx->allocated_size = cpu_to_sle64(ni->itype.compressed.size);
			else
	...
	if (base_ni->nr_extents > 0) {
		...
		for (i = 0; i < base_ni->nr_extents; i++) {
			...
			if (seq_no &&
				...
				ntfs_error(sb, "Found stale extent mft "
						"reference mft=%lld",
						(long long)ni->mft_no);
				unmap_mft_record(ni);
				goto out;
...
int ntfs_inode_add_attrlist(ntfs_inode *ni)
{
	...
	attr_al_len = offsetof(ATTR_RECORD, data.resident.reserved) + 1 +
		((al_len + 7) & ~7);
		...
		if (MREF_LE(ale->mft_reference) != ni->mft_no) {
			if (!ntfs_attr_lookup(ale->type, ale->name,
				...
				if (ntfs_attr_record_move_to(ctx, ni))
					ntfs_error(ni->vol->sb, "Rollback failed to "
							"move attribute");
			} else {
...
/**
 * ntfs_ext_inode_close - close an ntfs inode and free all associated memory
 ...
 * Return 0 on success or error.
 */
int ntfs_ext_inode_close(ntfs_inode *ni)
{
	int err = -1;
	ntfs_inode **tmp_nis;
	ntfs_inode *base_ni;
	...
	for (i = 0; i < base_ni->nr_extents; ++i) {
		...
		if (base_ni->nr_extents) {
			...
			tmp_nis = ntfs_realloc_nofs(tmp_nis, base_ni->nr_extents *
					sizeof(ntfs_inode *), base_ni->nr_extents *
					sizeof(ntfs_inode *));
...
void ntfs_destroy_ext_inode(ntfs_inode *ni)
{
	ntfs_debug("Entering.");
	if (ni == NULL)
		return;
...
static ntfs_inode *ntfs_inode_base(ntfs_inode *ni)
{
	if (ni->nr_extents == -1)
...
static int ntfs_attr_position(ATTR_TYPE type, ntfs_attr_search_ctx *ctx)
{
	int err;
					 ...
					 ntfs_attr_search_ctx *ctx)
{
	...
	if (pos + count > ni->data_size) {
		if (ntfs_attr_truncate(ni, pos + count)) {
			ntfs_error(sb, "Failed to truncate attribute");
			return -1;
		}
	...
	if (pos + count > ni->initialized_size) {
		ctx->attr->data.non_resident.initialized_size = cpu_to_sle64(pos + count);
						 ...
						 bool sync)
{
	...
	while (count) {
		...
		if (sync) {
			...
			lcn_count = max_t(LCN, 1, attr_len >> vol->cluster_size_bits);
...
void ntfs_sync_inode_size(ntfs_inode *ni)
{
	struct inode *vi;

	vi = VFS_I(ni);
	if (vi->i_size != (loff_t)ni->initialized_size)
		i_size_write(vi, (loff_t)ni->initialized_size);
}
