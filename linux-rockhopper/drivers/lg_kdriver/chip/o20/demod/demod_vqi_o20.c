 ...
 ************************************************************************************/
extern UINT8 gO20GBB_PowerOnInit;
unsigned int gO20GBB_PowerOnInitStartTimeISDBT = 0;

/***********************************************************************************
	Static Variable & Function Prototypes Declarations
 ************************************************************************************/
UINT16	_gO20GBB_VQI_UnlockCount_full_packeterr = 0;
UINT16 	_gO20GBB_FullVQIPkerrCount = 7;
UINT16 	_gO20GBB_FullVQPkerrThreshold = 300;
UINT16 	_gO20GBB_FullISDBTPkerrThreshold = 200;

BOOLEAN _gO20GBB_ISDBT_Mem_set = FALSE;
...
int DEMOD_O20_ATSC_SW_Init (void)
{
	...
	{
		DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_L2_ATSC_SW_Init() \n");
...
int _DEMOD_O20_LGDBB_VSB_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int _DEMOD_O20_VSB_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_O20_LGDBB_VSB_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	...
	new_tuneMode = _DEMOD_O20_VSB_CheckTuneMode(pDemodInfo);

	if(RET_OK != DEMOD_O20_Get_BW(pDemodInfo, &ter_bandwidth_KHz))
	{
		DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_Get_BW() \n");
		DEMOD_RETURN_FAIL(__F__);
	}
...
************************************************************************************/
/**
 * DEMOD_O20_VSB_Obtaining_Signal_Lock.
...
int DEMOD_O20_VSB_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_TUNE_START:
			{
				...
				if(RET_OK != DEMOD_O20_LGDBB_VSB_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_LGDBB_VSB_SetDemod() \n");
					DEMOD_RETURN_FAIL(__F__);
				}

				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.vsb.tuneMode)
				{
					if(RET_OK != DEMOD_O20_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_ON, &scanProcessing, &scanLock))
					{
						DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_NeverlockScan() \n");
						DEMOD_RETURN_FAIL(__F__);
					}
				...
				{
					if (RET_OK != DEMOD_O20_TPOutEnable(TRUE))
					{
						DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_TPOutEnable() \n");
						DEMOD_RETURN_FAIL(__F__);
					}

					pDemodInfo->scanJobFlag = LX_DEMOD_FLAG_FINISH;
					pDemodInfo->controlState = LX_DEMOD_SIGNAL_STABLE;
					DEMOD_PRINT( "[DEMOD_CHIP] LX_DEMOD_TUNE_START : Going to LX_DEMOD_SIGNAL_STABLE for SYNC CHECK DIRECTLY... ^0^\n");
				}
			}
			break;
		...
		case LX_DEMOD_TUNE_SCAN:
			{
				...
				if(RET_OK != DEMOD_O20_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_OFF, &scanProcessing, &scanLock))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_NeverlockScan() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if((LX_DEMOD_FLAG_FINISH == scanProcessing) && (LX_DEMOD_LOCKED == scanLock))
				{
					...
					DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] SCANNING FIRST LOCK! : Going to LX_DEMOD_SIGNAL_STABLE..... ^0^\n");
				}
				else if((LX_DEMOD_FLAG_FINISH == scanProcessing)&& (LX_DEMOD_UNLOCKED == scanLock))
				{
					...
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] LX_DEMOD_TUNE_SCAN : MSG_ATSC_FRONT_END_NOT_LOCKED..\n") ;
		...
		case LX_DEMOD_SYNC_CHECKING :
			{
				...
				if(RET_OK != DEMOD_O20_Get_Lock(LX_DEMOD_PSYNC_LOCK, &lockState))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_Get_Lock() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				{
					if(RET_OK != DEMOD_O20_Get_Lock(LX_DEMOD_CARR_LOCK, &lockState))
					{
						DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_Get_Lock() \n");
						DEMOD_RETURN_FAIL(__F__);
					}
					...
					if(LX_DEMOD_LOCKED == lockState)
					{
						DEMOD_PRINT( "[DEMOD_CHIP] LX_DEMOD_SYNC_CHECKING : catch only carrier lock..... ^0^\n");
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE:
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.vsb.tuneMode)
					{
						...
						if(RET_OK != DEMOD_O20_Get_Packet_Error(&packetError))
						{
							DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_Get_Packet_Error() \n");
							DEMOD_RETURN_FAIL(__F__);
						}
						else
						{
							...
							DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] LX_DEMOD_SIGNAL_STABLE : MSG_ATSC_FRONT_END_LOCKED..... ^^^0^^^\n");
							...
							if (RET_OK != _DEMOD_O20_LGDBB_VSB_CheckSignalState(pDemodInfo))
							{
								DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, _DEMOD_O20_LGDBB_VSB_CheckSignalState() \n");
								DEMOD_RETURN_FAIL(__F__);
							}
						}
					}
					else
					{
						...
						if (RET_OK != DEMOD_O20_Get_CarrierFreqOffset(&freqOffset))
						{
							DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_Get_CarrierFreqOffset() \n");
							DEMOD_RETURN_FAIL(__F__);
						}
...
************************************************************************************/
/**
 * DEMOD_O20_VSB_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O20_VSB_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
...
#if 1
	if(RET_OK != DEMOD_O20_VSB_Monitor(pDemodInfo))
	{
		DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_VSB_Monitor() \n");
		DEMOD_RETURN_FAIL(__F__);
	}
#endif
...
//			pauseTimeDemod = LX_DEMOD_LOCKED_PAUSE_TIME;
		...
		{
			DEMOD_PRINT( "[DEMOD O20 GBB] FAIL DEMOD_Get_signal_status() \n");
			DEMOD_RETURN_FAIL(__F__);
		}
		#endif
		...
		if(pDemodInfo->lockCount == 2)
		{
			...
			DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] MONITORING FUNC : MSG_ATSC_FRONT_END_LOCKED..... ^0^\n");
	...
	else
	{
		...
		if((LX_DEMOD_UNLOCKED != pDemodInfo->lockStatus)  && (TRUE != pDemodInfo->bNotifiedUnlock))
		{
			...
			if(pDemodInfo->unLockCount == 4)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] MSG_ATSC_FRONT_END_NOT_LOCKED..... -_-\n");
			}
			if(pDemodInfo->unLockCount >= 4)
			{
				pDemodInfo->unLockCount = 0;
...
int _DEMOD_O20_LGDBB_QAM_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int _DEMOD_O20_QAM_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_O20_LGDBB_QAM_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context * 	pFrontend;
	UINT8			new_tuneMode;
	UINT32 			freq_Khz = 0;
	...
	pFrontend = DEMOD_O20_GetContext();
	new_tuneMode = _DEMOD_O20_QAM_CheckTuneMode(pDemodInfo);
	...
	// printk("[O20 LGDBB] ################### chungiii SetDemod QAM check ter_bandwidth_KHz[%d] \n", ter_bandwidth_KHz);
...
************************************************************************************/
/**
 * DEMOD_O20_QAM_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O20_QAM_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	...
	switch (pDemodInfo->controlState)
	{
		...
		case LX_DEMOD_TUNE_START:
			{
				if(RET_OK != DEMOD_O20_LGDBB_QAM_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_LGDBB_QAM_SetDemod() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.qam.tuneMode)
				{
					...
					pDemodInfo->controlState = LX_DEMOD_TUNE_SCAN;
					DEMOD_PRINT( "[DEMOD_CHIP] LX_DEMOD_TUNE_START : (auto) Going to LX_DEMOD_TUNE_SCAN..... ^0^\n");
				...
				else
				{
					...
					DEMOD_PRINT( "[DEMOD_CHIP] LX_DEMOD_TUNE_START :(Manual) Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
		...
		case LX_DEMOD_TUNE_SCAN:
			{
				...
				else if((LX_DEMOD_FLAG_FINISH == scanProcessing)&& (LX_DEMOD_UNLOCKED == scanLock))
				{
					...
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] LX_DEMOD_TUNE_SCAN : MSG_QAM_FRONT_END_NOT_LOCKED..\n") ;
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] LX_DEMOD_TUNE_SCAN : Elapse time is %d msec\n", elapsedTime) ;
		...
		case LX_DEMOD_SYNC_CHECKING :
			{
				...
				if (RET_OK != _DEMOD_O20_LGDBB_QAM_CheckSignalState(pDemodInfo))
				{
						DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, _DEMOD_O20_LGDBB_QAM_CheckSignalState() \n");
						DEMOD_RETURN_FAIL(__F__);
				}

				if(RET_OK != DEMOD_O20_Get_Lock(LX_DEMOD_FEC_LOCK, &lockState))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_Get_Lock() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if(RET_OK != DEMOD_O20_QAM_EQ_Signal_Detector(FALSE))
					{
						DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_QAM_EQ_Signal_Detector() \n");
						DEMOD_RETURN_FAIL(__F__);
					}
				...
				else
				{
					if(LX_DEMOD_TUNE_NORMAL == realTuneMode)
					{
						...
						{
							if(RET_OK != DEMOD_O20_QAM_ModeAutoDetection(TRUE))
							{
								DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_QAM_ModeAutoDetection() \n");
								DEMOD_RETURN_FAIL(__F__);
							}
							#if 0
							if(RET_OK != DEMOD_O20_SoftwareReset())
							{
								DEMOD_PRINT("[DEMOD O20 GBB] FAIL (%s:%d) DEMOD_O20_SoftwareReset() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							#endif
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE:
			{
						...
						else
						{
							...
							else
							{
								if(packetError < LX_DEMOD_PACKET_ERROR_THD)
								{
									...
									DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] LX_DEMOD_SIGNAL_STABLE : MSG_QAM_FRONT_END_LOCKED..... ^^^0^^^\n");
									DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^^0^^^\n", elapsedTime);
									DEMOD_PRINT( "[DEMOD_CHIP] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^0^\n", freqOffset);
				...
				{
					pDemodInfo->controlState	= LX_DEMOD_SYNC_CHECKING;
					DEMOD_PRINT( "[DEMOD_CHIP] LX_DEMOD_SYNC_CHECKING : Going back to LX_DEMOD_SYNC_CHECKING.....\n");
				}
	   		}
			break;
...
************************************************************************************/
/**
 * DEMOD_O20_QAM_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O20_QAM_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_LOCK_STATE_T			lockState;
...
//			pauseTimeDemod = LX_DEMOD_LOCKED_PAUSE_TIME;
		...
		if(pDemodInfo->lockCount == 2)
		{
			...
			DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] MSG_QAM_FRONT_END_LOCKED..... ^0^\n");
	...
	{
		pDemodInfo->lockCount = 0;
		_gO20GBB_VQI_UnlockCount_full_packeterr = 0;
		...
		if((LX_DEMOD_UNLOCKED != pDemodInfo->lockStatus)  && (TRUE != pDemodInfo->bNotifiedUnlock))
		{
			...
			if(pDemodInfo->unLockCount == 4)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] MSG_QAM_FRONT_END_NOT_LOCKED..... -_-\n");
...
int _DEMOD_O20_LGDBB_ISDB_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int _DEMOD_O20_ISDB_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_O20_LGDBB_ISDB_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
	...
	new_tuneMode = _DEMOD_O20_ISDB_CheckTuneMode(pDemodInfo);
...
int DEMOD_O20_ISDBT_TDI_Enable (BOOLEAN bEnable)
{
	LX_DEMOD_L2_Context *	pFrontend;
	...
	{
		DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_L2_SendCommand() \n");
		return RET_ERROR;
	}
	...
	//printk("[DEMOD O20p]   DEMOD_O20_TDI_Enable  ###### chungiii  check TDI data[0x%x] \n", vqData);
...
int DEMOD_O20_ISDBT_Set_MemSconfig (void)
{
	LX_DEMOD_L2_Context *	pFrontend;

	UINT32			st_mem = gMemCfgDEMOD.memory_base;
	...
	{
		DEMOD_PRINT("[DEMOD_CHIP] st_mem[0x%x] \n", st_mem);
		if(RET_OK != DEMOD_L2_ISDB_Set_BaseAddr_AXI(pFrontend, st_mem))
		{
			DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_L2_ISDB_Set_BaseAddr_AXI() \n");
			return RET_ERROR;
		}
	}
...
int DEMOD_O20_ISDBT_GetEmergencyAlertFlagStatus (BOOLEAN *pEalarm)
{
	LX_DEMOD_L2_Context *	pFrontend;
	UINT8 			balarm;
	...
	{
		DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_L2_ISDB_Get_Emergency_Alarm() \n");
		return RET_ERROR;
	}

	if(balarm == 1)
	{
		DEMOD_PRINT("[DEMOD_CHIP]] EWBS is operated[0x%x]\n", balarm);
		*pEalarm = TRUE;
	}
	else
	{
		DEMOD_PRINT("[DEMOD_CHIP] EWBS is not operated[0x%x]\n", balarm);
		*pEalarm = FALSE;
	}
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O20_ISDBT_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	static LX_DEMOD_FLAG_T 		scanProcessing = LX_DEMOD_FLAG_START;
	static LX_DEMOD_LOCK_STATE_T 	scanLock = LX_DEMOD_UNLOCKED;
	static unsigned int 		sigStablestartTime = 0;
	...
	switch (pDemodInfo->controlState)
	{
		...
		case LX_DEMOD_TUNE_START:
			{
				if(RET_OK != DEMOD_O20_LGDBB_ISDB_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_LGDBB_ISDB_SetDemod() \n");
					DEMOD_RETURN_FAIL(__F__);
				}

				#if 1   // FALSE(0x88) = only when using FPGA board , TRUE(0x08) = only when using SOC
				if(RET_OK != DEMOD_O20_ISDBT_TDI_Enable(TRUE))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_ISDBT_TDI_Enable() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
				#endif
		...
		case LX_DEMOD_TUNE_SCAN:
			{
				...
				else if((LX_DEMOD_FLAG_FINISH == scanProcessing)&& (LX_DEMOD_UNLOCKED == scanLock))
				{
					...
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] LX_DEMOD_TUNE_SCAN : MSG_ISDBT_FRONT_END_NOT_LOCKED..\n") ;
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] LX_DEMOD_TUNE_SCAN : Elapse time is %d msec\n", elapsedTime) ;
		...
		case LX_DEMOD_SYNC_CHECKING :
			{
				...
				else
				{
					...
					if(pDemodInfo->bNotifiedUnlock == TRUE)
					{
						...
						{
							if(RET_OK != DEMOD_O20_ISDBT_SafetyReset())
							{
								DEMOD_PRINT("[DEMOD O20] FAIL (%s:%d) DEMOD_SoftwareReset() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							DEMOD_PRINT("[DEMOD O20](%s) DEMOD_SoftwareReset() !!!\n", __F__);
						}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.isdbt.tuneMode)
					{
						...
						else
						{
							...
							{
#if 0
								if(RET_OK != DEMOD_O20_ISDBT_Set_PhaseNoiseEnhancement(TRUE))
								{
									DEMOD_PRINT("[DEMOD_CHIP] FAIL, DEMOD_O20_ISDBT_Set_PhaseNoiseEnhancement() \n");
									DEMOD_RETURN_FAIL(__F__);
								}
#endif
								if(packetError < LX_DEMOD_PACKET_ERROR_THD)
								{
									...
									DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP][20200303] LX_DEMOD_SIGNAL_STABLE : MSG_ISDBT_FRONT_END_LOCKED..... ^^^0^^^\n");
									DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^^0^^^\n", elapsedTime);
									DEMOD_PRINT( "[DEMOD_CHIP] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^^^0^^^\n", freqOffset);
								...
								else
								{
									...
									if(elapsedTime > LX_DEMOD_LOCK_DEALY_100)
									{
										...
										DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] LX_DEMOD_SIGNAL_STABLE : MSG_ISDBT_FRONT_END_LOCKED..... ^^0^^\n");
										DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^0^^\n", elapsedTime);
										DEMOD_PRINT( "[DEMOD_CHIP] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^^0^^\n", freqOffset);
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O20_ISDBT_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
...
//			pauseTimeDemod = LX_DEMOD_LOCKED_PAUSE_TIME;
		...
		if (RET_OK != _DEMOD_O20_LGDBB_ISDB_CheckSignalState(pDemodInfo)) {
			DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, _DEMOD_O20_LGDBB_ISDB_CheckSignalState() \n");
			DEMOD_RETURN_FAIL(__F__);
		}

		if(pDemodInfo->signalStatus.packetError >= _gO20GBB_FullISDBTPkerrThreshold) {
			_gO20GBB_VQI_UnlockCount_full_packeterr++;
			DEMOD_PRINT( "^r^[DEMOD O20](%s) _gO20GBB_VQI_UnlockCount_full_packeterr = %d !!!\n", __F__,_gO20GBB_VQI_UnlockCount_full_packeterr);

			if(_gO20GBB_VQI_UnlockCount_full_packeterr == _gO20GBB_FullVQIPkerrCount)
			{
				...
				if(RET_OK != DEMOD_O20_ISDBT_SafetyReset())
				{
					DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_ISDBT_SafetyReset() \n");
					DEMOD_RETURN_FAIL(__F__);
				}
				DEMOD_PRINT("[DEMOD_CHIP] SUCCESS, DEMOD_O20_ISDBT_SafetyReset() \n");
			...
			if(gO20GBB_PowerOnInit == 2)
			{
				powerOnElapsedTime = jiffies_to_msecs(jiffies) - gO20GBB_PowerOnInitStartTimeISDBT;

				DEMOD_PRINT("[DEMOD_CHIP] powerOnElapsedTime = %d \n",powerOnElapsedTime);

				if(powerOnElapsedTime >= 15000)
				{
					gO20GBB_PowerOnInit = 0;

					if (RET_OK != DEMOD_O20_ISDBT_Set_CrGearshEn(TRUE ))
					{
						DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_O20_ISDBT_Set_CrGearshEn() \n");
						DEMOD_RETURN_FAIL(__F__);
					}
				}
			}
			...
			if(pDemodInfo->lockCount == 1)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] MSG_ISDBT_FRONT_END_LOCKED..... ^0^\n");
		...
		if((LX_DEMOD_UNLOCKED != pDemodInfo->lockStatus)  && (TRUE != pDemodInfo->bNotifiedUnlock))
		{
			...
			if(pDemodInfo->unLockCount == 2)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD_CHIP] MSG_ISDBT_FRONT_END_NOT_LOCKED..... -_-\n");
			}
			if(pDemodInfo->unLockCount >= 2)
			{
				pDemodInfo->unLockCount = 0;
...
************************************************************************************/
/**
 * DEMOD_O20_QAM_SoftwareResetFEC.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_QAM_SoftwareResetFEC (void)
{
	return RET_OK;
}
...
int DEMOD_O20_QAM_Monitor (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_QAM_ModeAutoDetection.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_QAM_ModeAutoDetection (BOOLEAN bEnable)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_QAM_256Mode (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_QAM_Set_DefaultRegisterValue.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_QAM_Set_DefaultRegisterValue (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_QAM_Set_NeverLockWaitTime
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_QAM_Set_NeverLockWaitTime (UINT32 waitMs)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_QAM_Get_TotalInfo (void)
{
	return RET_OK;
}

/**
 * DEMOD_O20_QAM_EQ_Signal_Detector.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_QAM_EQ_Signal_Detector (BOOLEAN OnOff)
{
	return RET_OK;
}

/**
 * DEMOD_O20_QAM_Set_Frame_resynchronizaiton.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_QAM_Set_Frame_resynchronizaiton (BOOLEAN OnOff)
{
	return RET_OK;
}

/**
 * DEMOD_O20_ISDBT_SetDefaultRegisterValue
...
int DEMOD_O20_ISDBT_Set_DefaultRegisterValue (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_Set_Config_auto
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_ISDBT_Set_Config_auto (BOOLEAN bAutoDetect)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_Set_PartialConfig
 * Set hierachical signal path and bandwidth.
 * This function is used when DEMOD_O20_ISDBT_Set_Config_auto is enabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_ISDBT_Set_PartialConfig (LX_DEMOD_ISDBT_CONFIG_T *pIsdbtConfigParam)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when DEMOD_O20_ISDBT_Set_Config_auto is disabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_ISDBT_Set_FullConfig (LX_DEMOD_ISDBT_CONFIG_T *pIsdbtConfigParam)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_ISDBT_Set_NeverLockWaitTime (UINT32 waitMs)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_Set_CrGearshEn
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_ISDBT_Set_CrGearshEn (BOOLEAN bEnable)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_Set_PhaseNoiseEnhancement
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_ISDBT_Set_PhaseNoiseEnhancement (BOOLEAN bEnhanced)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_Get_DelaySpreadStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_ISDBT_Get_DelaySpreadStatus (UINT16 *pDelaySpread)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_ISDBT_Get_FFTMode (LX_DEMOD_LOCK_STATE_T *pTxModeLock, LX_DEMOD_FFTMODE_T *pFftMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
//	UINT8	fftmoder_di_1_0;
	...
	*pFftMode = LX_DEMOD_DVBT_ISDBT_FFT_UNKNOWN;
	...
	if(RET_OK != DEMOD_O20_Get_Lock(LX_DEMOD_PSYNC_LOCK, &lockState))
	{
		DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_O20_Get_Lock() !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}
#endif
	...
	if(RET_OK != ret)
	{
		DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] FAIL, DEMOD_L2_ISDB_Get_TMCC_Info() \n");
		return RET_ERROR;
	}
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_Get_GuradIntervalMode
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_ISDBT_Get_GuradIntervalMode (LX_DEMOD_GIMODE_T *pGIMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_TMCC_INFO	tmcc;
	...
	*pGIMode = LX_DEMOD_DVBT_ISDBT_GIMODE_UNKNOWN;
	...
	if(RET_OK != DEMOD_O20_Get_Lock(LX_DEMOD_PSYNC_LOCK, &lockState))
	{
		DEMOD_PRINT("[DEMOD] FAIL, DEMOD_O20_Get_Lock() !!!\n");
		DEMOD_RETURN_FAIL(__F__);
	}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O20_ISDBT_Get_TMCCInfo (LX_DEMOD_ISDBT_TMCC_CONFIG_T *pTmccParams)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_TMCC_INFO	tmcc;
...
int DEMOD_O20_ISDBT_Get_TotalInfo (void)
{
	return RET_OK;
}
...
int DEMOD_O20_ISDBT_Get_IFO_LOCK (BOOLEAN *pIfoLock)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_Get_CochanDetIndicator
...
int DEMOD_O20_ISDBT_Get_CochanDetIndicator (BOOLEAN *pDetStatus)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_EqualizereReset.
...
int DEMOD_O20_ISDBT_EqualizereReset (void)
{
	return RET_OK;
}
...
int DEMOD_O20_ISDBT_SafetyReset (void)
{
#if 0
	DEMOD_O20_FecReset(TRUE);
	OS_MsecSleep(5);

	if(RET_OK != DEMOD_O20_SoftwareReset())
	{
		DEMOD_PRINT("[DEMOD_CHIP] FAIL, DEMOD_O20_SoftwareReset() !!!\n");
		DEMOD_RETURN_FAIL(__F__);
	}

	OS_MsecSleep(5);
	DEMOD_O20_FecReset(FALSE);
...
************************************************************************************/
/**
 * DEMOD_O20_ISDBT_MemoryBusCheck.
...
int DEMOD_O20_ISDBT_MemoryBusCheck (void)
{
	UINT32 m1_prevW = 0x0;
	UINT32 m1_prevR = 0x0;
	UINT32 m1_currW = 0x0;
...
int DEMOD_O20_ISDBT_Set_MemBus_flushing (void)
{
	UINT32 bus_gating;
	UINT32 bflush;
	...
	do {
		...
		DEMOD_PRINT_ALWAYS("[DEMOD_CHIP] Check flush_end = %d  \n", flush_end);
		if (flush_end) break;
	} while (1);
...
************************************************************************************/
/**
 * DEMOD_O20_VSB_Set_DefaultRegisterValue.
...
int DEMOD_O20_VSB_Set_DefaultRegisterValue (void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_VSB_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O20_VSB_Set_NeverLockWaitTime (UINT32 waitMs)
{
	return RET_OK;
}
...
int DEMOD_O20_VSB_CochannelExist (BOOLEAN *pCochannel)
{
	return RET_OK;
}
...
int DEMOD_O20_VSB_PreMonitor (void)
{
	DEMOD_TRACE("[DEMOD O20 DTV] @ %s =====\n",__F__);
...
int DEMOD_O20_VSB_Monitor (DEMOD_CTX_T *pDemodInfo)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O20_VSB_Get_MSEdynStatus.
...
int DEMOD_O20_VSB_Get_MSEdynStatus (BOOLEAN *pbEnable)
{
	return RET_OK;
}
...
int DEMOD_O20_VSB_Get_TotalInfo (void)
{
	return RET_OK;
}
...
int DEMOD_O20_LGDBB_VSB_SetDemod_Simple (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
