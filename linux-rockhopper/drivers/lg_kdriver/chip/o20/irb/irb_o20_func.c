...
void o20_irb_funcInit(void)
{

	KDRV_IRB_DEBUG("o20_irb init\n");
...
int o20_irb_tx(LX_IRB_PARAM_T * param)
{
	UINT32	micomcmd[5]	= {0,};
	UINT32 * pbuf = NULL;
	UINT32 * mbuf = NULL;
	...
	#if 0
	unsigned char cbuf[400] = {0,};
	unsigned char mbuf[4096] = {0,};
	unsigned char rbuf[400] = {0,};
	unsigned char bbuf[400] = {0,};

	if(param->numCharTimes)
		memcpy( cbuf, param->chartime, 4*param->numCharTimes);
	if(param->numMakeTimes)
		memcpy( mbuf, param->maketime, 4*param->numMakeTimes);
	if(param->numMakeTimes)
		memcpy( rbuf, param->reptime, 4*param->numReapeatTimes);
	if(param->numMakeTimes)
		memcpy( bbuf, param->brtime, 4*param->numBreakTimes);
	#else
	#if 0
	unsigned char mbuf = NULL;
	if(param->maketime == NULL)
	{
  		KDRV_IRB_NOTI("check make buf\n");
		return -1;
	}
	...
	if(midx)
	{
		...
		for(i = 0; i < index ; i++)
		{
			...
			/* 50 mulitfied and  - 300 is requested by micom */
			writel(pbuf[i]*50 - 300 , irb_key_base + 4*i);
		}
	}
	else
	{
	...
	KDRV_IRB_DEBUG("o20_irb tx makenum[%x] pbuf[%x]\n",
		midx,pbuf[0]);
	if(mbuf)
		kfree(mbuf);
	return 0;
...
void o20_irb_funcReInit(void)
{
  KDRV_IRB_NOTI("o20_irb reinit\n");
}
void o20_irb_exit(void)
{
  KDRV_IRB_DEBUG("o20_irb end\n");
}


static irbs_o20_func_t o20_irb_func =
{
	.init = o20_irb_funcInit,
	.reinit = o20_irb_funcReInit,
	.tx = o20_irb_tx,
	.exit = o20_irb_exit,
};

irbs_o20_func_t* get_o20_irb_func(void)
{
	return &o20_irb_func;
}
