...
static LX_MEMCFG_T	gMemCfgHdr[DDR_RGN_MAX]={{.name = "[header]VDEC/SYS",.base = 0x0, .size = 0 },
											{ .name = "[header]M0_DEV",	 .base = 0x0, .size = 0 },
											{ .name = "[header]M2_DEV",	 .base = 0x0, .size = 0 },
											{ .name = "[header]M1_DEV",	 .base = 0x0, .size = 0 },
											{ .name = "[header]M1_DEV_H",.base = 0x0, .size = 0 },
											{ .name = "[header]M2_DEV_H",.base = 0x0, .size = 0 }};	/* mem header */

static LX_MEMCFG_T static_rgn_sys0_start= { .name = "static(0)",	.base = 0x0, .size = 0 };		/* mem separator */
...
static LX_MEMCFG_T gMemCfgSYS1			= {.name = "sys(m1)",		.base = 0x0, .size = __KB(128)};

static LX_MEMCFG_T static_rgn_aud_sep	= {.name = "aud(sep)",		.base = 0x0, .size = 0 };
...
static LX_MEMCFG_T gMemCfgMemTest2      = {.name = "mt_ddr2",       .base = 0x0, .size = __MB(1) };
#endif
...
static LX_MEMCFG_T gMemCfgFreeMem5		= {.name = "free(5)", 		.base = 0x0, .size = 0 };
...
static LX_MEMCFG_DESC_T mem_desc[MEM_DESC_ID_MAX]	= {
	...
	DEFINE_MEM_DESC(MTEST0 		,MEMCFG_DESC_FLAG_CMA),		/* MTEST DDR0(M0) CMA 로 조치함 */
	DEFINE_MEM_DESC(MTEST1 		,MEMCFG_DESC_FLAG_CMA),		/* MTEST DDR1(M1) CMA 로 조치함 */
	DEFINE_MEM_DESC(MTEST2 		,MEMCFG_DESC_FLAG_CMA),		/* MTEST DDR2(M2) CMA 로 조치함 */
	...
	DEFINE_MEM_DESC(FREE5		,MEMCFG_DESC_FLAG_NONE), 	/* enable CMA (SICDTV-9260) */
...
static LX_MEMCFG_ENTRY_T g_mem_desc_table_DDR_5GB_ADV[] =
{
	...
	LX_MEMCFG_ENTRY(static_rgn_sys0_start),					/* 고정 - ??정???? 마세??*/
	LX_MEMCFG_ENTRY_EX(gMemCfgSYS0, MEM_DESC(SYS0)),		/* SYS (fixed) */
	...
	LX_MEMCFG_ENTRY_EX(static_rgn_aud_sep,	MEM_DESC(AUDIO_SEP)),	/* AUDIO */
#endif

#ifdef INCLUDE_KDRV_TE
	LX_MEMCFG_ENTRY_EX(gMemCfgTE,		MEM_DESC(TE)),		/* SDEC/TE */
	LX_MEMCFG_ENTRY_EX(gMemCfgSYS1, 	MEM_DESC(SYS1)),	/* SYS (fixed) */
#endif
...
//	LX_MEMCFG_ENTRY_EX(gMemCfgDEMOD, 	MEM_DESC(DEMOD)),	/* DEMOD */
...
#ifdef INCLUDE_KDRV_DE
	LX_MEMCFG_ENTRY_EX(gMemCfgDeFwE60[0], MEM_DESC(DE_FW0)),/* fw_mcu0 */
	LX_MEMCFG_ENTRY_EX(gMemCfgDeFwE60[1], MEM_DESC(DE_FW1)),/* fw_mcu1 */
	LX_MEMCFG_ENTRY_EX(gMemCfgE60[0],	MEM_DESC(DE_PREW)),	/* de_prew */
#ifdef INCLUDE_KDRV_BE
	LX_MEMCFG_ENTRY_EX(gMemCfgBeFrc[10], MEM_DESC(BE_FPP)),  /* be_fpp  */
	LX_MEMCFG_ENTRY_EX(gMemCfgBeFrc[11], MEM_DESC(BE_FPP)),  /* be_frc0  */
#endif
	LX_MEMCFG_ENTRY_EX(gMemCfgE60[3],	MEM_DESC(BT0)),		/* de_bt_m0 */
	LX_MEMCFG_ENTRY_EX(gMemCfgHDR[2], 	MEM_DESC(HDR)),		/* de_hdr */
#ifdef INCLUDE_KDRV_BE
	LX_MEMCFG_ENTRY_EX(gMemCfgBe[6], 	MEM_DESC(BE)),		/* be_db  */
#endif
	LX_MEMCFG_ENTRY_EX(gMemCfgDeFwE60[5], MEM_DESC(DE_FW5)),/* fw_dnn */
	LX_MEMCFG_ENTRY_EX(gMemCfgDeFwE60[2], MEM_DESC(DE_FW2)),/* fw_mcu2 */
	LX_MEMCFG_ENTRY_EX(gMemCfgDeFwE60[3], MEM_DESC(DE_FW3)),/* fw_mcu3 */
	LX_MEMCFG_ENTRY_EX(gMemCfgDeFwE60[4], MEM_DESC(DE_FW4)),/* fw_shared */
#ifdef INCLUDE_KDRV_BE
	LX_MEMCFG_ENTRY_EX(gMemCfgBeChInf[1], MEM_DESC(BE_CINF)), /* be_chip_info */
#endif
#endif
...
#ifdef INCLUDE_KDRV_GFX
	LX_MEMCFG_ENTRY_EX(gMemCfgGfx.surface[0], MEM_DESC(GFX0)),/* GFXOSD. see E60 r4.8 map */
#endif

#ifdef INCLUDE_KDRV_DE
#ifdef INCLUDE_KDRV_BE
	LX_MEMCFG_ENTRY_EX(gMemCfgBeFrc[13], MEM_DESC(BE_FRC2)),/* be_frc2 */
	LX_MEMCFG_ENTRY_EX(gMemCfgBeFrc[16], MEM_DESC(BE_FRC3)),/* be_frc3 */
#endif
	LX_MEMCFG_ENTRY_EX(gMemCfgE60[6], MEM_DESC(VT1)),       /* de_vt_m2 */
#endif
...
#ifdef INCLUDE_KDRV_FBDEV
	LX_MEMCFG_ENTRY_EX(gMemCfgFBBuf, MEM_DESC(FBDEV)),      /* FBDEV. see E60 r4.8 map */
#endif

#ifdef USE_MTEST_RGN
	LX_MEMCFG_ENTRY_EX(gMemCfgMemTest2, MEM_DESC(MTEST2)),  /* MTEST */
#endif
	LX_MEMCFG_ENTRY_EX(gMemCfgFreeMem2, MEM_DESC(FREE2)),	/* 고정 - ??정???? 마세??*/
...
#ifdef INCLUDE_KDRV_DE
	LX_MEMCFG_ENTRY_EX(gMemCfgE60[4], MEM_DESC(BT1)),	    /* de_bt_m1 */
	LX_MEMCFG_ENTRY_EX(gMemCfgE60[1], MEM_DESC(DE_FRM1)),	/* de_frm_m1 */
#ifdef INCLUDE_KDRV_BE
	LX_MEMCFG_ENTRY_EX(gMemCfgBeFrc[12], MEM_DESC(BE_FRC1)), /* be_frc1 */
#endif
	LX_MEMCFG_ENTRY_EX(gMemCfgE60[5], MEM_DESC(VT0)),		/* de_vt_m1 */
#endif

#ifdef USE_MTEST_RGN
	LX_MEMCFG_ENTRY_EX(gMemCfgMemTest1, MEM_DESC(MTEST1)),  /* MTEST */
#endif
	LX_MEMCFG_ENTRY_EX(gMemCfgFreeMem3, MEM_DESC(FREE3)),	/* 고정 - ??정???? 마세??*/
	...
	/* N/A */
	LX_MEMCFG_ENTRY_EX(gMemCfgFreeMem4, MEM_DESC(FREE4)),	/* 고정 - ??정???? 마세??*/
	#endif
	...
	//	M2 DEVICE (HIGH ADDR)
	//----------------------------------------------------------------------------------
	LX_MEMCFG_ENTRY(gMemCfgHdr[DDR_RGN5_DEV]),
#ifdef INCLUDE_KDRV_DE
	LX_MEMCFG_ENTRY_EX(gMemCfgE60[2], MEM_DESC(DE_FRM2)),   /* de_frm_m2 */
#endif
	LX_MEMCFG_ENTRY_EX(gMemCfgFreeMem5, MEM_DESC(FREE5)),	/* 고정 - ??정???? 마세??*/
...
{
	/* setup static mem region - 32MB@M0, 16KB@M1, 16KB@M2 */
	static_rgn_sys0_start.base = gMemCfgHdr[DDR_RGN0_DEV].base + gMemCfgHdr[DDR_RGN0_DEV].size - R0_SYSTEM_RGN_SZ;

	/* audio will be located at the start address of RGN1 */
	static_rgn_aud_sep.base = gMemCfgHdr[DDR_RGN1_DEV].base + AUDIO_MEM_SZ;

	/* setup GFX */
#ifdef INCLUDE_KDRV_GFX
	gMemCfgGfx.surface[0].size	= (g_model_ctx.graphic==MODEL_OPT_4K_GRAPHIC)? GFXOSD_MEM_4KOSD_SZ: GFXOSD_MEM_2KOSD_SZ;
#endif
...
//#endif
	...
	/* TODO: add more memory configuration if necessary */
}

static void BASE_PLT_Setup_4GB_ADV(void)
{
	printk("configure E60(%2X) kdriver memory (5.0GB DDR)\n", lx_chip_rev() & 0xff);
	...
	gMemCfgHdr[DDR_RGN5_DEV].base = DDR_RGN5_BASE; gMemCfgHdr[DDR_RGN5_DEV].size = DDR_RGN5_SZ;
...
static void BASE_PLT_DoPostMemoryLayout(void)
{
	...
	gMemCfgFreeMem5.size = DDR_RGN5_END - gMemCfgFreeMem5.base;
	...
	MEM_DESC_SIZE(FREE5) = gMemCfgFreeMem5.size;
	...
	{
		phys_addr_t base = mem_desc[i].m.base;
		unsigned int size = mem_desc[i].m.size;

		/* check high address. rebase if if address is between 0xc000_0000 and 0xffff_ffff */
		if ((base&0xc0000000) == 0xc0000000)
		{
			base |= 0x100000000uLL;
			printk("high address detected, %s. rebase addr=0x%09llx\n", mem_desc[i].m.name, base);
		}
		...
		switch(mem_desc[i].flag)
		{
			case MEMCFG_DESC_FLAG_NO_CMA:
			{
				if(base && size)
				{
					ret = hma_pool_register_type (mem_desc[i].m.name, base, size, CMA_POOL);

					if(ret==0)
					{
						printk("(x) CMA(%-12s %3dM@0x%09llx) disabled\n", mem_desc[i].m.name, size>>20, base);
						hma_alloc(mem_desc[i].m.name, size, 1<<12);
					}
					else
					{
						printk("(?) CMA(%-12s %3dM@0x%09llx) register error\n", mem_desc[i].m.name, size>>20, base);
					}
				}
				else
				{
					printk("(x) CMA(%-12s %3dM@0x%09llx) can't be disabled\n", mem_desc[i].m.name, size>>20, base);
				}
			}
			break;
			...
			case MEMCFG_DESC_FLAG_CMA:
			{
				...
				printk("(o) CMA(%-12s %3dM@0x%09llx) enabled\n", mem_desc[i].m.name, size>>20, base);
			}
			break;
			...
			default:
			{
				...
				printk("(/) CMA(%-12s %3dM@0x%09llx) skip\n", mem_desc[i].m.name, size>>20, base);
			}
			break;
		}
	}
	...
	hma_pool_fallback("venc0", "vpb8k");	/* reidrect venc0 to vpb8k if out of memory */
	...
	ret = hma_pool_register_type ("mtest-ddr0",     gMemCfgMemTest0.base, gMemCfgMemTest0.size, CMA_POOL);
	if (ret < 0) printk (KERN_ERR "cannot register \"%s\" pool\n", "mtest_ddr0");

	ret = hma_pool_register_type ("mtest-ddr1",     gMemCfgMemTest1.base, gMemCfgMemTest1.size, CMA_POOL);
	if (ret < 0) printk (KERN_ERR "cannot register \"%s\" pool\n", "mtest_ddr1");

	ret = hma_pool_register_type ("mtest-ddr2",     gMemCfgMemTest2.base, gMemCfgMemTest2.size, CMA_POOL);
	if (ret < 0) printk (KERN_ERR "cannot register \"%s\" pool\n", "mtest_ddr2");
#endif
...
void	BASE_PLT_Init (void)
{
	...
	switch(g_model_ctx.ddr_size)
	{
		...
		default:						BASE_PLT_Setup_4GB_ADV(); break;
	}
...
void  BASE_PLT_Probe (UINT32* plt, UINT32* board_opt)
{
	...
	if(webos_hwopt_probe)
	{
		...
		if (RET_OK == OS_ScanKernelCmdline("hwopt=%s",hwopt_txt))
		{
			...
			#if 0	/* E60 platform is fixed to 4GB DDR */
