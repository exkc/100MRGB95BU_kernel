...
static int kbase_gpu_secure_mode_enable(struct kbase_device *kbdev, struct kbase_context *secure_kctx)
{
	int err = -EINVAL;
...
int kbase_gpu_secure_mode_disable(struct kbase_device *kbdev)
{
	int err = -EINVAL;
			...
			case KBASE_ATOM_GPU_RB_WAITING_SECURE_MODE:
				...
				if (kbase_gpu_in_secure_mode(kbdev) != kbase_jd_katom_is_secure(katom[idx])) {
					...
					if (kbase_jd_katom_is_secure(katom[idx])) {
						err = kbase_gpu_secure_mode_enable(kbdev, katom[idx]->kctx);
					} else {
