...
static int kbase_open(struct inode *inode, struct file *filp)
{
...

	{
	    ...
	    list_for_each_entry(element, &kbdev->kctx_list, link) {
	        if (element->kctx->tgid == kctx->tgid)
	           proc_flag = 0;
	    }
	    ...
	    if(proc_flag)
	    {
	        kbasep_mem_profile_procfs_insert(kctx, kbdev);
	        kbasep_um_procfs_insert(kctx, kbdev);
	    }
	}
...
static int kbase_release(struct inode *inode, struct file *filp)
{
	...
	if( proc_count == 1)
	{
	    kbasep_mem_profile_procfs_remove(kctx, kbdev);
	    kbasep_um_procfs_remove(kctx, kbdev);
	}
#endif
...
static int kbase_device_debugfs_init(struct kbase_device *kbdev)
{
	...
	kbdev->webos_gpu_directory = proc_mkdir("gpu",
			NULL);
	if (!kbdev->webos_gpu_directory) {
		dev_err(kbdev->dev, "Couldn't create mali gpu procfs webos_proc directory\n");
		err = -ENOMEM;
		goto out;
	}

	kbdev->webos_um_directory = proc_mkdir("um",
			NULL);
	if (!kbdev->webos_um_directory) {
		dev_err(kbdev->dev, "Couldn't create mali um procfs webos_proc directory\n");
		err = -ENOMEM;
		goto out;
	}
	/* register gpu field into /proc/meminfo */
	kbdev->gpu_info.getTotal = get_gpu_total_memory;
	kbdev->gpu_info.getCached = get_gpu_cached_memory;
	kbdev->gpu_info.getReclaimable = get_gpu_pool_memory;
	gpu_info_register(&kbdev->gpu_info);

	kbdev->gpu_xr = low_mem_notify_add_xreclaimable("gpu_pool");
	...
	kbasep_gpu_utilization_debugfs_init(kbdev);
	...
	kbase_gpu_xreclaimable_init(kbdev);
...
{
	debugfs_remove_recursive(kbdev->mali_debugfs_directory);
	kbase_gpu_xreclaimable_term(kbdev);
}
...
static int kbase_platform_device_remove(struct platform_device *pdev)
{
	...
	// unregister gpu field into /proc/meminfo
	gpu_info_unregister(&kbdev->gpu_info);

	low_mem_notify_remove_xreclaimable(kbdev->gpu_xr);
	...
	if (kbdev->inited_subsys & inited_gpu_utilization) {
