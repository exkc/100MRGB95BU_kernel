...
// KyuYeunKim: add mali_kbase_device.h
...
// KyuYeunKim: Added #if 0 from 08495ee596568b4d8b5c64cb8ce8de8edde3073d
#if 0 // YUNHYE
...
// KyuYeunKim: adds kbase_pm_get_dvfs_utilization
int kbase_pm_get_dvfs_utilization(struct kbase_device *kbdev)
{
	int utilisation;
	struct kbasep_pm_metrics *diff;
...
#ifdef CONFIG_MALI_MIDGARD_DVFS
static int kbasep_gpu_util_seq_show(struct seq_file *sfile, void *data)
{
        ssize_t ret = 0;
        ...
        list_for_each(entry, kbdev_list) {
                ...
                utilization = kbase_pm_get_dvfs_utilization(kbdev);
                seq_printf(sfile, "%-16s utilization: %d\n", \
                                                kbdev->devname, utilization);
        }
        kbase_device_put_list(kbdev_list);
        return ret;
}

static int kbasep_gpu_util_debugfs_open(struct inode *in, struct file *file)
{
        return  single_open(file, kbasep_gpu_util_seq_show, NULL);
}

static const struct file_operations kbasep_gpu_util_debugfs_fops = {
        .open = kbasep_gpu_util_debugfs_open,
        .read = seq_read,
        .llseek = seq_lseek,
...
/**
 * Initialize debugfs entry for gpu utilization
 */
int kbasep_gpu_utilization_debugfs_init(struct kbase_device *kbdev)
{
        kbdev->gpu_util_dentry = debugfs_create_file("gpu_utilization", \
                                               S_IRUGO, \
                                               kbdev->mali_debugfs_directory, \
                                               NULL, \
                                               &kbasep_gpu_util_debugfs_fops);
        if (IS_ERR(kbdev->gpu_util_dentry))
                return 3; /* MALI_ERROR_FUNCTION_FAILED; */

        return 0; /* MALI_ERROR_NONE; */
}

/**
 * Terminate debugfs entry for gpu utilization
 */
