...
int ntfs_file_compare_values(const FILE_NAME_ATTR *file_name_attr1,
...
int ntfs_nlstoucs(const ntfs_volume *vol, const char *ins,
		const int ins_len, ntfschar **outs, int max_name_len)
{
	...
	if (likely(ins)) {
		if (max_name_len > NTFS_MAX_NAME_LEN)
			ucs = kvmalloc((max_name_len + 2) * sizeof(ntfschar),
				       GFP_NOFS | __GFP_ZERO);
		else
 ...
 */
static u32 ntfs_ucsnlen(const ntfschar *s, u32 maxlen)
{
	u32 i;
...
ntfschar *ntfs_ucsndup(const ntfschar *s, u32 maxlen)
{
	...
	dst = ntfs_malloc_nofs((len + 1) * sizeof(ntfschar));
	if (dst) {
