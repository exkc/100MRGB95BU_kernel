...
#undef BE_FRC_HW_O18_TEMP_USE_TYPE_BY_CTRL_SPATH
...
/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
//static BE_FRC_HW_O18_INFO_T _beFrcHwO18Info= {0,};
static BE_FRC_HW_O18_INFO_T _gBeFrcHwO18Info= {0,};
/*========================================================================================
...
int BE_FRC_HW_O18_Initialize(void)
{
	...
	do {
		memset(&_gBeFrcHwO18Info, 0, sizeof(BE_FRC_HW_O18_INFO_T));
		BE_NOTI("Done.\n ");
	} while (0);
...
int BE_FRC_HW_O18_ControlMemc(LX_BE_FRC_MEMC_MODE_T *pstParams)
{
	...
	do {
		...
		BE_TRACE1("%d, %d, %d -> %d, %d, %d\n", pstParams->u8MemcType, pstParams->u8Blur, pstParams->u8Judder,\
			_gBeFrcHwO18Info.u32MemcType, _gBeFrcHwO18Info.u32Blur, _gBeFrcHwO18Info.u32Judder);
		...
		stHwParams.uFRCUserLevel	= pstParams->u8MemcType == 0xFF ? 0xFFFFFFFF : pstParams->u8MemcType;
		 ...
		 if (stHwParams.uFRCBlur != 0xFFFFFFFF)
			_gBeFrcHwO18Info.u32Blur  = stHwParams.uFRCBlur;

  	        if (stHwParams.uFRCJudder != 0xFFFFFFFF)
			_gBeFrcHwO18Info.u32Judder        = stHwParams.uFRCJudder;

               if (stHwParams.uFRCUserLevel != 0xFFFFFFFF)
			_gBeFrcHwO18Info.u32MemcType  = stHwParams.uFRCUserLevel;

		BE_TRACE1("%d, %d, %d -> %d, %d, %d\n", pstParams->u8MemcType, pstParams->u8Blur, pstParams->u8Judder,\
		_gBeFrcHwO18Info.u32MemcType, _gBeFrcHwO18Info.u32Blur, _gBeFrcHwO18Info.u32Judder);

		BE_FRC_HW_O18_SetMemcMode(&_gBeFrcHwO18Info);
	} while (0);
...
int BE_FRC_HW_O18_SetMemcLowDelay(LX_BE_FRC_LOW_DELAY_T *pstParams)
{
	...
	do {
 		BE_TRACE1("%d -> %d\n", pstParams->u32Lowdelay, _gBeFrcHwO18Info.u32LowDelay);
		
		if (pstParams->u32Lowdelay != 0xFFFFFFFF)
			_gBeFrcHwO18Info.u32LowDelay = pstParams->u32Lowdelay;
...
int BE_FRC_HW_O18_SetMemcMotionPro(LX_BE_FRC_MOTION_PRO_T *pstParams)
{
	...
	do {
 		BE_TRACE1("%d -> %d\n", pstParams->u32MotionPro, _gBeFrcHwO18Info.u32MotionPro);
		
		 if (pstParams->u32MotionPro != 0xFFFFFFFF)
			_gBeFrcHwO18Info.u32MotionPro= pstParams->u32MotionPro;
...
int BE_FRC_HW_O18_SetMemcMode(BE_FRC_HW_O18_INFO_T *_gBeFrcHwO18Info)
{
	int ret = RET_OK;
	...
	do {
		...
		if (_gBeFrcHwO18Info->u32LowDelay == 1)
		{
			stHwParams.uFRCUserLevel= LX_BE_FRC_MEMC_DISABLE;
		}
		else
		{
			stHwParams.uFRCUserLevel= _gBeFrcHwO18Info->u32MemcType;
		}
		stHwParams.uFRCBlur	= _gBeFrcHwO18Info->u32Blur;
		stHwParams.uFRCJudder = _gBeFrcHwO18Info->u32Judder;
		stHwParams.uFRCMotionProEn = _gBeFrcHwO18Info->u32MotionPro;
		BE_TRACE1("%d,%d,%d/%d/%d -> MemcType(%d), Blur(%d), Judder(%d), MPro(%d)\n",\
			_gBeFrcHwO18Info->u32MemcType, _gBeFrcHwO18Info->u32Blur,_gBeFrcHwO18Info->u32Judder,\
			_gBeFrcHwO18Info->u32LowDelay, _gBeFrcHwO18Info->u32MotionPro, stHwParams.uFRCUserLevel,\
		...
		else if (_gBeFrcHwO18Info->u32MotionPro ==1)
		{
...
int BE_FRC_HW_O18_SetConfig(LX_BE_FRC_CFG_T *pstParams)
{
	...
	do {
		...
		{
			#ifdef BE_FRC_HW_O18_TEMP_USE_TYPE_BY_CTRL_SPATH
