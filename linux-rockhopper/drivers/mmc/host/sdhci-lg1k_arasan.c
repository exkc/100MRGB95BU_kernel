...
int lg1k_select_drive_strength(struct mmc_card *card,
...
static void lg1k_arasan_platform_init(struct sdhci_host *host)
{
	...
	host->mmc_host_ops.select_drive_strength = lg1k_select_drive_strength;
...
static int sdhci_lg1k_probe(struct platform_device *pdev)
{
	...
	if (of_device_is_compatible(np, "lge,lg1k-sdhci-5.0"))
		ret = sdhci_pltfm_init_and_add_host(pdev, &sdhci_lg1k_50_pdata, 0);
	else if (of_device_is_compatible(np, "lge,lg1k-sdhci-5.0-v2"))
		ret = sdhci_pltfm_init_and_add_host(pdev, &sdhci_lg1k_50_pdata, 0);
	else if (of_device_is_compatible(np, "lge,lg1k-sdhci-5.0-v3"))
		ret = sdhci_pltfm_init_and_add_host(pdev, &sdhci_lg1k_50_pdata_v3, 0);
	else if (of_device_is_compatible(np, "lge,lg1k-sdhci-5.1"))
		ret = sdhci_pltfm_init_and_add_host(pdev, &sdhci_lg1k_51_pdata, 0);
	else if (of_device_is_compatible(np, "lge,lg1154-sdhci")
		|| of_device_is_compatible(np, "lge,lg1k-sdhci-4.41"))
		ret = sdhci_pltfm_init_and_add_host(pdev, &sdhci_lg1k_441_pdata, 0);
	else if (of_device_is_compatible(np, "lge,lg1156-sdhci")
		|| of_device_is_compatible(np, "lge,lg1k-sdhci-4.51"))
		ret = sdhci_pltfm_init_and_add_host(pdev, &sdhci_lg1k_451_pdata, 0);
	else if (of_device_is_compatible(np, "lge,lg1k-sdhci-4.51-sd"))
		ret = sdhci_pltfm_init_and_add_host(pdev, &sdhci_lg1k_451_sd_pdata, 0);
	else
		dev_err(&pdev->dev, "Can't find compatible device \n");

	return ret;
#else
	return sdhci_pltfm_init_and_add_host(pdev, &sdhci_lg1k_441_pdata);
#endif
}

static void sdhci_lg1k_remove(struct platform_device *pdev)
{
