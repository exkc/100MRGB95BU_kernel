...
static int lg1k_i2c_xfer(struct i2c_adapter *adap, struct lg1k_i2c_req *req)
{
	...
	if ((req->flags & I2C_XFER_POLL) && priv->irq >= 0)
	...
	if (req->flags & I2C_XFER_POLL) {
		...
		r = wait_for_completion_timeout(&priv->completion,
		                                adap->timeout);
		if (!r) {
			dev_warn(&pdev->dev, "timed-out - reset\n");
			lg1k_i2c_init(adap);
...
static int lg1k_i2c_probe(struct platform_device *pdev)
{
	...
	if (priv->irq >= 0) {
		r = request_irq(priv->irq, lg1k_i2c_isr, 0,
	...
	dev_info(&pdev->dev, "lg1k-i2c bus-adapter (%s mode)\n",
	         priv->irq >= 0 ? "IRQ" : "polling");
...
static void lg1k_i2c_remove(struct platform_device *pdev)
{
