...
static int slt_open( void )
{
	...
	SLT_REG = (volatile O20_SLT_REG_T*)ioremap(O20_SLT_BASE, sizeof(O20_SLT_REG_T));
	if( SLT_REG == NULL )
	{
	...
	ret = request_irq(O20_IRQ_SLT, (irq_handler_t)slt_isr, NULL, "SLT", NULL);
#else
	ret = request_irq(O20_IRQ_SLT, (irq_handler_t)slt_isr, IRQF_DISABLED, "SLT", NULL);
#endif
	if( ret != RET_OK )
	{
