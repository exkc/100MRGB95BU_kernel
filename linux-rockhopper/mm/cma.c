...
static void __init cma_prepare_zone_setup(void)
{
	int i;
	struct zone *zone;
	pg_data_t *pgdat;
	...
	for_each_online_pgdat(pgdat) {
		...
		for (i = 0; i < cma_area_count; i++) {
			if (pfn_to_nid(cma_areas[i].base_pfn) != pgdat->node_id)
				continue;
...
static void __init cma_complete_zone_setup(void)
{
	struct zone *zone;
...
static int __init cma_init_reserved_areas(void)
{
	...
	cma_prepare_zone_setup();
	for (i = 0; i < cma_area_count; i++)
		cma_activate_area(&cma_areas[i]);
	cma_complete_zone_setup();
...
int is_cma_area(unsigned long pfn)
{
	int i;
	struct cma *cma;
