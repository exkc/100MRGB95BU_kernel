...
/*----------------------------------------------------------------------------------------
	Macro Definitions
----------------------------------------------------------------------------------------*/
static DEFINE_SPINLOCK(g_m17_pm_cg_lock);
...
// Clock Gating
LX_PM_CLOCKGATING_T m17_pms_clockgating;

typedef struct PM_M16P_CGINFO_t
{
	unsigned int module;
	unsigned int sub;
...
static int m17_GetCGInfo(PM_M16P_CGINFO_T *cginfo)
{
	int ret = 0;
...
int m17_clock_on(PM_M16P_CGINFO_T cginfo)
{
	...
	spin_lock_irqsave(&g_m17_pm_cg_lock, flags);

	// clock first when power on
	PM_DEBUG_PRINT("M16P ON CG 0x%X : 0x%X\n",cginfo.base,readl(pms_clock_gating_base));
	...
	{
		PM_DEBUG_PRINT("M16P ON CG2 0x%X : 0x%X\n",cginfo.base,readl(pms_clock_gating_base));
		reg = readl(pms_clock_gating_base);
	...
	spin_unlock_irqrestore(&g_m17_pm_cg_lock, flags);
...
int m17_clock_off(PM_M16P_CGINFO_T cginfo)
{
	void __iomem *pms_clock_gating_base = NULL;
	volatile unsigned int reg;
	unsigned long flags;
	...
	PM_DEBUG_PRINT("M16P OFF CG 0x%X : 0x%X\n",cginfo.base,readl(pms_clock_gating_base));
	reg = readl(pms_clock_gating_base);
	reg = reg | (1 << cginfo.clkbit);
	writel(reg, pms_clock_gating_base);
	PM_DEBUG_PRINT("M16P OFF CG 0x%X : 0x%X\n",cginfo.base,readl(pms_clock_gating_base));
	...
	if (cginfo.clkbit2 != 0x80000000)
	{
		PM_DEBUG_PRINT("M16P OFF CG2 0x%X : 0x%X\n",cginfo.base,readl(pms_clock_gating_base));
		reg = readl(pms_clock_gating_base);
		reg = reg | (1 << cginfo.clkbit2);
		writel(reg, pms_clock_gating_base);
		PM_DEBUG_PRINT("M16P OFF CG2 0x%X : 0x%X\n",cginfo.base,readl(pms_clock_gating_base));
...
{
	PM_M16P_CGINFO_T cginfo;
	int ret;

	if (( lx_chip_rev() & 0xFFFFFF00) != (LX_CHIP_REV(M16P,A0) & 0xFFFFFF00))
		return 0;
 ...
 */
int m17_pm_ioctl_cg (unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	int result = 0;
	...
	switch (cmd)
	{
		case PM_SET_CLOCK_GATING:
			result = copy_from_user((void *)&m17_pms_clockgating, (void __user *)arg, sizeof(m17_pms_clockgating));
			if (result) {
				PM_ERROR("PM_SET_CLOCK_GATING - CONFIG DATA COPY FAIL\n");
				break;
			}

			m17_set_clock_gating(m17_pms_clockgating.module, m17_pms_clockgating.sub, m17_pms_clockgating.state);
			break;
		case PM_GET_CLOCK_GATING:
			...
			m17_get_clock_gating(m17_pms_clockgating.module, m17_pms_clockgating.sub, &m17_pms_clockgating.state);

			ret = copy_to_user((void __user*)(arg), (void *)(&m17_pms_clockgating), sizeof(m17_pms_clockgating));
			if (ret)
...
static pms_m17_cg_t m17_pm_cg =
{
	.ioctl = m17_pm_ioctl_cg,
};
