...
#define	DE_TRACE(format, args...)		DBG_PRINT( g_de_debug_fd,         \
													...
													format,               \
													##args)
#define DE_VTV(format, args...)			DBG_PRINT( g_de_debug_fd,         \
													LX_LOGM_LEVEL_TRACE+1,\
													format,               \
													##args)
#define DE_SYNC(format, args...)		DBG_PRINT( g_de_debug_fd,         \
													LX_LOGM_LEVEL_TRACE+2,\
													format,               \
													##args)
#define DE_TRACE_20(format, args...)	DBG_PRINT( g_de_debug_fd,20,format,##args)
#define DE_TRACE_21(format, args...)	DBG_PRINT( g_de_debug_fd,21,format,##args)
#define DE_TRACE_22(format, args...)	DBG_PRINT( g_de_debug_fd,22,format,##args)
#define DE_TRACE_23(format, args...)	DBG_PRINT( g_de_debug_fd,23,format,##args)

/*----------------------------------------------------------------------------------------
 *   Type Definitions
 *---------------------------------------------------------------------------------------*/
typedef enum { // [v4l2] scaler status - SCDCR-2375
	vdev_open,
	connect_src,
	connect_index,
	connect_attr,
	...
	zorder_alpha,
	pattern,
	limited_win_ratio_h_scaleup,
	limited_win_ratio_v_scaleup,
	limited_win_ratio_h_scaledown,
	limited_win_ratio_v_scaledown,
	socts_option
} PROC_CMD_TYPE;
/*----------------------------------------------------------------------------------------
