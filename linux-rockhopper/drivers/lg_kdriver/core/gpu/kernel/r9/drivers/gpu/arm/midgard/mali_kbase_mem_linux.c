 ...
 #endif
{
	...
	while (i < map->alloc->nents && (addr < vma->vm_end >> PAGE_SHIFT)) {
...
#endif
		// if pfn is not valid, vm_insert_pfn can not be applied.
		// so adding vm_insert_mixed function to support it.
		...
		else if (vma->vm_flags & VM_MIXEDMAP)
		{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
			ret = vmf_insert_mixed(vma, addr << PAGE_SHIFT,
				__pfn_to_pfn_t(PFN_DOWN(as_phys_addr_t(map->alloc->pages[i])), PFN_DEV));
#else
			ret = vm_insert_mixed(vma, addr << PAGE_SHIFT,
		    		PFN_DOWN(as_phys_addr_t(map->alloc->pages[i])));
#endif
		}
		else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
		if (ret == VM_FAULT_SIGBUS || ret == VM_FAULT_OOM)
#else
		if (ret < 0 && ret != -EBUSY)
#endif
			goto locked_bad_fault;
