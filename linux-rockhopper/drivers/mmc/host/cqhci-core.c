...
#ifdef CONFIG_MMC_SDHCI_LG1K
#define CQE_BOUNDRY_RESERVE    2

static int cqe_log __read_mostly = 0;
module_param(cqe_log, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(cqe_log, "print out emmc read/write log for getting emmc healthy information");
#endif
...
{
	return cq_host->trans_desc_len * cq_host->mmc->max_segs * tag * CQE_BOUNDRY_RESERVE;
}
#else
static inline size_t get_trans_desc_offset(struct cqhci_host *cq_host, u8 tag)
{
...
static int cqhci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
...
#ifdef CONFIG_MMC_SDHCI_LG1K
	if (unlikely(cqe_log == 1)) {
		if (mrq->data) {
			if (mrq->data->flags & MMC_DATA_READ)
				pr_warn(":[EMMC Read]: addr: 0x%llx: size: 0x%X\n",
						(long long unsigned int)mrq->data->blk_addr * 512LL,
			...
			if (mrq->cmd) {
				if (mrq->cmd->opcode == MMC_SWITCH) {
					if ((mrq->cmd->arg & 0xffffff00) == 0x3200100)
						pr_warn(":[EMMC FLUSH]: 0x%x\n", mrq->cmd->arg);
					if ((mrq->cmd->arg & 0xffff0000) == 0x3220000)
						pr_warn(":[EMMC PON]: 0x%x\n", mrq->cmd->arg);
				}
			}
		}
	}
	...
	wmb();
#ifdef CONFIG_MMC_SDHCI_LG1K
	cqhci_writel(cq_host, CQHCI_EMMC_CLK_ON, CQHCI_VENDOR);
#endif
...
static void cqhci_finish_mrq(struct mmc_host *mmc, unsigned int tag)
{
	...
	cq_host->qcnt -= 1;
#ifdef CONFIG_MMC_SDHCI_LG1K
	cqhci_writel(cq_host, CQHCI_EMMC_CLK_OFF, CQHCI_VENDOR);
#endif
