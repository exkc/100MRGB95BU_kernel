...
#ifndef	_UCOM_O24_COMMON_H_
#define	_UCOM_O24_COMMON_H_
...
//#define USE_O24_IPC_INTR 

#ifdef USE_O24_IPC_INTR
#define O24_UCOM_TIMEOUT	300 //(ms)
#else
#define O24_UCOM_TIMEOUT	25  //(ms)
#endif

extern OS_SEM_T			o24_ucom_mutex;

#define O24_KDRV_UCOM_LOCK_INIT()		OS_InitMutex(&o24_ucom_mutex, OS_SEM_ATTR_DEFAULT)
#define O24_KDRV_UCOM_LOCK()                      \
    do{                                             \
        if(OS_LockMutex(&o24_ucom_mutex) < 0)             \
        {                                           \
...
#define O24_KDRV_UCOM_UNLOCK()                    \
    do{                                             \
        OS_UnlockMutex(&o24_ucom_mutex);                  \
    }while(0)
...
extern unsigned int  o24_get_micom_disable(void);
extern void o24_set_micom_disable( unsigned int);
/*----------------------------------------------------------------------------------------
	Extern Variables
----------------------------------------------------------------------------------------*/
