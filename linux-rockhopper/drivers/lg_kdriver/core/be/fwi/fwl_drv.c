...
static const UINT32 gui32FwIndexMask[BE_FW_DWLD_TYPE_MAXNUM] =
	{
...
int BE_FW_ProcHandler(BE_FWI_PROC_ID_T proc_id, struct seq_file* m, void* data)
{
	int	i;

	static const char* fw2str[BE_REG_RW_TYPE_MAXNUM] = { "frc","wcg","led","hdr" };

	seq_printf(m, "fw.mcu_live: %s\n", (BE_FWL_HAL_McuIsAlive())? "on":"off" );
	seq_printf(m, "fw.init_req_mask: 0x%08x\n", gstFwlInfo.ui32FwInitReqMask );
	seq_printf(m, "fw.download_flag: 0x%08x\n", gstFwlInfo.ui32FwDnloadedFlag );

	for(i=0; i<BE_FW_DWLD_TYPE_MAXNUM; i++)
	{
		if (gstFwlInfo.ui32FwBaseAddr[i])
		{
			seq_printf(m, "fw.mcu[%s].base_addr: 0x%08x\n", fw2str[i], gstFwlInfo.ui32FwBaseAddr[i]);
		}
	}

	return BE_FWL_HAL_ProcHandler(proc_id, m, data);
}
