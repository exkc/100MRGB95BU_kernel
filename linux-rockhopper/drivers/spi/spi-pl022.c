			      ...
			      struct spi_transfer *transfer)
{
	...
	if( pl022->cur_chip->speed_hz != pl022->cur_transfer->speed_hz){
		u32 save_hz;
		save_hz = spi->max_speed_hz;
		spi->max_speed_hz = pl022->cur_transfer->speed_hz;

		if(host->setup(spi) >= 0)
			pl022->cur_chip->speed_hz = pl022->cur_transfer->speed_hz;
		spi->max_speed_hz = save_hz;
	}
