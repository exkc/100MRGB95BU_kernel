 ...
 * Added support for o24 fpga/soc
 ...
 *---------------------------------------------------------------------------------------*/
static int DE_DHDR_IP_REG_O24_InitDevHandler(DOLBY_DEVICE_DATA_T *o_PtrDolbyInit);
static int DE_DHDR_IP_REG_O24_Phy2Virt(DOLBY_DEVICE_DATA_T *o_PtrDolbyInit);
...
static int DE_DHDR_IP_REG_O24_InitDevHandler(DOLBY_DEVICE_DATA_T *o_PtrDolbyInit)
{
	...
	do {
		...
		o_PtrDolbyInit->sMDRegMap.unHwAddr = DOLBY_HAL_O24A0_HDMI_MD_ADDR;
		o_PtrDolbyInit->sMDRegMap.unSize = DOLBY_HAL_O24_HDMI_TOTMD_SIZE;
...
int DE_DHDR_IP_REG_O24_Init(DOLBY_DEVICE_DATA_T *io_PtrDolbyInit)
{
	...
	do{
		...
		ret = DE_DHDR_IP_REG_O24_InitDevHandler(io_PtrDolbyInit);
		if(ret != RET_OK) {
			DE_ERROR("Error: Init Dev Handler Failed\n");
			break;
		}

		ret = DE_DHDR_IP_REG_O24_Phy2Virt(io_PtrDolbyInit);
	}while(0);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_O24_SetIrqStatus(DOLBY_DEVICE_DATA_T *i_PtrDolby,UINT32 *o_PtrIrqStatus,int i_unFlag)
{
	int ret = RET_OK;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_DHDR_IP_REG_O24_GetHDMIMetaData(DOLBY_DEVICE_DATA_T *i_sptrDolby,LX_DOLBY_DM_MD_T *o_sptrMetaData)
{
	int ret = RET_ERROR;
	DE_TRACE("@@ ## %s %d\n",__F__,__L__);
	...
	do{
		...
		if(i_sptrDolby->sMDRegMap.unPtrVirtAddr != NULL){
			memcpy(o_sptrMetaData->ucDMMetaData,(void *)i_sptrDolby->sMDRegMap.unPtrVirtAddr,DOLBY_HAL_O24_HDMI_TOTMD_SIZE);
			o_sptrMetaData->usSize = DOLBY_HAL_O24_HDMI_TOTMD_SIZE;
			ret = RET_OK;
		}
		else {
...
int DE_DHDR_IP_REG_O24_SetMdPath(DOLBY_DEVICE_DATA_T *io_PtrDolby,LX_HDR_HDMI_MD_PATH_T e_md_path)
{
	...
	do{
		if(e_md_path == E_MD_IPP_M0 || e_md_path == E_MD_HDR_HDMI_HW)
			io_PtrDolby->sMDRegMap.unHwAddr = DOLBY_HAL_O24A0_HDMI_MD_ADDR;
		else{
			DE_NOTI("Info: Not Supported.\n");
			return RET_ERROR;
		}

		io_PtrDolby->sMDRegMap.unSize = DOLBY_HAL_O24_HDMI_TOTMD_SIZE;
	}while(0);
