...
static OS_PROC_DESC_TABLE_T	_g_irb_m23_device_proc_table[] =
{
...
void m23_irb_proc_init (void);
void m23_irb_proc_cleanup (void);
...
static int m23_write_proc( UINT32 procId, char* command )
{
	...
	switch( procId )
	{
		case PROC_ID_TX:
			...
			irb_m23_func->tx(vendor,opt,key);
			#endif
			break;
		default:
			ret = -1;
...
void m23_irb_proc_init (void)
{
	...
	OS_PROC_CreateEntryEx ( IRB_MODULE, _g_irb_m23_device_proc_table,
										m23_read_proc,
										m23_write_proc );
}
...
static irbs_proc_t m23_irb_proc_func =
{
	.init = m23_irb_proc_init,
	.cleanup = m23_irb_proc_cleanup,
};

irbs_proc_t* get_m23_irb_proc(void)
{
	return &m23_irb_proc_func;
}
