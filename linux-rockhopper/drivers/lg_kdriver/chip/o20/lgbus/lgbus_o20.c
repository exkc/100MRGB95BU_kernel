...
/*----------------------------------------------------------------------------------------
  External Variables
----------------------------------------------------------------------------------------*/
extern lgbus_o20_mpro_func_t	lgbus_o20_mpro_func;
...
static void lgbus_reg_wr(volatile void* addr, UINT32 value, UINT32 paddr)
{
   ...
   else
   {
       ...
       if(i == REG_WR_RETRY_CNT)
       {
           ...
           LGBUS_ERROR("O20 reg_wr error(reg: 0x%08x, curr_val: 0x%08x)\n\n", paddr, LGBUS_RdFL(addr));
       }
   }
}
...
{
	int ret = lgbus_o20_mpro_func.resume();
...
/*========================================================================================
    Implementation Group (LGBUS O20 DESC)
========================================================================================*/
static LGBUS_HW_FUNC_T o20_lgbus_hw_func =
{
