	...
	bool rollup_mode)
{
	if (rollup_mode) {
		...
		SEQ_PUT_DECKB("RssAnon:\t", get_mm_counter(mm, MM_ANONPAGES));
		SEQ_PUT_DECKB(" kB\nRssFile:\t", get_mm_counter(mm, MM_FILEPAGES));
		SEQ_PUT_DECKB(" kB\nRssShmem:\t", get_mm_counter(mm, MM_SHMEMPAGES));
		SEQ_PUT_DECKB(" kB\nVmSwap:\t", get_mm_counter(mm, MM_SWAPENTS));
		seq_puts(m, " kB\n");
	}
