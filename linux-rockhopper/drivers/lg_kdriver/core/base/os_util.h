...
int	__must_check OS_LockMutex_Tag	( OS_SEM_T* pSem, UINT32 timeout, const char* szFile, const char* szFunc, const int nLine );
 ...
 *
 * @def OS_KMalloc
 *	allocate kernel memory and fill zero using kzalloc.
...
#define OS_InitRegionEx(name,pRgn,mgr_type,phys_addr,size,block_size) \
									OS_InitRegionEx_Tag(name,pRgn, mgr_type, (void*)phys_addr, size, block_size, " ", __F__, __L__ )
#define OS_InitRegion(pRgn,phys_addr,size)	\
									OS_InitRegionEx(NULL,pRgn, OS_RGN_MGR_TYPE_HMA, (void*)phys_addr, size, OS_RGN_DEFAULT_BLK_SIZE )
...
int		OS_InitRegionEx_Tag	( char* name, OS_RGN_T* pRgn, OS_RGN_MGR_TYPE_T mgr_type, const void* phys_addr, const int size,  const int block_size,
								const char* szFile, const char* szFunc, const int nLine );
...
/** scan linux cmdline string and find if token exists
 *	@param token [IN] target string
 *	@return RET_OK if token is found, RET_ERROR otherwise
 *
 */
...
// for new version migration
char* os_strdup(const char* str, size_t max_sz);
