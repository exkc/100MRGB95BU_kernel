...
unsigned int get_ucom_disable(void)
{
	unsigned int reg;

	if(ipc_disable == NULL)
		ipc_disable = ioremap(0xfd30004c, 4);

	reg = readl(ipc_disable);

	return reg;
}
EXPORT_SYMBOL(get_ucom_disable) ;

void set_ucom_disable(unsigned int disable)
{
	if(ipc_disable == NULL)
		ipc_disable = ioremap(0xfd30004c, 4);
...
EXPORT_SYMBOL(set_ucom_disable) ;
...
static int lg1k_cec_probe(struct platform_device *pdev)
{
	...
	cec->base = devm_ioremap(&pdev->dev, res->start,
			resource_size(res));
...
static void lg1k_cec_remove(struct platform_device *pdev)
{
