...
#define LBUS_QOS_CMN_BASE0			(0xC6000000)//N
#define LBUS_QOS_CMN_BASE1			(0xC6000400)//S
#define	LBUS_QOS_CMN_BASE2			(0xC6008000)//FAST
...
static ulong  o22_user_mpro[6] = { 0, 0, 0, 0, 0, 0 };
module_param_array(o22_user_mpro, ulong, &user_mpro_cnt, 0644);
...
static lgbus_o22_mpro_t mpro_ctrl[] =
{
	{"CORE",		0xCAA01000},
	{"AXI_PERI",	0xCAA00200},
	{"AHB_PERI",	0xCAA00400},
...
module_param_named(o22_mpro_cnt_limit, isr_cnt_limit, int, 0644);

static u32 mpro_logic = 1;
module_param_named(o22_mpro_logic, mpro_logic, int, 0644);
...
static int lgbus_mpro_resume(void)
{
#ifdef SUPPORT_O22_MPRO_CHIP_KDRV
	int ret;
...
static int lgbus_mpro_get_range(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		for(j = 0; j < O22_MPRO_RANGE_SLOT_CNT; j++)
		{
			//BUS MPRO REG
...
static int lgbus_mpro_regs_save(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		size = sizeof(lgbus_o22_mpro_reg_save_t) / sizeof(u32);
...
#undef LBUS_IRQ
#define LBUS_IRQ			O22_IRQ_LBUS_QOS_INTR
...
#undef CPU_LZMA_IRQ
#undef CPU_FAST_IRQ
#define CPU_IRQ				O22_IRQ_QOS_CPU
#define CPU_AXI_PERI_IRQ	O22_IRQ_QOS_AXI_PERI
#define CPU_AHB_PERI_IRQ	O22_IRQ_QOS_AHB_PERI
#define CPU_LZMA_IRQ		O22_IRQ_QOS_LZMA
#define CPU_FAST_IRQ		O22_IRQ_QOS_CPU_FAST
			...
			if(irq_status & MPRO_ISR_STATUS)
			{
...

				{
					...
					struct
					{
						...
						u32 wr_vio_addr;
						u32 wr_vio_id;
					}rw_vio_data[O22_MPRO_RANGE_SLOT_CNT];
...
int LGBUS_O22_SetMproRange(lgbus_mpro_range_t *mpro_range)
{
#ifdef SUPPORT_O22_MPRO_CHIP_KDRV
	int 		ret = RET_ERROR;
	uint32_t 	port, cnt;
	...
	CHECK_ERROR(mpro_range->port >= O22_MPRO_PORT_MAX, return RET_ERROR, "Invalid port\n");
	CHECK_ERROR(mpro_range->port < O22_MPRO_PORT_START, return RET_ERROR, "Invalid port\n");

	port = mpro_range->port - O22_MPRO_PORT_START;
	...
	for(cnt = 0; cnt < O22_MPRO_RANGE_SLOT_CNT; cnt++)//cnt 0,1,2
	{
		uint32_t ctr0, ctr1;
...
EXPORT_SYMBOL(LGBUS_O22_SetMproRange);

int LGBUS_O22_GetMproRange(lgbus_mpro_range_t *mpro_range)
{
#ifdef SUPPORT_O22_MPRO_CHIP_KDRV
	uint32_t 	port, cnt;
	ulong 		flag;
	...
	for(cnt = 0; cnt < O22_MPRO_RANGE_SLOT_CNT; cnt++)
	{
...
EXPORT_SYMBOL(LGBUS_O22_GetMproRange);

/*
 * port	: port number, ref>lgbus_o22_port_t
 * ctrl	: 1:enable stall, 0:disable stall
 */
int lgbus_o22_mpro_stall_ctrl_module(int port, int ctrl)
{
#ifdef SUPPORT_O22_MPRO_CHIP_KDRV
	ulong addr;
	uint32_t val;
	...
	CHECK_ERROR(port >= O22_MPRO_PORT_MAX, return RET_ERROR, "Invalid port\n");
	CHECK_ERROR(port < O22_MPRO_PORT_START, return RET_ERROR, "Invalid port\n");
	port = port - O22_MPRO_PORT_START;
...
EXPORT_SYMBOL(lgbus_o22_mpro_stall_ctrl_module);
...
static void lgbus_mpro_user_mpro(void)
{
	...
	if (!o22_user_mpro[0] && !o22_user_mpro[1] && !o22_user_mpro[2] && !o22_user_mpro[3] && !o22_user_mpro[4] && !o22_user_mpro[5])
		return;

	LGBUS_NOTI("user_mpro=0x%lx,0x%lx,0x%lx,0x%lx,0x%lx,0x%lx, isr_cnt=%d\n",
			o22_user_mpro[0], o22_user_mpro[1], o22_user_mpro[2], o22_user_mpro[3], o22_user_mpro[4], o22_user_mpro[5], isr_cnt_limit);

	mpro_range.range[0].start   = o22_user_mpro[0];
	mpro_range.range[0].end     = o22_user_mpro[1];
	mpro_range.range[1].start   = o22_user_mpro[2];
	mpro_range.range[1].end     = o22_user_mpro[3];
	mpro_range.range[2].start   = o22_user_mpro[4];
	mpro_range.range[2].end     = o22_user_mpro[5];

	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		mpro_range.port = i;
		mpro_range.port += O22_MPRO_PORT_START;
		LGBUS_O22_SetMproRange(&mpro_range);
	}
}

lgbus_o22_mpro_func_t	lgbus_o22_mpro_func =
{
...
static lgbus_addr_switch_t	o22_addr_switch[O22_ADDR_SWITCH_SLOT];
static lgbus_bwb_t			o22_bwb[O22_BWB_SLOT];
...
static int lgbus_get_addr_switch(void)
{
	...
	for(i = 0; i < O22_ADDR_SWITCH_SLOT; i++)
	{
		u32 d0, d1;
		...
		//[28]		1 : enable
		//[13:00]	14: offset
		o22_addr_switch[i].start = ((u64)(d0 >> 16) & 0x3fff) << 20; 	// 29~16	:14bit
		o22_addr_switch[i].end = (u64)(d0 & 0x3fff) << 20; 				// 13~0		:14bit
		o22_addr_switch[i].offset = (u64)(d1 & 0x3fff) << 20; 			// 13~0		:14bit

		if(o22_addr_switch[i].end)
			o22_addr_switch[i].end |= 0xfffff;

		LGBUS_PRINT("s:"HEX9_4_STR" ~ e:"HEX9_4_STR",o:"HEX9_4_STR"\n", HEX9_4_VAL(o22_addr_switch[i].start)
															 , HEX9_4_VAL(o22_addr_switch[i].end)
															 , HEX9_4_VAL(o22_addr_switch[i].offset));
	}
...
static int lgbus_get_bwb(void)
{
	...
	for(i = 0; i < O22_BWB_SLOT; i++)
	{
		...
		//[30:28] 3 : offset_mode
		//[13:0] 14 : offset
		o22_bwb[i].start = ((u64)(d0 >> 12) & 0x7ff) << 20;	//[22:12] 11 : start
		o22_bwb[i].end = (u64)(d0 & 0x7ff) << 20;			//[10:0]  11 : end

		o22_bwb[i].offset = (u64)(d1 & 0x3fff) << 20;		//[13:0] 14 : offset
		o22_bwb[i].offset_mode = (d1 >> 28) & 0x7;

		if(o22_bwb[i].end)
			o22_bwb[i].end |= 0xfffff;

		LGBUS_PRINT("s:0x%09llx ~ e:0x%09llx,o:0x%09llx[%x]\n", o22_bwb[i].start
															 , o22_bwb[i].end
															 , o22_bwb[i].offset
															 , o22_bwb[i].offset_mode);
	}
...
u64 lgbus_o22_get_ddr_addr(u64 phy_addr)
{
#ifdef SUPPORT_O22_MPRO_CHIP_KDRV
	int i;
	u64 tmp = phy_addr;
	...
	for(i = 0; i < O22_ADDR_SWITCH_SLOT; i++)
	{
		if(o22_addr_switch[i].start <= phy_addr &&
				o22_addr_switch[i].end >= phy_addr)
		{
			phy_addr += o22_addr_switch[i].offset;
	...
	for(i = 0; i < O22_BWB_SLOT; i++)
	{
		if(o22_bwb[i].offset)
		{
			if(o22_bwb[i].start <= bwb_addr
					&& o22_bwb[i].end >= bwb_addr
					&& o22_bwb[i].offset)
			{
				if(o22_bwb[i].offset_mode == BWB_OFFSET_M0
						&& phy_addr < 0x80000000)
				{
					phy_addr += o22_bwb[i].offset;
				}
				else if(o22_bwb[i].offset_mode == BWB_OFFSET_M1
						&& 	phy_addr >= 0x80000000 && phy_addr < 0x100000000)
				{
					phy_addr += o22_bwb[i].offset;
				}
				else if(o22_bwb[i].offset_mode == BWB_OFFSET_M2
...
EXPORT_SYMBOL(lgbus_o22_get_ddr_addr);
        ...
        , const char __user *buf, size_t size, loff_t *off)
{
	...
	int		i;
	int		addr_cnt;
	int		addr_max = O22_MPRO_RANGE_SLOT_CNT * 2;
	u64		addr[O22_MPRO_RANGE_SLOT_CNT * 2] = {0,};
	lgbus_mpro_range_t mpro_range;
	...
	else if(!strncmp(str, "stall_en", strlen("stall_en")))
	{
		...
		while(i < size)
		{
			if(*p++ == ':')
			{
				...
				port_num += O22_MPRO_PORT_START;
				lgbus_o22_mpro_stall_ctrl_module(port_num, 1);
				goto func_exit;
			}
			i++;
		}
	...
	else if(!strncmp(str, "stall_dis", strlen("stall_dis")))
	{
		...
		while(i < size)
		{
			if(*p++ == ':')
			{
				...
				port_num += O22_MPRO_PORT_START;
				lgbus_o22_mpro_stall_ctrl_module(port_num, 0);
				goto func_exit;
			}
			i++;
		}
...
static int lgbus_mpro_proc_asw_op(struct seq_file *s, void *data)
{
#if 0//def SUPPORT_O22_MPRO_CHIP_KDRV
