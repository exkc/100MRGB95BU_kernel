...
extern void kbase_pm_display_perf(void);
...
#endif
extern int kbase_gpu_secure_mode_disable(struct kbase_device *kbdev);
/*
		...
		struct list_head *completed_jobs_ctx)
{
	...
	while (!list_empty(&completed_jobs)) {
		...
		if (--kctx->jctx.job_nr == 0) {
			...
			if ( kbdev->secure_mode && (kbdev->secure_kctx == kctx) )
				kbase_gpu_secure_mode_disable(kbdev);
