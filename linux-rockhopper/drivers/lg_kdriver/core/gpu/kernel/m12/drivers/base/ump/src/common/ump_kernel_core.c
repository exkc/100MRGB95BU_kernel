...
static struct dentry *ump_list_dentry;
static struct dentry *ump_mem_dentry;

static void *ump_debug_list_seq_start(struct seq_file *s, loff_t *pos)
{
	umpp_allocation * alloc = UMP_DD_INVALID_MEMORY_HANDLE;
	umpp_descriptor_mapping * map = device.secure_id_map;
	unsigned int idx;

	mutex_lock(&device.secure_id_map_lock);

	if (map->current_nr_mappings == 0)
		return NULL;

	for (idx = *pos+1; idx < map->current_nr_mappings; idx++)
	{
		*pos = (loff_t)idx+1;

		if (test_bit(idx, map->table->usage))
		{
			alloc = map->table->mappings[idx];
...
static void ump_debug_list_seq_stop(struct seq_file *s, void *data)
{
	mutex_unlock(&device.secure_id_map_lock);
}

static void *ump_debug_list_seq_next(struct seq_file *s, void *data, loff_t *pos)
{
	umpp_allocation * alloc = UMP_DD_INVALID_MEMORY_HANDLE;
	umpp_descriptor_mapping * map = device.secure_id_map;
	unsigned int idx;

	for (idx = *pos; idx < map->current_nr_mappings; idx++)
	{
		*pos = (loff_t)idx+1;
...
static int ump_debug_list_seq_show(struct seq_file *sfile, void *data)
{
	umpp_allocation * alloc = data;
	struct pid *pid;
	struct task_struct *task;

	pid = find_get_pid(alloc->client);
	task = get_pid_task(pid, PIDTYPE_PID);

	seq_printf(sfile, "ump-handle-0x%p, Pid:%6d, Name:%20s, id:%4u, external:%d, size=%10llu\n",
			alloc,
			alloc->client,
			task->comm,
			alloc->id,
...
static const struct seq_operations ump_debug_list_seq_ops = {
	.start = ump_debug_list_seq_start,
	.next = ump_debug_list_seq_next,
	.stop = ump_debug_list_seq_stop,
	.show = ump_debug_list_seq_show,
};

static int ump_debugfs_list_open(struct inode *in, struct file *file)
{
	return seq_open(file, &ump_debug_list_seq_ops);
}

static const struct file_operations ump_debugfs_list_fops = {
	.open = ump_debugfs_list_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release_private,
};

static int ump_debug_mem_seq_show(struct seq_file *sfile, void *data)
{
	umpp_allocation * alloc = UMP_DD_INVALID_MEMORY_HANDLE;
	umpp_descriptor_mapping * map = device.secure_id_map;
	unsigned int idx;
	uint64_t external_import = 0;
	...
	for (idx = 1; idx < map->current_nr_mappings; idx++)
	{
		if (test_bit(idx, map->table->usage))
		{
			alloc = map->table->mappings[idx];
			...
			if(alloc->management_flags&UMP_MGMT_EXTERNAL)
				external_import += alloc->size;
			else
				internal_alloc += alloc->size;
		}
	}

	mutex_unlock(&device.secure_id_map_lock);

	seq_printf(sfile, "@external: %10llu\n@internal: %10llu\n",
			external_import, internal_alloc);

	return 0;
}

UMP_KERNEL_API_EXPORT int ump_debug_mem_for_meminfo(unsigned int *ump_memtotal)
{
	umpp_allocation * alloc = UMP_DD_INVALID_MEMORY_HANDLE;
	umpp_descriptor_mapping * map = device.secure_id_map;
	unsigned int idx;
	unsigned int total_alloc_pages = 0;
	...
	for (idx = 1; idx < map->current_nr_mappings; idx++)
	{
		if (test_bit(idx, map->table->usage))
		{
			...
			if(0 == (alloc->management_flags&UMP_MGMT_EXTERNAL)){
				total_alloc_pages += alloc->blocksCount;
			}
		}
	}
	...
	*ump_memtotal = total_alloc_pages;

	return 0;
}

/**
 * It will be call by mali debugfs.(/sys/kernel/debug/webos_mem/gpu file)
 * return value : total value of ump memory matching with client's pid
 */
UMP_KERNEL_API_EXPORT int ump_debug_mem_for_webos_mem(int client_pid)
{
	umpp_allocation * alloc = UMP_DD_INVALID_MEMORY_HANDLE;
	umpp_descriptor_mapping * map = device.secure_id_map;
	unsigned int idx;
...
static int ump_debugfs_mem_open(struct inode *in, struct file *file)
{
	return single_open(file, ump_debug_mem_seq_show , NULL);
}

static const struct file_operations ump_debugfs_mem_fops = {
	.open = ump_debugfs_mem_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static void ump_debugfs_init(void)
{
	ump_debugfs_dir = debugfs_create_dir("ump", NULL);

	ump_list_dentry = debugfs_create_file("list",
					S_IRUGO,
					ump_debugfs_dir,
					NULL,
					&ump_debugfs_list_fops);

	ump_mem_dentry = debugfs_create_file("memory",
					S_IRUGO,
					ump_debugfs_dir,
					NULL,
					&ump_debugfs_mem_fops);

}

static void ump_debugfs_term(void)
{
	debugfs_remove(ump_mem_dentry);
	debugfs_remove(ump_list_dentry);
	debugfs_remove(ump_debugfs_dir);
}
...
void ump_dd_release(ump_dd_handle mem)
{
	...
	{
		if(sizeof(ump_dd_physical_block_64)*alloc->blocksCount < (uint64_t)PAGE_SIZE)
		{
			kfree(alloc->block_array);
		}
		else
		{
			vfree(alloc->block_array);
		}
	}
...
UMP_KERNEL_API_EXPORT ump_dd_handle ump_dd_create_from_phys_blocks_64(const ump_dd_physical_block_64 * blocks, uint64_t num_blocks, ump_alloc_flags flags, ump_dd_security_filter filter_func, ump_dd_final_release_callback final_release_func, void* callback_data)
{
	...
	// by high ordering issue in kmalloc, replace kamlloc to vmalloc if alloc size is over a PAGE.
	if(sizeof(ump_dd_physical_block_64)*num_blocks < (uint64_t)PAGE_SIZE)
	{
		alloc->block_array = kzalloc(sizeof(ump_dd_physical_block_64) * num_blocks,__GFP_HARDWALL | GFP_KERNEL);
	}
	else
	{
		alloc->block_array = vmalloc(sizeof(ump_dd_physical_block_64)*num_blocks);
		memset(alloc->block_array,0,sizeof(ump_dd_physical_block_64)*num_blocks);
	}
