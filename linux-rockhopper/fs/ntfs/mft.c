 ...
 */
int ntfs_mft_record_check(const ntfs_volume *vol, MFT_RECORD *m,
			  unsigned long mft_no)
{
	...
	if (le32_to_cpu(m->bytes_allocated) != vol->mft_record_size) {
		ntfs_error(sb, "Record %llu has corrupt allocation size "
			       "(%u <> %u)\n", (unsigned long long)mft_no,
			       vol->mft_record_size,
	...
	if (le32_to_cpu(m->bytes_in_use) > vol->mft_record_size) {
		ntfs_error(sb, "Record %llu has corrupt in-use size "
			   "(%u > %u)\n", (unsigned long long)mft_no,
...
static inline MFT_RECORD *map_mft_record_folio(ntfs_inode *ni)
#else
...
#endif
{
	...
	if (!IS_ERR(folio)) {
		...
		post_read_mst_fixup((NTFS_RECORD*)ni->mrec, vol->mft_record_size);

		/* Catch multi sector transfer fixup errors. */
		if (!ntfs_mft_record_check(vol, (MFT_RECORD *)ni->mrec, ni->mft_no)) {
			kunmap_local(addr);
			ni->folio = folio;
			ni->folio_ofs = ofs;
			return ni->mrec;
...
err_out:
		ntfs_error(vol->sb, "Failed to synchronize $MFTMirr (error "
				...
				"on umount.  Run chkdsk on the partition "
...
int write_mft_record_nolock(ntfs_inode *ni, MFT_RECORD *m, int sync)
{
	...
	fixup_m = (MFT_RECORD *)(kaddr + ni->folio_ofs);
#else
	kaddr = kmap(page);
	fixup_m = (MFT_RECORD *)(kaddr + ni->page_ofs);
#endif
	memcpy(fixup_m, m, vol->mft_record_size);

	/* Apply the mst protection fixups. */
	err = pre_write_mst_fixup((NTFS_RECORD*)fixup_m, vol->mft_record_size);
	if (err) {
...
static int ntfs_mft_record_format(const ntfs_volume *vol, const s64 mft_no)
{
	...
	BUG_ON(!folio_test_uptodate(folio));
	folio_clear_uptodate(folio);
	m = (MFT_RECORD*)((u8*)kmap_local_folio(folio, 0) + ofs);
#else
	...
	pre_write_mst_fixup((NTFS_RECORD*)m, vol->mft_record_size);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0)
	flush_dcache_folio(folio);
	folio_mark_uptodate(folio);
	/*
	 ...
	 */
	mark_ntfs_record_dirty(folio, ofs);
	folio_unlock(folio);
	ntfs_unmap_folio(folio, m);
#else
	flush_dcache_page(page);
 ...
 */
int ntfs_mft_record_alloc(ntfs_volume *vol, const int mode,
			  ntfs_inode **ni, ntfs_inode *base_ni,
			  MFT_RECORD **ni_mrec)
{
...
mft_rec_already_initialized:
	...
	if (!record_formatted) {
		...
		if (ntfs_is_file_record(m->magic) &&
			...
			ntfs_warning(vol->sb, "Mft record 0x%llx was marked "
...
bitmap_rollback:
	...
	memalloc_nofs_restore(memalloc_flags);
sync_rollback:
	ntfs_error(vol->sb, "Eeek! Rollback failed in ntfs_mft_record_free().  "
...
int ntfs_mst_pre_write_fixup(NTFS_RECORD *b, const u32 size)
{
	...
	if (!b || ntfs_is_baad_record(b->magic) ||
		...
		pr_err("%s: bad argument", __FUNCTION__);
		return -EINVAL;
	}
	/* Setup the variables. */
