...
static void kbase_file_delete(struct kbase_file *const kfile)
{
	...
	if (atomic_read(&kfile->setup_state) == KBASE_FILE_COMPLETE) {
		...
		/* LGE ext. */
		{
			extern void kbase_lgsic_profile_remove(struct kbase_context *kctx, struct kbase_device *kbdev);
			kbase_lgsic_profile_remove(kfile->kctx, kfile->kbdev);
		}
				  ...
				  base_context_create_flags const flags)
{
	...
	if (IS_ERR_OR_NULL(kctx->kctx_dentry)) {
		...
		/* LGE ext. */
		{
			extern int kbase_lgsic_ext_profile_insert(struct kbase_context *kctx, struct kbase_device *kbdev);
			kbase_lgsic_ext_profile_insert(kctx, kbdev);
		}
	}
#endif /* CONFIG_DEBUG_FS */
...
static struct dentry *init_debugfs(struct kbase_device *kbdev)
{
	...
	/* LGE ext. */
	extern int get_gpu_total_memory (void);
	extern int get_gpu_cached_memory (void);
	extern int get_gpu_pool_memory (void);
	...
	kbdev->lgext_gpu_directory = proc_mkdir("gpu", NULL);
	kbdev->lgext_um_directory = proc_mkdir("um", NULL);
