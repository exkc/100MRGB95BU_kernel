...
int hdmi_rx_ctrl_hdcp_config(struct hdmi_rx_ctrl *ctx, const struct hdmi_rx_ctrl_hdcp *hdcp)
{
	...
	ctx->log_info("Port[%d] : Initiate HDCP1.4, Mode[%s]", ctx->device_id, hdcp->repeat?"Repeater":"Reciever");	
 ...
 * @return error code
 */
int hdmi_rx_ctrl_hdcp_ksv_list(struct hdmi_rx_ctrl *ctx, uint32_t *list, unsigned count, unsigned depth)
{
	int error = 0;
	unsigned i = 0;
	unsigned k = 0;
	...
	if (io_read(ctx, RA_HDCP_RPT_CTRL, WAITING_KSV) == 0) {
		ctx->log_info("Warning[%d] : HDCP14 Engine Not ready for KSV writing", ctx->device_id);	
	...
	for (i = 0; i < count; i++) {
		...
		if (k < HDCP_KEY_WR_TRIES) {
			...
			ctx->bsp_write(RA_HDCP_RPT_KSVFIFO1, list[i * 2 + 1]);
			ctx->bsp_write(RA_HDCP_RPT_KSVFIFO0, list[i * 2 + 0]);

			ctx->log_info("Notice [%d] : [%d] KSV => [0x%08x] [0x%08x]\n", ctx->device_id, i, list[i * 2 + 0], list[i * 2 + 1]);
		} else {
			ctx->log_info("Error[%d] : HDCP1.4 Engine give up on [%d] KSV due to Engine Wait timeout[%d]", ctx->device_id, i, k);
			error = -EAGAIN;
			break;
		}
	}
	if (error == 0) {
		...
		ctx->log_info("Notice [%d] : HDCP14 Engine successfully handled [%d]count of KSV", ctx->device_id, count);	
	}
	return error;
}
