...
static int _CBUF_Sync(CBUF_T * cb) {
	...
	do {
		...
		if (((cb->head & 0x3) || (cb->tail & 0x3)) || ((cb->head > cb->size) || (cb->tail > cb->size))) {
			DEMOD_WARN("retry %d, invalid head %08x or tail %08x\n", retry, cb->head, cb->tail);
			rmb();
			continue;
		}
	} while (retry--);
...
#if (DEMOD_PLATFORM == DEMOD_PLATFORM_XTENSA)
	DEMOD_MEM_Read(cb->regs.state, pstate);
#else
...
static int _CBUF_Status(CBUF_T * cb, char * dbglog) {
	...
	return SPRINTF(	dbglog,
			"H %3d(@x%08x), T %3d(@x%08x), seq %3d, size %3d, used %3d, left %3d, base @x%8X\n",
...
#if (DEMOD_PLATFORM == DEMOD_PLATFORM_XTENSA)
	DEMOD_MEM_Write_Buff(cb->base + cb->tail, (UINT32 *)&head, cont);
	if (sizeof(CBUF_H) != cont) {
		DEMOD_MEM_Write_Buff(cb->base, (UINT32 *)((char *)&head + cont), sizeof(CBUF_H) - cont);
	}
#else
...
int DEMOD_L0_CBUF_Read(CBUF_T * cb, CBUF_H * phead, char * pdata) {
	...
	if (_CBUF_Used(cb) == 0) {
		DEMOD_ERROR("cbuf_rd: empty, base 0x%08x, head %3d tail %3d\n", cb->base, cb->head, cb->tail);
...
#if (DEMOD_PLATFORM == DEMOD_PLATFORM_XTENSA)
	DEMOD_MEM_Read_Buff(cb->base + cb->head, (UINT32 *)phead, cont);

	if (sizeof(CBUF_H) != cont) {
		DEMOD_MEM_Read_Buff(cb->base, (UINT32 *)((char *)phead + cont), sizeof(CBUF_H) - cont);
	}
#else
