...
#define DBGLOG	"dbg-log"
#define DBGLOGGZ	"dbg-log.0.gz"
...
static int copy_log_raw_drv(struct file *src_file, size_t size, size_t start_offset,
					struct wdt_log_header *wlh)
{
	...
	while (size > 0) {
		...
		if (!is_first) {
			done_bytes = read_bytes = kernel_read(src_file, read_buf,
									len, &offset);
		} else {
			if ((len + sizeof(struct wdt_log_header)) > BUF_SIZE)
				len -= sizeof(struct wdt_log_header);

			done_bytes = read_bytes = kernel_read(src_file,
		...
		if (read_bytes) {
			...
			if (ll_mmc_write((u8 *)read_buf, save_addr,
						read_bytes) < 0) {
...
static void copy_log_to_mem(struct file *src_file, size_t size, size_t start_offset,
		struct wdt_log_header *wlh)
{
	int read_bytes;
	void *addr;
	...
	read_bytes = kernel_read(src_file,
			addr + sizeof(struct wdt_log_header), size, &pos);
	if (read_bytes < 0)
...
static bool iterate_copy_log(struct dir_context *ctx, const char *name,
		int namlen, loff_t off, u64 ino, unsigned int d_type)
{
	...
	if (!saveable_file(name, dc->log_type)) {
		...
		vfs_getattr_nosec(&src_file->f_path, &ks, STATX_INO, AT_STATX_SYNC_AS_STAT);
		size = (size_t)ks.size;

		if ((total_log_size + size) >= MAX_LOG_SIZE) {
			if (MAX_LOG_SIZE - total_log_size - BLOCK_SIZE <= 0)
				goto OUT;

			size = MAX_LOG_SIZE - total_log_size - BLOCK_SIZE;
			if (ks.size - size <= 0)
				goto OUT;
		...
		wlh.timestamp = ns_to_timespec64(ktime_get_real_fast_ns());
		wlh.size = size;
		wlh.written_size = size;

		if (!is_crash_image())
			copy_log_raw_drv(src_file, size, offset, &wlh);
		else
			copy_log_to_mem(src_file, size, offset, &wlh);
OUT:
