...
#include "be_pwm_hw_o26.h"
#include "ovi_hal.h"

#include "../reg/be_reg_def_o26.h"
#include "../reg/be_reg_pwm_o26.h"
...
#define BE_USE_CTOP_CODES_FOR_O26

#ifdef BE_USE_CTOP_CODES_FOR_O26
...
----------------------------------------------------------------------------------------*/
/* 2025509, http://jira.lge.com/issue/browse/SICDTV-15633 */
#define BE_PWM_HW_O26_DDR_DATA_OFFSET	0x200

#define BE_PWM_HW_O26_MAGIC 0xCAFE
...
typedef union {
	UINT32 udata32;
	struct {
	UINT32 duty  :16;//(15:00)
	UINT32 freq  :16;//(31:16)
	};
} PWM_DDR_INFO_O26_T;

typedef struct {
	PWM_DDR_H_O26_T                header;
	BE_O26A0_REG_PWM_PWM0_CTRL0_T  pwm0_ctrl0;//0x0010
	BE_O26A0_REG_PWM_PWM0_CTRL1_T  pwm0_ctrl1;//0x0014
	BE_O26A0_REG_PWM_PWM0_CTRL2_T  pwm0_ctrl2;//0x0018
	BE_O26A0_REG_PWM_PWM1_CTRL0_T  pwm1_ctrl0;//0x001C
	BE_O26A0_REG_PWM_PWM1_CTRL1_T  pwm1_ctrl1;//0x0020
	BE_O26A0_REG_PWM_PWM1_CTRL2_T  pwm1_ctrl2;//0x0024
	PWM_DDR_INFO_O26_T             pwm0_info;
	PWM_DDR_INFO_O26_T             pwm1_info;
} PWM_DDR_O26_T;
...
static volatile PWM_DDR_O26_T *gBE_PWM_DDR_O26 = NULL;
static phys_addr_t _g_pwm_o26_ddr_base = 0;
static struct be_pwm_ctx _g_be_pwm_ctx;

static uint _g_be_pwm_ddr = 0;//set bootopts lg1k.vbe_pwm_ddr=1; save ;
module_param_named(vbe_pwm_ddr, _g_be_pwm_ddr, uint, 0644);
...
static int _be_pwm_hw_o26_restore_pwm(void);

/*========================================================================================
	Implementation Group
========================================================================================*/
static int _be_pwm_hw_o26_init_pwm_ddr(void)
{
	PWM_DDR_H_O26_T header = {.udata32=0};
	UINT32 ddr_size = sizeof(PWM_DDR_O26_T) - sizeof(PWM_DDR_H_O26_T);
	...
	rmb();
	header.udata32 = gBE_PWM_DDR_O26->header.udata32;

	if (header.magic == BE_PWM_HW_O26_MAGIC && header.size == ddr_size) {
		BE_NOTI("already done(base:0x%08x)\n",(UINT32)_g_pwm_o26_ddr_base);
		return 0;
	}

	gBE_PWM_DDR_O26->pwm0_ctrl0.udata32 = 0;
	gBE_PWM_DDR_O26->pwm0_ctrl1.udata32 = 0;
	gBE_PWM_DDR_O26->pwm0_ctrl2.udata32 = 0;
	gBE_PWM_DDR_O26->pwm1_ctrl0.udata32 = 0;
	gBE_PWM_DDR_O26->pwm1_ctrl1.udata32 = 0;
	gBE_PWM_DDR_O26->pwm1_ctrl2.udata32 = 0;
	gBE_PWM_DDR_O26->pwm0_info.udata32 = 0;
	gBE_PWM_DDR_O26->pwm1_info.udata32 = 0;
	// memset((void *)gBE_PWM_DDR_O26,0,sizeof(PWM_DDR_O26_T));
	header.magic = BE_PWM_HW_O26_MAGIC;
	header.size = ddr_size;
	header.count = 1;
	gBE_PWM_DDR_O26->header.udata32 = header.udata32;
	wmb();
	BE_NOTI("done(base:0x%08x)\n",(UINT32)_g_pwm_o26_ddr_base);

	return 0;
}

static int _be_pwm_hw_o26_init_ctx(void)
{
	UINT32 temp = 0;
	...
	do {
		if (RET_OK != OS_ScanKernelCmdlineToken("WirelessAV") && !_g_be_pwm_ddr) break;
		if (!gpBeChInfMem) break;
		if (!gpBeChInfMem->frame_base) break;
		if (!gpBeChInfMem->frame_size) break;
		_g_pwm_o26_ddr_base = gpBeChInfMem->frame_base + BE_PWM_HW_O26_DDR_DATA_OFFSET;
		gBE_PWM_DDR_O26 = (volatile PWM_DDR_O26_T *)vmap_phys(_g_pwm_o26_ddr_base, sizeof(PWM_DDR_O26_T));
		_be_pwm_hw_o26_init_pwm_ddr();
	} while (0);

	return 0;
}

int BE_PWM_HW_O26_Init(void)
{
	int ret = RET_OK;
	BE_PWM_ID_T pwmId;
...
int BE_PWM_HW_O26_Suspend(void)
{
	BE_NOTI("done\n");
	return 0;
}

int BE_PWM_HW_O26_Resume(void)
{
	_be_pwm_hw_o26_init_pwm_ddr();
	_be_pwm_hw_o26_restore_pwm();
	return 0;
}

int BE_PWM_HW_O26_SetClk(UINT32 clk)
{
	BE_NOTI("clk:%d\n",clk);
...
int BE_PWM_HW_O26_SetCtrlStop(UINT32 stop)
{
	int	i;
	BE_NOTI("stop:%d\n",stop);
...
int BE_PWM_HW_O26_SetPwmControl(BE_PWM_CTRL_T *pstParams)
{
	int ret = RET_OK;
...
int BE_PWM_HW_O26_SetPwmAdaptFreqControl(BE_PWM_ADAPT_FREQ_CTRL_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		if (freqSet)
		{
			...
			ret = BE_PWM_HW_O26_SetPwmSync(&stParams);
...
int BE_PWM_HW_O26_SetPwmFrequency(BE_PWM_FREQ_T *pstParams)
{
	int ret = RET_OK;
	BE_PWM_ID_T nPort;
...
int BE_PWM_HW_O26_SetPwmDutyCycle(BE_PWM_DUTY_T *pstParams)
{
	int ret = RET_OK;
...
int BE_PWM_HW_O26_SetPwmDutyRatio(BE_PWM_DUTY_RATIO_T *pstParams)
{
	int ret = RET_OK;
	BE_PWM_ID_T nPort;
...
int BE_PWM_HW_O26_SetPwmParams(LX_BE_PWM_PARAMS_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		for(i = 0; i < LX_BE_PWM_PORT_MAX; i++)
		{
			if ((pwmParams.portMask >> i) & 0x01)
			{
				...
				ret = BE_PWM_HW_O26_SetPwmControl(&pwmCtrlParam);
				if(ret) BREAK_WRONG(ret);
				...
				ret = BE_PWM_HW_O26_SetPwmAdaptFreqControl(&pwmAdaptFreqCtrlParam);
				if(ret) BREAK_WRONG(ret);
				...
				if (!pwmParams.portParams[i].adaptFreqEn)
				{
					...
					ret = BE_PWM_HW_O26_SetPwmFrequency(&pwmFreqParams);//set freq,duty
					if(ret) BREAK_WRONG(ret);
				}
				else
				{
					pwmDutyParam.port = i;
					ret = BE_PWM_HW_O26_SetPwmDutyCycle(&pwmDutyParam);//set duty
					if(ret) BREAK_WRONG(ret);
				}
			}
		}
...
int BE_PWM_HW_O26_GetPwmInfo(BE_EXTERNAL_PWM_T *pstParams)
{
	int ret = RET_OK;
	UINT32 freq_mode = 0;
	UINT32 time_low[4] = { 0 };
...
int BE_PWM_HW_O26_SetPwmSync(BE_PWM_FREQ_T *pstParams)
{
	int ret = RET_OK;
	UINT32 pwmFreq;
...
static int PWM_SetFrequency(UINT32 pwmId, UINT32 freq)
{
	...
	if (1) // Check clock setting
	{
		...
		regField = CTOP_CTRL_O26Ax_RdFd(SCRG_CTRL_TCON, crg_tcon004, reg_pix2_clk_sel);//need to check
		#endif
...
static int _be_pwm_hw_o26_restore_pwm(void)
{
	...
	for (i=0; i<BE_PWM_MAX; i++) {
		if (gPwmInfo[i].ctrl.pwm_en) {
			UINT32 pwm_en;
			if (i == BE_PWM0) {
				BE_PWM_O26_RdFL(pwm0_ctrl0);
				BE_PWM_O26_Rd01(pwm0_ctrl0, pwm0_en, pwm_en);
			} else if (i == BE_PWM1) {
				BE_PWM_O26_RdFL(pwm1_ctrl0);
				BE_PWM_O26_Rd01(pwm1_ctrl0, pwm1_en, pwm_en);
			} else if (i == BE_PWM2) {
				BE_PWM_O26_RdFL(pwm2_ctrl0);
				BE_PWM_O26_Rd01(pwm2_ctrl0, pwm2_en, pwm_en);
			} else {
				BE_PWM_O26_RdFL(pwm3_ctrl0);
				BE_PWM_O26_Rd01(pwm3_ctrl0, pwm3_en, pwm_en);
			}

			if (pwm_en == 0) {
				memset(&gPrevPwmInfo[i],0xff,sizeof(PWM_INFO_S_T));
				BE_NOTI("pwm%d (duty,offs:%d,%d) on restore\n",i,gPwmInfo[i].duty,gPwmInfo[i].offs);
				ret = PWM_SetDuty(i, gPwmInfo[i].duty, gPwmInfo[i].offs);
				if (ret) {
					BE_ERROR("pwm%d PWM_SetDuty err\n",i);
					break;
				}
			} else {
				BE_NOTI("pwm%d (duty,offs:%d,%d) on skip\n",i,gPwmInfo[i].duty,gPwmInfo[i].offs);
			}
		} else {
			BE_NOTI("pwm%d (duty,offs:%d,%d) off skip\n",i,gPwmInfo[i].duty,gPwmInfo[i].offs);
		}
	}
...
static int PWM_SetRegCtrl0(UINT32 pwmId)
{
	...
	do {
		...
		switch (pwmId) {
			case 0 :
				BE_PWM_O26_RdFL(pwm0_ctrl0);
				BE_PWM_O26_Wr01(pwm0_ctrl0, pwm0_en, gPwmInfo[pwmId].ctrl.reg_en);
				BE_PWM_O26_Wr01(pwm0_ctrl0, pwm0_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
				BE_PWM_O26_Wr01(pwm0_ctrl0, pwm0_resolution, gPwmInfo[pwmId].ctrl.pwm_resolution);
				BE_PWM_O26_Wr01(pwm0_ctrl0, pwm0_inv, gPwmInfo[pwmId].ctrl.pwm_inv);
				BE_PWM_O26_Wr01(pwm0_ctrl0, pwm0_sel, gPwmInfo[pwmId].ctrl.pwm_sel);
				BE_PWM_O26_Wr01(pwm0_ctrl0, pwm0_width_falling_pos, gPwmInfo[pwmId].ctrl.pwm_width_falling_pos);
				BE_PWM_O26_WrFL(pwm0_ctrl0);
				break;
			case 1 :
				BE_PWM_O26_RdFL(pwm1_ctrl0);
				BE_PWM_O26_Wr01(pwm1_ctrl0, pwm1_en, gPwmInfo[pwmId].ctrl.reg_en);
				BE_PWM_O26_Wr01(pwm1_ctrl0, pwm1_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
				BE_PWM_O26_Wr01(pwm1_ctrl0, pwm1_resolution, gPwmInfo[pwmId].ctrl.pwm_resolution);
				BE_PWM_O26_Wr01(pwm1_ctrl0, pwm1_inv, gPwmInfo[pwmId].ctrl.pwm_inv);
				BE_PWM_O26_Wr01(pwm1_ctrl0, pwm1_sel, gPwmInfo[pwmId].ctrl.pwm_sel);
				BE_PWM_O26_Wr01(pwm1_ctrl0, pwm1_width_falling_pos, gPwmInfo[pwmId].ctrl.pwm_width_falling_pos);
				BE_PWM_O26_WrFL(pwm1_ctrl0);
				break;
			case 2 :
				BE_PWM_O26_RdFL(pwm2_ctrl0);
				BE_PWM_O26_Wr01(pwm2_ctrl0, pwm2_en, gPwmInfo[pwmId].ctrl.reg_en);
				BE_PWM_O26_Wr01(pwm2_ctrl0, pwm2_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
				BE_PWM_O26_Wr01(pwm2_ctrl0, pwm2_resolution, gPwmInfo[pwmId].ctrl.pwm_resolution);
				BE_PWM_O26_Wr01(pwm2_ctrl0, pwm2_inv, gPwmInfo[pwmId].ctrl.pwm_inv);
				BE_PWM_O26_Wr01(pwm2_ctrl0, pwm2_sel, gPwmInfo[pwmId].ctrl.pwm_sel);
				BE_PWM_O26_Wr01(pwm2_ctrl0, pwm2_width_falling_pos, gPwmInfo[pwmId].ctrl.pwm_width_falling_pos);
				BE_PWM_O26_WrFL(pwm2_ctrl0);
				break;
			case 3 :
				BE_PWM_O26_RdFL(pwm3_ctrl0);
				BE_PWM_O26_Wr01(pwm3_ctrl0, pwm3_en, gPwmInfo[pwmId].ctrl.reg_en);
				BE_PWM_O26_Wr01(pwm3_ctrl0, pwm3_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
				BE_PWM_O26_WrFL(pwm3_ctrl0);
				break;
			default :
				break;
		}
...
static int PWM_SetRegCtrl12(UINT32 pwmId, UINT32 force_ctrl)
{
	int ret = RET_OK;
	...
	switch (pwmId) {
		case 0 :
			...
			{
				BE_PWM_O26_RdFL(pwm0_ctrl1);
			}
			BE_PWM_O26_RdFL(pwm0_ctrl2);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				BE_PWM_O26_Wr01(pwm0_ctrl1, pwm0_free_width, gPwmInfo[pwmId].ctrl.pwm_free_width);
			}
			BE_PWM_O26_Wr01(pwm0_ctrl2, pwm0_method, gPwmInfo[pwmId].ctrl.pwm_method);
			BE_PWM_O26_Wr01(pwm0_ctrl2, pwm0_fc_h_disp, gPwmInfo[pwmId].ctrl.reg_fc_h_disp);
			BE_PWM_O26_Wr01(pwm0_ctrl2, pwm0_fc_l_disp, gPwmInfo[pwmId].ctrl.reg_fc_l_disp);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				BE_PWM_O26_WrFL(pwm0_ctrl1);
			}
			BE_PWM_O26_WrFL(pwm0_ctrl2);
			break;
		case 1 :
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				BE_PWM_O26_RdFL(pwm1_ctrl1);
			}
			BE_PWM_O26_RdFL(pwm1_ctrl2);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				BE_PWM_O26_Wr01(pwm1_ctrl1, pwm1_free_width, gPwmInfo[pwmId].ctrl.pwm_free_width);
			}
			BE_PWM_O26_Wr01(pwm1_ctrl2, pwm1_method, gPwmInfo[pwmId].ctrl.pwm_method);
			BE_PWM_O26_Wr01(pwm1_ctrl2, pwm1_fc_h_disp, gPwmInfo[pwmId].ctrl.reg_fc_h_disp);
			BE_PWM_O26_Wr01(pwm1_ctrl2, pwm1_fc_l_disp, gPwmInfo[pwmId].ctrl.reg_fc_l_disp);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				BE_PWM_O26_WrFL(pwm1_ctrl1);
			}
			BE_PWM_O26_WrFL(pwm1_ctrl2);
			break;
		case 2 :
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				BE_PWM_O26_RdFL(pwm2_ctrl1);
			}
			BE_PWM_O26_RdFL(pwm2_ctrl2);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				BE_PWM_O26_Wr01(pwm2_ctrl1, pwm2_free_width, gPwmInfo[pwmId].ctrl.pwm_free_width);
			}
			BE_PWM_O26_Wr01(pwm2_ctrl2, pwm2_method, gPwmInfo[pwmId].ctrl.pwm_method);
			BE_PWM_O26_Wr01(pwm2_ctrl2, pwm2_fc_h_disp, gPwmInfo[pwmId].ctrl.reg_fc_h_disp);
			BE_PWM_O26_Wr01(pwm2_ctrl2, pwm2_fc_l_disp, gPwmInfo[pwmId].ctrl.reg_fc_l_disp);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				BE_PWM_O26_WrFL(pwm2_ctrl1);
			}
			BE_PWM_O26_WrFL(pwm2_ctrl2);
			break;
		default :
			break;
	}
...
static int _be_pwm_set_ddr_ctrl(UINT32 pwm_id)
{
	PWM_DDR_H_O26_T header;
	BE_O26A0_REG_PWM_T *p_shw;
	PWM_DDR_INFO_O26_T info = {.udata32=0};
	UINT32 pwm_ctrl[3];
	do {
		if (gBE_PWM_DDR_O26 == NULL) break;
		if (pwm_id > 1) break;
		p_shw = (lx_chip_rev() <= LX_CHIP_REV(O26,A1))? gBE_PWM_O26.shdw.a0:gBE_PWM_O26.shdw.b0;
		info.duty = (gPwmInfo[pwm_id].duty_adj < 0xffff)? gPwmInfo[pwm_id].duty_adj:0xffff;
		info.freq = (gPwmInfo[pwm_id].freq < 0xffff)? gPwmInfo[pwm_id].freq:0xffff;
		if (pwm_id == 0) {
			pwm_ctrl[0] = p_shw->pwm0_ctrl0.udata32;
			pwm_ctrl[1] = p_shw->pwm0_ctrl1.udata32;
			pwm_ctrl[2] = p_shw->pwm0_ctrl2.udata32;
			gBE_PWM_DDR_O26->pwm0_ctrl0.udata32 = pwm_ctrl[0];
			gBE_PWM_DDR_O26->pwm0_ctrl1.udata32 = pwm_ctrl[1];
			gBE_PWM_DDR_O26->pwm0_ctrl2.udata32 = pwm_ctrl[2];
			gBE_PWM_DDR_O26->pwm0_info.udata32 = info.udata32;
			// *((volatile UINT32 *)(&(gBE_PWM_DDR_O26->pwm0_ctrl0))) = *((volatile UINT32 *)(&(p_shw->pwm0_ctrl0)));
			// *((volatile UINT32 *)(&(gBE_PWM_DDR_O26->pwm0_ctrl1))) = *((volatile UINT32 *)(&(p_shw->pwm0_ctrl1)));
			// *((volatile UINT32 *)(&(gBE_PWM_DDR_O26->pwm0_ctrl2))) = *((volatile UINT32 *)(&(p_shw->pwm0_ctrl2)));
			// *((volatile UINT32 *)(&(gBE_PWM_DDR_O26->pwm0_info))) = info.udata32;
		} else {
			pwm_ctrl[0] = p_shw->pwm1_ctrl0.udata32;
			pwm_ctrl[1] = p_shw->pwm1_ctrl1.udata32;
			pwm_ctrl[2] = p_shw->pwm1_ctrl2.udata32;
			gBE_PWM_DDR_O26->pwm1_ctrl0.udata32 = pwm_ctrl[0];
			gBE_PWM_DDR_O26->pwm1_ctrl1.udata32 = pwm_ctrl[1];
			gBE_PWM_DDR_O26->pwm1_ctrl2.udata32 = pwm_ctrl[2];
			gBE_PWM_DDR_O26->pwm1_info.udata32 = info.udata32;
			// *((volatile UINT32 *)(&(gBE_PWM_DDR_O26->pwm1_ctrl0))) = *((volatile UINT32 *)(&(p_shw->pwm1_ctrl0)));
			// *((volatile UINT32 *)(&(gBE_PWM_DDR_O26->pwm1_ctrl1))) = *((volatile UINT32 *)(&(p_shw->pwm1_ctrl1)));
			// *((volatile UINT32 *)(&(gBE_PWM_DDR_O26->pwm1_ctrl2))) = *((volatile UINT32 *)(&(p_shw->pwm1_ctrl2)));
			// *((volatile UINT32 *)(&(gBE_PWM_DDR_O26->pwm1_info))) = info.udata32;
		}
		rmb();
		header.udata32 = gBE_PWM_DDR_O26->header.udata32;
		...
		gBE_PWM_DDR_O26->header.udata32 = header.udata32;
		wmb();
		BE_TRACE2_PWM("%d] ddr 0x%08x 0x%08x 0x%08x 0x%08x\n",pwm_id,pwm_ctrl[0],pwm_ctrl[1],pwm_ctrl[2],info.udata32);
	} while(0);
	return 0;
}
...
static int PWM_SetRegCtrl(UINT32 pwmId, UINT32 force_ctrl)
{
	...
	PWM_SetRegCtrl12(pwmId,force_ctrl);
	_be_pwm_set_ddr_ctrl(pwmId);
...
static int PWM_SetTiming(UINT32 pwmId, PWM_ORD_S_T pwmOrd, UINT32 *pR1st, UINT32 *pR2nd, UINT32 *pF1st, UINT32 *pF2nd)
{
	...
	switch (pwmId) {
		case 0 :
			BE_PWM_O26_RdFL(pwm_v_load_write);
			BE_PWM_O26_RdFL(pwm0_v_r);
			BE_PWM_O26_RdFL(pwm0_v_f);
			BE_PWM_O26_RdFL(pwm0_v_sub_r);
			BE_PWM_O26_RdFL(pwm0_v_sub_f);
			BE_PWM_O26_Wr01(pwm_v_load_write, pwm0_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
			BE_PWM_O26_Wr01(pwm0_v_r, pwm0_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
			BE_PWM_O26_Wr01(pwm0_v_r, pwm0_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			BE_PWM_O26_Wr01(pwm0_v_f, pwm0_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
			BE_PWM_O26_Wr01(pwm0_v_f, pwm0_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			BE_PWM_O26_Wr01(pwm_v_load_write, pwm0_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			BE_PWM_O26_Wr01(pwm0_v_sub_r, pwm0_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
			BE_PWM_O26_Wr01(pwm0_v_sub_r, pwm0_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			BE_PWM_O26_Wr01(pwm_v_load_write, pwm0_v_sub_f_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			BE_PWM_O26_Wr01(pwm0_v_sub_f, pwm0_v_sub_f, gPwmInfo[pwmId].t2nd.pwm_v_f);
			BE_PWM_O26_Wr01(pwm0_v_sub_f, pwm0_v_sub_f_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			BE_PWM_O26_WrFL(pwm_v_load_write);
			BE_PWM_O26_WrFL(pwm0_v_r);
			BE_PWM_O26_WrFL(pwm0_v_f);
			BE_PWM_O26_WrFL(pwm0_v_sub_r);
			BE_PWM_O26_WrFL(pwm0_v_sub_f);
			break;
		case 1 :
			BE_PWM_O26_RdFL(pwm_v_load_write);
			BE_PWM_O26_RdFL(pwm1_v_r);
			BE_PWM_O26_RdFL(pwm1_v_f);
			BE_PWM_O26_RdFL(pwm1_v_sub_r);
			BE_PWM_O26_RdFL(pwm1_v_sub_f);
			BE_PWM_O26_Wr01(pwm_v_load_write, pwm1_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
			BE_PWM_O26_Wr01(pwm1_v_r, pwm1_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
			BE_PWM_O26_Wr01(pwm1_v_r, pwm1_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			BE_PWM_O26_Wr01(pwm1_v_f, pwm1_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
			BE_PWM_O26_Wr01(pwm1_v_f, pwm1_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			BE_PWM_O26_Wr01(pwm_v_load_write, pwm1_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			BE_PWM_O26_Wr01(pwm1_v_sub_r, pwm1_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
			BE_PWM_O26_Wr01(pwm1_v_sub_r, pwm1_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			BE_PWM_O26_Wr01(pwm_v_load_write, pwm1_v_sub_f_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			BE_PWM_O26_Wr01(pwm1_v_sub_f, pwm1_v_sub_f, gPwmInfo[pwmId].t2nd.pwm_v_f);
			BE_PWM_O26_Wr01(pwm1_v_sub_f, pwm1_v_sub_f_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			BE_PWM_O26_WrFL(pwm_v_load_write);
			BE_PWM_O26_WrFL(pwm1_v_r);
			BE_PWM_O26_WrFL(pwm1_v_f);
			BE_PWM_O26_WrFL(pwm1_v_sub_r);
			BE_PWM_O26_WrFL(pwm1_v_sub_f);
			break;
		case 2 :
			BE_PWM_O26_RdFL(pwm_v_load_write);
			BE_PWM_O26_RdFL(pwm2_v_r);
			BE_PWM_O26_RdFL(pwm2_v_f);
			BE_PWM_O26_RdFL(pwm2_v_sub_r);
			BE_PWM_O26_RdFL(pwm2_v_sub_f);
			BE_PWM_O26_Wr01(pwm_v_load_write, pwm2_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
			BE_PWM_O26_Wr01(pwm2_v_r, pwm2_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
			BE_PWM_O26_Wr01(pwm2_v_r, pwm2_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			BE_PWM_O26_Wr01(pwm2_v_f, pwm2_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
			BE_PWM_O26_Wr01(pwm2_v_f, pwm2_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			BE_PWM_O26_Wr01(pwm_v_load_write, pwm2_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			BE_PWM_O26_Wr01(pwm2_v_sub_r, pwm2_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
			BE_PWM_O26_Wr01(pwm2_v_sub_r, pwm2_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			BE_PWM_O26_Wr01(pwm_v_load_write, pwm2_v_sub_f_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			BE_PWM_O26_Wr01(pwm2_v_sub_f, pwm2_v_sub_f, gPwmInfo[pwmId].t2nd.pwm_v_f);
			BE_PWM_O26_Wr01(pwm2_v_sub_f, pwm2_v_sub_f_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			BE_PWM_O26_WrFL(pwm_v_load_write);
			BE_PWM_O26_WrFL(pwm2_v_r);
			BE_PWM_O26_WrFL(pwm2_v_f);
			BE_PWM_O26_WrFL(pwm2_v_sub_r);
			BE_PWM_O26_WrFL(pwm2_v_sub_f);
			break;
		default :
			break;
	}
...
static int PWM_GetTiming(UINT32 freq_mode, UINT32 *pRising, UINT32 *pFall)
{
	...
	do {
		...
		{
			case 0 :
				BE_PWM_O26_RdFL(pwm3_0_low);
				BE_PWM_O26_RdFL(pwm3_0_high);
				BE_PWM_O26_Rd01(pwm3_0_low, pwm3_0_low, *pRising);
				BE_PWM_O26_Rd01(pwm3_0_high, pwm3_0_high, *pFall);
				break;
			case 1 :
				BE_PWM_O26_RdFL(pwm3_1_low);
				BE_PWM_O26_RdFL(pwm3_1_high);
				BE_PWM_O26_Rd01(pwm3_1_low, pwm3_1_low, *pRising);
				BE_PWM_O26_Rd01(pwm3_1_high, pwm3_1_high, *pFall);
				break;
			case 2 :
				BE_PWM_O26_RdFL(pwm3_2_low);
				BE_PWM_O26_RdFL(pwm3_2_high);
				BE_PWM_O26_Rd01(pwm3_2_low, pwm3_2_low, *pRising);
				BE_PWM_O26_Rd01(pwm3_2_high, pwm3_2_high, *pFall);
				break;
			case 3 :
				BE_PWM_O26_RdFL(pwm3_3_low);
				BE_PWM_O26_RdFL(pwm3_3_high);
				BE_PWM_O26_Rd01(pwm3_3_low, pwm3_3_low, *pRising);
				BE_PWM_O26_Rd01(pwm3_3_high, pwm3_3_high, *pFall);
				break;
			default :
...
int BE_PWM_HW_O26_ProcHandler(BE_PWM_PROC_ID_T proc_id, struct seq_file* m, void* data)
{
	int	i;
	...
	switch(proc_id)
	{
		...
		{
			seq_printf(m, "ddr base : 0x%08x, size : %ld\n",(UINT32)_g_pwm_o26_ddr_base, sizeof(PWM_DDR_O26_T));
			if (gBE_PWM_DDR_O26 != NULL)
			{
				PWM_DDR_O26_T shdw;
				rmb();
				memcpy(&shdw,(const void *)gBE_PWM_DDR_O26,sizeof(PWM_DDR_O26_T));
				seq_printf(m, "%-20s : 0x%08x(size:%d,count:%d)\n","header",shdw.header.udata32,shdw.header.size,shdw.header.count);
				seq_printf(m, "%-20s : 0x%08x(en:%d)\n","pwm0_ctrl0",*((UINT32 *)(&(shdw.pwm0_ctrl0))),shdw.pwm0_ctrl0.pwm0_en);
				seq_printf(m, "%-20s : 0x%08x(width:%d)\n","pwm0_ctrl1",*((UINT32 *)(&(shdw.pwm0_ctrl1))),shdw.pwm0_ctrl1.pwm0_free_width);
				seq_printf(m, "%-20s : 0x%08x(fc_l,h:%d,%d)\n","pwm0_ctrl2",*((UINT32 *)(&(shdw.pwm0_ctrl2))),shdw.pwm0_ctrl2.pwm0_fc_l_disp,shdw.pwm0_ctrl2.pwm0_fc_h_disp);
				seq_printf(m, "%-20s : 0x%08x(en:%d)\n","pwm1_ctrl0",*((UINT32 *)(&(shdw.pwm1_ctrl0))),shdw.pwm1_ctrl0.pwm1_en);
				seq_printf(m, "%-20s : 0x%08x(width:%d)\n","pwm1_ctrl1",*((UINT32 *)(&(shdw.pwm1_ctrl1))),shdw.pwm1_ctrl1.pwm1_free_width);
				seq_printf(m, "%-20s : 0x%08x(fc_l,h:%d,%d)\n","pwm1_ctrl2",*((UINT32 *)(&(shdw.pwm1_ctrl2))),shdw.pwm1_ctrl2.pwm1_fc_l_disp,shdw.pwm1_ctrl2.pwm1_fc_h_disp);
				seq_printf(m, "%-20s : 0x%08x(freq:%d,duty:%d)\n","pwm0_info",shdw.pwm0_info.udata32,shdw.pwm0_info.freq,shdw.pwm0_info.duty);
				seq_printf(m, "%-20s : 0x%08x(freq:%d,duty:%d)\n","pwm1_info",shdw.pwm1_info.udata32,shdw.pwm1_info.freq,shdw.pwm1_info.duty);
			}
			seq_printf(m, "\n");
			...
			BE_PWM_O26_RdFL(pwm2_v_sub_r);
			BE_PWM_O26_RdFL(pwm2_v_sub_f);
			seq_printf(m, "apb base : 0x%08x\n",BE_O26A0_REG_PWM_T_BASE);
			seq_printf(m, "%-20s : 0x%08x\n","pwm0_ctrl0",BE_PWM_O26_Rd(pwm0_ctrl0));
			seq_printf(m, "%-20s : 0x%08x\n","pwm0_ctrl1",BE_PWM_O26_Rd(pwm0_ctrl1));
			seq_printf(m, "%-20s : 0x%08x\n","pwm0_ctrl2",BE_PWM_O26_Rd(pwm0_ctrl2));
			seq_printf(m, "%-20s : 0x%08x\n","pwm1_ctrl0",BE_PWM_O26_Rd(pwm1_ctrl0));
			seq_printf(m, "%-20s : 0x%08x\n","pwm1_ctrl1",BE_PWM_O26_Rd(pwm1_ctrl1));
			seq_printf(m, "%-20s : 0x%08x\n","pwm1_ctrl2",BE_PWM_O26_Rd(pwm1_ctrl2));
			seq_printf(m, "%-20s : 0x%08x\n","pwm2_ctrl0",BE_PWM_O26_Rd(pwm2_ctrl0));
			seq_printf(m, "%-20s : 0x%08x\n","pwm2_ctrl1",BE_PWM_O26_Rd(pwm2_ctrl1));
			seq_printf(m, "%-20s : 0x%08x\n","pwm2_ctrl2",BE_PWM_O26_Rd(pwm2_ctrl2));
			seq_printf(m, "%-20s : 0x%08x\n","pwm_v_load_write",BE_PWM_O26_Rd(pwm_v_load_write));
			seq_printf(m, "%-20s : 0x%08x\n","pwm0_v_r",BE_PWM_O26_Rd(pwm0_v_r));
			seq_printf(m, "%-20s : 0x%08x\n","pwm0_v_f",BE_PWM_O26_Rd(pwm0_v_f));
			seq_printf(m, "%-20s : 0x%08x\n","pwm0_v_sub_r",BE_PWM_O26_Rd(pwm0_v_sub_r));
			seq_printf(m, "%-20s : 0x%08x\n","pwm0_v_sub_f",BE_PWM_O26_Rd(pwm0_v_sub_f));
			seq_printf(m, "%-20s : 0x%08x\n","pwm1_v_r",BE_PWM_O26_Rd(pwm1_v_r));
			seq_printf(m, "%-20s : 0x%08x\n","pwm1_v_f",BE_PWM_O26_Rd(pwm1_v_f));
			seq_printf(m, "%-20s : 0x%08x\n","pwm1_v_sub_r",BE_PWM_O26_Rd(pwm1_v_sub_r));
			seq_printf(m, "%-20s : 0x%08x\n","pwm1_v_sub_f",BE_PWM_O26_Rd(pwm1_v_sub_f));
			seq_printf(m, "%-20s : 0x%08x\n","pwm2_v_r",BE_PWM_O26_Rd(pwm2_v_r));
			seq_printf(m, "%-20s : 0x%08x\n","pwm2_v_f",BE_PWM_O26_Rd(pwm2_v_f));
			seq_printf(m, "%-20s : 0x%08x\n","pwm2_v_sub_r",BE_PWM_O26_Rd(pwm2_v_sub_r));
			seq_printf(m, "%-20s : 0x%08x\n","pwm2_v_sub_f",BE_PWM_O26_Rd(pwm2_v_sub_f));
			seq_printf(m, "\n");
			for (i=0;i<BE_PWM_MAX;i++)
			{
				...
				if (p->ctrl.pwm_en)
				{
					...
					seq_printf(m, "duty:%d(0x%x), duty_adj:%d(0x%x), offs:%d(0x%x)\n",p->duty,p->duty,p->duty_adj,p->duty_adj,p->offs,p->offs);
					seq_printf(m, "freq:%d(0x%x), dutyRatio:%d(0x%x), lowPow:%d\n",p->freq,p->freq,p->dutyRatio,p->dutyRatio,p->lowPowerModeEn);
				}
				seq_printf(m, "\n");
			}
			seq_printf(m, "clk:%d\n",_g_be_pwm_ctx.clk);
