...
typedef enum
{
	...
	/** Allocations marked as display buffers  */
	UMP_DISPLAY_BUFFER = (1u << 26),
...
enum ump_external_memory_type
{
	...
	UMP_EXTERNAL_MEM_TYPE_LGFB = 2,
	UMPP_EXTERNAL_MEM_COUNT
};

#define UMP_LGFB_DEFAULT_ALLOCATION_FLAGS		UMP_PROT_CPU_RD | UMP_PROT_CPU_WR | \
												UMP_PROT_W_RD | UMP_PROT_W_WR |	\
												UMP_PROT_X_RD | UMP_PROT_X_WR |	\
												UMP_PROT_Y_RD | UMP_PROT_Y_WR |	\
												UMP_PROT_Z_RD | UMP_PROT_Z_WR |	\
...
#define UMP_LGFB_UNCACHED_ALLOCATION_FLAGS              UMP_PROT_CPU_RD | UMP_PROT_CPU_WR | \
                                                                                                UMP_PROT_W_RD | UMP_PROT_W_WR | \
                                                                                                UMP_PROT_X_RD | UMP_PROT_X_WR | \
                                                                                                UMP_PROT_Y_RD | UMP_PROT_Y_WR | \
                                                                                                UMP_PROT_Z_RD | UMP_PROT_Z_WR | \
                                                                                                UMP_PROT_SHAREABLE | UMP_CONSTRAINT_UNCACHED

#define UMP_LGFB_MAX_BUFFER_NUM (3)

typedef struct ump_lgfb_memory_info_64
{
	uint64_t addr; /* physical address */
	uint64_t size;
} ump_lgfb_memory_info_64;

typedef struct ump_lgfb_memory_handle
{
	ump_lgfb_memory_info_64 infos[UMP_LGFB_MAX_BUFFER_NUM];
	uint64_t number; /* a number of mem-info */
} ump_lgfb_memory_handle;
