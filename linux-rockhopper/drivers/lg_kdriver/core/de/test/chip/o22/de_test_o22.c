...
/*----------------------------------------------------------------------------------------
	global Functions
----------------------------------------------------------------------------------------*/
int de_test_o22_handler(struct de_test_handler *ph);
...
----------------------------------------------------------------------------------------*/
static int _de_test_o22_init(void);
static int _de_test_o22_run_unit(void *ptr);
static int _de_test_o22_task_unit(void *ptr);
static int _de_test_o22_check_mcu0(void);
static int _de_test_o22_check_mcu1(void);
static int _de_test_o22_check_hdmi(void);
static int _de_test_o22_check_mvi(void);

/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
static struct de_test_o22_ctx _g_de_test_o22_ctx[] = {
    {.name="run_test_pqe",.task=NULL,.func=_de_test_o22_run_unit},
    {.name=NULL}
};

static struct de_test_o22_func _g_de_test_o22_func[] = {
    {.name="check_mcu0",.func=_de_test_o22_check_mcu0},
    {.name="check_mcu1",.func=_de_test_o22_check_mcu1},
    {.name="check_hdmi",.func=_de_test_o22_check_hdmi},
    {.name="check_mvi", .func=_de_test_o22_check_mvi},
    {.name=NULL}
};
...
{
    int ret;
    struct de_test_o22_ctx *p = _g_de_test_o22_ctx;
...
static int _de_test_o22_run_unit(void *ptr)
{
    ...
    struct de_test_o22_ctx *p_ctx = (struct de_test_o22_ctx *)ptr;
    ...
    switch (p_handler->type)
    {
        case de_test_handler_type_start:
            ...
            p_ctx->state = 1;
            p_ctx->task = kthread_run(_de_test_o22_task_unit,(void *)p_ctx,"%s",p_handler->str);
            if (!p_ctx->task)
            {
...
{
    int ret = 0;
    struct de_test_o22_func *p_func = _g_de_test_o22_func;
    struct de_test_o22_ctx *p_ctx = (struct de_test_o22_ctx *)ptr;
...
static int _de_test_o22_check_mcu1(void)
{
    ...
    static volatile unsigned int *p_addr = NULL;
    ...
    pre_cnt = cur_cnt = *((unsigned int *)p_addr);

    for (i=0; i<5; i++)
    {
        msleep(100);
        cur_cnt = *((unsigned int *)p_addr);
        if (pre_cnt == cur_cnt) {DE_ERROR("(%d)no run\n",cur_cnt);ret = -1;}
        DE_NOTI("cnt (0x%08x)\n",cur_cnt);
        pre_cnt = cur_cnt;
    }
...
struct check_hdmi_ctx {
    ...
    struct {
        ...
        union {
            UINT32 udata32;
            struct {						//0xCC400540 RW 0x0000_0000
    ...
    struct {
        ...
        union {
            UINT32 udata32;
            struct {						//0xCC400548 RO 0x0000_0000
    ...
    struct {
        ...
        union {
            UINT32 udata32;
            struct {						//0xCC40054C RO 0x0000_0000
    ...
    struct {
        ...
        union {
            UINT32 udata32;
            struct {						//0xCC400550 RO 0x0000_0000
...
static int _de_test_o22_hdmi_map(struct check_hdmi_ctx *p_ctx)
{
    if (!p_ctx)
    {
        DE_ERROR("p_ctx null\n");
    ...
    p_ctx->misc_ctrl.addr = 0xCC840244;//REG_CVI_A_MISC_CTRL_T
    p_ctx->hdr_pxl_rd_ctrl0.addr = 0xCCCC0880;//REG_HDR10_E_HDR_PXL_RD_CTRL0_T
    p_ctx->hdr_pxl_rd_ctrl1.addr = 0xCCCC0884;//REG_HDR10_E_HDR_PXL_RD_CTRL1_T
    p_ctx->hdr_pxl_rd_data.addr = 0xCCCC0888;//REG_HDR10_E_HDR_PXL_RD_DATA_T
    p_ctx->ad_rp_hv_pos.addr = 0xCC400540;//REG_LED_AD_RP_HV_POS_T
    p_ctx->ad_rp_r_value.addr = 0xCC400548;//REG_LED_AD_RP_R_VALUE_T
    p_ctx->ad_rp_g_value.addr = 0xCC40054C;//REG_LED_AD_RP_G_VALUE_T
    p_ctx->ad_rp_b_value.addr = 0xCC400550;//REG_LED_AD_RP_B_VALUE_T
...
static int _de_test_o22_hdmi_unmap(struct check_hdmi_ctx *p_ctx)
{
    if (!p_ctx)
    {
        DE_ERROR("p_ctx null\n");
...
static int _de_test_o22_hdmi_open(struct check_hdmi_ctx *p_ctx)
{
    UINT32 mute;
    ...
    *p_ctx->hdr_pxl_rd_ctrl0.phy = p_ctx->hdr_pxl_rd_ctrl0.udata32;
    ...
    p_ctx->hdr_pxl_rd_ctrl1.r_pxl_rd_pos_x = 1920;
    p_ctx->hdr_pxl_rd_ctrl1.r_pxl_rd_pos_y = 1080;
    *p_ctx->hdr_pxl_rd_ctrl1.phy = p_ctx->hdr_pxl_rd_ctrl1.udata32;

    p_ctx->ad_rp_hv_pos.rp_h_pos = 1920;
    p_ctx->ad_rp_hv_pos.rp_v_pos = 1080;
...
static int _de_test_o22_hdmi_close(struct check_hdmi_ctx *p_ctx)
{
    struct v4l2_ext_vsc_connect_info input;
...
static int _de_test_o22_hdmi_pat(char *name, struct check_hdmi_ctx *p_ctx)
{
...
static int _de_test_o22_hdmi_rp(char *name, struct check_hdmi_ctx *p_ctx)
{
    int ret = 0;
    int i;
    struct hdmi_pat *p = _g_hdmi_pat;
...
static int _de_test_o22_check_hdmi(void)
{
    ...
    do {
        if (!ctx.count)
        {
            ...
            if (0 != (ret = _de_test_o22_hdmi_map(&ctx))) {DE_ERROR("_de_test_o22_hdmi_map err\n");break;}
            if (0 != (ret = _de_test_o22_hdmi_open(&ctx))) {DE_ERROR("_de_test_o22_hdmi_open err\n");break;}
            ret = ctx.count = 1;
            break;
        }
        ...
        {
            DE_NOTI("check pat(%s)\n",p->name);
            if (0 != (ret = _de_test_o22_hdmi_pat(p->name, &ctx))) {DE_ERROR("_de_test_o22_hdmi_pat(%s) err\n",p->name);break;}
            if (0 != (ret = _de_test_o22_hdmi_rp(p->name, &ctx))) {DE_ERROR("_de_test_o22_hdmi_rp(%s) err\n",p->name);break;}
            p++;
        };
    } while (0);
    ...
    {
        ctx.count = 0;
        if (0 != _de_test_o22_hdmi_pat("off", &ctx)) {DE_ERROR("_de_test_o22_hdmi_pat(off) err\n");return -1;}
        if (0 != _de_test_o22_hdmi_close(&ctx)) {DE_ERROR("_de_test_o22_hdmi_close err\n");return -1;}
        if (0 != _de_test_o22_hdmi_unmap(&ctx)) {DE_ERROR("_de_test_o22_hdmi_unmap err\n");return -1;}
    }
