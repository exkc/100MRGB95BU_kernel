...
/* This workaround is going to be used temporarily until
   CMA for devices is ready to use.
 */
#define	WORKAROUND_NON_PAGES	0
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
static int dma_buf_unified_attach(struct dma_buf *buf, struct dma_buf_attachment *attachment)
#else
...
static int __map_user(struct dma_buf_unified_alloc *alloc,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
static vm_fault_t dma_buf_unified_mmap_fault(struct vm_fault *vmf)
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
			err = vmf_insert_mixed(vma, vma->vm_start + (i << PAGE_SHIFT), __pfn_to_pfn_t(paddr >> PAGE_SHIFT,PFN_DEV));
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	.map		= dma_buf_unified_kmap,
	.unmap		= dma_buf_unified_kunmap,
#else
...
static int do_dma_buf_unified_ioctl_buffer_sync(struct dma_buf_unified_ioctl_buffer_sync __user *arg)
{
	...
	if(sync.direction == 0)
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
		dma_sync_sg_for_device(te_device.this_device, alloc->sg->sgl, alloc->nr_pages, DMA_TO_DEVICE);
#else
		dma_sync_sg_for_device(te_device.this_device, alloc->sg, alloc->nr_pages, DMA_TO_DEVICE);
#endif
	else
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
		dma_sync_sg_for_cpu(te_device.this_device, alloc->sg->sgl, alloc->nr_pages, DMA_FROM_DEVICE);
#else
