...
/* Module parameter for enabling memory dump feature */
static bool enable_memory_dump = false;
module_param(enable_memory_dump, bool, 0644);
MODULE_PARM_DESC(enable_memory_dump, "Enable memory dump via /proc/dma_buf_te/ (default: N)");
...
struct dma_buf_te_alloc {
	...
	char proc_name[64];  /* Store the proc filename */
};
...
static DEFINE_MUTEX(alloc_list_lock);

/* Proc directory for memory dumps */
static struct proc_dir_entry *dma_buf_te_proc_dir;

extern int lgext_dma_buf_te_memory(pid_t tgid);
int lgext_dma_buf_te_memory(pid_t tgid)
{
...
EXPORT_SYMBOL(lgext_dma_buf_te_memory);

static int show_allocated(struct seq_file *file, void *data)
{
	struct dma_buf_te_alloc *alloc;
	...
	list_for_each_entry(alloc, &alloc_list, list) {
		seq_printf(file, "%d: %zd, %dx%d", (int)alloc->tgid, alloc->nr_pages * PAGE_SIZE,
				alloc->width, alloc->height);
		if (alloc->fourcc)
			seq_printf(file, ", %c%c%c%c(0x%016llx)",
					(alloc->fourcc >>  0) & 0xff,
					(alloc->fourcc >>  8) & 0xff,
					(alloc->fourcc >> 16) & 0xff,
					(alloc->fourcc >> 24) & 0xff,
					(unsigned long long)alloc->modifier);
		else
			seq_printf(file, ", ____");
		if (alloc->contiguous)
			seq_printf(file, ",contig");
		seq_printf(file, "\n");
	}
...
/* Memory dump proc file operations */
static ssize_t dma_buf_te_proc_read(struct file *file, char __user *buf,
				    size_t count, loff_t *ppos)
{
	...
	alloc = pde_data(file_inode(file));
#else
	alloc = PDE_DATA(file_inode(file));
#endif
	if (!alloc)
	...
	total_size = alloc->nr_pages * PAGE_SIZE;
	if (pos >= total_size) {
		mutex_unlock(&alloc->lock);
		return 0;
	}

	bytes_to_copy = min(count, (size_t)(total_size - pos));

	while (bytes_copied < bytes_to_copy) {
		size_t chunk_size;

		page_idx = (pos + bytes_copied) / PAGE_SIZE;
		page_offset = (pos + bytes_copied) % PAGE_SIZE;

		if (page_idx >= alloc->nr_pages)
			break;

		/* Map the page for reading */
		kaddr = kbase_kmap(alloc->pages[page_idx]);
		if (!kaddr) {
			mutex_unlock(&alloc->lock);
			return bytes_copied ? bytes_copied : -ENOMEM;
		}

		/* Calculate how many bytes to copy from this page */
		chunk_size = min(PAGE_SIZE - page_offset,
				 bytes_to_copy - bytes_copied);

		/* Copy to user buffer */
		ret = copy_to_user(buf + bytes_copied, kaddr + page_offset, chunk_size);
		kbase_kunmap(alloc->pages[page_idx], kaddr);
	...
	*ppos = pos + bytes_copied;
	return bytes_copied;
}

#if KERNEL_VERSION(5, 6, 0) <= LINUX_VERSION_CODE
static const struct proc_ops dma_buf_te_proc_ops = {
	.proc_read = dma_buf_te_proc_read,
	.proc_lseek = default_llseek,
};
#else
static const struct file_operations dma_buf_te_proc_ops = {
	.read = dma_buf_te_proc_read,
...
static void dma_buf_te_release(struct dma_buf *buf)
{
	...
	/* Remove proc file if it exists */
	if (alloc->proc_entry && dma_buf_te_proc_dir) {
		proc_remove(alloc->proc_entry);
		alloc->proc_entry = NULL;
	}
...
static int do_dma_buf_te_ioctl_alloc2(struct dma_buf_te_ioctl_alloc2 *buf, bool contiguous, bool from_user)
{
	struct dma_buf_te_ioctl_alloc2 alloc_req;
	struct dma_buf_te_alloc *alloc;
	struct dma_buf *dma_buf;
	size_t i = 0;
	...
	if (from_user)
	{
		...
		memcpy(&alloc_req, buf, sizeof(*buf));
	...
	alloc->fourcc = alloc_req.fourcc;
	alloc->modifier = alloc_req.modifier;
	alloc->width = alloc_req.width;
	alloc->height = alloc_req.height;
	...
	/* Create proc file for memory dump if enabled */
	if (enable_memory_dump && dma_buf_te_proc_dir) {
		snprintf(alloc->proc_name, sizeof(alloc->proc_name),
			 "%d-0x%p", alloc->tgid, alloc);
		alloc->proc_entry = proc_create_data(alloc->proc_name,
						     0400,  /* root read-only */
						     dma_buf_te_proc_dir,
						     &dma_buf_te_proc_ops,
						     alloc);
		if (!alloc->proc_entry) {
			dev_warn(te_device.this_device,
...
static int do_dma_buf_te_ioctl_alloc(struct dma_buf_te_ioctl_alloc __user *buf, bool contiguous)
{
	...
	struct dma_buf_te_ioctl_alloc2 alloc_req2 = { };
	...
	alloc_req2.size = alloc_req.size;

	return do_dma_buf_te_ioctl_alloc2(&alloc_req2, contiguous, false);
}
...
static long dma_buf_te_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DMA_BUF_TE_ALLOC:
		return do_dma_buf_te_ioctl_alloc((struct dma_buf_te_ioctl_alloc __user *)arg,
						 false);
	case DMA_BUF_TE_ALLOC2:
		return do_dma_buf_te_ioctl_alloc2((struct dma_buf_te_ioctl_alloc2 __user *)arg, false, true);
...
static struct miscdevice te_device2;
...
static int __init dma_buf_te_init(void)
{
	...
	te_device2 = te_device;
	te_device2.name = "dma_buf_unified";
	...
	/* LGE ext */
	res = misc_register(&te_device2);
	proc_create_single("dma_buf_te-allocated", 0400, NULL, show_allocated);

	/* Create proc directory for memory dumps */
	dma_buf_te_proc_dir = proc_mkdir("dma_buf_te", NULL);
	if (!dma_buf_te_proc_dir) {
		dev_warn(te_device.this_device,
			"Failed to create /proc/dma_buf_te directory\n");
	}
...
static void __exit dma_buf_te_exit(void)
{
	...
	if (dma_buf_te_proc_dir) {
		proc_remove(dma_buf_te_proc_dir);
		dma_buf_te_proc_dir = NULL;
	}
