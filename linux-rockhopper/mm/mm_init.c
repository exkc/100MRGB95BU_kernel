...
static void __init free_area_init_core(struct pglist_data *pgdat)
{
	...
	for (j = 0; j < MAX_NR_ZONES; j++) {
		...
		if (movable_size) {
			...
			setup_usemap(zone);
			init_currently_empty_zone(zone, zone->zone_start_pfn, movable_size);
		} else {
			setup_usemap(zone);
...
#ifdef CONFIG_CMA
static void __init adjust_present_page_count_cma(struct page *page, long count)
{
	...
	if (early_section(__pfn_to_section(page_to_pfn(page))))
		zone->present_early_pages += count;
#endif
...
void __init init_cma_reserved_pageblock(struct page *page)
{
	 ...
	 */
	adjust_present_page_count_cma(page, -pageblock_nr_pages);
