...
static int omniphy_probe(struct phy_device *phydev)
{
	...
	for (i = 0; i < MAX_GPIOS; i++) {
		desc = gpiod_get_index(&phydev->mdio.dev, "isol", i, GPIOD_ASIS);
...
static int omniphy_resume(struct phy_device *phydev)
{
	...
	/* generate link-down if not IRQ-driven */
...
static int omniphy_config_aneg(struct phy_device *phydev)
{
	...
	switch (phydev->mdix) {
	case ETH_TP_MDI:
		dev_info(&phydev->mdio.dev, "forced MDI\n");
	...
	case ETH_TP_MDI_X:
		dev_info(&phydev->mdio.dev, "forced MDI-X\n");
	...
	default:
		dev_warn(&phydev->mdio.dev, "invalid media-type\n");

	case ETH_TP_MDI_AUTO:
		dev_info(&phydev->mdio.dev, "MDI/MDI-X auto-detect\n");
...
static void omniphy_switch_media(struct phy_device *phydev, int media)
{
	...
	case MEDIA_MDIX:
		dev_info(&phydev->mdio.dev, "MDI-X\n");
...
__retry__:
	...
	if (phydev->autoneg) {
		...
		mii_lpa_to_linkmode_lpa_t(phydev->lp_advertising, v);
		...
		if ((v & 0x000c) == 0x0000 || (v & 0x000c) == 0x000c) {
			...
			dev_warn(&phydev->mdio.dev, "auto-negotiation failed\n");
		...
		if (v & 0x0010) {
			...
			if (linkmode_test_bit(ETHTOOL_LINK_MODE_Pause_BIT, phydev->lp_advertising))
				phydev->pause = 1;
			if (linkmode_test_bit(ETHTOOL_LINK_MODE_Asym_Pause_BIT, phydev->lp_advertising))
				phydev->asym_pause = 1;
		}
	} else {
...
__try__:
	...
	if (v != wol_event) {
		dev_warn(&phydev->mdio.dev, "WOL-event not set: %04x\n", v);
		failure++;
	}
	...
	if (v != htons(mac_addr[2])) {
		dev_warn(&phydev->mdio.dev, "MAC-address[0] not set: %04x\n", v);
		failure++;
	}
	...
	if (v != htons(mac_addr[1])) {
		dev_warn(&phydev->mdio.dev, "MAC-address[1] not set: %04x\n", v);
		failure++;
	}
	...
	if (v != htons(mac_addr[0])) {
		dev_warn(&phydev->mdio.dev, "MAC-address[2] not set: %04x\n", v);
		failure++;
	}

	v = phy_read(phydev, OMNIPHY_IMR);
	if (v != 0x0e00) {
		dev_warn(&phydev->mdio.dev, "WOL-interrupt not set: %04x\n", v);
		failure++;
	}

	v = phy_read(phydev, OMNIPHY_MCSR);
	if (v & 0x0080) {
		dev_warn(&phydev->mdio.dev, "MDI-X auto-detection: %04x\n", v);
		failure++;
	}
	...
	if (failure) {
		...
		dev_warn(&phydev->mdio.dev, "WOL might not work\n");
	} else
		dev_info(&phydev->mdio.dev, "WOL activation confirmed\n");
...
static int __init omniphy_init(void)
{
	...
	v = phy_drivers_register(omniphy_drvs, ARRAY_SIZE(omniphy_drvs), THIS_MODULE);
	if (v < 0)
		return v;
