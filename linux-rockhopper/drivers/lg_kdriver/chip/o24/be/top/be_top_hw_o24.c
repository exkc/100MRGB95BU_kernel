...
/*----------------------------------------------------------------------------------------
	Control Constants
----------------------------------------------------------------------------------------*/
#define BE_TOP_HW_O24_USE_VX1_CHIP_INFO 1
...
#include "ipc_drv.h"
#include "be_top_hw_o24.h"
#include "../led/be_led_hw_o24.h"
...
#if BE_TOP_HW_O24_USE_VX1_CHIP_INFO
#include "ovi_hal.h"
...
----------------------------------------------------------------------------------------*/
#define BE_TOP_HW_O24_MAGIC_NUM ((UINT32)0x1234ABCD)

/*----------------------------------------------------------------------------------------
	Macro Definitions
----------------------------------------------------------------------------------------*/
#define BE_TOP_HW_O24_LSR_TABLE_OFFSET 0x800
#define BE_TOP_HW_O24_GSR_TABLE_OFFSET (BE_TOP_HW_O24_LSR_TABLE_OFFSET + 0x800)
#define BE_TOP_HW_O24_OSD_ALPHA_TABLE_OFFSET (BE_TOP_HW_O24_GSR_TABLE_OFFSET + 0x800)
#define BE_TOP_HW_O24_PATTERN_INFO_OFFSET (BE_TOP_HW_O24_OSD_ALPHA_TABLE_OFFSET + 0x1000)
#define BE_TOP_HW_O24_GSR2_TABLE_OFFSET (BE_TOP_HW_O24_PATTERN_INFO_OFFSET + 0x800)
#define BE_TOP_HW_O24_IRR_ALUM_TABLE_OFFSET (BE_TOP_HW_O24_GSR2_TABLE_OFFSET + 0x800)
#define BE_TOP_HW_O24_IRR_DATA_OFFSET (BE_TOP_HW_O24_IRR_ALUM_TABLE_OFFSET + 0x400)
#define BE_TOP_HW_O24_TPC_PARAM_OFFSET (BE_TOP_HW_O24_IRR_DATA_OFFSET + 0x2000)
#define BE_TOP_HW_O24_TPC_STATUS_OFFSET (BE_TOP_HW_O24_TPC_PARAM_OFFSET + 0x80)

#ifdef BE_TOP_HW_O24_TEST_INPUT_LAG
#define	BE_TOP_HW_O24_IPP_LAG_CTRL0_BASE		(0xC9014490) //o18: 0xC9014448, m16p3: 0xC9014448
#define	BE_TOP_HW_O24_IPP_PIXEL_ACTIVE_RIGING	(0xC901405C) // AVTASK-371

#define	BE_TOP_HW_O24_IPP_LAG_CTRL1_BASE	(0xC9014494) //SCDCR-2484
#define	BE_TOP_HW_O24_IPP_LAG_CTRL2_BASE	(0xC9014498)
#define BE_TOP_HW_O24_PATTE

#define BE_TOP_HW_O24_PATTERN_OFF		(0)
#define BE_TOP_HW_O24_PATTERN_ON		(1)
#define BE_TOP_HW_O24_PATTERN_BLACK		(0) //0x88080200
#define BE_TOP_HW_O24_PATTERN_WHITE		(1) //0xBFF80200

#define BE_TOP_HW_O24_PIN_MUX_PORT		(119)
#define BE_TOP_HW_O24_PIN_NORMAL		(0)
#define BE_TOP_HW_O24_PIN_GPIO			(1)
#endif
...
#define BE_TOP_HW_O24_PQE_PM_DEVICE_SUSPEND_PREPARE		0
//fw stall
#define BE_TOP_HW_O24_PQE_PM_DEVICE_SUSPEND				1
//cg on
#define BE_TOP_HW_O24_PQE_PM_DEVICE_SUSPEND_COMPLETED	2
#define BE_TOP_HW_O24_PQE_PM_DEVICE_RESUME_PREPARE		3
//cg off
#define BE_TOP_HW_O24_PQE_PM_DEVICE_RESUME				4
//fw restart
#define BE_TOP_HW_O24_PQE_PM_DEVICE_RESUME_COMPLETED	5
...
extern PE_OSD_REG_O24_T gPE_OSD_O24;
...
/*----------------------------------------------------------------------------------------
	global Variables
----------------------------------------------------------------------------------------*/
volatile LAG_CTRL1 *gBE_TOP_HW_O24_LAG_CTRL1 = NULL;
volatile LAG_CTRL2 *gBE_TOP_HW_O24_LAG_CTRL2 = NULL;

/*----------------------------------------------------------------------------------------
	Static Function Prototypes Declarations
----------------------------------------------------------------------------------------*/
#ifdef BE_TOP_HW_O24_TEST_INPUT_LAG
static int BE_TOP_HW_O24_InitLagPattern(void);
int BE_TOP_HW_O24_InputLagHandler(void);
static int BE_TOP_HW_O24_SetPattern(UINT32 bOnOff);
static int BE_TOP_HW_O24_SetGpioValue(UINT32 value);
static int BE_TOP_HW_O24_SetGpioMux(UINT32 value);
#endif

#ifdef BE_OVI_HW_O24_USE_VX1_CHIP_INFO
static int _BE_TOP_O24_InitVx1ChipInfoMem(void);
#endif

/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
__attribute__((unused)) static UINT32 g_be_top_hw_O24_pattern_init = 0;
...
static BE_TOP_HW_O24_INFO_T gTopHwO24Info;
/*========================================================================================
...
static int _BE_TOP_O24_InitVx1ChipInfoMem(void)
{
	...
	do {
		gTopHwO24Info.pVx1ChipInfoVMapAddr = NULL;

		if (gTopHwO24Info.dispInfo.frcChipType != LX_OVI_FRC_CHIP_UD_BE_F22) break;
		...
		gTopHwO24Info.pVx1ChipInfoVMapAddr = vmap_phys(pChInfMem->frame_base, pChInfMem->frame_size);
		CHECK_NULL(gTopHwO24Info.pVx1ChipInfoVMapAddr);
		...
		for (i=0; i<BE_OVI_HW_O24_VX1_CHIP_INFO_MEM_BUFF_NUM; i++)
		{
			...
			gTopHwO24Info.pVx1ChipInfo[i] = (BE_OVI_HW_O24_VX1_CHIP_INFO_T *)(gTopHwO24Info.pVx1ChipInfoVMapAddr \
	...
	if (ret && (gTopHwO24Info.pVx1ChipInfoVMapAddr != NULL))
	{
		vunmap_phys(gTopHwO24Info.pVx1ChipInfoVMapAddr);
		gTopHwO24Info.pVx1ChipInfoVMapAddr = NULL;
	}
...
int BE_TOP_HW_O24_Initialize(void)
{
	...
	do {
		...
		BE_REG_WR(0xCC86102C, pFwMem->fw_base + BE_TOP_HW_O24_OSD_ALPHA_TABLE_OFFSET);

		BE_REG_WR(0xCC86045C, 0x0); // init OSD0_GSR_CTRL03, crnt gain LUT
		BE_REG_WR(0xCC860468, 0x0); // init OSD0_GSR_CTRL06, value gain LUT
		BE_REG_WR(0xCC860474, 0x0); // init OSD0_GSR_CTRL09, sat gain LUT

		ret = OVI_HAL_GetHWDisplayOption(&gTopHwO24Info.dispInfo);
		if (ret) BREAK_WRONG(ret);
		
#ifdef BE_OVI_HW_O24_USE_VX1_CHIP_INFO
		ret = _BE_TOP_O24_InitVx1ChipInfoMem();
		if (ret) BREAK_WRONG(ret);
#endif
		gTopHwO24Info.frcStatus = 0;
		gTopHwO24Info.isIrrInit = FALSE;
		gTopHwO24Info.pmStatus = BE_TOP_HW_O24_PQE_PM_DEVICE_RESUME_COMPLETED;

		gTopHwO24Info.disp_mode.uDispResolution = 0xff;
		gTopHwO24Info.disp_mode.uFrameRate = 0xff;
		gTopHwO24Info.disp_mode.uFrate165 = 0xff;
	} while(0);
...
int BE_TOP_HW_O24_Resume(void)
{
	...
	do {
		...
		BE_REG_WR(0xCC86045C, 0x0); // init OSD0_GSR_CTRL23, crnt gain LUT
		BE_REG_WR(0xCC860468, 0x0); // init OSD0_GSR_CTRL26, value gain LUT
		BE_REG_WR(0xCC860474, 0x0); // init OSD0_GSR_CTRL29, sat gain LUT
...
int BE_TOP_HW_O24_Close(void)
{
	...
	do {
#ifdef BE_OVI_HW_O24_USE_VX1_CHIP_INFO
		if (gTopHwO24Info.pVx1ChipInfoVMapAddr != NULL)
		{
			vunmap_phys(gTopHwO24Info.pVx1ChipInfoVMapAddr);
			gTopHwO24Info.pVx1ChipInfoVMapAddr = NULL;
		}
...
int BE_TOP_HW_O24_SetBypass(LX_BE_BLOCK_BYPASS_T *pstParams)
{
	...
	do{
		...
		{
			DPE_LED_O24_RdFL(ad_led_detour);
			if (pstParams->bLEDBypass)
			{
				DPE_LED_O24_Wr01(ad_led_detour, led_detour, 0x1);
			}
			else
			{
				DPE_LED_O24_Wr01(ad_led_detour, led_detour, 0x0);
			}
			DPE_LED_O24_WrFL(ad_led_detour);
		}
...
int BE_TOP_HW_O24_SetOrbit(LX_BE_ORBIT_T *pstParams)
{
	...
	do{
		...
		memcpy(&gTopHwO24Info.orbitInfo, pstParams, sizeof(LX_BE_ORBIT_T));
	} while(0);
...
static int BE_TOP_HW_O24_InitLagPattern(void)
{
	gIPP_LAG_CTRL0 = (volatile UINT32 *)ioremap(BE_TOP_HW_O24_IPP_LAG_CTRL0_BASE, sizeof(UINT32));
	gIPP_PIXEL_CTRL_ACTIVE_RIGING = (volatile UINT32 *)ioremap(BE_TOP_HW_O24_IPP_PIXEL_ACTIVE_RIGING, sizeof(UINT32));

	gBE_TOP_HW_O24_LAG_CTRL1 = (volatile LAG_CTRL1 *)ioremap(BE_TOP_HW_O24_IPP_LAG_CTRL1_BASE, sizeof(LAG_CTRL1));
	gBE_TOP_HW_O24_LAG_CTRL2 = (volatile LAG_CTRL2 *)ioremap(BE_TOP_HW_O24_IPP_LAG_CTRL2_BASE, sizeof(LAG_CTRL2));

	/* move to below line to init(to get gpiod_de only one time */
	gpiod_de = gpiod_get(&de_gpio_dev, "119", GPIOD_OUT_HIGH);

	gPrePatternParam.enable = BE_TOP_HW_O24_PATTERN_OFF;
...
int BE_TOP_HW_O24_SetLagParam(LX_BE_INPUT_LAG_PATTERN_T *pstParams)
{
	...
	BE_NOTI("init:%d, bOnOff[%d] bPatternType[%d] rect[%d/%d/%d/%d]\n",\
		g_be_top_hw_O24_pattern_init,pstParams->enable,pstParams->type,\
		pstParams->hoffset,pstParams->voffset,pstParams->hsize,pstParams->vsize);

	if (!g_be_top_hw_O24_pattern_init)
	{
		BE_TOP_HW_O24_InitLagPattern();
		g_be_top_hw_O24_pattern_init = 1;
	}
	...
	do {
		CHECK_KNULL(gBE_TOP_HW_O24_LAG_CTRL1);
		CHECK_KNULL(gBE_TOP_HW_O24_LAG_CTRL2);

		if (pre_enable == cur_enable)		break;
		if (cur_enable == BE_TOP_HW_O24_PATTERN_ON)
		{
			/* interrupt enable */
#ifdef INCLUDE_KDRV_DE
			...
			*gBE_TOP_HW_O24_LAG_CTRL2 = pix_lag_ctrl2;
			...
			/* register handle */
#ifdef INCLUDE_KDRV_DE
			de_srcsync_register(TRUE, BE_TOP_HW_O24_InputLagHandler, "inputLag_handler");
#endif
			BE_NOTI("START\n");
		}
		else
		{
			...
			*gBE_TOP_HW_O24_LAG_CTRL2 = pix_lag_ctrl2;
#ifdef INCLUDE_KDRV_DE
			de_srcsync_register(FALSE, BE_TOP_HW_O24_InputLagHandler, "inputLag_handler");
#endif
			/* interrupt disable */
#ifdef INCLUDE_KDRV_DE
...
int BE_TOP_HW_O24_InputLagHandler(void)
{
	...
	do {
		...
		if (cur_type == BE_TOP_HW_O24_PATTERN_WHITE) //white pattern
		{
			BE_TOP_HW_O24_SetPattern(BE_TOP_HW_O24_PATTERN_WHITE); //White
		}
		else	//black pattern
		{
			BE_TOP_HW_O24_SetPattern(BE_TOP_HW_O24_PATTERN_BLACK); //Black
		}
	...
	if (gpio_cnt == 4)
	{
		gpiod_set_raw_value(gpiod_de, (cur_type == BE_TOP_HW_O24_PATTERN_WHITE)? 1:0);
...
static int BE_TOP_HW_O24_SetPattern(UINT32 bOnOff)
{
	if (bOnOff == BE_TOP_HW_O24_PATTERN_WHITE)
	{
		*gIPP_LAG_CTRL0 = 0xBFF80200;
	}
	else if (bOnOff == BE_TOP_HW_O24_PATTERN_BLACK)
	{
...
static int BE_TOP_HW_O24_SetGpioValue(UINT32 value)
{
//	GPIO_DevSetValue(BE_TOP_HW_O24_PIN_MUX_PORT, value);
...
static int BE_TOP_HW_O24_SetGpioMux(UINT32 value)
{
//	GPIO_DevSetPinMux(BE_TOP_HW_O24_PIN_MUX_PORT, value);
...
int BE_TOP_HW_O24_SetLSRParam(LX_BE_LSR_T *pstParams)
{
	...
	do{
		...
		gTopHwO24Info.lsrSize = inSize;
		
		pFwMem = (BE_CH_MEM_T *) &gpBeMem->fw[3];
		CHECK_NULL(pFwMem);
		CHECK_NULL(pFwMem->fw_base);

		if ((inSize + BE_TOP_HW_O24_LSR_TABLE_OFFSET) > pFwMem->fw_size) BREAK_WRONG(inSize);
		...
		CHECK_NULL(pAddr);
		memcpy((char *)((uintptr_t)pAddr + (uintptr_t)BE_TOP_HW_O24_LSR_TABLE_OFFSET), pBuffer, inSize);
		...
		stHwParams.uBaseAddr = pFwMem->fw_base + BE_TOP_HW_O24_LSR_TABLE_OFFSET;
...
int BE_TOP_HW_O24_SetGSR(LX_BE_GSR_T *pstParams)
{
	...
	do{
		...
		gTopHwO24Info.gsrSize = inSize;
		
		pFwMem = (BE_CH_MEM_T *) &gpBeMem->fw[3];
		CHECK_NULL(pFwMem);
		CHECK_NULL(pFwMem->fw_base);

		if ((inSize + BE_TOP_HW_O24_GSR_TABLE_OFFSET) > pFwMem->fw_size) BREAK_WRONG(inSize);
		...
		pAddr = (char *)vmap_phys(pFwMem->fw_base, pFwMem->fw_size);
		CHECK_NULL(pAddr);
		memcpy((char *)((uintptr_t)pAddr + (uintptr_t)BE_TOP_HW_O24_GSR_TABLE_OFFSET), pBuffer, inSize);
		...
		stHwParams.uBaseAddr = pFwMem->fw_base + BE_TOP_HW_O24_GSR_TABLE_OFFSET;
...
int BE_TOP_HW_O24_SetGSR2(LX_BE_GSR_T *pstParams)
{
	...
	do{
		...
		if ((inSize + BE_TOP_HW_O24_GSR2_TABLE_OFFSET) > pFwMem->fw_size) BREAK_WRONG(inSize);
		...
		pAddr = (char *)vmap_phys(pFwMem->fw_base, pFwMem->fw_size);
		CHECK_NULL(pAddr);
		memcpy((char *)((uintptr_t)pAddr + (uintptr_t)BE_TOP_HW_O24_GSR2_TABLE_OFFSET), pBuffer, inSize);
		...
		/* change value LX_BE to BE_IPC */
		stHwParams.bGSRMode = 1;
		stHwParams.uBaseAddr = pFwMem->fw_base + BE_TOP_HW_O24_GSR2_TABLE_OFFSET;
		stHwParams.uDbtype = (gsrTblVer << 16) | inSizeNum;
...
int BE_TOP_HW_O24_GetGSRStatus(BOOLEAN *pbParam)
{
	...
	do{
		...
		BE_REG_RD(0xCC861028, regVal); // CC_PE1_FSW_CTRL_02
...
int BE_TOP_HW_O24_GetGSRStressCount(UINT32 *pu32Param)
{
	...
	do{
		...
		BE_REG_RD(0xCC862314, regVal); // CCO_VERI, sys_veri_5
...
int BE_TOP_HW_O24_GetOSDAlphaAPL(LX_BE_LUT_T *pstParams)
{
	...
	do{
		...
		restSize = ((pFwMem->fw_base + BE_TOP_HW_O24_OSD_ALPHA_TABLE_OFFSET) & (PAGE_SIZE - 1));
		alignedBase = ((pFwMem->fw_base + BE_TOP_HW_O24_OSD_ALPHA_TABLE_OFFSET) & (~(PAGE_SIZE - 1)));
		...
		if (pstParams->u32SizeNum == 1) // update local data for vbe log
		{
			...
			gTopHwO24Info.osdLvl = tmpData/16;
...
int BE_TOP_HW_O24_SetOSDRGBLevel(LX_BE_OSD_RGB_LEVEL_T *pstParams)
{
	...
	do{
		...
		gTopHwO24Info.osdGain = arrData[0];
		...
		if (gTopHwO24Info.pVx1ChipInfoVMapAddr != NULL)
		{
			...
			for (i=0; i<BE_OVI_HW_O24_VX1_CHIP_INFO_MEM_BUFF_NUM; i++)
			{
				...
				gTopHwO24Info.pVx1ChipInfo[i]->osdLvCtrl.osdLvCtrlRegAddr = _B2L32(BE_OSD_HW_F22_REG_OSD_GSR_CTRL); dataSize++;
				gTopHwO24Info.pVx1ChipInfo[i]->osdLvCtrl.osdLvCtrlVal = _B2L32(regVal); dataSize++;
				dataSize = sizeof(UINT32) * dataSize;

#ifndef BE_OVI_HW_O24_USE_VX1_CHIP_INFO_APB_SLAVE
				gTopHwO24Info.pVx1ChipInfo[i]->osdLvCtrl.hwCrc = 0xFFFFFFFF;
#endif

				if (dataSize != BE_HW_O24_VX1_CHIP_INFO_OSD_RGBLV_CTRL_SIZE) BREAK_WRONG(dataSize);

				wmb();
				
#ifdef BE_OVI_HW_O24_USE_VX1_CHIP_INFO_APB_SLAVE
				count.u32Data = _B2L32(gTopHwO24Info.pVx1ChipInfo[i]->datHdr.cnt2.u32Data);
				...
				gTopHwO24Info.pVx1ChipInfo[i]->datHdr.cnt2.u32Data = _B2L32(count.u32Data);
#else
				gTopHwO24Info.pVx1ChipInfo[i]->osdLvCtrl.hwHdr = _B2L32((0x0A000000|BE_HW_O24_VX1_CHIP_INFO_OSD_RGBLV_CTRL_SIZE));
#endif
		...
		{
#endif
			PE_OSD_O24_RdFL(osd0_gsr_ctrl00);
			PE_OSD_O24_Wr01(osd0_gsr_ctrl00, reg_gsr_blend_en, pstParams->bOnOff ? 0x1 : 0x0);
			if (arrData[0] == 0xFF)
			{
				PE_OSD_O24_Wr01(osd0_gsr_ctrl00, reg_gsr_master_gain, 0x3FF);
			}
			else
			{
				PE_OSD_O24_Wr01(osd0_gsr_ctrl00, reg_gsr_master_gain, (( arrData[0]) << 2));
			}
			PE_OSD_O24_WrFL(osd0_gsr_ctrl00);
#ifdef BE_OVI_HW_O24_USE_VX1_CHIP_INFO
		}
#endif
...
int BE_TOP_HW_O24_SetIrrAdaptedLum(LX_BE_LUT_T *pstParams)
{
	...
	do{ 
		...
		if ((inSize + BE_TOP_HW_O24_IRR_ALUM_TABLE_OFFSET + 4) > pFwMem->fw_size) BREAK_WRONG(inSize); // 4: 4bytes for magic number
		...
		pOffsetAddr = (char *)((uintptr_t)pAddr + (uintptr_t)BE_TOP_HW_O24_IRR_ALUM_TABLE_OFFSET);
		...
		// set magic number
		((UINT32 *)pOffsetAddr)[0] = BE_TOP_HW_O24_MAGIC_NUM;
		
		wmb();

		if (gTopHwO24Info.dispInfo.frcChipType == LX_OVI_FRC_CHIP_UD_BE_F22)
		{
			IPC_SET_DEFAULT_PARAM(BE_IPC_IRR_T, stHwParams);

			stHwParams.bIrrDataLoad = 2;
			stHwParams.uBaseAddr = pFwMem->fw_base + BE_TOP_HW_O24_IRR_ALUM_TABLE_OFFSET;
...
int BE_TOP_HW_O24_GetIrrResult(UINT32 *pu32Param)
{
	...
	do{
		...
		BE_REG_RD(0xCC862378, regVal); //REG_CCO_VERI_SYS_VERI_30_T
...
int BE_TOP_HW_O24_GetIrrStatus(LX_BE_IRR_STATUS_T *pstParams)
{
	...
	do{
		...
		pstParams->bIsIrrInitDone = gTopHwO24Info.isIrrInit;
		pstParams->bIsIrrSuspended = (gTopHwO24Info.pmStatus == BE_TOP_HW_O24_PQE_PM_DEVICE_RESUME_COMPLETED) ? FALSE : TRUE;
	} while(0);
...
int BE_TOP_HW_O24_SetIrrData(LX_BE_LUT_T *pstParams)
{
	...
	do{ 
		...
		pBuffer = (char *)((uintptr_t)pAddr + (uintptr_t)BE_TOP_HW_O24_IRR_DATA_OFFSET);

		if ((pstParams->pu32Data == NULL) 
			|| (pstParams->u32SizeNum == 0))
		{
			if (pFwMem->fw_size < (BE_TOP_HW_O24_IRR_DATA_OFFSET + 4)) BREAK_WRONG(pFwMem->fw_size);
		...
		else
		{
			...
			if ((inSize + BE_TOP_HW_O24_IRR_DATA_OFFSET + 4) > pFwMem->fw_size) BREAK_WRONG(inSize); // 4 : Mem data header
		...
		gTopHwO24Info.isIrrInit = TRUE; // update init flag before sending IPC (to avoid following case : send IPC -> suspend -> resume(update flag to FALSE) -> update flag to TRUE)
		
		stHwParams.uBaseAddr = pFwMem->fw_base + BE_TOP_HW_O24_IRR_ALUM_TABLE_OFFSET; // IRR_DATA_OFFSET is not used => Base is IRR_ALUM_TABLE_OFFSET
		ret = BE_KIPC_SetData(TOP_PACK_CMD(BE_TOP_SET_IRR), (void *)&stHwParams, sizeof(BE_IPC_IRR_T));
		if(ret) BREAK_WRONG(ret);
	} while(0);
...
int BE_TOP_HW_O24_GetIrrData(LX_BE_IRR_READ_T *pstParams)
{
	...
	do{ 
		if (gTopHwO24Info.isIrrInit == FALSE) BREAK_WRONG(gTopHwO24Info.isIrrInit);
		...
		else
		{
			...
			if ((outSize + BE_TOP_HW_O24_IRR_DATA_OFFSET + 4) > pFwMem->fw_size) BREAK_WRONG(outSize); // 4 : Mem data header
...
int BE_TOP_HW_O24_SetTpcParam(LX_BE_LUT_T *pstParams)
{
	...
	do{
		...
		pBuffer = (char *)((uintptr_t)pAddr + (uintptr_t)BE_TOP_HW_O24_TPC_PARAM_OFFSET);

		inSizeNum = pstParams->u32SizeNum;

		inSize = inSizeNum * 4;

		if ((inSize + BE_TOP_HW_O24_TPC_PARAM_OFFSET) > pFwMem->fw_size) BREAK_WRONG(inSize);
		...
		stHwParams.uParamBaseAddr = pFwMem->fw_base + BE_TOP_HW_O24_TPC_PARAM_OFFSET;
		stHwParams.uParamDbtype = (ver << 16) | inSizeNum;
		stHwParams.uStatusBaseAddr = pFwMem->fw_base + BE_TOP_HW_O24_TPC_STATUS_OFFSET;
...
int BE_TOP_HW_O24_GetTpcStatus(LX_BE_LUT_T *pstParams)
{
	...
	do{
		...
		pBuffer = (char *)((uintptr_t)pAddr + (uintptr_t)BE_TOP_HW_O24_TPC_STATUS_OFFSET);
		...
		if ((outSize + BE_TOP_HW_O24_TPC_STATUS_OFFSET + 8) > pFwMem->fw_size) BREAK_WRONG(outSize); // magic num(4) + size num(4)
...
int BE_TOP_HW_O24_GetApiForTpc(LX_BE_LUT_T *pstParams)
{
	...
	do{
		...
		DPE_LED_O24_RdFL(ad_apl_dbg_out);
		DPE_LED_O24_Rd01(ad_apl_dbg_out, apl_l, aplL);
		DPE_LED_O24_Rd01(ad_apl_dbg_out, apl_r, aplR);

		DPE_OLED_O24_RdFL(addr_oapl_ctrl_70);
		DPE_OLED_O24_Rd01(addr_oapl_ctrl_70, naive_apl_tcon_mux, naiveApl);

		DPE_OLED_O24_RdFL(addr_oapl_ctrl_69);
		DPE_OLED_O24_Rd01(addr_oapl_ctrl_69, oled_apl_tcon, oapl);
...
int be_top_hw_o24_get_disp_apl(UINT32 *pu32Param)
{
	...
	if (pu32Param) {
		DPE_OLED_O24_RdFL(addr_oapl_ctrl_70);
		DPE_OLED_O24_Rd01(addr_oapl_ctrl_70, naive_apl_tcon_mux, naive_apl);
...
//			BE_ERROR("*******************************************************d:0x%08x,0x%08x,0x%08x,0x%08x (%d,%d,%d,%d)\n",p[0],p[1],p[2],p[3],p[0],p[1],p[2],p[3]);
	...
	do{
		...
		DPE_OLED_O24_Wr01(addr_oapl_ctrl_01, reg_eotf_en, 0x1);
		DPE_OLED_O24_WrFL(addr_oapl_ctrl_01);

		DPE_OLED_O24_RdFL(addr_oapl_ctrl_00);
		DPE_OLED_O24_Wr01(addr_oapl_ctrl_00, reg_in_sel_apl_hdr, pBuffer[0]);
		DPE_OLED_O24_Wr01(addr_oapl_ctrl_00, reg_in_sel_apl_tcon, pBuffer[1]);
		DPE_OLED_O24_Wr01(addr_oapl_ctrl_00, reg_apl_out_mode_tcon, pBuffer[2]);
		DPE_OLED_O24_Wr01(addr_oapl_ctrl_00, reg_apl_out_mode_hdr, pBuffer[3]);
		DPE_OLED_O24_WrFL(addr_oapl_ctrl_00);

		DPE_OLED_O24_RdFL(addr_oapl_ctrl_00);
		DPE_OLED_O24_Rd01(addr_oapl_ctrl_00, reg_in_sel_apl_hdr, sel_apl_hdr);
		DPE_OLED_O24_Rd01(addr_oapl_ctrl_00, reg_in_sel_apl_tcon, sel_apl_tcon);
		DPE_OLED_O24_Rd01(addr_oapl_ctrl_00, reg_apl_out_mode_hdr, out_mode_hdr);
		DPE_OLED_O24_Rd01(addr_oapl_ctrl_00, reg_apl_out_mode_tcon, out_mode_tcon);
...
int BE_TOP_HW_O24_SetDispMode(LX_BE_DISP_MODE_T *pstParams)
{
	...
	do{
		...
		gTopHwO24Info.disp_mode.uDispResolution = stHwParams.uDispResolution;
		gTopHwO24Info.disp_mode.uFrameRate = stHwParams.uFrameRate;

		BE_NOTI("uDispResolution:%d,uFrameRate:%d,uFrate165:%d\n",stHwParams.uDispResolution,stHwParams.uFrameRate,stHwParams.uFrate165);
...
int BE_TOP_HW_O24_SetDClock(UINT32 *pu32Param)
{
	...
	do{
		...
		stHwParams.uFrate165 = *pu32Param;
		gTopHwO24Info.disp_mode.uFrate165 = stHwParams.uFrate165;
...
int BE_TOP_HW_O24_SetVideoVReverse(BOOLEAN bParam)
{
	...
	do{
		...
		/* see http://clm.lge.com/issue/browse/SCDCR-4730 */
		{
			...
			FBDEV_O24_CMN_MirrorCtrl(bParam);
		}
#endif
...
int BE_TOP_HW_O24_SetFRCStatus(UINT32 u32Param)
{
	...
	do{
		if (gTopHwO24Info.frcStatus != u32Param)
		{
			BE_NOTI("FRC status. prev: 0x%x, curr: 0x%x\n", gTopHwO24Info.frcStatus, u32Param);
			BE_REG_WR(0xCC862300, u32Param); //REG_CCO_VERI_SYS_VERI_0_T
			gTopHwO24Info.frcStatus = u32Param;
		}
...
//	BE_NOTI(" %d (0:S_PREPARE,1:SUSPEND,2:S_COMPLETED,3:R_PREPARE,4:RESUME,5:R_COMPLETED)\n",u32Param);
	...
	switch (u32Param)
	{
		case BE_TOP_HW_O24_PQE_PM_DEVICE_SUSPEND_PREPARE:
			break;
		case BE_TOP_HW_O24_PQE_PM_DEVICE_SUSPEND:
			break;
		case BE_TOP_HW_O24_PQE_PM_DEVICE_SUSPEND_COMPLETED:
			break;
		case BE_TOP_HW_O24_PQE_PM_DEVICE_RESUME_PREPARE:
			break;
		case BE_TOP_HW_O24_PQE_PM_DEVICE_RESUME:
			break;
		case BE_TOP_HW_O24_PQE_PM_DEVICE_RESUME_COMPLETED:
			break;
		default:
			break;
	}

	gTopHwO24Info.pmStatus = u32Param;

	BE_LED_HW_O24_SetPmMode(u32Param);
...
int BE_TOP_HW_O24_SetOaplMaxWeight(int Param)
{
	...
	do{
		...
		DPE_OLED_O24_RdFL(addr_oapl_ctrl_04);
		DPE_OLED_O24_Wr01(addr_oapl_ctrl_04, reg_w_crnt_wgt, nParam);
		DPE_OLED_O24_WrFL(addr_oapl_ctrl_04);
...
int BE_TOP_HW_O24_InformFBDEVStatus(LX_BE_FBDEV_STATUS_T *pstParams)
{
	...
	do{
		...
		BE_REG_WR(0xCC862308, regVal); //REG_CCO_VERI->SYS_VERI_2
...
int BE_TOP_HW_O24_ProcHandler(BE_TOP_PROC_ID_T proc_id, struct seq_file* m, void* data)
{
	...
	switch(proc_id)
	{
		...
		{
			seq_printf(m, "uDispResolution:%d,uFrameRate:%d,uFrate165:%d\n",gTopHwO24Info.disp_mode.uDispResolution,gTopHwO24Info.disp_mode.uFrameRate,gTopHwO24Info.disp_mode.uFrate165);
		}
		break;
		case BE_TOP_PROC_ID_ORBIT_INFO:
		{
			seq_printf(m, "vbe_orbit_on=%d\n", gTopHwO24Info.orbitInfo.bOnOff);
			seq_printf(m, "vbe_orbit_set_mode=%d\n", gTopHwO24Info.orbitInfo.bStoreMode ? 2 : gTopHwO24Info.orbitInfo.bAutoMode);
		}
		break;
		case BE_TOP_PROC_ID_LSR_INFO:
		{
			...
			do{
				if (gTopHwO24Info.lsrSize > 0)
				{
					pFwMem = (BE_CH_MEM_T *) &gpBeMem->fw[3];
					CHECK_NULL(pFwMem);
					CHECK_NULL(pFwMem->fw_base);

					if ((gTopHwO24Info.lsrSize + BE_TOP_HW_O24_LSR_TABLE_OFFSET) > pFwMem->fw_size) BREAK_WRONG(gTopHwO24Info.lsrSize);

					pBuffer = (char *)vmalloc(gTopHwO24Info.lsrSize);
				 	CHECK_NULL(pBuffer);

					pAddr = (char *)vmap_phys(pFwMem->fw_base, pFwMem->fw_size);
					CHECK_NULL(pAddr);

					memcpy(pBuffer, (char *)((uintptr_t)pAddr + (uintptr_t)BE_TOP_HW_O24_LSR_TABLE_OFFSET), gTopHwO24Info.lsrSize);

					rmb();

					vunmap_phys(pAddr);

					for(i=0; i < (gTopHwO24Info.lsrSize/sizeof(UINT32)); i++)
					{
						seq_printf(m, "0x%x", ((UINT32 *)pBuffer)[i]);

						if (i+1 < (gTopHwO24Info.lsrSize/sizeof(UINT32)))
							seq_printf(m, ", ");
					}
				}
			} while(0);
		...
		case BE_TOP_PROC_ID_GSR_INFO:
		{
			...
			do{
				if (gTopHwO24Info.gsrSize > 0)
				{
					pFwMem = (BE_CH_MEM_T *) &gpBeMem->fw[3];
					CHECK_NULL(pFwMem);
					CHECK_NULL(pFwMem->fw_base);

					if ((gTopHwO24Info.gsrSize + BE_TOP_HW_O24_GSR_TABLE_OFFSET) > pFwMem->fw_size) BREAK_WRONG(gTopHwO24Info.gsrSize);

					pBuffer = (char *)vmalloc(gTopHwO24Info.gsrSize);
				 	CHECK_NULL(pBuffer);

					pAddr = (char *)vmap_phys(pFwMem->fw_base, pFwMem->fw_size);
					CHECK_NULL(pAddr);

					memcpy(pBuffer, (char *)((uintptr_t)pAddr + (uintptr_t)BE_TOP_HW_O24_GSR_TABLE_OFFSET), gTopHwO24Info.gsrSize);

					rmb();

					vunmap_phys(pAddr);

					for(i=0; i < (gTopHwO24Info.gsrSize/sizeof(UINT32)); i++)
					{
						seq_printf(m, "0x%x", ((UINT32 *)pBuffer)[i]);

						if (i+1 < (gTopHwO24Info.gsrSize/sizeof(UINT32)))
							seq_printf(m, ", ");
					}
				}
			} while(0);
		...
		case BE_TOP_PROC_ID_OSDLVL_INFO:
		{
			...
			BE_TOP_HW_O24_GetOSDAlphaAPL(&pstParams);
			seq_printf(m, "vbe_osd_alpha_val=%d\n", gTopHwO24Info.osdLvl);
			seq_printf(m, "vbe_osd_gain=%d\n", gTopHwO24Info.osdGain);
		}
		break;
		case BE_TOP_PROC_ID_CPC_INFO:
		{
			seq_printf(m, "vbe_cpc=%d\n", gTopHwO24Info.cpc);
			{
				...
				seq_printf(m, "vbe_mem_tpc_param_addr=0x%x\n", pFwMem->fw_base + BE_TOP_HW_O24_TPC_PARAM_OFFSET);
				seq_printf(m, "vbe_mem_tpc_status_addr=0x%x\n", pFwMem->fw_base + BE_TOP_HW_O24_TPC_STATUS_OFFSET);
			}
		}
		break;
...
int BE_TOP_HW_O24_GetPatternInfo(void *pParam, UINT32 *size)
{
	...
	do{
		...
		rdPtr = (UINT32 *)((char *)pAddr + BE_TOP_HW_O24_PATTERN_INFO_OFFSET);
