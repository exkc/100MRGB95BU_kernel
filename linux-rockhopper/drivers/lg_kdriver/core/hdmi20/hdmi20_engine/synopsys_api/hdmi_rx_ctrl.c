...
#define NEW_DCM_CURRENT_MODE		MSK(4, 4)
...
/** Audio Type changed */
#define		AUD_TYPE_CHG			BIT(29)
/** ACR N CHANGE */
#define		ACR_N_CHG				BIT(23)
/** ACR CTS CHANGE */
#define		ACR_CTS_CHG				BIT(22)
/** GCP AV MUTE CHANGE */
#define		GCP_AVMUTE_CHG			BIT(21)


#define		FIFO_TH_MIX_PASS		BIT(0)
#define		FIFO_TH_MAX_PASS		BIT(1)
#define		FIFO_TH_START_PASS		BIT(2)
#define		NTSCVBL_RCV				BIT(11)
#define		NTSCVBL_CKS_CHG			BIT(12)
#define		AMP_CHG					BIT(13)
#define		AMP_RCV					BIT(14)
#define		VSI_RCV					BIT(15)
#define		GCP_RCV					BIT(16)
#define		ACR_RCV					BIT(17)
#define		AVI_RCV					BIT(18)
#define		AIF_RCV					BIT(19)
#define		GMD_RCV					BIT(20)
#define		GMD_CKS_CHG				BIT(26)
#define		VSI_CKS_CHG				BIT(27)
#define		DRM_RCV					BIT(30)
#define		DRM_CKS_CHG				BIT(31)

#define		ADU_INTR				(AUD_TYPE_CHG|ACR_N_CHG|GCP_AVMUTE_CHG)
#define		ALL_INTR				(AUD_TYPE_CHG|ACR_N_CHG|ACR_CTS_CHG|GCP_AVMUTE_CHG|FIFO_TH_MIX_PASS|FIFO_TH_MAX_PASS|FIFO_TH_START_PASS|NTSCVBL_RCV|NTSCVBL_CKS_CHG|AMP_CHG|AMP_CHG|AMP_RCV|VSI_RCV|GCP_RCV|ACR_RCV|AVI_RCV|AIF_RCV|GMD_RCV|GMD_CKS_CHG|VSI_CKS_CHG|DRM_RCV|DRM_CKS_CHG)
...
//#define		VIDEO_MODE				(MSK(3,9) | MSK(1,6))
/* BIT  3 : Interlaced mode active flag status
 * BIT  6 : Hactive pixel passed threshold flag status
 * BIT  9 : Vactive passed threshold flag status
 * BIT 10 : Vtotal passed threshold flag status
 * BIT 11 : Voffset passed threshold flag status */
 ...
 * @return error code
 */
static int interrupts_hpd(struct hdmi_rx_ctrl *ctx, bool enable, bool disable)
{
	int error = 0;
	...
	if (enable) {
		/* set enable */
		ctx->bsp_write(RA_PDEC_IEN_SET, DVIDET|AIF_CKS_CHG|AVI_CKS_CHG|PD_FIFO_NEW_ENTRY|PD_FIFO_OVERFL|ADU_INTR);
		...
		/* clear enable */
		ctx->bsp_write(RA_PDEC_IEN_CLR, DVIDET|AIF_CKS_CHG|AVI_CKS_CHG|PD_FIFO_NEW_ENTRY|PD_FIFO_OVERFL|ADU_INTR);
		...
		/* clear status */
		ctx->bsp_write(RA_PDEC_ICLR, DVIDET|AIF_CKS_CHG|AVI_CKS_CHG|PD_FIFO_NEW_ENTRY|PD_FIFO_OVERFL|ADU_INTR);
...
int hdmi_rx_ctrl_irq_handler(struct hdmi_rx_ctrl *ctx)
{
	...
	if (intr != 0) {
		...
		if (get(intr, CLK_CHANGE) != 0) {
			knownISRType = 1;
			ctx->isr_count[HDMI20_ISR_CLK_CHANGE]++;
...
#if 1
			if (tclk == 0) {
				error |= interrupts_hpd(ctx, false, 0);
				error |= control_clk_range(ctx, TMDS_CLK_MIN, TMDS_CLK_MAX);
			}
			else {
				...
				error |= interrupts_hpd(ctx, true, 0);
			}
#endif
			ctx->tmds_clk = tclk;
...
#ifdef CTRL_13xa  /* RX Controller Release 1.30a and above */
			knownISRType = 1;
			ctx->isr_count[HDMI20_ISR_DCM_CURRENT_MODE_CHG]++;

			if(ctx->bsp_read(RA_PDEC_GCP_AVMUTE) & 0x00000002){
				ctx->log_info("--%d-----------DCM------------------- GCP AVMUTE 0x%08x",ctx->device_id, ctx->bsp_read(RA_PDEC_GCP_AVMUTE));
				ctx->isMute = 1;
		...
		if (get(intr, AKSV_RCV) != 0) {
			knownISRType = 1;
			ctx->isr_count[HDMI20_ISR_AKSV_RCV]++;
	...
	if (intr != 0) {
		...
		if (get(intr, VIDEO_MODE) != 0) {
			knownISRType = 1;
			ctx->isr_count[HDMI20_ISR_VIDEO_MODE]++;
	...
	if (intr != 0) {
		...
		if (get(intr, (DVIDET|AVI_CKS_CHG)) != 0) {
			knownISRType = 1;
			ctx->isr_count[HDMI20_ISR_DVIDET]++;
			ctx->isr_count[HDMI20_ISR_AVI_CKS_CHG]++;
			execute[hdmi_rx_ctrl_event_video_status] = true;
			//ctx->log_info("HDMI Rx Demo: video related event 0x%x", intr);
		}
		if (get(intr, AIF_CKS_CHG) != 0) {
			knownISRType = 1;
			ctx->isr_count[HDMI20_ISR_AIF_CKS_CHG]++;
			execute[hdmi_rx_ctrl_event_audio_status] = true;
		}
		if (get(intr, (AUD_TYPE_CHG)) != 0) {
			knownISRType = 1;
			ctx->isr_count[HDMI20_ISR_AUD_TYPE_CHG]++;
			execute[hdmi_rx_ctrl_event_audio_status2] = true;
		}
		if (get(intr, (ACR_N_CHG)) != 0) {
			knownISRType = 1;
			ctx->isr_count[HDMI20_ISR_ACR_N_CHG]++;
			execute[hdmi_rx_ctrl_event_audio_status3] = true;
		}
		if (get(intr, (GCP_AVMUTE_CHG)) != 0) {
			knownISRType = 1;
			ctx->isr_count[HDMI20_ISR_GCP_AVMUTE_CHG]++;
			execute[hdmi_rx_ctrl_event_audio_status4] = true;
		}
		if (get(intr, PD_FIFO_NEW_ENTRY) != 0) {
			knownISRType = 1;
			ctx->isr_count[HDMI20_ISR_PD_FIFO_NEW_ENTRY]++;
		...
		if (get(intr, PD_FIFO_OVERFL) != 0) {
			knownISRType = 1;
			ctx->isr_count[HDMI20_ISR_PD_FIFO_OVERFL]++;
			error |= hdmi_rx_ctrl_packet_fifo_rst(ctx);
		}
	...
	if (intr != 0) {
		knownISRType = 1;
		ctx->isr_count[HDMI20_ISR_RA_AUD_CLK_ISTS]++;
	...
	if (intr != 0) {
		if (get(intr, AFIF_OVERFL|AFIF_UNDERFL) != 0) {
			knownISRType = 1;
			error |= hdmi_rx_ctrl_audio_fifo_rst(ctx);
			ctx->isr_count[HDMI20_ISR_AFIF_OVERFL]++;
			ctx->isr_count[HDMI20_ISR_AFIF_UNDERFL]++;
		}
	...
	if(knownISRType == 0){
		ctx->isr_count[HDMI20_ISR_UNKNOWN]++;
	}
...
int hdmi_rx_ctrl_store(struct hdmi_rx_ctrl *ctx, enum hdmi_rx_ctrl_packet_t type, bool enable)
{
	...
	/* SW WA : for T7 HDR HDR Stuck case */
	error |= io_write(ctx, RA_PDEC_CTRL, PD_FIFO_CLR,  0);
...
int hdmi_rx_ctrl_get_video(struct hdmi_rx_ctrl *ctx, struct hdmi_rx_ctrl_video *params, uint8_t verbose)
{
	...
	//tmp = io_read(ctx, RA_PDEC_GCP_AVMUTE, NEW_DCM_CURRENT_MODE);
	...
	if (params->pixel_clk < PIXEL_CLK_MIN
...
void hdmi_rx_clear_isr_counter(struct hdmi_rx_ctrl *ctx, HDMI20_ISR_TYPE_T type)
{
	int i;
	...
	if(type >= HDMI20_ISR_MAXNUM){
		for ( i = 0; i< HDMI20_ISR_MAXNUM; i++) {
			ctx->isr_count[i] = 0;
		}
	}
	else{
		ctx->isr_count[type] = 0;
	}
}

char *isr_type_to_str(int id)
{
    switch (id)
	{
		case HDMI20_ISR_CLK_CHANGE:
			return "CLK_CHANGE";
		case HDMI20_ISR_DVIDET:
			return "DVIDET";
		case HDMI20_ISR_AIF_CKS_CHG:
			return "AIF_CKS_CHG";
		case HDMI20_ISR_AVI_CKS_CHG:
			return "AVI_CKS_CHG";
		case HDMI20_ISR_PD_FIFO_NEW_ENTRY:
			return "PD_FIFO_NEW_ENTRY";
		case HDMI20_ISR_PD_FIFO_OVERFL:
			return "PD_FIFO_OVERFL";
		case HDMI20_ISR_ADU_INTR:
			return "ADU_INTR";
		case HDMI20_ISR_AFIF_OVERFL:
			return "AFIF_OVERFL";
		case HDMI20_ISR_AFIF_UNDERFL:
			return "AFIF_UNDERFL";
		case HDMI20_ISR_VIDEO_MODE:
			return "VIDEO_MODE";
		case HDMI20_ISR_HDCP22_CAP:
			return "HDCP22_CAP";
		case HDMI20_ISR_HDCP22_NCAP:
			return "HDCP22_NCAP";
		case HDMI20_ISR_HDCP22_AUTH_LOST:
			return "HDCP22_AUTH_LOST";
		case HDMI20_ISR_HDCP22_AUTH_ISTS:
			return "HDCP22_AUTH_ISTS";
		case HDMI20_ISR_HDCP22_AUTH_FAIL:
			return "HDCP22_AUTH_FAIL";
		case HDMI20_ISR_HDCP22_DECR_CHANGE:
			return "HDCP22_DECR_CHANGE";
		case HDMI20_ISR_AKSV_RCV:
			return "AKSV_RCV";
		case HDMI20_ISR_DCM_CURRENT_MODE_CHG:
			return "DCM_CURRENT_MODE_CHG";
		case HDMI20_ISR_PLL_LOCK_STATUS:
			return "PLL_LOCK_STATUS";
		case HDMI20_ISR_PHY_I2C_DONE:
			return "PHY_I2C_DONE";
		case HDMI20_ISR_PHY_I2C_NACK:
			return "PHY_I2C_NACK";
		case HDMI20_ISR_SCDC_0:
			return "SCDC_0";
		case HDMI20_ISR_SCDC_1:
			return "SCDC_1";
		case HDMI20_ISR_AUD_TYPE_CHG:
			return "AUD_TYPE_CHG";
		case HDMI20_ISR_ACR_N_CHG:
			return "ACR_N_CHG";
		case HDMI20_ISR_GCP_AVMUTE_CHG:
			return "GCP_AVMUTE_CHG";
		case HDMI20_ISR_RA_AUD_CLK_ISTS:
			return "RA_AUD_CLK_ISTS";
		case HDMI20_ISR_UNKNOWN:
			return "UNKNOWN_TYPE";
		case HDMI20_ISR_LINK_TOP_5V:
			return "LINK_TOP_5V";
		case HDMI20_ISR_LINK_TOP_TCSDONE:
			return "LINK_TOP_TCS_DONE";
		case HDMI20_ISR_LINK_TOP_PCPD:
			return "LINK_TOP_PCPD";
		case HDMI20_ISR_LINK_TOP_UNKNOWN:
			return "LINK_TOP_UNKNOWN";
		default:
			return "Error";
	}
}
