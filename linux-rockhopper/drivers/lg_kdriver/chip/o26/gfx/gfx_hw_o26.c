...
#include "de_kapi.h"
extern int DE_REG_O26_GetSourceStatus(LX_DE_SRC_STATUS_T* pstParams);
#endif
...
#define GFX_WRITE_REG(reg,val)  \
    __raw_writel(val , (volatile void*)((uintptr_t)(g_gfx_reg) + offsetof(GFX_O26_REG_T,reg)))

#define GFX_READ_REG(reg)   \
    __raw_readl( (volatile void*)((uintptr_t)(g_gfx_reg) + offsetof(GFX_O26_REG_T,reg)))

//#define SHOW_GFX_REGISTER //only debugging
#if defined(SHOW_GFX_REGISTER)

#define PRINT_READ_REG(reg) \
   do { printk("GFX REG 0x%x -> 0x%0.8x\n",((UINT32)OFFSET(GFX_O26_REG_T,reg)), *((UINT32 *)(&g_gfx_reg_cache->reg))); } while(0);
...
GFX_O26_SCALE_VPHASE_DATA_T;

typedef struct
{
#define GFX_SCALE_PHASE_DATA_NUM    16
    GFX_O26_SCALE_HPHASE_DATA_T     hphase[GFX_SCALE_PHASE_DATA_NUM];       /* hhpase data */
    GFX_O26_SCALE_VPHASE_DATA_T     vphase[GFX_SCALE_PHASE_DATA_NUM];       /* vhpase data */
}
GFX_O26_SCALE_FILTER_DATA_T;

/*----------------------------------------------------------------------------------------
    Function Prototype Declarations
----------------------------------------------------------------------------------------*/
void GFX_O26_InitScaler(void);
int GFX_O26_RunFlushCommand(void);
int GFX_O26_RunStartCommand(void);
int GFX_O26_PauseCommand(void);
int GFX_O26_ResumeCommand(void);
int GFX_O26_SwResetCommand(void);
int GFX_O26_StopCommand(void);
void GFX_O26_GetComQueueStatus(GFX_CMD_QUEUE_CTRL_T* queue);
BOOLEAN GFX_O26_IsGfxIdle(void);
void GFX_O26_GetOperationStatus(GFX_CMD_OP_T* operate);
void GFX_O26_SetOperationStatus(GFX_CMD_OP_T* operate);
void GFX_O26_GetInputConfigure(int iPort, GFX_PORT_CONFIGURE_T* port);
void GFX_O26_SetInputConfigure(GFX_PORT_CONFIGURE_T* port);
void GFX_O26_GetBlendConfigure(GFX_BLEND_CONFIGURE_T* blend);
void GFX_O26_SetBlendingOut(GFX_ENUM_OUT_T type);
void GFX_O26_SetBlendConfigure(GFX_BLEND_CONFIGURE_T* blend);
void GFX_O26_GetOutputConfigure(GFX_OUT_CONFIGURE_T* port);
void GFX_O26_SetOutputConfigure(GFX_OUT_CONFIGURE_T* port);
void GFX_O26_SetScalerConfigure(GFX_SCALER_CONFIGURE_T* scaler);
void GFX_O26_SetInterruptMode(UINT32 uiMode);
void GFX_O26_SetAckMode(UINT32* mode);
void GFX_O26_GetAckMode(UINT32* mode);
int GFX_O26_SetCLUT(int port, int size, UINT32* data);
void GFX_O26_SetCommandDelayReg(UINT16 delay);
UINT16 GFX_O26_GetCommandDelayReg(void);
void GFX_O26_DumpRegister(void);
int GFX_O26_RunSuspend(void);
int GFX_O26_RunResume(void);
int GFX_O26_RunClkOnCommand(void);
int GFX_O26_RunClkOffCommand(void);
BOOLEAN GFX_O26_ClkCheck(void);

/*----------------------------------------------------------------------------------------
    External Variables
----------------------------------------------------------------------------------------*/
extern volatile GFX_O26_REG_T*   g_gfx_reg;
extern GFX_O26_REG_T*            g_gfx_reg_cache;

/*----------------------------------------------------------------------------------------
    global Variables
----------------------------------------------------------------------------------------*/
volatile UINT32 g_ui_ack_mode_o26 = 0;

#ifdef  KDRV_CONFIG_PM
unsigned int g_pm_cmd_delay_o26 = 0;
unsigned int g_pm_intr_ctrl_o26 = 0;
#endif
...
/*----------------------------------------------------------------------------------------
    Static Variables
----------------------------------------------------------------------------------------*/
static int  _g_gfx_o26_bw_val           = 0x7ff;
static int  _g_gfx_o26_bw_val_req       = 0x7ff;
module_param_named(gfx_o26_bw_val, _g_gfx_o26_bw_val_req, int, 0644);

// see http://clm.lge.com/issue/browse/SCDCR-6440
static int _g_gfx_down_scaler_phase_offset  = 0x30;
module_param_named(o26_gfx_down_scaler_phase_offset, _g_gfx_down_scaler_phase_offset, int, 0644);


#define GFX_O26_CUSTOM_FILTER_MAX   4

static int _g_gfx_o26_scaler_index = -1;
module_param_named(gfx_o26_scaler_index, _g_gfx_o26_scaler_index, int, 0644);

static GFX_O26_SCALE_FILTER_DATA_T  g_gfx_o26_custom_scale_filter_data[GFX_O26_CUSTOM_FILTER_MAX] =
{
    /* normal upscale */
    [0] = {
 ...
 *
 */
static int      GFX_O26_GetBestHVScalerIndex(GFX_SCALER_CONFIGURE_T* scaler)
{
    int h_scale_ratio = 0;
    int v_scale_ratio = 0;

    if (_g_gfx_o26_scaler_index >= 0) return _g_gfx_o26_scaler_index;
...
static int _o26_scaler_mux = -1;
module_param_named(o26_scaler_insel, _o26_scaler_mux, int, 0644);

static int _o26_swap_r0_r1 = -1;
module_param_named(o26_swap_read_port, _o26_swap_r0_r1, int, 0644);
...
static int _o26_dump_reg_cached = -1;
module_param_named(o26_dump_before_start, _o26_dump_reg_cached, int, 0644);

static inline void _gfx_dump_register_cached(void)
{
    int i;
    int j;
    UINT32* reg_ptr = (UINT32*)g_gfx_reg_cache;
...
static inline void GFX_O26_Bypass2ndBlender(void)
{
...
static int _o26_vreverse_r0 = -1;
module_param_named(o26_reverse_r0, _o26_vreverse_r0, int, 0644);

static int _o26_vreverse_r1 = -1;
module_param_named(o26_reverse_r1, _o26_vreverse_r1, int, 0644);

static int _o26_vreverse_r2 = -1;
module_param_named(o26_reverse_r2, _o26_vreverse_r2, int, 0644);

static int _o26_comp_sparse = -1;
module_param_named(o26_comp_mode, _o26_comp_sparse, int, 0644);
...
{
    int i, j;
    GFX_O26_SCALE_FILTER_DATA_T*    filter_data;

    GFX_WRITE_REG(scaler_frame_start, 0x1);                 /* scaler frame start (reset) */

    for (j = 0; j < GFX_O26_CUSTOM_FILTER_MAX; j++)              /* download custome filter data */
    {
        filter_data = &(g_gfx_o26_custom_scale_filter_data[j]);
...
int GFX_O26_RunStartCommand(void)
{
    ...
    if (_g_gfx_o26_bw_val != _g_gfx_o26_bw_val_req)
    {
        _g_gfx_o26_bw_val_req &= 0x7ff;
#if defined(FIXME) // do not support... defer ~!!
        GFX_WARN("+++ bw_limit changed 0x%x -> 0x%x\n", _g_gfx_o26_bw_val, _g_gfx_o26_bw_val_req);
        _g_gfx_o26_bw_val = _g_gfx_o26_bw_val_req;
        OS_WrReg(0xc000e4f8, _g_gfx_o26_bw_val);
#endif
    }

    if (_o26_dump_reg_cached > 0)
    {
    ...
    /* first, decoder */
    {
        if (-1 != _o26_comp_sparse)
        {
            GFX_Wr01(rd_comp_control, sparse_alloc, _o26_comp_sparse);
        }
        else
        {
    ...
    /* second, encoder .. ToDo.. */
    {
        if (-1 != _o26_comp_sparse)
        {
            GFX_Wr01(wr_comp_control, sparse_alloc, _o26_comp_sparse);
        }
        else
        {
            /* default is the compact mode (=0) */
...
BOOLEAN GFX_O26_IsGfxIdle(void)
{
    ...
    //          g_gfx_reg_cache->gfx_status0.exec_status, g_gfx_reg_cache->gfx_status0.exec_line, g_gfx_reg_cache->gfx_status1.cmd_que_status);
#endif
 ...
 *
 *  @note operation status should be read at GFX_O26_GetOperationStatus
 */
void GFX_O26_SetOperationStatus(GFX_CMD_OP_T* operate)
{
    GFX_TRACE_BEGIN();

    /* L18F,O26 추가 - multi-source blender 가 아니라면 2nd blender 를 사용하지 않는 것이구나. */
    if (operate->sPort != eReadThreePorts)
    {
        GFX_O26_Bypass2ndBlender();
    }
    ...
    if (-1 != _o26_swap_r0_r1)
    {
        ...
        if (operate->sPort == eReadTwoPorts)
        {
            ...
            GFX_DEBUG("debug mode from o26_swap_read_port(=%d)\n", _o26_swap_r0_r1);
            GFX_DEBUG(">>>> before <<<<\n"
            ...
            GFX_O26_SetInputConfigure(&port0);
            GFX_O26_SetInputConfigure(&port1);
...
void GFX_O26_SetInputConfigure(GFX_PORT_CONFIGURE_T* port)
{
    ...
    switch (port->sPort)
    {
        case ePort0:
        {
...
#ifdef BUILD_FEATURE_fpga
                GFX_DEBUG("%s:%d fbdc header offset : (0x%x)\n", __func__, __LINE__, port->header_offset);
#endif
            ...
            if (-1 != _o26_vreverse_r0)
            {
                printk("o26_reverse_r0 = (%d)\n", _o26_vreverse_r0);
                port->reversed = _o26_vreverse_r0;
            }
        ...
        case ePort1:
        {
            ...
            if (-1 != _o26_vreverse_r1)
            {
                printk("o26_reverse_r1 = (%d)\n", _o26_vreverse_r1);
                port->reversed = _o26_vreverse_r1;
            }
        ...
        case ePort2:
        {
            ...
            if (-1 != _o26_vreverse_r2)
            {
                printk("o26_reverse_r2 = (%d)\n", _o26_vreverse_r2);
                port->reversed = _o26_vreverse_r2;
            }
...
void GFX_O26_GetBlendConfigure(GFX_BLEND_CONFIGURE_T* blend)
{
    ...
    //  blend->ulBlendConstant  = g_gfx_reg->out_const_ctrl.abwConstantCtrl;
    GFX_Rd01(rop_ctrl,      rop_ctrl,       blend->sRaster);
    //  blend->sRaster          = g_gfx_reg->out_rop_ctrl.abwRasterSel;
 ...
 *
 *  @note out_sel should be read before this function at GFX_O26_GetBlendConfigure
 */
void GFX_O26_SetBlendingOut(GFX_ENUM_OUT_T type)
{
    GFX_TRACE_BEGIN();
 ...
 *
 *  @note all data should be read before this function at GFX_O26_GetBlendConfigure
 */
void GFX_O26_SetBlendConfigure(GFX_BLEND_CONFIGURE_T* blend)
{
    ...
    //  blend->sOut         = g_gfx_reg->out_sel.abwOutSel;
...
static int GFX_O26_SelectScalerOrder(GFX_SCALER_CONFIGURE_T* s)
{
    int vh_order;
...
void GFX_O26_SetScalerConfigure(GFX_SCALER_CONFIGURE_T* scaler)
{
    switch (scaler->cmd)
    {
        ...
        {
            int scaler_set_idx = GFX_O26_GetBestHVScalerIndex(scaler);
            int vh_order = GFX_O26_SelectScalerOrder(scaler);
            ...
            if (-1 != _o26_scaler_mux)
            {
#ifdef BUILD_FEATURE_fpga
                GFX_DEBUG("debug mode from o26_scaler_insel(=%d)\n", _o26_scaler_mux);
#endif

                switch (_o26_scaler_mux)
                {
                    case 0:
                    case 1:
                    case 2:
                        ...
                        GFX_WrFL(wr_size);
                        GFX_Wr01(scaler_bypass, scaler_insel, _o26_scaler_mux);
...
#ifdef BUILD_FEATURE_fpga
            GFX_DEBUG("scaler_bypass(=0x180) -> x%x\n", GFX_Rd(scaler_bypass));
            GFX_DEBUG("wr_size(=0x12c) -> x%x\n", GFX_Rd(wr_size));
#endif
...
int GFX_O26_SetCLUT(int port, int size, UINT32* data)
{
    ...
    switch (port)
    {
        ...
        case 2:
        {
            g_gfx_reg->r2_clut_ctrl.clut2_addr = 0;
            g_gfx_reg->r2_clut_ctrl.clut2_rw    = 0;
#ifdef GFX_PALETTE_AUTO_INC
            g_gfx_reg->r2_clut_ctrl.clut2_wai   = 1;
#else
            g_gfx_reg->r2_clut_ctrl.clut2_wai   = 0;
#endif
...
#else
                g_gfx_reg->r2_clut_ctrl.clut2_addr = i;
#endif
                g_gfx_reg->r2_clut_data         = *data++;
            }
        }
        break;
...
static UINT16   GFX_O26_CalcAutoCommandDelay(void)
{
#ifdef GFX_AUTO_CMD_DELAY
    int rc;
    ...
    rc = DE_REG_O26_GetSourceStatus(&de_src_param);
...
void GFX_O26_SetCommandDelayReg(UINT16 delay)
{
    ...
    if (delay == (UINT16) - 1)
    {
        delay = GFX_O26_CalcAutoCommandDelay();
    }
...
{
    int i;
    UINT32* reg_ptr = (UINT32*)g_gfx_reg;
...
#define GFX_O26_CG_MASK_APB     0x1
#define GFX_O26_CG_MASK_CORE    0x2

/** GFX clock gating function
 *
 * see http://clm.lge.com/issue/browse/SICDTV-10001
 */
static int GFX_O26_SetClockGatingReg(bool cg_en, UINT32 cg_mask)
{
#if 0
    UINT32 val = 0x1;
#endif
...
#if 0 // TBD: not ready
    CTOP_CTRL_O26Ax_Wr(SCRG_CTRL_GFX, crg_gfx02, 0x00000000);

    if (cg_mask & GFX_O26_CG_MASK_APB)
    {
        CTOP_CTRL_O26Ax_Wr01(SCRG_CTRL_GFX, crg_gfx02, reg_gfx_apb_clk_gate_en, cg_en);
    }

    if (cg_mask & GFX_O26_CG_MASK_CORE)
    {
        CTOP_CTRL_O26Ax_Wr01(SCRG_CTRL_GFX, crg_gfx02, reg_gfx_core_clk_gate_en, cg_en);
    }

    CTOP_CTRL_O26Ax_WrFL(SCRG_CTRL_GFX, crg_gfx02);

    val = CTOP_CTRL_O26Ax_Rd(SCRG_CTRL_GFX, crg_gfx02);
...
int GFX_O26_RunClkOnCommand(void)
{
    ...
    GFX_O26_SetClockGatingReg(true, GFX_O26_CG_MASK_APB | GFX_O26_CG_MASK_CORE); //GFX_O26_CG_MASK_CORE);
    return RET_OK;
}

int GFX_O26_RunClkOffCommand(void)
{
    GFX_O26_SetClockGatingReg(false, GFX_O26_CG_MASK_APB | GFX_O26_CG_MASK_CORE); //GFX_O26_CG_MASK_CORE);
    return RET_OK;
}
...
#if 0 // TBD: not ready
    CTOP_CTRL_O26Ax_RdFL(SCRG_CTRL_GFX, crg_gfx02);
    CTOP_CTRL_O26Ax_Rd01(SCRG_CTRL_GFX, crg_gfx02, reg_gfx_core_clk_gate_en, val);
#endif
