...
void BASE_DEVMEM_DumpMemCfg(struct seq_file* m, UINT32 opt);
...
static const char* fmtMemCfgHeadLine    =
    "---------------------------------------------------------------------------------------------\n"
    "Name (%-8s)    ,Alloc Base  ~ Alloc End [Alloc Size]     Alloc Base MB [Alloc Size MB]\n"
    "---------------------------------------------------------------------------------------------\n";
...
#if 0 /* detailed print for debug */

    if (lx_chip() == LX_CHIP_M19)
    {
        BASE_DEVMEM_Print(NULL, NULL, 1);
    }
...
static int BASE_DEVMEM_AllocDevMem(LX_MEMCFG_ENTRY_T* pEntry)
{
    ...
    while (pEntry && pEntry->pM)
    {
        ...
        for (; nM > 0; nM--, pM++)
        {
            ...
            mod_base_addr = min(mod_base_addr, pM->base);
            mod_end_addr  = max(mod_end_addr, EndAddr);

#if (DEVMEM_CHECK>0)
            /* verifiy memory address */
            {
                ...
                __CHECK_IF_ERROR(CurrModuleAddr     < DDRMemAddr,   printk, ErrCnt++ /* go on ?? */,
                                 "memchk error #1 (%12s).. module base address is outside DDR range. base=0x%08x, DDR=0x%08x~0x%08x\n",
                                 (pM->name) ? pM->name : "(nil)", CurrModuleAddr, DDRMemAddr, DDRMemEndAddr);
                __CHECK_IF_ERROR(CurrModuleEndAddr > DDRMemEndAddr, printk, ErrCnt++ /* go on ?? */,
                                 "memchk error #2 (%12s).. module end  address is outside DDR range. base=0x%08x, DDR=0x%08x~0x%08x\n",
                                 (pM->name) ? pM->name : "(nil)", CurrModuleEndAddr, DDRMemAddr, DDRMemEndAddr);
                /* check if memory overlap */
                __CHECK_IF_ERROR(CurrModuleAddr     < PrevModuleEndAddr,    printk, ErrCnt++ /* go on ?? */,
                                 "memchk error #3 (%12s).. module base address overlaps with previous one. base=0x%08x, prev=0x%08x\n",
                                 (pM->name) ? pM->name : "(nil)", CurrModuleAddr, PrevModuleEndAddr);
        ...
        /* if user want to know the layout result, report it ! */
        if (pEntry->flag & LX_MEMCFG_FLAG_ALIAS && pEntry->pAlias)
        {
            pEntry->pAlias->base = mod_base_addr;
            pEntry->pAlias->size = mod_end_addr - mod_base_addr;
        }
...
static void BASE_DEVMEM_Print(struct seq_file* m, LX_MEMCFG_ENTRY_T* pMemCfgEntry, UINT32 option)
{
    ...
    while (pEntry && pEntry->pM)
    {
        ...
        if (pM->name && !strncmp(pM->name, DEVMEM_HDR_STRING, strlen(DEVMEM_HDR_STRING)))
        {
            if (m)  seq_printf(m, fmtMemCfgHeadLine, pM->name + strlen(DEVMEM_HDR_STRING));
            else    printk(fmtMemCfgHeadLine, pM->name + strlen(DEVMEM_HDR_STRING));
...
static void BASE_DEVMEM_PRINT(struct seq_file* m, LX_MEMCFG_T* pM, int addr_shift, UINT32 prvEnd)
{
    ...
    if (m)
    {
        seq_printf(m, fmtEntryFormatSub,
...
void BASE_DEVMEM_DumpMemCfg(struct seq_file* m, UINT32 opt)
{
    BASE_DEVMEM_Print(m, NULL, opt);    // detailed dump
}
