...
static int do_ump_dd_allocate(umpp_session * session, ump_k_allocate * params)
{
	...
	if (UMP_DD_INVALID_MEMORY_HANDLE != new_allocation)
	{
		...
		if (NULL != tracker)
		{
			...
			alloc = (umpp_allocation *)new_allocation;
			alloc->client = params->client_pid;         // client pid
		...
		if (session->import_handler_data[params->type])
		{
			...
			if (is_compat_task())
			{
				new_allocation = handler->import( session->import_handler_data[params->type],
			                                      (void *)params->phandle.compat_value,
			                                      params->alloc_flags);
			}
			else
			{
...
EXPORT_SYMBOL(ump_debug_mem_for_meminfo);
EXPORT_SYMBOL(ump_debug_mem_for_webos_mem);
#ifdef CONFIG_KDS
