...
#ifdef CONFIG_ARCH_LG1K
	memcpy_fromio (&qtd_dummy, qtd, sizeof *qtd);
	mb();
#endif
	return count;
}
...
qh_refresh (struct ehci_hcd *ehci, struct ehci_qh *qh)
{
	...
	if (qh->hw->hw_token & ACTIVE_BIT(ehci)) {
		...
		if (qh->shortread_but_active)
			ehci_warn(ehci, "qh %p active with short read!\n", qh);
	} else {
		qh_update(ehci, qh, qtd);
	}
	qh->should_be_inactive = 0;
	qh->shortread_but_active = 0;
}
	...
	qtd->length - QTD_LENGTH(token), qtd->length, qh->xacterrs);
			 ...
			 */
					...
					&& !(qtd->hw_alt_next
						& EHCI_LIST_END(ehci))) {
				/*
				 * Synopsys bug: The overlay's token is wrongly
				 * still active even after a short read has
				 * occurred. However, unfortunately, the qtd
				 * token becomes inactive and could be cleaned.
				 * The queue structure will be corrupted if the
				 * cleaning routine frees this qTD before the
				 * queue stops. It should be prevented that the
				 * active overlay token references and updates
				 * the qtd which has been removed and then
				 * resubmitted. Just wait for another short
				 * read to stop the queue.
				 */
				if (ehci->has_synopsys_shortread_bug &&
					state == QH_STATE_LINKED &&
					(hw->hw_token & ACTIVE_BIT(ehci))) {
					static unsigned violations;

					ehci_warn(ehci, "short read but overlay"
						" is still active: count %d\n",
							++violations);
					ehci_warn(ehci, "* overlay token %08x "
							"qtd token %08x\n",
							hw->hw_token, token);
					qh->shortread_but_active = 1;
					break;
				} else if (qh->shortread_but_active) {
					ehci_warn(ehci, "  overlay token %08x "
							"qtd token %08x\n",
							hw->hw_token, token);
				}
