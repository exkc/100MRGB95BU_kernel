...
#include "../pe/pe_hw_o18.h"
#include "../pe/reg/pe_reg_o18.h"
#endif

#include "cvd_module.h"
#include "cvd_hw_o18a0.h"
#include "de_cvd_reg.h"
#include "mip_reg_o18.h"
#include "cvd_hal.h"

#include "cvd_pe_o18a0/CVD_L_AV_NTSC_443_default_o18.h"
#include "cvd_pe_o18a0/CVD_L_AV_NTSC_M_default_o18.h"
#include "cvd_pe_o18a0/CVD_L_AV_PAL60_default_o18.h"
#include "cvd_pe_o18a0/CVD_L_AV_PALI_default_o18.h"
#include "cvd_pe_o18a0/CVD_L_AV_PALM_default_o18.h"
#include "cvd_pe_o18a0/CVD_L_AV_PALNc_default_o18.h"
#include "cvd_pe_o18a0/CVD_L_AV_SECAM_default_o18.h"
#include "cvd_pe_o18a0/CVD_L_RF_NTSC_M_default_o18.h"
#include "cvd_pe_o18a0/CVD_L_RF_PALI_default_o18.h"
#include "cvd_pe_o18a0/CVD_L_RF_PALM_default_o18.h"
#include "cvd_pe_o18a0/CVD_L_RF_PALNc_default_o18.h"
#include "cvd_pe_o18a0/CVD_L_RF_SECAM_default_o18.h"

#include "cvd_pe_o18a0/CVD_L_RF_NTSC_M_Black_Level_High.h"
#include "cvd_pe_o18a0/CVD_L_RF_NTSC_M_Black_Level_Low.h"
#include "cvd_pe_o18a0/CVD_L_AV_NTSC_M_Black_Level_High.h"
#include "cvd_pe_o18a0/CVD_L_AV_NTSC_M_Black_Level_Low.h"
/*----------------------------------------------------------------------------------------
...
#define O18_IRIS_CLAMPAGC_V2_CONTROL_WA_FOR_CVD_UNLOCK
...
/*----------------------------------------------------------------------------------------
  global Variables
  ----------------------------------------------------------------------------------------*/
UINT32 g_O18_CVD_HW_DRV_REV = 0x21021500;

CVD_COLOR_SYSTEM_ARRAY_3CS_T g_SetColorSystem_3CS_O18A0 =
{ 
	{
...
int CVD_O18Ax_Clamp_Current_Control(UINT8 value)
{
	...
	if(value <= 0x3)
	{
		...
		else
		{
			...
			*/
			...
			BMC_REG_O18A0_RdFL(bmc_ctr36);
			BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_iclp, value);
			BMC_REG_O18A0_WrFL(bmc_ctr36);
...
// TRUE : Power Down
// FALSE : Power Up
void CVD_O18Ax_Power_Down(LX_AFE_CVD_SELECT_T select_main_sub, BOOLEAN PowerOnOFF)
{
	if(select_main_sub == LX_CVD_MAIN)
	{
		...
		if(PowerOnOFF==FALSE) // from cvd_test.cmm
		{
			...
			//ACE_REG_O18A0_RdFL(main_pll_4);
			//gmain_pll_4.dr3p_pdb = 0;
			//ACE_REG_O18A0_WrFL(main_pll_4);
			...
			BMC_REG_O18A0_RdFL(bmc_ctr35);
			BMC_REG_O18A0_RdFL(bmc_ctr36);
			BMC_REG_O18A0_Wr01(bmc_ctr35, reg_videoafe_cvbs_pdbm, 0x1);
		// cvbs_pdb ON , in CVD_O18Ax_Channel_Power_Control()
		//	ACE_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_pdb, 0x1);
			BMC_REG_O18A0_WrFL(bmc_ctr35);
			BMC_REG_O18A0_WrFL(bmc_ctr36);

#ifdef M16P_BRINGUP_TODO
			ACE_REG_O18A0_RdFL(afe_vbuf_0);

			/* VDAC & BUFFER Power Down Setting */
			// CVBS BYPASS
			ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb1, 1);
			// VDAC to SCART
			ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb2, 1);

			ACE_REG_O18A0_WrFL(afe_vbuf_0);
#endif
		}
		else
		{
			...
			BMC_REG_O18A0_RdFL(bmc_ctr35);
			BMC_REG_O18A0_RdFL(bmc_ctr36);
			BMC_REG_O18A0_Wr01(bmc_ctr35, reg_videoafe_cvbs_pdbm, 0);
			BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_pdb, 0);
			//ACE_REG_O18A0_Wr01(main_pll_4, dr3p_pdb, 0);
			//ACE_REG_O18A0_WrFL(main_pll_4);
			BMC_REG_O18A0_WrFL(bmc_ctr35);
			BMC_REG_O18A0_WrFL(bmc_ctr36);
			...
			/* VDAC & BUFFER Power Down Setting */
//			ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb1, 0);
			ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb2, 0);
...
void CVD_O18Ax_Reg_Init(LX_AFE_CVD_SELECT_T select_main_sub)
{
	...
	CTOP_CTRL_O18A0_Wr01(CTRL_BMC, crg_bmc01, swrst_vdac, 0);

	CTOP_CTRL_O18A0_Wr01(CTRL_BMC, crg_bmc01, swrst_f54m, 0);
	CTOP_CTRL_O18A0_Wr01(CTRL_BMC, crg_bmc01, swrst_cvbs, 0);
	CTOP_CTRL_O18A0_Wr01(CTRL_BMC, crg_bmc01, swrst_f27m, 0);
	CTOP_CTRL_O18A0_WrFL(CTRL_BMC, crg_bmc01);

	CTOP_CTRL_O18A0_RdFL(CTRL_BMC, crg_bmc17);
	CTOP_CTRL_O18A0_Wr01(CTRL_BMC, crg_bmc17, sel_inv_f54m_clock, 1);
	CTOP_CTRL_O18A0_WrFL(CTRL_BMC, crg_bmc17);

	//M16+ register change
	BMC_REG_O18A0_RdFL(bmc_ctr26);
	BMC_REG_O18A0_Wr01(bmc_ctr26, reg_videoafe_cvbs_cksel, 1);
	BMC_REG_O18A0_WrFL(bmc_ctr26);

#ifdef M16P_BRINGUP_TODO
//	MIP_REG_O18A0_RdFL(bmc_ctr01);
	ACE_REG_O18A0_RdFL(data_bridge_0);
//	MIP_REG_O18A0_Wr01(bmc_ctr01, swrst_f24m, 0);
//	MIP_REG_O18A0_Wr01(bmc_ctr01, swrst_f24m_bb, 0);		// 151127 : release reset of f24m_bb
	ACE_REG_O18A0_Wr01(data_bridge_0, reg_cvbs_clock_rate, 1);
	// CVD_H13_BRINGUP : clock_control_1 to clock_control_1
//	MIP_REG_O18A0_WrFL(bmc_ctr01);
	ACE_REG_O18A0_WrFL(data_bridge_0);
#endif

	CTOP_CTRL_O18A0_RdFL(CTRL_BMC, crg_bmc17);
	CTOP_CTRL_O18A0_Wr01(CTRL_BMC, crg_bmc17, sel_inv_vdac_clock, 1);
	CTOP_CTRL_O18A0_WrFL(CTRL_BMC, crg_bmc17);
	...
	BMC_REG_O18A0_RdFL(bmc_ctr36);
	BMC_REG_O18A0_RdFL(bmc_ctr37);

	BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_lpf, 1);
	BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_bw, 0);
	BMC_REG_O18A0_Wr01(bmc_ctr37, reg_videoafe_cvbs_dcatt, 1);

	BMC_REG_O18A0_WrFL(bmc_ctr36);
	BMC_REG_O18A0_WrFL(bmc_ctr37);
...
//	CVD_O18Ax_Init_Level_Detector();
...
#ifdef M16P_BRINGUP_TODO
	ACE_REG_O18A0_RdFL(afe_cvbs_8);
	ACE_REG_O18A0_Wr01(afe_cvbs_8, cvbs_bt, 0x1);	//default 0x0 : 12bit
	ACE_REG_O18A0_WrFL(afe_cvbs_8);

	/* M16 SCART out currnet setting */
	ACE_REG_O18A0_RdFL(afe_vdac_0);
	ACE_REG_O18A0_Wr01(afe_vdac_0, vdc_icnt, 0x8);	//default 0x3
	ACE_REG_O18A0_WrFL(afe_vdac_0);
#endif

	/* M16 cvbs_iclp2 : 0x3 */
	BMC_REG_O18A0_RdFL(bmc_ctr36);
	BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_iclp2, 0x3);	//160120 : iclp2
	BMC_REG_O18A0_WrFL(bmc_ctr36);
	...
	BMC_REG_O18A0_RdFL(bmc_ctr35);
	BMC_REG_O18A0_Wr01(bmc_ctr35, reg_videoafe_cvbs_dcsel, 0x0);  //160919 : DC reference level 0.8V -> 1.0V
	BMC_REG_O18A0_WrFL(bmc_ctr35);

	/* For SECAM auto burst position */
	CVD_O18Ax_Set_Burst_Gate_12(0x32, 0x3a, 0x62, 0x6a);
...
int CVD_O18Ax_Set_Source_Type(LX_AFE_CVD_SET_INPUT_T	cvd_input_info)
{
	//120118 : first disable SCART FB at source change
	CVD_O18Ax_Set_Scart_FB_En(0);
	gEnableScartFB = 0;
		...
		*/
		...
		switch(cvd_input_info.cvbs_input_port)
		{
			case LX_AFE_CVBS_IN1:
				BMC_REG_O18A0_RdFL(bmc_ctr37);
				BMC_REG_O18A0_Wr01(bmc_ctr37, reg_videoafe_cvbs_insel, 0x0);
				BMC_REG_O18A0_Wr01(bmc_ctr37, reg_videoafe_cvbs_shield_on, 0x1);
				BMC_REG_O18A0_WrFL(bmc_ctr37);
				break;
			case LX_AFE_CVBS_IN2:
				BMC_REG_O18A0_RdFL(bmc_ctr37);
				BMC_REG_O18A0_Wr01(bmc_ctr37, reg_videoafe_cvbs_insel, 0x1);
				BMC_REG_O18A0_Wr01(bmc_ctr37, reg_videoafe_cvbs_shield_on, 0x2);
				BMC_REG_O18A0_WrFL(bmc_ctr37);
				break;
		...
		if (cvd_input_info.cvd_input_source_attribute != LX_CVD_INPUT_SOURCE_ATTRIBUTE_RF)
		{
			...
			CVD_O18Ax_Set_Internal_Demod_Mode(0);
			CVD_O18Ax_agc_half_en_control(0);	// to prevent random VLOCK!
		}
		else
		{
			CVD_DEBUG("RF Mode : set abb_mux_sel to 0\n");
			//ACE_REG_O18A0_RdFL(abb_02);
			if(g_CVD_Status_3CS.use_internal_demod)	// if using internal demod
				CVD_O18Ax_Set_Internal_Demod_Mode(1);
			else
				CVD_O18Ax_Set_Internal_Demod_Mode(0);
...
int CVD_E60Ax_Get_Scart_FB_En(void)
{
	int ret;
	CVD_Reg_RdFL(fastblank_001);
...
int CVD_O18Ax_Channel_Power_Control(UINT32 on_off)
{
	...
	if(on_off)	// power ON
	{
		...
		BMC_REG_O18A0_Rd01(bmc_ctr35, reg_videoafe_cvbs_pdbm, cvbs_pdbm_status);
		BMC_REG_O18A0_Rd01(bmc_ctr36, reg_videoafe_cvbs_pdb, cvbs_pdb_status);
		//ACE_REG_O18A0_Rd01(main_pll_4, dr3p_pdb, dr3p_pdb_status);
		BMC_REG_O18A0_Rd01(bmc_ctr36, reg_videoafe_cvbs_clp, cvbs_cp_status);
		...
		BMC_REG_O18A0_Wr01(bmc_ctr35, reg_videoafe_cvbs_pdbm, 0x1);
		BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_pdb, 0x1);
		//ACE_REG_O18A0_Wr01(main_pll_4, dr3p_pdb, 0x1);

		//added 110622 : by tommy.lee to disable clamp control on power down condition
		BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_clp, 1);
		//ACE_REG_O18A0_WrFL(main_pll_4);
		BMC_REG_O18A0_WrFL(bmc_ctr35);
		BMC_REG_O18A0_WrFL(bmc_ctr36);

#ifdef O18_IRIS_CLAMPAGC_V2_CONTROL_WA_FOR_CVD_UNLOCK
...
/*
		ACE_REG_O18A0_RdFL(soft_reset_5);
		ACE_REG_O18A0_Wr01(soft_reset_5, swrst_f54m, 0);
		ACE_REG_O18A0_Wr01(soft_reset_5, swrst_cvbs, 0);
		ACE_REG_O18A0_WrFL(soft_reset_5);
		*/

#ifdef CVD_3DCOMB_ERROR_WORKAROUND
		CVD_O18Ax_3DComb_Workaround(20); // enable  workaround function
#endif
	...
	{
#ifdef CVD_3DCOMB_ERROR_WORKAROUND
		CVD_O18Ax_3DComb_Workaround(10); // Memory Reset & bypass workaround function
#endif
		OS_MsecSleep(5);
		...
		ACE_REG_O18A0_Wr01(hslvdstx2_0, pdb3, 0);
		ACE_REG_O18A0_Wr01(hslvdstx2_1, ch_en3, 0);
		...
		/*
		   ACE_REG_O18A0_RdFL(soft_reset_5);
		   ACE_REG_O18A0_Wr01(soft_reset_5, swrst_f54m, 0x1);
		   ACE_REG_O18A0_Wr01(soft_reset_5, swrst_cvbs, 0x1);
		   ACE_REG_O18A0_WrFL(soft_reset_5);
		 */
		...
		//L18Ax Still have problem on DTV SCART out when cvbs_pdb is OFF
//		if (lx_chip_rev() >= LX_CHIP_REV(M14, B0))
//			ACE_REG_O18A0_Wr01(bmc_ctr36, cvbs_pdb, 0);
//
//		// M16+ B1 : Component Noise when CVBS PDB is On : 161013
		BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_pdb, 0);

		//added 110622 : by tommy.lee to disable clamp control on power down condition
		BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_clp, 0);
		BMC_REG_O18A0_WrFL(bmc_ctr36);
		...
		CVD_O18Ax_Set_comb2d_only(1);
		CVD_O18Ax_Set_comb2d_only_md(1);
#endif
		OS_MsecSleep(5);
...
int CVD_O18Ax_Bypass_Control(LX_AFE_CVD_BYPASS_CONTROL_T *cvd_bypass_control_t)
{
	...
	switch(cvd_bypass_control_t->buf_out_1_sel)
	{
		case CVD_BYPASS_DAC:
#ifdef M16_BRINGUP_TMP
			ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb2, 0x1);	//DAC PDB ON
			ACE_REG_O18A0_Wr01(afe_vbuf_3, vbuf_rclp_on, 0x0);
			ACE_REG_O18A0_Wr01(afe_vbuf_3, vbuf_clp_off, 0x1);	//BUF1 Clamp Off
#endif
#ifdef O18_BRINGUP
			BMC_REG_O18A0_RdFL(bmc_ctr36);
			BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_vbuf_sel, 0x0);
			BMC_REG_O18A0_WrFL(bmc_ctr36);
#endif
	//		ACE_REG_O18A0_Wr01(afe_vbuf_3, bufclp_vref, 0x0);

#ifdef O18_BRINGUP
			ACE_REG_O18A0_RdFL(abb_02);
			ACE_REG_O18A0_Wr01(abb_02, abb_mux_sel_vdac, 0);		// 0:DAC
			ACE_REG_O18A0_WrFL(abb_02);
#endif

#ifdef M16_BRINGUP_TMP
			CTOP_CTRL_M16A0_RdFL(RIGHT, ctr130);
			CTOP_CTRL_M16A0_Wr01(RIGHT, ctr130, scart_feedback_enb, 0x0);
			CTOP_CTRL_M16A0_WrFL(RIGHT, ctr130);
#endif
			break;
		...
		case CVD_BYPASS_CVBS_WITH_CLAMPING:
			...
			BMC_REG_O18A0_RdFL(bmc_ctr36);
#ifdef O18_BRINGUP
			BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_vbuf_sel, 0x1);
#endif
			BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_clp, 0);
			BMC_REG_O18A0_WrFL(bmc_ctr36);
			break;
...
#ifdef M16P_BRINGUP_TODO
			ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb2, 0x0);	//DAC PDB Off
//			ACE_REG_O18A0_Wr01(afe_vbuf_1, buf_pdb1, 0x1);	//SCART PDB On
			ACE_REG_O18A0_Wr01(afe_vbuf_3, vbuf_rclp_on, 0x0);
			ACE_REG_O18A0_Wr01(afe_vbuf_3, vbuf_clp_off, 0x0);
#endif
#ifdef O18_BRINGUP
			BMC_REG_O18A0_RdFL(bmc_ctr36);
			BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_vbuf_sel, 0x1);
...
#ifdef M16P_BRINGUP_TODO
			ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb2, 0x1);	//DAC PDB ON
//			ACE_REG_O18A0_Wr01(afe_vbuf_1, buf_pdb1, 0x0);	//SCART PDB OFF
			ACE_REG_O18A0_Wr01(afe_vbuf_3, vbuf_rclp_on, 0x0);
			ACE_REG_O18A0_Wr01(afe_vbuf_3, vbuf_clp_off, 0x1);	//BUF1 Clamp Off
#endif
#ifdef O18_BRINGUP
...
#ifdef O18_BRINGUP
			ACE_REG_O18A0_RdFL(abb_02);
			ACE_REG_O18A0_Wr01(abb_02, abb_mux_sel_vdac, 1);	
			ACE_REG_O18A0_Wr01(abb_02, abb_mux_rcsel_vdac, 0);
			ACE_REG_O18A0_WrFL(abb_02);
#endif
...
//			ACE_REG_O18A0_Wr01(afe_vbuf_1, buf_pdb1, 0x0);	//SCART PDB OFF
...
#ifdef O18_BRINGUP
			ACE_REG_O18A0_RdFL(abb_02);
			ACE_REG_O18A0_Wr01(abb_02, abb_mux_rcsel_vdac, 1);
			ACE_REG_O18A0_WrFL(abb_02);
#endif
#ifdef M16_BRINGUP_TMP
			CTOP_CTRL_M16A0_RdFL(RIGHT, ctr130);
			CTOP_CTRL_M16A0_Wr01(RIGHT, ctr130, scart_feedback_enb, 0x1);
			CTOP_CTRL_M16A0_WrFL(RIGHT, ctr130);
#endif
			break;
	...
	if(cvd_bypass_control_t->cvbs_source_sel == CVD_BYPASS_CVBS_IN1)
		BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_ycm, 0x1);
	else
		BMC_REG_O18A0_Wr01(bmc_ctr36, reg_videoafe_cvbs_ycm, 0x0);
...
int CVD_O18Ax_OnOff_VDAC(BOOLEAN bonoff)
{
	...
	{
#ifdef M16P_BRINGUP_TODO
		CTOP_CTRL_M16A0_RdFL(CVI_SYN, crg_cvi02);	//M16 CTOP Register
		ACE_REG_O18A0_RdFL(afe_vbuf_0);
		BMC_REG_O18A0_RdFL(bmc_ctr36);

		ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb1, 0x1);	// CVBS Bypass
		ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb2, 1);	// VDAC to SCART
		ACE_REG_O18A0_Wr01(afe_vbuf_3, vdac_pdb, 0x1);
		CTOP_CTRL_M16A0_Wr01(CVI_SYN, crg_cvi02, swrst_sub_cvi_cve_cvbs, 0x0);	//De-Assert Reset	//M16 CTOP Register

		ACE_REG_O18A0_WrFL(afe_vbuf_3);
		ACE_REG_O18A0_WrFL(afe_vbuf_0);
		CTOP_CTRL_M16A0_WrFL(CVI_SYN, crg_cvi02);

#endif
		/*
		CVD_O18Ax_VDAC_mute_control(1);
		OS_MsecSleep(5);
		CVD_O18Ax_VDAC_mute_control(0);
		*/
		CTOP_CTRL_O18A0_RdFL(CTRL_BMC, crg_bmc01);
		CTOP_CTRL_O18A0_Wr01(CTRL_BMC, crg_bmc01, swrst_vdac, 0x1);
		CTOP_CTRL_O18A0_WrFL(CTRL_BMC, crg_bmc01);

		OS_MsecSleep(5);

		CTOP_CTRL_O18A0_RdFL(CTRL_BMC, crg_bmc01);
		CTOP_CTRL_O18A0_Wr01(CTRL_BMC, crg_bmc01, swrst_vdac, 0x0);
		CTOP_CTRL_O18A0_WrFL(CTRL_BMC, crg_bmc01);
	}
	else	// power off
	{
		...
		ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb1, 0x0);	//CVBS Bypass
		ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb2, 0);	// VDAC to SCART
		ACE_REG_O18A0_Wr01(afe_vbuf_3, vdac_pdb, 0x0);
		CTOP_CTRL_M16A0_Wr01(CVI_SYN, crg_cvi02, swrst_sub_cvi_cve_cvbs, 0x1);	//Assert Reset	//M16 CTOP Register
...
int CVD_O18Ax_VDAC_mute_control(int enable)
{
...
#ifdef M16P_BRINGUP_TODO
		ACE_REG_O18A0_RdFL(afe_vbuf_0);
		ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb1, 0x0);
		ACE_REG_O18A0_WrFL(afe_vbuf_0);
#endif
...
#ifdef M16P_BRINGUP_TODO
		ACE_REG_O18A0_RdFL(afe_vbuf_0);
		ACE_REG_O18A0_Wr01(afe_vbuf_0, vbuf_pdb1, 0x1);
		ACE_REG_O18A0_WrFL(afe_vbuf_0);
#endif
...
int CVD_O18Ax_Get_PE0_Motion_Value(int *p_tnr_x_avg_t, int *p_tnr_x_avg_s, int *p_tpd_s_status)
{

#ifdef INCLUDE_KDRV_PE
	/* w_apl_motion <= ma_motion <= X_Avg_T, w_apl_val <= ma_var <= X_Avg_S */
	/* s_status <= TPD_S : to be changed */
	PE_ND0_O18_QRd01(tnr_status_01,w_apl_motion,*p_tnr_x_avg_t);
	PE_ND0_O18_QRd01(tnr_status_01,w_apl_var,*p_tnr_x_avg_s);
	PE_CE_PE1_O18_QRd01(pe1_fsw_ctrl_00,reg_s_status_0,*p_tpd_s_status);
#endif

	return 0;
}

int CVD_O18Ax_Get_AFE_CVBS_status(UINT8 *cvbs_icon_value, UINT8 *cvbs_pdb_value, UINT8 *cvbs_cp_value, UINT8 *cvbs_insel_value, UINT8 *cvbs_pdbm_value)
{
	BMC_REG_O18A0_RdFL(bmc_ctr35);
	BMC_REG_O18A0_RdFL(bmc_ctr36);
	BMC_REG_O18A0_RdFL(bmc_ctr37);

	BMC_REG_O18A0_Rd01(bmc_ctr36, reg_videoafe_cvbs_iclp, *cvbs_icon_value);
	BMC_REG_O18A0_Rd01(bmc_ctr36, reg_videoafe_cvbs_pdb, *cvbs_pdb_value);
	BMC_REG_O18A0_Rd01(bmc_ctr36, reg_videoafe_cvbs_clp, *cvbs_cp_value);
	BMC_REG_O18A0_Rd01(bmc_ctr37, reg_videoafe_cvbs_insel, *cvbs_insel_value);
	BMC_REG_O18A0_Rd01(bmc_ctr35, reg_videoafe_cvbs_pdbm, *cvbs_pdbm_value);

	return 0;
}

int CVD_O18Ax_Get_AFE_VBUF_status(UINT8 *buf_ycm_value, UINT8 *buf_sel1_value, UINT8 *buf_pdb1_value, UINT8 *vdac_pdb_value)
{
#ifdef O18_BRINGUP
	BMC_REG_O18A0_RdFL(bmc_ctr36);

	BMC_REG_O18A0_Rd01(bmc_ctr36, reg_videoafe_cvbs_ycm, *buf_ycm_value);
	BMC_REG_O18A0_Rd01(bmc_ctr36, reg_videoafe_vbuf_sel, *buf_sel1_value);
#endif
	*buf_pdb1_value = 0;
	*vdac_pdb_value = 0;
...
int CVD_O18Ax_Set_Internal_Demod_Mode(BOOLEAN bInternal_Demod)
{
	...
	{	
		...
		//CVD_O18Ax_Set_Cpump_Auto_Stip_Mode(3);
		...
		CTOP_CTRL_O18A0_Wr01(CTRL_BMC, crg_bmc19, sel_cvbs, 0);		// 0:ABB
		CTOP_CTRL_O18A0_WrFL(CTRL_BMC, crg_bmc19);
	}
	else
	{
		//CVD_O18Ax_Set_Cpump_Auto_Stip_Mode(2);
		...
		CTOP_CTRL_O18A0_RdFL(CTRL_BMC, crg_bmc19);
		CTOP_CTRL_O18A0_Wr01(CTRL_BMC, crg_bmc19, sel_cvbs, 1);		// 1:CVBS
		CTOP_CTRL_O18A0_WrFL(CTRL_BMC, crg_bmc19);
	}
...
int CVD_O18Ax_OnOff_DMD_DAC(BOOLEAN bonoff)
{
...
#ifdef INCLUDE_KDRV_PE
#define CVD_O18Ax_CHK_DFLT_SIZE(_x)		\
	if (PE_DDR_O18_CVD_DFLT_SIZE != (sizeof(_x)/sizeof(CVD_REG_PARAM_T)))\
	{AFE_ERROR("%s size is wrong\n", #_x); ret=RET_ERROR;break;}

int CVD_O18Ax_SetPqModeToDdr(volatile UINT32 *p_ddr_reg)
{
	int ret = RET_OK;
	UINT32 i;
	UINT32 base_dflt[PE_DDR_O18_CVD_DFLT_ITEM_NUM];
	do{
		if (!p_ddr_reg)	{ AFE_ERROR("p_ddr_reg is Null\n"); ret=RET_ERROR;break; }
		CVD_O18Ax_CHK_DFLT_SIZE(cvd_av_ntsc_m_l_default_o18);
		CVD_O18Ax_CHK_DFLT_SIZE(cvd_rf_ntsc_m_l_default_o18);
		CVD_O18Ax_CHK_DFLT_SIZE(cvd_av_pali_l_default_o18);
		CVD_O18Ax_CHK_DFLT_SIZE(cvd_rf_pali_l_default_o18);
		CVD_O18Ax_CHK_DFLT_SIZE(cvd_av_secam_l_default_o18);
		CVD_O18Ax_CHK_DFLT_SIZE(cvd_rf_secam_l_default_o18);
		CVD_O18Ax_CHK_DFLT_SIZE(cvd_av_ntsc_443_l_default_o18);
		CVD_O18Ax_CHK_DFLT_SIZE(cvd_av_pal60_l_default_o18);
		CVD_O18Ax_CHK_DFLT_SIZE(cvd_av_palm_l_default_o18);
		CVD_O18Ax_CHK_DFLT_SIZE(cvd_rf_palm_l_default_o18);
		CVD_O18Ax_CHK_DFLT_SIZE(cvd_av_palnc_l_default_o18);
		CVD_O18Ax_CHK_DFLT_SIZE(cvd_rf_palnc_l_default_o18);
		base_dflt[0] = 0;
		for (i=1; i<PE_DDR_O18_CVD_DFLT_ITEM_NUM; i++)
		{
			base_dflt[i] = base_dflt[i-1]+PE_DDR_O18_CVD_DFLT_SIZE;
		}
		for (i=0; i<PE_DDR_O18_CVD_DFLT_SIZE; i++)
		{
			p_ddr_reg[base_dflt[0]+i] = cvd_av_ntsc_m_l_default_o18[i].cvd_pe_value;
			p_ddr_reg[base_dflt[1]+i] = cvd_rf_ntsc_m_l_default_o18[i].cvd_pe_value;
			p_ddr_reg[base_dflt[2]+i] = cvd_av_pali_l_default_o18[i].cvd_pe_value;
			p_ddr_reg[base_dflt[3]+i] = cvd_rf_pali_l_default_o18[i].cvd_pe_value;
			p_ddr_reg[base_dflt[4]+i] = cvd_av_secam_l_default_o18[i].cvd_pe_value;
			p_ddr_reg[base_dflt[5]+i] = cvd_rf_secam_l_default_o18[i].cvd_pe_value;
			p_ddr_reg[base_dflt[6]+i] = cvd_av_ntsc_443_l_default_o18[i].cvd_pe_value;
			p_ddr_reg[base_dflt[7]+i] = cvd_av_pal60_l_default_o18[i].cvd_pe_value;
			p_ddr_reg[base_dflt[8]+i]  = cvd_av_palm_l_default_o18[i].cvd_pe_value;
			p_ddr_reg[base_dflt[9]+i]  = cvd_rf_palm_l_default_o18[i].cvd_pe_value;
			p_ddr_reg[base_dflt[10]+i] = cvd_av_palnc_l_default_o18[i].cvd_pe_value;
			p_ddr_reg[base_dflt[11]+i] = cvd_rf_palnc_l_default_o18[i].cvd_pe_value;
		}
	}while (0);
...
int CVD_O18Ax_Set_PQ_Mode(LX_AFE_CVD_PQ_MODE_T cvd_pq_mode)
{
	...
	switch(cvd_pq_mode)
	{
		case LX_CVD_PQ_NTSC_M:		// AV NTSC
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_ntsc_m_l_default_o18;
			size	= sizeof(cvd_av_ntsc_m_l_default_o18) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_NTSC_M_RF:	// RF NTSC
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_ntsc_m_l_default_o18;
			size	= sizeof(cvd_rf_ntsc_m_l_default_o18) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL:			// AV PAL
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_pali_l_default_o18;
			size	= sizeof(cvd_av_pali_l_default_o18) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL_RF:		// RF PAL
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_pali_l_default_o18;
			size	= sizeof(cvd_rf_pali_l_default_o18) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_SECAM:		// AV SECAM
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_secam_l_default_o18;
			size	= sizeof(cvd_av_secam_l_default_o18) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_SECAM_RF:	// RF SECAM
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_secam_l_default_o18;
			size	= sizeof(cvd_rf_secam_l_default_o18) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_NTSC_443:		// AV NTSC 443
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_ntsc_443_l_default_o18;
			size	= sizeof(cvd_av_ntsc_443_l_default_o18) / sizeof(CVD_REG_PARAM_T) ;
			break;
		...
		case LX_CVD_PQ_PAL_60:			// AV PAL 60
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_pal60_l_default_o18;
			size	= sizeof(cvd_av_pal60_l_default_o18) / sizeof(CVD_REG_PARAM_T) ;
			break;
		...
		case LX_CVD_PQ_PAL_M:		// AV PAL M
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_palm_l_default_o18;
			size	= sizeof(cvd_av_palm_l_default_o18) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL_M_RF:	// RF PAL M
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_palm_l_default_o18;
			size	= sizeof(cvd_rf_palm_l_default_o18) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL_NC:		// AV PAL NC
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_palnc_l_default_o18;
			size	= sizeof(cvd_av_palnc_l_default_o18) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL_NC_RF:	// RF PAL NC
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_palnc_l_default_o18;
			size	= sizeof(cvd_rf_palnc_l_default_o18) / sizeof(CVD_REG_PARAM_T) ;
			break;
		default:
	...
	if ( ( gEnableTTXTestPatch_O18A0 == 1) && (gStatusTTXTestPatch_O18A0 == 1) )
	{
		CVD_DEBUG("!!! TTX Test Mode Disabled !!!\n");

		gStatusTTXTestPatch_O18A0 = 0;
...
int CVD_O18Ax_Black_Level_Control(LX_AFE_CVD_BLACK_LEVEL_T black_level, LX_AFE_CVD_SOURCE_ATTRIBUTE_T rf_av_mode)
{
