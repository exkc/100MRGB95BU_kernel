...
static  BOOLEAN g_pal_init_2 = FALSE;   /* palette_1 HW initialized ? */
static  UINT32  g_pal_data_0[256] = {[255] = 0x00020202};
static  UINT32  g_pal_data_1[256] = {[255] = 0x00020202};
static  UINT32  g_pal_data_2[256] = {[255] = 0x00020202};
#endif

// both GFX_ENUM_PIXEL_FORMAT_T and LX_GFX_PIXEL_FORMAT_T has the sample value
// because it's value is baed on HW register. To prevent compile warnings, make the simple
// type conversion inline function.
static inline GFX_ENUM_PIXEL_FORMAT_T GFX_ConvUserFmt2HWFmt(LX_GFX_PIXEL_FORMAT_T userFmt)
{
    return (GFX_ENUM_PIXEL_FORMAT_T)userFmt;
}
...
int GFX_DownloadPalette(int port_num, int size, UINT32* data)
{
    ...
    switch (port_num)
    {
        ...
        case 2:
        {
            ...
            if (g_pal_init_2 && !memcmp(g_pal_data_2, data, sizeof(UINT32)*size))
            {
                GFX_PRINT("<gfx> same palette 2 .. ignore\n"); goto func_exit;
            }

            memcpy(g_pal_data_2, data, sizeof(UINT32)*size);
            g_pal_init_2 = TRUE;
#endif
            cmd_op.sPort = eReadThreePorts;
            cmd_op.bUpdateCLUT = 0x0;       /* raxis.lim (2010/07/01) -- bugf fix . write should be zero */
        }
        break;
...
int GFX_RunBlendOp(LX_GFX_MANUAL_BLEND_CTRL_PARAM_T* param, UINT32 cmd_delay)
{
    ...
    src0_surface = GFX_GetSurfaceObject(param->src0.surface_fd);
    src1_surface = GFX_GetSurfaceObject(param->src1.surface_fd);
    src2_surface = GFX_GetSurfaceObject(param->src2.surface_fd);
    dst_surface  = GFX_GetSurfaceObject(param->dst.surface_fd);

    GFX_DEBUG("src0.surf fd=0x%x, obj=%p", param->src0.surface_fd, src0_surface);
    GFX_DEBUG("src1.surf fd=0x%x, obj=%p", param->src1.surface_fd, src1_surface);
    GFX_DEBUG("src2.surf fd=0x%x, obj=%p", param->src2.surface_fd, src2_surface);
    GFX_DEBUG(" dst.surf fd=0x%x, obj=%p", param->dst.surface_fd,  dst_surface);
...
#ifdef GFX_USE_LG_FBDC

    if (dst_surface && 0xFBDC == dst_surface->surf.pixel_format)
    {
        oport->compressed = 1;
        oport->header_offset = _get_header_size_fbdc(
                                   param->dst.rect.w,
                                   param->dst.rect.h
                               );
#if defined(FBDC_REVERSED_ENC)
        iport0->reversed = 1;
        iport1->reversed = 1;
        iport2->reversed = 1;
#endif
    ...
    if (src0_surface && 0xFBDC == src0_surface->surf.pixel_format)
    {
        iport0->compressed = 1;
        iport0->header_offset = _get_header_size_fbdc(
                                    param->src0.rect.w,
                                    param->src0.rect.h
                                );
#if defined(FBDC_REVERSED_ENC)
    ...
    if (src1_surface && 0xFBDC == src1_surface->surf.pixel_format)
    {
        iport1->compressed = 1;
        iport1->header_offset = _get_header_size_fbdc(
                                    param->src1.rect.w,
                                    param->src1.rect.h
                                );
#if defined(FBDC_REVERSED_ENC)
    ...
    if (src2_surface && 0xFBDC == src2_surface->surf.pixel_format)
    {
        iport2->compressed = 1;
        iport2->header_offset = _get_header_size_fbdc(
                                    param->src2.rect.w,
                                    param->src2.rect.h
                                );
#if defined(FBDC_REVERSED_ENC)
    ...
    {
        /* if scaler is in wr & scaler mux was enalbed then ... :
            (2016/09/08 by hwlee)
        */
        if (!(param->scaler.mode & 0xf0))
        {
    ...
    *   raxis.lim (2010/05/28)
            ...
            param->op_mode == LX_GFX_OP_MODE_PACK_Cb_Cr)
    {
        ...
        iport2->usHeight = param->src2.rect.h;

        if (param->src2.surface_type == LX_GFX_SURFACE_TYPE_COLOR)
        {
            iport2->usStride        = 0x0;
            iport2->ulAddr          = 0x0;
            iport2->sFmt            = GFX_ConvUserFmt2HWFmt(LX_GFX_PIXEL_FORMAT_INDEX_0);
            iport2->uiGlobalAlpha   = param->src2.surface_color.pixel_value;
        }
        else
        {
            GFX_CHECK_ERROR(!src2_surface, goto func_exit, "null surface\n");

            iport2->usStride        = (UINT16)src2_surface->surf.stride;
            iport2->ulAddr          = GFX_GetPortBaseAddr(src2_surface, &param->src2.rect);
            iport2->sFmt            = GFX_ConvUserFmt2HWFmt(src2_surface->surf.pixel_format);
            iport2->uiGlobalAlpha   = 0xffffffff;

            /* download palette if surface is index color format */
            if (src2_surface->surf.pixel_format <= LX_GFX_PIXEL_FORMAT_INDEX_8)
            {
                if (src2_surface->pal)
                {
                    GFX_DownloadPalette(2, src2_surface->PalSize, src2_surface->pal);
                }
            }
        }
    }
     ...
     **/
    {
        ...
        GFX_CHECK_ERROR(!dst_surface, goto func_exit, "null surface\n");
        ...
        if (param->op_mode == LX_GFX_OP_MODE_WRITE)
        {
            oport->sFmt     = GFX_ConvUserFmt2HWFmt(LX_GFX_PIXEL_FORMAT_INDEX_0);
            oport->ulGAlpha = param->src0.surface_color.pixel_value;
        }
        else
        {
            oport->sFmt     = GFX_ConvUserFmt2HWFmt(dst_surface->surf.pixel_format);
    ...
    if (0xFBDC == dst_surface->surf.pixel_format)
    {
        /* The usHSize should be a four align at FBDC encoding */
#if 1 // TODO: check wr_size & scaler for FBDC...
        if (LX_GFX_SCALER_MODE_NONE == param->scaler.mode)
#else
        if (LX_GFX_SCALER_MODE_NONE == param->scaler.mode ||
                !!(param->scaler.mode & 0xf0))
#endif
        {
            oport->usHSize = ALIGN(oport->usHSize, 4);
        }
    }
    ...
    else
    {
        ...
        switch ((param->scaler.mode & 0xf0))
        {
            case LX_GFX_SCALER_SELECT_WR:
                scaler->sc_insel = 0x3;
                break;

            case LX_GFX_SCALER_SELECT_R0:
                scaler->sc_insel = 0x0;
                break;

            case LX_GFX_SCALER_SELECT_R1:
                scaler->sc_insel = 0x1;
                break;

            case LX_GFX_SCALER_SELECT_R2:
                scaler->sc_insel = 0x2;
                break;

            default:
                scaler->cmd = GFX_SCALER_CMD_BYPASS;
                printk("invalid scaler.mode (=0x%x), sclaer off by force \n",
                       param->scaler.mode);
                break;
        }
...
int GFX_WaitSyncCommand(UINT32 msec_tm)
{
    ...
    UINT64 tick0;
    long diff_tick;
    long timeout = msecs_to_jiffies(msec_tm);

    while (1)
    {
        tick0 = OS_GetMsecTicks();
        rc = wait_event_interruptible_timeout(g_gfx_wq_done, GFX_IsGfxIdle(), timeout);

        if (rc >= 0) break;

        // handle -ERESTARTSYS;
        GFX_NOTI("interrupted by signal. rc=%d. retry\n", rc);

        diff_tick = OS_GetMsecTicks() - tick0;

        timeout -= msecs_to_jiffies(diff_tick);
    }
...
int GFX_RunResume(void)
{
    ...
    memset(g_pal_data_2, 0x0, sizeof(UINT32) * 256);
#endif
