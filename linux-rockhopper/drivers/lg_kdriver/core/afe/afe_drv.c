...
extern int v4l2_avd_device_init(void);
/*----------------------------------------------------------------------------------------
  External Variables
  ----------------------------------------------------------------------------------------*/
...
int		g_afe_wakeup = 0;
LX_AFE_EVENT_T		g_event_type = LX_AFE_EVENT_NONE;
int	g_otp_data_wait_condition = 0;
/*----------------------------------------------------------------------------------------
...
#ifdef CONFIG_COMPAT
static long     AFE_CompatIoctl (struct file * filp, unsigned int cmd, unsigned long arg);
#endif
#endif
...
DECLARE_WAIT_QUEUE_HEAD(WaitQueue_AFE_Event);
DECLARE_WAIT_QUEUE_HEAD(WaitQueue_AFE_OTP_Data);

//static unsigned int AFE_OTP_Data_In(void);
/*----------------------------------------------------------------------------------------
  Static Variables
  ----------------------------------------------------------------------------------------*/
...
static DEFINE_MUTEX(g_CVD_Sema);
...
static void  AFE_remove(struct platform_device *pdev)
#else
...
int AFE_Init(void)
{
	...
	{
		DBG_PRINT_ERROR("out of memory. can't allocate %zd bytes\n", sizeof(AFE_DEVICE_T)* AFE_MAX_DEVICE );
		return -ENOMEM;
	}
	...
	ADC_DEBUG("afe device initialized : 20240307\n");
 ...
 *
 */
static int AFE_Open(struct inode *inode, struct file *filp)
{
	int					major,minor;
	struct cdev*    	cdev;
	AFE_DEVICE_T*	my_dev;
 ...
 *
 */
static int AFE_Close(struct inode *inode, struct file *file)
{
	int					major,minor;
	AFE_DEVICE_T*	my_dev;
	struct cdev*		cdev;
	...
	// closing afe module without uninit CVD
	//CVD_UnInit();
 ...
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
static int AFE_Ioctl ( struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg )
#else
static long AFE_Ioctl (struct file * filp, unsigned int cmd, unsigned long arg)
#endif
{
	...
	switch(cmd)
	{
		...
		case AFE_IOR_CVD_GET_STATES:
			{
				...
				if (copy_from_user((void*)&cvd_states_info, (void __user *)arg, sizeof(LX_AFE_CVD_STATES_INFO_T)))
				{
					return -1;
				}
				//			AFE_PRINT("CVD GET STATE\n");
				ret = CVD_Get_States(&cvd_states_info);
				if (copy_to_user((void __user*)arg, (void*)&cvd_states_info, sizeof(LX_AFE_CVD_STATES_INFO_T)))
				{
					return -1;
				}
			}
		...
		case AFE_IOW_SET_ANALOG_COLOR_SYSTEM:
			{
				...
				if (copy_from_user((void*)&system_info, (void __user *)arg, sizeof(LX_AFE_VIDEO_SYSTEM_INFO_T)))
				{
					return -1;
				}

				ret = CVD_Set_Color_System(system_info);
			}
			break;
		...
		case AFE_IOR_CVD_GET_TIMING_INFO:
			{
				...
				if (copy_from_user((void*)&timing_info, (void __user *)arg, sizeof(LX_AFE_CVD_TIMING_INFO_T)))
				{
					return -EFAULT;
				}
				//AFE_TRACE("CVD Get Timing Info\n");
				CVD_Get_Timing_Info(&timing_info);

				if (copy_to_user((void __user*)arg, (void*)&timing_info, sizeof(LX_AFE_CVD_TIMING_INFO_T)))
				{
					return -EFAULT;
				}
				ret=0;
		...
		case AFE_IOR_CVD_GET_STATES_DETAIL:
			{
				...
				ret = CVD_Get_States_Detail(&cvd_states_detail_t);

				if (copy_to_user((void __user*)arg, (void*)&cvd_states_detail_t, sizeof(LX_AFE_CVD_STATES_DETAIL_T)))
				{
					return -1;
				}
		...
		case AFE_IOR_GET_EVENT:
			{
				if( copy_to_user((void *)arg, (void *)&g_event_type, sizeof(LX_AFE_EVENT_T)))
					return -EFAULT;
			}

			break;

		case AFE_IOR_GET_OPEN_COUNT:
			{
				if( copy_to_user((void *)arg, (void *)&my_dev->dev_open_count, sizeof(UINT32)))
					return -EFAULT;
			}
...
void CVD_LockDevice(void)
{
	mutex_lock(&g_CVD_Sema);
}

void CVD_UnlockDevice(void)
{
	mutex_unlock(&g_CVD_Sema);
}
///////////////////////////////////////////////////////////////////////////////////////////////////
