...
void fat_set_state(struct super_block *sb, unsigned int set,
		unsigned int force, const char *fname)
{
	...
	if (sbi->dirty) {
		fat_msg_ratelimit(sb, KERN_INFO,
				"Volume dirty already set before mount");
		return;
	}

	/* if volume already set, return here */
	if (sbi->dirty_flag == set)
		return;
	...
	fat_msg(sb, KERN_INFO, "Set volume %s: %s",
			set ? "dirty" : "clean", fname);
}
