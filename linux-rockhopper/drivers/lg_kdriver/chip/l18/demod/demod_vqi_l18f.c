...
unsigned int gL18FGBB_PowerOnInitStartTimeISDBT = 0;


/***********************************************************************************
	Static Variable & Function Prototypes Declarations
 ************************************************************************************/
UINT16	_gL18FGBB_VQI_UnlockCount_full_packeterr = 0;
UINT16 	_gL18FGBB_FullVQIPkerrCount = 7;
UINT16 	_gL18FGBB_FullVQPkerrThreshold = 300;
UINT16 	_gL18FGBB_FullISDBTPkerrThreshold = 200;

BOOLEAN _gL18FGBB_ISDBT_Mem_set = FALSE;


static int _DEMOD_L18F_LGDBB_VSB_CheckSignalState(DEMOD_CTX_T *pDemodInfo);
...
int DEMOD_L18F_ATSC_SW_Init(void)
{
	...
	{
		printk("[L18F LGDBB] (%s:%d) DEMOD_L2_ATSC_SW_Init  Fail!! \n", __F__, __L__);
...
int _DEMOD_L18F_LGDBB_VSB_CheckSignalState(DEMOD_CTX_T *pDemodInfo)
{
...
int _DEMOD_L18F_VSB_CheckTuneMode(DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_L18F_LGDBB_VSB_SetDemod(DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context 	*pFrontend;
	UINT8	new_tuneMode;
	UINT32 	freq_Khz;
	...
	printk("[L18F LGDBB][0716] ################### chungiii SetDemod VSB \n");

	new_tuneMode = _DEMOD_L18F_VSB_CheckTuneMode(pDemodInfo);

	if(RET_OK != DEMOD_L18F_Get_BW(pDemodInfo, &ter_bandwidth_KHz))
	{
		DEMOD_PRINT("[DEMOD L18F DBB] FAIL (%s:%d) DEMOD_L18F_Get_BW !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}
	 //printk("[L18F LGDBB] ################### chungiii SetDemod VSB check ter_bandwidth_KHz[%d] \n", ter_bandwidth_KHz);
...
************************************************************************************/
/**
 * DEMOD_L18F_VSB_Obtaining_Signal_Lock.
...
int DEMOD_L18F_VSB_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_L18F_LGDBB_VSB_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD L18F LGDBB] FAIL (%s:%d) DEMOD_L18F_LGDBB_VSB_SetDemod() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.vsb.tuneMode)
				{
					...
					{
						DEMOD_PRINT("[DEMOD L18F GBB] FAIL (%s:%d) DEMOD_M14B0_NeverlockScan() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				...
				{
					if (RET_OK != DEMOD_L18F_TPOutEnable(TRUE))
					{
						DEMOD_PRINT( "^r^[DEMOD L18F GBB] FAIL (%s:%d) DEMOD_L18F_TPOutEnable(FALSE) !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					pDemodInfo->scanJobFlag = LX_DEMOD_FLAG_FINISH;
					pDemodInfo->controlState = LX_DEMOD_SIGNAL_STABLE;
					DEMOD_PRINT( "^R^[DEMOD L18F GBB] LX_DEMOD_TUNE_START : Going to LX_DEMOD_SIGNAL_STABLE for SYNC CHECK DIRECTLY... ^0^\n");
				}
		...
		case LX_DEMOD_TUNE_SCAN:			/***************************************************/
			{
				...
				if((LX_DEMOD_FLAG_FINISH == scanProcessing) && (LX_DEMOD_LOCKED == scanLock))
				{
					...
					DEMOD_PRINT_ALWAYS( "^R^[DEMOD L18F GBB] SCANNING !!! FIRST LOCK! : Going to LX_DEMOD_SIGNAL_STABLE..... ^0^\n");
				}
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				{
					if(RET_OK != DEMOD_L18F_Get_Lock(LX_DEMOD_CARR_LOCK, &lockState))
					{
						DEMOD_PRINT("[DEMOD L18F GBB] FAIL (%s:%d) DEMOD_L18F_Get_Lock() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
					...
					if(LX_DEMOD_LOCKED == lockState)
					{
						DEMOD_PRINT( "^R^[DEMOD L18F GBB] LX_DEMOD_SYNC_CHECKING : catch only carrier lock..... ^0^\n");
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE: 		/********************************************/
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.vsb.tuneMode)
					{
						...
						{
							DEMOD_PRINT("[DEMOD L18F GBB] FAIL (%s:%d) DEMOD_L18F_Get_Packet_Error() !!!\n", __F__, __L__);
							DEMOD_RETURN_FAIL(__F__);
						}
						else
						{
							...
							DEMOD_PRINT_ALWAYS( "[DEMOD L18F GBB] LX_DEMOD_SIGNAL_STABLE[0716] : MSG_ATSC_FRONT_END_LOCKED..... ^^^0^^^\n");
							...
							if (RET_OK != _DEMOD_L18F_LGDBB_VSB_CheckSignalState(pDemodInfo))
							{
								DEMOD_PRINT( "^r^[DEMOD L18F GBB] FAIL (%s:%d) DEMOD_Get_signal_status() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
					...
					else
					{
						...
						DEMOD_PRINT_ALWAYS( "[DEMOD L18F GBB] LX_DEMOD_SIGNAL_STABLE : MSG_ATSC_FRONT_END_LOCKED..... ^0^\n");
						DEMOD_PRINT_ALWAYS( "[DEMOD L18F GBB] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^0^\n", elapsedTime);
...
************************************************************************************/
/**
 * DEMOD_L18F_VSB_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_L18F_VSB_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
...
#if 1
	if(RET_OK != DEMOD_L18F_VSB_Monitor(pDemodInfo))
	{
		DEMOD_PRINT("[DEMOD L18F GBB] FAIL (%s:%d) DEMOD_L18F_VSB_Monitor() !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}
#endif
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if(pDemodInfo->bIsTPoutEn == TRUE)
		{
			if (RET_OK != DEMOD_L18F_TPOutEnable(TRUE))
			{
				DEMOD_PRINT( "^r^[DEMOD L18F GBB] FAIL (%s:%d) DEMOD_L18F_TPOutEnable(TRUE) !!!\n", __F__, __L__);
				DEMOD_RETURN_FAIL(__F__);
			}
		}
		...
		if(pDemodInfo->lockCount == 2)
		{
			...
			DEMOD_PRINT_ALWAYS( "[DEMOD L18F GBB] MONITORING FUNC : MSG_ATSC_FRONT_END_LOCKED..... ^0^\n");
	...
	else
	{
		...
		if((LX_DEMOD_UNLOCKED != pDemodInfo->lockStatus)  && (TRUE != pDemodInfo->bNotifiedUnlock))
		{
			...
			if(pDemodInfo->unLockCount == 4)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD L18F GBB] MONITORING FUNC : MSG_ATSC_FRONT_END_NOT_LOCKED..... -_-\n");
...
int _DEMOD_L18F_LGDBB_QAM_CheckSignalState(DEMOD_CTX_T *pDemodInfo)
{
...
int _DEMOD_L18F_QAM_CheckTuneMode(DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_L18F_LGDBB_QAM_SetDemod(DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context 	*pFrontend;
	UINT8	new_tuneMode;
	UINT32 	freq_Khz;
	...
	freq_Khz = 0;
	 ...
	 printk("[L18F LGDBB][0716] ################### chungiii SetDemod QAM \n");

	new_tuneMode = _DEMOD_L18F_QAM_CheckTuneMode(pDemodInfo);
	...
	// printk("[L18F LGDBB] ################### chungiii SetDemod QAM check ter_bandwidth_KHz[%d] \n", ter_bandwidth_KHz);
...
************************************************************************************/
/**
 * DEMOD_L18F_QAM_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_L18F_QAM_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_L18F_LGDBB_QAM_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD L18F LGDBB] FAIL (%s:%d) DEMOD_L18F_LGDBB_QAM_SetDemod() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.qam.tuneMode)
				{
					...
					pDemodInfo->controlState = LX_DEMOD_TUNE_SCAN;
					DEMOD_PRINT( "[DEMOD L18F GBB] LX_DEMOD_TUNE_START : (auto) Going to LX_DEMOD_TUNE_SCAN..... ^0^\n");
				...
				else
				{
					...
					DEMOD_PRINT( "[DEMOD L18F GBB] LX_DEMOD_TUNE_START :(manual) Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				if (RET_OK != _DEMOD_L18F_LGDBB_QAM_CheckSignalState(pDemodInfo))
				{
						DEMOD_PRINT( "^r^[DEMOD L18F GBB] FAIL (%s:%d) DEMOD_Get_signal_status() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if(RET_OK != DEMOD_L18F_QAM_EQ_Signal_Detector(FALSE))
					{
						DEMOD_PRINT("[DEMOD L18F GBB] FAIL (%s:%d) DEMOD_L18F_QAM_EQ_Signal_Detector() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				...
				else
				{
					if(LX_DEMOD_TUNE_NORMAL == realTuneMode)
					{
						...
						{
							if(RET_OK != DEMOD_L18F_QAM_ModeAutoDetection(TRUE))
							{
								DEMOD_PRINT("[DEMOD L18F GBB] FAIL (%s:%d) DEMOD_L18F_QAM_ModeAutoDetection() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							#if 0
							if(RET_OK != DEMOD_L18F_SoftwareReset())
							{
								DEMOD_PRINT("[DEMOD L18F GBB] FAIL (%s:%d) DEMOD_L18F_SoftwareReset() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							#endif
						...
						else
						{
							...
							else
							{
								if(packetError < LX_DEMOD_PACKET_ERROR_THD)
								{
									...
									DEMOD_PRINT_ALWAYS( "[DEMOD L18F GBB] LX_DEMOD_SIGNAL_STABLE : MSG_QAM_FRONT_END_LOCKED..... ^^^0^^^\n");
									DEMOD_PRINT_ALWAYS( "[DEMOD L18F GBB] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^^0^^^\n", elapsedTime);
									DEMOD_PRINT( "^R^[DEMOD L18F GBB] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^0^\n", freqOffset);
...
************************************************************************************/
/**
 * DEMOD_L18F_QAM_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_L18F_QAM_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if(pDemodInfo->lockCount == 2)
		{
			...
			pDemodInfo->bNotifiedUnlock = FALSE;
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD L18F GBB] MONITORING FUNC : MSG_QAM_FRONT_END_LOCKED..... ^0^\n");
	...
	else
	{
		...
		pDemodInfo->lockCount = 0;
		_gL18FGBB_VQI_UnlockCount_full_packeterr = 0;
		...
		if((LX_DEMOD_UNLOCKED != pDemodInfo->lockStatus)  && (TRUE != pDemodInfo->bNotifiedUnlock))
		{
			...
			if(pDemodInfo->unLockCount == 4)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD L18F GBB] MONITORING FUNC : MSG_QAM_FRONT_END_NOT_LOCKED..... -_-\n");
...
int _DEMOD_L18F_LGDBB_ISDB_CheckSignalState(DEMOD_CTX_T *pDemodInfo)
{
...
int _DEMOD_L18F_ISDB_CheckTuneMode(DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int DEMOD_L18F_LGDBB_ISDB_SetDemod(DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context 	*pFrontend;
	UINT8	new_tuneMode;
	UINT32 	freq_Khz;
	...
	freq_Khz = 0;
	 ...
	 printk("[L18F LGDBB] ################### chungiii SetDemod ISDBT \n");

	new_tuneMode = _DEMOD_L18F_ISDB_CheckTuneMode(pDemodInfo);
	printk("[L18F LGDBB] ################### chungiii SetDemod ISDBT new_tuneMode[%d] \n", new_tuneMode);
	...
	if(RET_OK != DEMOD_L18F_Get_BW(pDemodInfo, &ter_bandwidth_KHz))
	{
	 ...
	 printk("[L18F LGDBB] ################### chungiii SetDemod ISDB check ter_bandwidth_KHz[%d] \n", ter_bandwidth_KHz);
...
int DEMOD_L18F_ISDBT_TDI_Enable(BOOLEAN bEnable)
{
	...
	printk("[DEMOD L18Fp]   DEMOD_L18F_TDI_Enable  ###### chungiii \n");
	...
	printk("[DEMOD L18Fp]   DEMOD_L18F_TDI_Enable  ###### chungiii  check TDI data[0x%x] \n", vqData);
...
int DEMOD_L18F_ISDBT_Set_MemSconfig(void)
{
	...
	if(st_mem == 0)
	{
		printk("######[0629] Error L18F DEMOD Wrong Memory Base Address ##### !!!!!!!!!!!!!!!!!!!!!!!\n");
	}
	else
	{
		printk("######[0629] first call DEMOD_L18F_ISDBT_Set_MemSconfig addr[0x%x] ##### !!!!!!\n", st_mem );
...
int DEMOD_L18F_ISDBT_GetEmergencyAlertFlagStatus(BOOLEAN *pEalarm)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_L18F_ISDBT_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_L18F_LGDBB_ISDB_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD L18F LGDBB] FAIL (%s:%d) DEMOD_L18F_LGDBB_ISDB_SetDemod() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}

				#if 1   // FALSE(0x88) = only when using FPGA board , TRUE(0x08) = only when using SOC
				if(RET_OK != DEMOD_L18F_ISDBT_TDI_Enable(TRUE))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD L18F LGDBB] FAIL (%s:%d) DEMOD_L18F_TDI_Enable() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				#endif
		...
		case LX_DEMOD_TUNE_SCAN:			/***************************************************/
			{
				...
				else if((LX_DEMOD_FLAG_FINISH == scanProcessing)&& (LX_DEMOD_UNLOCKED == scanLock))
				{
					...
					DEMOD_PRINT( "^G^[DEMOD L18F] (%s:%d) Signal is NOT Found  !!!\n", __F__, __L__);
				...
				else
				{
					...
					if(pDemodInfo->bNotifiedUnlock == TRUE)
					{
						...
						if((pDemodInfo->unLockLongCount & 0x3) == 0x3)
						{
							...
							if(RET_OK != DEMOD_L18F_ISDBT_SafetyReset())
							{
								DEMOD_PRINT("[DEMOD L18F] FAIL (%s:%d) DEMOD_SoftwareReset() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							DEMOD_PRINT("[DEMOD L18F](%s) DEMOD_SoftwareReset() !!!\n", __F__);
						}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.isdbt.tuneMode)
					{
						...
						else
						{
							...
							{
#if 0
								if(RET_OK != DEMOD_L18F_ISDBT_Set_PhaseNoiseEnhancement(TRUE))
								{
									DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_L18F_ISDBT_Set_PhaseNoiseEnhancement() !!!\n", __F__, __L__);
									DEMOD_RETURN_FAIL(__F__);
								}
#endif
								if(packetError < LX_DEMOD_PACKET_ERROR_THD)
								{
									...
									DEMOD_PRINT_ALWAYS( "[DEMOD L18F] LX_DEMOD_SIGNAL_STABLE : MSG_ISDBT_FRONT_END_LOCKED..... ^^^0^^^\n");
									DEMOD_PRINT_ALWAYS( "[DEMOD L18F] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^^0^^^\n", elapsedTime);
									DEMOD_PRINT( "^R^[DEMOD L18F] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^^^0^^^\n", freqOffset);
									...
									if (RET_OK != DEMOD_Get_signal_status(pDemodInfo))
									{
										DEMOD_PRINT( "^r^[DEMOD L18F] FAIL (%s:%d) DEMOD_Get_signal_status() !!!\n", __F__, __L__);
										DEMOD_RETURN_FAIL(__F__);
									}
								...
								else
								{
									...
									if(elapsedTime > LX_DEMOD_LOCK_DEALY_100)
									{
										...
										DEMOD_PRINT_ALWAYS( "[DEMOD L18F][0720] LX_DEMOD_SIGNAL_STABLE : MSG_ISDBT_FRONT_END_LOCKED..... ^^0^^\n");
										DEMOD_PRINT_ALWAYS( "[DEMOD L18F] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^0^^\n", elapsedTime);
										DEMOD_PRINT( "^R^[DEMOD L18F] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^^0^^\n", freqOffset);
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_L18F_ISDBT_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if (RET_OK != _DEMOD_L18F_LGDBB_ISDB_CheckSignalState(pDemodInfo))
		{
			DEMOD_PRINT( "^r^[DEMOD L18F] FAIL (%s:%d) DEMOD_Get_signal_status() !!!\n", __F__, __L__);
			DEMOD_RETURN_FAIL(__F__);
		}

		if(pDemodInfo->signalStatus.packetError >= _gL18FGBB_FullISDBTPkerrThreshold)
		{
			_gL18FGBB_VQI_UnlockCount_full_packeterr++;
			DEMOD_PRINT( "^r^[DEMOD L18F](%s) _gL18FGBB_VQI_UnlockCount_full_packeterr = %d !!!\n", __F__,_gL18FGBB_VQI_UnlockCount_full_packeterr);

			if(_gL18FGBB_VQI_UnlockCount_full_packeterr == _gL18FGBB_FullVQIPkerrCount)
			{
				DEMOD_PRINT( "^r^[DEMOD L18F] oops !! (%s) Force to NOT LOCK !!\n", __F__);
				...
				if(RET_OK != DEMOD_L18F_ISDBT_SafetyReset())
				{
					DEMOD_PRINT("[DEMOD L18F] FAIL (%s:%d) DEMOD_L18F_ISDBT_SafetyReset() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				DEMOD_PRINT("[DEMOD L18F](%s) DEMOD_SoftwareReset() !!!\n", __F__);
		...
		else
		{
			...
			if(gL18FGBB_PowerOnInit == 2)
			{

				powerOnElapsedTime = jiffies_to_msecs(jiffies) - gL18FGBB_PowerOnInitStartTimeISDBT;

				DEMOD_PRINT("[DEMOD L18F](%s:%d) powerOnElapsedTime = %d !!!\n", __F__, __L__,powerOnElapsedTime);

				if(powerOnElapsedTime >= 15000)
				{
					gL18FGBB_PowerOnInit = 0;

					if (RET_OK != DEMOD_L18F_ISDBT_Set_CrGearshEn(TRUE ))
					{
						DEMOD_PRINT("[DEMOD L18F] FAIL (%s:%d) DEMOD_L18F_ISDBT_Set_CrGearshEn() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				}
			}
			...
			if(pDemodInfo->lockCount == 1)
			{
				...
				pDemodInfo->bNotifiedUnlock = FALSE;
					...
					DEMOD_PRINT_ALWAYS( "[DEMOD L18F] MONITORING FUNC : MSG_ISDBT_FRONT_END_LOCKED..... ^0^\n");
			...
			if (RET_OK != DEMOD_Get_TunedCFG(pDemodInfo))
			{
				DEMOD_PRINT( "^r^[DEMOD L18F] FAIL (%s:%d) DEMOD_Get_TunedCFG() !!!\n", __F__, __L__);
				DEMOD_RETURN_FAIL(__F__);
			}
	...
	else
	{
		...
		if((LX_DEMOD_UNLOCKED != pDemodInfo->lockStatus)  && (TRUE != pDemodInfo->bNotifiedUnlock))
		{
			...
			if(pDemodInfo->unLockCount == 2)
			{
				...
				if(RET_OK != DEMOD_L18F_ISDBT_SafetyReset())
				{
					DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_L18F_ISDBT_SafetyReset() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}


				DEMOD_PRINT_ALWAYS( "[DEMOD L18F] MONITORING FUNC : MSG_ISDBT_FRONT_END_NOT_LOCKED..... -_-\n");
...
************************************************************************************/
/**
 * DEMOD_L18F_QAM_SoftwareResetFEC.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_QAM_SoftwareResetFEC(void)
{
...
int DEMOD_L18F_QAM_Monitor(void)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_QAM_ModeAutoDetection.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_QAM_ModeAutoDetection(BOOLEAN bEnable)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_QAM_64Mode(void)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_QAM_256Mode(void)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_QAM_Set_DefaultRegisterValue.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_QAM_Set_DefaultRegisterValue(void)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_QAM_Set_NeverLockWaitTime
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_QAM_Set_NeverLockWaitTime(UINT32 waitMs)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_QAM_Get_TotalInfo( void)
{
...
/**
 * DEMOD_L18F_QAM_EQ_Signal_Detector.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_QAM_EQ_Signal_Detector(BOOLEAN OnOff)
{
...
/**
 * DEMOD_L18F_QAM_Set_Frame_resynchronizaiton.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_QAM_Set_Frame_resynchronizaiton(BOOLEAN OnOff)
{
...
/**
 * DEMOD_L18F_ISDBT_SetDefaultRegisterValue
...
int DEMOD_L18F_ISDBT_Set_DefaultRegisterValue(void)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_Set_Config_auto
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Set_Config_auto (BOOLEAN bAutoDetect)
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_Set_PartialConfig
 * Set hierachical signal path and bandwidth.
 * This function is used when DEMOD_L18F_ISDBT_Set_Config_auto is enabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Set_PartialConfig (LX_DEMOD_ISDBT_CONFIG_T *pIsdbtConfigParam)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when DEMOD_L18F_ISDBT_Set_Config_auto is disabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Set_FullConfig (LX_DEMOD_ISDBT_CONFIG_T *pIsdbtConfigParam)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Set_NeverLockWaitTime(UINT32 waitMs)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_Set_CrGearshEn
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Set_CrGearshEn(BOOLEAN bEnable)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_Set_PhaseNoiseEnhancement
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Set_PhaseNoiseEnhancement(BOOLEAN bEnhanced)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_Get_DelaySpreadStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Get_DelaySpreadStatus(UINT16 *pDelaySpread)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Get_FFTMode( LX_DEMOD_LOCK_STATE_T *pTxModeLock, LX_DEMOD_FFTMODE_T *pFftMode )
{
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_Get_GuradIntervalMode
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Get_GuradIntervalMode( LX_DEMOD_GIMODE_T *pGIMode )
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Get_TMCCInfo( LX_DEMOD_ISDBT_TMCC_CONFIG_T *pTmccParams)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_Get_TotalInfo
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Get_TotalInfo( void)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Get_IFO_LOCK( BOOLEAN *pIfoLock)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_Get_CochanDetIndicator
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_Get_CochanDetIndicator( BOOLEAN *pDetStatus)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_EqualizereReset.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_EqualizereReset(void)
{
 ...
 * @author	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_SafetyReset(void)
{

#if 0
	DEMOD_L18F_FecReset(TRUE);
	OS_MsecSleep(5);
...
************************************************************************************/
/**
 * DEMOD_L18F_ISDBT_MemoryBusCheck.
 ...
 * @author	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_ISDBT_MemoryBusCheck(void)
{
...
int DEMOD_L18F_ISDBT_Set_MemBus_flushing(void)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_VSB_Set_DefaultRegisterValue.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_VSB_Set_DefaultRegisterValue(void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_L18F_VSB_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_VSB_Set_NeverLockWaitTime(UINT32 waitMs)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_VSB_CochannelExist(BOOLEAN *pCochannel)
{
	return RET_OK;
}
...
int DEMOD_L18F_VSB_PreMonitor(void)
{

	DEMOD_TRACE("[DEMOD L18F DTV] @ %s =====\n",__F__);
...
int DEMOD_L18F_VSB_Monitor(DEMOD_CTX_T *pDemodInfo)
{
...
************************************************************************************/
/**
 * DEMOD_L18F_VSB_Get_MSEdynStatus.
...
int DEMOD_L18F_VSB_Get_MSEdynStatus(BOOLEAN *pbEnable)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_VSB_Get_TotalInfo( void)
{
...
int DEMOD_L18F_LGDBB_VSB_SetDemod_Simple(void)
{
	LX_DEMOD_L2_Context 	*pFrontend;
