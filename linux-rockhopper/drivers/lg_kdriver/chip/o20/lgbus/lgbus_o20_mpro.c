...
#define GBUS_QOS_CMN_BASE			(0xC9210000)
...
static lgbus_o20_mpro_t mpro_ctrl[] =
{
	{ "CORE",		0xC9231200},
	{ "AXI_PERI",	0xC9231400},
	{ "AHB_PERI",	0xC9231600},
	...
	{ "FMC3",		0xC9823A00},
	{ "CCO0",		0xC9823C00},
	{ "CCO1_SRE",	0xC9823E00}, //CC01 read only, SRE write only
	{ "ND1",		0xC9824000},
	{ "ME0",		0xC9824200},
	{ "SSD",		0xC9824400},
	{ "FMS",		0xC9824600},
...
#define DDR_M0_BASE		(0xc991d000)
#define DDR_M1_BASE		(0xc993d000)

#define DDR_MONITOR_INTERVAL		(1000)// 1000ms
#define DDR_MON_DLLM_ADDR			(0x00c)
#define DDR_MON_IET_DIFF_ADDR		(0x82c)
#define DDR_MON_IET_PHASE_ADDR		(0x828)
#define DDR_MON_MRR_ADDR			(0x048)
#define DDR_MON_IOCAL_ADDR			(0x004)

#define ddr_reg_read(addr)				__raw_readl((void*)(addr))
#define ddr_reg_write(val,addr)		__raw_writel(val, (void*)(addr))

typedef struct
{
	u32	*base_addr_phy;
	u32 *base_addr;
}lgbus_o20_ddr_monitor_t;

static lgbus_o20_ddr_monitor_t m0_monitor;
static lgbus_o20_ddr_monitor_t m1_monitor;

static struct delayed_work ddr_work;
static u32 inited_ddr_work = 0;
static u32 o20_ddr_mon_interval = DDR_MONITOR_INTERVAL;
module_param(o20_ddr_mon_interval, uint, 0664);
...
========================================================================================*/
static void lgbus_ddr_monitor_work(struct work_struct *work)
{
	u32 m0_mon_dllm;
	u32 m1_mon_dllm;

	u32 m0_mon_iet_diff[4];
	u32 m1_mon_iet_diff[4];

	u32 m0_mon_iet_phase[4];
	u32 m1_mon_iet_phase[4];
	...
	LGBUS_DDRM_PRINT("[%s]monitor work\n", __func__);

	m0_mon_dllm = ddr_reg_read((ulong)m0_monitor.base_addr+DDR_MON_DLLM_ADDR);
	m1_mon_dllm = ddr_reg_read((ulong)m1_monitor.base_addr+DDR_MON_DLLM_ADDR);

	//LGBUS_DDRM_PRINT("temp 0x%lx\n", (ulong)m0_monitor.base_addr+DDR_MON_DLLM_ADDR);
	//LGBUS_DDRM_PRINT("temp 0x%lx\n", (ulong)m1_monitor.base_addr+DDR_MON_DLLM_ADDR);

	for(i = 0; i < 4; i++)
	{
		m0_mon_iet_diff[i] = ddr_reg_read((ulong)m0_monitor.base_addr + DDR_MON_IET_DIFF_ADDR + 0x200*i);
		m1_mon_iet_diff[i] = ddr_reg_read((ulong)m1_monitor.base_addr + DDR_MON_IET_DIFF_ADDR + 0x200*i);

		m0_mon_iet_phase[i] = ddr_reg_read((ulong)m0_monitor.base_addr + DDR_MON_IET_PHASE_ADDR + 0x200*i);
		m1_mon_iet_phase[i] = ddr_reg_read((ulong)m1_monitor.base_addr + DDR_MON_IET_PHASE_ADDR + 0x200*i);

		//LGBUS_DDRM_PRINT("temp 0x%lx\n", (ulong)m0_monitor.base_addr + DDR_MON_IET_DIFF_ADDR + 0x200*i);
		//LGBUS_DDRM_PRINT("temp 0x%lx\n", (ulong)m1_monitor.base_addr + DDR_MON_IET_DIFF_ADDR + 0x200*i);
	}

	m0_mon_mrr = ddr_reg_read((ulong)m0_monitor.base_addr+DDR_MON_MRR_ADDR);
	m1_mon_mrr = ddr_reg_read((ulong)m1_monitor.base_addr+DDR_MON_MRR_ADDR);

	//LGBUS_DDRM_PRINT("temp 0x%lx\n", (ulong)m0_monitor.base_addr+DDR_MON_MRR_ADDR);
	//LGBUS_DDRM_PRINT("temp 0x%lx\n", (ulong)m1_monitor.base_addr+DDR_MON_MRR_ADDR);

	m0_mon_iocal = ddr_reg_read((ulong)m0_monitor.base_addr+DDR_MON_IOCAL_ADDR);
	m1_mon_iocal = ddr_reg_read((ulong)m1_monitor.base_addr+DDR_MON_IOCAL_ADDR);

	if(pm_o20_avs)
		LGBUS_DDRM_PRINT("temp %u\n", pm_o20_avs->get_temperature());

	//result print
	LGBUS_DDRM_PRINT("m0 dllm 0x%08x\n", m0_mon_dllm);
	for(i = 0; i < 4; i++)
	{
		LGBUS_DDRM_PRINT("m0 iet diff [%u] 0x%08x\n", i, m0_mon_iet_diff[i]);
		LGBUS_DDRM_PRINT("m0 iet phase[%u] 0x%08x\n", i, m0_mon_iet_phase[i]);
	}

	LGBUS_DDRM_PRINT("m0 tDQS2DQ %08d, %08d\n",
			2560000/(((m0_mon_mrr&0x00FF0000)>>8|(m0_mon_mrr&0x0000000FF))*2),
			2560000/(((m0_mon_mrr&0xFF000000)>>16|(m0_mon_mrr&0x00000FF00)>>8)*2) );

	LGBUS_DDRM_PRINT("m0 iocal pu:0x%08x, pd:0x%08x\n", (m0_mon_iocal&0x00003F00)>>8,
			(m0_mon_iocal&0x003F0000)>>16);

	LGBUS_DDRM_PRINT("m1 dllm 0x%08x\n", m1_mon_dllm);
	for(i = 0; i < 4; i++)
	{
		LGBUS_DDRM_PRINT("m1 iet diff [%u] 0x%08x\n", i, m1_mon_iet_diff[i]);
		LGBUS_DDRM_PRINT("m1 iet phase[%u] 0x%08x\n", i, m1_mon_iet_phase[i]);
	}

	LGBUS_DDRM_PRINT("m1 tDQS2DQ %08d, %08d\n",
			2560000/(((m1_mon_mrr&0x00FF0000)>>8|(m1_mon_mrr&0x0000000FF))*2),
			2560000/(((m1_mon_mrr&0xFF000000)>>16|(m1_mon_mrr&0x00000FF00)>>8)*2) );

	LGBUS_DDRM_PRINT("m1 iocal pu:0x%08x, pd:0x%08x\n", (m1_mon_iocal&0x00003F00)>>8,
			(m1_mon_iocal&0x003F0000)>>16);

	schedule_delayed_work(&ddr_work, msecs_to_jiffies(o20_ddr_mon_interval));
}

static int lgbus_ddr_monitor_init(void)
{
	//set logm bit
	//LOGM_ObjBitMaskEnable(g_lgbus_debug_fd, LGBUS_LOG_LEVEL_DDR_M);

	//get avs func pointer
	pm_o20_avs = get_o20_pm_avs();

	//ddr register...
	m0_monitor.base_addr_phy = (u32*)DDR_M0_BASE;
	m1_monitor.base_addr_phy = (u32*)DDR_M1_BASE;

	m0_monitor.base_addr = (u32*)ioremap_nocache((ulong)m0_monitor.base_addr_phy, 0x1000);
	m1_monitor.base_addr = (u32*)ioremap_nocache((ulong)m1_monitor.base_addr_phy, 0x1000);

	if(!m0_monitor.base_addr || !m1_monitor.base_addr)
	{
		error("fail ioremap\n");
		return -1;
	}
	...
	//create work
	INIT_DELAYED_WORK(&ddr_work, lgbus_ddr_monitor_work);
	...
	LGBUS_DDRM_PRINT("[%s]monitor init, interval :%u\n", __func__, o20_ddr_mon_interval);

	return 0;
}

static int lgbus_ddr_monitor_start(void)
{
	if(!inited_ddr_work)
		return -1;

	LGBUS_DDRM_PRINT("[%s]monitor start\n", __func__);
	schedule_delayed_work(&ddr_work, msecs_to_jiffies(o20_ddr_mon_interval));

	return 0;
}

static int lgbus_ddr_monitor_stop(void)
{
	if(!inited_ddr_work)
		return -1;

	LGBUS_DDRM_PRINT("[%s]monitor stop\n", __func__);
	cancel_delayed_work_sync(&ddr_work);
...
static int lgbus_mpro_resume(void)
{
#ifdef SUPPORT_O20_MPRO_CHIP_KDRV
	int ret;
...
static int lgbus_mpro_get_range(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		for(j = 0; j < O20_MPRO_RANGE_SLOT_CNT; j++)
		{
...
static int lgbus_mpro_regs_save(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		size = sizeof(lgbus_o20_mpro_reg_save_t) / sizeof(u32);
...
#undef LBUS_IRQ
#undef GBUS_IRQ
#define LBUS_IRQ	O20_IRQ_INTR_LBUS
#define GBUS_IRQ	O20_IRQ_INTR_GBUS

#undef CPU_IRQ
#undef CPU_AXI_PERI_IRQ
#undef CPU_AHB_PERI_IRQ
#undef CPU_LZMA_IRQ
#define CPU_IRQ				O20_IRQ_QOS_CPU
#define CPU_AXI_PERI_IRQ	O20_IRQ_QOS_AXI_PERI
#define CPU_AHB_PERI_IRQ	O20_IRQ_QOS_AHB_PERI
#define CPU_LZMA_IRQ		O20_IRQ_QOS_LZMA
...
static irqreturn_t lgbus_mpro_isr(int irq, void *dev)
{
	...
	if(irq == LBUS_IRQ || irq == GBUS_IRQ || irq == CPU_IRQ || irq == CPU_AXI_PERI_IRQ || irq == CPU_AHB_PERI_IRQ || irq == CPU_LZMA_IRQ)
	{
		ulong flag;
		LGBUS_MPRO_CTRL_LOCK();
			...
			if(irq_status & MPRO_ISR_STATUS)
			{
...

				{
					...
					struct
					{
						...
						u32 wr_vio_addr;
						u32 wr_vio_id;
					}rw_vio_data[O20_MPRO_RANGE_SLOT_CNT];
...
int LGBUS_O20_SetMproRange(lgbus_mpro_range_t *mpro_range)
{
#ifdef SUPPORT_O20_MPRO_CHIP_KDRV
	int 		ret = RET_ERROR;
	uint32_t 	port, cnt;
	...
	CHECK_ERROR(mpro_range->port >= O20_MPRO_PORT_MAX, return RET_ERROR, "Invalid port\n");
	CHECK_ERROR(mpro_range->port < O20_MPRO_PORT_START, return RET_ERROR, "Invalid port\n");

	port = mpro_range->port - O20_MPRO_PORT_START;
	...
	for(cnt = 0; cnt < O20_MPRO_RANGE_SLOT_CNT; cnt++)
	{
		uint32_t ctr0, ctr1;
...
EXPORT_SYMBOL(LGBUS_O20_SetMproRange);

int LGBUS_O20_GetMproRange(lgbus_mpro_range_t *mpro_range)
{
#ifdef SUPPORT_O20_MPRO_CHIP_KDRV
	uint32_t 	port, cnt;
	ulong 		flag;
...
EXPORT_SYMBOL(LGBUS_O20_GetMproRange);

lgbus_o20_mpro_func_t	lgbus_o20_mpro_func =
{
...
static lgbus_addr_switch_t	o20_addr_switch[O20_ADDR_SWITCH_SLOT];
static lgbus_bwb_t			o20_bwb[O20_BWB_SLOT];
...
static int lgbus_get_addr_switch(void)
{
	...
	for(i = 0; i < O20_ADDR_SWITCH_SLOT; i++)
	{
		u32 d0, d1;
		...
		o20_addr_switch[i].start = ((u64)(d0 >> 16) & 0x1fff) << 20; // 28~16:13bit
		o20_addr_switch[i].end = (u64)(d0 & 0x1fff) << 20; // 12~0:13bit
		o20_addr_switch[i].offset = (u64)(d1 & 0x1fff) << 20; // 12~0:13bit

		if(o20_addr_switch[i].end)
			o20_addr_switch[i].end |= 0xfffff;

		LGBUS_PRINT("s:0x%010llx ~ e:0x%010llx,o:0x%010llx\n", o20_addr_switch[i].start
															 , o20_addr_switch[i].end
															 , o20_addr_switch[i].offset);
	}
...
static int lgbus_get_bwb(void)
{
	...
	for(i = 0; i < O20_BWB_SLOT; i++)
	{
		...
		o20_bwb[i].start = ((u64)(d0 >> 12) & 0x7ff) << 20;
		o20_bwb[i].end = (u64)(d0 & 0x7ff) << 20;

		o20_bwb[i].offset = (u64)(d1 & 0x1fff) << 20; /* o20 [12:0] rf_bw_bal_offset */
		o20_bwb[i].offset_mode = (d1 >> 28) & 0x7;

		if(o20_bwb[i].end)
			o20_bwb[i].end |= 0xfffff;

		LGBUS_PRINT("s:0x%010llx ~ e:0x%010llx,o:0x%010llx[%x]\n", o20_bwb[i].start
															 , o20_bwb[i].end
															 , o20_bwb[i].offset
															 , o20_bwb[i].offset_mode);
	}
...
u64 lgbus_o20_get_ddr_addr(u64 phy_addr)
{
#ifdef SUPPORT_O20_MPRO_CHIP_KDRV
	int i;
	u64 tmp = phy_addr;
	...
	for(i = 0; i < O20_ADDR_SWITCH_SLOT; i++)
	{
		if(o20_addr_switch[i].start <= phy_addr &&
				o20_addr_switch[i].end >= phy_addr)
		{
			phy_addr += o20_addr_switch[i].offset;
			phy_addr &= 0x1ffffffff; /* wrap around */
			break;
		}
	}
	...
	for(i = 0; i < O20_BWB_SLOT; i++)
	{
		if(o20_bwb[i].offset)
		{
			if(o20_bwb[i].start <= bwb_addr &&
					o20_bwb[i].end >= bwb_addr)
			{
				if(o20_bwb[i].offset_mode == BWB_OFFSET_M0 &&
						bwb_addr == phy_addr)
				{
					phy_addr += o20_bwb[i].offset;
				}
				else if(o20_bwb[i].offset_mode == BWB_OFFSET_M1 &&
						phy_addr >= 0x180000000 && phy_addr < 0x200000000 )
				{
					phy_addr += o20_bwb[i].offset;
				}
	...
	LGBUS_PRINT("%s: v:0x%010llx -> p:0x%010llx\n", __func__, tmp, phy_addr);
#endif
	return phy_addr;
}
EXPORT_SYMBOL(lgbus_o20_get_ddr_addr);
...
static int lgbus_mpro_proc_status_op(struct seq_file *s, void *data)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		{
			seq_printf(s, "\trange(%d) 0x%09llx ~ 0x%09llx\n", j
												, mpro_ctrl[i].range[j].start
												, mpro_ctrl[i].range[j].end);
		...
		if(mpro_ctrl[i].vio_type)
		{
			...
			seq_printf(s, COLOR_RED"\tvio addr(ddr phy) 0x%09llx, type %s\n"COLOR_NONE
        ...
        , const char __user *buf, size_t size, loff_t *off)
{
	...
	int		i;
	int		addr_cnt;
	int		addr_max = O20_MPRO_RANGE_SLOT_CNT * 2;
	u64		addr[O20_MPRO_RANGE_SLOT_CNT * 2] = {0,};
	...
	while(i < size)
	{
		...
		{
			mpro_range.port += O20_MPRO_PORT_START;
			LGBUS_O20_SetMproRange(&mpro_range);
		}
...
static int lgbus_mpro_proc_asw_op(struct seq_file *s, void *data)
{
	...
	/* copied from lgbus_get_bwb(void) */
	{
		...
		for(i = 0; i < O20_BWB_SLOT; i++)
		{
			...
			seq_printf(s, "bwb: 0x%04x 0x%04x 0x%04x 0x%x 0x%x\n", ((d0>>12) & 0x7ff), (d0 & 0x7ff), (d1 & 0x1fff), ((d0>>28)&0xf), ((d1>>28)&0x7));
		}
