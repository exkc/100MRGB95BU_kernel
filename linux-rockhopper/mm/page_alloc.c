...
#ifdef CONFIG_CMA
	if (!page || (alloc_flags & ALLOC_CMA))
		goto out;

	if (zone_cma_pages(zone)) {
		if (is_migrate_cma_page(page)) {
			list_add(&page->pcp_list, list);
			pcp->count += 1 << order;
			/*
			 * ZONE_MOVABLE has only movable list.
			 * Free all entry is safe.
			 */
						  ...
						  unsigned int alloc_flags)
{
	...
	if (gfp_migratetype(gfp_mask) != MIGRATE_MOVABLE)
		goto out;
	if (IS_ENABLED(CONFIG_CMA_ALLOW_HIGHUSER_MOVABLE_ALLOC) ||
			(gfp_mask & __GFP_CMA)) {
...
retry:
	...
	if (ret && ret != -EBUSY) {
		pr_warn("__alloc_contig_migrate_range(%lx, %lx) failed %d\n",
			start, end, ret);
		if(ret == -EINTR) goto done;
		else goto retry;
	}
	ret = 0;
	...
	if (!ret && is_cma_area(start))
		mod_zone_page_state(page_zone(pfn_to_page(start)),
				NR_ALLOC_CMA_PAGES, outer_end - outer_start);
...
void free_contig_range(unsigned long pfn, unsigned long nr_pages)
{
	...
	if (is_cma_area(pfn))
		mod_zone_page_state(page_zone(pfn_to_page(pfn)),
					NR_ALLOC_CMA_PAGES, -(long)nr_pages);
