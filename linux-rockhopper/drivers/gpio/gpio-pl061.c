...
static int pl061_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	if(of_property_read_s32(dev->of_node, "gpio-base", (s32 *)&pl061->gc.base))
		pl061->gc.base = -1;
...
#ifdef CONFIG_PM
	pl061->csave_regs.gpio_skip_resume = 0;

	of_property_read_u8(dev->of_node, "skip-resume",
			    (u8 *)&pl061->csave_regs.gpio_skip_resume);
#endif
...
static int pl061_resume(struct device *dev)
{
	...
	for (offset = 0; offset < PL061_GPIO_NR; offset++) {
		if (pl061->csave_regs.gpio_skip_resume & (BIT(offset)))
			continue;
		if (pl061->csave_regs.gpio_dir & (BIT(offset)))
