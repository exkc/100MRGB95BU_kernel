...
enum {
	DEBUG_ID_VERSION = 0,
//      PROC_ID_INPUT_MEASURING_TIME,
	DEBUG_ID_INSTANTBOOT,
	DEBUG_ID_MAX,
};

enum {
	STATUS_FILTER = (0x01 << ATSC3_DEBUG_STATUS_FILTER),
	STATUS_MCU_FILTER = (0x01 << ATSC3_DEBUG_STATUS_MCU_FILTER),
	STATUS_INPUT = (0x01 << ATSC3_DEBUG_STATUS_INPUT),
	STATUS_CLOCK = (0x01 << ATSC3_DEBUG_STATUS_CLOCK),
	STATUS_DUMP = (0x01 << ATSC3_DEBUG_STATUS_DUMP),
	STATUS_ALL = 0xFFFFFF,
};

static int suspend_resume_ctrl(struct atsc3_device *dev, u8 status)
{
	int ret;
...
static int debug_write(struct te_debug_item *item, char *str)
{
	int ret = 0;
	u32 val;
	struct atsc3_device *dev = item->private;
...
static int debug_status_show(struct te_debug_item *item, struct seq_file *m)
{
	...
	for (i = 0; i < ATSC3_DEBUG_STATUS_MAX; i++) {
		if (item->flags & (0x1 << i)) {
			atsc3_debug_show_t func = dev->debug.show_status[i];
			if (func) {
				if (cnt)
					dprintf("\n");
...
static const struct te_debug_item_info debug_list[] = {
	...
	DBGITM_SIM_R("filter", &debug_status_ops, STATUS_FILTER),
...
static int debug_dev_init(struct atsc3_device *dev)
{
	int ret, i;
	struct atsc3_dev_debug *d = &dev->debug;

	d->dir = te_debug_create_dir(ATSC3_MODULE, NULL);
...
static void debug_dev_release(struct atsc3_device *dev)
{
				     ...
				     atsc3_debug_show_t show)
{
	...
	if (status >= ATSC3_DEBUG_STATUS_MAX)
		return -EINVAL;
...
const struct atsc3_sub_driver atsc3_debug_driver = {
	.name = "atsc3_debug",
