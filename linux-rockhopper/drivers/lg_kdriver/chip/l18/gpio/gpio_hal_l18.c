...
----------------------------------------------------------------------------------------*/
#ifdef INCLUDE_L18_CHIP_KDRV
UINT32 		gpio_intr_num_l18[GPIO_IRQ_NUM_NR] = {L18_IRQ_GPIO0_2,L18_IRQ_GPIO3_5,L18_IRQ_GPIO6_8,L18_IRQ_GPIO9_11,L18_IRQ_GPIO12_14,L18_IRQ_GPIO15_17};
...
int abnormal_count = 0;
 ...
 void _GPIO_ISR_GPIO_L18( unsigned int ui32IRQNum, unsigned int IntData )
{
	UINT32	gpio_array_n;
	...
	if (ui32IRQNum >= L18_IRQ_GPIO0_2 && ui32IRQNum <=L18_IRQ_GPIO15_17)
		gpio_array_n = ui32IRQNum-L18_IRQ_GPIO0_2;
	else
	{
	...
	switch( ui32IRQNum )
	{
		case L18_IRQ_GPIO0_2 :
			gpio_array_n = 0;
			break;
		case L18_IRQ_GPIO3_5 :
			gpio_array_n = 3;
			break;
		case L18_IRQ_GPIO6_8 :
			gpio_array_n = 6;
			break;
		case L18_IRQ_GPIO9_11 :
			gpio_array_n = 9;
			break;
		case L18_IRQ_GPIO12_14 :
			gpio_array_n = 12;
			break;
		case L18_IRQ_GPIO15_17 :
			gpio_array_n = 15;
			break;
		default:
		...
		if ( ui32Status & ( 1 << ucGpio ) )
		{
			...
			if ( GPIONIE(gpio_array_n) & ( 1 << ucGpio ) )
			{
				...
				if (gpio_isr_info[realGpio].cb_enable)
				{
					...
					GPIO_DEBUG("_GPIO_ISR_GPIO_L18 		pin[%d] value[%d]\n",realGpio, value);
				...
				if (gpio_isr_info[realGpio].count_enable )
				{
					...
					else if (gpio_isr_info[realGpio].intr_count  == LAST_SAMPLE)
					{
						...
						{
							abnormal_count++;
							if(abnormal_count > DURATATION_RETRY_CNT )
							{
								GPIO_INFO("gpioabnormal measured[%d] abnormacnt[%d]\n",measure,abnormal_count );
