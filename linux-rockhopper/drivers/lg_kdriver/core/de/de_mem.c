...
int de_mem_init(void)
{
	...
	//ret = DE_HAL_SetMemByCma("de-fb");

	ret =  DE_HAL_SetMemByCma("de-btbuf");
	ret |= DE_HAL_SetMemByCma("de-hdrbuf0");
	ret |= DE_HAL_SetMemByCma("de-vtgshared");
	ret |= DE_HAL_SetMemByCma("de-vtgbuffer");
	ret |= DE_HAL_SetMemByCma("de-dbgbuf");

	{ // alloc for capture buffer
		ret = DE_HAL_GetMemFromCma("de-vtgbuffer", 1, NULL, 1); // webos4.5 MR
