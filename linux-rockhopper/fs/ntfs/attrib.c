...
runlist_element *__ntfs_attr_find_vcn_nolock(runlist *runlist, const VCN vcn)
{
		...
		const u8 *val, const u32 val_len, ntfs_attr_search_ctx *ctx)
{
	...
	for (;;	a = (ATTR_RECORD *)((u8 *)a + le32_to_cpu(a->length))) {
		...
		if ((space < offsetof(ATTR_RECORD, data.resident.reserved) + 1 ||
...
int load_attribute_list(ntfs_inode *base_ni, u8 *al_start, const s64 size)
{
	...
	if (IS_ERR(attr_vi)) {
		ntfs_error(base_ni->vol->sb, "Failed to open an inode for Attribute list, mft = %ld",
			   base_ni->mft_no);
	...
	if (ntfs_inode_attr_pread(attr_vi, 0, size, al_start) != size) {
		iput(attr_vi);
		ntfs_error(base_ni->vol->sb, "Failed to read attribute list, mft = %ld",
			   base_ni->mft_no);
		return -EIO;
	}
	iput(attr_vi);
	...
	for (al = al_start; al < al_start + size; al += le16_to_cpu(ale->length)) {
		...
		if (ale->name_offset != sizeof(ATTR_LIST_ENTRY))
			break;
		...
		const u8 *val, const u32 val_len, ntfs_attr_search_ctx *ctx)
{
	...
	if (ctx->is_first) {
		...
		if (((al_end - (u8 *)ctx->al_entry) < (long)offsetof(ATTR_LIST_ENTRY, name)) ||
		    (le16_to_cpu(ctx->al_entry->length) & 7) ||
		    (le16_to_cpu(ctx->al_entry->length) < offsetof(ATTR_LIST_ENTRY, name)))
			goto corrupt;
...
find_attr_list_attr:
	...
	for (;; al_entry = next_al_entry) {
		...
		if ((((u8 *)al_entry + offsetof(ATTR_LIST_ENTRY, name)) > al_end) ||
		    ...
		    (le16_to_cpu(al_entry->length) < offsetof(ATTR_LIST_ENTRY, name_length)) ||
		    (al_entry->name_length && ((u8 *)al_entry + al_entry->name_offset +
					       al_entry->name_length * sizeof(ntfschar)) > al_end))
			break; /* corrupt */
...
is_enumeration:
		...
		a = ctx->attr = (ATTR_RECORD *)((u8 *)ctx->mrec +
					le16_to_cpu(ctx->mrec->attrs_offset));
		/*
 ...
 */
static bool ntfs_attr_init_search_ctx(ntfs_attr_search_ctx *ctx,
...
int ntfs_attr_make_non_resident(ntfs_inode *ni, const u32 data_size)
{
	...
	if (NInoNonResident(ni)) {
		ntfs_warning(vol->sb, "Trying to make non-resident attribute "
...
int ntfs_attr_set(ntfs_inode *ni, s64 ofs, s64 cnt, const u8 val)
{
	...
	while (cnt) {
		...
		// to avoid race condition with mst write page, we lock buffer here for mst protected page
		folio_lock(folio);
...
int ntfs_attr_set_initialized_size(ntfs_inode *ni, loff_t new_size)
{
	ntfs_attr_search_ctx *ctx;
	int err = 0;
	...
	ctx->attr->data.non_resident.initialized_size = cpu_to_sle64(new_size);
...
static int ntfs_make_room_for_attr(MFT_RECORD *m, u8 *pos, u32 size)
{
	...
	if (!m || !pos || pos < (u8*)m) {
		pr_err("%s: pos=%p  m=%p", __FUNCTION__, pos, m);
		return -EINVAL;
	}
		...
		ATTR_FLAGS flags)
{
	...
	length = offsetof(ATTR_RECORD, data.resident.reserved) + sizeof(a->data.resident.reserved) +
	...
	a->name_offset = name_len ? cpu_to_le16((offsetof(ATTR_RECORD, data.resident.reserved) +
 ...
 */
static int ntfs_non_resident_attr_record_add(ntfs_inode *ni, ATTR_TYPE type,
		...
		ATTR_FLAGS flags)
{
	...
	ctx = ntfs_attr_get_search_ctx(ni, NULL);
	if (!ctx) {
		pr_err("%s: Failed to get search context", __FUNCTION__);
		return -ENOMEM;
	}
	/*
	 * Use ntfs_attr_find instead of ntfs_attr_lookup to find place for
	...
	length = offsetof(ATTR_RECORD, data.non_resident.compressed_size) +
	...
	if (err) {
		pr_err("%s: attribute lookup failed", __FUNCTION__);
		ntfs_attr_put_search_ctx(ctx);
		return err;
		...
		ntfschar *name, u8 name_len, u8 *val, s64 size)
{
	...
	if (is_resident)
		attr_rec_size = offsetof(ATTR_RECORD, data.resident.reserved) +
			1 +
			((name_len * sizeof(ntfschar) + 7) & ~7) +
			((size + 7) & ~7);
	else
		attr_rec_size = offsetof(ATTR_RECORD, data.non_resident.compressed_size) +
...
rm_attr_err_out:
	...
	if (!IS_ERR(ni_mrec)) {
		if (ntfs_attr_record_resize(ni_mrec,
					(ATTR_RECORD*)((u8*)ni_mrec + offset), 0))
 ...
 */
static void __ntfs_attr_init(ntfs_inode *ni,
 ...
 */
static void ntfs_attr_init(ntfs_inode *ni, const bool non_resident,
		...
		ntfschar *name, u32 name_len)
{
	...
	if (ni->type == AT_DATA && ni->name == AT_UNNAMED &&
		...
		err = -EIO;
		pr_err("Inode %lld has corrupt attribute flags "
	...
	if (a->non_resident) {
		if (((a->flags & ATTR_COMPRESSION_MASK) || a->data.non_resident.compression_unit) &&
				(ni->vol->major_ver < 3)) {
			err = -EIO;
			pr_err("Compressed inode %lld not allowed"
					" on NTFS %d.%d",
		...
		if ((a->flags & ATTR_IS_COMPRESSED) && !a->data.non_resident.compression_unit) {
			err = -EIO;
			pr_err("Compressed inode %lld attr 0x%x has "
					"no compression unit",
					(unsigned long long)ni->mft_no, type);
			goto put_err_out;
		}
		if ((a->flags & ATTR_COMPRESSION_MASK) &&
			...
			err = -EIO;
			pr_err("Compressed inode %lld attr 0x%lx has "
 ...
 * @na itself.
 */
void ntfs_attr_close(ntfs_inode *ni)
{
 ...
 */
int ntfs_attr_map_whole_runlist(ntfs_inode *ni)
{
	...
	while (1) {
		...
		if (!next_vcn) {
			if (a->data.non_resident.lowest_vcn) {
				err = -EIO;
				ntfs_error(sb, "First extent of inode %llu "
						"attribute has non-zero lowest_vcn",
						(unsigned long long)ni->mft_no);
				goto err_out;
			}
			/* Get the last vcn in the attribute. */
			last_vcn = sle64_to_cpu(a->data.non_resident.allocated_size) >>
				vol->cluster_size_bits;
		}
		...
		if (next_vcn < sle64_to_cpu(a->data.non_resident.lowest_vcn)) {
			err = -EIO;
	...
	if (not_mapped && highest_vcn && highest_vcn != last_vcn - 1) {
		err = -EIO;
		ntfs_error(sb, "Failed to load full runlist: inode: %llu "
...
int ntfs_attr_record_move_to(ntfs_attr_search_ctx *ctx, ntfs_inode *ni)
{
	 ...
	 * file record.
	 */
	err = ntfs_attr_find(a->type, (ntfschar*)((u8*)a + le16_to_cpu(a->name_offset)),
				a->name_length, CASE_SENSITIVE, NULL,
				0, nctx);
	if (!err) {
		ntfs_debug("Attribute of such type, with same name already "
 ...
 */
static int ntfs_attr_update_meta(ATTR_RECORD *a, ntfs_inode *ni, MFT_RECORD *m,
		hole_type holes, ntfs_attr_search_ctx *ctx)
{
	...
	a->data.non_resident.allocated_size = cpu_to_sle64(ni->allocated_size);

	/* Update sparse bit, unless this is an intermediate state */
	if (holes == HOLES_DELAY)
		sparse = (a->flags & ATTR_IS_SPARSE) != 0;
	else {
	...
	if ((holes != HOLES_DELAY) && sparse &&
		 ...
		 */
		if ((le32_to_cpu(a->length) - le16_to_cpu(a->data.non_resident.mapping_pairs_offset) == 8) &&
	...
	if (NInoFullyMapped(ni) && (sparse || NInoCompressed(ni))) {
		...
		a->data.non_resident.compressed_size = cpu_to_sle64(new_compr_size);
	}
 ...
 */
static int ntfs_attr_update_mapping_pairs_i(ntfs_inode *ni, VCN from_vcn,
		hole_type holes)
{
	ntfs_attr_search_ctx *ctx;
	ntfs_inode *base_ni;
...
retry:
	...
	sb = ni->vol->sb;
	if (!NInoNonResident(ni)) {
		ntfs_error(sb, "%s: resident attribute", __FUNCTION__);
		return -EINVAL;
	}
	...
	while (!(err = ntfs_attr_lookup(ni->type, ni->name, ni->name_len,
		...
		if (from_vcn) {
			...
			stop_vcn = sle64_to_cpu(a->data.non_resident.lowest_vcn);
			from_vcn = 0;
			/*
		...
		if (finished_build) {
			...
			a->data.non_resident.highest_vcn = cpu_to_sle64(NTFS_VCN_DELETE_MARK);
			mark_mft_record_dirty(ctx->ntfs_ino);
			continue;
		}

		err = ntfs_attr_update_meta(a, ni, m, holes, ctx);
		if (err < 0) {
			if (err == -EAGAIN) {
				ntfs_attr_put_search_ctx(ctx);
				goto retry;
		...
		if (mp_size <= 0) {
			err = mp_size;
			ntfs_error(sb, "%s: get MP size failed", __FUNCTION__);
			goto put_err_out;
		}
		...
		if (mp_size > exp_max_mp_size) {
			...
			if (ni->type == AT_ATTRIBUTE_LIST) {
				...
				if (ntfs_inode_free_space(base_ni, mp_size -
							cur_max_mp_size)) {
					ntfs_error(sb, "Attribute list is too "
							"big. Defragment the "
							"volume\n");
					return -EIO;
				}
		...
		/* Update lowest vcn. */
		a->data.non_resident.lowest_vcn = cpu_to_sle64(stop_vcn);
		mark_mft_record_dirty(ctx->ntfs_ino);
	...
	if (err && err != -ENOENT) {
		ntfs_error(sb, "%s: Attribute lookup failed", __FUNCTION__);
		goto put_err_out;
	}
	...
	if (finished_build) {
		...
		while (!(err = ntfs_attr_lookup(ni->type, ni->name, ni->name_len,
				CASE_SENSITIVE, 0, NULL, 0, ctx))) {
			if (sle64_to_cpu(ctx->attr->data.non_resident.highest_vcn) !=
		...
		if (err && err != -ENOENT) {
			ntfs_error(sb, "%s: Attr lookup failed", __FUNCTION__);
			goto put_err_out;
		}
...
int ntfs_attr_update_mapping_pairs(ntfs_inode *ni, VCN from_vcn)
{
	...
	ntfs_debug("Entering\n");
	ret = ntfs_attr_update_mapping_pairs_i(ni, from_vcn, HOLES_OK);
	ntfs_debug("\n");
	return ret;
}
 ...
 */
static int ntfs_attr_make_resident(ntfs_inode *ni, ntfs_attr_search_ctx *ctx)
{
	ntfs_volume *vol = ni->vol;
	struct super_block *sb = vol->sb;
	ATTR_REC *a = ctx->attr;
	...
	if (sle64_to_cpu(a->data.non_resident.lowest_vcn)) {
		ntfs_debug("Eeek!  Should be called for the first extent of the "
				"attribute.  Aborting...\n");
		return -EINVAL;
	}
	...
	if (!NInoNonResident(ni)) {
		ntfs_debug("Eeek!  Trying to make resident attribute resident.  "
				"Aborting...\n");
		return -EINVAL;
	}
	...
	if (NInoCompressed(ni) || NInoEncrypted(ni)) {
		ntfs_debug("Making compressed or encrypted files resident is not "
				"implemented yet.\n");
		return -EOPNOTSUPP;
	}
 ...
 */
static int ntfs_non_resident_attr_shrink(ntfs_inode *ni, const s64 newsize)
{
	ntfs_volume *vol;
	ntfs_attr_search_ctx *ctx;
	VCN first_free_vcn;
	...
	if ((ni->allocated_size >> vol->cluster_size_bits) != first_free_vcn) {
		...
		ctx = ntfs_attr_get_search_ctx(ni, NULL);
		if (!ctx) {
			ntfs_error(vol->sb, "%s: Failed to get search context", __FUNCTION__);
			return -ENOMEM;
		}
	...
	ctx->attr->data.non_resident.data_size = cpu_to_sle64(newsize);
	if (newsize < ni->initialized_size) {
		ni->initialized_size = newsize;
		ctx->attr->data.non_resident.initialized_size = cpu_to_sle64(newsize);
	}
 ...
 */
static int ntfs_non_resident_attr_expand(ntfs_inode *ni, const s64 newsize,
		const s64 prealloc_size, hole_type holes)
{
	LCN lcn_seek_from;
	VCN first_free_vcn;
	ntfs_volume *vol;
	ntfs_attr_search_ctx *ctx = NULL;
	runlist_element *rl, *rln;
	...
	err = ntfs_attr_size_bounds_check(vol, ni->type, newsize);
	if (err	< 0) {
		ntfs_error(sb, "%s: bounds check failed", __FUNCTION__);
		return err;
	}
	...
	if ((ni->allocated_size >> vol->cluster_size_bits) < first_free_vcn) {
		...
		if ((ni->type == AT_DATA && (vol->major_ver >= 3 || !NInoSparseDisabled(ni))) &&
		    (holes != HOLES_NO)) {
			rl = ntfs_malloc_nofs(sizeof(runlist_element) * 2);
			if (!rl) {
				err = -ENOMEM;
				goto put_err_out;
			}
			...
			if (IS_ERR(rl)) {
				ntfs_error(sb, "Cluster allocation failed "
						"(%lld)",
 ...
 */
static int ntfs_resident_attr_resize(ntfs_inode *attr_ni, const s64 newsize,
		const s64 prealloc_size, hole_type holes)
{
	ntfs_attr_search_ctx *ctx;
	ntfs_volume *vol = attr_ni->vol;
	struct super_block *sb = vol->sb;
	int err = -EIO;
	ntfs_inode *base_ni, *ext_ni = NULL;
...
attr_resize_again:
	...
	while (!(err = ntfs_attr_lookup(AT_UNUSED, NULL, 0, 0, 0, NULL, 0, ctx))) {
		 ...
		 */
		if (le32_to_cpu(a->length) <= (sizeof(ATTR_RECORD) - sizeof(s64)) +
	...
	/* Check whether error occurred. */
	if (err != -ENOENT) {
		ntfs_error(sb, "%s: Attribute lookup failed 1", __FUNCTION__);
		goto put_err_out;
	}
	...
	if (attr_ni->type == AT_STANDARD_INFORMATION ||
		...
		err = ntfs_inode_free_space(base_ni, sizeof(ATTR_RECORD));
		if (err) {
			err = -ENOSPC;
			ntfs_error(sb, "Couldn't free space in the MFT record to "
	...
	err = ntfs_attr_lookup(attr_ni->type, attr_ni->name, attr_ni->name_len,
			CASE_SENSITIVE, 0, NULL, 0, ctx);
	if (err) {
		ntfs_error(sb, "%s: Attribute lookup failed 2", __FUNCTION__);
		goto put_err_out;
	}
...
int ntfs_attr_map_cluster(ntfs_inode *ni, VCN vcn_start, LCN *lcn_start,
		LCN *lcn_count, VCN max_clu_count, bool *balloc, bool update_mp)
{
	ntfs_volume *vol = ni->vol;
	ntfs_attr_search_ctx *ctx;
	runlist_element *rl, *rlc;
	VCN vcn = vcn_start, lcn, clu_count;
	LCN lcn_seek_from = -1;
	int err = 0;
	size_t new_rl_count;
	...
	err = ntfs_attr_lookup(ni->type, ni->name, ni->name_len,
			CASE_SENSITIVE, vcn, NULL, 0, ctx);
	if (err) {
		ntfs_error(vol->sb, "ntfs_attr_lookup failed, ntfs inode(mft_no : %ld) type : 0x%x, err : %d",
	...
	if (update_mp) {
		...
		if (err) {
			...
			if (err2 < 0)
				ntfs_error(vol->sb,
					   "Failed to free cluster allocation. Leaving "
					   "inconstant metadata.\n");
			goto out;
		}
	}
...
int ntfs_attr_rm(ntfs_inode *ni)
{
	...
	if (NInoNonResident(ni)) {
		...
		if (ret < 0)
			ntfs_error(sb, "Failed to free cluster allocation. Leaving "
					"inconstant metadata.\n");
		ntfs_attr_put_search_ctx(ctx);
	}
	...
	while (!(err = ntfs_attr_lookup(ni->type, ni->name, ni->name_len,
		...
		err = ntfs_attr_record_rm(ctx);
		if (err) {
			ntfs_error(sb, "Failed to remove attribute extent. Leaving "
					"inconstant metadata.\n");
			ret = err;
		}
		ntfs_attr_reinit_search_ctx(ctx);
	...
	ntfs_attr_put_search_ctx(ctx);
	if (err != -ENOENT) {
		ntfs_error(sb, "Attribute lookup failed. Probably leaving inconstant "
				"metadata.\n");
		ret = err;
	}
		...
		u32 name_len)
{
	...
	if (IS_ERR(attr_vi)) {
		err = PTR_ERR(attr_vi);
		ntfs_error(sb, "Failed to open attribute 0x%02x of inode "
	...
	if (err)
		ntfs_error(sb, "Failed to remove attribute 0x%02x of inode "
				"0x%llx", type, (unsigned long long)ni->mft_no);
	iput(attr_vi);
	return err;
}
...
int ntfs_non_resident_attr_insert_range(ntfs_inode *ni, VCN start_vcn, VCN len)
{
	...
	ctx->attr->data.non_resident.data_size = cpu_to_sle64(ni->data_size);
	ctx->attr->data.non_resident.initialized_size = cpu_to_sle64(ni->initialized_size);
	if (ni->type == AT_DATA && ni->name == AT_UNNAMED)
		NInoSetFileNameDirty(ni);
	mark_mft_record_dirty(ctx->ntfs_ino);
	ntfs_attr_put_search_ctx(ctx);
...
int ntfs_non_resident_attr_collapse_range(ntfs_inode *ni, VCN start_vcn, VCN len)
{
	...
	len = min_t(VCN, len, end_vcn - start_vcn);
	down_write(&ni->runlist.lock);
	...
	ni->allocated_size -= len << vol->cluster_size_bits;
	// TODO: data_size should not be negative
...
int ntfs_non_resident_attr_punch_hole(ntfs_inode *ni, VCN start_vcn, VCN len)
{
	...
	len = min_t(VCN, len, end_vcn - start_vcn + 1);
	down_write(&ni->runlist.lock);
	for (rl = ni->runlist.rl, dst_cnt = 0; rl && rl->length; rl++)
		dst_cnt++;
	rl = ntfs_rl_find_vcn_nolock(ni->runlist.rl, start_vcn);
...
int ntfs_attr_fallocate(ntfs_inode *ni, loff_t start, loff_t byte_len, bool keep_size)
{
	...
	ntfs_attr_search_ctx *ctx;
	s64 old_data_size;
	VCN start_vcn, vcn_cnt_uninit, vcn_cnt, vcn, try_alloc_cnt;
	LCN lcn, alloc_cnt;
	int err = 0;
	runlist_element *rl;
	bool balloc;
	...
	start_vcn = (VCN)(start >> vol->cluster_size_bits);
	vcn_cnt = (VCN)(round_up(start + byte_len, vol->cluster_size) >>
			vol->cluster_size_bits) - start_vcn;
	vcn_cnt_uninit = (VCN)(round_up(ni->initialized_size, vol->cluster_size) /
			       vol->cluster_size);
	...
	while (vcn < vcn_cnt_uninit) {
		...
		if (rl->lcn > 0) {
			...
			vcn_start = vcn;
			lcn_total_cnt = 0;
			try_alloc_cnt = min_t(VCN, rl->length - (vcn_start - rl->vcn),
					      vcn_cnt_uninit - vcn_start);

			if (rl->lcn == LCN_DELALLOC) {
				vcn += (VCN)try_alloc_cnt;
				continue;
			}
			...
			while (try_alloc_cnt > 0) {
				...
				err = ntfs_attr_map_cluster(ni, vcn, &lcn, &alloc_cnt,
							    try_alloc_cnt, &balloc, false);
				up_write(&ni->runlist.lock);
				mutex_unlock(&ni->mrec_lock);
				if (err)
					goto out;

				err = blkdev_issue_zeroout(ni->vol->sb->s_bdev,
							   lcn << (ni->vol->cluster_size_bits - SECTOR_SHIFT),
							   alloc_cnt << (ni->vol->cluster_size_bits - SECTOR_SHIFT),
							   GFP_KERNEL, 0);
				if (err || signal_pending(current))
					goto out;

				vcn += (VCN)alloc_cnt;
				lcn_total_cnt += alloc_cnt;
				try_alloc_cnt -= (VCN)alloc_cnt;
			}
		} else {
			err = -EIO;
			goto out;
	...
	try_alloc_cnt = vcn_cnt - (vcn - start_vcn);
	while (try_alloc_cnt > 0) {
		mutex_lock(&ni->mrec_lock);
		down_write(&ni->runlist.lock);
		err = ntfs_attr_map_cluster(ni, vcn, &lcn, &alloc_cnt,
