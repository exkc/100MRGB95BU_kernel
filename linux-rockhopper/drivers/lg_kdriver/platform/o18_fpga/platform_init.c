...
static int 	tiny_hdr = 0;	/* support Dolby HDR in 1.5GB_STD model */
module_param_named(tiny_hdr, tiny_hdr, int, 0644 );
...
static LX_MEMCFG_DESC_T mem_desc[MEM_DESC_ID_MAX]	= {
	...
	DEFINE_MEM_DESC(BT_M2	,MEMCFG_DESC_FLAG_CMA),
	DEFINE_MEM_DESC(VT		,MEMCFG_DESC_FLAG_CMA),
	DEFINE_MEM_DESC(VT_M2	,MEMCFG_DESC_FLAG_CMA),
...
static LX_MEMCFG_ENTRY_T g_mem_desc_table_DDR_1_5_GB_STD[] =	/* default map */
{
...
#ifdef INCLUDE_KDRV_DE
	LX_MEMCFG_ENTRY_EX(gMemCfgDeFW[0], MEM_DESC(DE_FW)),	/* DE FW(2MB) */
	LX_MEMCFG_ENTRY_EX(gMemCfgDePreW[0], MEM_DESC(DE_PREW)),/* DE PREW */
#endif

#ifdef INCLUDE_KDRV_VDEC
	LX_MEMCFG_ENTRY_EX(vdec0_hma, 	 MEM_DESC(VDEC0)),		/* VDEC HMA */
	LX_MEMCFG_ENTRY_EX(vdec1_hma,	 MEM_DESC(VDEC1)),		/* VDEC CMA */
#endif
...
#ifdef INCLUDE_KDRV_DE
	LX_MEMCFG_ENTRY_EX(gMemCfgDe[1], MEM_DESC(DE)),		/* DE (FW+FRAME) for 60Hz Display */
	LX_MEMCFG_ENTRY_EX(gMemCfgBT_CMA[0], MEM_DESC(BT)),		/* BT speaker */
	LX_MEMCFG_ENTRY_EX(gMemCfgVT_CMA[0], MEM_DESC(VT)),		/* 2k frame buffer for VT */
	LX_MEMCFG_ENTRY_EX(gMemCfgHDR[0], MEM_DESC(HDR)),		/* HDR */
#endif
    ...
    LX_MEMCFG_ENTRY_EX(gMemCfgBeFrc[1], MEM_DESC(BE_FRC)),	/* BE FRC */
#endif
...
static LX_MEMCFG_ENTRY_T g_mem_desc_table_DDR_2_0_GB_STD[] = /* 2.0GB STD */
{
...
#ifdef INCLUDE_KDRV_DE
	LX_MEMCFG_ENTRY_EX(gMemCfgDeFW[0], MEM_DESC(DE_FW)),	/* DE FW(2MB) */
	LX_MEMCFG_ENTRY_EX(gMemCfgDePreW[1], MEM_DESC(DE_PREW)),/* DE PREW */
#endif
	...
	LX_MEMCFG_ENTRY_EX(gMemCfgDe[1], MEM_DESC(DE)),		/* DE (FW+FRAME) for 60Hz Display */
	LX_MEMCFG_ENTRY_EX(gMemCfgBT_CMA[0], MEM_DESC(BT)),		/* BT speaker */
	LX_MEMCFG_ENTRY_EX(gMemCfgVT_CMA[1], MEM_DESC(VT)),		/* 4k frame buffer for VT */
	LX_MEMCFG_ENTRY_EX(gMemCfgHDR[0], MEM_DESC(HDR)),		/* HDR */
#endif
...
static LX_MEMCFG_ENTRY_T g_mem_desc_table_DDR_2_0_GB_ADV[] =	/* 2.0GB ADV */
{
...
#ifdef INCLUDE_KDRV_DE
	LX_MEMCFG_ENTRY_EX(gMemCfgDeFW[1], MEM_DESC(DE_FW)),	/* DE FW(4MB)_M0 */
...
#endif
#ifdef INCLUDE_KDRV_BE
    LX_MEMCFG_ENTRY_EX(gMemCfgBeFrc[2], MEM_DESC(BE_FRC)),	/* BE FRC */
#endif
...
#ifdef INCLUDE_KDRV_DE
#ifdef O18_PQE_NEW_MEM
	LX_MEMCFG_ENTRY_EX(gMemCfgDeFW[2], MEM_DESC(DE_FW)),	/* DE FW(4MB)_M1 */
	...
	LX_MEMCFG_ENTRY_EX(gMemCfgO18[7], MEM_DESC(BT_M2)), 	/* DE_BT_M2 */

	LX_MEMCFG_ENTRY_EX(gMemCfgO18[8], MEM_DESC(VT)),		/* DE_VT_M1 */
	LX_MEMCFG_ENTRY_EX(gMemCfgO18[9], MEM_DESC(VT_M2)),		/* DE_VT_M2 */
#else
	LX_MEMCFG_ENTRY(gMemCfgDe[3]),                          /* DE (FW+FRAME) */
	LX_MEMCFG_ENTRY(gMemCfgDeFrameC[0]),                    /* DE (Chroma)   */ 
	LX_MEMCFG_ENTRY_EX(gMemCfgBT_CMA[1], MEM_DESC(BT)),		/* BT speaker  */
	LX_MEMCFG_ENTRY_EX(gMemCfgVT_CMA[3], MEM_DESC(VT)),		/* 4k frame buffer for VT */
#endif	
#ifdef HDR_TEST
	LX_MEMCFG_ENTRY_EX(gMemCfgHDR[0], MEM_DESC(HDR)),		/* HDR */
#endif
...
#ifdef INCLUDE_KDRV_BE
    LX_MEMCFG_ENTRY_EX(gMemCfgBe[5], MEM_DESC(BE)),			/* BE */
    LX_MEMCFG_ENTRY_EX(gMemCfgBeFrc[3], MEM_DESC(BE_FRC)),	/* BE FRC */
    LX_MEMCFG_ENTRY_EX(gMemCfgBeFrc[4], MEM_DESC(BE_FRC)),	/* BE FRC */
    LX_MEMCFG_ENTRY_EX(gMemCfgBeFrc[5], MEM_DESC(BE_FRC)),	/* BE FRC */
#endif
...
static void BASE_PLT_SetupCommon(void)
{
    ...
    {
        case MODEL_OPT_DDR_1_5_GB_STD: vdec0_hma.size = (tiny_hdr)? UHD_VDEC_4K_TINY_HDR_HMA0_SZ: UHD_VDEC_4K_HMA_SZ;
									   vdec1_hma.size = (tiny_hdr)? UHD_VDEC_4K_TINY_HDR_HMA1_SZ: 0; break;
...
static void BASE_PLT_Setup_1_5_GB_STD(void)
{
	...
	gMemCfgHdr[DDR_RGN_LBUS_DEV].base 	= (tiny_hdr)? UHD_LM_1_5GB_TINY_HDR_DEV_RGN_BASE: UHD_LM_1_5GB_DEV_RGN_BASE;
	gMemCfgHdr[DDR_RGN_LBUS_DEV].size 	= (tiny_hdr)? UHD_LM_1_5GB_TINY_HDR_DEV_RGN_SZ  : UHD_LM_1_5GB_DEV_RGN_SZ;
	gMemCfgHdr[DDR_RGN_GBUS_DEV].base 	= (tiny_hdr)? UHD_GM_1_5GB_TINY_HDR_DEV_RGN_BASE: UHD_GM_1_5GB_DEV_RGN_BASE;
	gMemCfgHdr[DDR_RGN_GBUS_DEV].size 	= (tiny_hdr)? UHD_GM_1_5GB_TINY_HDR_DEV_RGN_SZ  : UHD_GM_1_5GB_DEV_RGN_SZ;
...
static void BASE_PLT_Setup_2_0_GB_STD(void)
{
	...
	gMemCfgHdr[DDR_RGN_LBUS_DEV].base 	= UHD_LM_2_0GB_DEV_RGN_BASE;
	gMemCfgHdr[DDR_RGN_LBUS_DEV].size 	= UHD_LM_2_0GB_DEV_RGN_SZ;
	gMemCfgHdr[DDR_RGN_GBUS_DEV].base 	= UHD_GM_2_0GB_DEV_RGN_BASE;
	gMemCfgHdr[DDR_RGN_GBUS_DEV].size 	= UHD_GM_2_0GB_DEV_RGN_SZ;
...
static void BASE_PLT_Setup_3_0_GB_STD(void)
{
	...
	gMemCfgHdr[DDR_RGN_LBUS_DEV].base 	= UHD_LM_3_0GB_DEV_RGN_BASE;
	gMemCfgHdr[DDR_RGN_LBUS_DEV].size 	= UHD_LM_3_0GB_DEV_RGN_SZ;
	gMemCfgHdr[DDR_RGN_GBUS_DEV].base 	= UHD_GM_3_0GB_DEV_RGN_BASE;
	gMemCfgHdr[DDR_RGN_GBUS_DEV].size 	= UHD_GM_3_0GB_DEV_RGN_SZ;
...
static void BASE_PLT_DoPostMemoryLayout(void)
{
	...
	switch(g_model_ctx.ddr_size)
	{
		case MODEL_OPT_DDR_1_5_GB_STD:	ddr0_section[0].size = (tiny_hdr)? __MB(228):__MB(228); break;

		case MODEL_OPT_DDR_3_0_GB_STD:
		case MODEL_OPT_DDR_3_0_GB_ADV:	ddr0_section[0].size = __MB(436); break;

		case MODEL_OPT_DDR_2_0_GB_STD:
		case MODEL_OPT_DDR_2_0_GB_ADV:
		default:						ddr0_section[0].size = __MB(332); break;
	}
	...
	if (tiny_hdr)
	{
		ret = hma_pool_register_type ("dpb2", vdec1_hma.base, vdec1_hma.size, CMA_POOL);
		if (ret < 0)
		{
			printk (KERN_ERR "cannot register \"%s\" pool\n", "dpb2");
		}
	...
	if (0)	
	{
		...
		if (RET_OK == OS_ScanKernelCmdline("hwopt=%s",hwopt_txt))
		{
			...
			switch(hwopt_txt[11])
			{
				...
				case '6': g_model_ctx.ddr_size	= (hdmi_tx_output)? MODEL_OPT_DDR_3_0_GB_STD:MODEL_OPT_DDR_3_0_GB_ADV; break;
				case '4':
				case '7':
				default: g_model_ctx.ddr_size	= (hdmi_tx_output)? MODEL_OPT_DDR_2_0_GB_STD:MODEL_OPT_DDR_2_0_GB_ADV; break;	/* '4' OR '7' */
			}
	...
	for (i=0; i<NELEMENTS(pioneer_board_fmtstr); i++)
	{
		if (RET_OK == OS_ScanKernelCmdline(pioneer_board_fmtstr[i], optstr))
		{
			...
			while(tok)
			{
				...
				if(!strncasecmp(tok, "2.0GB_ADV",9)) 	g_model_ctx.ddr_size    = (hdmi_tx_output)?MODEL_OPT_DDR_2_0_GB_STD:MODEL_OPT_DDR_2_0_GB_ADV;
				if(!strncasecmp(tok, "3.0GB_ADV",9))	g_model_ctx.ddr_size    = (hdmi_tx_output)?MODEL_OPT_DDR_3_0_GB_STD:MODEL_OPT_DDR_3_0_GB_ADV;
