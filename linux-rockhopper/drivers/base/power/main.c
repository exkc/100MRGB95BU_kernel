...
int dpm_suspend(pm_message_t state)
{
	...
	if (IS_ENABLED(CONFIG_ARCH_LG1K)) {
		struct device *dev, *tmpdev;
		struct list_head user_resume_list;

		INIT_LIST_HEAD(&user_resume_list);
		list_for_each_entry_safe(dev, tmpdev,
				&dpm_prepared_list, power.entry) {
			if (is_userresume(dev))
				list_move_tail(&dev->power.entry,
						&user_resume_list);
		}
		/* suspend user-resume devices before normal ones */
		list_splice_tail(&user_resume_list, &dpm_prepared_list);
	}
