...
int _DEMOD_O18_DVBS_CheckTuneMode(DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
	...
	switch (pDemodInfo->setCfgParam.dvbs.tuneMode)
	{
		...
		default:			
		{
			printk("[_DEMOD_O18_DVBS_CheckTuneMode] ///// CheckTuneMode tuneMode = %x ///// \n", pDemodInfo->setCfgParam.dvbs.tuneMode);	//woohyung
...
int _DEMOD_O18_DVBS2_CheckTuneMode(DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
	...
	switch (pDemodInfo->setCfgParam.dvbs2.tuneMode)
	{
		...
		default:
		{
			printk("[O18 _DEMOD_O18_DVBS2_CheckTuneMode] ///// CheckTuneMode tuneMode = %x ///// \n", pDemodInfo->setCfgParam.dvbs2.tuneMode);	//woohyung
			return DEMOD_DD_MODE_PROP_TUNEMODE_AUTO; break;
		}
	}
	return new_tuneMode;
...
int _DEMOD_O18_DVBS_OperatingMode(DEMOD_CTX_T *pDemodInfo)
{
	UINT8  operatingMode;
...
int _DEMOD_O18_DVBS2_OperatingMode(DEMOD_CTX_T *pDemodInfo)
{
	UINT8  operatingMode;
...
int _DEMOD_O18_LGDBB_DVBS_SetDemod(DEMOD_CTX_T *pDemodInfo)
{
	UINT8  					standard;
	UINT32 					freq_Khz;
//	UINT16 					dvb_s_bandwidth_Khz;
	...
	printk("[O18 LGDBB]  SetDemod DVBS \n");

	pFrontend				= &gO18_LGDBB_Frontend;
	standard 					= DEMOD_DD_MODE_PROP_MODULATION_DVBS;
	new_tuneMode 			= _DEMOD_O18_DVBS_CheckTuneMode(pDemodInfo);
	operatingMode 			= _DEMOD_O18_DVBS_OperatingMode(pDemodInfo);
	symbol_rate_Kbps 			= (UINT16)(pDemodInfo->setCfgParam.dvbs.symbolRate);// KSym/s
	freq_Khz 					= pDemodInfo->setCfgParam.dvbs.frequency;

	printk("[O18 _DEMOD_O18_LGDBB_DVBS_SetDemod] ///// tuneMode = %x ///// \n", new_tuneMode);	//woohyung
	printk("[O18 _DEMOD_O18_LGDBB_DVBS_SetDemod] ///// operatingMode = %x ///// \n", operatingMode);	//woohyung
...
int _DEMOD_O18_LGDBB_DVBS2_SetDemod(DEMOD_CTX_T *pDemodInfo)
{
	UINT8  					standard;
	UINT32 					freq_Khz;
//	UINT16 					dvb_s2_bandwidth_Khz;
	...
	printk("[O18 LGDBB]  SetDemod DVBS2 \n");

	pFrontend				= &gO18_LGDBB_Frontend;
	standard 					= DEMOD_DD_MODE_PROP_MODULATION_DVBS2;
	new_tuneMode 			= _DEMOD_O18_DVBS2_CheckTuneMode(pDemodInfo);
	operatingMode 			= _DEMOD_O18_DVBS2_OperatingMode(pDemodInfo);
	symbol_rate_Kbps 			= (UINT16)(pDemodInfo->setCfgParam.dvbs2.symbolRate);// KSym/s
	freq_Khz 					= pDemodInfo->setCfgParam.dvbs2.frequency;

	printk("[O18 _DEMOD_O18_LGDBB_DVBS2_SetDemod] ///// tuneMode = %x ///// \n", new_tuneMode);	//woohyung
	printk("[O18 _DEMOD_O18_LGDBB_DVBS2_SetDemod] ///// operatingMode = %x ///// \n", operatingMode);	//woohyung
...
int _DEMOD_O18_LGDBB_SAT_CheckSignalState(DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context 		*pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O18_DVBS_Set_Config_auto
...
*/
int DEMOD_O18_DVBS_Set_Config_auto (BOOLEAN bAutoDetect)
{
/*
	LX_DEMOD_L2_Context *pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O18_DVBS_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when LX_DEMOD_O18_DVBS_Set_Config_auto is disabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O18_DVBS_Set_FullConfig (LX_DEMOD_DVBS_CONFIG_T *pDvbsConfigParam)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O18_DVBS_Get_CoderRate( LX_DEMOD_CODERATE_T *pCodeRate)
{
	LX_DEMOD_L2_Context 		*pFrontend;
	LX_DEMOD_L2_SIG_STATUS		signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O18_DVBS_Get_SymbolRate( UINT32 *pSymbolRate )
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O18_DVBS_Get_TotalInfo( void)
{
	UINT16 						ifagcValue = 0;
	UINT32 						errorCount;
	SINT32 						FreqOff;
	...
	DEMOD_PRINT("^p^[DEMOD O18] @ %s =====\n",__F__);

	if (RET_OK != DEMOD_O18_Get_Lock(LX_DEMOD_AGC_LOCK, &agcLockStatus))
	{
		DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_Get_Lock() !!!\n", __F__, __L__);
...
/*		
		...
		{
			DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_Get_IFAGC() !!!\n", __F__, __L__);
			ifagcValue = 0;
		}
*/
	}

	if (RET_OK != DEMOD_O18_Get_NeverLockStatus(LX_DEMOD_DVBS, &neverLockStatus))
	{
		DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_DVB_Get_NeverLockStatus() !!!\n", __F__, __L__);
		neverLockStatus = LX_DEMOD_UNKOWN_STATE;
	}

	if (RET_OK !=  DEMOD_O18_Get_SpectrumStatus(&spectrumAuto, &spectrumInv))
	{
		DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_Get_SpectrumStatus() !!!\n", __F__, __L__);
...
/*
	...
	{
		DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_DVBS_Get_TPError() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	errorCount 		= signalStatus.packetError;
	
	DEMOD_PRINT("[DEMOD O18] TP Error Count= %d !!!\n",errorCount);
/*
	if (RET_OK != DEMOD_O18_Get_CarrierFreqOffset( &FreqOff ))
	{
		DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
		FreqOff = 0xffff;
	}
*/
...
/*
	...
	{
		DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_DVBC_Get_SNR() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	sigMse.bMse 		= 0;
	...
	DEMOD_PRINT("[DEMOD O18] offset = %d [KHz]!!!\n",FreqOff);
	DEMOD_PRINT("[DEMOD O18] mse = %d, constPwr = %d, !!!\n",sigMse.mse, sigMse.constPwr);

/*	if (RET_OK != DEMOD_O18_DVBS_Get_CoderRate(&codeRate ))
	{
		DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_DVBS_Get_CoderRate() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	switch(signalStatus.codeRate)
...
/*	if (RET_OK != DEMOD_O18_DVBS_Get_SymbolRate(&symbolRate ))
	{
		DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_DVBS_Get_SymbolRate() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	...
	printk("[DEMOD O18] AGC LOCK is %s !!!\n",(agcLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	printk("[DEMOD O18] IF AGC value = %d !!!\n",ifagcValue);
	printk("[DEMOD O18] Neverlock is %s !!!\n",(neverLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	printk("[DEMOD O18] Spectrum mode = %s !!!\n",(spectrumInv == LX_DEMOD_NORMAL) ? " Normal" :
																	(spectrumInv == LX_DEMOD_INVERSION) ? "Inversion" : 	" Unknown");
	printk("[DEMOD O18] Code rate = %d !!!\n",codeRate);
	printk("[DEMOD O18] Symbol rate = %d !!!\n",symbolRate);


	printk("[DEMOD O18] TP Error Count= %d !!!\n",errorCount);
	printk("[DEMOD O18] MSE = %d, constPwr = %d !!!\n",sigMse.mse, sigMse.constPwr);
#endif
...
************************************************************************************/
/**
 * DEMOD_O18_DVBS_Set_DiseqcToneMessageCtrl
 * Get the information of symbol rate on DVB-S.
 *
 * @param *pLpCodeRate[OUT] LX_DEMOD_CODERATE_T - code rate : 1/2, 2/3, 3/4, 5/6 and 7/8.
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O18_DVBS_Set_DiseqcToneMessageCtrl( LX_DEMOD_DVBS_DiSEqC_MSG_T *pDiseqcMsg)
{
	LX_DEMOD_L2_Context 	*pFrontend;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O18_DVBS_Get_BlindInfo(UINT32 centerFreq, LX_DEMOD_DVBS_BLIND_PARAM_T *pBlindParam)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O18_DVBS_Obtaining_Signal_Lock.
...
int DEMOD_O18_DVBS_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				if(RET_OK != _DEMOD_O18_LGDBB_DVBS_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD O18 LGDBB] FAIL (%s:%d) _DEMOD_O18_LGDBB_DVBT_SetDemod() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(pDemodInfo->setCfgParam.dvbs.operatingMode == LX_DEMOD_DVBS_BLIND_SCAN_MODE)
				{
					...
					DEMOD_PRINT( "^R^[DEMOD O18] LX_DEMOD_TUNE_START : Blind Spectrum Mode..Going to LX_DEMOD_TUNE_SCAN_FINISH_UNLOCK..... ^0^\n");
					break;
				}
		...
		case LX_DEMOD_TUNE_SCAN:			/***************************************************/
			{
				...
				if((LX_DEMOD_FLAG_COMPLETE == pDemodInfo->scanJobFlag) || (LX_DEMOD_FLAG_FINISH == pDemodInfo->scanJobFlag))
				{
					...
					DEMOD_PRINT( "^c^[DEMOD O18] ERR (%s:%d) Already Done !!!\n", __F__, __L__);
					DEMOD_PRINT( "^R^[DEMOD O18] LX_DEMOD_TUNE_SCAN : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
					return RET_OK;
				}

				if(RET_OK != DEMOD_O18_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_OFF, &scanProcessing, &scanLock))
				{
					DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_NeverlockScan() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if((LX_DEMOD_FLAG_FINISH == scanProcessing) && (LX_DEMOD_LOCKED == scanLock))
				{
					...
					DEMOD_PRINT("[DEMOD O18] (%s:%d) operMode(%d) !!!\n", __F__, __L__,operMode);
					if(LX_DEMOD_DVBS2 == operMode)
					{
						...
						DEMOD_PRINT( "^G^[DEMOD O18] (%s:%d) Signal is Found (LX_DEMOD_DVBS2)  !!!\n", __F__, __L__);
						DEMOD_PRINT( "^R^[DEMOD O18] LX_DEMOD_TUNE_SCAN : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
					}
					else //(LX_DEMOD_DVBS == operMode)
					{
						...
						DEMOD_PRINT( "^G^[DEMOD O18] (%s:%d) Signal is Found (LX_DEMOD_DVBS)  !!!\n", __F__, __L__);
						DEMOD_PRINT( "^R^[DEMOD O18] LX_DEMOD_TUNE_SCAN : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
					}
				}
				...
				else
				{
					...
					DEMOD_PRINT( "^G^[DEMOD O18] (%s:%d) Detection is on processing	!!!\n", __F__, __L__);
				}
			}
			break;
													/********************************************/
...
//				BOOLEAN reacqflagOn;
				...
				DEMOD_PRINT("[DEMOD O18] LX_DEMOD_SYNC_CHECKING() operMode = 0x%x!!!\n", operMode);
				...
				if(LX_DEMOD_DVBS2 == operMode)
				{
					...
					DEMOD_PRINT( "^R^[DEMOD O18] LX_DEMOD_SYNC_CHECKING(DVBS) : Going to LX_DEMOD_SYNC_CHECKING(DVBS2 forcely)..... ^0^\n");
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					DEMOD_PRINT( "^R^[DEMOD O18] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_SIGNAL_STABLE..... ^0^\n");
				}
				else
				{
...
//				UINT32 							packetError;
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					{
						DEMOD_PRINT( "^r^[DEMOD O18] FAIL (%s:%d) DEMOD_O18_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
					...
					DEMOD_PRINT_ALWAYS( "[DEMOD O18] LX_DEMOD_SIGNAL_STABLE : MSG_DVBS_FRONT_END_LOCKED..... ^^^0^^^\n");
					...
					{
						DEMOD_PRINT( "^r^[DEMOD O18] FAIL (%s:%d) DEMOD_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if (RET_OK != _DEMOD_O18_LGDBB_SAT_CheckSignalState(pDemodInfo))
					{
				...
				{
//					pDemodInfo->controlState	= LX_DEMOD_SYNC_CHECKING;
//					DEMOD_PRINT( "^R^[DEMOD O18] LX_DEMOD_SYNC_CHECKING : Going back to LX_DEMOD_SYNC_CHECKING..... **;\n");
				}
	   		}
			break;
...
************************************************************************************/
/**
 * DEMOD_O18_DVBS_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O18_DVBS_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_LOCK_STATE_T			lockState;
	LX_DEMOD_OPER_MODE_T 			operMode;
	...
	if(RET_OK != DEMOD_O18_Get_Lock(LX_DEMOD_PSYNC_LOCK, &lockState))
	{
		DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_Get_Lock() !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}
	
#if 1
	DEMOD_PRINT( " [DEMOD_O18_DVBS_Monitoring_Signal_Lock]pDemodInfo->lockStatus = %d, pDemodInfo->bNotifiedLock = %d, (lockCount = %d) \n",pDemodInfo->lockStatus,pDemodInfo->bNotifiedLock,pDemodInfo->lockCount);
	DEMOD_PRINT( " [DEMOD_O18_DVBS_Monitoring_Signal_Lock]pDemodInfo->lockStatus = %d, pDemodInfo->bNotifiedUnlock = %d,(unlockCnt = %d) \n",pDemodInfo->lockStatus,pDemodInfo->bNotifiedUnlock,pDemodInfo->unLockCount);
	DEMOD_PRINT( " [DEMOD_O18_DVBS_Monitoring_Signal_Lock]lockState = %d) \n",lockState);
#endif
...
*/
		...
		if((operMode == LX_DEMOD_DVBS) || (operMode == LX_DEMOD_DVBS2))
		{
			if(operMode != LX_DEMOD_DVBS)
			{
				...
				DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) forcely changed to DVBS2 mode \n",__F__, __L__);
				DEMOD_PRINT("[DEMOD O18] (%s:%d) tuneMode(DVBS2)  = %d \n",__F__, __L__, pDemodInfo->setCfgParam.dvbs2.tuneMode);
			}

		}
		else
		{
			DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) Invaild operation mode !! \n",__F__, __L__);
			return RET_ERROR;
		}
		...
		if(pDemodInfo->lockCount == 2)
		{
			...
			DEMOD_PRINT_ALWAYS( "[DEMOD O18] MONITORING FUNC : MSG_DVBS_FRONT_END_LOCKED..... ^0^\n");
	...
	else
	{
		...
		if((LX_DEMOD_UNLOCKED != pDemodInfo->lockStatus)  && (TRUE != pDemodInfo->bNotifiedUnlock))
		{
			...
			if(pDemodInfo->unLockCount == UNLOCKCOUNT)
			{
				...
				pDemodInfo->unLockCount 			= 0;
				DEMOD_PRINT_ALWAYS( "[DEMOD O18] MONITORING FUNC : MSG_DVBS_FRONT_END_NOT_LOCKED..... -_-\n");
			}
		}
	}
	return RET_OK;
...
************************************************************************************/
/**
 * DEMOD_O18_DVBS2_Set_Config_auto
 * Control automatic setting of DVB-T config.
 *
 * @param bAutoDetect 	[IN] BOOLEAN - automatic setting (TRUE) / manual setting(FALSE).
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O18_DVBS2_Set_Config_auto (BOOLEAN bAutoDetect)
{
	LX_DEMOD_L2_Context 	*pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O18_DVBS2_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when LX_DEMOD_O18_DVBS_Set_Config_auto is disabled.
 *
 * @param *pDvbtConfigParam 	[IN] LX_DEMOD_DVBS_CONFIG_T - DVB-T config. structure
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O18_DVBS2_Set_FullConfig (LX_DEMOD_DVBS2_CONFIG_T *pDvbs2ConfigParam)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O18_DVBS2_Get_CoderRate( LX_DEMOD_CODERATE_T *pCodeRate )
{
	LX_DEMOD_L2_Context 		*pFrontend;
	LX_DEMOD_L2_SIG_STATUS		signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O18_DVBS2_Get_Pilot( BOOLEAN *pPilot )
{
	*pPilot = FALSE;
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O18_DVBS2_Get_SymbolRate
 * Get the information of symbol rate on DVB-S.
 *
 * @param *pLpCodeRate[OUT] LX_DEMOD_CODERATE_T - code rate : 1/2, 2/3, 3/4, 5/6 and 7/8.
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O18_DVBS2_Get_SymbolRate( UINT32 *pSymbolRate )
{
	LX_DEMOD_L2_Context 		*pFrontend;
	LX_DEMOD_L2_SIG_STATUS		signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O18_DVBS2_Get_TotalInfo( void)
{
	UINT16 						ifagcValue = 0;
	UINT32 						errorCount;
	SINT32 						FreqOff;
...
*/
	...
	if (RET_OK != DEMOD_O18_Get_NeverLockStatus(LX_DEMOD_DVBS2, &neverLockStatus))
	{
		DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_DVB_Get_NeverLockStatus() !!!\n", __F__, __L__);
		neverLockStatus = LX_DEMOD_UNKOWN_STATE;
	}
...
/*
	if (RET_OK !=  DEMOD_O18_Get_Packet_Error(&errorCount))
	{
		DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_Get_Packet_Error() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	errorCount 		= signalStatus.packetError;
...
*/
	FreqOff 			= signalStatus.offsetFrq_Hz/1000;	
	DEMOD_PRINT("[DEMOD O18] Frequency Offset = %d !!!\n",FreqOff);
/*
	if (RET_OK != DEMOD_O18_Get_MseInfo(&sigMse))
	{
		DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) DEMOD_O18_Get_MseInfo() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	sigMse.bMse 		= 0;
...
//	DEMOD_PRINT("[DEMOD O18] AGC LOCK is %s !!!\n",(agcLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD O18_%s] IF AGC value = %d(0x%x) !!!\n",__F__,ifagcValue,ifagcValue);
	DEMOD_PRINT("[DEMOD O18_%s] Neverlock is %s !!!\n",__F__,(neverLockStatus == LX_DEMOD_DETECTED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD O18_%s] Spectrum auto = %s, Inversion  = %s !!!\n",__F__,(spectrumAuto == LX_DEMOD_AUTO) ? " Auto" : " Manual",
																					(spectrumInv == LX_DEMOD_INVERSION) ? " Inverted" : " Normal" );
	DEMOD_PRINT("[DEMOD O18_%s] offset = %d [KHz]!!!\n",__F__,FreqOff);
	DEMOD_PRINT("[DEMOD O18_%s] mse = %d, constPwr = %d, !!!\n",__F__,sigMse.mse, sigMse.constPwr);


#ifndef  DEMOD_PRINT
	printk("[DEMOD O18_%s] IF AGC value = %d !!!\n",__F__,ifagcValue);
	printk("[DEMOD O18_%s] Neverlock is %s !!!\n",__F__,(neverLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	printk("[DEMOD O18_%s] Spectrum mode = %s !!!\n",__F__,(spectrumInv == LX_DEMOD_NORMAL) ? " Normal" :
																	(spectrumInv == LX_DEMOD_INVERSION) ? "Inversion" : 	" Unknown");

	printk("[DEMOD O18_%s] TP Error Count= %d !!!\n",__F__,errorCount);
	printk("[DEMOD O18_%s] MSE = %d, constPwr = %d !!!\n",__F__,sigMse.mse, sigMse.constPwr);
#endif

	return RET_OK;
}

int DEMOD_O18_DVBS2_Set_MemSconfig(void)
{
	...
	{
		printk("######[DEMOD O18_%s] Error O18 DEMOD DVB_S2 Wrong Memory Base Address ##### !!!!!!!!!!!!!!!!!!!!!!!\n",__F__);
	}
	else
	{
		printk("######[DEMOD O18_%s] first call DEMOD_O18_DVBS2_Set_MemSconfig TDI_addr[0x%x] , DJB_addr[0x%x] ##### !!!!!!\n", __F__,tdi_st_mem, djb_st_mem );
...
************************************************************************************/
/**
 * DEMOD_O18_DVBS2_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O18_DVBS2_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	static LX_DEMOD_FLAG_T 			scanProcessing = LX_DEMOD_FLAG_START;
	static LX_DEMOD_LOCK_STATE_T 	scanLock = LX_DEMOD_UNLOCKED;
	static unsigned int 					sigStablestartTime = 0;
	...
	switch (pDemodInfo->controlState)
	{
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				if(RET_OK != _DEMOD_O18_LGDBB_DVBS2_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD O18 LGDBB] FAIL (%s:%d) _DEMOD_O18_LGDBB_DVBT_SetDemod() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
		...
		case LX_DEMOD_TUNE_SCAN:			/***************************************************/
			{
				...
				if((LX_DEMOD_FLAG_FINISH == scanProcessing) && (LX_DEMOD_LOCKED == scanLock))
				{
					...
					DEMOD_PRINT("[DEMOD O18] DEMOD_O18_NeverlockScan() operMode = 0x%x!!!\n", operMode);
					...
					if(LX_DEMOD_DVBS == operMode)
					{
						...
						DEMOD_PRINT( "^G^[DEMOD O18] (%s:%d) Signal is Found (operMode = LX_DEMOD_DVBS  !!!\n", __F__, __L__);
						DEMOD_PRINT( "^R^[DEMOD O18] LX_DEMOD_TUNE_SCAN : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
					}
					else //(LX_DEMOD_DVBS2 == operMode)
					{
						...
						DEMOD_PRINT( "^G^[DEMOD O18] (%s:%d) Signal is Found (operMode = LX_DEMOD_DVBS2  !!!\n", __F__, __L__);
						DEMOD_PRINT( "^R^[DEMOD O18] LX_DEMOD_TUNE_SCAN : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
					}
...
//				BOOLEAN reacqflagOn;
				...
				if(LX_DEMOD_DVBS == operMode)
				{
					...
					DEMOD_PRINT( "^R^[DEMOD O18] LX_DEMOD_SYNC_CHECKING(DVBS2) : Going to LX_DEMOD_SYNC_CHECKING(DVBS forcely)..... ^0^\n");
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.dvbs2.tuneMode)
					{
						...
						else
						{
							...
							else
							{
								...
								{
									DEMOD_PRINT( "^R^[DEMOD O18] LX_DEMOD_SIGNAL_STABLE : Packet error = %d .....  --_--\n", packetError);
									elapsedTime = jiffies_to_msecs(jiffies) - sigStablestartTime;
...
************************************************************************************/
/**
 * DEMOD_O18_DVBS2_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O18_DVBS2_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_LOCK_STATE_T			lockState;
	LX_DEMOD_OPER_MODE_T 			operMode;
...
*/
		...
		if((operMode == LX_DEMOD_DVBS) || (operMode == LX_DEMOD_DVBS2))
		{
			if(operMode != LX_DEMOD_DVBS2)
			{
				...
				DEMOD_PRINT("[DEMOD O18] FAIL (%s:%d) forcely changed to DVBS mode \n",__F__, __L__);
				DEMOD_PRINT("[DEMOD O18] (%s:%d) tuneMode (DVBS)  = %d \n",__F__, __L__, pDemodInfo->setCfgParam.dvbs.tuneMode);
			}
		}
		else
		{
...
************************************************************************************/
/**
 * DEMOD_O18_DVBSX_BlindScan_Init
...
*/
int DEMOD_O18_DVBSX_BlindScan_Init(LX_DEMOD_BLIND_SCAN_INIT_FREQ_PARAM_T *initFreq)
{
	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD O18] DEMOD_O18_DVBSX_BlindScan_Init");

	pFrontend = &gO18_LGDBB_Frontend;

	DEMOD_PRINT("[DEMOD O18] DEMOD_O18_DVBSX_BlindScan_Init startFreqKhz : %d, endFreqKhz : %d", initFreq->startFreqKhz, initFreq->endFreqKhz);
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_O18_DVBSX_BlindScan_End(void)
{
	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD O18] DEMOD_O18_DVBSX_BlindScan_End");
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_O18_DVBSX_Get_TunerFreq(LX_DEMOD_BLIND_SCAN_FREQ_PARAM_T *freqInfo)
{
	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD O18] DEMOD_O18_DVBSX_Get_TunerFreq");
	...
	DEMOD_PRINT("[DEMOD_O18_DVBSX_Get_TunerFreq] CenterFreq= %d , BandWidth = %d \n",freqInfo->TunerCenterFreqKhz, freqInfo->TunerCutOffFreqKhz);
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O18_DVBSX_Get_OffsetFreq
 * Get Tuner Freq infomration on DVB-S/S2.
 *
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_O18_DVBSX_Get_OffsetFreq(LX_DEMOD_BLIND_SCAN_OFFSET_PARAM_T *pOffsetInfo)
{
	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD O18] DEMOD_O18_DVBSX_Get_OffsetFreq");
	...
	DEMOD_PRINT("[DEMOD BlindScan DEMOD_O18_DVBSX_Get_OffsetFreq] Frequency = %d \n", pOffsetInfo->FreqMhz);
	DEMOD_PRINT("[DEMOD BlindScan DEMOD_O18_DVBSX_Get_OffsetFreq] BandWidth = %d \n", pOffsetInfo->SymbolRateKhz);
	return RET_OK;
...
************************************************************************************/
/**
 * DEMOD_O18_DVBSX_SymbolRate_Detec
 * Notification Blind Scan Init on DVB-S/S2.
 *
 * @param None
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_O18_DVBSX_SymbolRate_Detec(void)
{	
	UINT8 return_code = RET_ERROR;

	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD O18] DEMOD_O18_DVBSX_SymbolRate_Detec");
...
************************************************************************************/
/**
 * DEMOD_O18_DVBSX_BlindLock_Loop
 * Notification Blind Scan Init on DVB-S/S2.
 *
 * @param None
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_O18_DVBSX_BlindLock_Loop(void)
{	
	UINT8 return_code = RET_ERROR;

	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD O18] DEMOD_O18_DVBSX_BlindLock_Loop");
...
************************************************************************************/
/**
 * DEMOD_O18_DVBSX_Get_Nextfreq_Status
 * Get Tuner Freq infomration on DVB-S/S2.
 *
 * @param SINT32* freqOff
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_O18_DVBSX_Get_Nextfreq_Status(BOOLEAN* pBlindScanEnd)
{
	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD O18] DEMOD_O18_DVBSX_Get_Nextfreq_Status");
	...
	DEMOD_PRINT("[DEMOD O18 BlindScan DEMOD_O18_DVBSX_Get_Nextfreq_Status] pBlindScanEnd = %d \n", *pBlindScanEnd);
