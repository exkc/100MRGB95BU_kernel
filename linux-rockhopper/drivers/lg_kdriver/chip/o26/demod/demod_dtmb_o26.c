 ...
 ************************************************************************************/
static const UINT8  _DTMB_Unlock_Count_Threshold = 4;
...
static int _DEMOD_O26_DTMB_CheckTuneMode (DEMOD_CTX_T *pDemodInfo)
{
...
static int _DEMOD_O26_DTMB_SetDemod (DEMOD_CTX_T *pDemodInfo)
{
	...
	tuneMode = _DEMOD_O26_DTMB_CheckTuneMode(pDemodInfo);

	if(RET_OK != DEMOD_O26_Get_BW(pDemodInfo, &dtmb_bandwidth_Khz))
	{
...
static int _DEMOD_O26_DTMB_CheckSignalState (DEMOD_CTX_T *pDemodInfo)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int DEMOD_O26_DTMB_SW_Init (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O26_DTMB_Get_CarrierMode
...
*/
int DEMOD_O26_DTMB_Get_CarrierMode (LX_DEMOD_CARRIERMODE_T *pCarrierMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int DEMOD_O26_DTMB_Get_Time_Interval (BOOLEAN *pbM720)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int DEMOD_O26_DTMB_Get_Pn_Mode (LX_DEMOD_PNMODE_T *pPnMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int DEMOD_O26_DTMB_Get_CodeRate (LX_DEMOD_CODERATE_T *pCodeRate)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
int DEMOD_O26_DTMB_Get_Constellation (LX_DEMOD_RF_MODE_T *pConstellation)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
...
************************************************************************************/
/**
 * DEMOD_O26_DTMB_Obtaining_Signal_Lock.
...
int DEMOD_O26_DTMB_Obtaining_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
...
//	static unsigned int 		sigStablestartTime 	= 0;
	...
	switch (pDemodInfo->controlState)
	{
		...
		case LX_DEMOD_TUNE_START:	/***************************************************/
			{
				if(RET_OK != _DEMOD_O26_DTMB_SetDemod(pDemodInfo))
				{
					DEMOD_ERROR("Fail, _DEMOD_O26_DTMB_SetDemod() \n");
					return RET_ERROR;
				}
		...
		case LX_DEMOD_SYNC_CHECKING : 	/********************************************/
			{
				...
				{
					DEMOD_PRINT("Fail, DEMOD_O26_Get_OperMode()\n");
					return RET_ERROR;
				}
				...
				{
					DEMOD_PRINT("Fail, DEMOD_O26_Get_Lock()\n");
					return RET_ERROR;
				}
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE: 	/********************************************/
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if (RET_OK != _DEMOD_O26_DTMB_CheckSignalState(pDemodInfo))
					{
						DEMOD_PRINT("Fail, _DEMOD_O26_DTMB_CheckSignalState()\n");
						return RET_ERROR;
					}
...
************************************************************************************/
/**
 * DEMOD_O26_DTMB_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_O26_DTMB_Monitoring_Signal_Lock (DEMOD_CTX_T *pDemodInfo)
{
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if(pDemodInfo->signalStatus.packetError >= _DTMB_Full_Pkerr_Threshold)
		{
			...
			if(_DTMB_Full_Pkerr_Count == _DTMB_Full_Pkerr_Count_Threshold)
			{
				...
				{
					DEMOD_PRINT("Fail, DEMOD_O26_SoftwareReset() \n");
					return RET_ERROR;
				}
		...
		else
		{
			...
			if(pDemodInfo->bIsTPoutEn == TRUE)
			{
				...
				{
					DEMOD_PRINT("Fail, DEMOD_O26_TPOutEnable(TRUE)\n");
					return RET_ERROR;
				}
			}
...
************************************************************************************/
/**
 * DEMOD_O26_DTMB_Set_Config_auto
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_DTMB_Set_Config_auto (BOOLEAN bAutoDetect)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 DEMOD_O26_DTMB_Set_PartialConfig
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_DTMB_Set_PartialConfig (LX_DEMOD_DTMB_CONFIG_T *pDtmbConfigParam)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int DEMOD_O26_DTMB_Set_MemSconfig (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
	...
	{
		DEMOD_PRINT("DEMOD_O26_DTMB_Set_MemSconfig Base_addr[0x%x]\n", base_st_mem );
