...
static OS_PROC_DESC_TABLE_T	_g_irb_e60_device_proc_table[] =
{
...
static int e60_write_proc( UINT32 procId, char* command )
{
	...
	switch( procId )
	{
		case PROC_ID_TX:
			...
			#if 0
			printk("vendor 0x%x opt 0x%x key 0x%x\n",vendor,opt,key);
			irb_e60_func->tx(vendor,opt,key);
			#endif
			break;
		default:
			ret = -1;
 ...
 * @see IRB_Init
*/
void e60_irb_proc_init (void)
{
	irb_e60_func = get_e60_irb_func();


	OS_PROC_CreateEntryEx ( IRB_MODULE, _g_irb_e60_device_proc_table,
										e60_read_proc,
										e60_write_proc );
}
 ...
 * @see IRB_Cleanup
*/
void e60_irb_proc_cleanup (void)
{
	OS_PROC_RemoveEntry( IRB_MODULE );
}

static irbs_proc_t e60_irb_proc_func =
{
	.init = e60_irb_proc_init,
	.cleanup = e60_irb_proc_cleanup,
};

irbs_proc_t* get_e60_irb_proc(void)
{
	return &e60_irb_proc_func;
}
