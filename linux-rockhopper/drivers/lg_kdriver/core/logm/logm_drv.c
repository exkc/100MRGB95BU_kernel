...
module_param_named(short_format, g_logm_short_format, int, 0644);
...
#ifdef CONFIG_COMPAT
static long LOGM_CompatIoctl(struct file *, UINT32, ULONG);
#endif
static ssize_t LOGM_Read(struct file *, char *, size_t, loff_t *);
static ssize_t LOGM_Write(struct file *, const char *, size_t, loff_t *);
static unsigned int LOGM_Poll(struct file *file, struct poll_table_struct *pt);
...
static int _logm_ioctl_mask_ctrl(unsigned long arg, unsigned int cmd);

static int _logm_ioctl_objmap_size(unsigned long arg, unsigned int cmd);

static int _logm_ioctl_obj_reg(unsigned long arg, unsigned int cmd);

static int _logm_ioctl_path_ctrl(unsigned long arg);

static int _logm_ioctl_user_write(unsigned long arg, unsigned int cmd);
...
#define lt(s,n)	{#s, sizeof(#s), LX_LOGM_LEVEL_##n, }
		lt(ERROR, ERROR),
		    lt(WARN, WARNING),
		    lt(NOTI, NOTI),
		    lt(INFO, INFO), lt(DEBUG, DEBUG), lt(TRACE, TRACE),
		    /* simple form */
		    lt(E, ERROR),
		    lt(W, WARNING),
		    lt(N, NOTI), lt(I, INFO), lt(D, DEBUG), lt(T, TRACE),
#undef lt
	};
	...
	if (*mask == 'B') {
		...
		for (a = 0; a < ARRAY_SIZE(level_table); a++) {
			...
			debug("name %s, len %d\n",
			      level_table[a].name, level_table[a].len);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(6,0,0)
		ret =
		    vfs_read(filp, (char *)g_logm_obj_map, rd_size,
			     &filp->f_pos);
#else
		ret =
		    kernel_read(filp, (char *)g_logm_obj_map, rd_size,
			     &filp->f_pos);
#endif
		if (ret != rd_size) {
			memset(g_logm_obj_map, 0, rd_size);
			error
			    ("fail read logm_mask file, clear logm obj table\n");
		} else {
...
int LOGM_Init(void)
{
	 ...
	 */
	g_logm_device =
	    (LOGM_DEVICE_T *) kmalloc(sizeof(LOGM_DEVICE_T) * LOGM_MAX_DEVICE,
				      GFP_KERNEL);

	if (NULL == g_logm_device) {
		error("out of memory. can't allocate %d bytes\n",
		      (int)sizeof(LOGM_DEVICE_T) * LOGM_MAX_DEVICE);
		return -ENOMEM;
	}
	...
	if (g_logm_major) {
		...
		err =
		    alloc_chrdev_region(&dev, g_logm_minor, LOGM_MAX_DEVICE,
					LOGM_MODULE);
	...
	for (i = 0; i < LOGM_MAX_DEVICE; i++) {
		...
		OS_CreateDeviceClass_mode(g_logm_device[i].devno, 0666, "%s%d",
				     LOGM_MODULE, i);
	}
 ...
 *
 */
static int LOGM_Open(struct inode *inode, struct file *filp)
{
	int major, minor;
	struct cdev *cdev;
	...
	LOGM_CHECK_ERROR(udata == NULL, return RET_ERROR, "fail kmalloc\n");
 ...
 *
 */
static int LOGM_Close(struct inode *inode, struct file *file)
{
	int major, minor;
	LOGM_DEVICE_T *my_dev;
	struct cdev *cdev;
 ...
 */
static long LOGM_Ioctl(struct file *filp, UINT32 cmd, ULONG arg)
{
	int err = 0, ret = 0;
	...
	if (_IOC_TYPE(cmd) != LOGM_IOC_MAGIC) {
		error("invalid magic. magic=0x%02X, 0x%08x\n", _IOC_TYPE(cmd),
		      cmd);
		return -ENOTTY;
	}
	if (_IOC_NR(cmd) >= LOGM_IOC_MAXNR) {
		error("out of ioctl command. cmd_idx=%d, 0x%08x\n",
		      _IOC_NR(cmd), cmd);
		return -ENOTTY;
	}
...
#ifdef CONFIG_COMPAT
static long LOGM_CompatIoctl(struct file *filp, UINT32 cmd, ULONG arg)
{
	long ret;
	...
	case LOGM_IOW_MASK_CTRL:
		{
			struct logm_mask_ctrl arg_logm_mask_ctrl;

			ret =
			    copy_from_user(&arg_logm_mask_ctrl,
					   (void __user *)arg,
					   sizeof(struct logm_mask_ctrl));
			if (ret) {
				return -EINVAL;
			}
			arg_logm_mask_ctrl.name =
			    (char *)(uintptr_t) arg_logm_mask_ctrl.compat_name;
			arg_logm_mask_ctrl.bit_name =
			    (struct logm_obj_bit_name *)(uintptr_t)
			    arg_logm_mask_ctrl.compat_bit_name;
			ret =
	...
	case LOGM_IOW_USER_WRITE:
		{
			struct logm_user_log arg_logm_user_log;

			ret =
			    copy_from_user(&arg_logm_user_log,
					   (void __user *)arg,
					   sizeof(struct logm_user_log));
			if (ret) {
				return -EINVAL;
			}
			arg_logm_user_log.data =
			    (const unsigned char *)(uintptr_t)
			    arg_logm_user_log.compat_data;
			ret =
			    copy_to_user((void __user *)arg, &arg_logm_user_log,
					 sizeof(struct logm_user_log));
			if (ret) {
				return -EINVAL;
			}
...
static int _logm_ioctl_user_write(unsigned long arg, unsigned int cmd)
{
	...
	ret = __copy_from_user(&u, (void *)arg, _IOC_SIZE(cmd));
	...
	LOGM_CHECK_ERROR(data == NULL, return RET_ERROR, "fail kmalloc\n");
	...
	LOGM_PutLine(&(map[u.fd]), u.level, u.type, NULL, 0, data, u.size);
...
static int _logm_bit_name_wr(struct logm_mask_ctrl *m)
{
	...
	LOGM_CHECK_ERROR(name == NULL, return RET_ERROR, "fail vmalloc\n");
	...
	//name : kernel space string
	name[m->name_size - 1] = 0;
	ret = LOGM_RegBitName(m->fd, m->bit, name);
	vfree(name);
	LOGM_CHECK_ERROR(ret != 0, return RET_ERROR, "fail reg bit name\n");
...
static int _logm_ioctl_mask_ctrl(unsigned long arg, unsigned int cmd)
{
	...
	ret = __copy_from_user(&m, (void *)arg, _IOC_SIZE(cmd));
	LOGM_CHECK_ERROR(ret, return -EFAULT, "fail __copy_from\n");
	...
	case LOGM_MASK_CTRL_ADD:
		{
			ret = LOGM_ObjMaskAdd(m.fd, m.mask);
			LOGM_CHECK_ERROR(ret != 0, return RET_ERROR,
					 "fail obj mask add\n");
		}
		break;
	...
	case LOGM_MASK_CTRL_READ:
		{
			...
			ret = __copy_to_user((void *)arg, &m, _IOC_SIZE(cmd));
			LOGM_CHECK_ERROR(ret, return -EFAULT,
					 "fail __copy_to\n");
		}
		break;
	...
	case LOGM_MASK_CTRL_BIT_NAME_RD:
		{
			...
			bit_name = (struct logm_obj_bit_name *)
			    vmalloc(sizeof(struct logm_obj_bit_name));
			LOGM_CHECK_ERROR(bit_name == NULL, return RET_ERROR,
					 "fail vmalloc\n");

			ret = LOGM_GetBitName(m.fd, bit_name);
			LOGM_CHECK_ERROR(ret != 0, vfree(bit_name);
					 return RET_ERROR,
					 "fail rd bit name\n");

			ret =
			    __copy_to_user(m.bit_name, bit_name,
					   sizeof(struct logm_obj_bit_name));
			vfree(bit_name);
	...
	case LOGM_HIDDEN_MASK_CTRL_WRITE:
		{
			ret = LOGM_ObjHiddenMaskSet(m.fd, m.mask);
			LOGM_CHECK_ERROR(ret != 0, return RET_ERROR,
					 "fail obj hidden mask set\n");
		}
		break;

	case LOGM_HIDDEN_MASK_CTRL_READ:
		{
			ret = LOGM_ObjHiddenMaskGet(m.fd, &(m.mask));
			LOGM_CHECK_ERROR(ret != 0, return RET_ERROR,
					 "fail obj hidden mask get\n");
	...
	case LOGM_HIDDEN_MASK_CTRL_ALL_WR:
		{
			int i;
			unsigned long flag;
			LX_LOGM_LOGOBJ_T *map = g_logm_obj_map;
			...
			for (i = 0; i < LOGM_LOGOBJ_MAX; i++) {
				if (map[i].name[0] == '\0')
					...
					map[i].hidden_mask = m.mask;
			}
			spin_unlock_irqrestore(&g_logm_obj_map_lock, flag);
		}
		break;

	case LOGM_HIDDEN_MASK_CTRL_BIT_ENABLE:
		{
			ret = LOGM_ObjBitHiddenMaskEnable(m.fd, m.bit);
			LOGM_CHECK_ERROR(ret != 0, return RET_ERROR,
					 "fail obj hidden mask bit enable\n");
		}
		break;

	case LOGM_HIDDEN_MASK_CTRL_BIT_DISABLE:
		{
			ret = LOGM_ObjBitHiddenMaskDisable(m.fd, m.bit);
			LOGM_CHECK_ERROR(ret != 0, return RET_ERROR,
					 "fail obj hidden mask bit disable\n");
		}
		break;
...
static int _logm_ioctl_obj_reg(unsigned long arg, unsigned int cmd)
{
	...
	ret = __copy_from_user(&obj_reg, (void *)arg, _IOC_SIZE(cmd));
	LOGM_CHECK_ERROR(ret, return -EFAULT, "fail __copy_from\n");
	...
	ret = __copy_to_user((void *)arg, &obj_reg, _IOC_SIZE(cmd));
			 ...
			 char __user * data, size_t size, loff_t * off)
{
	...
	LOGM_CHECK_ERROR(ret != 0, return ret, "fail wait_event_int! %d\n",
			 ret);

	debug("got data. %d, %d\n", bc->buffer_tail, bc->buffer_head);

	if (k_buf_size > LOGM_READ_BUFFER_MAX)
		k_buf_size = LOGM_READ_BUFFER_MAX;

	k_buf = (char *)kmalloc(k_buf_size, GFP_KERNEL);
	LOGM_CHECK_ERROR(k_buf == NULL, return RET_ERROR, "fail kmalloc\n");
...
static int LOGM_Mmap(struct file *file, struct vm_area_struct *vm)
{
	int ret;
	unsigned int size =
	    (1 << get_order(sizeof(LX_LOGM_LOGOBJ_T) * LOGM_LOGOBJ_MAX)) *
	    PAGE_SIZE;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6,0,0))
	vm_flags_set(vm, VM_IO | VM_DONTEXPAND | VM_DONTDUMP);	//to prevent swapout
...
#endif
	ret =
	    remap_pfn_range(vm, vm->vm_start,
			    __virt_to_phys((long unsigned int)g_logm_obj_map)
			    >> PAGE_SHIFT, size, vm->vm_page_prot);
			  ...
			  const char __user * data, size_t size, loff_t * off)
{
	...
	ret = copy_from_user(tmp, data, size);
	if (ret) {
		kfree(tmp);
		return -EFAULT;
	}
	...
	while (*p) {
		if (*p == '\r' || *p == '\n') {
			*p = 0;
			if (s != p) {
				debug("got a line. \"%s\"\n", s);
				LOGM_PutLine(NULL, LX_LOGM_LEVEL_NOTI,
					     LX_LOGM_LOGTYPE_0, NULL, 0, s,
					     (int)(p - s));
			}
	...
	if (s != p) {
		debug("got last line. \"%s\"\n", s);
		LOGM_PutLine(NULL, LX_LOGM_LEVEL_NOTI,
			     LX_LOGM_LOGTYPE_0, NULL, 0, s, (int)(p - s));
	}
