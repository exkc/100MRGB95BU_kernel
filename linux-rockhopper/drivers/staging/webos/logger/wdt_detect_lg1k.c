...
static int wdt_detect_proc(void *data)
{
	...
	while (!kthread_should_stop()) {
		if (priv->event == WDT_EVENT_REGISTER_POLL) {
			v = readl(priv->base);
			if (v == priv->register_value)
				wdt_log_save();
		}
		else {
...
static int wdt_detect_init_register(struct platform_device *pdev)
{
	struct wdt_detect_priv *priv = platform_get_drvdata(pdev);
	struct device_node *np = pdev->dev.of_node;
	struct sched_param param = { .sched_priority = 99 };
	...
	if (of_property_read_u32(np, "detect-addr", (u32 *)&addr)) {
		pr_info("watchdog detect: detect-addr is not valid.\n");
		return -ENOTSUPP;
	}

	if (of_property_read_u32(np, "detect-value", (u32 *)&value)) {
		pr_info("watchdog detect: detect-value is not valid.\n");
		return -ENOTSUPP;
	}

	priv->base = ioremap(addr, PAGE_SIZE);
	if (priv->base == NULL) {
		pr_err("watchdog detect: ioremap failed.(%x)\n", addr);
		return -EIO;
	}
	priv->register_value = value;
	priv->event = WDT_EVENT_REGISTER_POLL;
	...
	pr_info("watchdog detect: register initialized.\n");
	return 0;
}
...
static int wdt_detect_probe(struct platform_device *pdev)
{
	...
	if (!wdt_detect_init_register(pdev))
		return 0;

	return !wdt_detect_init_intr(pdev) ? 0 : wdt_detect_init_poll(pdev);
}

static void wdt_detect_remove(struct platform_device *pdev)
{
	struct wdt_detect_priv *priv = platform_get_drvdata(pdev);
	u8 v;
