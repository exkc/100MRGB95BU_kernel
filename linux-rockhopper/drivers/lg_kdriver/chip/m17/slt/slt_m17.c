...
#ifdef SLT_VSD
#define M17_IRQ_SLT			(M17_IRQ_INTR_CRC0)
#define M17_SLT_BASE		(M17_CRC_VSD_BASE)
#else
#define M17_IRQ_SLT			(M17_IRQ_INTR_CRC1)
#define M17_SLT_BASE		(M17_CRC_DPE_BASE)
#endif
...
static void slt_test_get_crc( void )
{
	...
	if( crc_check_count < SLT_CRC_NUM )
	{
		...
		crc_data.usec[crc_check_count] = OS_GetUsecTicks();
		crc_check_count++;
	}
	else
	{
...
static int slt_open( void )
{
	...
	SLT_REG = (volatile M17_SLT_REG_T*)ioremap(M17_SLT_BASE, sizeof(M17_SLT_REG_T));
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
	ret = request_irq(M17_IRQ_SLT, (irq_handler_t)slt_isr, NULL, "SLT", NULL);
#else
	ret = request_irq(M17_IRQ_SLT, (irq_handler_t)slt_isr, IRQF_DISABLED, "SLT", NULL);
#endif
	if( ret != RET_OK )
	{
