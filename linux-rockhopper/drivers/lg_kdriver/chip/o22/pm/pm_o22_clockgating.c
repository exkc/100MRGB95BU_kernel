...
/*----------------------------------------------------------------------------------------
	Macro Definitions
----------------------------------------------------------------------------------------*/
static DEFINE_SPINLOCK(g_o22_pm_cg_lock);
...
// Clock Gating
LX_PM_CLOCKGATING_T o22_pms_clockgating;

typedef struct PM_O22_CGINFO_t
{
	unsigned int module;
	unsigned int sub;
...
static int o22_GetCGInfo(PM_O22_CGINFO_T *cginfo)
{
	int ret = 0;
...
int o22_clock_on(PM_O22_CGINFO_T cginfo)
{
	...
	spin_lock_irqsave(&g_o22_pm_cg_lock, flags);

	// clock first when power on
	PM_DEBUG_PRINT("O22 ON CG 0x%X : 0x%X\n",cginfo.base,readl(pms_clock_gating_base));
	...
	{
		PM_DEBUG_PRINT("O22 ON CG2 0x%X : 0x%X\n",cginfo.base,readl(pms_clock_gating_base));
		reg = readl(pms_clock_gating_base);
	...
	spin_unlock_irqrestore(&g_o22_pm_cg_lock, flags);
...
int o22_clock_off(PM_O22_CGINFO_T cginfo)
{
	void __iomem *pms_clock_gating_base = NULL;
	volatile unsigned int reg;
	unsigned long flags;
	...
	PM_DEBUG_PRINT("O22 OFF CG 0x%X : 0x%X\n",cginfo.base,readl(pms_clock_gating_base));
	reg = readl(pms_clock_gating_base);
	reg = reg | (1 << cginfo.clkbit);
	writel(reg, pms_clock_gating_base);
	PM_DEBUG_PRINT("O22 OFF CG 0x%X : 0x%X\n",cginfo.base,readl(pms_clock_gating_base));
	...
	if (cginfo.clkbit2 != 0x80000000)
	{
		PM_DEBUG_PRINT("O22 OFF CG2 0x%X : 0x%X\n",cginfo.base,readl(pms_clock_gating_base));
		reg = readl(pms_clock_gating_base);
		reg = reg | (1 << cginfo.clkbit2);
		writel(reg, pms_clock_gating_base);
		PM_DEBUG_PRINT("O22 OFF CG2 0x%X : 0x%X\n",cginfo.base,readl(pms_clock_gating_base));
...
{
	PM_O22_CGINFO_T cginfo;
	int ret;

	if (( lx_chip_rev() & 0xFFFFFF00) != (LX_CHIP_REV(O22,A0) & 0xFFFFFF00))
		return 0;
 ...
 */
int o22_pm_ioctl_cg (unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	int result = 0;
	...
	switch (cmd)
	{
		case PM_SET_CLOCK_GATING:
			result = copy_from_user((void *)&o22_pms_clockgating, (void __user *)arg, sizeof(o22_pms_clockgating));
			if (result) {
				PM_ERROR("PM_SET_CLOCK_GATING - CONFIG DATA COPY FAIL\n");
				break;
			}

			o22_set_clock_gating(o22_pms_clockgating.module, o22_pms_clockgating.sub, o22_pms_clockgating.state);
			break;
		case PM_GET_CLOCK_GATING:
			...
			o22_get_clock_gating(o22_pms_clockgating.module, o22_pms_clockgating.sub, &o22_pms_clockgating.state);

			ret = copy_to_user((void __user*)(arg), (void *)(&o22_pms_clockgating), sizeof(o22_pms_clockgating));
			if (ret)
...
static pms_o22_cg_t o22_pm_cg =
{
	.ioctl = o22_pm_ioctl_cg,
};
