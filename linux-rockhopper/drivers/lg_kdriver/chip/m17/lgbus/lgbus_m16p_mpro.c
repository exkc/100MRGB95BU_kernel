...
static lgbus_m16p_mpro_t mpro_ctrl[] =
{
	{ "CPU",        0xC9231000 },
	{ "PERI",       0xC9231200 },
	{ "LZMA",       0XC9231400 },
	...
	{ "FMS0",       0xC9103000 },
	{ "DBB",        0xC9103400 }, /* GBB */
	{ "VENC",		0xC9103A00 }, /* VENC , valid at Bx, dummy at Ax */
};
...
static int lgbus_mpro_resume(void)
{
#ifdef SUPPORT_M16P_MPRO_CHIP_KDRV
	int ret;
...
static int lgbus_mpro_get_range(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		for(j = 0; j < M16P_MPRO_RANGE_SLOT_CNT; j++)
		{
...
static int lgbus_mpro_regs_save(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		memcpy_fromio(&(mpro_ctrl[i].reg_save), save_addr, sizeof(lgbus_m16p_mpro_reg_save_t));
	}
	ret = RET_OK;
...
static int lgbus_mpro_regs_restore(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		memcpy_toio(save_addr, &(mpro_ctrl[i].reg_save), sizeof(lgbus_m16p_mpro_reg_save_t));
	}
	ret = RET_OK;
...
#undef LBUS_IRQ
#undef GBUS_IRQ
#define LBUS_IRQ	M16P_IRQ_INTR_LBUS
#define GBUS_IRQ	M16P_IRQ_INTR_GBUS

#undef CPU_IRQ
#undef CPU_PERI_IRQ
#undef CPU_LZMA_IRQ

#define CPU_IRQ			M16P_IRQ_INTR_QOS_TIMER_CPU
#define CPU_PERI_IRQ	M16P_IRQ_INTR_QOS_TIMER_PERI
#define CPU_LZMA_IRQ	M16P_IRQ_INTR_QOS_TIMER_LZMA
			...
			if(irq_status & MPRO_ISR_STATUS)
			{
...

				{
					...
					struct
					{
						...
						u32 wr_vio_addr;
						u32 wr_vio_id;
					}rw_vio_data[M16P_MPRO_RANGE_SLOT_CNT];
...
int LGBUS_M16P_SetMproRange(lgbus_mpro_range_t *mpro_range)
{
#ifdef SUPPORT_M16P_MPRO_CHIP_KDRV
	int 		ret = RET_ERROR;
	uint32_t 	port, cnt;
	...
	CHECK_ERROR(mpro_range->port >= M16P_MPRO_PORT_MAX, return RET_ERROR, "Invalid port\n");
	CHECK_ERROR(mpro_range->port < M16P_MPRO_PORT_START, return RET_ERROR, "Invalid port\n");

	port = mpro_range->port - M16P_MPRO_PORT_START;
	...
	for(cnt = 0; cnt < M16P_MPRO_RANGE_SLOT_CNT; cnt++)
	{
		uint32_t ctr0, ctr1;
...
EXPORT_SYMBOL(LGBUS_M16P_SetMproRange);

int LGBUS_M16P_GetMproRange(lgbus_mpro_range_t *mpro_range)
{
#ifdef SUPPORT_M16P_MPRO_CHIP_KDRV
	uint32_t 	port, cnt;
	ulong 		flag;
...
EXPORT_SYMBOL(LGBUS_M16P_GetMproRange);

lgbus_m16p_mpro_func_t	lgbus_m16p_mpro_func =
{
...
#define M16P_ADDR_SWITCH_SLOT			(8)
#define _reg_read(addr)					__raw_readl((void*)(addr))

static lgbus_addr_switch_t m16p_addr_switch[M16P_ADDR_SWITCH_SLOT];
...
static int lgbus_get_addr_switch(void)
{
	...
	for(i = 0; i < M16P_ADDR_SWITCH_SLOT; i++)
	{
		u32 d0, d1;
		...
		m16p_addr_switch[i].start = ((u64)(d0 >> 16) & 0xfff) << 20;
		m16p_addr_switch[i].end = (u64)(d0 & 0xfff) << 20;
		m16p_addr_switch[i].offset = (u64)(d1 & 0xfff) << 20;

		if(m16p_addr_switch[i].end)
			m16p_addr_switch[i].end |= 0xfffff;

		LGBUS_PRINT("s:0x%010llx ~ e:0x%010llx,o:0x%010llx\n", m16p_addr_switch[i].start
															 , m16p_addr_switch[i].end
															 , m16p_addr_switch[i].offset);
	}
...
u64 lgbus_m16p_get_ddr_addr(u64 phy_addr)
{
#ifdef SUPPORT_M16P_MPRO_CHIP_KDRV
	int i;
	u64 tmp = phy_addr;
	...
	for(i = 0; i < M16P_ADDR_SWITCH_SLOT; i++)
	{
		if(m16p_addr_switch[i].start <= phy_addr &&
				m16p_addr_switch[i].end >= phy_addr)
		{
			phy_addr += m16p_addr_switch[i].offset;
...
EXPORT_SYMBOL(lgbus_m16p_get_ddr_addr);
        ...
        , const char __user *buf, size_t size, loff_t *off)
{
	...
	int		i;
	int		addr_cnt;
	int		addr_max = M16P_MPRO_RANGE_SLOT_CNT * 2;
	u64		addr[M16P_MPRO_RANGE_SLOT_CNT * 2] = {0,};
	...
	while(i < size)
	{
		...
		{
			mpro_range.port += M16P_MPRO_PORT_START;
			LGBUS_M16P_SetMproRange(&mpro_range);
		}
