...
void usb_sg_wait(struct usb_sg_request *io)
{
	...
	while (1) {
		...
		struct device *dev = &io->dev->dev;

		timeleft = wait_for_completion_timeout(&io->complete,
							msecs_to_jiffies(2000));
		if (timeleft)
			break;

		status = usb_get_port_status(io->dev, &portstatus, &portchange);
		dev_info(dev, "ep%d%s ret %d sts %04x chg %04x "
				"urb %d/%d byte %zu/%u\n",
				usb_pipeendpoint(io->pipe),
				usb_pipein(io->pipe) ? "in" : "out",
				status, portstatus, portchange,
				io->count, io->entries,
				io->bytes, io->urbs[0]->transfer_buffer_length);
		if (status || (portchange & USB_PORT_STAT_C_CONNECTION)) {
			dev_warn(dev, "dev has been disconnected\n");
			break;
		}
		if (pm_freezing == true) {
			dev_warn(dev, "system is suspending\n");
			io->dev->persist_enabled = 0;
			usb_sg_cancel(io);
			break;
		}
		if (io->dev->state == USB_STATE_NOTATTACHED) {
			dev_warn(dev, "dev is not attached\n");
			usb_sg_cancel(io);
			break;
		}
	}
