...
static void __iomem *ipc_disable_kdrv = NULL;

unsigned int get_ucom_disable_kdrv(void)
{
	unsigned int reg = 0;

	if(ipc_disable_kdrv != NULL)
		reg = readl(ipc_disable_kdrv);

	return reg;
}
EXPORT_SYMBOL(get_ucom_disable_kdrv) ;

void set_ucom_disable_kdrv(unsigned int disable)
{
	if(ipc_disable_kdrv !=NULL)
		writel(disable, ipc_disable_kdrv);
}
EXPORT_SYMBOL(set_ucom_disable_kdrv) ;
...
{
	if(ipc_disable_kdrv == NULL)
		ipc_disable_kdrv = ioremap(0xfd30004c, 4);
	/* enable ipc */
...
void o20_ucom_FuncChipReset(void)
{
...
static ucom_o20_func_t o20_ucom_func =
{
	...
	.chipreset = o20_ucom_FuncChipReset,
