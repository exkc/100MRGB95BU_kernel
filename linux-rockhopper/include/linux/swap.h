...
extern int get_swap_pages(int n, swp_entry_t swp_entries[], int order, long d);
...
static inline long mem_cgroup_get_distance(struct folio *folio)
{
	struct mem_cgroup *memcg = folio ? folio_memcg(folio) : NULL;

	return (!memcg) ? 0 : atomic_read(&(memcg->sw_dev_distance));
}
#else
static inline long mem_cgroup_get_distance(struct folio *folio)
{
