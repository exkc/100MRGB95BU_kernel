	...
	if (atomic_read(&kfile->setup_state) == KBASE_FILE_COMPLETE) {
...
#ifdef CONFIG_DEBUG_FS
		{
			...
			list_for_each_entry(kctx_element, &kbdev->kctx_list, kctx_list_link) {
				if (kctx_element != kctx && kctx_element->tgid == kctx->tgid) {
					remove_proc = false;
					break;
				}
			}
	...
	base_context_create_flags const flags)
{
	...
	if (IS_ERR_OR_NULL(kctx->kctx_dentry)) {
		...
		// KyuYeunKim: add support for umm memory info for /proc/um/pid
		{
			struct kbase_context *kctx_element;
...
// KyuYeunKim: add kbasep_gpu_utilization_debugfs_init
