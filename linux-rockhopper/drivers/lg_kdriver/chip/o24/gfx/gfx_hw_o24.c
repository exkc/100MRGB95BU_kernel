...
#include "de_kapi.h"
extern int DE_REG_O24_GetSourceStatus(LX_DE_SRC_STATUS_T* pstParams);
#endif
...
#define GFX_WRITE_REG(reg,val)  \
    __raw_writel(val , (volatile void*)((uintptr_t)(g_gfx_reg_o24) + offsetof(GFX_O24_REG_T,reg)))

#define GFX_READ_REG(reg)   \
    __raw_readl( (volatile void*)((uintptr_t)(g_gfx_reg_o24) + offsetof(GFX_O24_REG_T,reg)))

//#define SHOW_GFX_REGISTER //only debugging
#if defined(SHOW_GFX_REGISTER)

#define PRINT_READ_REG(reg) \
   do { printk("GFX REG 0x%x -> 0x%0.8x\n",((UINT32)OFFSET(GFX_O24_REG_T,reg)), *((UINT32 *)(&g_gfx_reg_o24_cache->reg))); } while(0);
...
GFX_O24_SCALE_VPHASE_DATA_T;

typedef struct
{
#define GFX_SCALE_PHASE_DATA_NUM    16
    GFX_O24_SCALE_HPHASE_DATA_T     hphase[GFX_SCALE_PHASE_DATA_NUM];       /* hhpase data */
    GFX_O24_SCALE_VPHASE_DATA_T     vphase[GFX_SCALE_PHASE_DATA_NUM];       /* vhpase data */
}
GFX_O24_SCALE_FILTER_DATA_T;

/*----------------------------------------------------------------------------------------
    Function Prototype Declarations
----------------------------------------------------------------------------------------*/
void GFX_O24_InitScaler(void);
int GFX_O24_RunFlushCommand(void);
int GFX_O24_RunStartCommand(void);
int GFX_O24_PauseCommand(void);
int GFX_O24_ResumeCommand(void);
int GFX_O24_SwResetCommand(void);
int GFX_O24_StopCommand(void);
void GFX_O24_GetComQueueStatus(GFX_CMD_QUEUE_CTRL_T* queue);
BOOLEAN GFX_O24_IsGfxIdle(void);
void GFX_O24_GetOperationStatus(GFX_CMD_OP_T* operate);
void GFX_O24_SetOperationStatus(GFX_CMD_OP_T* operate);
void GFX_O24_GetInputConfigure(int iPort, GFX_PORT_CONFIGURE_T* port);
void GFX_O24_SetInputConfigure(GFX_PORT_CONFIGURE_T* port);
void GFX_O24_GetBlendConfigure(GFX_BLEND_CONFIGURE_T* blend);
void GFX_O24_SetBlendingOut(GFX_ENUM_OUT_T type);
void GFX_O24_SetBlendConfigure(GFX_BLEND_CONFIGURE_T* blend);
void GFX_O24_GetOutputConfigure(GFX_OUT_CONFIGURE_T* port);
void GFX_O24_SetOutputConfigure(GFX_OUT_CONFIGURE_T* port);
void GFX_O24_SetScalerConfigure(GFX_SCALER_CONFIGURE_T* scaler);
void GFX_O24_SetInterruptMode(UINT32 uiMode);
void GFX_O24_SetAckMode(UINT32* mode);
void GFX_O24_GetAckMode(UINT32* mode);
int GFX_O24_SetCLUT(int port, int size, UINT32* data);
void GFX_O24_SetCommandDelayReg(UINT16 delay);
UINT16 GFX_O24_GetCommandDelayReg(void);
void GFX_O24_DumpRegister(void);
int GFX_O24_RunSuspend(void);
int GFX_O24_RunResume(void);
int GFX_O24_RunClkOnCommand(void);
int GFX_O24_RunClkOffCommand(void);
BOOLEAN GFX_O24_ClkCheck(void);

/*----------------------------------------------------------------------------------------
    External Variables
----------------------------------------------------------------------------------------*/
extern volatile GFX_O24_REG_T*   g_gfx_reg_o24;
extern GFX_O24_REG_T*            g_gfx_reg_o24_cache;

/*----------------------------------------------------------------------------------------
    global Variables
----------------------------------------------------------------------------------------*/
volatile UINT32 g_ui_ack_mode_o24 = 0;

#ifdef  KDRV_CONFIG_PM
unsigned int g_pm_cmd_delay_o24 = 0;
unsigned int g_pm_intr_ctrl_o24 = 0;
#endif
...
/*----------------------------------------------------------------------------------------
    Static Variables
----------------------------------------------------------------------------------------*/
static int  _g_gfx_o24_bw_val           = 0x7ff;
static int  _g_gfx_o24_bw_val_req       = 0x7ff;
module_param_named(gfx_o24_bw_val, _g_gfx_o24_bw_val_req, int, 0644);

// see http://clm.lge.com/issue/browse/SCDCR-6440
static int _g_gfx_down_scaler_phase_offset  = 0x30;
module_param_named(o24_gfx_down_scaler_phase_offset, _g_gfx_down_scaler_phase_offset, int, 0644);


#define GFX_O24_CUSTOM_FILTER_MAX   4

static int _g_gfx_o24_scaler_index = -1;
module_param_named(gfx_o24_scaler_index, _g_gfx_o24_scaler_index, int, 0644);

static GFX_O24_SCALE_FILTER_DATA_T  g_gfx_o24_custom_scale_filter_data[GFX_O24_CUSTOM_FILTER_MAX] =
{
    /* normal upscale */
    [0] = {
 ...
 *
 */
static int      GFX_O24_GetBestHVScalerIndex(GFX_SCALER_CONFIGURE_T* scaler)
{
    int h_scale_ratio = 0;
    int v_scale_ratio = 0;

    if (_g_gfx_o24_scaler_index >= 0) return _g_gfx_o24_scaler_index;
...
static int _o24_scaler_mux = -1;
module_param_named(o24_scaler_insel, _o24_scaler_mux, int, 0644);

static int _o24_swap_r0_r1 = -1;
module_param_named(o24_swap_read_port, _o24_swap_r0_r1, int, 0644);
...
static int _o24_dump_reg_cached = -1;
module_param_named(o24_dump_before_start, _o24_dump_reg_cached, int, 0644);

static inline void _gfx_dump_register_cached(void)
{
    int i;
    int j;
    UINT32* reg_ptr = (UINT32*)g_gfx_reg_o24_cache;
...
static inline void GFX_O24_Bypass2ndBlender(void)
{
...
static int _o24_vreverse_r0 = -1;
module_param_named(o24_reverse_r0, _o24_vreverse_r0, int, 0644);

static int _o24_vreverse_r1 = -1;
module_param_named(o24_reverse_r1, _o24_vreverse_r1, int, 0644);

static int _o24_vreverse_r2 = -1;
module_param_named(o24_reverse_r2, _o24_vreverse_r2, int, 0644);

static int _o24_comp_sparse = -1;
module_param_named(o24_comp_mode, _o24_comp_sparse, int, 0644);
...
{
    int i, j;
    GFX_O24_SCALE_FILTER_DATA_T*    filter_data;

    GFX_WRITE_REG(scaler_frame_start, 0x1);                 /* scaler frame start (reset) */

    for (j = 0; j < GFX_O24_CUSTOM_FILTER_MAX; j++)              /* download custome filter data */
    {
        filter_data = &(g_gfx_o24_custom_scale_filter_data[j]);

        for (i = 0; i < GFX_SCALE_PHASE_DATA_NUM; i++)
        {
            g_gfx_reg_o24->scaler_hphase_data0 = filter_data->hphase[i].data0;
            g_gfx_reg_o24->scaler_hphase_data1 = filter_data->hphase[i].data1;
            g_gfx_reg_o24->scaler_hphase_data2 = filter_data->hphase[i].data2;
            g_gfx_reg_o24->scaler_hphase_addr  = (j << 4) | filter_data->hphase[i].addr;
        }

        for (i = 0; i < GFX_SCALE_PHASE_DATA_NUM; i++)
        {
            g_gfx_reg_o24->scaler_vphase_data0 = filter_data->vphase[i].data0;
            g_gfx_reg_o24->scaler_vphase_data1 = filter_data->vphase[i].data1;
            g_gfx_reg_o24->scaler_vphase_addr  = (j << 4) | filter_data->vphase[i].addr;
        }
    }
}
...
int GFX_O24_RunStartCommand(void)
{
    ...
    if (_g_gfx_o24_bw_val != _g_gfx_o24_bw_val_req)
    {
        _g_gfx_o24_bw_val_req &= 0x7ff;
#if defined(FIXME) // do not support... defer ~!!
        GFX_WARN("+++ bw_limit changed 0x%x -> 0x%x\n", _g_gfx_o24_bw_val, _g_gfx_o24_bw_val_req);
        _g_gfx_o24_bw_val = _g_gfx_o24_bw_val_req;
        OS_WrReg(0xc000e4f8, _g_gfx_o24_bw_val);
#endif
    }

    if (_o24_dump_reg_cached > 0)
    {
    ...
    /* first, decoder */
    {
        if (-1 != _o24_comp_sparse)
        {
            GFX_Wr01(rd_comp_control, sparse_alloc, _o24_comp_sparse);
        }
        else
        {
    ...
    /* second, encoder .. ToDo.. */
    {
        if (-1 != _o24_comp_sparse)
        {
            GFX_Wr01(wr_comp_control, sparse_alloc, _o24_comp_sparse);
        }
        else
        {
            /* default is the compact mode (=0) */
...
int GFX_O24_PauseCommand(void)
{
    ...
    g_gfx_reg_o24->gfx_reset.gfx_reset              = 0x0;
    g_gfx_reg_o24->gfx_batch_run.gfx_batch_run      = 0x0;
    g_gfx_reg_o24->gfx_start.gfx_start              = 0x0;
    g_gfx_reg_o24->gfx_pause.gfx_pause              = 0x1;
...
int GFX_O24_ResumeCommand(void)
{
    ...
    g_gfx_reg_o24->gfx_batch_run.gfx_batch_run      = 0x0;
    g_gfx_reg_o24->gfx_start.gfx_start              = 0x0;
    g_gfx_reg_o24->gfx_pause.gfx_pause              = 0x2;
...
#else
    ret = (g_gfx_reg_o24_cache->gfx_status0.exec_status == 0 &&
           g_gfx_reg_o24_cache->gfx_status1.cmd_que_status == 0) ? TRUE : FALSE;
    //   ret = ( g_gfx_reg_o24->status0.bwCmdStatus == 0 &&
    //          g_gfx_reg_o24->status0.abwProcessingLine == 0 &&
    //          g_gfx_reg_o24->status1.bwCmdQueueStatus == 0 )? TRUE : FALSE ;

    //  printk("GFX IDLE = (%s) %d-%d-%d\n", (ret)? "IDLE":"BUSY",
    //          g_gfx_reg_o24_cache->gfx_status0.exec_status, g_gfx_reg_o24_cache->gfx_status0.exec_line, g_gfx_reg_o24_cache->gfx_status1.cmd_que_status);
#endif
...
void GFX_O24_GetOperationStatus(GFX_CMD_OP_T* operate)
{
    ...
    operate->bChromaFilterEnable = 0 ; //g_gfx_reg_o24->op_mode.bwEnChromaFilter;
 ...
 *
 *  @note operation status should be read at GFX_O24_GetOperationStatus
 */
void GFX_O24_SetOperationStatus(GFX_CMD_OP_T* operate)
{
    GFX_TRACE_BEGIN();

    /* L18F,O24 추가 - multi-source blender 가 아니라면 2nd blender 를 사용하지 않는 것이구나. */
    if (operate->sPort != eReadThreePorts)
    {
        GFX_O24_Bypass2ndBlender();
    }
    ...
    if (-1 != _o24_swap_r0_r1)
    {
        ...
        if (operate->sPort == eReadTwoPorts)
        {
            ...
            printk("[fv:%s] debug mode from o24_swap_read_port(=%d)\n",
                   __func__,
                   _o24_swap_r0_r1
                  );
            ...
            GFX_O24_SetInputConfigure(&port0);
            GFX_O24_SetInputConfigure(&port1);
...
void GFX_O24_SetInputConfigure(GFX_PORT_CONFIGURE_T* port)
{
    ...
    switch (port->sPort)
    {
        case ePort0:
        {
            ...
            if (-1 != _o24_vreverse_r0)
            {
                printk("o24_reverse_r0 = (%d)\n", _o24_vreverse_r0);
                port->reversed = _o24_vreverse_r0;
            }
        ...
        case ePort1:
        {
            ...
            if (-1 != _o24_vreverse_r1)
            {
                printk("o24_reverse_r1 = (%d)\n", _o24_vreverse_r1);
                port->reversed = _o24_vreverse_r1;
            }
        ...
        case ePort2:
        {
            ...
            if (-1 != _o24_vreverse_r2)
            {
                printk("o24_reverse_r2 = (%d)\n", _o24_vreverse_r2);
                port->reversed = _o24_vreverse_r2;
            }
...
void GFX_O24_GetBlendConfigure(GFX_BLEND_CONFIGURE_T* blend)
{
    ...
    //  blend->ulBlendConstant  = g_gfx_reg_o24->out_const_ctrl.abwConstantCtrl;
    GFX_Rd01(rop_ctrl,      rop_ctrl,       blend->sRaster);
    //  blend->sRaster          = g_gfx_reg_o24->out_rop_ctrl.abwRasterSel;
 ...
 *
 *  @note out_sel should be read before this function at GFX_O24_GetBlendConfigure
 */
void GFX_O24_SetBlendingOut(GFX_ENUM_OUT_T type)
{
    GFX_TRACE_BEGIN();
 ...
 *
 *  @note all data should be read before this function at GFX_O24_GetBlendConfigure
 */
void GFX_O24_SetBlendConfigure(GFX_BLEND_CONFIGURE_T* blend)
{
    ...
    //  blend->sOut         = g_gfx_reg_o24->out_sel.abwOutSel;
...
static int GFX_O24_SelectScalerOrder(GFX_SCALER_CONFIGURE_T* s)
{
    int vh_order;
...
void GFX_O24_SetScalerConfigure(GFX_SCALER_CONFIGURE_T* scaler)
{
    switch (scaler->cmd)
    {
        ...
        {
            int scaler_set_idx = GFX_O24_GetBestHVScalerIndex(scaler);
            int vh_order = GFX_O24_SelectScalerOrder(scaler);
...
#else
            g_gfx_reg_o24->scaler_line_init.scaler_line_init = 0x1; /* init scaler line */
            g_gfx_reg_o24->scaler_line_init.scaler_line_init = 0x0;
#endif
            ...
            if (-1 != _o24_scaler_mux)
            {
#if defined(DEBUG_FPGA)
                printk("[fv:%s] debug mode from o24_scaler_insel(=%d)\n",
                       __func__,
                       _o24_scaler_mux
                      );
#endif

                switch (_o24_scaler_mux)
                {
                    case 0:
                    case 1:
                    case 2:
                        ...
                        GFX_WrFL(wr_size);
                        GFX_Wr01(scaler_bypass, scaler_insel, _o24_scaler_mux);
        ...
        default:
        {
            g_gfx_reg_o24->scaler_bypass.scaler_bypass = 0x1;
        }
        break;
    }
}
...
int GFX_O24_SetCLUT(int port, int size, UINT32* data)
{
    ...
    switch (port)
    {
        case 0:
        {
            g_gfx_reg_o24->r0_clut_ctrl.clut0_addr  = 0;
            g_gfx_reg_o24->r0_clut_ctrl.clut0_rw    = 0;
#ifdef GFX_PALETTE_AUTO_INC
            g_gfx_reg_o24->r0_clut_ctrl.clut0_wai   = 1;
#else
            g_gfx_reg_o24->r0_clut_ctrl.clut0_wai   = 0;
#endif
...
#else
                g_gfx_reg_o24->r0_clut_ctrl.clut0_addr = i;
#endif
                g_gfx_reg_o24->r0_clut_data         = *data++;
            }
        }
        break;
        ...
        case 1:
        {
            g_gfx_reg_o24->r1_clut_ctrl.clut1_addr  = 0;
            g_gfx_reg_o24->r1_clut_ctrl.clut1_rw    = 0;
#ifdef GFX_PALETTE_AUTO_INC
            g_gfx_reg_o24->r1_clut_ctrl.clut1_wai   = 1;
#else
            g_gfx_reg_o24->r1_clut_ctrl.clut1_wai   = 0;
#endif
...
#else
                g_gfx_reg_o24->r1_clut_ctrl.clut1_addr = i;
#endif
                g_gfx_reg_o24->r1_clut_data         = *data++;
            }
        }
        break;
        ...
        case 2:
        {
            g_gfx_reg_o24->r2_clut_ctrl.clut2_addr = 0;
            g_gfx_reg_o24->r2_clut_ctrl.clut2_rw    = 0;
#ifdef GFX_PALETTE_AUTO_INC
            g_gfx_reg_o24->r2_clut_ctrl.clut2_wai   = 1;
#else
            g_gfx_reg_o24->r2_clut_ctrl.clut2_wai   = 0;
#endif
...
#else
                g_gfx_reg_o24->r2_clut_ctrl.clut2_addr = i;
#endif
                g_gfx_reg_o24->r2_clut_data         = *data++;
            }
        }
        break;
...
static UINT16   GFX_O24_CalcAutoCommandDelay(void)
{
#ifdef GFX_AUTO_CMD_DELAY
    int rc;
    ...
    rc = DE_REG_O24_GetSourceStatus(&de_src_param);
...
void GFX_O24_SetCommandDelayReg(UINT16 delay)
{
    ...
    if (delay == (UINT16) - 1)
    {
        delay = GFX_O24_CalcAutoCommandDelay();
    }
...
UINT16 GFX_O24_GetCommandDelayReg(void)
{
    UINT16  val;
    GFX_TRACE_BEGIN();
    val = g_gfx_reg_o24->gfx_cmd_dly.gfx_cmd_dly;
    GFX_TRACE_END();
    return val;
}
...
{
    int i;
    UINT32* reg_ptr = (UINT32*)g_gfx_reg_o24;
...
#define GFX_O24_CG_MASK_APB     0x1
#define GFX_O24_CG_MASK_CORE    0x2

/** GFX clock gating function
 *
 * see http://clm.lge.com/issue/browse/SICDTV-10001
 */
static int GFX_O24_SetClockGatingReg(bool cg_en, UINT32 cg_mask)
{
#if 0
    UINT32 val = 0x1;
#endif
...
#if 0 // TBD: not ready
    CTOP_CTRL_O24Ax_Wr(SCRG_CTRL_GFX, crg_gfx02, 0x00000000);

    if (cg_mask & GFX_O24_CG_MASK_APB)
    {
        CTOP_CTRL_O24Ax_Wr01(SCRG_CTRL_GFX, crg_gfx02, reg_gfx_apb_clk_gate_en, cg_en);
    }

    if (cg_mask & GFX_O24_CG_MASK_CORE)
    {
        CTOP_CTRL_O24Ax_Wr01(SCRG_CTRL_GFX, crg_gfx02, reg_gfx_core_clk_gate_en, cg_en);
    }

    CTOP_CTRL_O24Ax_WrFL(SCRG_CTRL_GFX, crg_gfx02);

    val = CTOP_CTRL_O24Ax_Rd(SCRG_CTRL_GFX, crg_gfx02);
...
int GFX_O24_RunClkOnCommand(void)
{
    ...
    GFX_O24_SetClockGatingReg(true, GFX_O24_CG_MASK_APB | GFX_O24_CG_MASK_CORE); //GFX_O24_CG_MASK_CORE);
    return RET_OK;
}

int GFX_O24_RunClkOffCommand(void)
{
    GFX_O24_SetClockGatingReg(false, GFX_O24_CG_MASK_APB | GFX_O24_CG_MASK_CORE); //GFX_O24_CG_MASK_CORE);
    return RET_OK;
}
...
#if 0 // TBD: not ready
    CTOP_CTRL_O24Ax_RdFL(SCRG_CTRL_GFX, crg_gfx02);
    CTOP_CTRL_O24Ax_Rd01(SCRG_CTRL_GFX, crg_gfx02, reg_gfx_core_clk_gate_en, val);
#endif
