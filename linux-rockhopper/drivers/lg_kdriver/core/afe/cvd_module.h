...
/*----------------------------------------------------------------------------------------
	Macro Definitions
----------------------------------------------------------------------------------------*/
#define VBI_INTERRUPT_MASK	0x00000004

#define VSYNC_INTERRUPT_MASK	0x08000000

#define NOSIGNAL_INTERRUPT_MASK	0x00000080
#define HUNLOCK_INTERRUPT_MASK	0x00000010
#define VUNLOCK_INTERRUPT_MASK	0x00000004

#define UNLOCK_INTERRUPT_MASK	(NOSIGNAL_INTERRUPT_MASK|HUNLOCK_INTERRUPT_MASK|VUNLOCK_INTERRUPT_MASK)
...
#define CVD_3DCOMB_ERROR_WORKAROUND
#define CVD_FAST_3DCOMB_WORKAROUND
#ifdef	CVD_3DCOMB_ERROR_WORKAROUND
#undef	CVD_FAST_3DCOMB_WORKAROUND
#endif
...
// From cvd_hw_xxx.h
#undef	CVD_STABLE_HSYNC_WORKAROUND
#define	CVD_HSYNC_ENHANCEMENT		//*_hw.h
#define	CVD_VSYNC_ENHANCEMENT		//*_hw.h

#define CVD_DONT_KILL_CHROMALOCK_ON_CLOCK_LOST

#define CVD_NTSC_NOISY_STABLE_VSYNC_WORKAROUND

// for H15 Color Bar Enhance in driver
#define CVD_COLORBAR_PARAMS_TEMP
...
typedef struct {
	...
	/* variables for CVD workaround codes */
	int 		gHstateMaxCounter;
	UINT8	vcr_detect_count;
	int	  gAdaptive_Burst_Gate_Counter;
	UINT32 gVariance_Status_Cdto_Inc;
	int	Adaptive_Burst_Gate_Enable;
	int Prev_Adaptive_Burst_Gate_Enable;
	UINT32 gAccumulated_Cordic_Freq_Value;
	UINT32	cvd_noise_sum;
	UINT8 burst_mag_prev;
	UINT8 burst_mag_next;
	UINT32 burst_mag_array[10];
	UINT32 burst_mag_sum ;
	UINT32 burst_mag_avg;
	UINT32 prev_burst_mag_avg ;
	UINT32 cagc_target ;
	UINT32 cagc_written ;
	UINT32 hdto_abnormal_count;
	int	no_signal_intr;
	int h_unlock_intr;
	int v_unlock_intr;
...
}	CVD_SET_SYSTEM_3CS_T;

typedef struct {
	CVD_SET_SYSTEM_3CS_T Color_System_Params[CVD_STATE_VideoNotReady];
} CVD_COLOR_SYSTEM_ARRAY_3CS_T;
/*----------------------------------------------------------------------------------------
...
int CVD_agc_half_en_control(int enable);
...
int CVD_Set_Burst_Mag_12(int burst1_start, int burst1_end, int burst2_start, int burst2_end);
