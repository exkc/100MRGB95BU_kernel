...
static LX_MEMCFG_T	vdec_l_static_rgn_start 	= { .name = "static(DE_L)",	.base = 0x0, .size = 0 };	/* mem separator */
...
#if 1   //nalm
   	LX_MEMCFG_ENTRY(de_l_static_rgn_start), //nalm
#ifdef INCLUDE_KDRV_DE
...
//	LX_MEMCFG_ENTRY(de_g_static_rgn_start), //nalm  remove L&G separater
#ifdef INCLUDE_KDRV_DE
...
//-----------------------------------------------------------------------------------
	//	DDR (RGN_GBUS_DEV)
	//----------------------------------------------------------------------------------
	//LX_MEMCFG_ENTRY(gMemCfgHdr[DDR_RGN_GBUS_DEV]),  //nalm  //0x80000000
	...
	//LX_MEMCFG_ENTRY(ddr1_static_rgn_start), //nalm 0x80000000????????????????????
   // LX_MEMCFG_ENTRY_EX(gMemCfgM1Calib, MEM_DESC(CALIB)),	/* CALIB */     //nalm
...
static void BASE_PLT_SetupCommon(void)
{
...
#ifdef PLATFORM_FPGA
	vdec_l_static_rgn_start.base = 0x1c000000;   //nalm
	de_l_static_rgn_start.base = 0x68000000;   //nalm
	de_g_static_rgn_start.base = 0x80000000;   //nalm => not use
#else
#endif
    ...
    switch(g_model_ctx.ddr_size)
    {
	 ...
	 vdec0_hma.base = 0x1c000000;   //nalm 
	 //vdec0_hma.base = ddr0_static_rgn_start.base - (vdec0_hma.size + vdec1_hma.size);
#endif

	gfx_static_rgn_start.base = 0xB8000000;   //nalm
	/* setup AUDIO */
#if 1    
    switch(g_model_ctx.ddr_size)
    {
...
static void BASE_PLT_Setup_2_0_GB_ADV(void)
{
	...
	printk("@@@@@@@@@@@@@@@@@@@@@@@@@@  2.0GB  @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
	printk("configure O20(%2X) kdriver memory (2.0GB DDR/ADV/CMA_STAGE_%d)\n",
...
static void BASE_PLT_DoPostMemoryLayout(void)
{
	...
	switch(g_model_ctx.ddr_size)
	{
		...
		default:						ddr0_section[0].size = UHD_VDEC_4K_2K_DPB_SZ; /* 960 MB */ break;
	}
