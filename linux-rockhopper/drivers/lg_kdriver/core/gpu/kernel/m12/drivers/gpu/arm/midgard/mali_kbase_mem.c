...
int kbase_gpu_mmap(struct kbase_context *kctx, struct kbase_va_region *reg, u64 addr, size_t nr_pages, size_t align)
{
	...
	if (reg->gpu_alloc->type == KBASE_MEM_TYPE_ALIAS) {
		...
		KBASE_DEBUG_ASSERT( reg->gpu_alloc->imported.umm.current_mapping_usage_count < UINT_MAX );
...
int kbase_gpu_munmap(struct kbase_context *kctx, struct kbase_va_region *reg)
{
	...
	if (reg->gpu_alloc && reg->gpu_alloc->type == KBASE_MEM_TYPE_ALIAS) {
		...
		KBASE_DEBUG_ASSERT( reg->gpu_alloc->imported.umm.current_mapping_usage_count > 0 );
		reg->gpu_alloc->imported.umm.current_mapping_usage_count--;
		if (0 == reg->gpu_alloc->imported.umm.current_mapping_usage_count) {
