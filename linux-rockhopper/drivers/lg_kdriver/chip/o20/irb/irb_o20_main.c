...
/*----------------------------------------------------------------------------------------
	global Functions
---------------------------------------------------------------------------------------*/
static int o20_irbs_enable = 1;
static int o20_irb_inited = 0;

module_param(o20_irbs_enable, int, S_IRUGO);
MODULE_PARM_DESC(o20_irbs_enable, "o20 irbs disable");


static irbs_o20_func_t* irb_o20_func = NULL;
 ...
 */
int o20_irb_ioctl_main (unsigned int cmd, unsigned long arg)
{
...
static int o20_irb_ioctl (unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	...
	ret = o20_irb_ioctl_main(cmd, arg);

    return ret;
}

static int o20_irb_suspend(void)
{

	return 0;
}

static int o20_irb_resume(void)
{
	irb_o20_func = get_o20_irb_func();
	irb_o20_func->reinit();
...
static irbs_driver_t o20_irbs_driver =
{
	.init = o20_irb_init,
	.ioctl = o20_irb_ioctl,
	.suspend = o20_irb_suspend,
	.resume = o20_irb_resume,
};

irbs_driver_t* get_o20_irb_driver(void)
{
	return &o20_irbs_driver;
}
