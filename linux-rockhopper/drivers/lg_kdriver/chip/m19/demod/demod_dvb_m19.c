...
UINT16 			gM19GBB_FullDefaultPkerrCount		= 9;// orignal value is 9
UINT16 			gM19GBB_FullDVBT2PkerrCount			= 12;// orignal value is 9
UINT16 			gM19GBB_FullDVBPkerrThreshold		= DVB_FULL_PACKET_ERROR;

extern 	UINT8 	gM19GBB_PowerOnInit;
unsigned int 	gM19GBB_PowerOnInitStartTimeDVB	= 0;
 ...
 ************************************************************************************/
static	UINT16			gM19GBB_DVB_UnlockCount_full_packeterr	= 0;

#if 0
#endif

int DEMOD_M19_DVB_SW_Init(void)
{
	...
	{
		printk("[M19 LGDBB] (%s:%d) DEMOD_M19_DVB_SW_Init  Fail!! \n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}

	if(DEMOD_L2_SAT_SW_Init(pFrontend) != NO_DEMOD_ERROR)     // SOC
	{
		printk("[M19 LGDBB] (%s:%d) DEMOD_L2_SAT_SW_Init  Fail!! \n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}

	printk("########## DEMOD_M19_DVB_SW_Init##########\n");

	return RET_OK;
}

int _DEMOD_M19_DVBC_CheckTuneMode(DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int _DEMOD_M19_DVBT_CheckTuneMode(DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int _DEMOD_M19_DVBT2_CheckTuneMode(DEMOD_CTX_T *pDemodInfo)
{
	UINT8  new_tuneMode;
...
int _DEMOD_M19_LGDBB_DVBC_SetDemod(DEMOD_CTX_T *pDemodInfo)
{
	...
	symbol_rate_Kbps 			= 0;
	new_tuneMode 			= _DEMOD_M19_DVBC_CheckTuneMode(pDemodInfo);
	symbol_rate_Kbps 			= pDemodInfo->setCfgParam.dvbc.symbolRate;// KSym/s


	if(RET_OK != DEMOD_M19_Get_BW(pDemodInfo, &dvb_c_bandwidth_Khz))
	{
		DEMOD_PRINT("[DEMOD M19 DBB] FAIL (%s:%d) DEMOD_M19_Get_BW !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}
	 DEMOD_PRINT("[_DEMOD_M19_LGDBB_DVBC_SetDemod] SetDemod DVBC check ter_bandwidth_KHz[%d] \n", dvb_c_bandwidth_Khz);
...
int _DEMOD_M19_LGDBB_DVBT_SetDemod(DEMOD_CTX_T *pDemodInfo)
{
	...
	freq_Khz					= 0;
	new_tuneMode			= _DEMOD_M19_DVBT_CheckTuneMode(pDemodInfo);
	...
	printk("[M19 _DEMOD_M19_LGDBB_DVBT_SetDemod] SetDemod DVBT \n");

	if(RET_OK != DEMOD_M19_Get_BW(pDemodInfo, &dvb_t_bandwidth_Khz))
	{
		DEMOD_PRINT("[DEMOD M19 DBB] FAIL (%s:%d) DEMOD_M19_Get_BW !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}
	 DEMOD_PRINT("[_DEMOD_M19_LGDBB_DVBT_SetDemod] SetDemod DVBT check dvb_t_bandwidth_Khz[%d] \n", dvb_t_bandwidth_Khz);
...
int _DEMOD_M19_LGDBB_DVBT2_SetDemod(DEMOD_CTX_T *pDemodInfo)
{
	...
	freq_Khz					= 0;
	new_tuneMode			= _DEMOD_M19_DVBT2_CheckTuneMode(pDemodInfo);
	symbol_rate_Kbps			= 0;// KSym/s
	dvbt2_constellation			= DEMOD_DD_CONSTELATION_PROP_AUTO;
	dvbt2_PLPId				= pDemodInfo->setCfgParam.dvbt2.unPLP;
	symbol_rate_Kbps			= 0;

	if(RET_OK != DEMOD_M19_Get_BW(pDemodInfo, &dvbt2_bandwidth_Khz))
	{
		DEMOD_PRINT("[DEMOD M19 DBB] FAIL (%s:%d) DEMOD_M19_Get_BW !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}
	DEMOD_PRINT("[_DEMOD_M19_LGDBB_DVBT2_SetDemod] SetDemod DVBT2 check dvbt2_bandwidth_Khz[%d] \n", dvbt2_bandwidth_Khz);
	DEMOD_PRINT("[_DEMOD_M19_LGDBB_DVBT2_SetDemod] new_tuneMode : 0x%02x*****************\n", new_tuneMode);
	...
	DEMOD_PRINT("[DEMOD M19 DBB] _DEMOD_M19_LGDBB_DVBT2_SetDemod => PLP ID = 0x%02x !!!!!\n", dvbt2_PLPId);
	return RET_OK;

}


int _DEMOD_M19_LGDBB_DVB_CheckSignalState(DEMOD_CTX_T *pDemodInfo)
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBC_AutoSymbolRateDet
...
*/
int DEMOD_M19_DVBC_AutoSymbolRateDet(LX_DEMOD_SYMRDET_MODE_T symrateMode, UINT16 SymbolRate )
{
	LX_DEMOD_L2_Context 	*pFrontend;
	UINT8 return_code;
...
************************************************************************************/
/**
 * DEMOD_M19_DVBC_IsSymbolRateAuto
 * Enable/Diable automatic symbol rate detection on DVB-C mode.
 *
 * @param  *bAuto - BOOLEAN.
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBC_IsSymbolRateAuto(BOOLEAN *bAuto)
{
	UINT8 					autoEn;
	LX_DEMOD_L2_Context 	*pFrontend;
...
************************************************************************************/
/**
 * DEMOD_M19_DVBC_IsQammodeAutoDet
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBC_IsQammodeAutoDet (BOOLEAN *bAutoQam)
{
	UINT8 					autoDet;
	LX_DEMOD_L2_Context 	*pFrontend;
...
************************************************************************************/
/**
 * DEMOD_M19_DVBC_Set_DefaultRegisterValue
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBC_Set_DefaultRegisterValue(void)
{
	gM19GBB_DVB_UnlockCount_full_packeterr = 0;
...
************************************************************************************/
/**
 * DEMOD_M19_DVBC_Set_Config_auto
 * Control automatic setting of DVBC config.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBC_Set_Config_auto (BOOLEAN bAutoDetect)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBC_Set_QamMode (LX_DEMOD_RF_MODE_T constel)
{
	UINT8  					dvb_c_constellation;
	UINT8 					return_code;
	LX_DEMOD_L2_Context 	*pFrontend;
...
************************************************************************************/
/**
 * DEMOD_M19_DVBC_Set_NeverLockWaitTime
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBC_Set_NeverLockWaitTime(UINT32 waitMs)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_M19_DVBC_Get_SymbolRateDetect
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBC_Get_SymbolRateDetect(UINT16 *pSymbolRate)
{
	LX_DEMOD_L2_SIG_STATUS 	signalStatus;
	LX_DEMOD_L2_Context 		*pFrontend;
	...
	if (DEMOD_L2_SignalStatus(pFrontend, &signalStatus))
	{
		DEMOD_PRINT_ALWAYS("[M19 LGDBB] (%s:%d) DEMOD_L2_SignalStatus  Fail!! \n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}

	*pSymbolRate = signalStatus.sym_rate ;
	DEMOD_PRINT("[DEMOD_M19_DVBC_Get_SymbolRateDetect] (sym_rate: %d) \n", *pSymbolRate);
...
************************************************************************************/
/**
 * DEMOD_M19_DVBC_Get_SymbolRateStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBC_Get_SymbolRateStatus(LX_DEMOD_SYMBOLRATE_STATE_T *pSymbolRateDet)
{
	LX_DEMOD_L2_SIG_STATUS 	signalStatus;
	LX_DEMOD_L2_Context 		*pFrontend;
	pFrontend = &gM19_LGDBB_Frontend;
	...
	DEMOD_PRINT("[DEMOD_M19_DVBC_Get_SymbolRateStatus] (symbolRate: %d) \n", pSymbolRateDet->symbolRate);
...
************************************************************************************/
/**
 * DEMOD_M19_DVBC_Get_QamModeDetectStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBC_Get_QamModeDetectStatus(LX_DEMOD_QAMMODE_STATE_T *pQamModeDet)
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBC_Obtaining_Signal_Lock.
...
int DEMOD_M19_DVBC_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				if(RET_OK != _DEMOD_M19_LGDBB_DVBC_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT("[DEMOD M19] FAIL (%s:%d) _DEMOD_M19_LGDBB_DVBC_SetDemod() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
					...
					(LX_DEMOD_TUNE_SPEC_DVBC_FIXED_DATA != pDemodInfo->setCfgParam.dvbc.tuneMode))
				{
					...
					gM19GBB_FullDVBPkerrThreshold 	= DVB_FULL_PACKET_ERROR;
					DEMOD_PRINT( "^R^[DEMOD M19 GBB] LX_DEMOD_SYNC_CHECKING(AUTO or Manual SCAN) : Going to LX_DEMOD_TUNE_SCAN..... ^0^\n");
				}
				else
				{
					pDemodInfo->scanJobFlag 	  	= LX_DEMOD_FLAG_FINISH;
					pDemodInfo->controlState 		= LX_DEMOD_SIGNAL_STABLE;
					gM19GBB_FullDVBPkerrThreshold 	= DVB_UNCHECK_PACKET_ERROR;

					DEMOD_PRINT( "^R^[DEMOD M19 GBB] LX_DEMOD_SYNC_CHECKING(NORMAL SCAN) : Going to LX_DEMOD_SIGNAL_STABLE..... ^0^\n");
				}
			}
			break;
											/***************************************************/
		...
		case LX_DEMOD_TUNE_SCAN:			/***************************************************/
			{
				...
				{
					DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_NeverlockScan() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					DEMOD_PRINT( "^R^[DEMOD M19 GBB] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_SIGNAL_STABLE..... %d [ms] ^0^\n",sigStablestartTime);
				}
				else
				{
...
//				UINT16 							symbolRateDiff;
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.dvbc.tuneMode)
					{
						...
						DEMOD_PRINT_ALWAYS( "[DEMOD M19 GBB] LX_DEMOD_SIGNAL_STABLE : MSG_DVBC_FRONT_END_LOCKED..... ^^0^^\n");
						...
						{
							DEMOD_PRINT_ALWAYS( "[DEMOD M19 GBB] LX_DEMOD_SIGNAL_STABLE : break, Elapsed Time = %d [ms] ..... ---0---\n", elapsedTime);
							break;
						}
						else
						{
...
/*
						if(LX_DEMOD_TUNE_SPEC_DVBC_FIXED_DATA == pDemodInfo->setCfgParam.dvbc.tuneMode)
						{
							...
							if (RET_OK != DEMOD_M19_DVBC_Get_SymbolRateStatus(&symbolRateDet))
							{
							...
							DEMOD_PRINT( "[DEMOD M19 GBB] symbolRateDiff  = %d\n",symbolRateDiff);
							DEMOD_PRINT( "[DEMOD M19 GBB] symbolRateDet.symbolRate = %d, pDemodInfo->setCfgParam.dvbc.symbolRate = %d\n",symbolRateDet.symbolRate, pDemodInfo->setCfgParam.dvbc.symbolRate);

							if(((symbolRateDet.status == LX_DEMOD_DETECTED) && (symbolRateDiff > 60)) || (symbolRateDet.status != LX_DEMOD_DETECTED))
							{
								if(RET_OK != DEMOD_M19_DVBC_AutoSymbolRateDet(LX_DEMOD_SYMR_FIXED_MODE, pDemodInfo->setCfgParam.dvbc.symbolRate))
								{
									DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBC_AutoSymbolRateDet() !!!\n", __F__, __L__);
									DEMOD_RETURN_FAIL(__F__);
								}
...
*/
					...
					if (RET_OK != _DEMOD_M19_LGDBB_DVB_CheckSignalState(pDemodInfo))
					{
						DEMOD_PRINT( "^r^[DEMOD M19 GBB] FAIL (%s:%d) _DEMOD_M19_LGDBB_DVB_CheckSignalState() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				}
				else
				{
					pDemodInfo->controlState	= LX_DEMOD_SIGNAL_STABLE;
					DEMOD_PRINT( "^R^[DEMOD M19 GBB] LX_DEMOD_SYNC_CHECKING : Going back to LX_DEMOD_SIGNAL_STABLE.....\n");
				}
	   		}
			break;
...
int DEMOD_M19_DVBC_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		DEMOD_PRINT( "^r^[DEMOD M19 GBB] (%s:%d) DVB_C Skip reset of LX_DEMOD_LOCKED_PAUSE_TIME !!!\n", __F__, __L__);
...
*/		
		...
		if(pDemodInfo->signalStatus.packetError >= gM19GBB_FullDVBPkerrThreshold)
		{
			gM19GBB_DVB_UnlockCount_full_packeterr++;
			DEMOD_PRINT( "^r^[DEMOD M19 GBB](%s) gM19GBB_DVB_UnlockCount_full_packeterr = %d !!!\n", __F__,gM19GBB_DVB_UnlockCount_full_packeterr);

			if(gM19GBB_DVB_UnlockCount_full_packeterr == gM19GBB_FullDefaultPkerrCount)
			{
				DEMOD_PRINT( "^r^[DEMOD M19 GBB] oops !! (%s) Force to NOT LOCK !!\n", __F__);
				gM19GBB_DVB_UnlockCount_full_packeterr = 0;
/*
				if(RET_OK != DEMOD_M19_DVBC_Set_Config_auto(TRUE))
				{
					DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBC_Set_Config_auto() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				DEMOD_PRINT("[DEMOD M19 GBB](%s) DEMOD_M19_DVBC_Set_Config_auto(TRUE) !!!\n", __F__);
*/
				if(RET_OK != DEMOD_M19_SoftwareReset())
				{
					DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_SoftwareReset() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				DEMOD_PRINT("[DEMOD M19 GBB](%s) DEMOD_SoftwareReset() !!!\n", __F__);

				pDemodInfo->lockStatus				= LX_DEMOD_UNLOCKED;
				pDemodInfo->bNotifiedUnlock 			= TRUE;
				pDemodInfo->bNotifiedLock			= FALSE;
				pDemodInfo->signalStatus.bSignalValid 	= FALSE;
				DEMOD_PRINT_ALWAYS( "[DEMOD M19 GBB] MONITORING FUNC : MSG_DVBC_FRONT_END_NOT_LOCKED..... -_-;;\n");
			}
		}
		else
		{
			...
			if(pDemodInfo->lockCount == 2)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD M19 GBB] MONITORING FUNC : MSG_DVBC_FRONT_END_LOCKED..... ^0^\n");
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBC_Get_DvbInfo(void)
{
	UINT32 							errorCount = 0;
	UINT32 							vBer = 0;
	UINT16 							ifAGCGain = 0;
	...
	//IF AGC
/*	if (RET_OK != DEMOD_M19_Get_IFAGC(&ifAGCGain))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M16_Get_IFAGC() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	...
	DEMOD_PRINT("[DEMOD M19 GBB] IF AGC Gain = 0x%x !!!\n",ifAGCGain);

 	//NeverLock status
	if (RET_OK != DEMOD_M19_Get_NeverLockStatus(LX_DEMOD_DVBC, &neverLockStatus))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M16_DVBC_Get_NeverLockStatus() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	...
	switch(neverLockStatus)
	{
		case LX_DEMOD_DETECTION_PROCESSING :
			DEMOD_PRINT("[DEMOD M19 GBB] Signal Checking process !!!\n");
			break;

		case LX_DEMOD_DETECTED :
			DEMOD_PRINT("[DEMOD M19 GBB] normal State (available channel)  !!!\n");
			break;

		case LX_DEMOD_DETECTION_FAIL :
			DEMOD_PRINT("[DEMOD M19 GBB] neverlock state (empty channel) !!!\n");
			break;

		default :
			DEMOD_PRINT("[DEMOD M19 GBB] INVALID neverLockStatus !!!\n");
			break;
	}

	// Carrier LOCK status  and offset monitoring

	if (RET_OK != DEMOD_M19_Get_Lock(LX_DEMOD_CARR_LOCK,&carrLockState) )
	{
	...
	{
		DEMOD_PRINT("[DEMOD M19 GBB] DVB CARRIER is LOCKED  !!!\n");
	}
	else
	{
		DEMOD_PRINT("[DEMOD M19 GBB] DVB CARRIER is NOT LOCKED  !!!\n");
	}

	// Sampling frequency LOCK status and offset monitoring
	if (RET_OK != DEMOD_M19_Get_Lock( LX_DEMOD_TR_LOCK, &samplingLock))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M16_Get_Lock() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if(LX_DEMOD_LOCKED == samplingLock)
	{
		DEMOD_PRINT("[DEMOD M19 GBB] Sampling frequency is Locked   !!!\n");
	}
	else
	{
		DEMOD_PRINT("[DEMOD M19 GBB] Sampling frequency is NOT Locked !!!\n");
	}
	//Symbol rate detection

	if (RET_OK != DEMOD_M19_DVBC_IsSymbolRateAuto(&isAutoSymbol))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M16_DVBC_Get_SymbolRateStatus() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
...
*/
		...
		DEMOD_PRINT("[DEMOD M19 GBB] Symbol rate  is detected !!!\n");
		DEMOD_PRINT("[DEMOD M19 GBB] Symbol rate = %d [KHz] !!!\n",symbolRateDet.symbolRate);
		
/*		switch(symbolRateDet.status)
		{
			case LX_DEMOD_IDLE :
			{
				DEMOD_PRINT("[DEMOD M19 GBB] Symbol rate status is idle status!!!\n");
				break;
			}
			case LX_DEMOD_DETECTED :
			{
			...
			case LX_DEMOD_DETECTION_PROCESSING :
			{
				DEMOD_PRINT("[DEMOD M19 GBB] Symbol rate detection is processing !!!\n");
				break;
			}
			case LX_DEMOD_DETECTION_FAIL :
			{
				DEMOD_PRINT("[DEMOD M19 GBB] Symbol rate detection is failed !!!\n");
				break;
			}
			default :
			{
				DEMOD_PRINT("[DEMOD M19 GBB] INVALID Symbol rate detection status  !!!\n");
				break;
			}
		}
*/
	...
	if (RET_OK != DEMOD_M19_DVBC_IsQammodeAutoDet(&isAutoQammode))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M16_DVBC_IsQammodeAutoDet() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	
	if(TRUE == isAutoQammode)
	{
		if (RET_OK != DEMOD_M19_DVBC_Get_QamModeDetectStatus(&qamModeDet))
		{
		...
		switch(qamModeDet.status)
		{
			case LX_DEMOD_IDLE :
			{
				DEMOD_PRINT("[DEMOD M19 GBB] QAM mode detect is idle status!!!\n");
				break;
			}
			case LX_DEMOD_DETECTED :
			{
				DEMOD_PRINT("[DEMOD M19 GBB] QAM mode is detected!!!\n");
				DEMOD_PRINT("[DEMOD M19 GBB] QAM mode is %s !!!\n",(qamModeDet.qamMode == LX_DEMOD_16QAM) ? "16QAM" :
			...
			case LX_DEMOD_DETECTION_PROCESSING :
			{
				DEMOD_PRINT("[DEMOD M19 GBB] QAM mode detection is processing !!!\n");
				break;
			}
			case LX_DEMOD_DETECTION_FAIL :
			{
				DEMOD_PRINT("[DEMOD M19 GBB] QAM mode detection is failed !!!\n");
				break;
			}
			case LX_DEMOD_DETECTION_MANUAL :
			{
				DEMOD_PRINT("[DEMOD M19 GBB] Manual QAM mode detection mode !!!\n");
				break;
			}
			default :
			{
				DEMOD_PRINT("[DEMOD M19 GBB] INVALID QAM mode detection  !!!\n");
				break;
			}
		}
	}
	...
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_Get_SpectrumStatus() !!!\n", __F__, __L__);
	...
	//Frame lock status
	if (RET_OK != DEMOD_M19_Get_Lock(LX_DEMOD_FEC_LOCK, &fecLockState) )
	{
		DEMOD_PRINT("[LX_DEMOD_DTV] FAIL (%s:%d) DEMOD_M19_Get_Lock() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
...
/*	if (RET_OK != DEMOD_M19_Get_MseInfo(&sigMse))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_Get_MseInfo() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	sigMse.bMse 		= 0;
	...
	DEMOD_PRINT("[DEMOD M19 GBB] mse = %d, constPwr = %d, !!!\n",sigMse.mse, sigMse.constPwr);
 	//BER after Viterbi
/*	if (RET_OK != DEMOD_M19_Get_VABER(&vBer))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_Get_VABER() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	vBer = signalStatus.ber;
	DEMOD_PRINT("[DEMOD M19 GBB] vBER = %d  !!!\n",vBer);
	//TP Error count
...
*/
	...
	DEMOD_PRINT("[DEMOD M19 GBB] TP Error Count= %d !!!\n",errorCount);
	
#ifndef DEMOD_PRINT

	printk("[DEMOD M19 GBB] IF AGC Gain = 0x%x !!!\n",ifAGCGain);
	printk("[DEMOD M19 GBB] DVB CARRIER is %s  !!!\n",(carrLockState == LX_DEMOD_LOCKED) ? "locked" : "NOT locked" );
	printk("[DEMOD M19 GBB] Carrier offset Frequency = %d [KHz] !!!\n",carrierLock.carrOffset);
	printk("[DEMOD M19 GBB] Sampling frequency is %s   !!!\n",(samplingLock == LX_DEMOD_LOCKED) ? "locked" : "NOT locked");
	printk("[DEMOD M19 GBB] QAM mode is %s   !!!\n",(qamModeDet.status == LX_DEMOD_DETECTED) ? "detected" : "NOT detected");
	...
	printk("[DEMOD M19] Spectrum auto = %s, Inversion  = %s !!!\n",(spectrumAuto == LX_DEMOD_AUTO) ? " Auto" : " Manual",
																		(spectrumInv == LX_DEMOD_INVERSION) ? " Inverted" : " Normal" );
	printk("[DEMOD M19 GBB] FEC lock is %s !!!\n",(fecLockState == LX_DEMOD_LOCKED) ? "locked" : "NOT locked");
	printk("[DEMOD M19 GBB] MSE = %d, constPwr = %d !!!\n",sigMse.mse, sigMse.constPwr);
	printk("[DEMOD M19 GBB] vBER = %d  !!!\n",vBer);
	printk("[DEMOD M19 GBB] TP Error Count= %d !!!\n",errorCount);
...
/**
 * DEMOD_M19_DVBT_Set_DefaultRegisterValue
 * Set defualt register value on DVB-T.
 * This function is applied only to A0 version, not to A1 version.
 *
 * @param none
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Set_DefaultRegisterValue(LX_DEMOD_BWMODE_T band)
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Set_Config_auto
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Set_Config_auto (BOOLEAN bAutoDetect)
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Set_PartialConfig
 * Set hierachical signal path and bandwidth.
 * This function is used when LX_DEMOD_M19_DVBT_Set_Config_auto is enabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Set_PartialConfig (LX_DEMOD_DVBT_CONFIG_T *pDvbtConfigParam)
{
	LX_DEMOD_L2_Context *pFrontend;
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when LX_DEMOD_M19_DVBT_Set_Config_auto is disabled.
 *
 * @param *pDvbtConfigParam 	[IN] LX_DEMOD_DVBT_CONFIG_T - DVB-T config. structure
 * @return if Succeeded - RET_OK else-RET_ERROR
 * @see
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Set_FullConfig (LX_DEMOD_DVBT_CONFIG_T *pDvbtConfigParam)
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Set_NeverLockWaitTime(UINT32 waitMs)
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Set_TafcWaitSymbol
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Set_TafcWaitSymbol(BOOLEAN bDelayed)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Set_CrGearshEn(BOOLEAN bEnable)
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Get_PreGhostDetection
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Get_PreGhostDetection(BOOLEAN *pIsExist)
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Get_DelaySpreadStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Get_DelaySpreadStatus(UINT16 *pDelaySpread)
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Get_Hierach_HPSel
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Get_Hierach_HPSel( BOOLEAN *pBSetHp )
{
	LX_DEMOD_L2_Context 		*pFrontend;
	LX_DEMOD_L2_SIG_STATUS		signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Get_FFTMode( LX_DEMOD_LOCK_STATE_T *pTxModeLock, LX_DEMOD_FFTMODE_T *pFftMode )
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Get_GuradIntervalMode
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Get_GuradIntervalMode( LX_DEMOD_GIMODE_T *pGIMode )
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Get_HierachyMode
 * Get the information of hierachical mode on DVB-T.
 *
 * @param *pHierachMode[OUT] DEMOD_M19_DVB_TPS_HIERACHY_T - alpha = none, 1, 2 and 4.
 * @return if Succeeded - RET_OK else-RET_ERROR
 * @see
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Get_HierachyMode( LX_DEMOD_DVB_TPS_HIERACHY_T *pHierachMode )
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Get_LpCoderRate
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Get_LpCoderRate( LX_DEMOD_CODERATE_T *pLpCodeRate )
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Get_HpCoderRate
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Get_HpCoderRate( LX_DEMOD_CODERATE_T *pHpCodeRate )
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Get_CellId(UINT8 *pCellIDLength, UINT16 *cell_id)
{
	LX_DEMOD_L2_Context 			*pFrontend;
	LX_DEMOD_L2_SIG_STATUS			signalStatus;
	...
	*cell_id = signalStatus.cell_id;
	...
	DEMOD_PRINT("[DEMOD_M19_DVBT_Get_CellId] Cell ID = 0x%x !!!\n", *cell_id);
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Get_TpsInfo( LX_DEMOD_DVBT_CONFIG_T *pTpsParams)
{
	BOOLEAN 						bSetHp = TRUE;
	...
	if(NULL == pTpsParams)
	{
		DEMOD_PRINT("[DEMOD M19 GBB] ERROR, DEMOD_M19_DVBT_CONFIG_T TpsParams is NULL !!!\n");
		return RET_ERROR;
	}

	// Get bSetHp
/*
	if (RET_OK != DEMOD_M19_DVBT_Get_Hierach_HPSel( &bSetHp ))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBT_Get_Hierach_HPSel() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	if(signalStatus.hierarchy_sel == DEMOD_DD_DVBT_STATUS_RESPONSE_HIERARCHY_SEL_HP)
	{
		bSetHp = TRUE;		
		DEMOD_PRINT("[DEMOD M19 GBB] Set HP selection = %s !!!\n", " HP Mode");
	}
	else
	{
		bSetHp = FALSE;		
	...
	// Get channelBW
/*
	if (RET_OK != DEMOD_M19_Get_BandWidthMode( &channelBW ))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBT_Get_BandWidthMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	switch(signalStatus.bandwidth)
	{
		case 0x08:
			...
			DEMOD_PRINT("[DEMOD M19 GBB] Bandwidth = %s !!!\n", "LX_DEMOD_BW_8MHZ" );
			break;
	...
	// Get FFT mode
/*
	if (RET_OK != DEMOD_M19_DVBT_Get_FFTMode( &txModeLock, &FFTMode ))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBT_Get_FFTMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	switch(signalStatus.fftMode)
	{
		case DEMOD_DD_DVBT_STATUS_RESPONSE_FFT_MODE_2K :
			...
			DEMOD_PRINT("[DEMOD M19 GBB] FFTMode = %s  !!!\n","FFT_2K");
			break;
...
/*	if (RET_OK != DEMOD_M19_DVBT_Get_GuradIntervalMode( &gIMode ))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBT_Get_GuradIntervalMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	switch(signalStatus.guardInterval)
	{
		case DEMOD_DD_DVBT_STATUS_RESPONSE_GI_1_32 :
			...
			DEMOD_PRINT("[DEMOD M19 GBB] Guard Intterval = %s  !!!\n", "1_32" );
			break;
		case DEMOD_DD_DVBT_STATUS_RESPONSE_GI_1_16:
...
/*
	if (RET_OK != DEMOD_M19_DVBT_Get_HierachyMode( &hierachMode ))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBT_Get_HierachyMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	switch(signalStatus.hierarchy)
	{
		case DEMOD_DD_DVBT_STATUS_RESPONSE_HIERARCHY_NONE :
			...
			DEMOD_PRINT("[DEMOD M19 GBB] Hierachical Mode = %s  !!!\n", "NON Hierachical" );
			break;
		case DEMOD_DD_DVBT_STATUS_RESPONSE_HIERARCHY_ALFA1:
	...
	// Get LP coder rate
/*	if (RET_OK != DEMOD_M19_DVBT_Get_LpCoderRate( &lpCodeRate ))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBT_Get_LpCoderRate() !!!\n", __F__, __L__);
		return RET_ERROR;
	}*/
	...
	if(signalStatus.hierarchy_sel == DEMOD_DD_DVBT_STATUS_RESPONSE_HIERARCHY_SEL_LP)
	{
		switch(signalStatus.codeRate)
		{
			case DEMOD_DD_DVBT_STATUS_RESPONSE_LP_CR_1_2 :
				...
				DEMOD_PRINT("[DEMOD M19 GBB] LP code rate = %s  !!!\n", "Code_1_2" );
				break;
			case DEMOD_DD_DVBT_STATUS_RESPONSE_LP_CR_2_3:
...
/*	if (RET_OK != DEMOD_M19_DVBT_Get_HpCoderRate( &hpCodeRate ))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBT_Get_HpCoderRate() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	if(signalStatus.hierarchy_sel == DEMOD_DD_DVBT_STATUS_RESPONSE_HIERARCHY_SEL_HP)
	{
		switch(signalStatus.codeRate)
		{
			case DEMOD_DD_DVBT_STATUS_RESPONSE_HP_CR_HP_1_2 :
				...
				DEMOD_PRINT("[DEMOD M19 GBB] HP code rate = %s  !!!\n", "Code_1_2" );
				break;
			case DEMOD_DD_DVBT_STATUS_RESPONSE_HP_CR_HP_2_3:
	...
	// get QAM mode
	if (RET_OK != DEMOD_M19_Get_QAMMode(&qamMode ))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVB_Get_QAMMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	pTpsParams->qamMode = qamMode;
	DEMOD_PRINT("[DEMOD M19 GBB] modulation  = %s  !!!\n",
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Get_TotalInfo( void)
{
...
/*		if (RET_OK != DEMOD_M19_Get_IFAGC(&ifagcValue))
		{
			DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_Get_IFAGC() !!!\n", __F__, __L__);
			ifagcValue = 0;
		}
*/
	...
	if (RET_OK != DEMOD_M19_Get_NeverLockStatus(LX_DEMOD_DVBT, &neverLockStatus))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVB_Get_NeverLockStatus() !!!\n", __F__, __L__);
...
/*	if (RET_OK != DEMOD_M19_DVBT_Get_FFTMode( &txModeLock, &fftMode ))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBT_Get_SpectrumInvStatus() !!!\n", __F__, __L__);
	}
*/
	switch(signalStatus.fftMode)
	{
...
/*	if (RET_OK !=  DEMOD_M19_Get_Packet_Error(&errorCount))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBT_Get_TPError() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	errorCount = signalStatus.packetError;
...
/*
	...
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
		FreqOff = 0xffff;
	}
*/
...
/*	if (RET_OK != DEMOD_M19_Get_MseInfo(&sigMse))
	{
		DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBC_Get_SNR() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	sigMse.bMse 		= 0;
	...
	DEMOD_PRINT("[DEMOD M19] Spectrum auto = %s, Inversion  = %s !!!\n",(spectrumAuto == LX_DEMOD_AUTO) ? " Auto" : " Manual",
																					(spectrumInv == LX_DEMOD_INVERSION) ? " Inverted" : " Normal" );
	DEMOD_PRINT("[DEMOD M19] FFT mode LOCK = %s !!!\n",(txModeLock == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD M19] offset = %d [KHz]!!!\n",FreqOff);
	DEMOD_PRINT("[DEMOD M19] mse = %d, constPwr = %d, !!!\n",sigMse.mse, sigMse.constPwr);

	if (RET_OK != DEMOD_M19_DVBT_Get_TpsInfo( &tppsParams))
	{
		DEMOD_PRINT("[DEMOD M19] FAIL (%s:%d) DEMOD_M19_DVBT_Get_TpsInfo() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	...
	printk("[DEMOD M19] FFT mode LOCK = %s !!!\n",(txModeLock == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");


	// Get bSetHp
	printk("[DEMOD M19] Set HP selection = %s !!!\n",(tppsParams->bSetHp == TRUE) ? " HP Mode" : " LP Mode");

	// Get channelBW
	printk("[DEMOD M19] Bandwidth = %s  !!!\n",
	...
	// Get Guard Interval mode
	printk("[DEMOD M19] Guard Intterval = %s  !!!\n",
...
//	tppsParams->hierachMode = hierachMode;
	printk("[DEMOD M19] Hierachical Mode = %s  !!!\n",
	...
	// Get LP coder rate
	printk("[DEMOD M19] LP code rate = %s  !!!\n",
	...
	// Get HP coder rate
	printk("[DEMOD M19] HP code rate = %s  !!!\n",
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_Get_IFO_LOCK( BOOLEAN *pIfoLock)
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_EqualizereReset.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT_EqualizereReset(void)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_M19_DVBT_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	...
	switch (pDemodInfo->controlState)
	{
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{		
				...
				currentTime = jiffies_to_msecs(jiffies);
				elapsedTime = currentTime - g_RetuneStartTime;
				DEMOD_PRINT("[DEMOD M19 LGDBB] ####START_TIME##### (%d ms)****^0^****\n", elapsedTime);
					
				if(RET_OK != _DEMOD_M19_LGDBB_DVBT_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT("[DEMOD M19 LGDBB] FAIL (%s:%d) _DEMOD_M19_LGDBB_DVBT_SetDemod() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.dvbt.tuneMode)
				{
					...
					pDemodInfo->controlState 		= LX_DEMOD_TUNE_SCAN;
					gM19GBB_FullDVBPkerrThreshold 	= DVB_FULL_PACKET_ERROR;
					DEMOD_PRINT( "^R^[DEMOD M19 GBB] LX_DEMOD_TUNE_START(AUTO or Manual SCAN) : Going to LX_DEMOD_TUNE_SCAN..... ^0^\n");
				}
				else
				{
					pDemodInfo->scanJobFlag 		= LX_DEMOD_FLAG_FINISH;
					pDemodInfo->controlState 		= LX_DEMOD_SYNC_CHECKING;
					gM19GBB_FullDVBPkerrThreshold 	= DVB_UNCHECK_PACKET_ERROR;
					DEMOD_PRINT( "^R^[DEMOD M19 GBB] LX_DEMOD_TUNE_START(NORMAL SCAN) : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
				}
				currentTime = jiffies_to_msecs(jiffies);
				elapsedTime = currentTime - g_RetuneStartTime;
				DEMOD_PRINT("[DEMOD M19 LGDBB] ***END_TIME*** (%d ms)****^0^****\n", elapsedTime);
			}
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				{
					DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_Get_OperMode() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				...
				{
					DEMOD_PRINT("[DEMOD ] before pDemodInfo->setCfgParam.dvbt2.unPLP = %d (%s:%d) DEMOD_M19_DVBT_Set_PartialConfig() !!!\n", pDemodInfo->setCfgParam.dvbt2.unPLP, __F__, __L__);
					...
					DEMOD_PRINT("[DEMOD ] after pDemodInfo->setCfgParam.dvbt2.tuneMode = %d (%s:%d) DEMOD_M19_DVBT_Set_PartialConfig() !!!\n", pDemodInfo->setCfgParam.dvbt2.tuneMode, __F__, __L__);
					DEMOD_PRINT("[DEMOD ] after pDemodInfo->setCfgParam.dvbt2.unPLP = %d (%s:%d) DEMOD_M19_DVBT_Set_PartialConfig() !!!\n", pDemodInfo->setCfgParam.dvbt2.unPLP, __F__, __L__);

#if 0
					if(RET_OK != DEMOD_M19_DVBT2_Set_PartialConfig(&(pDemodInfo->setCfgParam.dvbt2)))
					{
						DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_DVBT_Set_PartialConfig() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
#endif
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE: 		/********************************************/
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					DEMOD_PRINT_ALWAYS( "[DEMOD M19 GBB] LX_DEMOD_SIGNAL_STABLE : MSG_DVBT_FRONT_END_LOCKED..... ^^^0^^^\n");
					DEMOD_PRINT_ALWAYS( "[DEMOD M19 GBB] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^^0^^^\n", elapsedTime);
					...
					if (RET_OK != _DEMOD_M19_LGDBB_DVB_CheckSignalState(pDemodInfo))
					{
						DEMOD_PRINT( "^r^[DEMOD ] FAIL (%s:%d) _DEMOD_M19_LGDBB_DVB_CheckSignalState() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_M19_DVBT_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
...
*/	
		if (RET_OK != _DEMOD_M19_LGDBB_DVB_CheckSignalState(pDemodInfo))
		{
			DEMOD_PRINT( "[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_Get_signal_status() !!!\n", __F__, __L__);
			DEMOD_RETURN_FAIL(__F__);
		}
		...
		if(pDemodInfo->signalStatus.packetError >= gM19GBB_FullDVBPkerrThreshold)
		{
			...
			if(gM19GBB_DVB_UnlockCount_full_packeterr == gM19GBB_FullDefaultPkerrCount) //gM19GBB_FullDVBTPkerrCount
			{
				DEMOD_PRINT( "^r^[DEMOD M19 GBB] oops !! (%s) Force to NOT LOCK !!\n", __F__);
				gM19GBB_DVB_UnlockCount_full_packeterr = 0;
/*
				if(RET_OK != DEMOD_M19_DVBT_Set_Config_auto(TRUE))
				{
					DEMOD_PRINT("[DEMOD M19 GBB] FAIL (%s:%d) DEMOD_M19_DVBT_Set_Config_auto() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				DEMOD_PRINT("[DEMOD M19 GBB](%s) DEMOD_M19_DVBT_Set_Config_auto(TRUE) !!!\n", __F__);
*/
				if(RET_OK != DEMOD_M19_SoftwareReset())
				{
				...
				DEMOD_PRINT("[DEMOD M19 GBB](%s) DEMOD_M19_SoftwareReset() !!!\n", __F__);
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD M19 GBB] MONITORING FUNC : MSG_DVBT_FRONT_END_NOT_LOCKED..... -_-;;\n");
			}
		}
		else
		{
			...
			{
				pDemodInfo->lockCount++;
				DEMOD_PRINT( "^r^[DEMOD M19 GBB](%s) pDemodInfo->lockCount = %d !!!\n", __F__,pDemodInfo->lockCount);
			}
			if(pDemodInfo->lockCount == 1)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD M19 GBB] MONITORING FUNC : MSG_DVBT_FRONT_END_LOCKED..... ^0^\n");
...
*/
	...
	{	
		...
		{
			pDemodInfo->unLockCount++;
			DEMOD_PRINT( "^r^[DEMOD M19 GBB](%s) pDemodInfo->unLockCount = %d !!!\n", __F__,pDemodInfo->unLockCount);
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT2_Set_Config_auto
 * Control automatic setting of DVB-T config.
 *
 * @param bAutoDetect 	[IN] BOOLEAN - automatic setting (TRUE) / manual setting(FALSE).
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT2_Set_Config_auto (BOOLEAN bAutoDetect)
{
	LX_DEMOD_L2_Context *pFrontend;
	pFrontend = &gM19_LGDBB_Frontend;
...
************************************************************************************/
/**
 DEMOD_M19_DVBT2_Set_PartialConfig
 * Set all config parameters on DVB-T.
 * This function is used when LX_DEMOD_M19_DVBT_Set_Config_auto is disabled.
 *
 * @param *pDvbtConfigParam 	[IN] LX_DEMOD_DVBT_CONFIG_T - DVB-T config. structure
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT2_Set_PartialConfig (LX_DEMOD_DVBT2_CONFIG_T *pDvbt2ConfigParam)
{
	UINT8					plp_id;				
	...
	DEMOD_PRINT	("[DEMOD_M19_DVBT2_Set_PartialConfig] M19_DVBT2_PLP ID = [%02x]\n", plp_id);

	return_code 	= DEMOD_L2_DVB_Set_PLP_Select(pFrontend,  plp_id, DEMOD_DD_DVBT2_PLP_SELECT_CMD_PLP_ID_SEL_MODE_MANUAL);

	if(return_code != NO_DEMOD_ERROR)
	{
		DEMOD_PRINT ("[DEMOD_M19_DVBT2_Set_PartialConfig] DEMOD_L2_DVB_Set_PLP_Select error 0x%02x: %s\n", return_code, DEMOD_L1_API_Error_Text(return_code) );
		return RET_ERROR;
	}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT2_Get_FFTMode( LX_DEMOD_LOCK_STATE_T *pTxModeLock, LX_DEMOD_FFTMODE_T *pFftMode )
{
	LX_DEMOD_L2_Context 		*pFrontend;
	LX_DEMOD_L2_SIG_STATUS		signalStatus;
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT2_Get_GuradIntervalMode
 * Get the information of guard interval on DVB-T.
 *
 * @param *pGIMode 	[OUT] LX_DEMOD_GIMODE_T - 1/32, 1/16, 1/8 and 1/4.
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT2_Get_GuradIntervalMode( LX_DEMOD_GIMODE_T *pGIMode )
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT2_Get_CoderRate( LX_DEMOD_CODERATE_T *pCodeRate )
{
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT2_Get_CurrentPLP_ID
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT2_Get_CurrentPLP_ID(UINT8 *pCurrPlpID)
{
	LX_DEMOD_L2_Context 		*pFrontend;
	LX_DEMOD_L2_SIG_STATUS		signalStatus;
	pFrontend		= &gM19_LGDBB_Frontend;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT2_Get_PlpInfo( LX_DEMOD_DVBT2_CONFIG_T *pPlpParams)
{
//	UINT8 						currentPlpID;
	LX_DEMOD_BWMODE_T 		channelBW 	= LX_DEMOD_BW_UNKNOWN;
	LX_DEMOD_FFTMODE_T 		FFTMode 	= LX_DEMOD_DVBT_ISDBT_FFT_UNKNOWN ;
	...
	if(NULL == pPlpParams)
	{
		DEMOD_PRINT("[DEMOD ] ERROR, DEMOD_M19_DVBT_CONFIG_T TpsParams is NULL !!!\n");
		return RET_ERROR;
	}
	// Get channelBW
/*	if (RET_OK != DEMOD_M19_Get_BandWidthMode( &channelBW ))
	{
		DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_DVBT_Get_BandWidthMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	switch(signalStatus.bandwidth)
	...
	// Get FFT mode
/*	if (RET_OK != DEMOD_M19_DVBT2_Get_FFTMode( &txModeLock, &FFTMode ))
	{
		DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_DVBT_Get_FFTMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	switch(signalStatus.fftMode)
	...
	// Get Guard Interval mode
/*	if (RET_OK != DEMOD_M19_DVBT2_Get_GuradIntervalMode( &gIMode ))
	{
		DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_DVBT_Get_GuradIntervalMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	switch(signalStatus.guardInterval)
	...
	// Get LP coder rate
/*	if (RET_OK != DEMOD_M19_DVBT2_Get_CoderRate( &codeRate ))
	{
		DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_DVBT_Get_LpCoderRate() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	switch(signalStatus.codeRate)
...
/*	if (RET_OK != DEMOD_M19_Get_QAMMode(&qamMode ))
	{
		DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_DVB_Get_QAMMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	switch(signalStatus.constellation)
	...
	// get current PLP ID
/*
	if (RET_OK != DEMOD_M19_DVBT2_Get_CurrentPLP_ID(&currentPlpID ))
	{
		DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_DVBT2_Get_CurrentPLP_ID() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
//	pPlpParams->unPLP = pFrontend->set_PLP_ID;
	pPlpParams->unPLP = signalStatus.current_plp_id;
	DEMOD_PRINT("[DEMOD ]current PLP ID = 0x%x set PLP ID = 0x%x !!!\n",pPlpParams->unPLP, pFrontend->set_PLP_ID);
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT2_Get_multiPLP_ID
 * Get TPS information on DVB-T
 *
 * @param *pTpsParams [OUT] LX_DEMOD_DVBT_CONFIG_T - TPS data structure.
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT2_Get_multiPLP_ID( LX_DEMOD_DVBT2_MULTI_PLP_INFO_T *pPlpParams)
{
	UINT8 								plpCount;
	LX_DEMOD_L2_Context 				*pFrontend;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M19_DVBT2_Get_TotalInfo( void)
{
	UINT16 								ifagcValue = 0;
	UINT32 								errorCount;
	SINT32 								FreqOff;
...
/*		if (RET_OK != DEMOD_M19_Get_IFAGC(&ifagcValue))
		{
			DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_Get_IFAGC() !!!\n", __F__, __L__);
			ifagcValue = 0;
		}
*/
		ifagcValue = signalStatus.if_agc;
	...
	if (RET_OK != DEMOD_M19_Get_NeverLockStatus(LX_DEMOD_DVBT2, &neverLockStatus))
	{
		DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_DVB_Get_NeverLockStatus() !!!\n", __F__, __L__);
		neverLockStatus = LX_DEMOD_UNKOWN_STATE;
	}

	if (RET_OK !=  DEMOD_M19_Get_SpectrumStatus(&spectrumAuto, &spectrumInv))
	{
		DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_Get_SpectrumStatus() !!!\n", __F__, __L__);
		spectrumInv = LX_DEMOD_UNKOWN_STATE;
	}
...
/*	if (RET_OK != DEMOD_M19_Get_CarrierFreqOffset( &FreqOff ))
	{
		DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
		FreqOff = 0xffff;
	}
*/
	FreqOff = signalStatus.offsetFrq_Hz/1000;
...
/*	if (RET_OK != DEMOD_M19_Get_MseInfo(&sigMse))
	{
		DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_Get_MseInfo() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
*/
	sigMse.bMse = 0;
	...
	if (RET_OK != DEMOD_M19_DVBT2_Get_PlpInfo( &plpParams))
	{
		DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_DVBT2_Get_PlpInfo() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	if (RET_OK != DEMOD_M19_DVBT2_Get_multiPLP_ID(&plpInfo))
	{
		DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_DVBT2_Get_multiPLP_ID() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
...
int DEMOD_M19_DVBT2_Set_MemSconfig(void)
{
	...
	{
		printk("######[DEMOD_M19_DVBT2_Set_MemSconfig] Error M19 DEMOD DVB T2 Wrong Memory Base Address ##### !!!!!!!!!!!!!!!!!!!!!!!\n");
	}
	else
	{
		printk("######[DEMOD_M19_DVBT2_Set_MemSconfig] first call DEMOD_M19_DVBT2 _Set_MemSconfig TDI_addr[0x%x] , DJB_addr[0x%x] ##### !!!!!!\n", tdi_st_mem, djb_st_mem );
		if(RET_OK != DEMOD_L2_DVBT2_Set_BaseAddr_AXI(pFrontend, tdi_st_mem, djb_st_mem ))
		{
			DEMOD_PRINT("[DEMOD_M19_DVBT2_Set_MemSconfig] FAIL (%s:%d) DEMOD_L2_ATSC_Set_BaseAddr_AXI() !!!\n", __F__, __L__);
			return RET_ERROR;
		}
	}
	return RET_OK;
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT2_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_M19_DVBT2_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	...
	switch (pDemodInfo->controlState)
	{
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				if(RET_OK != _DEMOD_M19_LGDBB_DVBT2_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT("[DEMOD M19 LGDBB] FAIL (%s:%d) _DEMOD_M19_LGDBB_DVBT2_SetDemod() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				if(LX_DEMOD_DVBT == operMode)
				{
					...
					if(RET_OK != DEMOD_M19_DVBT_Set_PartialConfig(&(pDemodInfo->setCfgParam.dvbt)))
					{
						DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_DVBT_Set_PartialConfig() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				...
				if(RET_OK != DEMOD_M19_Get_Lock(LX_DEMOD_L1_DONE, &lockState))
				{
					DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_Get_Lock() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE: 		/********************************************/
			{
				...
				if(RET_OK != DEMOD_M19_Get_Lock(LX_DEMOD_FEC_DEC_EN, &lockState))
				{
					DEMOD_PRINT("[DEMOD ] FAIL (%s:%d) DEMOD_M19_Get_Lock() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
...
************************************************************************************/
/**
 * DEMOD_M19_DVBT2_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_M19_DVBT2_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
...
*/	
		...
		DEMOD_PRINT( "^r^[DEMOD M19 GBB](%s) pDemodInfo->signalStatus.packetError = %d,!!!\n", __F__,pDemodInfo->signalStatus.packetError);
		DEMOD_PRINT( "^r^[DEMOD M19 GBB](%s) gM19GBB_FullDVBPkerrThreshold = %d !!!\n", __F__, gM19GBB_FullDVBPkerrThreshold);
		...
		if(pDemodInfo->signalStatus.packetError >= gM19GBB_FullDVBPkerrThreshold)
		{
			...
			if(gM19GBB_DVB_UnlockCount_full_packeterr == gM19GBB_FullDVBT2PkerrCount)
			{
				DEMOD_PRINT( "^r^[DEMOD M19 GBB] oops !! (%s) Force to NOT LOCK !!\n", __F__);
				gM19GBB_DVB_UnlockCount_full_packeterr = 0;
/*
