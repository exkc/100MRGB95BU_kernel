...
static OS_PROC_DESC_TABLE_T	_g_pm_m23_device_proc_table[] =
{
...
static OS_PROC_DESC_TABLE_T	_g_pm_m23_gov_proc_table[] =
{
...
static OS_PROC_DESC_TABLE_T	_g_pm_m23_debug_proc_table[] =
{
...
#define M23_AVS  0
extern m23_sidd_t gSidd_check_table_m23[6];
extern char * cpu_volttype_m23[6];
extern char * core_volttype_high_m23[6];
extern char * core_volttype_m23[6];
extern pms_m23_avsinto_t	m23_avs_info;


static void _PM_PrintStatus(struct seq_file *m, void* data)
{
	pms_m23_avsinto_t info ;
	memset (&info,0x0, sizeof(pms_m23_avsinto_t));

	seq_printf( m, "governor: %s\n", pm_m23_gov->get_gov_str(pm_m23_gov->get_gov()) );
	seq_printf( m, "cpu_num: %d\n", pm_m23_gov->get_cpunum());
	seq_printf( m, "load: %u %u %u %u\n", pm_m23_gov->get_load(0),pm_m23_gov->get_load(1),pm_m23_gov->get_load(2),pm_m23_gov->get_load(3)) ;
	seq_printf( m, "loadmax: %u \n", pm_m23_gov->get_load(4)) ;
	seq_printf( m, "loadavg: %u \n", pm_m23_gov->get_load(5)) ;
	seq_printf( m, "freq: %u Mhz\n", pm_m23_gov->get_max_freq()) ;
	seq_printf( m, "===========================\n"	);
	seq_printf( m, "coreavs_type: %s \n", pm_m23_avs->get_core_avs_type_str()) ;
	pm_m23_avs->get_core_avs_type(&info);
	seq_printf( m, "coreavs_current(mA): %u\n", info.avscurrent);
	pm_m23_avs->get_cpu_avs_type(&info);
	seq_printf( m, "cpuavs_type: %s \n", pm_m23_avs->get_cpu_avs_type_str()) ;
	seq_printf( m, "cpuavs_current(mA): %u\n", info.avscurrent);
	seq_printf( m, "avs high [%u]\n", m23_avs_info.is_avshigh);
	seq_printf( m, "===========================\n"	);
	seq_printf( m, "|m23 core avs %5s table|\n",(m23_avs_info.is_avshigh)?"HIGH":"NORMAL");
	seq_printf( m, "===========================\n"	);

	if(m23_avs_info.is_avshigh)
	{
		seq_printf( m, "|    <= %3d |  %smV      |\n", gSidd_check_table_m23[M23_AVS].core_sidd_f_min,core_volttype_high_m23[1] );
		seq_printf( m, "|     > %3d |  %smV      |\n", gSidd_check_table_m23[M23_AVS].core_sidd_f_min,core_volttype_high_m23[3]);
	}
	else
	{
		seq_printf( m, "| 	 <= %3d |  %smV      |\n", gSidd_check_table_m23[M23_AVS].core_sidd_f_min,core_volttype_m23[1] );
		seq_printf( m, "|     > %3d |  %smV      |\n", gSidd_check_table_m23[M23_AVS].core_sidd_f_min,core_volttype_m23[3]);
	}
		seq_printf( m, "===========================\n"	);
		seq_printf( m, "|  m23a0  cpu avs table |\n");
		seq_printf( m, "===========================\n"	);
		seq_printf( m, "|    <= %3d |  %smV      |\n", gSidd_check_table_m23[M23_AVS].cpu_sidd_f_min,cpu_volttype_m23[1]);
		seq_printf( m, "| %3d ~ %3d |  %smV      |\n", gSidd_check_table_m23[M23_AVS].cpu_sidd_f_min,gSidd_check_table_m23[M23_AVS].cpu_sidd_ff_min,cpu_volttype_m23[3] );
		seq_printf( m, "|     > %3d |  %smV      |\n", gSidd_check_table_m23[M23_AVS].cpu_sidd_ff_min,cpu_volttype_m23[4]);
		seq_printf( m, "===========================\n"	);

		pm_m23_avs->get_cpu_avs_type(&info);
		seq_printf( m, "===========================\n"	);
		seq_printf( m,"      m23 CPU TEMP[%d]'C    \n",pm_m23_avs->get_temperature());
		seq_printf( m, "===========================\n"	);
 ...
 *
*/
static int	m23_read_debug_proc(UINT32 procId, char* buffer )
{
	int ret = 0 ;
...
static int	m23_read_gov_proc(UINT32 procId, char* buffer )
{
	...
	switch( procId ) {
		...
		case GOV_ID_MIN_FREQ:
			ret = sprintf( buffer, "%u\n", pm_m23_gov->get_min_freq());
			break;
		case GOV_ID_HARD_UP_THRESHOLD:
			ret = sprintf( buffer, "%u\n", pm_m23_gov->get_hard_up_threshold() );
			break;
		case GOV_ID_EASY_UP_THRESHOLD:
			ret = sprintf( buffer, "%u\n", pm_m23_gov->get_easy_up_threshold() );
			break;
		case GOV_ID_EASY_COUNT:
			ret = sprintf( buffer, "%u\n", pm_m23_gov->get_easy_count());
			break;
		case GOV_ID_FREQDOWN_COUNT:
			ret = sprintf( buffer, "%u\n", pm_m23_gov->get_freqdown_count());
			break;
		case GOV_ID_MPD_COUNT:
			ret = sprintf( buffer, "%u\n", pm_m23_gov->get_mpd_count());
			break;
		case GOV_ID_MPD_UP_TH:
			ret = sprintf( buffer, "%u\n", pm_m23_gov->get_mpd_up_threshold());
			break;
		case GOV_ID_MPD_DN_TH:
			ret = sprintf( buffer, "%u\n", pm_m23_gov->get_mpd_dn_threshold());
			break;
		default:
			ret = -1;
			break;
...
static int	m23_read_proc(UINT32 procId, char* buffer )
{
	int	ret = 0 ;
	pms_m23_avsinto_t info;
...
static int m23_write_debug_proc( UINT32 procId, char* command )
{
	int ret = 0;
...
static int m23_write_gov_proc( UINT32 procId, char* command )
{
	...
	switch( procId )
	{
		case GOV_ID_MAX_FREQ:
			...
			if( input_value < pm_m23_gov->get_min_freq()) {
				printk("input frequency is lower then min frequency\n");
				break;
			}
			pm_m23_gov->set_max_freq(input_value);
			break;
		case GOV_ID_MIN_FREQ:
			input_n = sscanf(command,"%u\n", &input_value);
			if( input_value < pm_m23_gov->get_max_freq()) {
				printk("input frequency is higher then max frequency\n");
				break;
			}
			pm_m23_gov->set_min_freq(input_value);
			break;
		case GOV_ID_HARD_UP_THRESHOLD:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_gov->set_hard_up_threshold(input_value);
			break;
		case GOV_ID_EASY_UP_THRESHOLD:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_gov->set_easy_up_threshold(input_value);
			break;
		case GOV_ID_EASY_COUNT:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_gov->set_easy_count(input_value);
			break;
		case GOV_ID_FREQDOWN_COUNT:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_gov->set_freqdown_count(input_value);
			break;
		case GOV_ID_MPD_COUNT:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_gov->set_mpd_count(input_value);
			break;
		case GOV_ID_MPD_UP_TH:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_gov->set_mpd_up_threshold(input_value);
			break;
		case GOV_ID_MPD_DN_TH:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_gov->set_mpd_dn_threshold(input_value);
			break;
		default:
			ret = -1;
			break;
...
static int m23_write_proc( UINT32 procId, char* command )
{
	unsigned int input_value;
	unsigned int input_n;
	unsigned int temp;
	...
	switch( procId )
	{
		case PROC_ID_GOVERNOR:
			...
			if (input_value <= 10)
			{
				pm_m23_test->set_testmode(0);

				pm_m23_gov->set_gov(input_value);
				...
				if( input_value == 6)
					pm_m23_func->setfreq(M23_CPU_DFTFREQ);
				else if( input_value == 1)
					pm_m23_func->setfreq(M23_CPU_NORMALFREQ);
			}
			break;
		case PROC_ID_BOOST:
			pm_m23_gov->boost();
		...
		case PROC_ID_MP_ENABLE:
			...
			if (input_value != 0)
				input_value = 1;
			pm_m23_gov->set_mp_enable(input_value);
			break;
		case PROC_ID_FREQ_ENABLE:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value != 0)
				input_value = 1;
			pm_m23_gov->set_freq_enable(input_value);
			break;
		case PROC_ID_TS_ENABLE:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value != 0)
				input_value = 1;
			pm_m23_gov->set_ts_enable(input_value);
			break;
		case PROC_ID_TS_DEBUG_ENABLE:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value != 0)
				input_value = 1;
			pm_m23_avs->set_ts_debug_enable(input_value);
			break;

		case PROC_ID_TS_CPU_APARAM:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_avs->set_cpu_ts_aparam(input_value);
			break;
		case PROC_ID_TS_CPU_BPARAM:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_avs->set_cpu_ts_bparam(input_value);
			break;
		case PROC_ID_TS_GPU_APARAM:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_avs->set_gpu_ts_aparam(input_value);
			break;
		case PROC_ID_TS_GPU_BPARAM:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_avs->set_gpu_ts_bparam(input_value);
			break;
		case PROC_ID_TO_SUSPEND:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value == 1)
			{
				temp = pm_m23_gov->get_gov() | PMS_GOV_TOSUSPEND_FLAG;
				pm_m23_gov->set_gov(temp);
				pm_m23_gov->set_startup_timer(0);
				pm_m23_func->set_max_performance();
			}
			break;
		case PROC_ID_RESET_LOADMON:
			pm_m23_gov->reset_load_monitor();
			break;
		case PROC_ID_SAMPLING_RATE:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_func->set_sampling_rate(input_value);
			break;
		case PROC_ID_FREQ:
			input_n = sscanf(command,"%u\n", &input_value);
			#if 0
			...
			pm_m23_func->setfreq(input_value);
			#endif
			break;

		case PROC_ID_CPU_AVS:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_avs->set_avs_force(GPIO_AVS_CPU, input_value);
			break;

			break;
		case PROC_ID_CORE_AVS:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m23_avs->set_avs_force(GPIO_AVS_CORE, input_value);
...
void m23_proc_init (void)
{
	...
	OS_PROC_CreateEntryEx ( PM_MODULE, _g_pm_m23_device_proc_table,
										m23_read_proc,
										m23_write_proc );
	OS_PROC_CreateEntryEx ( PM_GOV_MODULE, _g_pm_m23_gov_proc_table,
										m23_read_gov_proc,
										m23_write_gov_proc );
	OS_PROC_CreateEntryEx ( PM_DEBUG_MODULE, _g_pm_m23_debug_proc_table,
										m23_read_debug_proc,
										m23_write_debug_proc );
...
static pms_proc_t m23_proc_func =
{
	.init = m23_proc_init,
	.cleanup = m23_proc_cleanup,
};

pms_proc_t* get_m23_pm_proc(void)
{
