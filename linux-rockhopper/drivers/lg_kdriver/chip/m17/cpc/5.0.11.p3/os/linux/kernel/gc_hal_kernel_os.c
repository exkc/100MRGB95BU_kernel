...
#if defined(CONFIG_ARM64) && defined(CONFIG_LG_EXT)
/* Please refer to SICDTV-4857.
   Oppsss!! In aarch64,a invalid PFN was found on this walk, intermittently.
   So we will be walking on the page-table, when vma(user-mapping) is found.
*/
	if (!find_vma(current->mm, logical)) {
	    gcmkTRACE(gcvLEVEL_INFO, "not found vma\n");
	    return gcvSTATUS_NOT_FOUND;
	}
#endif
    ...
    )
{
    ...
    addr = dma_alloc_coherent(&platform->device->dev,
#else
    ...
    if (((Os->device->baseAddress & 0x80000000) != (mdl->dmaHandle & 0x80000000)) &&
          kernel != gcvNULL && kernel->hardware->mmuVersion == 0)
    {
    ...
    )
{
	...
	dma_free_coherent(&platform->device->dev,
#else
    ...
    )
{
    ...
    {
/*
	Ignore register writing to GC520 base register when value is '0x207'
	for M16+, O18, M16++, M16P3
*/
#if defined(CONFIG_LG_EXT)
...
#if defined(CONFIG_LG_EXT)
    /* just printout for debug */
    gcmkTRACE(gcvLEVEL_INFO, "%s(%d),%p->%x\n",
        __func__, __LINE__, Logical, *Address);
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
    *Time = ktime_get_real_ns() / NSEC_PER_USEC;
#else
    do_gettimeofday(&tv);
    ...
    )
{
    ...
    /* Copy data from user. */
    rc = copy_from_user(KernelPointer, Pointer, Size);
    if (rc != 0)
    {
    	printk("%s: Os=%p KernelPointer=%p Pointer=%p Size=%lu rc=%lu\n", __FUNCTION__, Os, KernelPointer, Pointer, Size, rc);
...
#else
{
    ...
    do
    {
        ...
        for (i = 0; i < pageCount; i++)
        {
            ...
            {
                if (info->extraPage && i == (pageCount - 1))
                {
                    phys = page_to_phys(Os->paddingPage);
                }
                else
                {
                    phys = Physical + i * PAGE_SIZE;
...
#elif defined(CONFIG_ARM64)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
    __dma_flush_area((void *) Logical, Bytes);
#else
    __dma_flush_range((void *) Logical, (void *) Logical + Bytes);
#endif
#elif defined(CONFIG_MIPS)
