...
static int ntfs_file_release(struct inode *vi, struct file *filp)
{
	...
	if (aligned_data_size < ni->allocated_size) {
		int err;
		VCN vcn_ds = aligned_data_size >> vol->cluster_size_bits;
		VCN vcn_tr = -1;
		runlist_element *rl = ni->runlist.rl;
...
#endif
{
	...
	if (ia_valid & ATTR_MODE) {
		mode_t mask, perm;
		mask = (S_ISREG(vi->i_mode) || S_ISLNK(vi->i_mode)) ?
			vol->fmask : vol->dmask;
		perm = attr->ia_mode & ~(S_IFMT | mask);
		vi->i_mode = attr->ia_mode & (S_IFMT | perm);
	}
	...
	if (whence == SEEK_DATA || whence == SEEK_HOLE) {
		...
		while (1) {
			if (!rl || !NInoFullyMapped(ni) || rl[i].lcn == LCN_RL_NOT_MAPPED) {
				...
				if ((whence == SEEK_DATA && (rl[i].lcn >= 0 || rl[i].lcn == LCN_DELALLOC)) ||
...
static ssize_t ntfs_file_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
	...
	if (iocb->ki_flags & IOCB_DIRECT) {
		...
		ret = iomap_dio_rw(iocb, to, &ntfs_read_iomap_ops, NULL, IOMAP_DIO_PARTIAL, NULL, 0);
#else
...
static ssize_t ntfs_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	if (iocb->ki_pos + ret > old_data_size) {
		mutex_lock(&ni->mrec_lock);
		if (iocb->ki_pos + ret > ni->allocated_size &&
...
static long ntfs_fallocate(struct file *file, int mode, loff_t offset, loff_t len)
{
	...
	if (mode & FALLOC_FL_INSERT_RANGE) {
		...
		need_space = max_t(VCN, start_vcn,
				   ni->allocated_size << vol->cluster_size_bits);
		need_space = end_vcn - need_space;
		if (need_space > 0 && need_space > atomic64_read(&vol->free_clusters)) {
			err = -ENOSPC;
			goto out;
		}

		err = ntfs_attr_fallocate(ni, start_vcn, end_vcn - start_vcn,
		...
		if (NInoNonResident(ni)) {
			if (offset < ni->initialized_size) {
				loff_t first_len = min_t(loff_t, len,
							 ni->initialized_size - offset);
				err = iomap_zero_range(vi, offset, first_len,
						       NULL, &ntfs_read_iomap_ops);
				if (err < 0)
					goto out;
			}
			if (offset + len > ni->initialized_size) {
				err = ntfs_extend_initialized_size(VFS_I(ni),
								   offset, offset + len);
				if (err)
					goto out;
			}
