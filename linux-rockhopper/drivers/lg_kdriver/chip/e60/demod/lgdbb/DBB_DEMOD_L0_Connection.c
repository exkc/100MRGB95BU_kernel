...
#if defined(DEBUG_MUTEX) && (DEMOD_PLATFORM == DEMOD_PLATFORM_GUI)
...
//	}
	...
	DEMOD_DEBUG("[API,L0] %lu, req 0x%08x, %d, %02x, %02x => 0x%08x, %d\n",
		GetCurrentThreadId(), Addr, iNbBytes, Addr_Msb, Addr_Lsb, i2cAddr, alen);
#else
	DEMOD_DEBUG("[API,L0] req 0x%08x, %d, %02x, %02x => 0x%08x, %d\n",
...
*/
	...
	DEMOD_DEBUG("[API,L0] req 0x%08x, %d, %02x, %02x => 0x%08x, len %d\n", Addr, iNbBytes, Addr_Msb, Addr_Lsb, i2cAddr, alen);
	//DEMOD_INFO("[API,L0] req 0x%08x, %d, %02x, %02x => 0x%08x, len %d\n", Addr, iNbBytes, Addr_Msb, Addr_Lsb, i2cAddr, alen);
