...
#define LBUS_QOS_CMN_BASE0			(0xC9820000)
#define LBUS_QOS_CMN_BASE1			(0xC9820400)
...
static ulong  e60_user_mpro[6] = { 0, 0, 0, 0, 0, 0 };
module_param_array(e60_user_mpro, ulong, &user_mpro_cnt, 0644);
...
static lgbus_e60_mpro_t mpro_ctrl[] =
{
	{ "CORE",		0xC3033000},
	{ "AXI_PERI",	0xC3032200},
	{ "AHB_PERI",	0xC3032400},
	...
	{ "WOV",		0xC9823800},
	{ "DBB",		0xC9823A00},
	{ "LGSR_SMX3",	0xC9823C00},	//#define		{ "SMX3			",	0xC9823C00 , common address (read only + write only)
	{ "OASR",		0xC9823E00},
	{ "LNE",		0xC9824000},
	{ "VDEC0_M0",	0xC9824200},
	{ "VDEC0_M1",	0xC9824400},
	{ "VDEC1_M0",	0xC9824600},
	{ "VDEC1_M1",	0xC9824800},
	{ "VDEC2A_M0",	0xC9824A00},
	{ "VDEC2A_M1",	0xC9824C00},
	{ "VDEC2B_M0",	0xC9824E00},
	{ "VDEC2B_M1",	0xC9825000},
	{ "VDEC3A_M0",	0xC9825200},
	{ "VDEC3A_M1",	0xC9825400},
	{ "VDEC3B_M0",	0xC9825600},
	{ "VDEC3B_M1",	0xC9825800},
	{ "MCU",		0xC9825A00},
	{ "FMC0",		0xC9826000},
	{ "FMC1",		0xC9826200},
	{ "FMC2",		0xC9826400},
	...
	{ "GSC2",		0xC9827400},
	{ "GSC3",		0xC9827600},
	{ "GSC4_SRE0",	0xC9827800},	//#define		{ "SRE0			",	0xC9827800 , common address (read only + write only)
	{ "GSC5_SRE1",	0xC9827A00},	//#define		{ "SRE1			",	0xC9827A00 , common address (read only + write only)
	{ "CCO0",		0xC9827C00},
	{ "CCO1",		0xC9827E00},
	{ "SMX0",		0xC9828000},
	{ "SMX1",		0xC9828200},
	{ "SMX2",		0xC9828400},
	{ "SMX4",		0xC9828800},
	{ "SMX5_ND2",	0xC9829800},	//#define		{ "ND2			",	0xC9829800 , common address (read only + write only)
	{ "SMX6",		0xC9828C00},
	{ "SMX7",		0xC9828E00},
	{ "VDM0",		0xC9829000},
	{ "VDM1",		0xC9829200},
...
static ulong lbus_qos_cmn_base0;
static ulong lbus_qos_cmn_base1;

static u32 isr_cnt_limit = 3;
module_param_named(e60_mpro_cnt_limit, isr_cnt_limit, int, 0644);
...
static int lgbus_mpro_resume(void)
{
#ifdef SUPPORT_E60_MPRO_CHIP_KDRV
	int ret;
...
static int lgbus_mpro_get_range(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		for(j = 0; j < E60_MPRO_RANGE_SLOT_CNT; j++)
		{
			//BUS MPRO REG
...
static int lgbus_mpro_regs_save(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		size = sizeof(lgbus_e60_mpro_reg_save_t) / sizeof(u32);
...
#undef LBUS_IRQ
#define LBUS_IRQ			E60_IRQ_INTR_LBUS_QOS

#undef CPU_IRQ
#undef CPU_AXI_PERI_IRQ
#undef CPU_AHB_PERI_IRQ
#undef CPU_LZMA_IRQ
#define CPU_IRQ				E60_IRQ_QOS_CPU
#define CPU_AXI_PERI_IRQ	E60_IRQ_QOS_AXI_PERI
#define CPU_AHB_PERI_IRQ	E60_IRQ_QOS_AHB_PERI
#define CPU_LZMA_IRQ		E60_IRQ_QOS_LZMA
#define CPU_FAST_IRQ		(E60_IRQ_QOS_AHB_PERI+1)
...
static int lgbus_mpro_ioremap(void)
{
	...
	lbus_qos_cmn_base0 = (ulong)ioremap_nocache(LBUS_QOS_CMN_BASE0, 0x200);
	lbus_qos_cmn_base1 = (ulong)ioremap_nocache(LBUS_QOS_CMN_BASE1, 0x200);
			...
			if(irq_status & MPRO_ISR_STATUS)
			{
...

				{
					...
					struct
					{
						...
						u32 wr_vio_addr;
						u32 wr_vio_id;
					}rw_vio_data[E60_MPRO_RANGE_SLOT_CNT];
						...
						for(j = 0; j < E60_MPRO_RANGE_SLOT_CNT; j++)
						{
							if((rw_vio_data[j].wr_vio_id & 0x1) && (rw_vio_data[j].wr_vio_addr != 0xffffffff))
							{
								LGBUS_ERROR(COLOR_RED"[%-10s] Detect write violation!. ddr phy 0x%09llx(%d)"COLOR_NONE"\n"
										,mpro_ctrl[i].name, (u64)rw_vio_data[j].wr_vio_addr << 2, j);
						...
						for(j = 0; j < E60_MPRO_RANGE_SLOT_CNT; j++)
						{
							if((rw_vio_data[j].rd_vio_id & 0x1) && (rw_vio_data[j].rd_vio_addr != 0xffffffff))
							{
								LGBUS_ERROR(COLOR_RED"[%-10s] Detect read violation!. ddr phy 0x%09llx(%d)"COLOR_NONE"\n"
										,mpro_ctrl[i].name, (u64)rw_vio_data[j].rd_vio_addr << 2, j);
...
int LGBUS_E60_SetMproRange(lgbus_mpro_range_t *mpro_range)
{
#ifdef SUPPORT_E60_MPRO_CHIP_KDRV
	int 		ret = RET_ERROR;
	uint32_t 	port, cnt;
	...
	CHECK_ERROR(mpro_range->port >= E60_MPRO_PORT_MAX, return RET_ERROR, "Invalid port\n");
	CHECK_ERROR(mpro_range->port < E60_MPRO_PORT_START, return RET_ERROR, "Invalid port\n");

	port = mpro_range->port - E60_MPRO_PORT_START;
	...
	for(cnt = 0; cnt < E60_MPRO_RANGE_SLOT_CNT; cnt++)
	{
		uint32_t ctr0, ctr1;
		...
		LGBUS_PRINT("[%-10s][%u] 0x%09llx ~ 0x%09llx\n"
									, mpro_ctrl[port].name
									, cnt
...
EXPORT_SYMBOL(LGBUS_E60_SetMproRange);

int LGBUS_E60_GetMproRange(lgbus_mpro_range_t *mpro_range)
{
#ifdef SUPPORT_E60_MPRO_CHIP_KDRV
	uint32_t 	port, cnt;
	ulong 		flag;
...
EXPORT_SYMBOL(LGBUS_E60_GetMproRange);

static void lgbus_mpro_user_mpro(void)
{
	if (e60_user_mpro[0] && e60_user_mpro[1] && e60_user_mpro[2] && e60_user_mpro[3] && e60_user_mpro[4] && e60_user_mpro[5])
	{
		int	i;
		lgbus_mpro_range_t mpro_range;

		LGBUS_NOTI("user_mpro=0x%lx,0x%lx,0x%lx,0x%lx,0x%lx,0x%lx, isr_cnt=%d\n",
		e60_user_mpro[0], e60_user_mpro[1], e60_user_mpro[2], e60_user_mpro[3], e60_user_mpro[4], e60_user_mpro[5], isr_cnt_limit);

		mpro_range.range[0].start   = e60_user_mpro[0];
		mpro_range.range[0].end     = e60_user_mpro[1];
		mpro_range.range[1].start   = e60_user_mpro[2];
		mpro_range.range[1].end     = e60_user_mpro[3];
		mpro_range.range[2].start   = e60_user_mpro[4];
		mpro_range.range[2].end     = e60_user_mpro[5];

		for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
		{
			mpro_range.port = i;
			mpro_range.port += E60_MPRO_PORT_START;
			LGBUS_E60_SetMproRange(&mpro_range);
		}
	}
}

lgbus_e60_mpro_func_t	lgbus_e60_mpro_func =
{
...
static lgbus_addr_switch_t	e60_addr_switch[E60_ADDR_SWITCH_SLOT];
static lgbus_bwb_t			e60_bwb[E60_BWB_SLOT];
...
static int lgbus_get_addr_switch(void)
{
	...
	for(i = 0; i < E60_ADDR_SWITCH_SLOT; i++)
	{
		u32 d0, d1;
		...
		//[28]		1 : enable
		//[13:00]	14: offset
		e60_addr_switch[i].start = ((u64)(d0 >> 16) & 0x3fff) << 20; 	// 29~16	:14bit
		e60_addr_switch[i].end = (u64)(d0 & 0x3fff) << 20; 				// 13~0		:14bit
		e60_addr_switch[i].offset = (u64)(d1 & 0x3fff) << 20; 			// 13~0		:14bit

		if(e60_addr_switch[i].end)
			e60_addr_switch[i].end |= 0xfffff;

		LGBUS_PRINT("s:0x%09llx ~ e:0x%09llx,o:0x%09llx\n", e60_addr_switch[i].start
															 , e60_addr_switch[i].end
															 , e60_addr_switch[i].offset);
	}
...
static int lgbus_get_bwb(void)
{
	...
	for(i = 0; i < E60_BWB_SLOT; i++)
	{
		...
		//[30:28] 3 : offset_mode
		//[13:0] 14 : offset
		e60_bwb[i].start = ((u64)(d0 >> 12) & 0x7ff) << 20;	//[22:12] 11 : start
		e60_bwb[i].end = (u64)(d0 & 0x7ff) << 20;			//[10:0]  11 : end

		e60_bwb[i].offset = (u64)(d1 & 0x3fff) << 20;		//[13:0] 14 : offset
		e60_bwb[i].offset_mode = (d1 >> 28) & 0x7;

		if(e60_bwb[i].end)
			e60_bwb[i].end |= 0xfffff;

		LGBUS_PRINT("s:0x%09llx ~ e:0x%09llx,o:0x%09llx[%x]\n", e60_bwb[i].start
															 , e60_bwb[i].end
															 , e60_bwb[i].offset
															 , e60_bwb[i].offset_mode);
	}
...
u64 lgbus_e60_get_ddr_addr(u64 phy_addr)
{
#ifdef SUPPORT_E60_MPRO_CHIP_KDRV
	int i;
	u64 tmp = phy_addr;
	...
	for(i = 0; i < E60_ADDR_SWITCH_SLOT; i++)
	{
		if(e60_addr_switch[i].start <= phy_addr &&
				e60_addr_switch[i].end >= phy_addr)
		{
			phy_addr += e60_addr_switch[i].offset;
	...
	for(i = 0; i < E60_BWB_SLOT; i++)
	{
		if(e60_bwb[i].offset)
		{
			if(e60_bwb[i].start <= bwb_addr
					&& e60_bwb[i].end >= bwb_addr
					&& e60_bwb[i].offset)
			{
				if(e60_bwb[i].offset_mode == BWB_OFFSET_M0
						&& phy_addr < 0x80000000)
				{
					phy_addr += e60_bwb[i].offset;
				}
				else if(e60_bwb[i].offset_mode == BWB_OFFSET_M1
						&& 	phy_addr >= 0x80000000 && phy_addr < 0x100000000)
				{
					phy_addr += e60_bwb[i].offset;
				}
				else if(e60_bwb[i].offset_mode == BWB_OFFSET_M2
	...
	LGBUS_PRINT("%s: v:0x%09llx -> p:0x%09llx\n", __func__, tmp, phy_addr);
#endif
	return phy_addr;
}
EXPORT_SYMBOL(lgbus_e60_get_ddr_addr);
        ...
        , const char __user *buf, size_t size, loff_t *off)
{
	...
	int		i;
	int		addr_cnt;
	int		addr_max = E60_MPRO_RANGE_SLOT_CNT * 2;
	u64		addr[E60_MPRO_RANGE_SLOT_CNT * 2] = {0,};
	lgbus_mpro_range_t mpro_range;
...
static int lgbus_mpro_proc_asw_op(struct seq_file *s, void *data)
{
#if 0//def SUPPORT_E60_MPRO_CHIP_KDRV
