...
typedef struct
{
    ...
    UINT16                  usHeight;    /*for vreverse*/
    int                     reversed;   /*for vreverse*/
...
#define GFX_SCALER_FILTER_MAX           2
    ...
    UINT8           sc_insel;               /** scaler mux selection */
}
...
void        GFX_SetInterruptMode(UINT32 mode);
void        GFX_SetAckMode(UINT32* mode);
void        GFX_GetAckMode(UINT32* mode);
void        GFX_Stop(void);
