...
#include "be_pwm_hw_o22.h"

#include "../reg/be_reg_def_o22.h"
#include "../reg/be_reg_pwm_o22.h"
...
----------------------------------------------------------------------------------------*/
#define BE_PWM_HW_O22_PHDR_W22_DATA_OFFSET	0x2800
#define BE_PWM_HW_O22_PHDR_PWM_DATA_OFFSET	0x200
...
typedef union {
	UINT32 udata32;
	struct {
	UINT32 pwm0_duty  :8;//(7:0)
	UINT32 pwm0_freq  :8;//(15:8)
	UINT32 pwm1_duty  :8;//(23:16)
	UINT32 pwm1_freq  :8;//(31:24)
	};
} PWM_DDR_INFO0_O22_T;

typedef struct {
	PWM_DDR_H_O22_T                                  header;
...
/*----------------------------------------------------------------------------------------
	External Variables
----------------------------------------------------------------------------------------*/
extern PE_PWM_REG_O22_T gPE_PWM_O22;
...
static volatile PWM_DDR_O22_T *gBE_PWM_DDR_O22 = NULL;
static UINT32 _g_pwm_o22_ddr_base = 0;
struct pwm_ddr_ctx _g_pwm_o22_ctx = {.year=0,.opt.u32=0};
...
========================================================================================*/
static int _be_pwm_hw_o22_ddr_init(void)
{
	int ret = RET_OK;
	...
	struct pwm_ddr_ctx *p_ctx = &_g_pwm_o22_ctx;

	ret = OVI_HAL_GetHWDisplayOption(&ovi_opt);
	p_ctx->opt.lcd = (ovi_opt.panelBacklight == LX_OVI_PANEL_BACKLIGHT_OLED)? 0:1;
	...
	{
		p_ctx->opt.wireless = 1;
	}

	BE_NOTI("year:%d,wireless:%d,lcd:%d\n",p_ctx->year,p_ctx->opt.wireless,p_ctx->opt.lcd);

	if (p_ctx->opt.wireless == 0 || p_ctx->opt.lcd == 0 || p_ctx->year < 23 || gBE_PWM_DDR_O22 != NULL) {
		BE_NOTI("skip (not support)\n");
		return 0;
	}

	_g_pwm_o22_ddr_base = gpBeMem->fw[3].fw_base + BE_PWM_HW_O22_PHDR_W22_DATA_OFFSET + BE_PWM_HW_O22_PHDR_PWM_DATA_OFFSET;
	gBE_PWM_DDR_O22 = (volatile PWM_DDR_O22_T *)vmap_phys(_g_pwm_o22_ddr_base, sizeof(PWM_DDR_O22_T));

	do {
		if (gBE_PWM_DDR_O22 == NULL) break;
		memset((void *)gBE_PWM_DDR_O22,0,sizeof(PWM_DDR_O22_T));
		header.udata32 = 0;
		header.size = sizeof(PWM_DDR_O22_T) - sizeof(PWM_DDR_H_O22_T);
		gBE_PWM_DDR_O22->header.udata32 = header.udata32;
		wmb();
		p_ctx->opt.init = 1;
	} while (0);
	...
	param.portParams[0].pwmOutput      = 1;
	...
	param.portParams[1].duty = 204;
	BE_PWM_HW_O22_SetPwmParams(&param);
	#endif
	#if 0
	/* panel brigntness : 80 (duty : 204) */
	gBE_PWM_DDR_O22->header.udata32 = 0x1;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_ctrl0)))       = 0x00000021;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_ctrl1)))       = 0x004b87f0;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_ctrl2)))       = 0x00010000;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_ctrl0)))       = 0x00000021;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_ctrl1)))       = 0x004b87f0;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_ctrl2)))       = 0x00010000;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm_v_load_write))) = 0x0000031b;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_v_r)))         = 0x00000000;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_v_f)))         = 0x00000f1a;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_v_r)))         = 0x00000000;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_v_f)))         = 0x00000f1a;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_v_sub_r)))     = 0x00000000;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_v_sub_f)))     = 0x00000f1a;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_v_sub_r)))     = 0x00000000;
	*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_v_sub_f)))     = 0x00000f1a;
	wmb();
	#endif
...
int BE_PWM_HW_O22_Init(void)
{
	...
	_be_pwm_hw_o22_ddr_init();

	return ret;
}

int BE_PWM_HW_O22_SetPwmControl(BE_PWM_CTRL_T *pstParams)
{
	int ret = RET_OK;
...
int BE_PWM_HW_O22_SetPwmAdaptFreqControl(BE_PWM_ADAPT_FREQ_CTRL_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		if (freqSet)
		{
			...
			ret = BE_PWM_HW_O22_SetPwmSync(&stParams);
...
int BE_PWM_HW_O22_SetPwmFrequency(BE_PWM_FREQ_T *pstParams)
{
	int ret = RET_OK;
	BE_PWM_ID_T nPort;
...
int BE_PWM_HW_O22_SetPwmDutyCycle(BE_PWM_DUTY_T *pstParams)
{
	int ret = RET_OK;
...
int BE_PWM_HW_O22_SetPwmDutyRatio(BE_PWM_DUTY_RATIO_T *pstParams)
{
	int ret = RET_OK;
	BE_PWM_ID_T nPort;
...
int BE_PWM_HW_O22_SetPwmParams(LX_BE_PWM_PARAMS_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		for(i = 0; i < LX_BE_PWM_PORT_MAX; i++)
		{
			if ((pwmParams.portMask >> i) & 0x01)
			{
				...
				ret = BE_PWM_HW_O22_SetPwmControl(&pwmCtrlParam);
				if(ret) BREAK_WRONG(ret);
				...
				ret = BE_PWM_HW_O22_SetPwmAdaptFreqControl(&pwmAdaptFreqCtrlParam);
				if(ret) BREAK_WRONG(ret);
				...
				if (!pwmParams.portParams[i].adaptFreqEn)
				{
					...
					ret = BE_PWM_HW_O22_SetPwmFrequency(&pwmFreqParams);
					if(ret) BREAK_WRONG(ret);
				}

				pwmDutyParam.port = i;
				ret = BE_PWM_HW_O22_SetPwmDutyCycle(&pwmDutyParam);
				if(ret) BREAK_WRONG(ret);
			}
		}
...
int BE_PWM_HW_O22_GetPwmInfo(BE_EXTERNAL_PWM_T *pstParams)
{
	int ret = RET_OK;
	UINT32 freq_mode = 0;
	UINT32 time_low[4] = { 0 };
...
int BE_PWM_HW_O22_SetPwmSync(BE_PWM_FREQ_T *pstParams)
{
	int ret = RET_OK;
	UINT32 pwmFreq;
...
static int PWM_SetFrequency(UINT32 pwmId, UINT32 freq)
{
	...
	if (1) // Check clock setting
	{
		...
		regField = CTOP_CTRL_O22Ax_RdFd(SCRG_CTRL_TCON, crg_tcon04, reg_pix2_clk_sel);
		...
		BE_PRINT_PWM("pwm%d: freq:%d, clk_sel:%d, unit:%d, res:%d, method:%d\n",pwmId,freq,regField,gPwmInfo[pwmId].unit,gPwmInfo[pwmId].ctrl.pwm_resolution,gPwmInfo[pwmId].ctrl.pwm_method);
	}
...
static int PWM_SetDuty(UINT32 pwmId, UINT16 duty, UINT16 offset)
{
	...
	{
		BE_PRINT_PWM("Port[%d] : ordr[%d] -  unit_pixel[%d] offs[%d] falt[%d] high[%d] \n", pwmId, i, unit_pixel, offs, falt, high);
...
static int _REG_SetPwmCtrl(UINT32 pwmId, BOOLEAN zeroDuty)
{
	...
	duty = (gPwmInfo[pwmId].duty * gPwmInfo[pwmId].dutyRatio)/PWM_DUTY_RATIO_MAX_S;

	switch (pwmId) {
		case 0 :
			PE_PWM_O22_RdFL(pe_pwm0_ctrl0);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O22_RdFL(pe_pwm0_ctrl1);
			}
			PE_PWM_O22_RdFL(pe_pwm0_ctrl2);
			PE_PWM_O22_Wr01(pe_pwm0_ctrl0, pwm0_en, gPwmInfo[pwmId].ctrl.pwm_en);
			PE_PWM_O22_Wr01(pe_pwm0_ctrl0, pwm0_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
			PE_PWM_O22_Wr01(pe_pwm0_ctrl0, pwm0_resolution, gPwmInfo[pwmId].ctrl.pwm_resolution);
			PE_PWM_O22_Wr01(pe_pwm0_ctrl0, pwm0_inv, gPwmInfo[pwmId].ctrl.pwm_inv);
			PE_PWM_O22_Wr01(pe_pwm0_ctrl0, pwm0_sel, gPwmInfo[pwmId].ctrl.pwm_sel);
			PE_PWM_O22_Wr01(pe_pwm0_ctrl0, pwm0_width_falling_pos, gPwmInfo[pwmId].ctrl.pwm_width_falling_pos);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O22_Wr01(pe_pwm0_ctrl1, pwm0_free_width, gPwmInfo[pwmId].ctrl.pwm_free_width);
			}
			PE_PWM_O22_Wr01(pe_pwm0_ctrl2, pwm0_method, gPwmInfo[pwmId].ctrl.pwm_method);
			PE_PWM_O22_Wr01(pe_pwm0_ctrl2, pwm0_fc_h_disp, gPwmInfo[pwmId].ctrl.pwm_fc_h_disp);
			if(zeroDuty && (!gPwmInfo[pwmId].ctrl.pwm_fc_h_disp))
			{
				PE_PWM_O22_Wr01(pe_pwm0_ctrl2, pwm0_fc_l_disp, 1);
			}
			else
			{
				PE_PWM_O22_Wr01(pe_pwm0_ctrl2, pwm0_fc_l_disp, gPwmInfo[pwmId].ctrl.pwm_fc_l_disp);
			}
			PE_PWM_O22_WrFL(pe_pwm0_ctrl0);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O22_WrFL(pe_pwm0_ctrl1);
			}
			PE_PWM_O22_WrFL(pe_pwm0_ctrl2);
			break;
		case 1 :
			PE_PWM_O22_RdFL(pe_pwm1_ctrl0);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O22_RdFL(pe_pwm1_ctrl1);
			}
			PE_PWM_O22_RdFL(pe_pwm1_ctrl2);
			PE_PWM_O22_Wr01(pe_pwm1_ctrl0, pwm1_en, gPwmInfo[pwmId].ctrl.pwm_en);
			PE_PWM_O22_Wr01(pe_pwm1_ctrl0, pwm1_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
			PE_PWM_O22_Wr01(pe_pwm1_ctrl0, pwm1_resolution, gPwmInfo[pwmId].ctrl.pwm_resolution);
			PE_PWM_O22_Wr01(pe_pwm1_ctrl0, pwm1_inv, gPwmInfo[pwmId].ctrl.pwm_inv);
			PE_PWM_O22_Wr01(pe_pwm1_ctrl0, pwm1_sel, gPwmInfo[pwmId].ctrl.pwm_sel);
			PE_PWM_O22_Wr01(pe_pwm1_ctrl0, pwm1_width_falling_pos, gPwmInfo[pwmId].ctrl.pwm_width_falling_pos);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O22_Wr01(pe_pwm1_ctrl1, pwm1_free_width, gPwmInfo[pwmId].ctrl.pwm_free_width);
			}
			PE_PWM_O22_Wr01(pe_pwm1_ctrl2, pwm1_method, gPwmInfo[pwmId].ctrl.pwm_method);
			PE_PWM_O22_Wr01(pe_pwm1_ctrl2, pwm1_fc_h_disp, gPwmInfo[pwmId].ctrl.pwm_fc_h_disp);
			if(zeroDuty && (!gPwmInfo[pwmId].ctrl.pwm_fc_h_disp))
			{
				PE_PWM_O22_Wr01(pe_pwm1_ctrl2, pwm1_fc_l_disp, 1);
			}
			else
			{
				PE_PWM_O22_Wr01(pe_pwm1_ctrl2, pwm1_fc_l_disp, gPwmInfo[pwmId].ctrl.pwm_fc_l_disp);
			}
			PE_PWM_O22_WrFL(pe_pwm1_ctrl0);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O22_WrFL(pe_pwm1_ctrl1);
			}
			PE_PWM_O22_WrFL(pe_pwm1_ctrl2);
			break;
		case 2 :
			PE_PWM_O22_RdFL(pe_pwm2_ctrl0);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O22_RdFL(pe_pwm2_ctrl1);
			}
			PE_PWM_O22_RdFL(pe_pwm2_ctrl2);
			PE_PWM_O22_Wr01(pe_pwm2_ctrl0, pwm2_en, gPwmInfo[pwmId].ctrl.pwm_en);
			PE_PWM_O22_Wr01(pe_pwm2_ctrl0, pwm2_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
			PE_PWM_O22_Wr01(pe_pwm2_ctrl0, pwm2_resolution, gPwmInfo[pwmId].ctrl.pwm_resolution);
			PE_PWM_O22_Wr01(pe_pwm2_ctrl0, pwm2_inv, gPwmInfo[pwmId].ctrl.pwm_inv);
			PE_PWM_O22_Wr01(pe_pwm2_ctrl0, pwm2_sel, gPwmInfo[pwmId].ctrl.pwm_sel);
			PE_PWM_O22_Wr01(pe_pwm2_ctrl0, pwm2_width_falling_pos, gPwmInfo[pwmId].ctrl.pwm_width_falling_pos);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O22_Wr01(pe_pwm2_ctrl1, pwm2_free_width, gPwmInfo[pwmId].ctrl.pwm_free_width);
			}
			PE_PWM_O22_Wr01(pe_pwm2_ctrl2, pwm2_method, gPwmInfo[pwmId].ctrl.pwm_method);
			PE_PWM_O22_Wr01(pe_pwm2_ctrl2, pwm2_fc_h_disp, gPwmInfo[pwmId].ctrl.pwm_fc_h_disp);
			if(zeroDuty && (!gPwmInfo[pwmId].ctrl.pwm_fc_h_disp))
			{
				PE_PWM_O22_Wr01(pe_pwm2_ctrl2, pwm2_fc_l_disp, 1);
			}
			else
			{
				PE_PWM_O22_Wr01(pe_pwm2_ctrl2, pwm2_fc_l_disp, gPwmInfo[pwmId].ctrl.pwm_fc_l_disp);
			}
			PE_PWM_O22_WrFL(pe_pwm2_ctrl0);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O22_WrFL(pe_pwm2_ctrl1);
			}
			PE_PWM_O22_WrFL(pe_pwm2_ctrl2);
			break;
		case 3 :
			PE_PWM_O22_RdFL(pe_pwm3_ctrl0);
			PE_PWM_O22_Wr01(pe_pwm3_ctrl0, pwm3_en, gPwmInfo[pwmId].ctrl.pwm_en);
			PE_PWM_O22_Wr01(pe_pwm3_ctrl0, pwm3_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
			PE_PWM_O22_WrFL(pe_pwm3_ctrl0);
			break;
		default :
			break;
	}
	...
	do {
		PWM_DDR_H_O22_T header;
		static PWM_DDR_INFO0_O22_T info0 = {.udata32=0};
		if (gBE_PWM_DDR_O22 == NULL) break;
		if (pwmId > 1) break;
		if (pwmId == 0)
		{
			*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_ctrl0))) = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_ctrl0)));
			*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_ctrl1))) = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_ctrl1)));
			*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_ctrl2))) = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_ctrl2)));
			info0.pwm0_duty = (duty < 0xff)? duty:0xff;
			info0.pwm0_freq = (gPwmInfo[pwmId].freq < 0xff)? gPwmInfo[pwmId].freq:0xff;
			*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->info0))) = info0.udata32;
		}
		else
		{
			*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_ctrl0))) = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_ctrl0)));
			*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_ctrl1))) = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_ctrl1)));
			*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_ctrl2))) = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_ctrl2)));
			info0.pwm1_duty = (duty < 0xff)? duty:0xff;
			info0.pwm1_freq = (gPwmInfo[pwmId].freq < 0xff)? gPwmInfo[pwmId].freq:0xff;
			*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->info0))) = info0.udata32;
		}
		rmb();
		header.udata32 = gBE_PWM_DDR_O22->header.udata32;
		if (header.count == 0xff)
		{
			header.count = 1;
		}
		else
		...
		gBE_PWM_DDR_O22->header.udata32 = header.udata32;
		wmb();
		BE_PRINT_PWM("ddr pwm%d ctrl (h:%08x)(i0:%08x) : %08x,%08x,%08x, %08x,%08x,%08x\n",pwmId,header.udata32,info0.udata32,\
		*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_ctrl0))),*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_ctrl1))),*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_ctrl2))),\
		*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_ctrl0))),*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_ctrl1))),*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_ctrl2))));
	} while(0);

	BE_PRINT_PWM("[%d] 0(en:%d,mod:%d,res:%d,inv:%d,sel:%d,pos:%d) 1(lp:%d,w:%d) 2(mth:%d,dspH:%d,zero:%d,dspL:%d) i(duty:%d,dutyRatio:%d,calc_duty:%d,freq:%d)\n",pwmId,\
	gPwmInfo[pwmId].ctrl.pwm_en,gPwmInfo[pwmId].ctrl.pwm_freq_mode,gPwmInfo[pwmId].ctrl.pwm_resolution,gPwmInfo[pwmId].ctrl.pwm_inv,gPwmInfo[pwmId].ctrl.pwm_sel,gPwmInfo[pwmId].ctrl.pwm_width_falling_pos,\
	gPwmInfo[pwmId].lowPowerModeEn,gPwmInfo[pwmId].ctrl.pwm_free_width,gPwmInfo[pwmId].ctrl.pwm_method,gPwmInfo[pwmId].ctrl.pwm_fc_h_disp,zeroDuty,gPwmInfo[pwmId].ctrl.pwm_fc_l_disp,\
	gPwmInfo[pwmId].duty,gPwmInfo[pwmId].dutyRatio,duty,gPwmInfo[pwmId].freq);
...
static int _REG_SetPwmTiming(UINT32 pwmId)
{
	...
	switch (pwmId) {
		case 0 :
			PE_PWM_O22_RdFL(pe_pwm_v_load_write);
			PE_PWM_O22_RdFL(pe_pwm0_v_r);
			PE_PWM_O22_RdFL(pe_pwm0_v_f);
			PE_PWM_O22_RdFL(pe_pwm0_v_sub_r);
			PE_PWM_O22_RdFL(pe_pwm0_v_sub_f);
			PE_PWM_O22_Wr01(pe_pwm_v_load_write, pwm0_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
			PE_PWM_O22_Wr01(pe_pwm0_v_r, pwm0_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
			PE_PWM_O22_Wr01(pe_pwm0_v_r, pwm0_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			PE_PWM_O22_Wr01(pe_pwm0_v_f, pwm0_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
			PE_PWM_O22_Wr01(pe_pwm0_v_f, pwm0_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			PE_PWM_O22_Wr01(pe_pwm_v_load_write, pwm0_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			PE_PWM_O22_Wr01(pe_pwm0_v_sub_r, pwm0_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
			PE_PWM_O22_Wr01(pe_pwm0_v_sub_r, pwm0_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			PE_PWM_O22_Wr01(pe_pwm_v_load_write, pwm0_v_sub_f_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			PE_PWM_O22_Wr01(pe_pwm0_v_sub_f, pwm0_v_sub_f, gPwmInfo[pwmId].t2nd.pwm_v_f);
			PE_PWM_O22_Wr01(pe_pwm0_v_sub_f, pwm0_v_sub_f_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			PE_PWM_O22_WrFL(pe_pwm_v_load_write);
			PE_PWM_O22_WrFL(pe_pwm0_v_r);
			PE_PWM_O22_WrFL(pe_pwm0_v_f);
			PE_PWM_O22_WrFL(pe_pwm0_v_sub_r);
			PE_PWM_O22_WrFL(pe_pwm0_v_sub_f);
			break;
		case 1 :
			PE_PWM_O22_RdFL(pe_pwm_v_load_write);
			PE_PWM_O22_RdFL(pe_pwm1_v_r);
			PE_PWM_O22_RdFL(pe_pwm1_v_f);
			PE_PWM_O22_RdFL(pe_pwm1_v_sub_r);
			PE_PWM_O22_RdFL(pe_pwm1_v_sub_f);
			PE_PWM_O22_Wr01(pe_pwm_v_load_write, pwm1_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
			PE_PWM_O22_Wr01(pe_pwm1_v_r, pwm1_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
			PE_PWM_O22_Wr01(pe_pwm1_v_r, pwm1_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			PE_PWM_O22_Wr01(pe_pwm1_v_f, pwm1_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
			PE_PWM_O22_Wr01(pe_pwm1_v_f, pwm1_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			PE_PWM_O22_Wr01(pe_pwm_v_load_write, pwm1_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			PE_PWM_O22_Wr01(pe_pwm1_v_sub_r, pwm1_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
			PE_PWM_O22_Wr01(pe_pwm1_v_sub_r, pwm1_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			PE_PWM_O22_Wr01(pe_pwm_v_load_write, pwm1_v_sub_f_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			PE_PWM_O22_Wr01(pe_pwm1_v_sub_f, pwm1_v_sub_f, gPwmInfo[pwmId].t2nd.pwm_v_f);
			PE_PWM_O22_Wr01(pe_pwm1_v_sub_f, pwm1_v_sub_f_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			PE_PWM_O22_WrFL(pe_pwm_v_load_write);
			PE_PWM_O22_WrFL(pe_pwm1_v_r);
			PE_PWM_O22_WrFL(pe_pwm1_v_f);
			PE_PWM_O22_WrFL(pe_pwm1_v_sub_r);
			PE_PWM_O22_WrFL(pe_pwm1_v_sub_f);
			break;
		case 2 :
			PE_PWM_O22_RdFL(pe_pwm_v_load_write);
			PE_PWM_O22_RdFL(pe_pwm2_v_r);
			PE_PWM_O22_RdFL(pe_pwm2_v_f);
			PE_PWM_O22_RdFL(pe_pwm2_v_sub_r);
			PE_PWM_O22_RdFL(pe_pwm2_v_sub_f);
			PE_PWM_O22_Wr01(pe_pwm_v_load_write, pwm2_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
			PE_PWM_O22_Wr01(pe_pwm2_v_r, pwm2_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
			PE_PWM_O22_Wr01(pe_pwm2_v_r, pwm2_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			PE_PWM_O22_Wr01(pe_pwm2_v_f, pwm2_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
			PE_PWM_O22_Wr01(pe_pwm2_v_f, pwm2_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			PE_PWM_O22_Wr01(pe_pwm_v_load_write, pwm2_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			PE_PWM_O22_Wr01(pe_pwm2_v_sub_r, pwm2_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
			PE_PWM_O22_Wr01(pe_pwm2_v_sub_r, pwm2_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			PE_PWM_O22_Wr01(pe_pwm_v_load_write, pwm2_v_sub_f_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			PE_PWM_O22_Wr01(pe_pwm2_v_sub_f, pwm2_v_sub_f, gPwmInfo[pwmId].t2nd.pwm_v_f);
			PE_PWM_O22_Wr01(pe_pwm2_v_sub_f, pwm2_v_sub_f_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			PE_PWM_O22_WrFL(pe_pwm_v_load_write);
			PE_PWM_O22_WrFL(pe_pwm2_v_r);
			PE_PWM_O22_WrFL(pe_pwm2_v_f);
			PE_PWM_O22_WrFL(pe_pwm2_v_sub_r);
			PE_PWM_O22_WrFL(pe_pwm2_v_sub_f);
			break;
		default :
			break;
	}
	...
	do {
		if (gBE_PWM_DDR_O22 == NULL) break;
		if (pwmId != 0) break;
		if (gPwmInfo[pwmId].t1st.pwm_v_id != 0) break;
		*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm_v_load_write))) = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm_v_load_write)));
		*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_v_r)))         = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_v_r)));
		*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_v_f)))         = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_v_f)));
		*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_v_sub_r)))     = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_v_sub_r)));
		*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm0_v_sub_f)))     = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_v_sub_f)));
		wmb();
		BE_PRINT_PWM("ddr pwm0_v : %08x, %08x,%08x,%08x,%08x\n",*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm_v_load_write))),\
		*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_v_r))),*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_v_f))),\
		*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_v_sub_r))),*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm0_v_sub_f))));
	} while(0);
	...
	do {
		...
		if (gPwmInfo[pwmId].t1st.pwm_v_id != 0) break;
		*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm_v_load_write))) = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm_v_load_write)));
		*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_v_r)))         = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_v_r)));
		*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_v_f)))         = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_v_f)));
		*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_v_sub_r)))     = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_v_sub_r)));
		*((volatile UINT32 *)(&(gBE_PWM_DDR_O22->pe_pwm1_v_sub_f)))     = *((volatile UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_v_sub_f)));
		wmb();
		BE_PRINT_PWM("ddr pwm1_v : %08x, %08x,%08x,%08x,%08x\n",*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm_v_load_write))),\
		*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_v_r))),*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_v_f))),\
		*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_v_sub_r))),*((UINT32 *)(&(gPE_PWM_O22.shdw.b0->pe_pwm1_v_sub_f))));
	} while(0);

	BE_PRINT_PWM("[%d] we:%d, id:%d, r:%d, f:%d\n",pwmId,gPwmInfo[pwmId].t1st.pwm_v_we,gPwmInfo[pwmId].t1st.pwm_v_id,gPwmInfo[pwmId].t1st.pwm_v_r,gPwmInfo[pwmId].t1st.pwm_v_f);
...
static int _REG_GetPwmTiming(UINT32 freq_mode, UINT32 *pRising, UINT32 *pFall)
{
	...
	do {
		...
		{
			case 0 :
				PE_PWM_O22_RdFL(pe_pwm3_0_low);
				PE_PWM_O22_RdFL(pe_pwm3_0_high);
				PE_PWM_O22_Rd01(pe_pwm3_0_low, pwm3_0_low, *pRising);
				PE_PWM_O22_Rd01(pe_pwm3_0_high, pwm3_0_high, *pFall);
				break;
			case 1 :
				PE_PWM_O22_RdFL(pe_pwm3_1_low);
				PE_PWM_O22_RdFL(pe_pwm3_1_high);
				PE_PWM_O22_Rd01(pe_pwm3_1_low, pwm3_1_low, *pRising);
				PE_PWM_O22_Rd01(pe_pwm3_1_high, pwm3_1_high, *pFall);
				break;
			case 2 :
				PE_PWM_O22_RdFL(pe_pwm3_2_low);
				PE_PWM_O22_RdFL(pe_pwm3_2_high);
				PE_PWM_O22_Rd01(pe_pwm3_2_low, pwm3_2_low, *pRising);
				PE_PWM_O22_Rd01(pe_pwm3_2_high, pwm3_2_high, *pFall);
				break;
			case 3 :
				PE_PWM_O22_RdFL(pe_pwm3_3_low);
				PE_PWM_O22_RdFL(pe_pwm3_3_high);
				PE_PWM_O22_Rd01(pe_pwm3_3_low, pwm3_3_low, *pRising);
				PE_PWM_O22_Rd01(pe_pwm3_3_high, pwm3_3_high, *pFall);
				break;
			default :
...
int BE_PWM_HW_O22_ProcHandler(BE_PWM_PROC_ID_T proc_id, struct seq_file* m, void* data)
{
	int	i;
	struct pwm_ddr_ctx *p_ctx = &_g_pwm_o22_ctx;
	...
	switch(proc_id)
	{
		...
		{
			seq_printf(m, "ddr ctx : year:%d, init:%d, wireless:%d, lcd:%d\n",p_ctx->year,p_ctx->opt.init,p_ctx->opt.wireless,p_ctx->opt.lcd);
			seq_printf(m, "ddr base : 0x%08x, size : %ld\n",_g_pwm_o22_ddr_base, sizeof(PWM_DDR_O22_T));
			if (gBE_PWM_DDR_O22 != NULL)
			{
				PWM_DDR_O22_T shdw;
				rmb();
				memcpy(&shdw,(const void *)gBE_PWM_DDR_O22,sizeof(PWM_DDR_O22_T));
				seq_printf(m, "%-20s : 0x%08x\n","header",shdw.header.udata32);
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_ctrl0",*((UINT32 *)(&(shdw.pe_pwm0_ctrl0))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_ctrl1",*((UINT32 *)(&(shdw.pe_pwm0_ctrl1))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_ctrl2",*((UINT32 *)(&(shdw.pe_pwm0_ctrl2))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_ctrl0",*((UINT32 *)(&(shdw.pe_pwm1_ctrl0))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_ctrl1",*((UINT32 *)(&(shdw.pe_pwm1_ctrl1))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_ctrl2",*((UINT32 *)(&(shdw.pe_pwm1_ctrl2))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm_v_load_write",*((UINT32 *)(&(shdw.pe_pwm_v_load_write))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_v_r",*((UINT32 *)(&(shdw.pe_pwm0_v_r))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_v_f",*((UINT32 *)(&(shdw.pe_pwm0_v_f))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_v_r",*((UINT32 *)(&(shdw.pe_pwm1_v_r))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_v_f",*((UINT32 *)(&(shdw.pe_pwm1_v_f))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_v_sub_r",*((UINT32 *)(&(shdw.pe_pwm0_v_sub_r))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_v_sub_f",*((UINT32 *)(&(shdw.pe_pwm0_v_sub_f))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_v_sub_r",*((UINT32 *)(&(shdw.pe_pwm1_v_sub_r))));
				seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_v_sub_f",*((UINT32 *)(&(shdw.pe_pwm1_v_sub_f))));
				seq_printf(m, "%-20s : 0x%08x\n","info0",shdw.info0.udata32);
			}
			seq_printf(m, "\n");
			...
			PE_PWM_O22_RdFL(pe_pwm2_v_sub_r);
			PE_PWM_O22_RdFL(pe_pwm2_v_sub_f);
			seq_printf(m, "apb base : 0x%08x\n",PE_PWM_REG_O22A0_BASE);
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_ctrl0",PE_PWM_O22_Rd(pe_pwm0_ctrl0));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_ctrl1",PE_PWM_O22_Rd(pe_pwm0_ctrl1));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_ctrl2",PE_PWM_O22_Rd(pe_pwm0_ctrl2));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_ctrl0",PE_PWM_O22_Rd(pe_pwm1_ctrl0));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_ctrl1",PE_PWM_O22_Rd(pe_pwm1_ctrl1));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_ctrl2",PE_PWM_O22_Rd(pe_pwm1_ctrl2));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm2_ctrl0",PE_PWM_O22_Rd(pe_pwm2_ctrl0));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm2_ctrl1",PE_PWM_O22_Rd(pe_pwm2_ctrl1));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm2_ctrl2",PE_PWM_O22_Rd(pe_pwm2_ctrl2));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm_v_load_write",PE_PWM_O22_Rd(pe_pwm_v_load_write));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_v_r",PE_PWM_O22_Rd(pe_pwm0_v_r));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_v_f",PE_PWM_O22_Rd(pe_pwm0_v_f));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_v_sub_r",PE_PWM_O22_Rd(pe_pwm0_v_sub_r));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm0_v_sub_f",PE_PWM_O22_Rd(pe_pwm0_v_sub_f));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_v_r",PE_PWM_O22_Rd(pe_pwm1_v_r));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_v_f",PE_PWM_O22_Rd(pe_pwm1_v_f));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_v_sub_r",PE_PWM_O22_Rd(pe_pwm1_v_sub_r));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm1_v_sub_f",PE_PWM_O22_Rd(pe_pwm1_v_sub_f));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm2_v_r",PE_PWM_O22_Rd(pe_pwm2_v_r));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm2_v_f",PE_PWM_O22_Rd(pe_pwm2_v_f));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm2_v_sub_r",PE_PWM_O22_Rd(pe_pwm2_v_sub_r));
			seq_printf(m, "%-20s : 0x%08x\n","pe_pwm2_v_sub_f",PE_PWM_O22_Rd(pe_pwm2_v_sub_f));
			seq_printf(m, "\n");
			for (i=0;i<BE_PWM_MAX;i++)
			{
