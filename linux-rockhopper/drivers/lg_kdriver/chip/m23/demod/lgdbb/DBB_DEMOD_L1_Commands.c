...
#define SEND_TIME_WARNING	3000
#define RECV_TIME_WARNING	3000
#define WAIT_TIME_WARNING	3000
#define SPEND_TIME_WARNING	10000
#define SPEND_TIME_LIMIT	30000
#else
...
static int _DEMOD_L1_CommandRequest(LX_DEMOD_L1_Context * core, UINT8 cmd, UINT8 seq, UINT8 * preq, UINT32 len) {
	UINT64 start_clock;
	UINT64 spend_clock;
	UINT32 spend_us;
	UINT32 spend_ms;
	...
	DEMOD_DEBUG("send req start, cmd x%02X(%u), %d, %s\n", cmd, seq, len, DEMOD_BYTE_TO_HEXSTR((char *)preq, len));
	do {
		err = DEMOD_L1_IPC_Send(core, CMD_TYPE(CMD_MAJOR_TYPE_API, cmd), seq, (char *)preq, len);
		...
		if (state == CBUF_STATE_DONE) {
			...
			if ((retry--) && (spend_ms < SPEND_TIME_LIMIT)) {
				...
				seq = core->cmd_req_cnt++;
				DEMOD_NOTI("resend, x%02X(%u), retry %d, tx state %d, tooks %3u.%03ums\n", cmd, seq & 0xFF, retry, state, spend_ms, spend_us);
			} else {
...
static int _DEMOD_L1_WaitResponse(LX_DEMOD_L1_Context * core, UINT8 wait_cmd, UINT8 wait_seq, UINT8 * prsp, int * plen)
{
	UINT64 start_clock;
	UINT64 spend_clock;
	UINT32 spend_us;
	...
	do {
		...
		} else if (err == -ERROR_DEMOD_FW_NOT_READY) {
			DEMOD_NOTI("fw stopped, wait x%02x(%3u), err 0x%02x, %s\n", wait_cmd, wait_seq, -err, DEMOD_L1_API_Error_Text(err));
			break;
		} else {
...
static int _DEMOD_L1_Process (LX_DEMOD_L1_Context * core, UINT8 command, UINT8 req_len, void * req_buf, UINT8 rsp_len, void * rsp_buf)
{
	...
	err = _DEMOD_L1_CommandRequest(core, command, seq, (UINT8 *)req_buf, req_len);
	if (err != NO_DEMOD_ERROR) {
	...
	/* wait & recv response */
	err = _DEMOD_L1_WaitResponse(core, command, seq, (UINT8 *)rsp_buf, &rcv_len);
	if(err != NO_DEMOD_ERROR) {
...
#if defined(DEMOD_SPEND_TIME_CHECK)
	DEMOD_INFO("command tooks %u.%03u, cmd x%02X(%u), send %3u.%03u, recv %3u.%03u\n", spend_ms, spend_us, command, seq, _cmd_req_ms, _cmd_req_us, _cmd_res_ms, _cmd_res_us);
#else
	if (spend_ms > SPEND_TIME_WARNING)
...
int DEMOD_L1_CommandDump (LX_DEMOD_L1_CmdReplyObj *rsp, UINT32 cmd_code, char *logbuf, int *pidx)
{
	...
	switch (cmd_code) {
		case DEMOD_CMD_STATUS_CODE:
			...
			idx += SPRINTF(logbuf + idx, "\t""FEC_LOCK  : %s\n",				(rsp->dd_status.lock_fec == DEMOD_LOCK_LOCKED) ? "Locked" : "UnLocked");
		...
		case DEMOD_CMD_VSB_STATUS_CODE:
			...
			idx += SPRINTF(logbuf + idx, "\t""PKT ERROR : %u\n",				(rsp->vsb_status.pe));
			break;
#endif
