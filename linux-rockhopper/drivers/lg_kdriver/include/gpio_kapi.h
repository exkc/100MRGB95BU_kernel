...
#define GPIO_IORW_COMMAND_GET		_IOWR(GPIO_IOC_MAGIC,	1, LX_GPIO_PARAM_T)

#define GPIO_IOW_EX_COMMAND_SET		_IOW(GPIO_IOC_MAGIC,	10, LX_GPIO_PARAM_T)
#define GPIO_IORW_EX_COMMAND_GET	_IOWR(GPIO_IOC_MAGIC,	11, LX_GPIO_PARAM_T)
...
#define HIGH_EDGE_ENABLE 			4
#define LOW_EDGE_ENABLE 			5
...
#define DISCARD_SAMPLE_NUMBER   (1)
#define EXTRA_SAMPLE_FOR_COUNT  (1)

#define DURATATION_RETRY_CNT	(10)
#define VALID_DURATION_MIN	(200)//us
#define INT_ARRAY_NUM 	6

#if defined (CHIP_NAME_o18) || defined (CHIP_NAME_m19)
#define GPIO_IRQ_NUM_NR 18
#define GPIO_NUM_IN_INT_ARRAY 		8			//gpio 8 pin share 1 interrupt
#else
...
typedef struct GPIO_INTR_CALLBACK{
	...
	UINT32 pinNumber;							// valid for only GPIO_KERNEL_API mode
	UINT32 stop;								// valid for only GPIO_KERNEL_API mode

	union
	{
		void (*pfnGPIO_CB)	(UINT32 value);		///< The audio event message callback.
		UINT32 compat_pfnGPIO_CB;
		UINT64 sizer;
	};
...
typedef struct
{
	...
	GPIO_INTR_CALLBACK_T	cb __attribute__((aligned(8)));
...
/*----------------------------------------------------------------------------------------
	Configuration
----------------------------------------------------------------------------------------*/
#define LX_GPIO_CFG_NONE         0x00000000
#define LX_GPIO_CFG_LEGACY_MODE  0x00000001

extern UINT32   lx_gpio_cfg(void);
