...
static void e60_ucom_init(void);
static void e60_ucom_chipreset(void);
...
/*----------------------------------------------------------------------------------------
	global Functions
---------------------------------------------------------------------------------------*/
static int e60f_ucom_enable = 1;
static int e60_ucom_inited = 0;

module_param(e60f_ucom_enable, int, S_IRUGO);
MODULE_PARM_DESC(e60f_ucom_enable, "ucom enable");


// CPU Governor

static ucom_e60_func_t* ucom_e60_func = NULL;
...
//static UINT8 e60_ucom_kbuf[100];
int e60_ucom_ioctl_main (unsigned int cmd, unsigned long arg, struct file *file)
{
	int ret = 0;
	UINT8	sbuf[100];
	UINT8*	kbuf;
...
static int e60_ucom_ioctl (unsigned int cmd, unsigned long arg, struct file *file)
{
	int ret = 0;

	if ((_IOC_TYPE(cmd) != UCOM_IOC_MAGIC) )
		return -EINVAL;
	if(get_micom_disable() == 1)
	{
		KDRV_UCOM_DEBUG("skip micom[%d]\n",get_micom_disable());
		return 0;
	}
	ret = e60_ucom_ioctl_main(cmd, arg, file);

    return ret;
}

static int e60_ucom_suspend(void)
{
	printk("ucom suspend[%d]\n",get_micom_disable());
	return 0;
}

static int e60_ucom_resume(void)
{
	set_micom_disable(0);
...
struct file * e60_ucom_getfp(void)
{
	return g_ucomfp;
}
...
static void e60_ucom_init(void)
{
	...
	if(e60_ucom_inited == 0)
	{
		init_completion(&nvram_completion);
		KDRV_UCOM_LOCK_INIT();
		ucom_e60_func = get_e60_ucom_func();
		ucom_e60_func->init();
		e60_ucom_inited = 1;
	}
...
{
	ucom_e60_func = get_e60_ucom_func();
	ucom_e60_func->chipreset();
	return;
}
static ucom_driver_t e60_ucom_driver =
{
	.init = e60_ucom_init,
	.ioctl = e60_ucom_ioctl,
	.suspend = e60_ucom_suspend,
	.resume = e60_ucom_resume,
	.getfp = e60_ucom_getfp,
};

ucom_driver_t* get_e60_ucom_driver(void)
{
	return &e60_ucom_driver;
}
