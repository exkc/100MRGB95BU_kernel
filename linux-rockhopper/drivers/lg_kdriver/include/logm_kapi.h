...
typedef enum
{
	...
	LX_LOGM_LOGTYPE_1,		//hidden log, just put the log at buffer.
	LX_LOGM_LOGTYPE_2 = 0,
	LX_LOGM_LOGTYPE_3 = 0,
}LX_LOGM_LOGTYPE_T;
...
struct logm_mask_ctrl
{
	...
	{
		char *name;
		unsigned int compat_name;
		unsigned long long sizer_name;
	};
	unsigned int name_size;
	union
	{
		struct logm_obj_bit_name *bit_name;
		unsigned int compat_bit_name;
		unsigned long long sizer_bit_name;
	};
...
#define LOGM_HIDDEN_MASK_CTRL_WRITE 		0x20
#define LOGM_HIDDEN_MASK_CTRL_READ  		0x21
#define LOGM_HIDDEN_MASK_CTRL_ALL_WR 		0x22
#define LOGM_HIDDEN_MASK_CTRL_BIT_ENABLE 	0x23
#define LOGM_HIDDEN_MASK_CTRL_BIT_DISABLE 	0x24
...
int LOGM_ObjMaskAdd(int fd, unsigned int mask);
...
int LOGM_ObjHiddenMaskSet(int fd, unsigned int mask);
int LOGM_ObjHiddenMaskGet(int fd, unsigned int *mask);
int LOGM_ObjBitHiddenMaskEnable(int fd, int bit);
int LOGM_ObjBitHiddenMaskDisable(int fd, int bit);
