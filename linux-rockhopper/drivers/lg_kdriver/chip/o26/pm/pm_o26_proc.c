...
void o26_proc_init (void);
void o26_proc_cleanup (void);
...
OS_PROC_SEQRD_DEFINE_EX(pm_status_proc_fops, _PM_PrintStatus,NULL)
	...
	static OS_PROC_DESC_TABLE_T	_g_pm_o26_device_proc_table[] =
{
	...
	{ "cpuvol",					PROC_ID_CPUVOL			, OS_PROC_FLAG_READ | OS_PROC_FLAG_WRITE  },
	{ "corevol",				PROC_ID_COREVOL			, OS_PROC_FLAG_READ | OS_PROC_FLAG_WRITE  },
...
static OS_PROC_DESC_TABLE_T	_g_pm_o26_gov_proc_table[] =
{
...
static OS_PROC_DESC_TABLE_T	_g_pm_o26_debug_proc_table[] =
{
...
#define O26_AVS  0
extern o26_sidd_t gSidd_check_table_o26[6];
extern char * cpu_volttype_o26[6];
extern char * core_volttype_high_o26[6];
extern char * core_volttype_o26[6];
extern pms_o26_avsinto_t	o26_avs_info;


static void _PM_PrintStatus(struct seq_file *m, void* data)
{
	pms_o26_avsinto_t info ;
	memset (&info,0x0, sizeof(pms_o26_avsinto_t));

	seq_printf( m, "governor: %s\n", pm_o26_gov->get_gov_str(pm_o26_gov->get_gov()) );
	seq_printf( m, "cpu_num: %d\n", pm_o26_gov->get_cpunum());
	seq_printf( m, "load: %u %u %u %u\n", pm_o26_gov->get_load(0),pm_o26_gov->get_load(1),pm_o26_gov->get_load(2),pm_o26_gov->get_load(3)) ;
	seq_printf( m, "loadmax: %u \n", pm_o26_gov->get_load(4)) ;
	seq_printf( m, "loadavg: %u \n", pm_o26_gov->get_load(5)) ;
	seq_printf( m, "freq: %u Mhz\n", pm_o26_gov->get_max_freq()) ;
	seq_printf( m, "===========================\n"	);
	seq_printf( m, "coreavs_type: %s \n", pm_o26_avs->get_core_avs_type_str()) ;
	pm_o26_avs->get_core_avs_type(&info);
	seq_printf( m, "coreavs_current(mA): %u\n", info.avscurrent);
	pm_o26_avs->get_cpu_avs_type(&info);
	seq_printf( m, "cpuavs_type: %s \n", pm_o26_avs->get_cpu_avs_type_str()) ;
	seq_printf( m, "cpuavs_current(mA): %u\n", info.avscurrent);
	seq_printf( m, "avs high [%u]\n", o26_avs_info.is_avshigh);
	seq_printf( m, "===========================\n"	);
	seq_printf( m, "|o26 core avs %5s table|\n",(o26_avs_info.is_avshigh)?"HIGH":"NORMAL");
	seq_printf( m, "===========================\n"	);

	if(o26_avs_info.is_avshigh)
	{
		seq_printf( m, "|    <= %3d |  %smV      |\n", gSidd_check_table_o26[O26_AVS].core_sidd_f_min,core_volttype_high_o26[1] );
		seq_printf( m, "|     > %3d |  %smV      |\n", gSidd_check_table_o26[O26_AVS].core_sidd_f_min,core_volttype_high_o26[3]);
	}
	else
	{
		seq_printf( m, "| 	 <= %3d |  %smV      |\n", gSidd_check_table_o26[O26_AVS].core_sidd_f_min,core_volttype_o26[1] );
		seq_printf( m, "|     > %3d |  %smV      |\n", gSidd_check_table_o26[O26_AVS].core_sidd_f_min,core_volttype_o26[3]);
	}
	seq_printf( m, "===========================\n"	);
	seq_printf( m, "|  o26a0  cpu avs table |\n");
	seq_printf( m, "===========================\n"	);
	seq_printf( m, "|    <= %3d |  %smV      |\n", gSidd_check_table_o26[O26_AVS].cpu_sidd_f_min,cpu_volttype_o26[1]);
	seq_printf( m, "| %3d ~ %3d |  %smV      |\n", gSidd_check_table_o26[O26_AVS].cpu_sidd_f_min,gSidd_check_table_o26[O26_AVS].cpu_sidd_ff_min,cpu_volttype_o26[3] );
	seq_printf( m, "|     > %3d |  %smV      |\n", gSidd_check_table_o26[O26_AVS].cpu_sidd_ff_min,cpu_volttype_o26[4]);
	seq_printf( m, "===========================\n"	);

	pm_o26_avs->get_cpu_avs_type(&info);
	seq_printf( m, "===========================\n"	);
	seq_printf( m,"      o26 CPU TEMP[%d]'C    \n",pm_o26_avs->get_temperature());
	seq_printf( m, "===========================\n"	);
 ...
 *
 */
static int	o26_read_debug_proc(UINT32 procId, char* buffer )
{
	int ret = 0 ;
...
static int	o26_read_gov_proc(UINT32 procId, char* buffer )
{
	...
	switch( procId ) {
		...
		case GOV_ID_MIN_FREQ:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_min_freq());
			break;
		case GOV_ID_HARD_UP_THRESHOLD:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_hard_up_threshold() );
			break;
		case GOV_ID_EASY_UP_THRESHOLD:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_easy_up_threshold() );
			break;
		case GOV_ID_EASY_COUNT:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_easy_count());
			break;
		case GOV_ID_FREQDOWN_COUNT:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_freqdown_count());
			break;
		case GOV_ID_MPD_COUNT:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_mpd_count());
			break;
		case GOV_ID_MPD_UP_TH:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_mpd_up_threshold());
			break;
		case GOV_ID_MPD_DN_TH:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_mpd_dn_threshold());
			break;
		case GOV_ID_MPD_UP_TH2:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_mpd_up_threshold2());
			break;
		case GOV_ID_MPD_DN_TH2:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_mpd_dn_threshold2());
			break;
		case GOV_ID_FREQ_UP_TH:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_freq_up_threshold());
			break;
		case GOV_ID_FREQ_DN_TH:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_freq_dn_threshold());
			break;
		case GOV_ID_FV_TIME:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_fv_time());
			break;
		case GOV_ID_HIST:
			ret = sprintf( buffer, "%u\n", pm_o26_gov->get_hist());
			break;
...
static int o26_write_debug_proc( UINT32 procId, char* command )
{
	int ret = 0;
...
static int o26_write_gov_proc( UINT32 procId, char* command )
{
	...
	switch( procId )
	{
		case GOV_ID_MAX_FREQ:
			...
			if( input_value < pm_o26_gov->get_min_freq()) {
				printk("input frequency is lower then min frequency\n");
				break;
			}
			pm_o26_gov->set_max_freq(input_value);
			break;
		case GOV_ID_MIN_FREQ:
			input_n = sscanf(command,"%u\n", &input_value);
			if( input_value < pm_o26_gov->get_max_freq()) {
				printk("input frequency is higher then max frequency\n");
				break;
			}
			pm_o26_gov->set_min_freq(input_value);
			break;
		case GOV_ID_HARD_UP_THRESHOLD:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_hard_up_threshold(input_value);
			break;
		case GOV_ID_EASY_UP_THRESHOLD:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_easy_up_threshold(input_value);
			break;
		case GOV_ID_EASY_COUNT:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_easy_count(input_value);
			break;
		case GOV_ID_FREQDOWN_COUNT:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_freqdown_count(input_value);
			break;
		case GOV_ID_MPD_COUNT:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_mpd_count(input_value);
			break;
		case GOV_ID_MPD_UP_TH:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_mpd_up_threshold(input_value);
			break;
		case GOV_ID_MPD_DN_TH:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_mpd_dn_threshold(input_value);
			break;
		case GOV_ID_MPD_UP_TH2:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_mpd_up_threshold2(input_value);
			break;
		case GOV_ID_MPD_DN_TH2:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_mpd_dn_threshold2(input_value);
			break;
		case GOV_ID_FREQ_UP_TH:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_freq_up_threshold(input_value);
			break;
		case GOV_ID_FREQ_DN_TH:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_freq_dn_threshold(input_value);
			break;
		case GOV_ID_FV_TIME:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_fv_time(input_value);
			break;
		case GOV_ID_HIST:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_gov->set_hist(input_value);
			break;
		default:
			ret = -1;
			break;
...
static int o26_write_proc( UINT32 procId, char* command )
{
	...
	switch( procId )
	{
		case PROC_ID_GOVERNOR:
			...
			if (input_value <= 10)
			{
				pm_o26_test->set_testmode(0);

				pm_o26_gov->set_gov(input_value);
		...
		case PROC_ID_MP_ENABLE:
			...
			if (input_value != 0)
				input_value = 1;
			pm_o26_gov->set_mp_enable(input_value);
			break;
		case PROC_ID_FREQ_ENABLE:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value != 0)
				input_value = 1;
			pm_o26_gov->set_freq_enable(input_value);
			break;
		case PROC_ID_FREQ_DEBUG_ENABLE:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value != 0)
				input_value = 1;
			pm_o26_gov->set_freq_debug_enable(input_value);
			break;
		case PROC_ID_DEBUG_ENABLE:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value != 0)
				input_value = 1;
			pm_o26_gov->set_debug_enable(input_value);
			break;
		case PROC_ID_TS_ENABLE:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value != 0)
				input_value = 1;
			pm_o26_gov->set_ts_enable(input_value);
			break;
		case PROC_ID_TS_DEBUG_ENABLE:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value != 0)
				input_value = 1;
			pm_o26_avs->set_ts_debug_enable(input_value);
			break;

		case PROC_ID_TS_CPU_APARAM:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_avs->set_cpu_ts_aparam(input_value);
			break;
		case PROC_ID_TS_CPU_BPARAM:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_avs->set_cpu_ts_bparam(input_value);
			break;
		case PROC_ID_TS_GPU_APARAM:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_avs->set_gpu_ts_aparam(input_value);
			break;
		case PROC_ID_TS_GPU_BPARAM:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_avs->set_gpu_ts_bparam(input_value);
			break;
		case PROC_ID_TO_SUSPEND:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value == 1)
			{
				temp = pm_o26_gov->get_gov() | PMS_GOV_TOSUSPEND_FLAG;
				pm_o26_gov->set_gov(temp);
				pm_o26_gov->set_startup_timer(0);
				pm_o26_func->set_max_performance();
			}
			break;
		case PROC_ID_RESET_LOADMON:
			pm_o26_gov->reset_load_monitor();
			break;
		case PROC_ID_SAMPLING_RATE:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_func->set_sampling_rate(input_value);
			break;
		case PROC_ID_FREQ:
			input_n = sscanf(command,"%u\n", &input_value);
#if 1
			...
			pms_work.do_dvfs = 1;
			pm_o26_func->run_clk(&pms_work);
#else

			pm_o26_func->setfreq(input_value);
#endif
			break;

		case PROC_ID_CPU_AVS:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_avs->set_avs_force(GPIO_AVS_CPU, input_value);
			break;
		case PROC_ID_CORE_AVS:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_avs->set_avs_force(GPIO_AVS_CORE, input_value);
			break;
		case PROC_ID_CPUVOL:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_avs->set_avs_force(I2C_AVS_CPU, input_value);
			break;
		case PROC_ID_COREVOL:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_avs->set_avs_force(I2C_AVS_CORE, input_value);
			break;
		case PROC_ID_TEST:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_o26_test->set_testmode(input_value);
			break;
		default:
			ret = -1;
			break;
...
void o26_proc_init (void)
{
	...
	OS_PROC_CreateEntryEx ( PM_MODULE, _g_pm_o26_device_proc_table,
			o26_read_proc,
			o26_write_proc );
	OS_PROC_CreateEntryEx ( PM_GOV_MODULE, _g_pm_o26_gov_proc_table,
			o26_read_gov_proc,
			o26_write_gov_proc );
	OS_PROC_CreateEntryEx ( PM_DEBUG_MODULE, _g_pm_o26_debug_proc_table,
			o26_read_debug_proc,
			o26_write_debug_proc );
...
static pms_proc_t o26_proc_func =
{
	.init = o26_proc_init,
	.cleanup = o26_proc_cleanup,
};

pms_proc_t* get_o26_pm_proc(void)
{
