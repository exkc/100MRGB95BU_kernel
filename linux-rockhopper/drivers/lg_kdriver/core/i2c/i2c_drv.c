...
static int      I2C_Close(struct inode * inode, struct file * fp);
static long		I2C_Ioctl(struct file *file, unsigned int cmd, unsigned long arg);
#ifdef CONFIG_COMPAT
static long		I2C_CompatIoctl(struct file *file, unsigned int cmd, unsigned long arg);
#endif
...
static int I2C_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if(g_iic_open_done == false)
	{
		printk("I2C - suspend legacy ignore\n");
		return 0;
	}
 ...
 static int I2C_resume(struct device *dev)
{
	...
	else if(lx_chip() == LX_CHIP_L18)	/* L18 */
	{
	}
#endif
#ifdef INCLUDE_M17_CHIP_KDRV
	else if(lx_chip() == LX_CHIP_M17)	/* M16 */
	{
		/* pll reset code move to sys resume driver */
	}
#endif
#ifdef INCLUDE_O20_CHIP_KDRV
	else if(lx_chip() == LX_CHIP_O20)	/* M16 */
	{
		/* pll reset code move to sys resume driver */
	}
#endif
#ifdef INCLUDE_E60_CHIP_KDRV
	else if(lx_chip() == LX_CHIP_E60)	/* M16 */
	{
		/* pll reset code move to sys resume driver */
	}
#endif
#ifdef INCLUDE_O22_CHIP_KDRV
	else if(lx_chip() == LX_CHIP_O22)	/* M16 */
	{
		/* pll reset code move to sys resume driver */
	}
#endif
#ifdef INCLUDE_M23_CHIP_KDRV
	else if(lx_chip() == LX_CHIP_M23)	/* M16 */
	{
		/* pll reset code move to sys resume driver */
	}
#endif
...
int I2C_Init(void)
{
	...
	g_i2c_device = (I2C_DEVICE_T*)vmalloc( sizeof(I2C_DEVICE_T)*g_i2c_num_devices );
	if ( NULL == g_i2c_device )
	{
		I2C_PRINT("out of memory. can't allocate \n");
		return -ENOMEM;
	}
...
static long I2C_CompatIoctl (struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case I2C_IORW_READ:
	{
		LX_I2C_RW_DATA_T arg_lx_i2c_rw_data_t;

		ret = copy_from_user(&arg_lx_i2c_rw_data_t, (void *)arg,
							 sizeof(LX_I2C_RW_DATA_T));
		if (ret)
		{
			return -EINVAL;
		}

		arg_lx_i2c_rw_data_t.buf =
			(UINT8 *)(uintptr_t)arg_lx_i2c_rw_data_t.compat_buf;

		ret = copy_to_user((void *)arg, &arg_lx_i2c_rw_data_t,
						   sizeof(LX_I2C_RW_DATA_T));
		if (ret)
		{
			return -EINVAL;
