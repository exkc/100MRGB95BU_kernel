...
#if defined(CONFIG_LG_INSTANT_BOOT)
#define URB_COMPLETION_STEP     1000    /* 1000 msec */
#define RESET_RECOVERY_STEP     100     /* 100 msec */
#endif
/***********************************************************************
...
#if defined(CONFIG_ARCH_LG1K)
static void usb_stor_transport_debug(struct us_data *us, const char *label)
{
	struct device *dev = &us->pusb_dev->dev;
	struct urb *urb = us->current_urb;

	if (urb)
		dev_info(dev, "%s on ep%d%s\n", label,
			usb_pipeendpoint(urb->pipe),
			usb_pipein(urb->pipe) ? "in" : "out");

	if (us->srb)
		dev_info(dev, "cmnd: %*ph\n", min_t(int, us->srb->cmd_len, 16),
				(const unsigned char *)us->srb->cmnd);
}
#else
static void usb_stor_transport_debug(struct us_data *us, const char *label)
{
...
static int usb_stor_msg_common(struct us_data *us, int timeout)
{
	...
	unsigned long total_timeout = timeout ? : MAX_SCHEDULE_TIMEOUT;

	timeout = msecs_to_jiffies(URB_COMPLETION_STEP);
	timeout = (timeout < total_timeout) ? timeout : total_timeout;
#endif
	...
	while (timeleft == 0 && waiting < total_timeout) {
		usb_stor_transport_debug(us, "usb_stor_msg_common: timed out");

		if (pm_freezing == true) {
			dev_warn(&us->pusb_intf->dev,
				"system is suspending, mark disconnecting\n");
		...
		if (us->pusb_dev->state == USB_STATE_NOTATTACHED) {
			dev_warn(&us->pusb_intf->dev,
				"dev is not attached, mark disconnecting\n");
			set_bit(US_FLIDX_DISCONNECTING, &us->dflags);
			break;
		}
		timeleft = wait_for_completion_interruptible_timeout(
...
#endif // defined(CONFIG_LG_INSTANT_BOOT)
		...
		u16 value, u16 index, void *data, u16 size)
{
	...
	unsigned long waiting = 0;
	long timeout = msecs_to_jiffies(RESET_RECOVERY_STEP);
#endif
	 ...
	 */
#if defined(CONFIG_LG_INSTANT_BOOT)
	usb_stor_transport_debug(us, "usb_stor_reset_common: issued");

	while (waiting < HZ*6) {
		timeleft = wait_event_interruptible_timeout(us->delay_wait,
			test_bit(US_FLIDX_DISCONNECTING, &us->dflags), timeout);

		if (timeleft) // disconnecting or signaled
			break;

		usb_stor_transport_debug(us, "usb_stor_reset_common: timedout");
	...
	if (result < 0)
		...
		if (test_bit(US_FLIDX_DISCONNECTING, &us->dflags)) {
			...
			usb_stor_dbg(us, "usb_reset_device returns %d\n",
				     result);
			usb_stor_transport_debug(us,
				"usb_stor_port_reset: done");
		}
