...
static int _DEMOD_M23_IPC_Handler (LX_DEMOD_L2_Context * frontend);
...
static int _DEMOD_M23_IPC_POLL_Handler(void * data) {
	int prv_en = -1;
	...
	while (!kthread_should_stop()) {
		...
		if (_ipc_enable == TRUE) {
			_DEMOD_M23_IPC_Handler((LX_DEMOD_L2_Context *)data);
		}
	}
	return NO_DEMOD_ERROR;
}

static int _DEMOD_M23_IPC_Handler_Enable(LX_DEMOD_L2_Context * frontend, int enable)
{
...
static int _DEMOD_M23_IPC_Handler_Init(LX_DEMOD_L2_Context * frontend) {
	int err = NO_DEMOD_ERROR;

	_DEMOD_M23_IPC_Handler_Enable(frontend, FALSE);

	_ipc_poll_thread = kthread_run(_DEMOD_M23_IPC_POLL_Handler, (void *)frontend, "demod_ipc_poll");
...
static irqreturn_t _DEMOD_M23_IPC_INTR_Handler (int irq, void * dev_id, struct pt_regs * regs) {
	DEMOD_DEBUG("triggerd irq %d\n", irq);

	if (_ipc_enable == TRUE) {
		_DEMOD_M23_IPC_Handler((LX_DEMOD_L2_Context *)dev_id);
	}
...
static int _DEMOD_M23_IPC_Handler_Init(LX_DEMOD_L2_Context * frontend) {
	...
	if (_ipc_irq < 0) {
		_ipc_irq = M23_IRQ_INTR_DBB_CPU;

		/* request_irq from mcu */
		err = request_irq(_ipc_irq , (irq_handler_t)_DEMOD_M23_IPC_INTR_Handler, IRQF_TRIGGER_HIGH, "demod_ipc", (void *)frontend);
		disable_irq(_ipc_irq);
...
static UINT _DEMOD_M23_IPC_GUI_POLL_Handler(LPVOID pParam) {
	DEMOD_INFO("POLL start, tid %lu\n", GetCurrentThreadId());

	while (_ipc_poll_enable == TRUE) {
		if (_ipc_enable == TRUE) {
			_DEMOD_M23_IPC_Handler((LX_DEMOD_L2_Context *)pParam);
		}
...
static int _DEMOD_M23_IPC_Handler_Init(LX_DEMOD_L2_Context * frontend) {
	if (_ipc_poll_enable == FALSE) {
		...
		if (_ipc_poll_thread == NULL) {
			_ipc_poll_thread = AfxBeginThread(_DEMOD_M23_IPC_GUI_POLL_Handler, frontend);
		}
	}
...
static int _DEMOD_M23_IPC_DMsg_Reader(LX_DEMOD_L2_Context * frontend) {
...
static int _DEMOD_M23_IPC_DMsg_Handler(void * data) {
	int retc = NO_DEMOD_ERROR;
	...
	while (!kthread_freezable_should_stop(&was_frozen)) {
		...
		if (TRUE == _ipc_enable) {
			...
			while (DEMOD_L0_CBUF_Used(&ipc->dbg) > 0) {
				if (NO_DEMOD_ERROR != _DEMOD_M23_IPC_DMsg_Reader(frontend))
					break;
			}
		}
	}
...
static int _DEMOD_M23_IPC_DMsg_Handler_Wakeup(void) {
	wake_up_interruptible(&_ipc_dmsg_wait);
	return NO_DEMOD_ERROR;
}

static int _DEMOD_M23_IPC_DMsg_Handler_Init(LX_DEMOD_L2_Context * frontend) {
	int err = NO_DEMOD_ERROR;

	init_waitqueue_head(&_ipc_dmsg_wait);
	_ipc_dmsg_thread = kthread_run(_DEMOD_M23_IPC_DMsg_Handler, (void *)frontend, "demod_ipc_msg_poll");
...
static int _DEMOD_M23_IPC_DMsg_Handler_Enable(LX_DEMOD_L2_Context * frontend, int enable)
{
...
int DEMOD_M23_IPC_Send (UINT32 type, UINT32 seq, char *ptr, UINT32 len) {
	LX_DEMOD_L2_Context * frontend = DEMOD_Get_LGDBB_Context();
	return DEMOD_L1_IPC_Send(frontend->demod, type, seq, ptr, len);
}

int DEMOD_M23_IPC_Status(char * pDbglog, int * pDbgidx) {
	LX_DEMOD_L2_Context * frontend = DEMOD_Get_LGDBB_Context();
	int dbgidx = 0;
...
int DEMOD_M23_IPC_Dump(char * pDbglog, int * pDbgidx) {
	LX_DEMOD_L2_Context * frontend = DEMOD_Get_LGDBB_Context();
	int dbgidx = 0;
...
int DEMOD_M23_IPC_Reset(void) {
	LX_DEMOD_L2_Context * frontend = DEMOD_Get_LGDBB_Context();

	return DEMOD_L1_IPC_Reset(frontend->demod);
}

int DEMOD_M23_IPC_Enable(int enable) {
	LX_DEMOD_L2_Context * frontend = DEMOD_Get_LGDBB_Context();
	...
	if (_ipc_enable != enable) {
		...
		_DEMOD_M23_IPC_Handler_Enable(frontend, enable);
		_DEMOD_M23_IPC_DMsg_Handler_Enable(frontend, enable);
	}
...
#if defined(DEMOD_IPC_DMSG_POLLING)
	_DEMOD_M23_IPC_DMsg_Handler_Wakeup();
#else
	while (DEMOD_L0_CBUF_Used(&ipc->dbg) > 0) {
		if (NO_DEMOD_ERROR != _DEMOD_M23_IPC_DMsg_Reader(frontend))
			break;
...
int DEMOD_M23_IPC_Init(void) {
	...
	_DEMOD_M23_IPC_Handler_Init(frontend);
	_DEMOD_M23_IPC_DMsg_Handler_Init(frontend);
