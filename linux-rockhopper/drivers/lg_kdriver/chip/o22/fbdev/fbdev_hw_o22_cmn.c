...
----------------------------------------------------------------------------------------*/
int FBDEV_O22_CMN_MirrorCtrl(BOOLEAN v_mirror_en);
...
----------------------------------------------------------------------------------------*/
static void FBDEV_O22_CMN_InitFBCEnv(void);
#ifdef FBDEV_O22_USE_FBC_CTRL
static void FBDEV_O22_CMN_SetupFBC(int fb_dev_id, LX_FBDEV_FBC_FORMAT_T fbc_fmt);
#endif

static void FBDEV_O22_CMN_InitCropEnv(void);
static void FBDEV_O22_CMN_SetupSRScaler(BOOLEAN onoff, BOOLEAN bForce);

static void FBDEV_O22_CMN_SetupExternalRenderer(int fb_dev_id, struct fb_info* fbinfo);
static void FBDEV_O22_CMN_AllocExternalRendererFrameBuffer(void);
static void FBDEV_O22_CMN_DestroyExternalRendererFrameBuffer(void);
static void FBDEV_O22_CMN_SetOutputCropRegion(BOOLEAN crop_en, int img_width, int img_height, int xoffset, int yoffset,
        LX_RECT_T* crop_pos);

static void FBDEV_O22_CMN_ReInitHW(void);
static void FBDEV_O22_CMN_InitMirrorEnv(void);
static FBDEV_VSYNC_CLK_T FBDEV_O22_CMN_DetectVSyncClk(void);

static int  FBDEV_O22_CMN_CheckPQEDriverInitialization(void);
static void FBDEV_O22_CMN_NotifyPQEDriver(void);
static void FBDEV_O22_CMN_WaitPQEDriverEstreamerSync(int checker);

static void FBDEV_O22_CMN_BootLogoClearScheduler(struct work_struct* work);

static void FBDEV_O22_CMN_ShutdownOSDPath(void);
...
static struct workqueue_struct* fbdev_o22_wq;
static DECLARE_DELAYED_WORK(boot_logo_clear_work, FBDEV_O22_CMN_BootLogoClearScheduler);
...
static BOOLEAN osd_pqe_ipc_enabled = TRUE; /* see SICDTV-12981 */

static int  fbdev_afbc_version = FBDEV_HW_AFBC_VER_1_1; /* 0x10:AFBC1.0, 0x11:AFBC 1.1, see SICDTV-9405 */

module_param_named(fbdev_o22_boot_freeze_timeout, fbdev_boot_freeze_timeout, int, 0644);
module_param_named(fbdev_o22_resume_freeze_timeout, fbdev_resume_freeze_timeout, int, 0644);

module_param_named(fbdev_o22_pqefw_osd0_out_ctrl, fbdev_pqefw_osd0_out_ctrl, int, 0644);

module_param_named(fbdev_o22_enable_logo_clear, fbdev_enable_logo_clear, int, 0644);
module_param_named(fbdev_o22_boot_logo_clear_timeout, fbdev_boot_logo_clear_timeout, int, 0644);
module_param_named(fbdev_o22_resume_logo_clear_timeout, fbdev_resume_logo_clear_timeout, int, 0644);

module_param_named(fbdev_o22_afbc_version, fbdev_afbc_version, int, 0644);
...
module_param_named(fbdev_o22_use_shutdown_path_on_suspend, fbdev_use_shutdown_path_on_suspend, int, 0644);
...
module_param_named(fbdev_o22_afbc_estreamer, fbdev_afbc_estreamer, int, 0644);

// see http://clm.lge.com/issue/browse/SCDCR-6506
static int osd_afbc_line_wait_cnt = 0x0;
module_param_named(fbdev_o22_afbc_line_wait_cnt, osd_afbc_line_wait_cnt, int, 0644);
/*----------------------------------------------------------------------------------------
    Static Functions
----------------------------------------------------------------------------------------*/
 ...
 *
 */
int FBDEV_O22_CMN_GetHWXList(int fb_dev_id, int xlst[2])
{
    int afbc_xlst[2];

    return FBDEV_O22_CMN_GetHWXListEx(fb_dev_id, xlst, afbc_xlst);
}

int FBDEV_O22_CMN_GetHWXListEx(int fb_dev_id, int xlst[2], int afbc_xlst[2])
{
    int num_xlst = 0;
...
BOOLEAN  FBDEV_O22_CMN_IsUHDDispMode(void)
{
    FBDEV_O22_HW_CTX_T* hw_ctx = FBDEV_O22_GET_HW_CTX();
    return (hw_ctx->hw_disp_width > 2048) ? TRUE : FALSE;
}

#ifdef FBDEV_O22_USE_REVERSE_HW_CTRL
/** check if current OSD HW is set as the reverse mode
 *
 */
static BOOLEAN  FBDEV_O22_CMN_IsReverseMode(int fb_dev_id)
{
 ...
 *
 */
static void FBDEV_O22_CMN_EnableOSDHdrFlush(ULONG data)
{
    int i;

    FBDEV_NOTI("<!> enable osd_hdr_flush & exit OSD freeze stage. afbc=%d\n",
               FBDEV_O22_CMN_GetHWStatus(FBDEV_O22_HW_STATUS_AFBC, LX_FBDEV_ID_OSD0));
 ...
 *
 */
void FBDEV_O22_CMN_SetBootFreezeTimeout(int val)
{
...
static void FBDEV_O22_CMN_BootLogoClearScheduler(struct work_struct* work)
{
    ...
    FBDEV_O22_CMN_MigrateBootLogo(BOOT_LOGO_MODE_IMAGE_CLEAR);
}

/** move boot logo framebuffer to safe region (prototype implementation)
 *
 */
void        FBDEV_O22_CMN_MigrateBootLogo(FBDEV_O22_BOOT_LOGO_MODE_T mode)
{
#define OSD_CTRL_BASE(layer)    ((UINT32)(O22_CCO_BASE + 0x200 + (0x100*(layer))))
    ...
    rc = OS_RdReg(O22_CCO_BASE + 0x238, &fbmem_addr_src);
    FBDEV_CHECK_CODE(rc != RET_OK, goto func_exit, "reg op fail\n");
    FBDEV_WARN("logo fbmem (old) = 0x%08x\n", fbmem_addr_src);

    afbc_mode = (FBDEV_O22_CMN_GetHWStatus(FBDEV_O22_HW_STATUS_AFBC, LX_FBDEV_ID_OSD0)) ? 1 : 0;
    ...
    if (afbc_mode)
    {
        rc = OS_WrReg(O22_CCO_AFBC0_BASE + 0x004, fbmem_addr_dst);
        FBDEV_CHECK_CODE(rc != RET_OK, goto func_exit, "reg op fail\n");
        ...
        rc = OS_WrReg(O22_CCO_AFBC0_BASE + 0x020, fbmem_addr_dst);
        FBDEV_CHECK_CODE(rc != RET_OK, goto func_exit, "reg op fail\n");
    ...
    for (i = 0; i < FBDEV_O22_MAX_OSD_BLK_NUM; i++)
    {
     ...
     */
    OSDX_O22_RdFL(osdx_ctrl[0], osd_ctrl_main);
    OSDX_O22_Wr01(osdx_ctrl[0], osd_ctrl_main, osd_hdr_ready_flag_en, 1);
    OSDX_O22_Wr01(osdx_ctrl[0], osd_ctrl_main, osd_hdr_ready_flag, 1);
    OSDX_O22_WrFL(osdx_ctrl[0], osd_ctrl_main);
 ...
 *
 */
void FBDEV_O22_CMN_RunBootLogoTimer(UINT32 ms, BOOLEAN bForce)
{
    ...
    if (b_timer_req || bForce)
    {
        ...
        if (ms > 0)
        {
            ...
            OS_StartTimer(&osd_en_timer, FBDEV_O22_CMN_EnableOSDHdrFlush, OS_TIMER_TIMEOUT, ms, 0x0);
        }
        else
        {
            FBDEV_O22_CMN_EnableOSDHdrFlush(0x0);
        }
    }
...
static FBDEV_VSYNC_CLK_T FBDEV_O22_CMN_DetectVSyncClk(void)
{
    ...
    CTOP_CTRL_O22Ax_RdFL(SCRG_CTRL_TCON, crg_tcon04);
    val = CTOP_CTRL_O22Ax_RdFd(SCRG_CTRL_TCON, crg_tcon04, reg_pix2_clk_sel);
...
#else
        pqe2osd_status.bFwEstreamerOn = checker;
...
int FBDEV_O22_CMN_InitHW(void)
{
    ...
    FBDEV_HW_DEBUG("(Re)Init OSD(O22) HW.. disp_size %dx%d\n", disp_width, disp_height);

    /* initialize S.OSD & B.OSD mixer disp size */
    for (i = 0; i < sosd_num; i++)
    {
        OSDX_O22_RdFL(osdx_sep_osd_mixer[i], ctrl_sosd_mixer_disp_size);
        OSDX_O22_Wr02(osdx_sep_osd_mixer[i], ctrl_sosd_mixer_disp_size, reg_sosd_disp_size_w, hw_ctx->sosd_disp.out_width,
                      reg_sosd_disp_size_h, hw_ctx->sosd_disp.out_height);
        OSDX_O22_WrFL(osdx_sep_osd_mixer[i], ctrl_sosd_mixer_disp_size);
    }

    for (i = 0; i < bosd_num; i++)
    {
        OSDX_O22_RdFL(osdx_blend_osd_mixer[i], ctrl_bosd_mixer_disp_size);
        OSDX_O22_Wr02(osdx_blend_osd_mixer[i], ctrl_bosd_mixer_disp_size,   reg_bosd_disp_size_w, hw_ctx->bosd_disp.out_width,
                      reg_bosd_disp_size_h, hw_ctx->bosd_disp.out_height);
        OSDX_O22_WrFL(osdx_blend_osd_mixer[i], ctrl_bosd_mixer_disp_size);
    }

    /* setup all OSD common disp size */
    for (i = 0; i < FBDEV_O22_MAX_OSD_DISP_NUM /*8*/; i++)
    {
        OSD_O22_RdFL(osdx_common_disp_size[i]);
        OSD_O22_Wr02(osdx_common_disp_size[i],  reg_common_disp_size_w, disp_width,
                     reg_common_disp_size_h, disp_height);
        OSD_O22_WrFL(osdx_common_disp_size[i]);
    }


    /* O22 has the fixed mux value. DO NOT change */
    {
        UINT32 mixer_mux_value = 0x76543210;
        FBDEV_NOTI("(Re)Init OSD(O22) mixer mux order = 0x%08x\n", mixer_mux_value);

        OSDX_O22_Wr(osdx_sep_osd_mixer[0],      ctrl_sosd_mixer_mux1, mixer_mux_value);
        OSDX_O22_WrFL(osdx_sep_osd_mixer[0],    ctrl_sosd_mixer_mux1);

        OSDX_O22_Wr(osdx_blend_osd_mixer[0],    ctrl_bosd_mixer_mux1, mixer_mux_value);
        OSDX_O22_WrFL(osdx_blend_osd_mixer[0],  ctrl_bosd_mixer_mux1);

        OSDX_O22_Wr(osdx_blend_osd_mixer[1],    ctrl_bosd_mixer_mux1, mixer_mux_value);
        OSDX_O22_WrFL(osdx_blend_osd_mixer[1],  ctrl_bosd_mixer_mux1);
    }
     ...
     *
     * raxis.lim (2016/03/28) set bosd_pulse as 0x3000 (used at O22 for the first time)
     *
     */
    OSD_TOP_O22_RdFL(ctrl_sosd_pulse);
    OSD_TOP_O22_Wr01(ctrl_sosd_pulse, reg_disp_pulse_timer, hw_ctx->sosd_disp.pulse_timer_dly);
    OSD_TOP_O22_WrFL(ctrl_sosd_pulse);

    OSD_TOP_O22_RdFL(ctrl_bosd_pulse);
    OSD_TOP_O22_Wr01(ctrl_bosd_pulse, reg_disp_pulse_timer, hw_ctx->bosd_disp.pulse_timer_dly);
    OSD_TOP_O22_WrFL(ctrl_bosd_pulse);
     ...
     *
     * (2020/07/14)
     * when EXT_FRC(F22) is connected, OSD_SR is handled by F22 not O22
     */
    if (hw_ctx->b_support_4kosd || hw_ctx->b_support_ext_frc || !FBDEV_O22_CMN_IsUHDDispMode())
    {
        osd_shp_en = FALSE;
    }

    FBDEV_NOTI("initial OSD_SR=%d (4k_osd=%d, ext_frc=%d, uhd_disp=%d)\n",
               osd_shp_en, hw_ctx->b_support_4kosd, hw_ctx->b_support_ext_frc, FBDEV_O22_CMN_IsUHDDispMode());
    FBDEV_O22_CMN_SetupSRScaler(osd_shp_en, TRUE);

    FBDEV_O22_CMN_InitMirrorEnv();
    FBDEV_O22_CMN_InitCropEnv();
    FBDEV_O22_CMN_InitFBCEnv();
 ...
 *  @param layerId
 */
int FBDEV_O22_CMN_InitOSDLayer(int fb_dev_id)
{
    int i;
    int xidx;
    int xnum, xlst[2];
    ...
    FBDEV_O22_HW_CTX_T* hw_ctx = FBDEV_O22_GET_HW_CTX();
    FBDEV_O22_MIRROR_CTRL_T* mirror_ctrl = FBDEV_O22_GET_MIRROR_CTRL(hw_ctx);
    ...
    xnum = FBDEV_O22_CMN_GetHWXList(fb_dev_id, xlst);
    ...
    v_mirror_en = (mirror_ctrl->status & FBDEV_O22_V_MIRROR_ON) ? 0x1 : 0x0;
...
#ifdef FBDEV_O22_USE_HDR_RDY_REG
    hdr_rdy_flag = 0x1;
#endif
    ...
    for (i = 0; i < xnum; i++)
    {
        ...
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_main);
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_pdec);
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_swap);
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_base_addr);
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_sync0);
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_sync1);
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_scaler);
        ...
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_main,   osd_en,         osd_en);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_sync0,  osd_sync_enable, osd_en);
#endif
        ...
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_sync0, osd_sync_hdouble_en, sync_hdouble_en);

        /* use osd_hdr_flag to avoid the possible OSD mis-configuration when OSD header is set across vsync */
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_main, osd_hdr_ready_flag_en, hdr_rdy_flag);

        /* normally osd_hdr_src_sel is fixed to "read from hw register" not "DDR contains osd_hdr data" */
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_main, osd_hdr_src_sel,      hdr_src_sel);

        /* configure mirror mode every layer initialization */
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_main, osd_v_reverse_en,     v_mirror_en);
        ...
        {
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_sync0, osd_sync_v_reverse, v_mirror_en);
        }
         ...
         */
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_base_addr,   osd_base_addr, osd_memhdr_addr);

#if 0 /* KEEP HW DEFAULT */
        /* set scaler sample mode to the same behavior as that of GFX */
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_scaler, osd_sc_sampling_mode,    0x1);
#endif

        /* initialize cursor attribute (OSD4 only) */
        if (fb_dev_id == LX_FBDEV_ID_CSR0)
        {
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_scaler, reg_crsr_h_x2_en,    0x0);
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_scaler, reg_crsr_v_x2_en,    0x0);
        }

        /* MIF feeder cfg : "10 (0x2)" L to R (HW default value)
         * this value has ben fixed to "10" inside OSD HW since older chip.
         * In O22/O22, this value can be configurable by SW But I keep this value as 0x2 forever
         *
         * HW default value = 0x2
         */
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_main,   osd_manual_dlr,     0x2);
         ...
         *
         */
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_pdec,   osd_ch_alpha,           0x0);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_pdec,   osd_ch_red,             0x1);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_pdec,   osd_ch_green,           0x2);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_pdec,   osd_ch_blue,            0x3);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_pdec,   osd_round,              0x0);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_pdec,   osd_pure_ck,            0x0);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_pdec,   osd_lfsr_seed,          0x0);

        OSDX_O22_Wr(osdx_ctrl[xidx], osd_ctrl_swap,   0x00000333);

#ifdef FBDEV_O22_USE_EXPERIMENTAL_CONFIG
        ...
        {
            UINT32 buf_mode_en = (FBDEV_O22_CMN_IsUHDDispMode()) ? 0x0 : 0x1;
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_scaler, reg_osd_buf_mode_en_n, buf_mode_en);
        }

#endif


        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_main);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_pdec);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_swap);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_base_addr);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_scaler);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_sync0);
        ...
        if (i == 0x0)
        {
            OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_path);
            ctrl_direc = (hw_ctx->hw_conn_type & LX_FBDEV_CONN_OSD_VIDEO_BLEND) ? 0x0 : 0x1;
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_path, osd_ctrl_direc, ctrl_direc);
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_path, osd_ctrl_eo2s,  ctrl_direc);
             ...
             */
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_path, reg_ctrl_path_width,      sync_disp_width);
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_path, osd_ctrl_bosd_pos,        0x0);   /* fixed to 0x0(BOSD1) */
            OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_path);

            OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_sync1);
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_sync1, osd_sync_disp_w, sync_disp_width);
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_sync1, osd_sync_disp_h, disp_height);
            OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_sync1);
        }

        /* setup 4K osd configuration. OSD_IMAGE0_1 only */
        if (hw_ctx->b_support_4kosd && fb_dev_id == LX_FBDEV_ID_OSD0 && i == 0x1 /* second posd */)
        {
            OSDX_O22_RdFL(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_0);
            OSDX_O22_RdFL(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_1);
            OSDX_O22_RdFL(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_2);
            OSDX_O22_RdFL(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_3);

            OSDX_O22_Wr01(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_0, reg_4k_mode_en, 1);
            OSDX_O22_Wr01(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_2, reg_sc_i_w, hw_ctx->hw_fb0_width);
            OSDX_O22_Wr01(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_2, reg_sc_i_h, hw_ctx->hw_fb0_height);
            OSDX_O22_Wr01(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_3, reg_sc_o_w, disp_width);
            OSDX_O22_Wr01(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_3, reg_sc_o_h, disp_height);

            OSDX_O22_Wr01(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_1, reg_4k_xpos, 0);
            OSDX_O22_Wr01(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_1, reg_4k_ypos, 0);

            OSDX_O22_WrFL(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_0);
            OSDX_O22_WrFL(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_1);
            OSDX_O22_WrFL(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_2);
            OSDX_O22_WrFL(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_3);
        }
    }
...
static void FBDEV_O22_CMN_SafeSyncEnable(void)
{
    FBDEV_O22_HW_CTX_T* hw_ctx = FBDEV_O22_GET_HW_CTX();
    ...
    */
    OSD_TOP_O22_RdFL(pic_init);
    OSD_TOP_O22_Wr01(pic_init, g0_auto_init, 0x1);
    OSD_TOP_O22_Wr01(pic_init, g1_auto_init, 0x1);
    OSD_TOP_O22_Wr01(pic_init, g4_auto_init, 0x1);
    OSD_TOP_O22_WrFL(pic_init);
}
...
static void FBDEV_O22_CMN_ReInitHW(void)
{
    ...
    /* O22 OSD supports only OSD0, OSD1 & OSD3 */
    FBDEV_O22_CMN_InitOSDLayer(LX_FBDEV_ID_OSD0);
    FBDEV_O22_CMN_InitOSDLayer(LX_FBDEV_ID_OSD1);
    FBDEV_O22_CMN_InitOSDLayer(LX_FBDEV_ID_OSD3);

    /* refer to SICDTV-4209 if you have troube to enable OSD during I-BOOT */
    if (hw_ctx->hw_wa_flag & FBDEV_O22_HW_WA_OSD0_SYNC_ASYNC_WR_ERROR)
    {
        FBDEV_O22_CMN_SafeSyncEnable();
    }
}

int FBDEV_O22_CMN_ExecHWCommand(FBDEV_HW_CMD_T cmd, FBDEV_HW_CMD_DATA_T* data)
{
    ...
    switch (cmd)
    {
        ...
        {
            fbdev_o22_wq = alloc_workqueue("fbdev_o22_events", 0, 0);

            if (fbdev_enable_logo_clear)
            {
                FBDEV_NOTI("start logo clear work after %dms\n", fbdev_boot_logo_clear_timeout);
                queue_delayed_work(fbdev_o22_wq, &boot_logo_clear_work, msecs_to_jiffies(fbdev_boot_logo_clear_timeout));
            }
        }
        break;
        ...
        case FBDEV_HW_CMD_IO_POSD_OPEN:
        {
            ...
            FBDEV_O22_CMN_RunBootLogoTimer(fbdev_boot_freeze_timeout, FALSE);

            FBDEV_O22_CMN_SetupExternalRenderer(fbdev_id, fbinfo);
        ...
        case FBDEV_HW_CMD_RD_VSYNC_CLK:
        {
             ...
             * remember that I can call FBDEV_O22_CMN_DetectVSyncClk() only when B.OSD is active.
             */
            if (hw_ctx->hw_conn_type & LX_FBDEV_CONN_OSD_VIDEO_BLEND)
            {
                data->gen[0] = FBDEV_O22_CMN_DetectVSyncClk();
            }
            else /* S.OSD */
            {
        ...
        case FBDEV_HW_CMD_WR_PM_PREPARE_FREEZE:
        {
            ...
            hw_ctx->hw_wa_flag &= ~FBDEV_O22_HW_WA_PQEFW_OSD0_OUT_CTRL;
            ret = RET_OK;
        }
        break;
        ...
        case FBDEV_HW_CMD_WR_PM_PREPARE_SUSPEND:
        {
            ...
            {
                FBDEV_O22_CMN_ShutdownOSDPath();
            }

            /* raxis.lim (2017/09/18) -- disable PQEFW_OSD0_OUT_CTRL to re-enable at FBDEV_O22_CMN_EnableOSDHdrFlush() during RESUME */
            hw_ctx->hw_wa_flag &= ~FBDEV_O22_HW_WA_PQEFW_OSD0_OUT_CTRL;
            ret = RET_OK;
        }
        break;
        ...
        case FBDEV_HW_CMD_WR_PM_PREPARE_RESTORE:
        {
            ...
            fbdev_logo_migration_required = TRUE;
            FBDEV_O22_CMN_MigrateBootLogo(BOOT_LOGO_MODE_IMAGE_KEEP);

            if (fbdev_enable_logo_clear)
            {
                FBDEV_NOTI("start logo clear work after %dms (resume mode)\n", fbdev_resume_logo_clear_timeout);
                queue_delayed_work(fbdev_o22_wq, &boot_logo_clear_work, msecs_to_jiffies(fbdev_resume_logo_clear_timeout));
            }
            ...
            FBDEV_NOTI("fast enable OSD vsync\n");
            FBDEV_SetInterruptEnable(MIXER_INTR, 1);

            /* raxis.lim (2017/09/18) -- DO NOT set any PQEFW_OSD0_OUT_CTRL attribute. it will be enabled at FBDEV_O22_CMN_EnableOSDHdrFlush() */
        ...
        case FBDEV_HW_CMD_WR_PM_PREPARE_RESUME:
        {
            ...
            hw_ctx->hw_wa_flag |= FBDEV_O22_HW_WA_OSD0_SYNC_ASYNC_WR_ERROR;
        ...
        case FBDEV_HW_CMD_WR_USER_INITCALL_PREPARE:
        {
            ...
            if (!hw_ctx->user_initcall_ctrl.ready)
            {
                ...
                FBDEV_O22_CMN_ReInitHW();
            }
        }
        break;

        // USER_INITCALL_COMPLETE 는 모든 OSD 설정의 완료를 의미하므로, 반드시 PQE FW 쪽으로 제어권을 넘겨야 한다.
        // 즉 FBDEV_O22_HW_WA_PQEFW_OSD0_OUT_CTRL 플래그 설정이 가장 중요
        case FBDEV_HW_CMD_WR_USER_INITCALL_COMPLETE:
        {
            ...
            {
                UINT64 base_tick;
                FBDEV_O22_MIRROR_CTRL_T* mirror_ctrl = &hw_ctx->mirror_ctrl;
                ...
                hw_ctx->user_initcall_ctrl.status.osd_v_mirror = (mirror_ctrl->status == FBDEV_O22_V_MIRROR_ON) ? 1 : 0;
                hw_ctx->user_initcall_ctrl.status.osd_estreamer = 0;

                FBDEV_O22_CMN_NotifyPQEDriver();
                ...
                for (i = 0; i < 10; i++)
                {
                    ...
                    if (RET_OK == FBDEV_O22_CMN_CheckPQEDriverInitialization()) break;

                    msleep_interruptible(10);
                }

                FBDEV_O22_CMN_SetOSDEnable(fbdev_id, TRUE);
        ...
        case FBDEV_HW_CMD_WR_EXTERNAL_RENDERER_CONFIGURE_CROP_REGION:
        {
            ...
            FBDEV_O22_EXT_RENDER_CTRL_T* ext_render_ctrl = FBDEV_O22_GET_EXT_RENDER_CTRL(hw_ctx);
        ...
        // (2021/07/08) O228K, E608K 의 경우는 fbdev_drv.c 에서 호출이 되었으나, O22 8K 의 경우는
        // 일반 OSD 처럼 입력되어 처리되어야 하므로, fbdev_drv.c 에서 호출되지 않는다.
        // SW 구조를 유지하기 위하여 fbdev_hw_o22_cmn.c 에서 호출하도록 조치한다.
        // 수정된 IPC 에서는 crop_x,y,w,h 정보와 xoffset, enable 정보만 전달된다.
        //
        // (2023/11/15)
        // F22 출력시 제약 사항을 반영함.
        // (1) F22 전달되는 x 값에 대하여 8 의 배수 제약 사항 있음. 미준수시 출력 이미지가 사선으로 깨짐
        // (2) x + w <= panel.width 조건을 준수해야 함. 미준수시 원하는 위치에 출력 안될수 있음
        //     y + h <= panel.height 조건을 준수해야 함. 미준수시 원하는 위치에 출력 안됨수 있음
        //
        // see http://hlm.lge.com/qi/browse/QEVENTTF-5627
        //
        // (2023/11/29)
        // DRM 체제로 바뀐후 region 정보가 오지 않는다.
        // 따라서 crop 체크는 매 프레임마다 이루어져야 한다.
        // ext_render_ctrl->hwparam 의 값은 단지 최신 설정값으로 유지하고, crop 계산 결과 등은 hwparam_tx
        // 변수에 임시 저장하고, F22 전송하도록 한다.
        //
        // see http://hlm.lge.com/qi/browse/QEVENTTF-5868
        //
        case FBDEV_HW_CMD_WR_EXTERNAL_RENDERER_REQUEST_IMAGE:
        {
            ...
            FBDEV_O22_EXT_RENDER_HW_PARAM_T hwparam_tx;
            FBDEV_O22_EXT_RENDER_CTRL_T* ext_render_ctrl = FBDEV_O22_GET_EXT_RENDER_CTRL(hw_ctx);
            ...
            memcpy(&hwparam_tx, &ext_render_ctrl->hwparam, sizeof(FBDEV_O22_EXT_RENDER_HW_PARAM_T));

            if (hw_ctx->b_support_ext_render)
            {
                int max_cw = hw_ctx->hw_panel_width;
                int max_ch = hw_ctx->hw_panel_height;
                int cw;
                int ch;

                xoffset = ALIGN(xoffset, 8);

                cw = xoffset + hwparam_tx.crop_w;
                ch = yoffset + hwparam_tx.crop_h;

                if (cw > max_cw)
                {
                    hwparam_tx.crop_w -= (cw - max_cw);

                    FBDEV_HW_NOTI("++ external render crop.w modified %d => %d\n", ext_render_ctrl->hwparam.crop_w, hwparam_tx.crop_w);
                }

                if (ch > max_ch)
                {
                    hwparam_tx.crop_h -= (ch - max_ch);

                    FBDEV_HW_NOTI("++ external render crop.h modified %d => %d\n", ext_render_ctrl->hwparam.crop_h, hwparam_tx.crop_h);
                }
            }

            hwparam_tx.xoffset = xoffset;
            hwparam_tx.yoffset = yoffset;
...
#ifdef FBDEV_O22_USE_EXT_RENDERER_IPC

            if (hw_ctx->b_support_ext_render)
            {
                FBDEV_HW_NOTI("++ write external render regs 0x%08x=0x%08x, 0x%08x=0x%08x, 0x%08x=0x%08x\n",
                              FBDEV_OS0_EXT_RENDER_REG_BASE + 0x0, hwparam_tx.command,
                              FBDEV_OS0_EXT_RENDER_REG_BASE + 0x4, hwparam_tx.crop_xy,
                              FBDEV_OS0_EXT_RENDER_REG_BASE + 0x8, hwparam_tx.crop_wh);

                OS_WrReg(FBDEV_OS0_EXT_RENDER_REG_BASE + 0x4, hwparam_tx.crop_xy);
                OS_WrReg(FBDEV_OS0_EXT_RENDER_REG_BASE + 0x8, hwparam_tx.crop_wh);
                OS_WrReg(FBDEV_OS0_EXT_RENDER_REG_BASE + 0x0, hwparam_tx.command);
            }
            ...
            else
            {
                ...
                crop_rgn.x = hwparam_tx.crop_x;
                crop_rgn.y = hwparam_tx.crop_y;
                crop_rgn.w = hwparam_tx.crop_w;
                crop_rgn.h = hwparam_tx.crop_h;
                ...
                FBDEV_O22_CMN_SetOutputCropRegion(TRUE, hw_ctx->hw_fb1_width, hw_ctx->hw_fb1_height, xoffset, yoffset, &crop_rgn);
            }
        ...
        case FBDEV_HW_CMD_WR_PQE_PM_RUNTIME_ACTION:
        {
            ...
            if (pqe_pm_action == PQE_PM_DEVICE_SUSPEND_PREPARE)
            {
                ...
                // see FBDEV_O22_CMN_WaitPQEDriverEstreamerSync()
                osd_hw_pm_suspend_stage = 1;
                ...
                FBDEV_O22_CMN_SetInterruptEnable(MIXER_INTR, FALSE);
            ...
            else if (pqe_pm_action == PQE_PM_DEVICE_RESUME_COMPLETED)
            {
                ...
                osd_pqe_ipc_enabled = TRUE;

                FBDEV_O22_CMN_SetInterruptEnable(MIXER_INTR, TRUE);
            }
            else
            {
...
// enable interrupt
int FBDEV_O22_CMN_SetInterruptEnable(INTERRUPT_INFORM_T intr, int enable)
{
    FBDEV_O22_HW_CTX_T* hw_ctx = FBDEV_O22_GET_HW_CTX();
    ...
    switch (intr)
    {
        ...
        {
            int curr_status;
            OSD_O22_RdFL(osd_common_ctrl_irq);

            if (hw_ctx->hw_conn_type & LX_FBDEV_CONN_OSD_VIDEO_BLEND)
            {
                OSD_O22_Rd01(osd_common_ctrl_irq, reg_common_ctrl_irq_en_mixer_bos, curr_status);

                if (curr_status != enable)
                {
                    FBDEV_NOTI("INTR <%s> for B.OSD\n", enable ? "ON" : "OFF");
                    OSD_O22_Wr01(osd_common_ctrl_irq, reg_common_ctrl_irq_en_mixer_bos, enable);
                    OSD_O22_Wr01(osd_common_ctrl_irq, reg_common_ctrl_irq_en_mixer_sos, 0x0);
                }
            }
            else
            {
                OSD_O22_Rd01(osd_common_ctrl_irq, reg_common_ctrl_irq_en_mixer_sos, curr_status);

                if (curr_status != enable)
                {
                    FBDEV_NOTI("INTR <%s> for S.OSD\n", enable ? "ON" : "OFF");
                    OSD_O22_Wr01(osd_common_ctrl_irq, reg_common_ctrl_irq_en_mixer_bos, 0x0);
                    OSD_O22_Wr01(osd_common_ctrl_irq, reg_common_ctrl_irq_en_mixer_sos, enable);
                }
            }

            OSD_O22_WrFL(osd_common_ctrl_irq);
        }
        break;
    }
...
int FBDEV_O22_CMN_GetInterruptStatus(INTERRUPT_INFORM_T intr)
{
    UINT32  status = 0;
    ...
    {
        UINT32 bosd_mixer;
        OSD_O22_Rd01(osd_common_ctrl_irq, reg_common_ctrl_irq_clear_bosd, bosd_mixer);
    ...
    {
        UINT32 sosd_mixer;
        OSD_O22_Rd01(osd_common_ctrl_irq, reg_common_ctrl_irq_clear_sosd, sosd_mixer);
...
int FBDEV_O22_CMN_SetInterruptClear(INTERRUPT_INFORM_T intr)
{
    OSD_O22_RdFL(osd_common_ctrl_irq);

    /* clear both bosd, sosd mixer intr for the safe operation */
    OSD_O22_Wr01(osd_common_ctrl_irq, reg_common_ctrl_irq_clear_bosd, 0x1);
    OSD_O22_Wr01(osd_common_ctrl_irq, reg_common_ctrl_irq_clear_sosd, 0x1);
...
static void FBDEV_O22_CMN_SetupExternalRenderer(int fb_dev_id, struct fb_info* fbinfo)
{
    ...
    if (fb_dev_id == LX_FBDEV_ID_OSD1 && hw_ctx->b_support_ext_render)
    {
        ...
        // O22, E60 의 경우 post processing 을 위한 정보를 넘겨야 했으나,
        // O22 에서는 OSD kdriver 에서 진행한다.
...
static void FBDEV_O22_CMN_AllocExternalRendererFrameBuffer(void)
{
    ...
    {
        ext_render_ctrl->fbmem_phys[0] = (UINT32)hma_alloc("debug1", fbmem_sz, PAGE_SIZE);
        FBDEV_CHECK_CODE(0x0 == ext_render_ctrl->fbmem_phys[0], return, "can't alloc external render fbmem[0]\n");
    }

    if (0x0 == ext_render_ctrl->fbmem_phys[1])
    {
        ext_render_ctrl->fbmem_phys[1] = (UINT32)hma_alloc("debug1", fbmem_sz, PAGE_SIZE);
        FBDEV_CHECK_CODE(0x0 == ext_render_ctrl->fbmem_phys[1], return, "can't alloc external render fbmem[1]\n");
    }
...
static void FBDEV_O22_CMN_DestroyExternalRendererFrameBuffer(void)
{
    ...
    {
        hma_free("debug1", ext_render_ctrl->fbmem_phys[0]);
        ext_render_ctrl->fbmem_phys[0] = 0x0;
    }

    if (ext_render_ctrl->fbmem_phys[1])
    {
        hma_free("debug1", ext_render_ctrl->fbmem_phys[1]);
        ext_render_ctrl->fbmem_phys[1] = 0x0;
    }
}

// O22 OSD 에서 제공하는 output crop 기능을 E-Streamer 에서 사용하도록 한다.
// 통상적인 잘라내기 기능 외에 출력 지점도 지정할 수 있다.
// (노트) M23 에서 구현된 것을 O22 에서 차용하여 구현한다.
static void FBDEV_O22_CMN_SetOutputCropRegion(BOOLEAN crop_en, int img_width, int img_height, int xoffset,
        int yoffset, LX_RECT_T* crop_rgn)
{
    ...
    for (i = 0; i < xnum && VALID_XIDX(xlst[i]); i++)
    {
        ...
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_post_crop_ctrl_0);
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_post_crop_ctrl_1);
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_post_crop_ctrl_2);
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_post_crop_ctrl_3);

        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_post_crop_ctrl_0, reg_crop_en, crop_en);

        if (fb_dev_id == LX_FBDEV_ID_OSD1)
        {
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_post_crop_ctrl_0, reg_in_pic_width, img_width);
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_post_crop_ctrl_0, reg_in_pic_height, img_height);
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_post_crop_ctrl_1, hoffset, crop_rgn->x);
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_post_crop_ctrl_1, hactive, crop_rgn->w);
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_post_crop_ctrl_2, voffset, crop_rgn->y);
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_post_crop_ctrl_2, vactive, crop_rgn->h);
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_post_crop_ctrl_3, crop_mixer_xpos, xoffset);
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_post_crop_ctrl_3, crop_mixer_ypos, yoffset);
        }

        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_post_crop_ctrl_0);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_post_crop_ctrl_1);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_post_crop_ctrl_2);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_post_crop_ctrl_3);
    }
}
...
//
static void FBDEV_O22_CMN_ReformatAFBCExternalRenderer(struct scanvas* canvas, FBDEV_O22_OSD_HDR_T* osd_hdr)
{
    int fbmem_buf_idx;
    UINT32 fbmem_phys;
    ...
    ctx = afbc_sw_init_context((fbdev_afbc_version == FBDEV_HW_AFBC_VER_1_1) ? AFBC_SW_VER_1_1 : FBDEV_HW_AFBC_VER_1_0);
...
//
static void FBDEV_O22_CMN_ReformatARGBExternalRenderer(struct scanvas* canvas, FBDEV_O22_OSD_HDR_T* osd_hdr)
{
    FBDEV_O22_HW_CTX_T* hw_ctx = FBDEV_O22_GET_HW_CTX();
    FBDEV_O22_EXT_RENDER_CTRL_T* ext_render_ctrl = FBDEV_O22_GET_EXT_RENDER_CTRL(hw_ctx);
...
#define get_stride(w) ((w)*FBDEV_O22_EXT_RENDER_BYTE_PER_PIXEL)
...
static void FBDEV_O22_CMN_InitMirrorEnv(void)
{
    ...
    if (mirror_ctrl->status == FBDEV_O22_MIRROR_NONE)
    {
        UINT32 hw_val;

#ifdef V_MIRROR_TEST
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_main);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_main, osd_v_reverse_en, 0x1);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_main);
#endif
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_main);
        OSDX_O22_Rd01(osdx_ctrl[xidx], osd_ctrl_main, osd_v_reverse_en, hw_val);

        mirror_ctrl->status = (hw_val) ? FBDEV_O22_V_MIRROR_ON : 0x0;
...
static void FBDEV_O22_CMN_SetupSRScaler(BOOLEAN onoff, BOOLEAN bForce)
{
    ...
    FBDEV_O22_HW_CTX_T* hw_ctx = FBDEV_O22_GET_HW_CTX();
    FBDEV_O22_SR_CTRL_T* sr_ctrl = FBDEV_O22_GET_SR_CTRL(hw_ctx, fb_dev_id);
    ...
    SHP0_CTRL_Wr01(shp_core_ctrl_00, reg_top_bypass, 0x0);
    ...
    for (i = 0; i < xnum; i++)
    {
        ...
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_bitmap_ctrl0);
        OSDX_O22_RdFL(hdr_ui, osd_hdrui_yc2rgb_ctrl00);

        /* check HW status for debug. hw_onoff is not used yet */
        OSDX_O22_Rd01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_osd_shp_detour_en, hw_onoff);
        hw_onoff ^= 1;
        FBDEV_NOTI("OSD SR(HW) = %d\n", hw_onoff);

        sr_ctrl->status   = (onoff) ? FBDEV_O22_SR_STATUS_ON : FBDEV_O22_SR_STATUS_OFF;

#ifdef FBDEV_O22_USE_SR_CTRL
        // configure OSD SR
        //
        ...
        // enable O22 random dither to improve OSD quality when gradation appears
        // OSD0_BITMAP_CTRL0 -> reg_rgb2yc_csc_en_de_clk [31:31] = 0
        // OSD0_BITMAP_CTRL0 -> reg_yc2rgb_csc_en [22:22] = 0
        //
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_rgb2yc_csc_en_de_clk, 0x0);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_yc2rgb_csc_en, 0x0);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_rgb2yc_csc_p_en_oclk, (onoff) ? 0x1 : 0x0);
        OSDX_O22_Wr01(hdr_ui, osd_hdrui_yc2rgb_ctrl00, reg_yuv2rgb_en, (onoff) ? 0x1 : 0x0);

        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_osd_shp_detour_en, (onoff) ? 0x0 : 0x1);
        ...
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_rgb2yc_csc_en_de_clk, 0x0);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_yc2rgb_csc_en, 0x0);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_rgb2yc_csc_p_en_oclk, 0x0);
        OSDX_O22_Wr01(hdr_ui, osd_hdrui_yc2rgb_ctrl00, reg_yuv2rgb_en, 0x0);

        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_osd_shp_detour_en, 0x1);

        FBDEV_NOTI("OSD SR disabled (%s)\n", "off");
#endif
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_bitmap_ctrl0);
        OSDX_O22_WrFL(hdr_ui, osd_hdrui_yc2rgb_ctrl00);
    }
     ...
     */
    sr_ctrl->ti_gain = FBDEV_O22_CMN_GetHWStatus(FBDEV_O22_HW_STATUS_SR_GAIN, 0x0);
 ...
 *  The real HW register will be set inside FBDEV_O22_CMN_WriteOSDHeader()
...
static void FBDEV_O22_CMN_InitFBCEnv(void)
{
    ...
    {
        FBDEV_O22_HW_CTX_T* hw_ctx = FBDEV_O22_GET_HW_CTX();
        FBDEV_O22_FBC_CTRL_T* fbc_ctrl = FBDEV_O22_GET_FBC_CTRL(hw_ctx, i);
        int hw_afbc_status = 0;

        fbc_ctrl->fbc_fmt = LX_FBDEV_FBC_FORMAT_RAW;
        fbc_ctrl->status  = FBDEV_O22_FBC_STATUS_NONE;
        FBDEV_O22_FBC_DEBUG("fb(%d) initialize fbc env\n", LX_FBDEV_ID_OSD0);
         ...
         *
         */
        hw_afbc_status = FBDEV_O22_CMN_GetHWStatus(FBDEV_O22_HW_STATUS_AFBC, i);
        ...
        if (hw_afbc_status)
        {
            FBDEV_O22_FBC_DEBUG("fb(%d) recover FBC HW config\n", i);
            FBDEV_O22_CMN_SetupFBC(i, LX_FBDEV_FBC_FORMAT_AFBC);
        }
    }
...
static void FBDEV_O22_CMN_SetupFBC(int fb_dev_id, LX_FBDEV_FBC_FORMAT_T fbc_fmt)
{
    ...
    FBDEV_O22_HW_CTX_T* hw_ctx = FBDEV_O22_GET_HW_CTX();
    FBDEV_O22_FBC_CTRL_T* fbc_ctrl = FBDEV_O22_GET_FBC_CTRL(hw_ctx, fb_dev_id);
    ...
    {
#ifdef FBDEV_O22_SW_RECOVERY_AFBC_FLAG_CORRUPTION
        int hw_afbc_status = FBDEV_O22_CMN_GetHWStatus(FBDEV_O22_HW_STATUS_AFBC, fb_dev_id);
...
afbc_config:
#endif
    xnum = FBDEV_O22_CMN_GetHWXListEx(fb_dev_id, xlst, afbc_xlst);
    FBDEV_CHECK_CODE(INVALID_XNUM(xnum), return, "invalid fb(%d)\n", fb_dev_id);

    FBDEV_NOTI("fb(%d) setup afbc_path=%d\n", fb_dev_id, fbc_fmt);

    OSD_TOP_O22_RdFL(ctrl_dpath);
    OSD_TOP_O22_RdFL(ctrl_auto_init_afbc);
...
#ifdef FBDEV_O22_USE_AFBC_READ_BW_SAVING
        UINT32 enable = FBDEV_O22_CMN_GetOSDEnable(fb_dev_id);
#else
        UINT32 enable = 1;
#endif

        FBDEV_O22_FBC_DEBUG("setup fbc_fmt = 0x1 (enable AFBC mode)\n");

        if (afbc_xlst[0] == 0 || afbc_xlst[1] == 0)
        {
            OSD_TOP_O22_Wr01(ctrl_dpath, osd0_afbc_en, 0x1);
            OSD_TOP_O22_Wr01(ctrl_auto_init_afbc, reg_afbc0_init_en,        enable);
            OSD_TOP_O22_Wr01(ctrl_auto_init_afbc, reg_afbc0_auto_init_en,   enable);
            OSD_TOP_O22_Wr01(ctrl_auto_init_afbc, idx_afbc0_pic_init_src,   0x0); /* g0_pic_init. 이건 고정 ? */

            OSD_TOP_O22_Wr01(ctrl_auto_init_afbc, reg_afbc0_auto_init_src,  afbc_auto_init_src_val);

            FBDEV_O22_FBC_DEBUG("fb(%d) afbc enabled. xlst=%d,%d view=%d\n", fb_dev_id, afbc_xlst[0], afbc_xlst[1], enable);
        }

        if (afbc_xlst[0] == 1 || afbc_xlst[1] == 1)
        {
            OSD_TOP_O22_Wr01(ctrl_dpath, osd1_afbc_en, 0x1);
            OSD_TOP_O22_Wr01(ctrl_auto_init_afbc, reg_afbc1_init_en,        enable);
            OSD_TOP_O22_Wr01(ctrl_auto_init_afbc, reg_afbc1_auto_init_en,   enable);
            OSD_TOP_O22_Wr01(ctrl_auto_init_afbc, idx_afbc1_pic_init_src,   0x1); /* g1_pic_init. 이거 선택 가능 ? */

            OSD_TOP_O22_Wr01(ctrl_auto_init_afbc, reg_afbc1_auto_init_src,  afbc_auto_init_src_val);
    ...
    {
        FBDEV_O22_FBC_DEBUG("setup fbc_fmt = 0x0 (disable AFBC mode)\n");

        if (afbc_xlst[0] == 0 || afbc_xlst[1] == 0)
        {
            OSD_TOP_O22_Wr01(ctrl_dpath, osd0_afbc_en, 0x0);
            OSD_TOP_O22_Wr01(ctrl_auto_init_afbc, reg_afbc0_init_en,        0x0);
            OSD_TOP_O22_Wr01(ctrl_auto_init_afbc, reg_afbc0_auto_init_en,   0x0);

            FBDEV_O22_FBC_DEBUG("fb(%d) afbc disabled. xlst=%d,%d\n", fb_dev_id, afbc_xlst[0], afbc_xlst[1]);
        }

        if (afbc_xlst[0] == 1 || afbc_xlst[1] == 1)
        {
            OSD_TOP_O22_Wr01(ctrl_dpath, osd1_afbc_en, 0x0);
            OSD_TOP_O22_Wr01(ctrl_auto_init_afbc, reg_afbc1_init_en,        0x0);
            OSD_TOP_O22_Wr01(ctrl_auto_init_afbc, reg_afbc1_auto_init_en,   0x0);
    ...
    for (i = 0; i < xnum && VALID_XIDX(xlst[i]); i++)
    {
        ...
        {
            /* enable BITMAP mode */
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_main,   osd_bitmap_only_mode_en, 0x1);

            fbc_ctrl->status |=  FBDEV_O22_FBC_STATUS_PATH_CHANGE;;
        }
        else
        {
            /* disable BITMAP mode (default in raw mode) */
            OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_main,   osd_bitmap_only_mode_en, 0x0);

            fbc_ctrl->status &= ~FBDEV_O22_FBC_STATUS_HDR_SETUP;
            fbc_ctrl->status |=  FBDEV_O22_FBC_STATUS_PATH_CHANGE;
        }
    ...
    /* flush top registers */
    OSD_TOP_O22_WrFL(ctrl_dpath);
    OSD_TOP_O22_WrFL(ctrl_auto_init_afbc);
    ...
    FBDEV_O22_FBC_DEBUG("fb(%d) fbc_mode=0x%x setup\n", fb_dev_id, fbc_fmt);
...
//
static void FBDEV_O22_CMN_WriteFBCHdrRegs(int fb_dev_id, FBDEV_O22_OSD_HDR_T osd_hdr[2],
        FBDEV_O22_OSD_EXHDR_T* osd_exhdr)
{
    UINT32 afbc_mode;
    UINT32 img_addr;
    int img_x;
    ...
    FBDEV_O22_HW_CTX_T* hw_ctx = FBDEV_O22_GET_HW_CTX();
    FBDEV_O22_FBC_CTRL_T* fbc_ctrl = &hw_ctx->fbc_ctrl[fb_dev_id];
    FBDEV_O22_MIRROR_CTRL_T* mirror_ctrl = &hw_ctx->mirror_ctrl;
    ...
    afbc_mode = (fbdev_afbc_version == FBDEV_HW_AFBC_VER_1_1) ? 0x0 : 0x1;
    ...
    img_bpp = FBDEV_O22_BITS_2_BYTES(osd_exhdr->bits_per_pixel);
    ...
    for (i = 0; i < xnum; i++)
    {
        ...
        FBDEV_O22_FBC_DEBUG("fb(%d) afbc.xidx(%d) paddr=0x%08x pos=%d,%d dim=%dx%d, stride=%d\n",
        ...
        AFBC_CTRL_Rd01(afbc_xidx, ctrl_afbc_4, img_width,  img_width);
        AFBC_CTRL_Rd01(afbc_xidx, ctrl_afbc_4, line_wait_cnt, line_wait_cnt);
        ...
        if (img_width != new_img_width)
        {
            ...
            AFBC_CTRL_Wr01(afbc_xidx, ctrl_afbc_4, img_width,  new_img_width);
        ...
        if (!(fbc_ctrl->status & FBDEV_O22_FBC_STATUS_HDR_SETUP))
        {
             ...
             * 0xffff_8080 @ARGB (*)
             *
             * I 've used 0xffff_fffff since O22 AFBC integration
             */
#if 0
            ...
            AFBC_CTRL_Wr01(afbc_xidx, ctrl_afbc_0, flip_mode, (mirror_ctrl->status & FBDEV_O22_V_MIRROR_ON) ? 1 : 0);
...
static void FBDEV_O22_CMN_FlushFBCHdrRegs(int fb_dev_id)
{
    int i;
    int afbc_xidx;
    int xnum, xlst[2], afbc_xlst[2];
    ...
    {
        xnum = FBDEV_O22_CMN_GetHWXListEx(fb_dev_id, xlst, afbc_xlst);
        FBDEV_O22_FBC_DEBUG("fb(%d) xnum=%d, afbc_xlst=(%d,%d)\n", fb_dev_id, xnum, afbc_xlst[0], afbc_xlst[1]);
        ...
        for (i = 0; i < xnum; i++)
        {
            ...
            FBDEV_O22_FBC_DEBUG("fb(%d) afbc_xidx(%d) flush AFBC regs\n", fb_dev_id, afbc_xidx);
        }
    }
}
#endif
...
//
//
void    FBDEV_O22_CMN_WriteHdrRegs(int fb_dev_id, FBDEV_O22_OSD_HDR_T* osd_hdr, FBDEV_O22_OSD_EXHDR_T* osd_exhdr)
{
    int i;
    int xidx;
    int xnum, xlst[2];

    FBDEV_O22_OSD_HDR_T osd_hw_hdr[2];  // 4K OSD 출력을 위하여 최대 2개 layer 필요
    UINT32* hdr_regs;
    ...
    if (!wctx->status.b_osd_hdr_flush)
    {
        ...
        for (i = 0; i < xnum; i++)
        {
            ...
            OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_hdr6);
            disp_fbmem = OSDX_O22_Rd(osdx_ctrl[xidx], osd_ctrl_hdr6);
    ...
    for (i = 0; i < xnum; i++)
    {
        ...
        memcpy(&osd_hw_hdr[i], osd_hdr, sizeof(FBDEV_O22_OSD_HDR_T));

        // xnum = 2 (4K OSD 처리)
        //
        // fb(0) 은 3840x2160 통으로 이미지가 전달되어, L/R 을 처리한다.
        // fb(1) 은 1920x2160 으로 리포맷팅된 이미지가, L/R 동일하게 복사되어 출력된다.
        //
        // 이를 정리하면 4K 통이미지로 전달되는 경우, L/R 를 분리하여 각각의 osd_hw_hdr 를 만든다.
        // 그 외의 경우는 이미 앞에서 조작된 header 정보를 그대로 이용한다.
        //
        // (노트) 4K 통이미지로 구성된 AFBC 이미지에 대한 데이터 해석은 FBDEV_O22_CMN_WriteFBCHdrRegs 참조할것
        //
        if (xnum == 2 && osd_hdr->osd_hdr_w_mem > 1920)
        {
        ...
        FBDEV_O22_HDR_DEBUG("fb(%d) osd_hw_hdr in=%dx%d out=%dx%d\n",
                            fb_dev_id,
        ...
        OSDX_O22_Wr(osdx_ctrl[xidx], osd_ctrl_hdr0, hdr_regs[0]);
        OSDX_O22_Wr(osdx_ctrl[xidx], osd_ctrl_hdr1, hdr_regs[1]);
        OSDX_O22_Wr(osdx_ctrl[xidx], osd_ctrl_hdr2, hdr_regs[2]);
        OSDX_O22_Wr(osdx_ctrl[xidx], osd_ctrl_hdr3, hdr_regs[3]);
        OSDX_O22_Wr(osdx_ctrl[xidx], osd_ctrl_hdr4, hdr_regs[4]);
        OSDX_O22_Wr(osdx_ctrl[xidx], osd_ctrl_hdr5, hdr_regs[5]);
        OSDX_O22_Wr(osdx_ctrl[xidx], osd_ctrl_hdr6, hdr_regs[6]);

        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_hdr0);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_hdr1);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_hdr3);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_hdr4);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_hdr5);
        OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_hdr6);
        ...
        // EXT_FRC 가 붙었을 경우에는 orbit control 을 O22 에서 하지 않는다.
        // 향후에는 flag 를 좀 더 조정하자.
        //
        if (fb_dev_id == LX_FBDEV_ID_OSD0)
        {
            if (fbdev_pqefw_osd0_out_ctrl && (hw_ctx->hw_wa_flag & FBDEV_O22_HW_WA_PQEFW_OSD0_OUT_CTRL))
            {
                /* do nothing */
            }
            else
            {
                UINT32 w, h;
                OSDX_O22_Rd01(osdx_ctrl[xidx], osd_ctrl_hdr2, osd_hdr_w_out, w);
                OSDX_O22_Rd01(osdx_ctrl[xidx], osd_ctrl_hdr2, osd_hdr_h_out, h);
                FBDEV_HW_DEBUG("fb(%d) xidx(%d) flush OSD_CTRL_HDR2. %dx%d\n", fb_dev_id, xidx, w, h);
                OSDX_O22_WrFL(osdx_ctrl[xidx], osd_ctrl_hdr2);
            }
        }
        else
        {
    ...
    /* flush post header (AFBC,OSD_SHP etc ) */
    {
        ...
        if (fb_dev_id == LX_FBDEV_ID_OSD0 || fb_dev_id == LX_FBDEV_ID_OSD1)
        {
            ...
            FBDEV_O22_CMN_SetupFBC(fb_dev_id, hCanvas->fbc_fmt);

            if (hCanvas->fbc_fmt == LX_FBDEV_FBC_FORMAT_AFBC)
            {
                FBDEV_O22_CMN_WriteFBCHdrRegs(fb_dev_id, osd_hw_hdr, osd_exhdr);
            }

            FBDEV_O22_CMN_FlushFBCHdrRegs(fb_dev_id);
        }
        ...
        {
            xidx = xlst[i];
            g_osd_o22_reg->osdx_ctrl[xidx].osd_ctrl_main.osd_hdr_ready_flag = 0x1;
            FBDEV_O22_HDR_DEBUG("fb(%d) xidx(%d) flush osd_hdr regs/data\n", fb_dev_id, xidx);
        }

#ifdef FBDEV_O22_SUPPORT_4KOSD_UPSCALER
        ...
        if (fb_dev_id == LX_FBDEV_ID_OSD0 && xnum == 2)
        {
            xidx = xlst[1];
            OSDX_O22_RdFL(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_2);
            OSDX_O22_Wr01(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_2, reg_sc_i_w, osd_hdr->osd_hdr_w_mem);
            OSDX_O22_Wr01(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_2, reg_sc_i_h, osd_hdr->osd_hdr_h_mem);
            OSDX_O22_WrFL(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_2);
        }
 ...
 *
 */
int FBDEV_O22_CMN_WriteOSDHeader(struct scanvas* pCanvas)
{
    int                     fb_dev_id;
    UINT32                  bitmap_ptr;
    FBDEV_O22_OSD_HDR_T     osd_hdr;
    FBDEV_O22_OSD_EXHDR_T   osd_exhdr;
    ...
    memset(&osd_hdr,   0x0, sizeof(FBDEV_O22_OSD_HDR_T));
    memset(&osd_exhdr, 0x0, sizeof(FBDEV_O22_OSD_EXHDR_T));
     ...
     * Originally, osd_hdr_flush is checked at FBDEV_O22_CMN_WriteHdrRegs().
    ...
    switch (fb_dev_id)
    {
        case LX_FBDEV_ID_CSR0:
        {
...
#ifdef FBDEV_O22_USE_UNLIMITED_CURSOR_POSITION
            int csr_h_min_pxl = FBDEV_O22_BYTES_2_PIXEL(FBDEV_O22_GMAU_WORD_BYTES); /* 16 bytes -> 4 pixel */
#endif
            FBDEV_O22_HDR_DEBUG("cursor_pos req(%d,%d)\n", canvas.output_win.x, canvas.output_win.y);
            ...
            //
            csr_x_max = canvas.stride / FBDEV_O22_BITS_2_BYTES(canvas.bits_per_pixel);
            ...
            FBDEV_O22_HDR_DEBUG("fb(%d) pos %dx%d csr_sz %d,%d info( %dx%d h %d,%d ) max_x %d disp_x %d\n", fb_dev_id,
            ...
            if (csr_y_pos < 0)
            {
                ...
                if (csr_y_size < FBDEV_O22_CSR_MIN_HEIGHT)
                {
                    csr_y_size = FBDEV_O22_CSR_MIN_HEIGHT;
                    csr_y_off  = csr_y_size - FBDEV_O22_CSR_MIN_HEIGHT;
                }
            }
            ...
            // see the description for FBDEV_O22_CSR_MIN_WIDTH
            if (csr_x_pos + csr_x_size > csr_disp_w)
            {
                if (csr_x_pos > csr_disp_w - FBDEV_O22_CSR_MIN_WIDTH)
                {
                    csr_x_pos = csr_disp_w - FBDEV_O22_CSR_MIN_WIDTH;
                }
                ...
                if (hw_ctx->hw_wa_flag & FBDEV_O22_HW_WA_1X_CSR_DISP_ON_BLEND_PATH)
                {
                    if (FBDEV_O22_CMN_IsUHDDispMode())
                    {
                        csr_x_size *= 2;
            ...
            if (csr_y_pos > csr_disp_h - FBDEV_O22_CSR_MIN_HEIGHT)
            {
                csr_y_pos = csr_disp_h - FBDEV_O22_CSR_MIN_HEIGHT;
            }
            ...
            bitmap_ptr += (csr_y_off * canvas.stride) + FBDEV_O22_GMAU_ALIGNED_BYTES(csr_x_off * FBDEV_O22_BITS_2_BYTES(
                              canvas.bits_per_pixel));

            osd_hdr.osd_hdr_pixel_order = canvas.pixel_order_en;
            osd_hdr.osd_hdr_wpl         = FBDEV_O22_BYTES_2_OSD_WORDS(canvas.stride);

            FBDEV_O22_HDR_DEBUG("fb(%d) pos %dx%d csr_sz %d,%d <- req( %dx%d h %d,%d)\n", fb_dev_id,
                                csr_x_pos, csr_y_pos, csr_x_size, csr_y_size,
                                canvas.output_win.x, canvas.output_win.y, canvas.csr_hotspot.x, canvas.csr_hotspot.y);
            FBDEV_O22_HDR_DEBUG("fb(%d) cursor pre-processing done\n", fb_dev_id);
        }
        break;
        ...
        case LX_FBDEV_ID_OSD1:
        {
            ...
            if (hw_ctx->b_support_ext_render)
            {
                ...
                if (pCanvas->fbc_fmt == LX_FBDEV_FBC_FORMAT_AFBC)
                {
                    FBDEV_O22_CMN_ReformatAFBCExternalRenderer(&canvas, &osd_hdr);
                }
                else
                {
                    FBDEV_O22_CMN_ReformatARGBExternalRenderer(&canvas, &osd_hdr);
                }
                ...
                canvas.stride       = FBDEV_O22_PIXEL_2_BYTES(hw_ctx->hw_ext_render_width); // stride(1920)
            }
            else
            {
        ...
        default:
        {
            ...
            {
                FBDEV_O22_HDR_DEBUG("fb(%d) convert odd out_w %d to %d\n", fb_dev_id, canvas.output_win.w,
            ...
            {
                FBDEV_O22_HDR_DEBUG("fb(%d) bitmap based moved ! by %d,%d\n", fb_dev_id, canvas.input_win.x, canvas.input_win.y);

                /* gMAU requests 16byte aligned address */
                bitmap_ptr += canvas.input_win.y * canvas.stride;
                bitmap_ptr += FBDEV_O22_GMAU_ALIGNED_BYTES(FBDEV_O22_BITS_2_BYTES(canvas.input_win.x * canvas.bits_per_pixel));
            }

            FBDEV_O22_HDR_DEBUG("fb(%d) bitmap hwparam (%d,%d,%d,%d)(%d,%d,%d,%d) stride=%d fbmem=0x%08x fbc=0x%x, pixel_format=%d\n",
                                fb_dev_id,
                    ...
                    canvas.pixel_format != canvas.pixel_format_current)
            {
                ...
                for (i = 0; i < xnum; i++)
                {
                    ...
                    {
                        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_pdec,   osd_ch_alpha,           0x0);
                        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_pdec,   osd_ch_red,             0x3);
                        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_pdec,   osd_ch_green,           0x2);
                        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_pdec,   osd_ch_blue,            0x1);
                    }
    ...
    FBDEV_O22_CMN_HandleReverseOSD(fb_dev_id, &canvas, &osd_hdr);

    FBDEV_O22_HDR_DEBUG("fb(%d) bitmap hwparam (%d,%d,%d,%d)(%d,%d,%d,%d) stride=%d fbmem=0x%08x fbc=0x%x (final)\n",
                        fb_dev_id,
                        canvas.input_win.x, canvas.input_win.y, canvas.input_win.w, canvas.input_win.h,
                        canvas.output_win.x, canvas.output_win.y, canvas.output_win.w, canvas.output_win.h,
    ...
    /* save osd header */
    memcpy(&hw_ctx->osd_hdr_sav[fb_dev_id], &osd_hdr, sizeof(FBDEV_O22_OSD_HDR_T));

    /* raxis.lim (2019/05/21)
     *
     * I don't care here whether two POSDs sare necessary or not. refer to FBDEV_O22_CMN_WriteHdrRegs()
     */
    ...
    FBDEV_O22_CMN_WriteHdrRegs(fb_dev_id, &osd_hdr, &osd_exhdr);
    ...
    if (fb_dev_id == LX_FBDEV_ID_OSD0)
    {
        ...
        {
            if (hw_ctx->hw_wa_flag & FBDEV_O22_HW_WA_PQEFW_OSD0_OUT_CTRL)
            {
                fbdev_pqefw_osd0_out_ctrl = 0;
                hw_ctx->hw_wa_flag &= ~FBDEV_O22_HW_WA_PQEFW_OSD0_OUT_CTRL;
                FBDEV_NOTI("PQEFW_OSD0_OUT_CTRL disabled\n");
 ...
 *
 */
int FBDEV_O22_CMN_SetOSDEnable(int fb_dev_id, int enable)
{
    int     i;
    int     xidx;
    int     xnum, xlst[2], afbc_xlst[2];
    ...
    hw_state = FBDEV_O22_CMN_GetOSDEnable(fb_dev_id);
    ...
    if (fb_dev_id == LX_FBDEV_ID_OSD1 && FBDEV_O22_CMN_GetOSDEnable(LX_FBDEV_ID_OSD0))
    {
    ...
    if (fb_dev_id == LX_FBDEV_ID_OSD1)
    {
        if (enable)
        {
            ...
            //
            FBDEV_O22_CMN_AllocExternalRendererFrameBuffer();
            ...
            if (FBDEV_O22_CMN_GetOSDEnable(LX_FBDEV_ID_CSR0))
            {
                int csr_xnum, csr_xlst[2], csr_afbc_xlst[2];

                csr_xnum = FBDEV_O22_CMN_GetHWXListEx(LX_FBDEV_ID_CSR0, csr_xlst, csr_afbc_xlst);

                if (csr_xnum > 0)
                {
                    int csr_xidx = csr_xlst[0];

                    OSDX_O22_RdFL(osdx_ctrl[csr_xidx], osd_ctrl_sync0);
                    OSDX_O22_Wr01(osdx_ctrl[csr_xidx], osd_ctrl_sync0, osd_sync_enable, 0x0);
                    OSDX_O22_WrFL(osdx_ctrl[csr_xidx], osd_ctrl_sync0);
    ...
    if ((fb_dev_id == LX_FBDEV_ID_OSD0 || fb_dev_id == LX_FBDEV_ID_OSD1) && enable)
    {
        ...
        FBDEV_O22_CMN_WaitPQEDriverEstreamerSync(pqe_fw_estreamer_flag);
        ...
        //FBDEV_O22_CMN_SetOutputCropRegion(FALSE, hw_ctx->hw_fb0_width, hw_ctx->hw_fb0_height, 0, 0, &crop_rgn);
    }
    ...
    for (i = 0; i < xnum; i++)
    {
        ...
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_main, osd_en, val);
        OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_sync0, osd_sync_enable, val);
        ...
        {
            if (FBDEV_O22_CMN_GetOSDEnable(LX_FBDEV_ID_OSD1))
            {
                OSDX_O22_Wr01(osdx_ctrl[xidx], osd_ctrl_sync0, osd_sync_enable, 0x0);
                FBDEV_NOTI("++ set cursor invisible (sync.en=0) when EXT_RENDER is shown\n");
            }
        }
    }
    ...
    // raixs.lim (2021/06/01) E60 의 상황을 참조하여, O22+F22 의 경우를 미리 대비함.
    //
    ...
    //
    // (NOTE) O22 에서 아래 코드가 반드시 필요할지에 대하여 검증이 필요하다.
    ...
    if (hw_osd0_owner != hw_ctx->hw_osd0_owner)
    {
        ...
        if (hw_ctx->b_support_fb1 && !hw_ctx->b_support_ext_render)
        {
            ...
            FBDEV_O22_CMN_SetOutputCropRegion(FALSE, fb_width, fb_height, 0, 0, &crop_rgn);
        }
        ...
        if (hw_osd0_owner_canvas)
        {
            ...
            FBDEV_O22_CMN_WriteOSDHeader(hw_osd0_owner_canvas);
        }
    }
...
#ifdef FBDEV_O22_USE_AFBC_READ_BW_SAVING
    {
        int afbc_status = FBDEV_O22_CMN_GetHWStatus(FBDEV_O22_HW_STATUS_AFBC, fb_dev_id);

        OSD_TOP_O22_RdFL(ctrl_auto_init_afbc);

        FBDEV_HW_NOTI("fb(%d) afbc_status=%d, enable=%d\n", fb_dev_id, afbc_status, enable);
        ...
        FBDEV_O22_FBC_DEBUG("fb(%d) afbc=%d, view=%d\n", fb_dev_id, afbc_status, enable);

        OSD_TOP_O22_WrFL(ctrl_auto_init_afbc);

        if (afbc_status)
        {
            FBDEV_NOTI("fb(%d) afbc configured\n", fb_dev_id);
        }
    }
#endif
#endif
    ...
    // see http://clm.lge.com/issue/browse/SICDTV-8669?focusedCommentId=2657619&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-2657619
    if (fb_dev_id == LX_FBDEV_ID_OSD0 && enable && FBDEV_O22_CMN_GetOSDEnable(LX_FBDEV_ID_CSR0))
    {
        int csr_xnum, csr_xlst[2], csr_afbc_xlst[2];
        ...
        if (csr_xnum > 0)
        {
            ...
            OSDX_O22_RdFL(osdx_ctrl[csr_xidx], osd_ctrl_sync0);
            OSDX_O22_Wr01(osdx_ctrl[csr_xidx], osd_ctrl_sync0, osd_sync_enable, 0x1);
            OSDX_O22_WrFL(osdx_ctrl[csr_xidx], osd_ctrl_sync0);
...
#ifdef FBDEV_O22_SUPPORT_SW_WA_SICDTV_10362

    // O22+F22 osd_change_flag 인터페이스 미진한 부분 보강
    ...
    if (hw_ctx->b_support_ext_frc)
    {
        ...
        if (fb_dev_id == LX_FBDEV_ID_CSR0 && !enable)
        {
            ...
            if (canvas)
            {
                ...
                FBDEV_O22_CMN_WriteOSDHeader(canvas);
    ...
    {
        FBDEV_O22_CMN_DestroyExternalRendererFrameBuffer();
    }
...
static void FBDEV_O22_CMN_ShutdownOSDPath(void)
{
    ...
    for (fb_dev_id = 0; fb_dev_id < LX_FBDEV_ID_MAX; fb_dev_id++)
    {
        if (g_fbdev_cfg->fb[fb_dev_id].b_active)
        {
            ...
            FBDEV_O22_CMN_SetOSDEnable(fb_dev_id, FALSE);
        }
    }
 ...
 *
 */
int FBDEV_O22_CMN_GetOSDEnable(int fb_dev_id)
{
    UINT32 val;
    int xidx;
    int xnum, xlst[2];
    ...
    OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_main);
    OSDX_O22_Rd01(osdx_ctrl[xidx], osd_ctrl_main, osd_en, val);
...
int     FBDEV_O22_CMN_SetViewCtrl(LX_FBDEV_VIEW_CTRL_T* ctrl)
{
    int i;
    ULONG flags;
    ...
    for (i = 0; i < LX_FBDEV_ID_MAX; i++)
    {
        ...
        {
            FBDEV_O22_CMN_SetOSDEnable(i, ctrl->view_status[i]);
        }
    }
 ...
 *
 */
int     FBDEV_O22_CMN_SetConnCtrl(LX_FBDEV_CONN_CTRL_T* pConnCtrl)
{
    int     i;
    ...
    for (i = 0; i < LX_FBDEV_ID_MAX; i++)
    {
        osd_view_status[i] = FBDEV_O22_CMN_GetOSDEnable(i);
        FBDEV_O22_CMN_SetOSDEnable(i, FALSE);
    }

    /* disable ISR to change ISR source */
    FBDEV_O22_CMN_SetInterruptEnable(MIXER_INTR, FALSE);

    /* sleep until OSD stablized. not necessary to sleep in O22 */

    if (pConnCtrl->conn_type & LX_FBDEV_CONN_OSD_VIDEO_SEPARATE)
    {
        FBDEV_HW_DEBUG("O22::S.OSD\n");
...
func_exit:
    ...
    for (i = 0; i < LX_FBDEV_ID_MAX; i++)
    {
        FBDEV_O22_CMN_SetOSDEnable(i, osd_view_status[i]);
    }
 ...
 *
 */
int     FBDEV_O22_CMN_SRCtrl(BOOLEAN onoff)
{
    FBDEV_O22_CMN_SetupSRScaler(onoff, TRUE);
    return RET_OK;
}
...
int     FBDEV_O22_CMN_MirrorCtrl(BOOLEAN v_mirror_en)
{
    ...
    FBDEV_O22_HW_CTX_T* hw_ctx = FBDEV_O22_GET_HW_CTX();
    FBDEV_O22_MIRROR_CTRL_T* mirror_ctrl = FBDEV_O22_GET_MIRROR_CTRL(hw_ctx);
    FBDEV_O22_FBC_CTRL_T* fbc_ctrl = FBDEV_O22_GET_FBC_CTRL(hw_ctx, LX_FBDEV_ID_OSD0);

    /* change mirror configuratin */
    mirror_ctrl->status = (v_mirror_en) ? FBDEV_O22_V_MIRROR_ON : 0x0;
    ...
    for (j = 0; j < NELEMENTS(fb_list); j++)
    {
        ...
        xnum = FBDEV_O22_CMN_GetHWXList(fb_list[j], xlst);
        ...
        for (i = 0; i < xnum; i++)
        {
            ...
            FBDEV_NOTI("fb(%d) before: 0x%08x 0x%08x\n", fb_list[j],
                       OSDX_O22_Rd(osdx_ctrl[xidx], osd_ctrl_main),
                       OSDX_O22_Rd(osdx_ctrl[xidx], osd_ctrl_sync0));
...

    {
        ...
        if (wctx)
        {
            FBDEV_O22_CMN_WriteOSDHeader(&wctx->pixel);
        }
    }
 ...
 *
 */
void    FBDEV_O22_CMN_HandleReverseOSD(int fb_dev_id, struct scanvas* hCanvas, FBDEV_O22_OSD_HDR_T* osd_hdr)
{
#ifdef FBDEV_O22_USE_REV_HW_CTRL
 ...
 *
 */
void     FBDEV_O22_CMN_ISRHook(void)
{
 ...
 *
 */
int      FBDEV_O22_CMN_GetHWStatus(FBDEV_O22_HW_STATUS_T status, UINT32 opt)
{
    int val = 0;
    ...
    switch (status)
    {
        ...
        case FBDEV_O22_HW_STATUS_AFBC:
        {
            OSD_TOP_O22_RdFL(ctrl_dpath);

            if (opt == LX_FBDEV_ID_OSD0)
            {
                OSD_TOP_O22_Rd01(ctrl_dpath, osd0_afbc_en, val);
            }
        ...
        case FBDEV_O22_HW_STATUS_VISIBLE:
        {
            val = FBDEV_O22_CMN_GetOSDEnable(opt);
        }
        break;

        case FBDEV_O22_HW_STATUS_SR_GAIN:
        {
 ...
 *
 */
void FBDEV_O22_CMN_DiagHW(struct seq_file* m)
{
    UINT32 val;
    ...
    /* print HW core info */
    {
        seq_printf(m, "panel.width: %d\n", hw_ctx->hw_panel_width);
        seq_printf(m, "panel.height %d\n", hw_ctx->hw_panel_height);
        ...
        seq_printf(m, "\n");
        seq_printf(m, "disp.uhd_mode: %d\n", FBDEV_O22_CMN_IsUHDDispMode() ? 1 : 0);
        ...
        seq_printf(m, "\n");
        seq_printf(m, "vsync_clk: %dHz\n", (FBDEV_O22_CMN_DetectVSyncClk() == FBDEV_VSYNC_CLK_60HZ) ? 60 : 120);
        ...
        OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_main);
        OSDX_O22_Rd01(osdx_ctrl[xidx], osd_ctrl_main, osd_v_reverse_en, val);
        ...
        seq_printf(m, "hw_wa.1x_csr_disp_on_blend_path: 0x%x\n",
                   (hw_ctx->hw_wa_flag & FBDEV_O22_HW_WA_1X_CSR_DISP_ON_BLEND_PATH) ? 1 : 0);
        seq_printf(m, "hw_wa.osd0_async_wr_error:       0x%x\n",
                   (hw_ctx->hw_wa_flag & FBDEV_O22_HW_WA_OSD0_SYNC_ASYNC_WR_ERROR) ? 1 : 0);
        seq_printf(m, "hw_wa.pqwfw_osd0_out_ctrl:       0x%x\n",
                   (hw_ctx->hw_wa_flag & FBDEV_O22_HW_WA_PQEFW_OSD0_OUT_CTRL) ? 1 : 0);
    ...
    /* print OSD_SHP info */
    {
        ...
        OSDX_O22_Rd01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_osd_shp_detour_en, detour_en);
        OSDX_O22_Rd01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_rgb2yc_csc_en_de_clk, rgb2yc_csc_en_de_clk);
        OSDX_O22_Rd01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_yc2rgb_csc_en, yc2rgb_csc_en);
        OSDX_O22_Rd01(osdx_ctrl[xidx], osd_bitmap_ctrl0, reg_rgb2yc_csc_p_en_oclk, rgb2yc_csc_p_en_oclk);
        OSDX_O22_Rd01(hdr_ui, osd_hdrui_yc2rgb_ctrl00, reg_yuv2rgb_en, yuv2rgb_en);
    ...
    /* print 4kosd */
    {
        ...
        OSDX_O22_RdFL(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_0);
        OSDX_O22_Rd01(osdx_ctrl[xidx], reg_4k_osd_main_ctrl_0, reg_4k_mode_en, osd_4k_mux);
 ...
 *
 */
void FBDEV_O22_CMN_ProcHandler(FBDEV_HW_PROC_ID_T proc_id, struct seq_file* m, void* data)
{
    switch (proc_id)
    {
        ...
        case FBDEV_HW_PROC_ID_OSD_HDR:
        {
            ...
            UINT32  hdr_lst[7];
            FBDEV_O22_OSD_HDR_T* osd_hdr = (FBDEV_O22_OSD_HDR_T*)hdr_lst;
            ...
            for (i = 0; i < xnum && VALID_XIDX(xlst[i]); i++)
            {
                ...
                OSDX_O22_Rd01(osdx_ctrl[xidx], osd_ctrl_main, osd_en, osd_en);
                OSDX_O22_Rd01(osdx_ctrl[xidx], osd_ctrl_sync0, osd_sync_enable, sync_en);

                OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_hdr0);
                OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_hdr1);
                OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_hdr2);
                OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_hdr3);
                OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_hdr4);
                OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_hdr5);
                OSDX_O22_RdFL(osdx_ctrl[xidx], osd_ctrl_hdr6);

                hdr_lst[0] = OSDX_O22_Rd(osdx_ctrl[xidx], osd_ctrl_hdr0);
                hdr_lst[1] = OSDX_O22_Rd(osdx_ctrl[xidx], osd_ctrl_hdr1);
                hdr_lst[2] = OSDX_O22_Rd(osdx_ctrl[xidx], osd_ctrl_hdr2);
                hdr_lst[3] = OSDX_O22_Rd(osdx_ctrl[xidx], osd_ctrl_hdr3);
                hdr_lst[4] = OSDX_O22_Rd(osdx_ctrl[xidx], osd_ctrl_hdr4);
                hdr_lst[5] = OSDX_O22_Rd(osdx_ctrl[xidx], osd_ctrl_hdr5);
                hdr_lst[6] = OSDX_O22_Rd(osdx_ctrl[xidx], osd_ctrl_hdr6);
        ...
        case FBDEV_HW_PROC_ID_REG_DUMP:
        {
            ...
            {
                { .desc = "OSD",  .reg_base = O22_CCO_BASE,                 .reg_size = sizeof(OSD_O22_REG_T) }, //1536 }, //sizeof(OSD_O22_REG_T) },
                { .desc = "TOP",  .reg_base = O22_CCO_TOP_CTRL_BASE,        .reg_size = sizeof(OSD_TOP_O22_REG_T) }, //256 }, //sizeof(OSD_TOP_O22_REG_T) },
                { .desc = "AFBC0", .reg_base = O22_CCO_AFBC0_BASE,           .reg_size = sizeof(OSD_AFBC_O22_REG_T) }, //256 }, //sizeof(OSD_AFBC_O22_REG_T) },
                { .desc = "AFBC1", .reg_base = O22_CCO_AFBC1_BASE,           .reg_size = sizeof(OSD_AFBC_O22_REG_T) }, //256 }, //sizeof(OSD_AFBC_O22_REG_T) },
                { .desc = "SHP0", .reg_base = O22_CCO_SHP0_BASE,            .reg_size = sizeof(OSD_SHP_O22_REG_T) }, //768 }, //sizeof(OSD_SHP_O22_REG_T) },
                { .desc = "SHP1", .reg_base = O22_CCO_SHP1_BASE,            .reg_size = sizeof(OSD_SHP_O22_REG_T) }, //768 }, //sizeof(OSD_SHP_O22_REG_T) },
                { .desc = "MIF0", .reg_base = O22_CCO_MIF0_BASE,            .reg_size = 0xc0 },
                { .desc = "MIF1", .reg_base = O22_CCO_MIF1_BASE,            .reg_size = 0xc0 },
            };
 ...
 *
 */
int FBDEV_O22_CMN_RunResume(void)
{
    /* OSD freeze will be stopped after 500 ms */
    FBDEV_O22_CMN_RunBootLogoTimer(fbdev_resume_freeze_timeout, TRUE);
...
int FBDEV_O22_CMN_RunSuspend(void)
{
    /* do nothing */
    return RET_OK;
}
...
========================================================================================*/
FBDEV_O22_HW_FUNC_T  g_fbdev_o22_cmn_hw_func =
{
    /* variable is initialize at fbdev_drv_O22.c */

    .O22_InitHW          = FBDEV_O22_CMN_InitHW,
};
