...
static int emac_mdio_probe(struct mii_bus *mii)
{
	...
	linkmode_and(priv->phy_dev->supported, priv->phy_dev->supported, mask);
	...
	netdev_info(dev, "PHY attached: %s @ address %d using irq %d\n",
	            priv->phy_dev->drv->name, priv->phy_dev->mdio.addr,
	            priv->phy_dev->irq);
...
static int emac_init(struct net_device *dev)
{
	...
	eth_hw_addr_set(dev, priv->mac_addr);
	...
	memcpy(priv->mii_bus->irq, priv->phy_irq, sizeof(priv->phy_irq));
...
static void emac_tx_timeout(struct net_device *dev, unsigned int txqueue)
{
}
...
static int emac_probe(struct platform_device *pdev)
{
	...
	err = of_get_mac_address(dev_node, priv->mac_addr);
	if (err) {
		netdev_info(dev, "no valid HW address -- generate random\n");
		eth_random_addr(priv->mac_addr);
	}

	err = of_get_phy_mode(dev_node, &phy_mode);
	if (!err) {
	...
	netif_napi_add_weight(dev, &priv->napi, emac_poll, 32);
