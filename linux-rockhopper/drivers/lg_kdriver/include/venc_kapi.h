...
/* TODO: Change argument type from pointer to structure */
#define LX_VENC_IOWR_CACHE_CTRL				_IOR(VENC_IOC_MAGIC, 24, VENC_CACHE_CTRL_T)

#define LX_VENC_IOW_ENC_SET_RC				_IOW(VENC_IOC_MAGIC, 25, LX_VENC_ENC_RC_T )
#define LX_VENC_IO_SET_IDR					_IO(VENC_IOC_MAGIC, 26 )
#define LX_VENC_IOWR_ENC_SCALE				_IOWR(VENC_IOC_MAGIC, 27, LX_VENC_ENC_SCALE_T )
#define LX_VENC_IOWR_ENC_PARAM				_IOWR(VENC_IOC_MAGIC, 28, LX_VENC_ENC_PARAM_T )
#define LX_VENC_IOW_SET_PORT 				_IOWR(VENC_IOC_MAGIC, 29, UINT8)
#define LX_VENC_IOW_VALID_KADPCALL 			_IOW(VENC_IOC_MAGIC, 30, LX_VENC_KADPCALL_PARAM_T )
...
typedef struct {
	...
	VENC_CACHE_OP operation;	/**< Cache Control Operation */
} VENC_CACHE_CTRL_T;
...
typedef struct
{
    struct source_frame_info source_frame;    // [IN]
    struct target_frame_info target_frame;    // [OUT]
} LX_VENC_ENC_SCALE_T;

/*
   Setting parameter for adjust encoding option
*/
typedef enum
{
    ENCODE_PARAM_NONE = 0,
    ENCODE_PARAM_FRAMERATE = (1 << 0),
    ENCODE_PARAM_GOP_LENGHT = (1 << 1),
    ENCODE_PARAM_BITRATE = (1 << 2),
    ENCODE_PARAM_SET_IDR = (1 << 3),
} LX_VENC_ENC_ADJUST_PARAM_T;

typedef struct
{
    LX_VENC_ENC_ADJUST_PARAM_T adjustParam;
    UINT32 framerate;
    UINT32 gopLength;
    UINT32 bitrate;
    UINT32 setIDRFrame;
...
/*
   Setting parameter for rate control or quality
*/
typedef struct
{
	int id;
	LX_VENC_RC_TYPE_T rcType;
	UINT32 value;	// In case of VBR/CBR the value means target bitrate (kbit/s),
			// otherwise it means fixed qp value.
} LX_VENC_ENC_RC_T;
...
typedef struct
{
	...
	UINT8	frameType;		// 1: BOTTOM 2:TOP
	LX_VENC_RECORD_FRAME_RATE_T	frameRateCode;

	UINT32	timeStamp;
	UINT64	msec;
} LX_VENC_NOTIFY_PARAM_T, *P_LX_VENC_NOTIFY_PARAM_T;
