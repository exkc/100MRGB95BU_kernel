...
int _DEMOD_L18F_DVBS_CheckTuneMode(DEMOD_CTX_T *pDemodInfo)
{
	//DEMOD_L0_system_wait(25); //hwh	
	UINT8  new_tuneMode;
...
int _DEMOD_L18F_DVBS2_CheckTuneMode(DEMOD_CTX_T *pDemodInfo)
{
	//DEMOD_L0_system_wait(25); //hwh	
	UINT8  new_tuneMode;
...
int _DEMOD_L18F_DVBS_OperatingMode(DEMOD_CTX_T *pDemodInfo)
{
	//DEMOD_L0_system_wait(25); //hwh	
	UINT8  operatingMode;
...
int _DEMOD_L18F_DVBS2_OperatingMode(DEMOD_CTX_T *pDemodInfo)
{
	//DEMOD_L0_system_wait(25); //hwh	
	UINT8  operatingMode;
...
int _DEMOD_L18F_LGDBB_DVBS_SetDemod(DEMOD_CTX_T *pDemodInfo)
{
	//DEMOD_L0_system_wait(25); //hwh	
	UINT8  standard;
	UINT32 freq_Khz;
//	UINT16 dvb_s_bandwidth_Khz;
	...
	printk("[L18F LGDBB]  SetDemod DVBS \n");
	standard = DEMOD_DD_MODE_PROP_MODULATION_DVBS;
	new_tuneMode = _DEMOD_L18F_DVBS_CheckTuneMode(pDemodInfo);
	operatingMode = _DEMOD_L18F_DVBS_OperatingMode(pDemodInfo);

	symbol_rate_Kbps = (UINT16)(pDemodInfo->setCfgParam.dvbs.symbolRate);// KSym/s
	freq_Khz = pDemodInfo->setCfgParam.dvbs.frequency;

	printk("[L18F LGDBB] ///// tuneMode = %x ///// \n", new_tuneMode);	//woohyung
	printk("[L18F LGDBB] ///// operatingMode = %x ///// \n", operatingMode);	//woohyung
...
int _DEMOD_L18F_LGDBB_DVBS2_SetDemod(DEMOD_CTX_T *pDemodInfo)
{
	//DEMOD_L0_system_wait(25); //hwh	
	UINT8  standard;
	UINT32 freq_Khz;
//	UINT16 dvb_s2_bandwidth_Khz;
	...
	printk("[L18F LGDBB]  SetDemod DVBS2 \n");
	standard = DEMOD_DD_MODE_PROP_MODULATION_DVBS2;
	new_tuneMode = _DEMOD_L18F_DVBS2_CheckTuneMode(pDemodInfo);
	operatingMode = _DEMOD_L18F_DVBS2_OperatingMode(pDemodInfo);
...
int _DEMOD_L18F_LGDBB_SAT_CheckSignalState(DEMOD_CTX_T *pDemodInfo)
{
	//DEMOD_L0_system_wait(25); //hwh	
	LX_DEMOD_L2_Context *pFrontend;
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS_Set_Config_auto
...
*/
int DEMOD_L18F_DVBS_Set_Config_auto (BOOLEAN bAutoDetect)
{
	//DEMOD_L0_system_wait(5); //hwh	
	LX_DEMOD_L2_Context *pFrontend;
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when LX_DEMOD_L18F_DVBS_Set_Config_auto is disabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_DVBS_Set_FullConfig (LX_DEMOD_DVBS_CONFIG_T *pDvbsConfigParam)
{
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_DVBS_Get_CoderRate( LX_DEMOD_CODERATE_T *pCodeRate)
{
	//DEMOD_L0_system_wait(5); //hwh	
	LX_DEMOD_L2_Context 	*pFrontend;
	LX_DEMOD_L2_SIG_STATUS		signalStatus;
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS_Get_SymbolRate
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_DVBS_Get_SymbolRate( UINT32 *pSymbolRate )
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_DVBS_Get_TotalInfo( void)
{
	//DEMOD_L0_system_wait(25); //hwh	
	...
	{
		DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_Get_Lock() !!!\n", __F__, __L__);
	...
	if(LX_DEMOD_LOCKED == agcLockStatus)
	{
		...
		{
			DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_Get_IFAGC() !!!\n", __F__, __L__);
			ifagcValue = 0;
		}
	}

	if (RET_OK != DEMOD_L18F_Get_NeverLockStatus(LX_DEMOD_DVBS, &neverLockStatus))
	{
		DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_DVB_Get_NeverLockStatus() !!!\n", __F__, __L__);
		neverLockStatus = LX_DEMOD_UNKOWN_STATE;
	}

	if (RET_OK !=  DEMOD_L18F_Get_SpectrumStatus(&spectrumAuto, &spectrumInv))
	{
		DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_Get_SpectrumStatus() !!!\n", __F__, __L__);
	...
	{
		DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_DVBS_Get_TPError() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	DEMOD_PRINT("[DEMOD H15] TP Error Count= %d !!!\n",errorCount);

	if (RET_OK != DEMOD_L18F_Get_CarrierFreqOffset( &FreqOff ))
	{
		DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
		FreqOff = 0xffff;
	}
	...
	{
		DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_DVBC_Get_SNR() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	DEMOD_PRINT("[DEMOD H15] AGC LOCK is %s !!!\n",(agcLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD H15] IF AGC value = %d(0x%x) !!!\n",ifagcValue,ifagcValue);
	DEMOD_PRINT("[DEMOD H15] Neverlock is %s !!!\n",(neverLockStatus == LX_DEMOD_DETECTED) ? " Locked" : " NOT Locked");
	...
	DEMOD_PRINT("[DEMOD H15] offset = %d [KHz]!!!\n",FreqOff);
	DEMOD_PRINT("[DEMOD H15] mse = %d, constPwr = %d, !!!\n",sigMse.mse, sigMse.constPwr);

	if (RET_OK != DEMOD_L18F_DVBS_Get_CoderRate(&codeRate ))
	{
		DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_DVBS_Get_CoderRate() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK != DEMOD_L18F_DVBS_Get_SymbolRate(&symbolRate ))
	{
		DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_DVBS_Get_SymbolRate() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	...
	printk("[DEMOD H15] AGC LOCK is %s !!!\n",(agcLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	printk("[DEMOD H15] IF AGC value = %d !!!\n",ifagcValue);
	printk("[DEMOD H15] Neverlock is %s !!!\n",(neverLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	printk("[DEMOD H15] Spectrum mode = %s !!!\n",(spectrumInv == LX_DEMOD_NORMAL) ? " Normal" :
																	(spectrumInv == LX_DEMOD_INVERSION) ? "Inversion" : 	" Unknown");
	printk("[DEMOD H15] Code rate = %d !!!\n",codeRate);
	printk("[DEMOD H15] Symbol rate = %d !!!\n",symbolRate);


	printk("[DEMOD H15] TP Error Count= %d !!!\n",errorCount);
	printk("[DEMOD H15] MSE = %d, constPwr = %d !!!\n",sigMse.mse, sigMse.constPwr);
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS_Set_DiseqcToneMessageCtrl
 * Get the information of symbol rate on DVB-S.
 *
 * @param *pLpCodeRate[OUT] LX_DEMOD_CODERATE_T - code rate : 1/2, 2/3, 3/4, 5/6 and 7/8.
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_DVBS_Set_DiseqcToneMessageCtrl( LX_DEMOD_DVBS_DiSEqC_MSG_T *pDiseqcMsg)
{
	//DEMOD_L0_system_wait(25); //hwh	
	LX_DEMOD_L2_Context 	*pFrontend;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_DVBS_Get_BlindInfo(UINT32 centerFreq, LX_DEMOD_DVBS_BLIND_PARAM_T *pBlindParam)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS_Obtaining_Signal_Lock.
...
int DEMOD_L18F_DVBS_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	//DEMOD_L0_system_wait(25); //hwh	
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				if(RET_OK != _DEMOD_L18F_LGDBB_DVBS_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD L18F LGDBB] FAIL (%s:%d) _DEMOD_L18F_LGDBB_DVBT_SetDemod() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(pDemodInfo->setCfgParam.dvbs.operatingMode == LX_DEMOD_DVBS_BLIND_SCAN_MODE)
				{
					...
					DEMOD_PRINT( "^R^[DEMOD H15] LX_DEMOD_TUNE_START : Blind Spectrum Mode..Going to LX_DEMOD_TUNE_SCAN_FINISH_UNLOCK..... ^0^\n");
					break;
		...
		case LX_DEMOD_TUNE_SCAN:			/***************************************************/
			{
				...
				if((LX_DEMOD_FLAG_COMPLETE == pDemodInfo->scanJobFlag) || (LX_DEMOD_FLAG_FINISH == pDemodInfo->scanJobFlag))
				{
					...
					DEMOD_PRINT( "^c^[DEMOD H15] ERR (%s:%d) Already Done !!!\n", __F__, __L__);
					DEMOD_PRINT( "^R^[DEMOD H15] LX_DEMOD_TUNE_SCAN : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
					return RET_OK;
				}

				if(RET_OK != DEMOD_L18F_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_OFF, &scanProcessing, &scanLock))
				{
					DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_NeverlockScan() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if((LX_DEMOD_FLAG_FINISH == scanProcessing) && (LX_DEMOD_LOCKED == scanLock))
				{
					...
					DEMOD_PRINT("[DEMOD H15] (%s:%d) operMode(%d) !!!\n", __F__, __L__,operMode);
					if(LX_DEMOD_DVBS2 == operMode)
					{
						...
						DEMOD_PRINT( "^G^[DEMOD H15] (%s:%d) Signal is Found (LX_DEMOD_DVBS2)  !!!\n", __F__, __L__);
						DEMOD_PRINT( "^R^[DEMOD H15] LX_DEMOD_TUNE_SCAN : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
					}
					else //(LX_DEMOD_DVBS == operMode)
					{
						...
						DEMOD_PRINT( "^G^[DEMOD H15] (%s:%d) Signal is Found (LX_DEMOD_DVBS)  !!!\n", __F__, __L__);
						DEMOD_PRINT( "^R^[DEMOD H15] LX_DEMOD_TUNE_SCAN : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
					}
				...
				else if((LX_DEMOD_FLAG_FINISH == scanProcessing)&& (LX_DEMOD_UNLOCKED == scanLock))
				{
					...
					DEMOD_PRINT( "^G^[DEMOD H15] (%s:%d) Signal is NOT Found  !!!\n", __F__, __L__);
				...
				else
				{
					...
					DEMOD_PRINT( "^G^[DEMOD H15] (%s:%d) Detection is on processing	!!!\n", __F__, __L__);
				}
...
//				BOOLEAN reacqflagOn;
				...
				if(RET_OK != DEMOD_L18F_Get_OperMode(&operMode))
				{
					DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_Get_OperMode() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);

				}
				DEMOD_PRINT("[DEMOD H15] LX_DEMOD_SYNC_CHECKING() operMode = 0x%x!!!\n", operMode);
				...
				if(LX_DEMOD_DVBS2 == operMode)
				{
					...
					DEMOD_PRINT( "^R^[DEMOD H15] LX_DEMOD_SYNC_CHECKING(DVBS) : Going to LX_DEMOD_SYNC_CHECKING(DVBS2 forcely)..... ^0^\n");
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					DEMOD_PRINT( "^R^[DEMOD H15] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_SIGNAL_STABLE..... ^0^\n");
				}
				else
				{
...
//				UINT32 							packetError;
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					{
						DEMOD_PRINT( "^r^[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
					...
					DEMOD_PRINT_ALWAYS( "[DEMOD H15] LX_DEMOD_SIGNAL_STABLE : MSG_DVBS_FRONT_END_LOCKED..... ^^^0^^^\n");
					DEMOD_PRINT_ALWAYS( "[DEMOD H15] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^^0^^^\n", elapsedTime);
					DEMOD_PRINT( "^R^[DEMOD H15] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^^^0^^^\n", freqOffset);


					if (RET_OK != DEMOD_Get_TunedCFG(pDemodInfo))
					{
						DEMOD_PRINT( "^r^[DEMOD H15] FAIL (%s:%d) DEMOD_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if (RET_OK != _DEMOD_L18F_LGDBB_SAT_CheckSignalState(pDemodInfo))
					{
						DEMOD_PRINT( "^r^[DEMOD H15] FAIL (%s:%d) DEMOD_Get_signal_status() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				...
				{
//					pDemodInfo->controlState	= LX_DEMOD_SYNC_CHECKING;
//					DEMOD_PRINT( "^R^[DEMOD H15] LX_DEMOD_SYNC_CHECKING : Going back to LX_DEMOD_SYNC_CHECKING..... **;\n");
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_L18F_DVBS_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	//DEMOD_L0_system_wait(25); //hwh	
	LX_DEMOD_LOCK_STATE_T			lockState;
	LX_DEMOD_OPER_MODE_T 			operMode;
	...
	if(RET_OK != DEMOD_L18F_Get_Lock(LX_DEMOD_PSYNC_LOCK, &lockState))
	{
		DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_Get_Lock() !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}
...
*/
		...
		if((operMode == LX_DEMOD_DVBS) || (operMode == LX_DEMOD_DVBS2))
		{
			if(operMode != LX_DEMOD_DVBS)
			{
				...
				pDemodInfo->setCfgParam.dvbs2.tuneMode = pDemodInfo->setCfgParam.dvbs.tuneMode;
				pDemodInfo->setCfgParam.dvbs2.symbolRate = pDemodInfo->setCfgParam.dvbs.symbolRate;
				DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) forcely changed to DVBS2 mode \n",__F__, __L__);
				DEMOD_PRINT("[DEMOD H15] (%s:%d) tuneMode(DVBS2)  = %d \n",__F__, __L__, pDemodInfo->setCfgParam.dvbs2.tuneMode);
			}

		}
		else
		{
			DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) Invaild operation mode !! \n",__F__, __L__);
			return RET_ERROR;
		...
		if(pDemodInfo->lockCount == 2)
		{
			...
			DEMOD_PRINT_ALWAYS( "[DEMOD H15] MONITORING FUNC : MSG_DVBS_FRONT_END_LOCKED..... ^0^\n");
		...
		if (RET_OK != DEMOD_Get_TunedCFG(pDemodInfo))
		{
			DEMOD_PRINT( "^r^[DEMOD H15] FAIL (%s:%d) DEMOD_Get_TunedCFG() !!!\n", __F__, __L__);
			DEMOD_RETURN_FAIL(__F__);
		}
	...
	else
	{
		...
		if((LX_DEMOD_UNLOCKED != pDemodInfo->lockStatus)  && (TRUE != pDemodInfo->bNotifiedUnlock))
		{
			...
			if(pDemodInfo->unLockCount == 4)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD H15] MONITORING FUNC : MSG_DVBS_FRONT_END_NOT_LOCKED..... -_-\n");
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS2_Set_Config_auto
 * Control automatic setting of DVB-T config.
 *
 * @param bAutoDetect 	[IN] BOOLEAN - automatic setting (TRUE) / manual setting(FALSE).
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_DVBS2_Set_Config_auto (BOOLEAN bAutoDetect)
{
	//DEMOD_L0_system_wait(25); //hwh	
	LX_DEMOD_L2_Context *pFrontend;
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS2_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when LX_DEMOD_L18F_DVBS_Set_Config_auto is disabled.
 *
 * @param *pDvbtConfigParam 	[IN] LX_DEMOD_DVBS_CONFIG_T - DVB-T config. structure
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_DVBS2_Set_FullConfig (LX_DEMOD_DVBS2_CONFIG_T *pDvbs2ConfigParam)
{
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS2_Get_CoderRate
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_DVBS2_Get_CoderRate( LX_DEMOD_CODERATE_T *pCodeRate )
{
	//DEMOD_L0_system_wait(25); //hwh	
	LX_DEMOD_L2_Context 	*pFrontend;
	LX_DEMOD_L2_SIG_STATUS		signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_DVBS2_Get_Pilot( BOOLEAN *pPilot )
{
	*pPilot = FALSE;
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS2_Get_SymbolRate
 * Get the information of symbol rate on DVB-S.
 *
 * @param *pLpCodeRate[OUT] LX_DEMOD_CODERATE_T - code rate : 1/2, 2/3, 3/4, 5/6 and 7/8.
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_DVBS2_Get_SymbolRate( UINT32 *pSymbolRate )
{
	//DEMOD_L0_system_wait(25); //hwh	
	LX_DEMOD_L2_Context 	*pFrontend;
	LX_DEMOD_L2_SIG_STATUS		signalStatus;
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS2_Get_TotalInfo
 * Get all information on DVB-T for debug
 *
 * @param NONE.
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_L18F_DVBS2_Get_TotalInfo( void)
{
	//DEMOD_L0_system_wait(25); //hwh	
	LX_DEMOD_LOCK_STATE_T 	agcLockStatus;
	LX_DEMOD_LOCK_STATE_T 	neverLockStatus;
	...
	if (RET_OK != DEMOD_L18F_Get_NeverLockStatus(LX_DEMOD_DVBS2, &neverLockStatus))
	{
		DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_DVB_Get_NeverLockStatus() !!!\n", __F__, __L__);
		neverLockStatus = LX_DEMOD_UNKOWN_STATE;
	}
	...
	if (RET_OK !=  DEMOD_L18F_Get_Packet_Error(&errorCount))
	{
		DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_Get_Packet_Error() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	DEMOD_PRINT("[DEMOD H15] TP Error Count= %d !!!\n",errorCount);
	...
	if (RET_OK != DEMOD_L18F_Get_MseInfo(&sigMse))
	{
		DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) DEMOD_L18F_Get_MseInfo() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
...
int DEMOD_L18F_DVBS2_Set_MemSconfig(void)
{
...
//	djb_st_mem =(UINT32 *)vmap_phys(gMemCfgDEMOD_DVB_DJB.memory_base, sizeof(UINT32));
	...
	{
		printk("######[0629] Error L18F DEMOD DVB_S2 Wrong Memory Base Address ##### !!!!!!!!!!!!!!!!!!!!!!!\n");
	}
	else
	{
		printk("######[0629] first call DEMOD_L18F_DVBS2_Set_MemSconfig TDI_addr[0x%x] , DJB_addr[0x%x] ##### !!!!!!\n", tdi_st_mem, djb_st_mem );
		if(RET_OK != DEMOD_L2_DVBT2_Set_BaseAddr_AXI(pFrontend, tdi_st_mem, djb_st_mem ))
		{
			DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_L2_DVBT2_Set_BaseAddr_AXI() !!!\n", __F__, __L__);
			return RET_ERROR;
		}
	}
	return RET_OK;
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS2_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_L18F_DVBS2_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	//DEMOD_L0_system_wait(25); //hwh	
	static LX_DEMOD_FLAG_T 			scanProcessing = LX_DEMOD_FLAG_START;
	static LX_DEMOD_LOCK_STATE_T 	scanLock = LX_DEMOD_UNLOCKED;
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				if(RET_OK != _DEMOD_L18F_LGDBB_DVBS2_SetDemod(pDemodInfo))
				{
					DEMOD_PRINT_ALWAYS("[DEMOD L18F LGDBB] FAIL (%s:%d) _DEMOD_L18F_LGDBB_DVBT_SetDemod() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
		...
		case LX_DEMOD_TUNE_SCAN:			/***************************************************/
			{
				...
				if((LX_DEMOD_FLAG_FINISH == scanProcessing) && (LX_DEMOD_LOCKED == scanLock))
				{
					...
					DEMOD_PRINT("[DEMOD H15] DEMOD_L18F_NeverlockScan() operMode = 0x%x!!!\n", operMode);
					...
					if(LX_DEMOD_DVBS == operMode)
					{
						...
						DEMOD_PRINT( "^G^[DEMOD H15] (%s:%d) Signal is Found (operMode = LX_DEMOD_DVBS  !!!\n", __F__, __L__);
						DEMOD_PRINT( "^R^[DEMOD H15] LX_DEMOD_TUNE_SCAN : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
					}
					else //(LX_DEMOD_DVBS2 == operMode)
					{
						...
						DEMOD_PRINT( "^G^[DEMOD H15] (%s:%d) Signal is Found (operMode = LX_DEMOD_DVBS2  !!!\n", __F__, __L__);
						DEMOD_PRINT( "^R^[DEMOD H15] LX_DEMOD_TUNE_SCAN : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
					}
...
//				BOOLEAN reacqflagOn;
				...
				if(LX_DEMOD_DVBS == operMode)
				{
					...
					DEMOD_PRINT( "^R^[DEMOD H15] LX_DEMOD_SYNC_CHECKING(DVBS2) : Going to LX_DEMOD_SYNC_CHECKING(DVBS forcely)..... ^0^\n");
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.dvbs2.tuneMode)
					{
						...
						else
						{
							...
							else
							{
								...
								{
									DEMOD_PRINT( "^R^[DEMOD H15] LX_DEMOD_SIGNAL_STABLE : Packet error = %d .....  --_--\n", packetError);
									elapsedTime = jiffies_to_msecs(jiffies) - sigStablestartTime;
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBS2_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_L18F_DVBS2_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	//DEMOD_L0_system_wait(25); //hwh	
	LX_DEMOD_LOCK_STATE_T			lockState;
	LX_DEMOD_OPER_MODE_T 			operMode;
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if((operMode == LX_DEMOD_DVBS) || (operMode == LX_DEMOD_DVBS2))
		{
			if(operMode != LX_DEMOD_DVBS2)
			{
				...
				DEMOD_PRINT("[DEMOD H15] FAIL (%s:%d) forcely changed to DVBS mode \n",__F__, __L__);
				DEMOD_PRINT("[DEMOD H15] (%s:%d) tuneMode (DVBS)  = %d \n",__F__, __L__, pDemodInfo->setCfgParam.dvbs.tuneMode);
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBSX_BlindScan_Init
...
*/
int DEMOD_L18F_DVBSX_BlindScan_Init(LX_DEMOD_BLIND_SCAN_INIT_FREQ_PARAM_T *initFreq)
{
	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD L18F] DEMOD_L18F_DVBSX_BlindScan_Init");

	pFrontend = &gL18F_LGDBB_Frontend;

	DEMOD_PRINT("[DEMOD L18F] DEMOD_L18F_DVBSX_BlindScan_Init startFreqKhz : %d, endFreqKhz : %d", initFreq->startFreqKhz, initFreq->endFreqKhz);
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBSX_BlindScan_End
 * Notification Blind Scan Init on DVB-S/S2.
 *
 * @param None
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_L18F_DVBSX_BlindScan_End(void)
{
	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD L18F] DEMOD_L18F_DVBSX_BlindScan_End");
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBSX_Get_TunerFreq
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_L18F_DVBSX_Get_TunerFreq(LX_DEMOD_BLIND_SCAN_FREQ_PARAM_T *freqInfo)
{
	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD L18F] DEMOD_L18F_DVBSX_Get_TunerFreq");
	...
	DEMOD_PRINT("[DEMOD L18F BlindScan] (%s:%d) CenterFreq= %d , BandWidth = %d \n",__F__, __L__, freqInfo->TunerCenterFreqKhz, freqInfo->TunerCutOffFreqKhz);
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBSX_Get_OffsetFreq
 * Get Tuner Freq infomration on DVB-S/S2.
 *
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_L18F_DVBSX_Get_OffsetFreq(UINT32* freqOff)
{
	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD L18F] DEMOD_L18F_DVBSX_Get_OffsetFreq");

	pFrontend = &gL18F_LGDBB_Frontend;
	
	DEMOD_L2_SAT_Blind_FoundInfo(pFrontend);

	*freqOff = pFrontend->demod->rsp->blind_foundinfo.symrate;

	DEMOD_PRINT("[DEMOD L18F BlindScan DEMOD_L18F_DVBSX_Get_OffsetFreq] BandWidth = %d \n", *freqOff);
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBSX_SymbolRate_Detec
 * Notification Blind Scan Init on DVB-S/S2.
 *
 * @param None
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_L18F_DVBSX_SymbolRate_Detec(void)
{	
	UINT8 return_code = RET_ERROR;

	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD L18F] DEMOD_L18F_DVBSX_SymbolRate_Detec");
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBSX_BlindLock_Loop
 * Notification Blind Scan Init on DVB-S/S2.
 *
 * @param None
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_L18F_DVBSX_BlindLock_Loop(void)
{	
	UINT8 return_code = RET_ERROR;

	LX_DEMOD_L2_Context *pFrontend;

	DEMOD_PRINT("[DEMOD L18F] DEMOD_L18F_DVBSX_BlindLock_Loop");
...
************************************************************************************/
/**
 * DEMOD_L18F_DVBSX_Get_Nextfreq_Status
 * Get Tuner Freq infomration on DVB-S/S2.
 *
 * @param SINT32* freqOff
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jinwoo.han (jinwoo.han@lge.com)
*/
int DEMOD_L18F_DVBSX_Get_Nextfreq_Status(BOOLEAN* pBlindScanEnd)
{
	LX_DEMOD_L2_Context *pFrontend;
	...
	DEMOD_PRINT("[DEMOD L18F BlindScan DEMOD_L18F_DVBSX_Get_Nextfreq_Status] pBlindScanEnd = %d \n", *pBlindScanEnd);
