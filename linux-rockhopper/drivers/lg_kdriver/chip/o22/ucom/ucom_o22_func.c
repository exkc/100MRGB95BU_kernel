...
#define O22_UCOM_STATUS_PRINTNUM	3
...
static void __iomem *o22_ipc_disable = NULL;
void __iomem *o22_micom_eeprom_base = NULL;
static void __iomem *o22_chipreset_base = NULL;
...
void o22_set_micom_disable(unsigned int disable);
void o22_ucom_FuncInit(void);
void o22_ucom_FuncChipReset(void);
int o22_ucom_FuncWrite(LX_UCOM_PARAM_T * param);
int o22_ucom_FuncRead(LX_UCOM_PARAM_T * param);
int o22_ucom_FuncGetStatus(void );

unsigned int o22_get_micom_disable(void)
{
	unsigned int reg = 0;

	if(o22_ipc_disable == NULL)
		o22_ipc_disable = ioremap(0xF30D0000, 4);

	reg = readl(o22_ipc_disable);

	return reg;
}
EXPORT_SYMBOL(o22_get_micom_disable) ;
...
void o22_set_micom_disable(unsigned int disable)
{
	...
	writel(disable, o22_ipc_disable);
}
EXPORT_SYMBOL(o22_set_micom_disable) ;
#if 0
...
//	a2m_ipc_base = ioremap(0xFD3004c0, A2M_IPC_SIZE);
	...
	o22_micom_eeprom_base = ioremap( 0xF3082104,4 );
	o22_chipreset_base = ioremap( 0xFD3004C8,4 );

	/* to make mainsoc access eeprom */
	if(o22_micom_eeprom_base != NULL){
		reg = readl(o22_micom_eeprom_base);
		reg = reg &(~0xf0f0) ;
		reg = reg | 0x3030;
		writel(reg, o22_micom_eeprom_base );
	}
...
void o22_ucom_FuncChipReset(void)
{
	...
	if(o22_chipreset_base != NULL)
		writel(0x1 , o22_chipreset_base );
}
	...
	if(flag != UCOM_FAKE_CMD )
	{
		...
		while (!(readl(m2a_irq_base) & M2A_IPC_DONE)) {  // if m2a_irq_base == 0 , waiting
			...
			if (cur - start > timeout) {
				...
				writel(0,	m2a_irq_base);
				preempt_enable();
				if(ucom_abnormal < O22_UCOM_STATUS_PRINTNUM)
				{
					KDRV_UCOM_NOTI("ukdrv %s:[%llu] [%d][%x][%x][%x][%x]\n", get_cmd_string(readl(a2m_ipc_base + MAILBOX(0) + REG(2)) & 0xff),
						cur - start, ucom_abnormal,readl(m2a_ipc_base + MAILBOX(0) + REG(1)),
						readl(a2m_ipc_base + MAILBOX(0) + REG(1)),key,readl(a2m_ipc_base + MAILBOX(0) + REG(2)));
				}
				err = -ETIME;
				goto quit;
			}
...
static ucom_o22_func_t o22_ucom_func =
{
	.init = o22_ucom_FuncInit,
	.chipreset = o22_ucom_FuncChipReset,
	.write = o22_ucom_FuncWrite,
	.read = o22_ucom_FuncRead,
	.getstatus= o22_ucom_FuncGetStatus,

};

ucom_o22_func_t* get_o22_ucom_func(void)
{
	return &o22_ucom_func;
}
