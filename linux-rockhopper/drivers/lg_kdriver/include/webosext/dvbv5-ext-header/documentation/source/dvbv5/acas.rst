...
| This document provides an overview of the Advanced Conditional Access System (ACAS) and details its functionalities and implementation requirements.
...
======= ========== =============== =======
1.0.5   2024-11-18 `crystal.moon`_  Fixed formmating issues; mentioned that standard functions do not exist; deleted Extended IOCTL and assetization/standardization column from the API list
1.0.4   2024-10-28 `crystal.moon`_  Corrected grammatical errors and improved formatting; added assetization/standardization status to the API list
1.0.3   2023-11-23 `amit21.kumar`_  First release of the ACAS Module subject to assetization
1.0.2   2020-05-20 `yongsu.yoo`_    The ACAS module enabling condition is specified in detail
1.0.1   2019-04-02 `seong.lee`_     Change path name from "/dev/isdb/adapter0/acas0" to "/dev/dvb/adapter0/acas0"
======= ========== =============== =======
...
| The following table lists the terms used in the ACAS module guide. Refer to the MPEG-2 specification and ATSC / DVB standards for frequently used terms in the field of digital video decoding.
...
Term                            Description
=============================== ===============================
ACAS                            Advance Conditional Access System
...
| The Advanced Conditional Access System (ACAS) is a new standard for UHD broadcasting introduced after Broadcast Conditional Access System (BCAS).

| CAM ensures that only the authorized subscriber can watch the services and contents. It achieves this by using the encrypted control word, which is sent by the broadcaster in the form of ECM and EMM packets. 
  Along with the ECM/EMM packets, the audio, video, and data PIDs are also sent in the packetized elementary stream, which is governed by the MPEG and DVB standards.

| The ACAS module is responsible for controlling the ACAS chip to support ACAS functionality defined by the ARIB standard. ACAS processes descramble key extraction and chip control from ECM/EMM data.

| In Japanese models, FHD models must support BCAS, while UHD models must support ACAS. 
  For a family of models that only support UHD, a BCAS Node must be created, but the functional implementation is optional.
  For a SoC vendor that only supports FHD, an ACAS Node must be created, and going forward, the implementation of this feature (ACAS) is mandatory.

Features
========

| The basic operation is the same as BCAS, but there is no interface for card connection/disconnection because the executable command is extended and fixed type.
  Therefore, the features are divided into module initialization, chip interface reset, and APDU interface functions.

Architecture
============

| The following diagram shows the general architecture of the CAS module that helps descramble audio, video, and data PIDs.

.. image:: resources/ACAS_Architecture.png
  :width: 100%

| The broadcast signal is received through the RF port and demodulated into MPEG TS packet data by the FE/Demod components.

| The Descrambler module passes the scrambled MPEG TS packet data to the ACAS module, where the filtration occurs to generate the PIDs.

| This is then passed to the ACAS MW (middleware), which communicates with the ACAS chip through the driver interface to descramble the audio and video PIDs using the control word.

| Finally, the unscrambled data is passed to the audio and video decoder for further processing.

ACAS System
-----------

| The following diagram shows the flow of ACAS operating in conjunction with related modules.

.. image:: resources/ACAS_SYSTEM.png
  :width: 100%

| The ACAS chip is an IC chip in which data that can obtain default key values and fluctuating key values for watching paid broadcasts is stored.
  It is a module that is in charge of the interface needed to send APDU commands to obtain the required data from the IC chip.
...
| The following diagram shows the RESET and APDU sequence used for communicating between the ACAS chip and the ACAS interface.
...
| The following diagram shows the ATR and APDU (data exchange) sequence that occurs when the ACAS chip is powered on.
...
| This section describes the main functionalities of the ACAS module in terms of the module's requirements and constraints.


Functional Requirements
=======================

| The data types and functions used in the ACAS module are described in the API List section of this document. The Functional Requirements section provides the background information necessary to implement the ACAS interface.

Background
----------

| This section provides additional information to help understanding of ACAS (Reset and APDU Sequence).
...
| This section describes the non-functional requirements for ACAS.
...
- File Location: The ACAS interfaces are defined in `dvbv5-ext-acas.h <http://10.157.97.248:8000/bsp_document/master/latest_html/api/file_full_build_source_part2_dvbv5-ext-header_linux_dvbv5-ext-acas.h.html#file-full-build-source-part2-dvbv5-ext-header-linux-dvbv5-ext-acas-h>`_ , and the SoC vendor can obtain the header file from `https://swfarmhub.lge.com/ <https://swfarmhub.lge.com/>`_.
...
| This section describes what data types and APIs are used for ACAS.
...
Name                                    Description
======================================= ===============================
:cpp:any:`acas_ext_control`             struct for ACAS_EXT_S_CTL or ACAS_EXT_G_CTL
:cpp:any:`acas_ext_transfer_apdu`       struct for Transfer APDU
======================================= ===============================
...
Function                                Description
======================================= ===============================================
:c:macro:`ACAS_EXT_CID_INIT`            Initialize ACAS chip interface module.
:c:macro:`ACAS_EXT_CID_RESET`           Reset ACAS chip.
:c:macro:`ACAS_EXT_CID_TRANSFER_APDU`   Transmit the APDU to the ACAS chip.
:c:macro:`ACAS_EXT_CID_GET_RESET_TIME`  Read waiting time for reset from ACAS chip.
======================================= ===============================================
...
| To test the implementation of the ACAS module, webOS TV provides SoCTS (SoC Test Suite) tests. The SoCTS checks the basic operations of the ACAS module and verifies the kernel event operations for the module by using a test execution file. 
For more information, see :doc:`ACAS's SoCTS Unit Test manual </part4/socts/Documentation/source/producer-manual/producer-manual_dvbv5/producer-manual_dvbv5-acas>`.

.. image:: resources/ACAS_TEST.png
  :width: 100%
