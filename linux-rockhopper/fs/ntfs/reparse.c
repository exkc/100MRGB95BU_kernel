...
ntfschar reparse_index_name[] = { cpu_to_le16('$'),
				  cpu_to_le16('R') };
...
static unsigned int ntfs_reparse_tag_mode(REPARSE_POINT *reparse_attr)
{
	unsigned int mode = 0;
...
unsigned int ntfs_reparse_tag_dt_types(ntfs_volume *vol, unsigned long mref)
{
	...
	reparse_attr = (REPARSE_POINT *)ntfs_attr_readall(NTFS_I(vi), AT_REPARSE_POINT,
							  NULL, 0, &attr_size);
		...
		le32 reparse_tag)
{
	...
	indx.header.data.vi.data_offset =
		cpu_to_le16(sizeof(INDEX_ENTRY_HEADER) + sizeof(REPARSE_INDEX_KEY));
	...
	indx.header.key_length = cpu_to_le16(sizeof(REPARSE_INDEX_KEY));
	indx.header.flags = 0;
 ...
 */
static int remove_reparse_index(struct inode *rp, ntfs_index_context *xr,
				le32 *preparse_tag)
{
	REPARSE_INDEX_KEY key;
	u64 file_id_cpu;
		...
		char *value, size_t size)
{
	...
	if (set_reparse_index(ni, xr, ((const REPARSE_POINT *)value)->reparse_tag) &&
	    oldsize > 0) {
		/*
 ...
 */
static int ntfs_set_ntfs_reparse_data(ntfs_inode *ni, char *value, size_t size)
{
	int err = 0;
	ntfs_inode *xrni;
	ntfs_index_context *xr;
	 ...
	 */
	if (valid_reparse_data(ni, (const REPARSE_POINT *)value, size) == false)
		return -EINVAL;
		...
		const ntfschar *target, int target_len)
{
	...
	reparse = (REPARSE_POINT *)ntfs_malloc_nofs(reparse_len);
	if (!reparse) {
		err = -ENOMEM;
		ntfs_free(utarget);
	} else {
