...
static struct kbase_va_region *kbase_mem_from_umm(struct kbase_context *kctx, int fd, u64 *va_pages, u64 *flags)
{
	...
	*flags &= ~BASE_MEM_SAME_VA;
	...
	for_each_sg(sgt->sgl, s, sgt->nents, i) {
		int page_idx;
		size_t nr_pages = PFN_UP(sg_dma_len(s));
		...
		for (page_idx = 0; (page_idx < nr_pages) && (count < reg->nr_pages); page_idx++, count++ )
			*pa++ = sg_dma_address(s) + (page_idx << PAGE_SHIFT);

		WARN_ONCE(page_idx < nr_pages,
	...
	if (WARN_ONCE(count < reg->nr_pages,
			"sg list from dma_buf_map_attachment < dma_buf->size=%zu\n",
			dma_buf->size)) {
		goto invalid_nr_pages;
	}

	/* Update nents and flags as we now have pages to map */
	reg->gpu_alloc->nents = count;

	reg->flags &= ~KBASE_REG_FREE;
	reg->flags |= KBASE_REG_GPU_NX;	/* UMM is always No eXecute */
	reg->flags &= ~KBASE_REG_GROWABLE;	/* UMM cannot be grown */
	reg->flags |= KBASE_REG_GPU_CACHED;
	// commented out by SCJUNG
	// KBASE_MEM_IMPORT_HAVE_PAGES flag conflict to KBASE_REG_SECURE, and it doesn't use for reg->flags
//	reg->flags |= KBASE_MEM_IMPORT_HAVE_PAGES;

	/* Update flags to be used by the caller function as well */
	*flags |= KBASE_MEM_IMPORT_HAVE_PAGES;
...
invalid_nr_pages:
invalid_pa:
	dma_buf_unmap_attachment(reg->gpu_alloc->imported.umm.dma_attachment,
			reg->gpu_alloc->imported.umm.sgt, DMA_BIDIRECTIONAL);
no_dma_buf_map:
	kbase_mem_phy_alloc_put(reg->gpu_alloc);
no_alloc_obj:
	kfree(reg);
