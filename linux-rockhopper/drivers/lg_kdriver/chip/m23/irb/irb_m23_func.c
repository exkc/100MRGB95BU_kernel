...
void m23_irb_funcInit(void);
int m23_irb_tx(LX_IRB_PARAM_T * param);
void m23_irb_funcReInit(void);
void m23_irb_exit(void);

void m23_irb_funcInit(void)
{

	KDRV_IRB_DEBUG("m23_irb init\n");
...
int m23_irb_tx(LX_IRB_PARAM_T * param)
{
	...
	/*  busy status is same in m23  checked by micom engineer 210428*/
	...
	/*  no done clear  in m23  checked by micom engineer 210428*/

	KDRV_IRB_DEBUG("m23_irb tx makenum[%x] pbuf[%x]\n",
		midx,pbuf[0]);
	if(mbuf)
		kfree(mbuf);
	return 0;
...
void m23_irb_funcReInit(void)
{
  ...
  KDRV_IRB_NOTI("m23_irb reinit\n");
}
void m23_irb_exit(void)
{
  KDRV_IRB_DEBUG("m23_irb end\n");
}


static irbs_m23_func_t m23_irb_func =
{
	.init = m23_irb_funcInit,
	.reinit = m23_irb_funcReInit,
	.tx = m23_irb_tx,
	.exit = m23_irb_exit,
};

irbs_m23_func_t* get_m23_irb_func(void)
{
	return &m23_irb_func;
}
