...
#include "de_ipc_def_o26.h"
#include "de_int_def_o26.h"
#include "de_reg_def_o26.h"
#include "de_reg_o26.h"

#include "de_ipc_reg_o26.h"
#include "de_ctr_reg_o26.h"
...
#define DE_REG_O26_NOTI_BUF_MAX     100
#define DE_REG_O26_NOTI_STR_SIZE    64

#define DE_IPC_GEN_O26_SIZE (4*128)

/*----------------------------------------------------------------------------------------
 *	 Macro Definitions
 *---------------------------------------------------------------------------------------*/
#define DE_REG_O26_NOTI_BACKUP(_fmt, args...)	\
	{\
		char _de_io_str_buf[DE_REG_O26_NOTI_STR_SIZE];\
		DE_NOTI(_fmt, ##args);\
		snprintf(_de_io_str_buf, DE_REG_O26_NOTI_STR_SIZE, _fmt, ##args);\
		DE_REG_O26_BackupTraceDataBuf(_de_io_str_buf);\
	}
...
DE_REG_O26_NOTI_TIME_T;

typedef struct {
	DE_REG_O26_NOTI_TIME_T time;		///< time
	char str[DE_REG_O26_NOTI_STR_SIZE];	///< str
}
DE_REG_O26_NOTI_BUF_T;

typedef struct {
	LX_DE_SYSTEM_VERSION_T      system_info; // 0xDECAFE00 + 1:ASIC,2:FPGA

	LX_DE_IPC_SYSTEM_MODEL_T	model;// STB, HD/FHD panel type info.
	LX_DE_IPC_SYSTEM_MODEL1_T	model1;
	UINT32						model2;
	UINT32						model3;
	...
	LX_DE_IPC_SYSTEM_MEM_T		mcu0_dbg;  // mcu0 dbg
	LX_DE_IPC_SYSTEM_MEM_T		fw_shared; // fw shared
	LX_DE_IPC_SYSTEM_MEM_T		mcu1_dbg;  // mcu1 dbg
...
DE_REG_O26_IPC_DAR_INFO_3_T;

typedef struct {
	DE_REG_O26_IPC_DAR_INFO_0_T info_0;
	DE_REG_O26_IPC_DAR_INFO_1_T info_1;
	DE_REG_O26_IPC_DAR_INFO_2_T info_2;
	DE_REG_O26_IPC_DAR_INFO_3_T info_3;
}
DE_REG_O26_IPC_DAR_INFO_T;
...
}DE_O26_EDMA_EDMA_CTRL_T;

typedef struct {
	DE_O26_EDMA_TX_LOCKED_ID_T   tx_locked_id;//0xCC0C0000
	DE_O26_EDMA_TX_SRC_ADDR_T     tx_src_addr;//0xCC0C0004
	DE_O26_EDMA_TX_DST_ADDR_T     tx_dst_addr;//0xCC0C0008
	DE_O26_EDMA_TX_NXT_ADDR_T     tx_nxt_addr;//0xCC0C000C
	DE_O26_EDMA_TX_LENGTH_T         tx_length;//0xCC0C0010
	DE_O26_EDMA_EDMA_CTRL_T         edma_ctrl;//0xCC0C0014
}DE_MCU_EDMA_REG_O26_T;
...
typedef struct {
	UINT32 exception;//0xCCC10E3C
	UINT32 no_touch[3];//0xCCC10E40~0xCCC10E48
	UINT32 exception1;//0xCCC10E4C
}DE_REG_O26_IMX_VERI_15_T;

typedef union {
	UINT32 u32;
	struct {
	UINT32 magic    :24; //[23:0]
	UINT32 ver      :8;  //[31:24]
	};
}de_reg_o26_ctop_header;
#define DEF_DE_REG_O26_CTOP_HEADER_MAGIC    (0xDECAFE)
typedef union {
	UINT32 u32;
	struct {
	UINT32 dnsr0_02_cg_en  :1; //[0] <- addr(SCRG_CTRL_DNSR0(0xCC490000) + crg_dnsr002(0x8)), bit([6]reg_dnsr_sr_clk_gate_en)
	UINT32 dnsr1_02_cg_en  :1; //[1] <- addr(SCRG_CTRL_DNSR1(0xCC4B0000) + crg_dnsr002(0x8)), bit([6]reg_dnsr_sr_clk_gate_en)
	UINT32 rsvd            :30;//[31:2] reserved
	};
}de_reg_o26_ctop0;

typedef struct {
	de_reg_o26_ctop_header  header;
	de_reg_o26_ctop0        c0;
	UINT32                  rsvd[9];
}DE_REG_O26_CTOP_T;
 ...
 *	 global Variables
 *---------------------------------------------------------------------------------------*/
DE_IPC_REG_O26_T gDE_IPC_O26;
volatile UINT32 *gDE_MUTE_O26[2] = {NULL, NULL};//REG_VSD_SHP_SHP_FSW_CTRL_03_T shp_fsw_ctrl_03;	//0xCCA228FC
volatile LX_DE_FW_STATUS_REG_T *gDE_FW_STATUS_O26 = NULL;//REG_CCO_VERI_SYS_VERI_10_T video_delay_sts0;	//0xCC862328
volatile LX_DE_VDEC_IPC_REG_T *gDE_VDEC_O26[LX_DE_WIN_MAX];
volatile DE_REG_O26_IPC_DAR_INFO_T *gDE_DAR_INFO_O26[2] = {NULL, NULL};
volatile LX_DE_DDR_MVI_T *gDE_DDR_MVI_O26 = NULL;
volatile DE_MCU_EDMA_REG_O26_T *gDE_MCU_EDMA_O26 = NULL;
volatile DE_REG_O26_WDOG_T *gDE_MCU_WDOG0_O26 = NULL;
volatile DE_REG_O26_WDOG_T *gDE_MCU_WDOG1_O26 = NULL;
volatile DE_REG_O26_IMX_VERI_15_T *gDE_IMX_VERI_15_O26 = NULL;
volatile UINT32 *gDE_IPC_GEN_O26 = NULL;
volatile DE_REG_O26_CTOP_T *gDE_CTOP_M2C_O26 = NULL;
volatile DE_REG_O26_CTOP_T *gDE_CTOP_C2M_O26 = NULL;

LX_DE_IN_SRC_T g_WinsrcMap_O26[NUM_WIN_MAX] = { LX_DE_IN_SRC_MVI,	LX_DE_IN_SRC_MVI};
UINT32 g_WinsrcPort_O26[NUM_WIN_MAX] = { 0, 0};
UINT32 g_HdmiPort_O26[NUM_WIN_MAX] = { 0, };
UINT32 g_QuadWin_O26  = 0;
LX_DE_OPER_CONFIG_T g_SrcOperType_O26 = LX_DE_OPER_ONE_WIN;
LX_DE_SUB_OPER_CONFIG_T	g_SrcSubOperType_O26 = LX_DE_SUB_OPER_OFF;
UINT16 g_SrcOperCtrlFlag_O26 = 0;
UINT16 g_SrcSubOperCtrlFlag_O26 = 0;
LX_DE_DISPLAY_MIRROR_T	g_Display_mirror_O26 = LX_DE_DIS_MIRROR_OFF;
UINT32                  g_Zero_delay_mode_O26 = 0;
...
static UINT32 _g_de_reg_o26_mvi_magic = 0;

static struct vsc_fw_load_param _g_de_reg_o26_fw_load_p[] = {
	[0] = {
		.name = "pqefw0_o26",
		.idx = 0,
		.hma_pool = "debug0",
...
static int DE_REG_O26_ClearTraceDataBuf(void);
static int DE_REG_O26_BackupTraceDataBuf(char *data);
static int DE_REG_O26_ViewTraceDataBuf(void);
static void _print_status_info(void);
static void _print_tag_info(void);

int DE_IPC_O26_ctop_shared_memory_init(void);
int DE_REG_O26_CviMap(LX_DE_WIN_ID_T win_id, LX_DE_IN_SRC_T in_port, UINT32 port_num);
int de_reg_o26_fwload_register(struct vsc_fw_load *load);
LX_MEMCFG_T de_reg_o26_getmeminfo(char* str);
int DE_REG_O26_SetClockGate(CTOP_PQE_CG_O26_MODULE mod, BOOLEAN onOff);

/*----------------------------------------------------------------------------------------
 *	 Static Variables
 *---------------------------------------------------------------------------------------*/
static DE_REG_O26_IPC_SYSTEM_T  g_Mem_Info = { 0, };
static LX_DE_MEM_SYS_INFO_T   g_Mem_Sys_Info = { 0, };
static DE_REG_O26_NOTI_BUF_T _g_de_reg_o26_buf[DE_REG_O26_NOTI_BUF_MAX];
...
int DE_REG_O26_InitPHY2VIRT(void)
{
	...
	gDE_IPC_O26.shdw.addr  = (UINT32 *)OS_KMalloc(sizeof(DE_IPC_REG_O26A0_T));
	gDE_IPC_O26.phys.addr  = (volatile UINT32 *)ioremap(DE_IPC_REG_O26_BASE, sizeof(DE_IPC_REG_O26A0_T));
	gDE_IPC_GEN_O26 = (volatile UINT32 *)ioremap(DE_IPC_GEN_O26_BASE, DE_IPC_GEN_O26_SIZE);
	gDE_VDEC_O26[0] = (volatile LX_DE_VDEC_IPC_REG_T *)ioremap(DE_VDEC_O26_BASE + 0x00, sizeof(LX_DE_VDEC_IPC_REG_T));
	gDE_VDEC_O26[1] = (volatile LX_DE_VDEC_IPC_REG_T *)ioremap(DE_VDEC_O26_BASE + 0x40, sizeof(LX_DE_VDEC_IPC_REG_T));
	gDE_VDEC_O26[2] = (volatile LX_DE_VDEC_IPC_REG_T *)ioremap(DE_VDEC_O26_BASE + 0x80, sizeof(LX_DE_VDEC_IPC_REG_T));
	gDE_VDEC_O26[3] = (volatile LX_DE_VDEC_IPC_REG_T *)ioremap(DE_VDEC_O26_BASE + 0xC0, sizeof(LX_DE_VDEC_IPC_REG_T));
	gDE_MUTE_O26[0] = (volatile UINT32 *)ioremap(DE_MUTE_O26_BASE, sizeof(UINT32));//REG_VSD_SHP_SHP_FSW_CTRL_03_T mute_info_w1_w0;	//0xCCA228FC
	gDE_MUTE_O26[1] = (volatile UINT32 *)ioremap(DE_MUTE_O26_BASE - 4, sizeof(UINT32));// REG_VSD_SHP_SHP_FSW_CTRL_02_T          mute_info_w3_w2; //0xCCA228F8
	gDE_FW_STATUS_O26 = (volatile LX_DE_FW_STATUS_REG_T *)ioremap(DE_FW_STATUS_O26_BASE, sizeof(LX_DE_FW_STATUS_REG_T));//REG_CCO_VERI_SYS_VERI_10_T video_delay_sts0;	//0xCC862328
	gDE_DAR_INFO_O26[0] = (volatile DE_REG_O26_IPC_DAR_INFO_T *)ioremap(DE_FW_DARINFO_O26_BASE, sizeof(DE_REG_O26_IPC_DAR_INFO_T));
	gDE_DAR_INFO_O26[1] = (volatile DE_REG_O26_IPC_DAR_INFO_T *)ioremap((DE_FW_DARINFO_O26_BASE+sizeof(DE_REG_O26_IPC_DAR_INFO_T)), sizeof(DE_REG_O26_IPC_DAR_INFO_T));
	gDE_DDR_MVI_O26 = (volatile LX_DE_DDR_MVI_T *)vmap_phys((g_fw_shared.base + VIDEO_O26_FW_SHARED_OFST_MVI), sizeof(LX_DE_DDR_MVI_T));
	gDE_MCU_EDMA_O26 = (volatile DE_MCU_EDMA_REG_O26_T *)ioremap(DE_MCU_EDMA_O26_BASE, sizeof(DE_MCU_EDMA_REG_O26_T));
	gDE_MCU_WDOG0_O26 = (volatile DE_REG_O26_WDOG_T *)ioremap(DE_MCU_WDOG0_O26_BASE, sizeof(DE_REG_O26_WDOG_T));
	gDE_MCU_WDOG1_O26 = (volatile DE_REG_O26_WDOG_T *)ioremap(DE_MCU_WDOG1_O26_BASE, sizeof(DE_REG_O26_WDOG_T));
	gDE_IMX_VERI_15_O26 = (volatile DE_REG_O26_IMX_VERI_15_T *)ioremap(DE_IMX_VERI_15_O26_BASE, sizeof(DE_REG_O26_IMX_VERI_15_T));
	gDE_CTOP_M2C_O26 = (volatile DE_REG_O26_CTOP_T *)vmap_phys((g_fw_shared.base + VIDEO_O26_FW_SHARED_OFST_CTOP_M2C), sizeof(DE_REG_O26_CTOP_T));
	gDE_CTOP_C2M_O26 = (volatile DE_REG_O26_CTOP_T *)vmap_phys((g_fw_shared.base + VIDEO_O26_FW_SHARED_OFST_CTOP_C2M), sizeof(DE_REG_O26_CTOP_T));

	DE_NOTI("ioremap wdog 0:0x%08x, 1:0x%08x.\n",DE_MCU_WDOG0_O26_BASE,DE_MCU_WDOG1_O26_BASE);

	DE_REG_O26_ClearTraceDataBuf();
	_set_tag_info();
...
{
	if (gDE_IPC_O26.shdw.addr) OS_Free((void *)gDE_IPC_O26.shdw.addr);
	if (gDE_IPC_O26.phys.addr) iounmap((void *)gDE_IPC_O26.phys.addr);
	if (gDE_IPC_GEN_O26) iounmap((void *)gDE_IPC_GEN_O26);
	if (gDE_VDEC_O26[0]) iounmap((void *)gDE_VDEC_O26[0]);
	if (gDE_VDEC_O26[1]) iounmap((void *)gDE_VDEC_O26[1]);
	if (gDE_VDEC_O26[2]) iounmap((void *)gDE_VDEC_O26[2]);
	if (gDE_VDEC_O26[3]) iounmap((void *)gDE_VDEC_O26[3]);
	if (gDE_MUTE_O26[0]) {iounmap((void *)gDE_MUTE_O26[0]);iounmap((void *)gDE_MUTE_O26[1]);}
	if (gDE_FW_STATUS_O26) iounmap((void *)gDE_FW_STATUS_O26);
	if (gDE_DAR_INFO_O26[0]) {iounmap((void *)gDE_DAR_INFO_O26[0]);iounmap((void *)gDE_DAR_INFO_O26[1]);}
	if (gDE_DDR_MVI_O26) vunmap_phys((void*)gDE_DDR_MVI_O26);
	if (gDE_MCU_EDMA_O26) iounmap((void *)gDE_MCU_EDMA_O26);
	if (gDE_MCU_WDOG0_O26) iounmap((void *)gDE_MCU_WDOG0_O26);
	if (gDE_MCU_WDOG1_O26) iounmap((void *)gDE_MCU_WDOG1_O26);
	if (gDE_IMX_VERI_15_O26) iounmap((void *)gDE_IMX_VERI_15_O26);
	if (gDE_CTOP_M2C_O26) vunmap_phys((void*)gDE_CTOP_M2C_O26);
	if (gDE_CTOP_C2M_O26) vunmap_phys((void*)gDE_CTOP_C2M_O26);
...
int DE_IPC_O26_GetStatusReg(UINT32 mcu_id, VIDEO_IPC_TYPE_T ipcType, UINT32 *pStatus)
{
	...
	switch (ipcType) {
		...
		case VIDEO_WEL_MCU :
		case VIDEO_WER_MCU :
			DE_IPC_O26_RdFL(int_intr_status);
			*pStatus = DE_IPC_O26_Rd(int_intr_status);
			break;
		case VIDEO_IPC_CPU :
		case VIDEO_DMA_CPU :
		case VIDEO_JPG_CPU :
		case VIDEO_WEL_CPU :
		case VIDEO_WER_CPU :
			DE_IPC_O26_RdFL(ext_intr_status);
			*pStatus = DE_IPC_O26_Rd(ext_intr_status);
			break;
		default :
			BREAK_WRONG(ipcType);
	}
...
int DE_WDG_O26_WakeUpReg(UINT32 mcu_id, BOOLEAN turnOn)
{
	...
	switch(mcu_id)
	{
		case 0:
			if (turnOn) {
				DE_IPC_O26_FLWf(ext_intr_event, wdg_interrupt_event, 1);
			} else {
				DE_IPC_O26_FLWf(ext_intr_enable, wdg_interrupt_enable, 0);
				DE_IPC_O26_FLWf(ext_intr_clear, wdg_interrupt_clear, 1);
			}
		break;
		case 1:
			if (turnOn) {
				DE_IPC_O26_FLWf(ext_intr_event, wdg_interrupt_event_mcu1, 1);
			} else {
				DE_IPC_O26_FLWf(ext_intr_enable, wdg_interrupt_enable_mcu1, 0);
				DE_IPC_O26_FLWf(ext_intr_clear, wdg_interrupt_clear_mcu1, 1);
			}
			break;
		default:
			break;
	}
	DE_IPC_O26_RdFL(ext_intr_enable);
	rd_data = DE_IPC_O26_Rd(ext_intr_enable);
...
int DE_IPC_O26_WakeUpReg(UINT32 mcu_id, VIDEO_IPC_TYPE_T ipcType, BOOLEAN turnOn)
{
	...
	switch (ipcType) {
		...
		case VIDEO_IPC_CPU :
			if (turnOn) {
				DE_IPC_O26_FLWf(ext_intr_event,\
						ipc_interrupt_event_arm,\
						GET_PMSK(VIDEO_IPC_INTERRUPT_ARM_BIT, 1));
			} else {
				DE_IPC_O26_FLWf(ext_intr_clear,\
...
int DE_IPC_O26_ClearStatus(UINT32 mcu_id, VIDEO_IPC_TYPE_T ipcType, UINT32 *pStatus)
{
	...
	switch (ipcType) {
		...
		case VIDEO_LED_CPU :
			...
			break;
		case VIDEO_CTOP_CPU :	//mhlee, ctop ctrl by interrupt
			DE_IPC_O26_FLWf(ext_intr_clear,\
					ipc_interrupt_clear_arm,\
					GET_PMSK(VIDEO_CTOP_INTERRUPT_ARM_BIT, 1));
			break;
		default :
			BREAK_WRONG(ipcType);
			break;
...
{
	DE_IPC_O26_FLWf(ext_intr2_clear, ipc_interrupt2_clear_arm, 0xffffffff);
	return 0;
}

int DE_IPC_O26_ClearStatus3(UINT32 type)
{
	DE_IPC_O26_FLWf(ext_intr3_clear, ipc_interrupt3_clear_arm, 0xffffffff);
	return 0;
}
...
int DE_IPC_O26_ctop_shared_memory_init(void)
{
	DE_REG_O26_CTOP_T m2c_shared, c2m_shared;

	if (gDE_CTOP_M2C_O26 == NULL)	return RET_ERROR;
	if (gDE_CTOP_C2M_O26 == NULL)	return RET_ERROR;

	memset((char*)&m2c_shared, 0, sizeof(DE_REG_O26_CTOP_T));
	memset((char*)&c2m_shared, 0, sizeof(DE_REG_O26_CTOP_T));
	c2m_shared.header.magic = DEF_DE_REG_O26_CTOP_HEADER_MAGIC;

	// 1. copy register to local
	#if 0 /* following process is skipped. because register's init. value is 0. : http://jira.lge.com/issue/browse/SICDTV-15967 sj.youm's comment */
	CTOP_CTRL_O26Ax_RdFL(SCRG_CTRL_DNSR0, crg_dnsr002);	// read(reg.->global) whole
	CTOP_CTRL_O26Ax_Rd01(SCRG_CTRL_DNSR0, crg_dnsr002, reg_dnsr_sr_clk_gate_en, m2c_shared.c0.dnsr0_02_cg_en); // global->local
	CTOP_CTRL_O26Ax_Rd01(SCRG_CTRL_DNSR0, crg_dnsr002, reg_dnsr_sr_clk_gate_en, c2m_shared.c0.dnsr0_02_cg_en); // global->local
	#endif
	#if 0 /* following process is skipped. because register's init. value is 0. */
	CTOP_CTRL_O26Ax_RdFL(SCRG_CTRL_DNSR1, crg_dnsr002);	// read(reg.->global) whole
	CTOP_CTRL_O26Ax_Rd01(SCRG_CTRL_DNSR1, crg_dnsr002, reg_dnsr_sr_clk_gate_en, m2c_shared.c0.dnsr1_02_cg_en); // global->local
	CTOP_CTRL_O26Ax_Rd01(SCRG_CTRL_DNSR1, crg_dnsr002, reg_dnsr_sr_clk_gate_en, c2m_shared.c0.dnsr1_02_cg_en); // global->local
	#endif
	#if 1 /* if register's init. value is not 0, following process is required. */
	// CTOP_CTRL_O26Ax_RdFL(XXX, yyy);	// read(reg.->global) whole
	// CTOP_CTRL_O26Ax_Rd01(XXX, yyy, zzz, m2c_shared.cX.~~~); // global->local
	// CTOP_CTRL_O26Ax_Rd01(XXX, yyy, zzz, c2m_shared.cX.~~~); // global->local
	#endif

	// 2. copy local to shared mem
	memcpy((char*)gDE_CTOP_M2C_O26, (char*)&m2c_shared, sizeof(DE_REG_O26_CTOP_T));
	memcpy((char*)gDE_CTOP_C2M_O26, (char*)&c2m_shared, sizeof(DE_REG_O26_CTOP_T));
	wmb();/* after write DDR */
	DE_NOTI(" done.\n");
	return 0;
}
...
int DE_IPC_O26_ctop_intr(void)
{
	...
	// 1. copy mcu2cpu(m2c), cpu2mcu(c2m) shared memory to local
	rmb();/* before read DDR*/
	memcpy((char*)&m2c_shared, (char*)gDE_CTOP_M2C_O26, sizeof(DE_REG_O26_CTOP_T));
	memcpy((char*)&c2m_shared, (char*)gDE_CTOP_C2M_O26, sizeof(DE_REG_O26_CTOP_T));

	// 2. check magic number -> NG ? -> do nothing
	if (m2c_shared.header.magic != DEF_DE_REG_O26_CTOP_HEADER_MAGIC)
	{
		DE_NOTI("err: magic number(%d) from mcu2cpu(%d) not matched\n", DEF_DE_REG_O26_CTOP_HEADER_MAGIC, m2c_shared.header.magic);
		return RET_ERROR;
	}

	// 3. compare
	//    -> same ? -> do nothing
	//    -> diff ? -> copy m2c to register
	//              -> copy m2c to c2m (ack)
	if (c2m_shared.c0.dnsr0_02_cg_en != m2c_shared.c0.dnsr0_02_cg_en)
	{
		// read(reg.->global) -> change global -> write(global->reg.)
		CTOP_CTRL_O26Ax_RdFL(SCRG_CTRL_DNSR0, crg_dnsr002);	// read(reg.->global) whole
		CTOP_CTRL_O26Ax_Wr01(SCRG_CTRL_DNSR0, crg_dnsr002, reg_dnsr_sr_clk_gate_en, m2c_shared.c0.dnsr0_02_cg_en);	// new value->global (1-field)
		CTOP_CTRL_O26Ax_WrFL(SCRG_CTRL_DNSR0, crg_dnsr002);	// write(global->reg.) whole
		// ack (copy m2c to c2m shared memory)
		gDE_CTOP_C2M_O26->c0.dnsr0_02_cg_en = m2c_shared.c0.dnsr0_02_cg_en;	wmb();/* after write DDR */
		DE_NOTI("dnsr0 sr_clk_gate_en done(%d)\n", m2c_shared.c0.dnsr0_02_cg_en);
	}
	if (c2m_shared.c0.dnsr1_02_cg_en != m2c_shared.c0.dnsr1_02_cg_en)
	{
		// read(reg.->global) -> change global -> write(global->reg.)
		CTOP_CTRL_O26Ax_RdFL(SCRG_CTRL_DNSR1, crg_dnsr002);
		CTOP_CTRL_O26Ax_Wr01(SCRG_CTRL_DNSR1, crg_dnsr002, reg_dnsr_sr_clk_gate_en, m2c_shared.c0.dnsr1_02_cg_en);
		CTOP_CTRL_O26Ax_WrFL(SCRG_CTRL_DNSR1, crg_dnsr002);
		// ack (copy m2c to c2m shared memory)
		gDE_CTOP_C2M_O26->c0.dnsr1_02_cg_en = m2c_shared.c0.dnsr1_02_cg_en;	wmb();/* after write DDR */
		DE_NOTI("dnsr1 sr_clk_gate_en done(%d)\n", m2c_shared.c0.dnsr1_02_cg_en);
	}
	return 0;
}
...
int DE_REG_O26_CheckMuteStatus(UINT32 win_id, UINT32 *mute_status)
{
	...
	if (gDE_MUTE_O26[0]==NULL)	return RET_ERROR;
...
int DE_REG_O26_CheckFwStatus(UINT32 win_id, void *status)
{
	...
	if (gDE_FW_STATUS_O26==NULL)	return RET_ERROR;
	if (status==NULL)	return RET_ERROR;
	memcpy(dst, (const void *)gDE_FW_STATUS_O26, sizeof(LX_DE_FW_STATUS_REG_T));
	return ret;
}
...
{
	int ret;
	O26A0_INTR_CPU_FLAG_T intr_flag_cpu;
	O26A0_IMX_CTRL_INTR_REG_T imx_intr_reg;
	O26A0_HDR_CTRL_INTR_CPU_REG_T hdr_intr_reg;
	UINT32 videoIntrDe;
	BOOLEAN vsyncIrq = 0;

	do {
		vsyncIrq = 0;
		ret = DE_REG_O26_UpdateVideoIrqStatus(VIDEO_INTR_TYPE_FUNC, &videoIntrDe);
		if (ret) break;
		...
		if (intr_flag_cpu.imx_mcu_intr_cpu)
		{
			vsyncIrq = 0;
			videoIntrDe = 0;
			ret = DE_REG_O26_UpdateVideoIrqStatus(VIDEO_INTR_TYPE_IMX, &videoIntrDe);
			if (ret) break;
		...
		if (intr_flag_cpu.hdr_mcu_intr_cpu)
		{
			vsyncIrq = 0;
			videoIntrDe = 0;
			ret = DE_REG_O26_UpdateVideoIrqStatus(VIDEO_INTR_TYPE_HDR, &videoIntrDe);
			if (ret) break;
		...
		if (intr_flag_cpu.cvi_mcu_intr_cpu)
		{
			vsyncIrq = 0;
			videoIntrDe = 0;
			ret = DE_REG_O26_UpdateVideoIrqStatus(VIDEO_INTR_TYPE_CVI, &videoIntrDe);
			if (ret) break;
		}
	} while (0);
	return vsyncIrq;
	...
	do {
		...
		switch (intrType) {
			...
			case VIDEO_INTR_TYPE_FUNC :
				DE_IPC_O26_RdFL(intr_cpu_flag);
				*pVideoIrqStatus = DE_IPC_O26_Rd(intr_cpu_flag);
				break;
			default :
...
int DE_REG_O26_CviMap(LX_DE_WIN_ID_T win_id, LX_DE_IN_SRC_T in_port, UINT32 port_num)
{
	...
	do {
		sel_cvi[0] = CVI_NONE_O26;
		sel_cvi[1] = CVI_NONE_O26;
		sel_clk[0] = CLK_NONE_O26;
		sel_clk[1] = CLK_NONE_O26;
		...
		switch(win_id)
		{
			...
			case LX_DE_WIN_4:
				...
				{
					sel_cvi[0] = CVI_CVD_O26;
					sel_clk[0] = CLK_CVD_O26;
				}
				if(in_port == LX_DE_IN_SRC_YPBPR || \
						in_port == LX_DE_IN_SRC_VGA)
				{
					sel_cvi[0] = CVI_ADC_O26;
					sel_clk[0] = CLK_ADC_O26;
				}
				if(in_port == LX_DE_IN_SRC_HDMI)
				{
					g_HdmiPort_O26[win_id] = port_num;  // assign for resume from instanr boot !!!
					switch(port_num)
					{
						case HDMI_O26_PORT_0:
							sel_cvi[0] = CVI_HDMI_O26_A;
							sel_cvi[1] = CVI_HDMI_O26_A;
							sel_clk[0] = CLK_HDA_O26;
							sel_clk[1] = CLK_HDA_O26;
							break;
						case HDMI_O26_PORT_1:
							sel_cvi[0] = CVI_HDMI_O26_B;
							sel_cvi[1] = CVI_HDMI_O26_B;
							sel_clk[0] = CLK_HDB_O26;
							sel_clk[1] = CLK_HDB_O26;
							break;
						case HDMI_O26_PORT_2:
							sel_cvi[0] = CVI_HDMI_O26_C;
							sel_cvi[1] = CVI_HDMI_O26_C;
							sel_clk[0] = CLK_HDC_O26;
							sel_clk[1] = CLK_HDC_O26;
							break;
						case HDMI_O26_PORT_3:
							sel_cvi[0] = CVI_HDMI_O26_D;
							sel_cvi[1] = CVI_HDMI_O26_D;
							sel_clk[0] = CLK_HDD_O26;
							sel_clk[1] = CLK_HDD_O26;
							break;
						case HDMI_O26_PORT_DSC:
							sel_cvi[0] = CVI_HDMI_O26_DSC;
							sel_cvi[1] = CVI_HDMI_O26_DSC;
							sel_clk[0] = CLK_DSC_O26;
							sel_clk[1] = CLK_DSC_O26;
							break;
						default:
							sel_cvi[0] = CVI_HDMI_O26_A;
							sel_cvi[1] = CVI_HDMI_O26_A;
...
#if 0
O26_CRG_SUB_Register_Manual.xlsx
CVI(cvi_syn)
...
#define O26_A0_SCRG_CTRL_CVI_TYPE SCRG_CTRL_CVI_REG_O26A0_T;//sys/o26a0/scrg_ctrl_cvi.h
#endif
...
typedef enum {
	...
	CVI_NONE_O26     = 0xFF
		...
		CTOP_CTRL_O26Ax_RdFL(SCRG_CTRL_CVI, crg_cvi004);//0xCC4F0010
		switch(win_id)
		{
		case LX_DE_WIN_MAIN:
			CTOP_CTRL_O26Ax_Wr01(SCRG_CTRL_CVI, crg_cvi004, reg_cvi0_clk_sel, sel_clk[0]);
			if(!g_QuadWin_O26) { // reserved for 8k path (in case of o26 8k model, it use 0/2/3/4)
				CTOP_CTRL_O26Ax_Wr01(SCRG_CTRL_CVI, crg_cvi004, reg_cvi1_clk_sel, sel_clk[0]);
			//	CTOP_CTRL_O26Ax_Wr01(SCRG_CTRL_CVI, crg_cvi004, reg_cvi2_clk_sel, sel_clk[0]);
			//	CTOP_CTRL_O26Ax_Wr01(SCRG_CTRL_CVI, crg_cvi004, reg_cvi3_clk_sel, sel_clk[0]);
			//	CTOP_CTRL_O26Ax_Wr01(SCRG_CTRL_CVI, crg_cvi004, reg_cvi4_clk_sel, sel_clk[0]);
			}
		...
		CTOP_CTRL_O26Ax_WrFL(SCRG_CTRL_CVI, crg_cvi004);
#endif
	} while(0);
...
int DE_REG_O26_SetWinPortSrc(LX_DE_INPUT_CFG_T *pstParams)
{
	...
	do {
		...
		ret = DE_REG_O26_CviMap(win_id, in_port, srcPort);

		if(win_id < NUM_WIN_MAX)
		{
			g_WinsrcMap_O26[win_id] = in_port;
			g_WinsrcPort_O26[win_id] = srcPort;
		}
...
O26_CTOP_SYN_Register_Manual.xlsx : WOC_SYN : [4] reg_main_uart_sel0
d0  : tz     
d2  : cpu0   
d3  : cpu1   
d4  : cpu2   
...
d14 : dram
	...
	CTOP_CTRL_O26Ax_RdFL(CPU_BND_CTRL_WOC, mem6);// UART0 0xCA3A6124 ??
	switch (whichUart)
	{
		case 0 :
				CTOP_CTRL_O26Ax_Wr01(CPU_BND_CTRL_WOC, mem6, reg_main_uart_sel0, 2); // UART0 = cpu0
			break;
		case 1 :
				CTOP_CTRL_O26Ax_Wr01(CPU_BND_CTRL_WOC, mem6, reg_main_uart_sel0, 6); // UART0 = pmcu0
			break;
		case 2 :
				CTOP_CTRL_O26Ax_Wr01(CPU_BND_CTRL_WOC, mem6, reg_main_uart_sel0, 7); // UART0 = pmcu1
			break;
		default :
			break;
	}
	CTOP_CTRL_O26Ax_WrFL(CPU_BND_CTRL_WOC, mem6);// UART0
#endif
	return ret;
}
...
int DE_REG_O26_FW_DumpStatus(void)
{
	...
	DE_NOTI("CPU->MCU[ 0x%08X ]\n", DTVSOC_IPC_FROM_CPU_O26_BASE);
	p_pqe_debug_info = (UINT32 *)ioremap(DTVSOC_IPC_FROM_CPU_O26_BASE, sizeof(UINT32) * size);
	for(i=0;i<size;i+=4)
	{
	...
	DE_NOTI("MCU->CPU[ 0x%08X ]\n", DTVSOC_IPC_FROM_MCU_O26_BASE);
	if(p_pqe_debug_info) iounmap(p_pqe_debug_info);

	p_pqe_debug_info = (UINT32 *)ioremap(DTVSOC_IPC_FROM_MCU_O26_BASE, sizeof(UINT32) * size);
	for(i=0;i<size;i+=4)
	{
		DE_NOTI("[%02d] 0x%08X 0x%08X 0x%08X 0x%08X\n",i,p_pqe_debug_info[i],p_pqe_debug_info[i+1],p_pqe_debug_info[i+2],p_pqe_debug_info[i+3]);
	}
...
static int DE_REG_O26_ClearTraceDataBuf(void)
{
	memset(_g_de_reg_o26_buf, 0, sizeof(DE_REG_O26_NOTI_BUF_T)*DE_REG_O26_NOTI_BUF_MAX);
	return RET_OK;
}
...
static int DE_REG_O26_BackupTraceDataBuf(char *data)
{
	UINT32 i;
	char *p_str;
	DE_REG_O26_NOTI_TIME_T *p_time;
	static UINT32 cnt = 0;

	if (!data)	return RET_OK;

	if(cnt>=DE_REG_O26_NOTI_BUF_MAX)	cnt = 0;

	p_str = _g_de_reg_o26_buf[cnt].str;
	p_time = &(_g_de_reg_o26_buf[cnt].time);
	OS_GetCurrentTicks(&(p_time->sec), &(p_time->msec), &(p_time->usec));
	memset(p_str, 0, DE_REG_O26_NOTI_STR_SIZE);

	for(i=0;i<DE_REG_O26_NOTI_STR_SIZE;i++)
	{
		p_str[i] = data[i];
		if(data[i] == 0) break;
	}
...
static int DE_REG_O26_ViewTraceDataBuf(void)
{
	...
	for (i=0; i<DE_REG_O26_NOTI_BUF_MAX; i++)
	{
		p_time = &(_g_de_reg_o26_buf[i].time);
		if(p_time->sec==0 && p_time->msec==0 && p_time->usec==0)	break;
		p_str = _g_de_reg_o26_buf[i].str;
...
int DE_REG_O26_ViewInfo(int type)
{
	...
	if (type == 0)
	{
		ret = DE_REG_O26_ViewTraceDataBuf();
		_print_tag_info();
		_print_status_info();
	}
...
static void _print_status_info(void)
{
	...
	DE_IPC_O26_RdFL(pfault_status0);
	DE_IPC_O26_RdFL(pfault_status1);
	DE_IPC_O26_RdFL(pfault_status2);
	r[0] = DE_IPC_O26_Rd(pfault_status0);
	r[1] = DE_IPC_O26_Rd(pfault_status1);
	r[2] = DE_IPC_O26_Rd(pfault_status2);
	DE_NOTI("pfault_status:0x%08x,0x%08x,0x%08x\n",r[0],r[1],r[2]);

	if (gDE_IMX_VERI_15_O26) {
		r[0] = gDE_IMX_VERI_15_O26->exception;
		r[1] = gDE_IMX_VERI_15_O26->exception1;
		DE_NOTI("exc:0x%08x,0x%08x\n",r[0],r[1]);
	}
}

static void _print_tag_info(void)
{
	DE_REG_O26_IPC_SYSTEM_T *p = &g_Mem_Info;

	#define SIZE_KB(a)    (a>>10)
	#define SIZE_MB(a)    ((SIZE_KB(a)>>10))
	DE_NOTI("tag_size = %d (max:%d)\n",(int)sizeof(DE_REG_O26_IPC_SYSTEM_T),DTVSOC_IPC_TOTAL_SIZE);
	DE_NOTI("info:0x%x, model:0x%x,0x%x,0x%x,0x%x\n",p->system_info.u32Data,p->model.u32Data,p->model1.u32Data,p->model2,p->model3);
	DE_NOTI("panel:%d,set:%d,maker:%d,res:%d,fr:%d,ext:%d\n",p->model.panel_type,p->model.set_type,p->model.panel_maker,p->model.panel_resolution,p->model.panel_frame_rate,p->model.external_chip);
	DE_NOTI("tcon:%d,carry:%d,vx1:%d,panel4:%d,out_b:%d,oledx4:%d\n",p->model.tcon_Scramble,p->model.panel_carry_over,p->model.vx1_byte,p->model1.panel_frame_rate4,p->model1.pqe_out_bit,p->model1.oled_boost_x4);
	...
	if (p->mcu0_dbg.size)    DE_NOTI("mcu0_dbg   = 0x%08x, %4dMB\n",p->mcu0_dbg.base,    SIZE_KB(p->mcu0_dbg.size));
	if (p->fw_shared.size)   DE_NOTI("fw_shared  = 0x%08x, %4dMB\n",p->fw_shared.base,   SIZE_MB(p->fw_shared.size));
	if (p->mcu1_dbg.size)    DE_NOTI("mcu1_dbg   = 0x%08x, %4dMB\n",p->mcu1_dbg.base,    SIZE_KB(p->mcu1_dbg.size));
...
static int _de_memcpy(void *dst, void *src, unsigned int size)
{
    unsigned int num, i, *p_d, *p_s;
    ...
    if (dst && src && size) {
        ...
        p_d = (unsigned int *)dst;
        p_s = (unsigned int *)src;
        for (i=0; i<num; i++) {
            p_d[i] = p_s[i];
        }

        DE_NOTI("size:%d, num:%d\n",size,num);
    }
...
{
	UINT32 tag_size = (UINT32)sizeof(g_Mem_Info);
	if (!gDE_IPC_GEN_O26) {
		DE_ERROR("gDE_IPC_GEN_O26 is null\n");
		return;
	}

	if (tag_size > DE_IPC_GEN_O26_SIZE) {
		tag_size = DE_IPC_GEN_O26_SIZE;
	}

	_de_memcpy((void *)gDE_IPC_GEN_O26,(void *)&g_Mem_Info,tag_size);
	// memcpy(gDE_IPC_GEN_O26, &g_Mem_Info,tag_size);
	_print_tag_info();
}
...
#define SYSTEM_MAGIC_WORD 0xDECAFE
	...
	memset(&tag,0,sizeof(DE_REG_O26_IPC_SYSTEM_T));
	tag.system_info.magic_word = SYSTEM_MAGIC_WORD;

#ifdef USE_VIDEO_FOR_FPGA
	tag.system_info.asic = 2/*1:ASIC,2:FPGA*/;
#else
	tag.system_info.asic = 1/*1:ASIC,2:FPGA*/;
#endif
	if ( lx_chip_rev() >= LX_CHIP_REV(O26,A1) ) {
		tag.system_info.chip_version = 1; // A0:0, A1:1
	}

	tag.model.u32Data = opt[0];
	tag.model1.u32Data = opt[1];

	tag.fw_mcu0.base   = g_fw_mcu0.base;
	tag.fw_mcu0.size   = g_fw_mcu0.size;
	tag.de_svp_m0.base = g_de_svp_m0.base;
	tag.de_svp_m0.size = g_de_svp_m0.size;
	tag.de_svp_m1.base = g_de_svp_m1.base;
	tag.de_svp_m1.size = g_de_svp_m1.size;
	tag.de_svp_m2.base = g_de_svp_m2.base;
	tag.de_svp_m2.size = g_de_svp_m2.size;
	tag.de_nsvp_m1.base = g_de_nsvp_m1.base;
	tag.de_nsvp_m1.size = g_de_nsvp_m1.size;
	tag.de_nsvp_m2.base = g_de_nsvp_m2.base;
	tag.de_nsvp_m2.size = g_de_nsvp_m2.size;
	tag.de_vt_m1.base  = g_de_vt_m1.base;
	tag.de_vt_m1.size  = g_de_vt_m1.size;
	tag.de_vt_m2.base  = g_de_vt_m2.base;
	tag.de_vt_m2.size  = g_de_vt_m2.size;

	tag.de_hdr.base    = g_de_hdr.base;
	tag.de_hdr.size    = g_de_hdr.size;

	tag.be_fpp.base    = g_be_fpp.base;
	tag.be_fpp.size    = g_be_fpp.size;
	tag.be_frc0.base   = g_be_frc0.base;
	tag.be_frc0.size   = g_be_frc0.size;
	tag.be_frc1.base   = g_be_frc1.base;
	tag.be_frc1.size   = g_be_frc1.size;
	tag.be_frc2.base   = g_be_frc2.base;
	tag.be_frc2.size   = g_be_frc2.size;
	tag.be_wcg.base    = g_be_wcg.base;
	tag.be_wcg.size    = g_be_wcg.size;
	tag.be_led.base    = g_be_led.base;
	tag.be_led.size    = g_be_led.size;
	tag.be_phdr.base   = g_be_phdr.base;
	tag.be_phdr.size   = g_be_phdr.size;
	tag.be_chipinfo.base = g_chipinfo.base;
	tag.be_chipinfo.size = g_chipinfo.size;

	tag.pe_db.base     = g_pe_db.base;
	tag.pe_db.size     = g_pe_db.size;
	tag.pe_hdr.base    = g_pe_hdr.base;
	tag.pe_hdr.size    = g_pe_hdr.size;

	tag.fw_dnn.base    = g_fw_dnn.base;
	tag.fw_dnn.size    = g_fw_dnn.size;
	tag.fw_mcu1.base   = g_fw_mcu1.base;
	tag.fw_mcu1.size   = g_fw_mcu1.size;
	tag.fw_mcu2.base   = g_fw_mcu2.base;
	tag.fw_mcu2.size   = g_fw_mcu2.size;
	tag.mcu0_dbg.base  = g_mcu0_dbg.base;
	tag.mcu0_dbg.size  = g_mcu0_dbg.size;
	tag.fw_shared.base = g_fw_shared.base;
	tag.fw_shared.size = g_fw_shared.size;
	tag.mcu1_dbg.base  = g_mcu1_dbg.base;
	tag.mcu1_dbg.size  = g_mcu1_dbg.size;

	tag.de_dbg_m1.base = g_de_dbg_m1.base;
	tag.de_dbg_m1.size = g_de_dbg_m1.size;
	tag.de_dbg_m2.base = g_de_dbg_m2.base;
	tag.de_dbg_m2.size = g_de_dbg_m2.size;

	tag.fpp_info.base  = g_fw_shared.base + VIDEO_O26_FW_SHARED_OFST_FPP;
	tag.fpp_info.size  = VIDEO_O26_FW_SHARED_SIZE_FPP;

	tag.shared_mem_info.video_delay_info.base = g_fw_shared.base + VIDEO_O26_FW_SHARED_OFST_DLY;
	tag.shared_mem_info.video_delay_info.size = VIDEO_O26_FW_SHARED_SIZE_DLY;

	tag.shared_mem_info.de_gps.base = g_fw_shared.base + VIDEO_O26_FW_SHARED_OFST_GPS;
	tag.shared_mem_info.de_gps.size = VIDEO_O26_FW_SHARED_SIZE_GPS;
	//tag.de_gps_crc.base = tag.shared_mem_info.de_gps.base + VIDEO_O26_FW_SHARED_SIZE_GPS - 8;
	//tag.de_gps_crc.size = 8;

	if ( 0 == hma_pool_info ("pqe-lne", (phys_addr_t *)&hma_addr, &hma_size) )
	{
		tag.lne_pool.base = (UINT32)(hma_addr & 0xFFFFFFFF);
		tag.lne_pool.size = hma_size;
	}
	...
	do {
		...
		if (valid_i == 0) {
			DE_REG_O26_NOTI_BACKUP("no valid vdec_pool : skip\n");
			break;
		}
		...
		DE_REG_O26_NOTI_BACKUP("vdec_pool:0x%08x ~ 0x%08x (%dMB)\n",(UINT32)(pool_start & 0xFFFFFFFF),(UINT32)(pool_end & 0xFFFFFFFF),hma_size>>20);

		tag.vdec_pool_0.base = (UINT32)(hma_addr & 0xFFFFFFFF);
		tag.vdec_pool_0.size = hma_size;

		lpAddr_win_update	= (LX_DE_VIDEO_DELAY_INFO_MEM_T *)vmap_phys(tag.shared_mem_info.video_delay_info.base,  tag.shared_mem_info.video_delay_info.size);
	} while(0);

	memcpy(&g_Mem_Info,&tag,sizeof(DE_REG_O26_IPC_SYSTEM_T));

	// backup_mem_sys_info
	do {
		LX_DE_MEM_SYS_INFO_T *p = &g_Mem_Sys_Info;
		p->de_prew.base      = tag.de_svp_m0.base;
		p->de_prew.size      = tag.de_svp_m0.size;
		p->de_frm_m1.base    = tag.de_svp_m1.base;
		p->de_frm_m1.size    = tag.de_svp_m1.size;
		p->de_frm_m2.base    = tag.de_svp_m2.base;
		p->de_frm_m2.size    = tag.de_svp_m2.size;
		p->de_vt_m1.base     = tag.de_vt_m1.base;
		p->de_vt_m1.size     = tag.de_vt_m1.size;
		p->de_vt_m2.base     = tag.de_vt_m2.base;
		p->de_vt_m2.size     = tag.de_vt_m2.size;
		p->de_hdr.base       = tag.de_hdr.base;
		p->de_hdr.size       = tag.de_hdr.size;
		p->vdec_pool_0.base  = tag.vdec_pool_0.base;
		p->vdec_pool_0.size  = tag.vdec_pool_0.size;
		p->vdec_pool_1.base  = tag.vdec_pool_1.base;
		p->vdec_pool_1.size  = tag.vdec_pool_1.size;
		p->de_gps.base       = tag.shared_mem_info.de_gps.base;
		p->de_gps.size       = tag.shared_mem_info.de_gps.size;
		p->de_gps_crc.base   = 0;//tag.de_gps_crc.base;
		p->de_gps_crc.size   = 0;//tag.de_gps_crc.size;
	} while(0);
}

#define DE_REG_O26_FLUSH_TIMEOUT_CNT  (50)
#define DE_REG_O26_FLUSH_TIMEOUT_OK   (2)
static int _de_reg_o26_chk_flush_rst_ctrl_status(void)
{
	int ret = RET_OK;
	O26A0_SET_REG_CTRL0_T set_reg_ctrl0;//0xCC080000
	...
	while (1) {
		...
		DE_IPC_O26_RdFL(set_reg_ctrl0);
		set_reg_ctrl0.udata32 = DE_IPC_O26_Rd(set_reg_ctrl0);
		if (set_reg_ctrl0.flush_rst_ctrl_status == 0) {
			ok_cnt++;
			//DE_NOTI("flush ok(retry:%d)\n",retry_cnt);
			if (ok_cnt >= DE_REG_O26_FLUSH_TIMEOUT_OK) {
				DE_REG_O26_NOTI_BACKUP("flush done(retry:%d)(%d)\n",retry_cnt,ok_cnt);
				break;
			}
		}

		if (retry_cnt >= DE_REG_O26_FLUSH_TIMEOUT_CNT) {
			DE_REG_O26_NOTI_BACKUP("flush time out(retry:%d)(%d)\n",retry_cnt,ok_cnt);
			ret = RET_TIMEOUT;
			break;
		}
...
static int _de_reg_o26_mcu_stall_on(int idx)
{
	int is_2nd_try = 0;
	O26A0_SROM_BOOT_MAP1_T srom_boot_map1;//0xCC080028
	O26A0_SROM_BOOT_MAP3_T srom_boot_map3;//0xCC0800F4

	/* do idx 1 only (download order : fw1, fw0) */
	if (idx != 1) return 0;

	DE_IPC_O26_RdFL(srom_boot_map1);
	DE_IPC_O26_RdFL(srom_boot_map3);
	srom_boot_map1.udata32 = DE_IPC_O26_Rd(srom_boot_map1);
	srom_boot_map3.udata32 = DE_IPC_O26_Rd(srom_boot_map3);
	is_2nd_try = (srom_boot_map1.udata32 == 0x50000000 && srom_boot_map3.udata32 == 0x50000000)? 0:1;
	// DE_REG_O26_NOTI_BACKUP("is_2nd_try:%d(0x%08x)\n",is_2nd_try,srom_boot_map1.udata32);

	if (is_2nd_try) {
		DE_IPC_O26_FLWr(ext_intr_enable,0);
		DE_IPC_O26_FLWr(int_intr_enable,0);
		DE_IPC_O26_FLWr(intr_mcu_flag_en,0);
		DE_IPC_O26_FLWr(intr_cpu_flag_en,0);
		DE_IPC_O26_FLWr(intr_mcu1_flag_en,0);

		if (gDE_MCU_WDOG0_O26) {
			gDE_MCU_WDOG0_O26->int_clr = 0x1;
			gDE_MCU_WDOG0_O26->control = 0x0;
			// gDE_MCU_WDOG0_O26->load = 0xffffffff;
		}

		if (gDE_MCU_WDOG1_O26) {
			gDE_MCU_WDOG1_O26->int_clr = 0x1;
			gDE_MCU_WDOG1_O26->control = 0x0;
			// gDE_MCU_WDOG1_O26->load = 0xffffffff;
		}

		msleep(1);

		DE_IPC_O26_RdFL(set_reg_ctrl0);
		DE_IPC_O26_Wr01(set_reg_ctrl0,run_stall,1);
		DE_IPC_O26_Wr01(set_reg_ctrl0,run_stall1,1);
		DE_IPC_O26_WrFL(set_reg_ctrl0);
		msleep(1);

		DE_IPC_O26_RdFL(set_reg_ctrl0);
		DE_IPC_O26_Wr01(set_reg_ctrl0,soft_reset,1);
		DE_IPC_O26_WrFL(set_reg_ctrl0);
		DE_REG_O26_NOTI_BACKUP("wintr off,reset,stall:on\n");

		_de_reg_o26_chk_flush_rst_ctrl_status();
	}

	return 0;
}

static int _de_reg_o26_mcu_stall_off(int idx, UINT32 base)
{
	switch (idx)
	{
		case 0 :
			//if (fwBaseAddr < USE_VIDEO_MCU_ROM_BASE_ADDR) BREAK_WRONG(fwBaseAddr);
			DE_IPC_O26_FLWr(srom_boot_map1, base);
			DE_IPC_O26_FLWr(srom_boot_map2, base);
			DE_IPC_O26_RdFL(set_reg_ctrl0);
			DE_IPC_O26_Wr01(set_reg_ctrl0, start_vector_sel, 0);
			DE_IPC_O26_Wr01(set_reg_ctrl0, mask_pslverr, 0xf);
			// removed DE_IPC_O26_Wr01(set_reg_ctrl0, mcu_sw_reset, 1);

			// msleep(1); // wait ddr to ddr transition
			DE_IPC_O26_Wr01(set_reg_ctrl0, p_debug_enable, 1);
			DE_IPC_O26_Wr01(set_reg_ctrl0, run_stall, 0);
			DE_IPC_O26_WrFL(set_reg_ctrl0);
			break;
		case 1 :
			//if (fwBaseAddr < USE_VIDEO_MCU_ROM_BASE_ADDR) BREAK_WRONG(fwBaseAddr);
			DE_IPC_O26_FLWr(srom_boot_map3, (base-0x400000));
			DE_IPC_O26_FLWr(srom_boot_map4, (base-0x400000));
			DE_IPC_O26_RdFL(set_reg_ctrl0);
			DE_IPC_O26_Wr01(set_reg_ctrl0, start_vector_sel1, 0);
			// removed DE_IPC_O26_Wr01(set_reg_ctrl0, mcu_sw_reset, 1);

			//msleep(1); // wait ddr to ddr transition
			DE_IPC_O26_Wr01(set_reg_ctrl0, p_debug_enable1, 1);
			DE_IPC_O26_Wr01(set_reg_ctrl0, run_stall1, 0);
			DE_IPC_O26_WrFL(set_reg_ctrl0);
			break;
		default :
			return -1;
	...
	DE_REG_O26_NOTI_BACKUP("stall off(fw%d)(0x%x)(0x%x)\n",idx,base,gDE_IPC_O26.shdw.a0->set_reg_ctrl0.udata32);
	return 0;
}

static int _de_reg_o26_mcu_update_pc(struct vsc_fw_load_param *param)
{
	if (!param) {
		DE_ERROR("null param\n");
		return -1;
	...
	{
		case 0 :
			DE_IPC_O26_RdFL(pdebug_pc);
			param->pc = DE_IPC_O26_Rd(pdebug_pc);
			break;
		case 1 :
			DE_IPC_O26_RdFL(pdebug_pc1);
			param->pc = DE_IPC_O26_Rd(pdebug_pc1);
			break;
		default:
			return -1;
	}
	...
	if (param->idx == 0) {
		struct vsc_fw_load_param *p0 = &_g_de_reg_o26_fw_load_p[0];
		struct vsc_fw_load_param *p1 = &_g_de_reg_o26_fw_load_p[1];
...
static int _de_reg_o26_mcu_update_exce(struct vsc_fw_load_param *param)
{
	if (!param) {
		DE_ERROR("null param\n");
		return -1;
	...
	if (param->idx == 0) {
		param->exce = gDE_IMX_VERI_15_O26->exception;
	}
	else if (param->idx == 1) {
		param->exce = gDE_IMX_VERI_15_O26->exception1;
	}
	else {
...
static int _de_reg_o26_mcu_mem_cpy(UINT32 src, UINT32 dst, UINT32 size)
{
	DE_O26_EDMA_EDMA_CTRL_T edma_ctrl;//0xCC0C0014

	DE_NOTI("src:0x%08x, dst:0x%08x, size:%d\n",src,dst,size);

	if (gDE_MCU_EDMA_O26 == NULL) {DE_ERROR("null gDE_MCU_EDMA_O26\n");return -1;}
	if (!src) {DE_ERROR("null src\n");return -1;}
	if (!dst) {DE_ERROR("null dst\n");return -1;}
	if (!size) {DE_ERROR("no size\n");return -1;}

	edma_ctrl.udata32 = gDE_MCU_EDMA_O26->edma_ctrl.udata32;
	if (edma_ctrl.tx_done == 0) DE_WARN("edma busy\n");

	gDE_MCU_EDMA_O26->tx_src_addr.udata32 = src;
	gDE_MCU_EDMA_O26->tx_dst_addr.udata32 = dst;
	gDE_MCU_EDMA_O26->tx_length.udata32 = size;
	edma_ctrl.tx_start = 1;
	gDE_MCU_EDMA_O26->edma_ctrl.udata32 = edma_ctrl.udata32;

	return 0;
}

static int _de_reg_o26_fwload_handler(struct vsc_fw_load_param *param, int action)
{
	int ret = 0;
	...
	if (action == VSC_FW_LOAD_STOP) {
		DE_REG_O26_NOTI_BACKUP("stop(%s)(%d)\n",param->name,param->idx);
		_de_reg_o26_mcu_stall_on(param->idx);
	}
	else if (action == VSC_FW_LOAD_PREPARE) {
		DE_REG_O26_NOTI_BACKUP("prepare(%s)(%d)\n",param->name,param->idx);
		_de_reg_o26_mcu_stall_on(param->idx);
		if (param->idx == 0) {
			_resume_tag_info();
			DE_IPC_O26_ctop_shared_memory_init();
		}
	}
	else if (action == VSC_FW_LOAD_COMPLETE) {
		DE_REG_O26_NOTI_BACKUP("complete(%s)(%d)(0x%08x)\n",param->name,param->idx,param->rom_base);
		_de_reg_o26_mcu_stall_off(param->idx,param->rom_base);
	}
	else if (action == VSC_FW_MEM_CPY) {
		_de_reg_o26_mcu_mem_cpy(param->mem_src,param->mem_dst,param->mem_size);
	}
	else if (action == VSC_FW_UPDATE_PC) {
		_de_reg_o26_mcu_update_pc(param);
	}
	else if (action == VSC_FW_UPDATE_EXCE) {
		_de_reg_o26_mcu_update_exce(param);
	}
...
int de_reg_o26_fwload_register(struct vsc_fw_load *load)
{
	...
	load->handler = _de_reg_o26_fwload_handler;

	p = &_g_de_reg_o26_fw_load_p[0];
	p->secure = de_cfg_o26_is_mcu_se_map(p->idx);
	p->dst_base = (phys_addr_t)g_fw_mcu0.base;
	p->int_pool = g_m0_pqe_nsv.name;
	p->int_base = (phys_addr_t)g_m0_pqe_nsv.base;
	load->param[0] = p;

	p = &_g_de_reg_o26_fw_load_p[1];
	p->secure = de_cfg_o26_is_mcu_se_map(p->idx);
	...
	DE_REG_O26_NOTI_BACKUP("fwload_init done\n");
...
int DE_REG_O26_FW_Download(LX_DE_FW_DWLD_T *pstParams)
{
	...
	do {
		...
		DE_REG_O26_NOTI_BACKUP("fw_dl start(fw:%d)(cfg:%d)(size:%d)\n",pstParams->inx,pstParams->cfg,pstParams->size);
		...
		if (pFwMem==NULL)	{DE_REG_O26_NOTI_BACKUP("fw_dl not support(fw:%d)\n",pstParams->inx);break;}
		if (!pFwMem->base)	{DE_REG_O26_NOTI_BACKUP("fw_dl skip(base0)(fw:%d)\n",pstParams->inx);break;}
		if (!pFwMem->size)	{DE_REG_O26_NOTI_BACKUP("fw_dl skip(size0)(fw:%d)\n",pstParams->inx);break;}

		_de_reg_o26_mcu_stall_on(pstParams->inx);
		...
		do {
			...
			DE_REG_O26_NOTI_BACKUP("fw_dl load(fw%d)(addr:0x%08x+0x%x)(size:%d)\n",pstParams->inx,fwBaseAddr,fwBaseOfst,fwSize);

			if (pFwMem->size < fwSize)
			{
				DE_REG_O26_NOTI_BACKUP("fw_dl skip loading(abnormal size)\n");
				break;
			}

			if (pstParams->cfg==DE_FW_CFG_CPY_TO_DDR_SKIP)
			{
				DE_REG_O26_NOTI_BACKUP("fw_dl skip loading(run fw)\n");
				break;
			}
			...
			if (pstParams->cfg==DE_FW_CFG_COPY_BIN_TO_DDR)
			{
				...
				vsc_filp->f_pos = 0;
				ret = kernel_read(vsc_filp, (char*)((uintptr_t)lpAddr + (uintptr_t)fwBaseOfst),pstParams->size,&vsc_filp->f_pos);
				DE_REG_O26_NOTI_BACKUP("fw_dl kernel_read ret(%d)\n",ret);
			...
			else if (pstParams->cfg==DE_FW_CFG_COPY_USR_TO_DDR)
			{
				...
				DE_REG_O26_NOTI_BACKUP("fw_dl copy_from_user ret(%d)\n",ret);
			}
			else
			{
		...
		_de_reg_o26_mcu_stall_off(pstParams->inx,fwBaseAddr);

		DE_REG_O26_NOTI_BACKUP("fw_dl done(fw:%d)\n",pstParams->inx);
...
BOOLEAN DE_REG_O26_IPCisAlive(void)
{
	...
	DE_IPC_O26_FLRf(int_intr_enable, ipc_interrupt_enable_mcu, isFwDownloaded);
...
int DE_REG_O26_SetVTM(LX_DE_VTM_FRAME_INFO_T *pstParams)
{
	...
	do {
		CHECK_KNULL(pstParams);
		pstParams->address = g_fw_shared.base + VIDEO_O26_FW_SHARED_OFST_VTM;
	} while (0);
...
int DE_REG_O26_SetGeneralCapturePath(LX_DE_GCP_INFO_T *pstParams)
{
	...
	do {
		CHECK_KNULL(pstParams);
		pstParams->address = g_fw_shared.base + VIDEO_O26_FW_SHARED_OFST_VTV; // base of VTV shared mem
	} while (0);
 ...
 * get irq number
 */
int DE_REG_O26_GetIrqNum(UINT32 mcu_id, UINT32 *ipc_irq_num, UINT32 *sync_irq_num)
{
	switch(mcu_id)
	{
		case 0:
			*ipc_irq_num  = O26_IRQ_IPC_BCPU;
			*sync_irq_num = O26_IRQ_DE_BCPU;
			break;
		case 1:
			break;
		default:
...
{
	if (!irq_num || num < 2) {
		DE_ERROR("irq_num:null or num(%d):wrong\n",num);
		return RET_ERROR;
	}

	memset((void *)irq_num,0,sizeof(UINT32)*num);
	irq_num[0] = O26_IRQ_INTR_PQE_IPC2_CPU;
	irq_num[1] = O26_IRQ_INTR_PQE_IPC3_CPU;
	return RET_OK;
}
	...
	do {
		...
		switch (pstParams->config_type) {
			...
			case LX_DE_CONFIG_TYPE_DISPLAY_MIRROR:
				g_Display_mirror_O26 = pstParams->display_mirror;
				break;
			default :
				break;
		}
	...
	do {
		...
		switch (pstParams->status_type) {
			...
			case LX_DE_SYS_STATUS_DISPALY_MIRROR:
				pstParams->display_mirror = g_Display_mirror_O26;
				break;
			default :
				break;
		}
...
static int DE_REG_O26_GetMviInfo_CheckValid(void)
{
	int ret = RET_OK;
#define DE_REG_O26_MVI_INFO_MAGIC	0xDECAFE

	if (gDE_DDR_MVI_O26 == NULL)	return RET_ERROR;

	if (_g_de_reg_o26_mvi_magic != DE_REG_O26_MVI_INFO_MAGIC)
	{
		rmb();
		_g_de_reg_o26_mvi_magic = gDE_DDR_MVI_O26->magic;
	}

	ret = (_g_de_reg_o26_mvi_magic == DE_REG_O26_MVI_INFO_MAGIC)? RET_OK:RET_ERROR;

	DE_TRACE_20("magic : 0x%08x (ret : %d)\n",_g_de_reg_o26_mvi_magic,ret);

	return ret;
}

static int DE_REG_O26_GetMviInfo_lagecy(LX_DE_SRC_STATUS_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		p_vdec_ipc_reg = gDE_VDEC_O26[vdec_port];
		...
		{
			UINT32 idx,ins_id;
			DE_REG_O26_IPC_DAR_INFO_T shdw;
			idx = (pstParams->win_id == 2)? 0:1;

			memcpy(&shdw,(const void *)gDE_DAR_INFO_O26[idx],sizeof(DE_REG_O26_IPC_DAR_INFO_T));
...
static int DE_REG_O26_GetMviInfo(LX_DE_SRC_STATUS_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		rmb();
		phys = &gDE_DDR_MVI_O26->mvi[win_id];
	...
	do {
		...
		switch (pstParams->status_type) {
			case LX_DE_SRC_STATUS_ALL :
				pstParams->inSrc[LX_DE_WIN_MAIN] = g_WinsrcMap_O26[LX_DE_WIN_MAIN];
				pstParams->inSrcPort[LX_DE_WIN_MAIN] = g_WinsrcPort_O26[LX_DE_WIN_MAIN];
				pstParams->inSrc[LX_DE_WIN_SUB]	 = g_WinsrcMap_O26[LX_DE_WIN_SUB];
				pstParams->inSrcPort[LX_DE_WIN_SUB] = g_WinsrcPort_O26[LX_DE_WIN_SUB];

				pstParams->operType= g_SrcOperType_O26;
				pstParams->operCtrlFlag = g_SrcOperCtrlFlag_O26;

				pstParams->subOperType =  g_SrcSubOperType_O26;
				pstParams->subOperCtrlFlag = g_SrcSubOperCtrlFlag_O26;
				break;
			...
			case LX_DE_SRC_STATUS_VDEC_INFO :
				if (RET_OK == DE_REG_O26_GetMviInfo_CheckValid())
				{
					ret = DE_REG_O26_GetMviInfo(pstParams);
				}
				else
				{
					ret = DE_REG_O26_GetMviInfo_lagecy(pstParams);
				}
				break;
...
int DE_REG_O26_SetOperation(LX_DE_OPERATION_CTRL_T *pstParams)
{
	...
	do {
		...
		g_SrcOperType_O26 = type;
		g_SrcOperCtrlFlag_O26 = flag;
	} while (0);
	...
	do {
		...
		switch (pstParams->operation) {
			case LX_DE_SUB_OPER_OFF :
				g_SrcSubOperType_O26 = LX_DE_SUB_OPER_OFF;
				break;
			case LX_DE_SUB_OPER_CAPTURE:
				g_SrcSubOperType_O26 = LX_DE_SUB_OPER_CAPTURE;
				flag = pstParams->capture_enable;
				break;
			case LX_DE_SUB_OPER_VENC:
				g_SrcSubOperType_O26 = LX_DE_SUB_OPER_VENC;
				flag = pstParams->vencCtrl.bOnOff;
				break;
			case LX_DE_SUB_OPER_SCART_OUT:
				g_SrcSubOperType_O26 = LX_DE_SUB_OPER_SCART_OUT;
				flag = 1;
				break;
...
int DE_REG_O26_GetFIR(LX_DE_CVI_SRC_TYPE_T *pstParams, LX_DE_CVI_FIR_T *fir)
{
	...
	UINT16 coef_FIR_O26_CVD_CbCr_Normal[] = {160, 49, -1, 0, 0, 0};// CVI FIR Filter for 1 pixel & 422
				...
				for (i=0; i<NUM_OF_CVI_FIR_COEF; i++) {
					...
					if (i<(NUM_OF_CVI_FIR_COEF-2)) {
						if (pstParams->cvi_input_src == LX_DE_CVI_SRC_ATV || pstParams->cvi_input_src == LX_DE_CVI_SRC_CVBS)
							fir->fir_coef_CbCr[i] = coef_FIR_O26_CVD_CbCr_Normal[i];
						else
...
int DE_REG_O26_SetHdmiPort(LX_DE_HDMIPORT_T *pstParams)
{
	...
	g_HdmiPort_O26[pstParams->win_id] = pstParams->port_num;  // assign from vp_kadp
...
static int _g_de_reg_o26_vtvr_size_m0 = (25*1024*1024);
static int _g_de_reg_o26_vtvr_size_m1 = (50*1024*1024);
int DE_REG_O26_GetMemFromCma(char *mem_name, int count, void *info, BOOLEAN flag)
{
	...
	{
		size_m0 = _g_de_reg_o26_vtvr_size_m0;
		size_m1 = _g_de_reg_o26_vtvr_size_m1;
...
LX_MEMCFG_T de_reg_o26_getmeminfo(char* str)
{
	...
	if (!strncasecmp(str, "vdec_pool_0", strlen("vdec_pool_0"))) {
		...
		if (strstr(str,"m1")) {
			meminfo.size = _g_de_reg_o26_vtvr_size_m1;
		}
		else {
			meminfo.size = _g_de_reg_o26_vtvr_size_m0;
		}
	}
...
int DE_REG_O26_SetMultiWin(UINT32 *pstParams)
{
	...
	g_QuadWin_O26 = (*pstParams==4)?TRUE:FALSE;
...
int DE_REG_O26_GetLowDelayClock(UINT32 *pstParams)
{
	...
	*pstParams = g_Zero_delay_mode_O26;
...
int DE_REG_O26_SetLowDelayClock(UINT32 *pstParams)
{
	...
	ret = DE_REG_O26_SetClockPath((BOOLEAN)*pstParams);

	g_Zero_delay_mode_O26 = *pstParams; // 0:OFF, 1:5V_HPD, 2:CVI_INTR

	DE_NOTI("g_Zero_delay_mode_O26 = %d  (0:OFF, 1:5V_HPD, 2:CVI_INTR)", g_Zero_delay_mode_O26);
...
int DE_REG_O26_GetFPPInfo(UINT32 *start_address)
{
	...
	*start_address = g_fw_shared.base + VIDEO_O26_FW_SHARED_OFST_FPP;

	DE_NOTI(" start_address = 0x%08x , size = 0x%08x", *start_address, VIDEO_O26_FW_SHARED_SIZE_FPP);
...
int DE_REG_O26_GetDNNStatus(UINT32 *a, UINT32 *b, UINT32 *c, UINT32 *d, UINT32 *e, UINT32 *f)
{
	int ret = RET_OK;
	UINT32 *lpAddr = NULL;
	UINT32 phys = g_Mem_Info.fpp_info.base + VIDEO_O26_FW_SHARED_OFST_FPP_DNN;// fpp_info.base + 32*1024
	// O26 phys = ?
...
int DE_REG_O26_GetInitData(LX_DE_IPC_INIT_DATA_T *pstParams)
{
	...
	if (gDE_DDR_MVI_O26 == NULL) {DE_NOTI("gDE_DDR_MVI_O26 null\n");return RET_ERROR;}

	do {
		CHECK_KNULL(pstParams);
		memset((void *)gDE_DDR_MVI_O26,0,sizeof(LX_DE_DDR_MVI_T));
		wmb();

		pstParams->head.magic = 1;
		pstParams->head.idx_num = 1;

		pstParams->u[0].data = g_fw_shared.base + VIDEO_O26_FW_SHARED_OFST_MVI;
		pstParams->u[0].size = VIDEO_O26_FW_SHARED_SIZE_MVI;

		_g_de_reg_o26_mvi_magic = 0;
