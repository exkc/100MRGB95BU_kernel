...
static void dw_spi_hw_init(struct device *dev, struct dw_spi *dws)
{
	...
	if (dws->rx_dly) {
		dw_spi_set_rx_dly(dws);
	}

	if (dws->nstrength == 1) {
		val = readl(dws->reg_strength[0]);
		val &= ~0x000f0f00;
		val |= (dws->strength[0]) << 16 | (dws->strength[0]) << 8;
		writel(val, dws->reg_strength[0]);
	} else if (dws->nstrength > 1) {
		for(i = 0; i< dws->nstrength / 2; i++){
			val = readl(dws->reg_strength[i]);
			val &= ~(dws->strength[2*i]);
			val |= dws->strength[2*i+1];
			writel(val, dws->reg_strength[i]);
		}
	}
