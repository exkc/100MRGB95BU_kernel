...
void kbase_csf_tiler_heap_reclaim_mgr_init(struct kbase_device *kbdev)
{
	...
	reclaim_mgr = &scheduler->reclaim_mgr;
	reclaim_mgr->heap_reclaim = shrinker_alloc(0, "mali-mem-pool");
	reclaim_mgr->heap_reclaim->count_objects = kbase_csf_tiler_heap_reclaim_count_objects;
	reclaim_mgr->heap_reclaim->scan_objects = kbase_csf_tiler_heap_reclaim_scan_objects;
	reclaim_mgr->heap_reclaim->private_data = kbdev;
	}
#endif
#endif
}
...
void kbase_csf_tiler_heap_reclaim_mgr_term(struct kbase_device *kbdev)
{
	...
	struct kbase_csf_sched_heap_reclaim_mgr *reclaim_mgr;
	reclaim_mgr = &scheduler->reclaim_mgr;
	shrinker_free(reclaim_mgr->heap_reclaim);
	}
#endif
#endif
