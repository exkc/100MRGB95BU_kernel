...
#define LBUS_QOS_CMN_BASE			(0xC9860000)
#define GBUS_QOS_CMN_BASE			(0xC97F0000)

//#define mpro_reg_read(addr)				__raw_readl((void*)(addr))
//#define mpro_reg_write(val,addr)		__raw_writel(val, (void*)(addr))

#define mpro_reg_write_wa(val,addr,phy_base,clear) \
	do{\
		if(((u64)phy_base >> 16) != 0xc97f) { \
			__raw_writel(val, (void*)(addr)); \
		}\
		else {\
			u32 t_val; \
			__raw_readl((void*)(addr)); \
			/*LGBUS_INFO("mpro_reg_write_wa(0x%08x, v:0x%08lx, p:0x%08lx, %d)\n", val, addr, phy_base, clear);*/ \
			while(1) {\
				__raw_writel(val, (void*)(addr)); \
				__raw_writel(val, (void*)(addr)); \
				t_val = __raw_readl((void*)(addr)); \
				t_val = __raw_readl((void*)(addr)); \
				if(val == t_val)\
					break;\
				else {\
					if(clear == 1)\
						break;\
					LGBUS_ERROR("reg retry 0x%08x,0x%08x v:%08lx p:0x%08lx, clear:%d\n" \
									,val,t_val,(ulong)addr,(ulong)phy_base, clear); \
				}\
			}\
		}\
	}while(0)
...
static lgbus_o18_mpro_t mpro_ctrl[] =
{
	{ "CORE",		0xC7FF1200 },
	{ "AXI_PERI",	0xC7FF1400 },
	{ "AHB_PERI",	0xC7FF1600 },
...
========================================================================================*/
/* o18 gbus apb WA */
inline static u32 mpro_reg_read(ulong addr)
{
	__raw_readl((void*)(addr));
...
static int lgbus_mpro_resume(void)
{
#ifdef SUPPORT_O18_MPRO_CHIP_KDRV
	int ret;
...
static int lgbus_mpro_get_range(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		for(j = 0; j < O18_MPRO_RANGE_SLOT_CNT; j++)
		{
 			// 28~8:21bit
			start = (u64)((mpro_reg_read((u64)(range_addr++)) >> 8) & 0x1FFFFF) << 12;
			end = ((u64)((mpro_reg_read((u64)(range_addr++)) >> 8) & 0x1FFFFF) << 12 ) | 0xFFF;
			if(end == 0xfff) end = 0x0;

			mpro_ctrl[i].range[j].start = start;
			mpro_ctrl[i].range[j].end = end;

			LGBUS_PRINT("[%-10s] mpro range(%d) 0x%09llx ~ 0x%09llx\n", mpro_ctrl[i].name
																, j
...
static int lgbus_mpro_regs_save(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		memcpy_fromio(&(mpro_ctrl[i].reg_save), save_addr, sizeof(lgbus_o18_mpro_reg_save_t));
	}
	ret = RET_OK;
...
static int lgbus_mpro_regs_restore(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		memcpy_toio(save_addr, &(mpro_ctrl[i].reg_save), sizeof(lgbus_o18_mpro_reg_save_t));
	}
	ret = RET_OK;
...
#undef LBUS_IRQ
#undef GBUS_IRQ
#define LBUS_IRQ	O18_IRQ_INTR_LBUS
#define GBUS_IRQ	O18_IRQ_INTR_GBUS

#undef CPU_IRQ
#undef CPU_AXI_PERI_IRQ
#undef CPU_AHB_PERI_IRQ
#undef CPU_LZMA_IRQ
#define CPU_IRQ				O18_IRQ_QOS_CPU
#define CPU_AXI_PERI_IRQ	O18_IRQ_QOS_AXI_PERI
#define CPU_AHB_PERI_IRQ	O18_IRQ_QOS_AHB_PERI
#define CPU_LZMA_IRQ		O18_IRQ_QOS_LZMA
...
static int lgbus_mpro_isr_enable(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		val |= MPRO_ISR_ENABLE;
		mpro_reg_write_wa(val, addr, mpro_ctrl[i].base_addr_phy, 0);
	...
	mpro_reg_write_wa(0xffffffff, lbus_qos_cmn_base + MPRO_ISR_ENABLE_OFFSET, LBUS_QOS_CMN_BASE, 0);
	mpro_reg_write_wa(0xffffffff, gbus_qos_cmn_base + MPRO_ISR_ENABLE_OFFSET, GBUS_QOS_CMN_BASE, 0);
	...
	if(irq == LBUS_IRQ || irq == GBUS_IRQ || irq == CPU_IRQ || irq == CPU_AXI_PERI_IRQ || irq == CPU_AHB_PERI_IRQ || irq == CPU_LZMA_IRQ)
	{
		...
		for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
		{
			...
			if(irq_status & MPRO_ISR_STATUS)
			{
				for(j = 0; j < O18_MPRO_RANGE_SLOT_CNT; j++)
				{
					LGBUS_INFO("[%-10s] mpro range(%d) 0x%09llx ~ 0x%09llx\n", mpro_ctrl[i].name
							, j
							, mpro_ctrl[i].range[j].start
							, mpro_ctrl[i].range[j].end);
				}

				{
					...
					struct
					{
						...
						u32 wr_vio_addr;
						u32 wr_vio_id;
					}rw_vio_data[O18_MPRO_RANGE_SLOT_CNT];
					...
					if(irq_status & MPRO_ISR_STATUS_WR)
					{
						for(j = 0; j < O18_MPRO_RANGE_SLOT_CNT; j++)
						{
							...
							{
								LGBUS_ERROR(COLOR_RED"[%-10s] Detect write violation!. ddr phy 0x%09llx(%d)"COLOR_NONE"\n"
										,mpro_ctrl[i].name, (u64)rw_vio_data[j].wr_vio_addr << 1, j);

								mpro_ctrl[i].vio_addr = (u64)rw_vio_data[j].wr_vio_addr << 1;
					...
					if(irq_status & MPRO_ISR_STATUS_RD)
					{
						for(j = 0; j < O18_MPRO_RANGE_SLOT_CNT; j++)
						{
							...
							{
								LGBUS_ERROR(COLOR_RED"[%-10s] Detect read violation!. ddr phy 0x%09llx(%d)"COLOR_NONE"\n"
										,mpro_ctrl[i].name, (u64)rw_vio_data[j].rd_vio_addr << 1, j);

								mpro_ctrl[i].vio_addr = (u64)rw_vio_data[j].rd_vio_addr << 1;
				...
				if(++mpro_ctrl[i].isr_cnt >= isr_cnt_limit)
					lgbus_mpro_isr_disable_module(i); // isr disable when a prot happened interrupt in 3times.

				irq_clear = mpro_reg_read(irq_clear_addr);
				irq_clear |= MPRO_ISR_CLEAR;
				mpro_reg_write_wa(irq_clear, irq_clear_addr, mpro_ctrl[i].base_addr_phy, 1);

				mpro_reg_write_wa(0xffffffff, lbus_qos_cmn_base+MPRO_ISR_CLEAR_OFFSET, LBUS_QOS_CMN_BASE, 1);
				mpro_reg_write_wa(0xffffffff, gbus_qos_cmn_base+MPRO_ISR_CLEAR_OFFSET, GBUS_QOS_CMN_BASE, 1);
			}
		}
...
int LGBUS_O18_SetMproRange(lgbus_mpro_range_t *mpro_range)
{
#ifdef SUPPORT_O18_MPRO_CHIP_KDRV
	int 		ret = RET_ERROR;
	uint32_t 	port, cnt;
	...
	CHECK_ERROR(mpro_range->port >= O18_MPRO_PORT_MAX, return RET_ERROR, "Invalid port\n");
	CHECK_ERROR(mpro_range->port < O18_MPRO_PORT_START, return RET_ERROR, "Invalid port\n");

	port = mpro_range->port - O18_MPRO_PORT_START;
	...
	for(cnt = 0; cnt < O18_MPRO_RANGE_SLOT_CNT; cnt++)
	{
		uint32_t ctr0, ctr1;
		...
		mpro_reg_write_wa(ctr0, range_addr++, mpro_ctrl[port].base_addr_phy, 0);
		mpro_reg_write_wa(ctr1, range_addr++, mpro_ctrl[port].base_addr_phy, 0);

		LGBUS_PRINT("[%-10s][%u] 0x%09llx ~ 0x%09llx, 0x%09x-0x%09x\n"
									, mpro_ctrl[port].name
									, cnt
									, mpro_ctrl[port].range[cnt].start
									, mpro_ctrl[port].range[cnt].end, ctr0, ctr1);
	}

	//slot enable
	enable_addr = (u32 *)(mpro_ctrl[port].base_addr + MPRO_SLOT_ENABLE_OFFSET);
	ctr = (1<<8) | (1<<4) | 1;
	mpro_reg_write_wa(ctr, enable_addr, mpro_ctrl[port].base_addr_phy, 0);
...
EXPORT_SYMBOL(LGBUS_O18_SetMproRange);

int LGBUS_O18_GetMproRange(lgbus_mpro_range_t *mpro_range)
{
#ifdef SUPPORT_O18_MPRO_CHIP_KDRV
	uint32_t 	port, cnt;
	ulong 		flag;
...
EXPORT_SYMBOL(LGBUS_O18_GetMproRange);

lgbus_o18_mpro_func_t	lgbus_o18_mpro_func =
{
...
#define CPU_ADDR_SWITCH_BASE			(0xc7ff1080)
#define O18_ADDR_SWITCH_SLOT			(8)

#define CPU_BWB_BASE					(0xc7ff1100)
#define O18_BWB_SLOT					(4)
...
static lgbus_addr_switch_t o18_addr_switch[O18_ADDR_SWITCH_SLOT];
static lgbus_bwb_t			o18_bwb[O18_BWB_SLOT];
...
static int lgbus_get_addr_switch(void)
{
	...
	for(i = 0; i < O18_ADDR_SWITCH_SLOT; i++)
	{
		u32 d0, d1;
		...
		o18_addr_switch[i].start = ((u64)(d0 >> 16) & 0x1fff) << 20; // 28~16:13bit
		o18_addr_switch[i].end = (u64)(d0 & 0x1fff) << 20; // 12~0:13bit
		o18_addr_switch[i].offset = (u64)(d1 & 0x1fff) << 20; // 12~0:13bit

		if(o18_addr_switch[i].end)
			o18_addr_switch[i].end |= 0xfffff;

		LGBUS_PRINT("s:0x%010llx ~ e:0x%010llx,o:0x%010llx\n", o18_addr_switch[i].start
															 , o18_addr_switch[i].end
															 , o18_addr_switch[i].offset);
	}
...
static int lgbus_get_bwb(void)
{
	...
	for(i = 0; i < O18_BWB_SLOT; i++)
	{
		...
		o18_bwb[i].start = ((u64)(d0 >> 12) & 0x7ff) << 20;
		o18_bwb[i].end = (u64)(d0 & 0x7ff) << 20;

		o18_bwb[i].offset = (u64)(d1 & 0x1fff) << 20; /* o18 [12:0] rf_bw_bal_offset */
		o18_bwb[i].offset_mode = (d1 >> 28) & 0x7;

		if(o18_bwb[i].end)
			o18_bwb[i].end |= 0xfffff;

		LGBUS_PRINT("s:0x%010llx ~ e:0x%010llx,o:0x%010llx[%x]\n", o18_bwb[i].start
															 , o18_bwb[i].end
															 , o18_bwb[i].offset
															 , o18_bwb[i].offset_mode);
	}
...
u64 lgbus_o18_get_ddr_addr(u64 phy_addr)
{
#ifdef SUPPORT_O18_MPRO_CHIP_KDRV
	int i;
	u64 tmp = phy_addr;
	...
	for(i = 0; i < O18_ADDR_SWITCH_SLOT; i++)
	{
		if(o18_addr_switch[i].start <= phy_addr &&
				o18_addr_switch[i].end >= phy_addr)
		{
			phy_addr += o18_addr_switch[i].offset;
	...
	for(i = 0; i < O18_BWB_SLOT; i++)
	{
		if(o18_bwb[i].offset)
		{
			if(o18_bwb[i].start <= bwb_addr &&
					o18_bwb[i].end >= bwb_addr)
			{
				if(o18_bwb[i].offset_mode == BWB_OFFSET_M0 &&
						bwb_addr == phy_addr)
				{
					phy_addr += o18_bwb[i].offset;
				}
				else if(o18_bwb[i].offset_mode == BWB_OFFSET_M1 &&
						phy_addr >= 0x80000000 && phy_addr < 0xC0000000 )
				{
					phy_addr += o18_bwb[i].offset;
				}
				else if(o18_bwb[i].offset_mode == BWB_OFFSET_M2 &&
						phy_addr >= 0x100000000 && phy_addr < 0x180000000 )
				{
					phy_addr += o18_bwb[i].offset;
				}
...
EXPORT_SYMBOL(lgbus_o18_get_ddr_addr);
...
static int lgbus_mpro_proc_status_op(struct seq_file *s, void *data)
{
	...
	for(i = 0; i < ARRAY_SIZE(mpro_ctrl); i++)
	{
		...
		if(mpro_ctrl[i].vio_type)
		{
			...
			seq_printf(s, COLOR_RED"\tvio addr 0x%09llx, type %s\n"COLOR_NONE
        ...
        , const char __user *buf, size_t size, loff_t *off)
{
	...
	int		i;
	int		addr_cnt;
	int		addr_max = O18_MPRO_RANGE_SLOT_CNT * 2;
	u64		addr[O18_MPRO_RANGE_SLOT_CNT * 2] = {0,};
	...
	while(i < size)
	{
		...
		{
			mpro_range.port += O18_MPRO_PORT_START;
			LGBUS_O18_SetMproRange(&mpro_range);
		}
...
{
#ifdef SUPPORT_O18_MPRO_CHIP_KDRV
	/* copied from lgbus_get_addr_switch() */
	{
		int 	i;
		u32*	base_addr;
		u32*	reg_base; // It's 32bit register. 64bit kernel ulong is 64bit.
