...
#define __MEM_VALUE_SELECT(cat0,cat1)       ((UINT32)(g_model_ctx.ddr_size==MODEL_OPT_3_5_GB)? (cat0):(cat1))
...
typedef enum
{
    ...
    MODEL_OPT_3_0_GB        = 0x3,  /* 3.0GB 64Bit: M0=2.0GB, M1=1GB */
    MODEL_OPT_3_5_GB        = 0x4,  /* 3.5GB 64Bit: M0=1.5GB, M1=1GB, M2=1GB */
...
static LX_MEMCFG_T free_mem0        = {.name = "free_mem0",     .base = 0x0, .size = 0 };
...
static LX_MEMCFG_DESC_T mem_desc[MEM_DESC_ID_MAX]   =
{
    ...
    DEFINE_MEM_DESC(DEBUG0, MEMCFG_DESC_FLAG_NO_CMA, "debug0"),        // see http://clm.lge.com/issue/browse/SICDTV-14326
    DEFINE_MEM_DESC(DEBUG1, MEMCFG_DESC_FLAG_CMA, "debug1"),           /* CMA 조치 완료 */

    DEFINE_MEM_DESC(FREE0, MEMCFG_DESC_FLAG_CMA, NULL),                 /* CMA 조치 완료 */
    DEFINE_MEM_DESC(FREE1, MEMCFG_DESC_FLAG_CMA, NULL),                 /* CMA 조치 완료 */
};
...
static LX_MEMCFG_ENTRY_T g_mem_desc_table_4ktv[] =
{
    //-----------------------------------------------------------------------------------
    //  BWB GROUP
    //-----------------------------------------------------------------------------------
...
static void BASE_PLT_SetupMemLayout(void)
{
    ...
    gMemCfgHdr[DDR_RGN_BWB_DEV].base = __MEM_VALUE_SELECT(CAT0_RGN_BWB_BASE, CAT1_RGN_BWB_BASE);
    gMemCfgHdr[DDR_RGN_M0_DEV].base = __MEM_VALUE_SELECT(CAT0_RGN_M0_BASE, CAT1_RGN_M0_BASE);
    gMemCfgHdr[DDR_RGN_M1_DEV].base = __MEM_VALUE_SELECT(CAT0_RGN_M1_BASE, CAT1_RGN_M1_BASE);
    gMemCfgHdr[DDR_RGN_SYS_DEV].base = __MEM_VALUE_SELECT(CAT0_RGN_SYS_BASE, CAT1_RGN_SYS_BASE);

    gMemCfgHdr[DDR_RGN_BWB_DEV].size = __MEM_VALUE_SELECT(CAT0_RGN_BWB_SZ, CAT1_RGN_BWB_SZ);
    gMemCfgHdr[DDR_RGN_M0_DEV].size = __MEM_VALUE_SELECT(CAT0_RGN_M0_SZ, CAT1_RGN_M0_SZ);
    gMemCfgHdr[DDR_RGN_M1_DEV].size = __MEM_VALUE_SELECT(CAT0_RGN_M1_SZ, CAT1_RGN_M1_SZ);
    gMemCfgHdr[DDR_RGN_SYS_DEV].size = __MEM_VALUE_SELECT(CAT0_RGN_SYS_SZ, CAT1_RGN_SYS_SZ);

    BASE_PLT_SetupCommon();

    g_mem_desc_table = g_mem_desc_table_4ktv;
}
...
static void BASE_PLT_SetupCommon(void)
{
    ...
    __m0_ns_mem_grp.base = __m0_s_mem_grp.base + __MEM_VALUE_SELECT(CAT0_GRP_M0_S_MEM_SZ, CAT1_GRP_M0_S_MEM_SZ);
    __m1_s_mem_grp.base = gMemCfgHdr[DDR_RGN_M1_DEV].base;
    __m1_ns_mem_grp.base = __m1_s_mem_grp.base + __MEM_VALUE_SELECT(CAT0_GRP_M1_S_MEM_SZ, CAT1_GRP_M1_S_MEM_SZ);
    ...
    vdec_vpb0.size  = MOD_VPB4K_SZ;
    vdec_vpb1.size  = __MEM_VALUE_SELECT(MOD_CAT0_VPB8K_SZ, MOD_CAT1_VPB8K_SZ);
    vdec_cpb0.size  = MOD_VDEC_CPB_SZ;
    vdec_gen.size   = MOD_VDEC_GEN_SZ;

    audio0_buffer.size = __MEM_VALUE_SELECT(MOD_CAT0_AUDIO0_SZ, MOD_CAT1_AUDIO0_SZ);
    audio1_buffer.size = __MEM_VALUE_SELECT(MOD_CAT0_AUDIO1_SZ, MOD_CAT1_AUDIO1_SZ);
    audio_lnx_buffer.size = MOD_AUDIO_LNX_SZ;

    lne_buffer.size = MOD_LNE_SZ;
    npu0_buffer.size = MOD_NPU0_SZ;
    npu1_buffer.size = MOD_NPU1_SZ;

    pqe_bwb.size = __MEM_VALUE_SELECT(MOD_CAT0_PQE_BWB_SZ, MOD_CAT1_PQE_BWB_SZ);

    free_mem0.size = __MEM_VALUE_SELECT(MOD_CAT0_FREEMEM0_SZ, MOD_CAT1_FREEMEM0_SZ);
}
...
static void BASE_PLT_DoPostMemLayout(void)
{
    ...
    unsigned int debug0_min_size = __MEM_VALUE_SELECT(MOD_CAT0_M0_DEBUG_SZ, MOD_CAT1_M0_DEBUG_SZ);
    unsigned int debug1_min_size = __MEM_VALUE_SELECT(MOD_CAT0_M1_DEBUG_SZ, MOD_CAT1_M1_DEBUG_SZ);
...
void  BASE_PLT_Probe(UINT32* plt, UINT32* board_opt)
{
    ...
    if (webos_hwopt_probe)
    {
        ...
        if (RET_OK == OS_ScanKernelCmdline("hwopt=%s", hwopt_txt))
        {
            ...
            switch (hwopt_txt[2])
            {
                ...
                default: g_model_ctx.panel_if = MODEL_OPT_PANEL_IF_OTHERS; break;
            }
            ...
            switch (hwopt_txt[11])
            {
                ...
                case '6': g_model_ctx.ddr_size = MODEL_OPT_3_0_GB; break;
        ...
        if (RET_OK == OS_ScanKernelCmdline("disp=%s", optstr))
        {
            printk("disp=%s detected\n", optstr);

            if (!strncmp(optstr, "hdmi2k", 6))
            {
        ...
        for (i = 0; i < NELEMENTS(pioneer_board_fmtstr); i++)
        {
            if (RET_OK == OS_ScanKernelCmdline(pioneer_board_fmtstr[i], optstr))
            {
                ...
                while (tok)
                {
                    ...
                    if (!strncasecmp(tok, "3.0GB", 5))        g_model_ctx.ddr_size    = MODEL_OPT_3_0_GB;
    ...
    switch (g_model_ctx.ddr_size)
    {
        ...
        case MODEL_OPT_3_0_GB: board_opt_val |= (LX_BOARD_OPT_DDR_3_0GB | LX_BOARD_OPT_ADV); break;
