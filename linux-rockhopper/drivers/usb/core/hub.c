...
static void get_bMaxPacketSize0_status(struct usb_device *udev, bool first_time,
		int i, int size, int ret, struct usb_device_descriptor *buf)
{
	char *b = (char *)buf;

	if (ret < 8)
		dev_info(&udev->dev, "(%d %d %d %d) [0:%02x 1:%02x 7:%02x]\n",
				first_time, i, size, ret, b[0], b[1], b[7]);
}
...
int usb_get_port_status(struct usb_device *udev, u16 *status, u16 *change)
{
	struct usb_hub *hub;
	...
	if (!udev->parent)
		return -ENODEV;
	hub = usb_hub_to_struct_hub(udev->parent);
	return hub_ext_port_status(hub, udev->portnum, HUB_PORT_STATUS,
			status, change, NULL);
}
		...
		struct usb_device_descriptor *buf, int size, bool first_time)
{
	...
	for (i = 0; i < GET_MAXPACKET0_TRIES; ++i) {
		...
		get_bMaxPacketSize0_status(udev, first_time, i, size, rc, buf);
		...
		int retry_counter, struct usb_device_descriptor *dev_descr)
{
	...
	if (retval < 0) {	/* error or disconnect */
		if (udev->speed > USB_SPEED_UNKNOWN)
			dev_err(&udev->dev, "device reset failed! ret %d\n",
					retval);
		goto fail;
	}
	...
	for (retries = 0; retries < GET_DESCRIPTOR_TRIES; (++retries, msleep(100))) {
		...
		if (do_new_scheme) {
			...
			retval = hub_port_reset(hub, port1, udev, delay, false);
			if (retval < 0)	{	/* error or disconnect */
				dev_err(&udev->dev, "device reset failed! error %d\n",
					retval);
				goto fail;
			}
