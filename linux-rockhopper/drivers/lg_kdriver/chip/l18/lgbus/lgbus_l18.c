...
typedef struct
{
    volatile UINT32     bus_reg[QOS_IDX_MAX];       // ioremapped bus register
}
LGBUS_CHIP_CTX_T;
...
/*----------------------------------------------------------------------------------------
  External Variables
----------------------------------------------------------------------------------------*/
extern lgbus_l18_mpro_func_t	lgbus_l18_mpro_func;
...
{
{"NA",	{{{0x00000000,0x00000000},{0x00000000,0x00000000},{0x00000000,0x00000000}},	{{0x00000000,0x00000000},{0x00000000,0x00000000},{0x00000000,0x00000000}}}},
};
...
static LGBUS_HW_CFG_T	lgbus_chip_cfg =
{
	...
	.ddr_num		= 1,										// LM only
	.ip_num			= 1,										// not implemented
	.port_num		= 0,										// not implemented

	.bus_reg_base	= {0xC9820000, 0xC9820000, 0xC9231620 },	// LBUS, NA, CPU
	.freq			= {264, 264, 198},							// LBUS, NA, CPU
};
...
static int lgbus_config(LGBUS_HW_CFG_T* cfg)
{
	...
	lgbus_chip_cfg.max_bw[0] = DDR_CLK*4*2;
...
{
	int ret = lgbus_l18_mpro_func.resume();
