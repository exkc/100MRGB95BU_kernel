 ...
 *
 */
UINT32 MJPEG_HDR_Decode(LX_MJPEG_VMEM_BUF_T *i_pstBuffer,
						LX_MJPEG_HEADER_T *o_pstHeader, UINT8 *o_pui8CheckList)
{
	...
	_g_pui8Buffer = (UINT8 *)(i_pstBuffer->virt_addr);
	...
	while (bDecoding) {
		...
		// Check buffer
		if (_g_i32ReadBytes + 2 >= _g_i32BufferSize) {
			return _g_i32ReadBytes;
		}

		// Get Marker code
		MarkerCode = (_g_pui8Buffer[_g_i32ReadBytes] << 8) |
					 _g_pui8Buffer[_g_i32ReadBytes + 1];
...
static SINT32 _DecodeAPPHeader(void)
{
	...
	// Check buffer
	if (ReadPointer + 2 > _g_i32BufferSize) {
		return -1;
	}
	...
	if (ReadPointer + Length - 2 > _g_i32BufferSize) {
		return -1;
	}
...
static SINT32 _DecodeSOFHeader(void)
{
	...
	pstFrameHeader->ui16Height =
			pstFrameHeader->ui16Height << 8 | _g_pui8Buffer[ReadPointer++];
	pstFrameHeader->ui16Width = _g_pui8Buffer[ReadPointer++];
	pstFrameHeader->ui16Width =
			pstFrameHeader->ui16Width << 8 | _g_pui8Buffer[ReadPointer++];
	...
	for (i32Count = 0; i32Count < pstFrameHeader->ui8NumberOfComponents;
		 ++i32Count) {
		// Component ID, Ci 0 ~ 255
		pstFrameHeader->stComponents[i32Count].ui8ComponentID =
				_g_pui8Buffer[ReadPointer++];
		// Horizontal Sampling Factor, Hi
		pstFrameHeader->stComponents[i32Count].ui8HSampleFreq =
				(_g_pui8Buffer[ReadPointer] & 0xF0) >> 4;
		// Vertical Sampling Factor, Vi
		pstFrameHeader->stComponents[i32Count].ui8VSampleFreq =
				(_g_pui8Buffer[ReadPointer++] & 0x0F);
		// Quantization Table Selector, Tqi
		pstFrameHeader->stComponents[i32Count].ui8QTableID =
				_g_pui8Buffer[ReadPointer++];
	}
...
static SINT32 _DecodeSOSHeader(void)
{
	...
	for (i32Count = 0; i32Count < pstScanHeader->ui8NumberOfComponents;
		 ++i32Count) {
		pstScanHeader->stComponents[i32Count].ui8ComponentID =
				_g_pui8Buffer[ReadPointer++];
		pstScanHeader->stComponents[i32Count].ui8DC_HuffmanTableID =
				(_g_pui8Buffer[ReadPointer] & 0xF0) >> 4;
		pstScanHeader->stComponents[i32Count].ui8AC_HuffmanTableID =
				(_g_pui8Buffer[ReadPointer++] & 0x0F);
	}
...
static SINT32 _DecodeDRIHeader(void)
{
	...
	*pui16ResetInterval =
			*pui16ResetInterval << 8 | _g_pui8Buffer[ReadPointer++];
...
static SINT32 _DecodeDQTHeader(void)
{
	...
	SINT32 i32Count;
	UINT16 Length;
	LX_MJPEG_QUANTIZATION_TABLE_T *pstQuantizationTable =
	...
	for (i32Count = 0; i32Count < Length - 2; ++i32Count) {
		if (i32Count % 65 == 0) {
			...
			pstQuantizationTable[ui8Tq].ui8Elements[(i32Count % 65) - 1] =
					_g_pui8Buffer[ReadPointer++];
		}
	}
...
static SINT32 _DecodeDHTHeader(void)
{
	...
	for (i32Count = 0; i32Count < Length - 2; ++i32Count) {
		...
		i32BitCount = 0;
		for (i32TableCount = 0; i32TableCount < 16;
			 ++i32Count, ++i32TableCount) {
			pstHuffmanTables[ui8ThTc].ui8CodeLengths[i32TableCount] =
					_g_pui8Buffer[ReadPointer++];
			i32BitCount +=
					pstHuffmanTables[ui8ThTc].ui8CodeLengths[i32TableCount];
		}

		for (i32TableCount = 0; i32TableCount < i32BitCount;
			 ++i32Count, ++i32TableCount) {
			pstHuffmanTables[ui8ThTc].ui8CodeValues[i32TableCount] =
					_g_pui8Buffer[ReadPointer++];
		}
	}
