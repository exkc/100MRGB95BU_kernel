...
----------------------------------------------------------------------------------------*/
static int _LOGM_ProcStatusOpen(struct inode *inode, struct file *file);
static int _LOGM_ProcStatusShow(struct seq_file *s, void *data);
static ssize_t _LOGM_ProcStatusWrite(struct file *file, const char __user * buf,
				     size_t size, loff_t * off);
static int _LOGM_ObjMaskOpen(struct inode *inode, struct file *file);
static ssize_t _LOGM_ObjMaskWrite(struct file *file,
				  const char __user * buf, size_t size,
				  loff_t * off);
static int _LOGM_ObjMaskRelease(struct inode *inode, struct file *file);
static int _LOGM_ObjMaskShow(struct seq_file *s, void *data);
static int __obj_mask_write(LX_LOGM_LOGOBJ_T * obj, const char *buf, int size);
static int _LOGM_ProcLMsgOpen(struct inode *inode, struct file *file);
...
static void __obj_mask_set(LX_LOGM_LOGOBJ_T * obj, enum logm_set_mask_type type,
			   unsigned int mask);
...
static struct proc_ops proc_status_fops = {
	.proc_open = _LOGM_ProcStatusOpen,
	.proc_read = seq_read,
	.proc_write = _LOGM_ProcStatusWrite,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};

static struct proc_ops proc_fops = {
	.proc_open = _LOGM_ObjMaskOpen,
	.proc_write = _LOGM_ObjMaskWrite,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = _LOGM_ObjMaskRelease,
};

static struct proc_ops proc_lmsg_fops = {
	.proc_open = _LOGM_ProcLMsgOpen,
	.proc_write = _LOGM_ProcLMsgWrite,
	.proc_read = seq_read,
	.proc_release = single_release,
};
#else
...
static struct mutex _g_logm_bit_name_list_mutex;
	...
	*p = 0;
	...
	//path ctrl
		...
		if (i)
			...
			g_logm_log_color = 0;
	} else if (!strncmp(name, "print", strlen("print"))) {
		u32 i;
		u32 mask;
		unsigned long flag;
		LX_LOGM_LOGOBJ_T *map = g_logm_obj_map;

		mask = simple_strtoul(sep, NULL, 0);
		...
		printk("\nall logm module - print mask : 0x%08x\n", mask);
	} else if (!strncmp(name, "force", strlen("force"))) {
		u32 i;
		u32 mask;
		unsigned long flag;
		LX_LOGM_LOGOBJ_T *map = g_logm_obj_map;
		...
		for (i = 0; i < LOGM_LOGOBJ_MAX; i++) {
			if (map[i].name[0] == '\0')
				...
				map[i].user_mask = mask;
		}
		spin_unlock_irqrestore(&g_logm_obj_map_lock, flag);

		printk("\nall logm module - force disable mask : 0x%08x\n",
		       mask);
	} else if (!strncmp(name, "hidden", strlen("hidden"))) {
		u32 i;
		u32 mask;
		unsigned long flag;
		LX_LOGM_LOGOBJ_T *map = g_logm_obj_map;
		...
		for (i = 0; i < LOGM_LOGOBJ_MAX; i++) {
			if (map[i].name[0] == '\0')
				...
				map[i].hidden_mask = mask;
		}
		spin_unlock_irqrestore(&g_logm_obj_map_lock, flag);

		printk("\nall logm module - hidden mask : 0x%08x\n", mask);
	} else {
		int fd;
		uint32_t i;
		...
		ret = RET_ERROR;
		LOGM_CHECK_ERROR(sep_size <= 0, goto func_exit,
				 "fail get index\n");

		fd = LOGM_ObjFDGet(name);
		if (fd < 0) {
			fd = LOGM_ObjRegister(name);
			LOGM_CHECK_ERROR(fd < 0, goto func_exit,
					 "fail fd_get\n");
		}
...
static int _LOGM_BitNameListInit(void)
{
	...
	mutex_init(&_g_logm_bit_name_list_mutex);
...
int LOGM_RegBitName(int fd, unsigned int bit, char *name)
{
	...
	LOGM_CHECK_ERROR(name == NULL, return RET_ERROR, "name is NULL\n");

	name_len = strlen(name);
	bit_name = (char *)vmalloc(name_len + 1);
	LOGM_CHECK_ERROR(bit_name == NULL, return RET_ERROR, "fail vmalloc\n");

	strncpy(bit_name, name, name_len + 1);
	bit_name[name_len] = 0;

	mutex_lock(&_g_logm_bit_name_list_mutex);
	if (_g_logm_bit_name_list[fd].name[bit]) {
		vfree(_g_logm_bit_name_list[fd].name[bit]);
		_g_logm_bit_name_list[fd].name[bit] = NULL;
	}

	_g_logm_bit_name_list[fd].name[bit] = bit_name;
	mutex_unlock(&_g_logm_bit_name_list_mutex);
...
int LOGM_GetBitName(int fd, struct logm_obj_bit_name *bit_name)
{
	...
	for (i = 0; i < LX_LOGM_LEVEL_MAX + 1; i++) {
		if (_g_logm_bit_name_list[fd].name[i]) {
			strncpy(bit_name->name[i],
				_g_logm_bit_name_list[fd].name[i],
				LOGM_MODULE_NAME_MAX);
...
void LOGM_PROC_Init(void)
{
	...
	proc_create("status", 0440, proc_entry, &proc_status_fops);
	proc_create("lmsg", 0440, proc_entry, &proc_lmsg_fops);
...
{
	if (!in_interrupt()) {
		proc_create_data(name, 0640, proc_entry, &proc_fops, obj);
	...
	while (TRUE) {
		...
		if (length > 0) {
			...
			seq_printf(s,
				   ...
				   " (%05d:%05d) ", (unsigned long)t,
				   nanosec_rem / 1000, _h->obj->name,
			...
			LOGM_CHECK_ERROR(b_str == NULL, ret = RET_ERROR;
					 goto func_exit;
					 , "fail kmalloc\n");
			_h_str = (void *)_h + sizeof(struct logm_header);
			memcpy(b_str, _h_str, length);
			b_str[length] = '\0';

			if (g_logm_log_color
			    && _h->level == LX_LOGM_LEVEL_ERROR)
				seq_printf(s, "%s",
					   g_logm_clrStrings[DBG_COLOR_RED]);
			else if (g_logm_log_color
				 && _h->level == LX_LOGM_LEVEL_WARNING)
				seq_printf(s, "%s",
					   g_logm_clrStrings[DBG_COLOR_PURPLE]);
...
#else
	return single_open(file, _LOGM_ObjMaskShow, _pde_data(inode));
#endif
}
...
static int _LOGM_ObjMaskShow(struct seq_file *s, void *data)
{
	...
	spin_lock_irqsave(&g_logm_obj_map_lock, flag);
	mask = obj->mask;
	hidden_mask = obj->hidden_mask;
	dis_mask = obj->user_mask;
	...
	//display user mask
	seq_printf(s, "[force disable mask] : ");
	sep = "";
	for (a = 0; a < 32; a++) {
		if (dis_mask & (1 << a)) {
			seq_printf(s, "%s%s", sep, LOGM_LevelName(a));
...
static int __obj_mask_write(LX_LOGM_LOGOBJ_T * obj, const char *buf, int size)
{
	...
	unsigned int mask;
	int sep;
	enum logm_set_mask_type set = logm_level_mask;
...
#define lt(s,n)	{#s, sizeof(#s), LX_LOGM_LEVEL_##n, }
				lt(ERROR, ERROR),
				    ...
				    lt(NOTI, NOTI),
				    lt(INFO, INFO),
				    lt(DEBUG, DEBUG), lt(TRACE, TRACE),
				    /* simple form */
				    lt(E, ERROR),
				    lt(W, WARNING),
				    lt(N, NOTI),
				    lt(I, INFO), lt(D, DEBUG), lt(T, TRACE),
#undef lt
			};
			...
			if (*p == 'B') {
				...
				for (a = 0; a < ARRAY_SIZE(level_table); a++) {
					...
					if (!strncmp
					    (p, level_table[a].name, len)) {
						debug("got it\n");
						mask |=
						    1 << level_table[a].level;
				...
				if (a == ARRAY_SIZE(level_table)) {
					if (*p == 'P')
						...
						set = logm_force_dis_mask;
					else if (*p == 'H')
						set = logm_hidden_mask;
					else if (*p >= '0' && *p <= '9') {
						set = logm_print_mask;
						p--;
					} else {
						error
						    ("unknown format. %02x, %c\n",
						     *p, *p);
						return -EINVAL;
					}

					p++;
					mask = (u32) simple_strtoul(p, NULL, 0);
					break;
				}
			}
			   ...
			   unsigned int mask)
{
	...
	if (type == logm_add_mask) {
		...
		obj->mask |= mask;
		goto func_exit;
	} else if (type == logm_del_mask) {
		debug("%s: del %08x from %08x\n", obj->name, mask, obj->mask);
		obj->mask &= ~mask;
		goto func_exit;
	} else if (type == logm_print_mask) {
		debug("%s: set %08x to %08x\n", obj->name, mask, obj->mask);
		obj->mask = mask;
		goto func_exit;
	} else if (type == logm_force_dis_mask) {
		debug("%s: force dis mask %08x to %08x\n",
		      obj->name, mask, obj->user_mask);
		obj->user_mask = mask;
		goto func_exit;
	} else if (type == logm_hidden_mask) {
		debug("%s: hidden mask %08x to %08x\n",
		      obj->name, mask, obj->hidden_mask);
		obj->hidden_mask = mask;
		goto func_exit;
	}
	//else logm_level_mask --
