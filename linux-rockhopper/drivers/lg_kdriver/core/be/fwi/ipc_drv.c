...
void FWI_CPU_ISR_FROM_MCU(struct tasklet_struct *temp);
void FWI_CPU_ACK_FROM_MCU(struct tasklet_struct *temp);
void FWI_CPU_WDG_FROM_MCU(struct tasklet_struct *temp);
#else
...
{
	int ret = RET_OK;
	void *addrIPCBuffToMCU = 0;
	void *addrIPCBuffFrMCU = 0;
...
int BE_IPC_Free(void)
{
	...
	do {
		...
		addrIPCBuffToMCU = (void *)gpIPCBuffToMCU;
		addrIPCBuffFrMCU = (void *)gpIPCBuffFrMCU;
...
int BE_MCU_SetData(UINT32 ipcCmd, void *pMsg, UINT32 msgCnt)
{
	...
	do {
		...
		BE_DEBUG("%s:%d:%s, IPC CMD length(%d)  data = [0x%x, 0x%x, 0x%x, 0x%x]\n", 
			KBUILD_BASENAME, __LINE__, __FUNCTION__, (int)sizeof(ipcMsgBuff), ipcMsgBuff.u16From, ipcMsgBuff.u16Type, ipcMsgBuff.u32Cmd, ipcMsgBuff.u32Data);
...
#ifdef INCLUDE_KDRV_DE
	if (h_mcu1_ipc == NULL) return RET_ERROR;
	if (h_mcu1_ipc->m_ipc_client->write == NULL) return RET_ERROR;
...
#ifdef INCLUDE_KDRV_DE
	if (h_mcu1_ipc == NULL) return RET_ERROR;
	if (h_mcu1_ipc->m_ipc_client->read == NULL) return RET_ERROR;
...
static int FWI_CPU_Send2MCU(void)
{
	...
	do {
		...
		switch (ipcStatus) {
			...
			case FWI_IPC_BUSY :
			default :
				if (ipcStatus == FWI_IPC_LAST || ipcStatus == FWI_IPC_OK) {
				...
				if (ipcStatus == FWI_IPC_LAST || ipcStatus == FWI_IPC_OK || ipcStatus == FWI_IPC_BUSY) {
					// don't insert break;
...
#endif
{
	...
	switch (ipcMsgFrMcu->u16Type) {
		...
		case FWI_IPC_CMD_VAL :
		default :
			if (ipcMsgFrMcu->u16Type == FWI_IPC_CMD_RET) {
				FWI_IPC_CopyStr2Queue(&stBeIpcQueFrMCU, gpIPCBuffFrMCU);
			}
			fwi_wakeup_send2mcu_timeout = 0;
