...
/*----------------------------------------------------------------------------------------
	Macro Definitions
----------------------------------------------------------------------------------------*/
#define VBI_DRV_VERSION (18090700)
...
extern int v4l2_vbi_device_init(void);
/*----------------------------------------------------------------------------------------
	External Variables
----------------------------------------------------------------------------------------*/
...
#ifdef CONFIG_COMPAT
static long     VBI_CompatIoctl (struct file * filp, unsigned int cmd, unsigned long arg);
#endif
#endif
...
static int VBI_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if(gVBIInitiateMode ==LX_VBI_INIT_NTSC)
	{
		ret |=  VBI_CC_SetInterrupt(OFF);
	}
	else if(gVBIInitiateMode ==LX_VBI_INIT_PAL)
	{
...
int VBI_Init(void)
{
	...
	VBI_INFO("%s entered : version [%d]\n" , __func__, VBI_DRV_VERSION);
	...
	{
		VBI_ERROR("out of memory. can't allocate %zd bytes\n", sizeof(VBI_DEVICE_T)* VBI_MAX_DEVICE );
		return -ENOMEM;
	}
 ...
 *
 */
static int VBI_Open(struct inode *inode, struct file *filp)
{
    int					major,minor;
    struct cdev*    	cdev;
    VBI_DEVICE_T*	    my_dev;
 ...
 *
 */
static int VBI_Close(struct inode *inode, struct file *file)
{
    int					major,minor;
    VBI_DEVICE_T*   	my_dev;
    struct cdev*		cdev;
...
static int VBI_MMap(struct file *file, struct vm_area_struct *vma)
{
	...
	vbi_mem_temp = (void *)gVBI_data_mem_addr;
...
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0))
	vm_flags_set (vma, VM_DONTCOPY | VM_DONTEXPAND | /*VM_RESERVED |*/ VM_IO | VM_MIXEDMAP);
#else
	vma->vm_flags |= VM_DONTCOPY | VM_DONTEXPAND | /*VM_RESERVED |*/ VM_IO | VM_MIXEDMAP;
#endif
	for (i=0; i<(gstMemCfgVBI->buf_memory_size + PAGE_SIZE - 1)/PAGE_SIZE; i++)
	{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
		vmf_insert_mixed(vma, vma->vm_start + (i << PAGE_SHIFT), __pfn_to_pfn_t(vmalloc_to_pfn(vbi_mem_temp), 0));
#else
		vm_insert_mixed(vma, vma->vm_start + (i << PAGE_SHIFT), vmalloc_to_pfn(vbi_mem_temp));
#endif
		vbi_mem_temp += (uintptr_t)PAGE_SIZE;
	}
	return 0;
#endif
}
 ...
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
static int VBI_Ioctl ( struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg )
#else
static long VBI_Ioctl (struct file * filp, unsigned int cmd, unsigned long arg)
#endif
{
	...
	/* For Status Keeper */
	__attribute__((unused)) LX_VP_ST_KEEPER_MODULE_TYPE_T	mtype = LX_VP_ST_KEEPER_MODULE_VBI;
	__attribute__((unused)) LX_VP_ST_KEEPER_HISTORY_TYPE_T	htype = LX_VP_ST_KEEPER_HISTORY_IO;
	...
	switch(cmd)
	{
		...
		case VBI_CC_IOR_GET_CC_PACKET:
			ret = VBI_CC_GetVBIData((void *)&cc_data , 1);
			ret |= copy_to_user((void *)arg , (void *)&cc_data , sizeof(LX_CC_DATA_T));
			break;
		...
		case VBI_CC_IOW_SET_DEBUGMODE_PRINT:
			...
			else if(arg == 3) ret = VBI_CC_DebugPrintMode(3);
			else if(arg == 4) ret = VBI_CC_DebugPrintMode(4);
			else
			{
