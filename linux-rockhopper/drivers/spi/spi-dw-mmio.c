...
static int dw_spi_mmio_probe(struct platform_device *pdev)
{
	...
	dws->nstrength = device_property_count_u32(&pdev->dev, "strength");

	if (dws->nstrength < 0) {
		dev_err(&pdev->dev, "SPI drive nstrengh get failed\n");
	} else if (dws->nstrength == 1) {
		dws->reg_strength[0] = devm_platform_ioremap_resource(pdev, 1);

		if (IS_ERR(dws->reg_strength[0])) {
			dev_err(&pdev->dev, "SPI drive Strength get failed\n");
		}

		device_property_read_u32(&pdev->dev, "strength", strength);
		if (strength[0] > 0) {
			dws->strength[0] = strength[0];
		}
	} else if (dws->nstrength > 1) {

		for(i = 0;i < dws->nstrength / 2;i++) {
			dws->reg_strength[i] = devm_platform_ioremap_resource(pdev, i+1);
			if (IS_ERR(dws->reg_strength[i])) {
				dev_err(&pdev->dev, "SPI drive strength[%d] get failed\n", i);
				break;
			}
		}

		device_property_read_u32_array(&pdev->dev,"strength",strength,dws->nstrength);

		for(i = 0;i < dws->nstrength;i++){
				dws->strength[i] = strength[i];
		}
	...
	dws->bus_num = pdev->id;
	ret = device_property_read_u32(&pdev->dev, "bus-num", &bus_num);
	if (ret == 0) {
		dws->bus_num = bus_num;
	}
	...
	ret = device_property_read_u32(&pdev->dev, "rx-dly", &rx_dly);
	if (ret == 0) {
		dws->rx_dly = rx_dly;
	}
...
#endif
static SIMPLE_DEV_PM_OPS(dw_spi_mmio_pm_ops, spi_dw_suspend, spi_dw_resume);
