...
struct _ntfs_inode {
	...
	struct folio *folio;	/* The folio containing the mft record of the
				   inode. This should only be touched by the
				   (un)map_mft_record*() functions. */
	int folio_ofs;		/* Offset into the folio at which the mft record
	...
	int page_ofs;		/* Offset into the page at which the mft record
				   ...
				   (un)map_mft_record*() functions. */
#endif
	LCN mft_lcn[2];		/* LCN number containing the mft record */
...
typedef enum {
	...
	NI_IndexAllocPresent,	/* 1: $I30 index alloc attr is
				   present (d). */
...
extern int ntfs_drop_big_inode(struct inode *inode);
...
extern int ntfs_truncate_vfs(struct inode *vi, loff_t new_size, loff_t i_size);
