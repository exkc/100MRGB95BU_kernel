...
int VIDEO_SKeeper_InitHistoryLog(LX_VP_ST_KEEPER_MODULE_TYPE_T module, LX_VP_ST_KEEPER_HISTORY_TYPE_T type)
{
	...
	do{
		/* Step 0. Check Status Keeper Database */
		if(gSKeeperDataBase == NULL){
			ST_KEEPER_PRINT("[%s:%d] Error! ST Manager was not initialized.\n",__F__, __L__);
			break;
		}

		/* Step 1. Check module type argument */
		if(module >= LX_VP_ST_KEEPER_MODULE_TYPE_MAXNUM) {
			ST_KEEPER_PRINT("[%s:%d] Error! Unknown module type[%d] \n", __F__, __L__, module);
			break;
		}
		...
		if(type == LX_VP_ST_KEEPER_HISTORY_IO)
		{
			...
			gSKeeperDataBase->io_log[module].pEndAddress = (unsigned char *)((uintptr_t)pStartAddress + (sizeof(unsigned char) * LX_VP_ST_KEEPER_CMD_HISTORY_BUFFER_SIZE));
		...
		else if(type == LX_VP_ST_KEEPER_HISTORY_DISPATCH)
		{
			...
			gSKeeperDataBase->dispatch_log[module].pEndAddress = (unsigned char *)((uintptr_t)pStartAddress + (sizeof(unsigned char) * LX_VP_ST_KEEPER_CMD_HISTORY_BUFFER_SIZE));
...
int VIDEO_SKeeper_PushHistoryLog(LX_VP_ST_KEEPER_MODULE_TYPE_T module, LX_VP_ST_KEEPER_HISTORY_TYPE_T type, LX_VP_ST_KEEPER_HISTORY_INDEX_T *data)
{
	...
	do{
		...
		if((historyDatabase.pStartAddress == NULL)||(historyDatabase.pEndAddress == NULL)){
			ST_KEEPER_PRINT("[%s:%d] Null pointer! One of the pointers was NULL. S[%p]/E[%p]\n",__F__, __L__, historyDatabase.pStartAddress, historyDatabase.pEndAddress);
			break;
		}

		/* Step 5. Check pointer of pushing data */
		if(data == NULL){
			ST_KEEPER_PRINT("[%s:%d] Error! Data pointer is NULL[%p] on module[%d], type[%d]\n",__F__, __L__, data, module, type);
			break;
		}
		...
		leftMemorySize = (unsigned int)((uintptr_t)historyDatabase.pEndAddress - (uintptr_t)historyDatabase.pStartAddress) - (uintptr_t)historyDatabase.Head;
		
		/* 6-3) Get write position */
		pWriteAddress = (unsigned char *)((uintptr_t)historyDatabase.pStartAddress + (uintptr_t)historyDatabase.Head);
		
		/* Debugging */
		ST_KEEPER_LOG_PRINT("\n[%s:%d] Begin writing.. HEAD[%d] StartAddr[%p] EndAddr[%p] Status[%d]\
						\n\t\tPID[0x%08x] TIME[0x%08x] IOCTL[0x%08x] SIZE[%d] ADDR[%p]\n \
						\n\t\tleftMem[%d] pWriteAddr[%p]\n",\
				__F__, __L__, historyDatabase.Head, historyDatabase.pStartAddress, historyDatabase.pEndAddress, historyDatabase.status, \
				writeData.pid, writeData.time, (unsigned int)data->ioctl, data->size, data->pData, leftMemorySize, pWriteAddress);\
		...
		if(leftMemorySize >= (sizeof(LX_VP_ST_KEEPER_HISTORY_DATA_TYPE_T) - sizeof(unsigned int))) {
			...
			ST_KEEPER_LOG_PRINT("S1 : pWriteAddress[%p], T_size[0x%x] - U16size[0x%x] = [0x%x]\n", pWriteAddress, (unsigned int)sizeof(LX_VP_ST_KEEPER_HISTORY_DATA_TYPE_T), (unsigned int)sizeof(unsigned int), (unsigned int)(sizeof(LX_VP_ST_KEEPER_HISTORY_DATA_TYPE_T) - sizeof(unsigned int)));
			pWriteAddress = (unsigned char *)((uintptr_t)pWriteAddress + (uintptr_t)(sizeof(LX_VP_ST_KEEPER_HISTORY_DATA_TYPE_T) - sizeof(unsigned int)));
			ST_KEEPER_LOG_PRINT("S2 : pWriteAddress[%p]\n", pWriteAddress );
			...
			memcpy((void *)pWriteAddress, (void *)((uintptr_t)&writeData + (uintptr_t)(leftMemorySize)) ,  (sizeof(LX_VP_ST_KEEPER_HISTORY_DATA_TYPE_T) - sizeof(unsigned int)) - leftMemorySize);
			pWriteAddress = (unsigned char *)((uintptr_t) pWriteAddress + (sizeof(LX_VP_ST_KEEPER_HISTORY_DATA_TYPE_T) - sizeof(unsigned int)) - leftMemorySize);

			leftMemorySize =  (unsigned int)((uintptr_t)historyDatabase.pEndAddress - (uintptr_t)historyDatabase.pStartAddress - ((sizeof(LX_VP_ST_KEEPER_HISTORY_DATA_TYPE_T) - sizeof(unsigned int)) - leftMemorySize));
		}
		
		/* 6-3-2) Write Data */
		if(leftMemorySize >= data->size) {
			ST_KEEPER_LOG_PRINT("S3 : Read *pWriteAddress[0x%08x] pWriteAddress[%p], data->size[0x%x]\n", (unsigned int)*pWriteAddress, pWriteAddress, data->size); 
			memcpy((void *)pWriteAddress, (void *)data->pData, data->size);
			pWriteAddress = (unsigned char *)((uintptr_t)pWriteAddress + data->size);
			ST_KEEPER_LOG_PRINT("S4 : Read *pWriteAddress[0x%08x] pWriteAddress[%p]\n", (unsigned int)*pWriteAddress, pWriteAddress); 
			...
			memcpy((void *)pWriteAddress, (void *)((uintptr_t)data->pData + (uintptr_t)(leftMemorySize)) ,  data->size - leftMemorySize);
			pWriteAddress = (unsigned char *)((uintptr_t)pWriteAddress +  data->size - leftMemorySize);

			leftMemorySize =  (unsigned int)((uintptr_t)historyDatabase.pEndAddress - (uintptr_t)historyDatabase.pStartAddress - data->size - leftMemorySize);
		}

		/* 6-3-3) Write Tail */
		if(leftMemorySize >= sizeof(unsigned int)) {
			ST_KEEPER_LOG_PRINT("S5 : Read *pWriteAddress[0x%08x] pWriteAddress[%p], sizeof(unsigned int)[0x%x]\n", (unsigned int)*pWriteAddress, pWriteAddress, (unsigned int)sizeof(unsigned int)); 
			memcpy((void *)pWriteAddress, (void *)&writeData.tail, sizeof(unsigned int));
			pWriteAddress = (unsigned char *)((uintptr_t)pWriteAddress + sizeof(unsigned int) + 1);
			ST_KEEPER_LOG_PRINT("S6 : Read *pWriteAddress[0x%08x] pWriteAddress[%p]\n", (unsigned int)*pWriteAddress, pWriteAddress); 
			...
			memcpy((void *)pWriteAddress, (void *)((uintptr_t)&writeData.tail + (uintptr_t)(leftMemorySize)) ,  sizeof(unsigned int) - leftMemorySize);
			pWriteAddress = (unsigned char *)((uintptr_t)pWriteAddress +  sizeof(unsigned int) - leftMemorySize + 1);

			leftMemorySize =  (unsigned int)((uintptr_t)historyDatabase.pEndAddress - (uintptr_t)historyDatabase.pStartAddress - sizeof(unsigned ) - leftMemorySize);
		}	

		
		/* 7) Update Global Header */
		historyDatabase.Head = (unsigned int)((uintptr_t)pWriteAddress - (uintptr_t)historyDatabase.pStartAddress);
...
int VIDEO_SKeeper_DBG_Ioctl(unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		case BE_IOW_STATUS_KEEPER_DBG:
		{
			...
			ret = RET_OK;//__VIDEO_SKeeper_DBG_Debugger(param);
			BE_NOTI("(STATUS_KEEPER_DBG) not support\n");
		}
		break;
...
int __VIDEO_SKeeper_DBG_CheckStatus(void)
{
	...
	do{
		...
		ST_KEEPER_PRINT("    -> TABLE : Index count[%d]/pRsrTable[%p]\n",\
						 				gSKeeperDataBase->table[i].rsrIndexCount,\
										gSKeeperDataBase->table[i].pRsrIndex);
		/*               12345678901234567890123456789012345678901234567890 */
		ST_KEEPER_PRINT("    -> IO_LOG :pStartAddr[%p]/pEndAddr[%p]\n",\
										gSKeeperDataBase->io_log[i].pStartAddress,\
										gSKeeperDataBase->io_log[i].pEndAddress);
		ST_KEEPER_PRINT("               Head[0x%x]/status[%d]\n",\
										gSKeeperDataBase->io_log[i].Head,\
										gSKeeperDataBase->io_log[i].status);
		ST_KEEPER_PRINT("    -> DP_LOG :pStartAddr[%p]/pEndAddr[%p]\n",\
										gSKeeperDataBase->dispatch_log[i].pStartAddress,\
										gSKeeperDataBase->dispatch_log[i].pEndAddress);
		ST_KEEPER_PRINT("               Head[0x%x]/status[%d]\n",\
...
int __VIDEO_SKeeper_DBG_PrintHistoryLog( LX_VP_ST_KEEPER_MODULE_TYPE_T module, LX_VP_ST_KEEPER_HISTORY_TYPE_T type)
{
	...
	do{
		...
		if((historyDatabase.pStartAddress == NULL)||(historyDatabase.pEndAddress == NULL)){
			ST_KEEPER_PRINT("[%s:%d] Error! One of hisotry pointer was NULL. pS[%p], pE[%p]\n",\
								__F__, __L__, historyDatabase.pStartAddress, historyDatabase.pEndAddress);
			break;
		}
		...
		draw_size = (unsigned int)((uintptr_t)historyDatabase.pEndAddress - (uintptr_t)historyDatabase.pStartAddress);
