			 ...
			 enum kmsg_dump_reason reason)
{
	...
	if (!inited) {
		...
		if (reason == KMSG_DUMP_PANIC)
			hdr.type = DUMP_PANIC;
		else if (reason == KMSG_DUMP_DMVERITY)
			hdr.type = DUMP_DMVERITY;
		else
			hdr.type = DUMP_WDT;
		...
		hdr.timestamp = ns_to_timespec64(ktime_get_real_fast_ns());
	...
	while (1) {
		if (!kmsg_dump_get_line(&iter, false, text,
					sizeof(text), &len)) {
			if (offset) {
				last = 1;
				need_to_write = 1;
