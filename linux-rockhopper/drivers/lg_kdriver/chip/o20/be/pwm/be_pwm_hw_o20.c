...
#include "be_pwm_hw_o20.h"

#include "../reg/be_reg_pwm_o20.h"
...
/*----------------------------------------------------------------------------------------
	External Variables
----------------------------------------------------------------------------------------*/
extern PE_PWM_REG_O20_T gPE_PWM_O20;
...
int BE_PWM_HW_O20_SetPwmAdaptFreqControl(BE_PWM_ADAPT_FREQ_CTRL_T *pstParams)
{
	...
	do {
		...
		if (freqSet)
		{
			...
			ret = BE_PWM_HW_O20_SetPwmSync(&stParams);
	...
	do {
		...
		for(i = 0; i < LX_BE_PWM_PORT_MAX; i++)
		{
			if ((pwmParams.portMask >> i) & 0x01)
			{
				...
				ret = BE_PWM_HW_O20_SetPwmControl(&pwmCtrlParam);
				if(ret) BREAK_WRONG(ret);
				...
				ret = BE_PWM_HW_O20_SetPwmAdaptFreqControl(&pwmAdaptFreqCtrlParam);
				if(ret) BREAK_WRONG(ret);
				...
				if (!pwmParams.portParams[i].adaptFreqEn)
				{
					...
					ret = BE_PWM_HW_O20_SetPwmFrequency(&pwmFreqParams);
					if(ret) BREAK_WRONG(ret);
				}

				pwmDutyParam.port = i;
				ret = BE_PWM_HW_O20_SetPwmDutyCycle(&pwmDutyParam);
				if(ret) BREAK_WRONG(ret);
			}
		}
...
static int PWM_SetFrequency(UINT32 pwmId, UINT32 freq)
{
	...
	if (1) // Check clock setting
	{
		...
		//CTOP_CTRL_O20A0_RdFL(CTRL_TCON, crg_tcon01); // block for bring-up
		//regField = CTOP_CTRL_O20A0_RdFd(CTRL_TCON, crg_tcon01, tcon_pix2_clk_sel); // block for bring-up
		BE_PWM_REG_RD(0xC930F008,regField);
		regField = (regField & 0x3800) >> 11;
...
static int _REG_SetPwmCtrl(UINT32 pwmId, BOOLEAN zeroDuty)
{
	...
	switch (pwmId) {
		case 0 :
			PE_PWM_O20_RdFL(pe_pwm0_ctrl0);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O20_RdFL(pe_pwm0_ctrl1);
			}
			PE_PWM_O20_RdFL(pe_pwm0_ctrl2);
			PE_PWM_O20_Wr01(pe_pwm0_ctrl0, pwm0_en, gPwmInfo[pwmId].ctrl.pwm_en);
			PE_PWM_O20_Wr01(pe_pwm0_ctrl0, pwm0_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
			PE_PWM_O20_Wr01(pe_pwm0_ctrl0, pwm0_resolution, gPwmInfo[pwmId].ctrl.pwm_resolution);
			PE_PWM_O20_Wr01(pe_pwm0_ctrl0, pwm0_inv, gPwmInfo[pwmId].ctrl.pwm_inv);
			PE_PWM_O20_Wr01(pe_pwm0_ctrl0, pwm0_sel, gPwmInfo[pwmId].ctrl.pwm_sel);
			PE_PWM_O20_Wr01(pe_pwm0_ctrl0, pwm0_width_falling_pos, gPwmInfo[pwmId].ctrl.pwm_width_falling_pos);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O20_Wr01(pe_pwm0_ctrl1, pwm0_free_width, gPwmInfo[pwmId].ctrl.pwm_free_width);
			}
			PE_PWM_O20_Wr01(pe_pwm0_ctrl2, pwm0_method, gPwmInfo[pwmId].ctrl.pwm_method);
			PE_PWM_O20_Wr01(pe_pwm0_ctrl2, pwm0_fc_h_disp, gPwmInfo[pwmId].ctrl.pwm_fc_h_disp);
			if(zeroDuty && (!gPwmInfo[pwmId].ctrl.pwm_fc_h_disp))
			{
				PE_PWM_O20_Wr01(pe_pwm0_ctrl2, pwm0_fc_l_disp, 1);
			}
			else
			{
				PE_PWM_O20_Wr01(pe_pwm0_ctrl2, pwm0_fc_l_disp, gPwmInfo[pwmId].ctrl.pwm_fc_l_disp);
			}
			PE_PWM_O20_WrFL(pe_pwm0_ctrl0);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O20_WrFL(pe_pwm0_ctrl1);
			}
			PE_PWM_O20_WrFL(pe_pwm0_ctrl2);
			break;
		case 1 :
			PE_PWM_O20_RdFL(pe_pwm1_ctrl0);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O20_RdFL(pe_pwm1_ctrl1);
			}
			PE_PWM_O20_RdFL(pe_pwm1_ctrl2);
			PE_PWM_O20_Wr01(pe_pwm1_ctrl0, pwm1_en, gPwmInfo[pwmId].ctrl.pwm_en);
			PE_PWM_O20_Wr01(pe_pwm1_ctrl0, pwm1_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
			PE_PWM_O20_Wr01(pe_pwm1_ctrl0, pwm1_resolution, gPwmInfo[pwmId].ctrl.pwm_resolution);
			PE_PWM_O20_Wr01(pe_pwm1_ctrl0, pwm1_inv, gPwmInfo[pwmId].ctrl.pwm_inv);
			PE_PWM_O20_Wr01(pe_pwm1_ctrl0, pwm1_sel, gPwmInfo[pwmId].ctrl.pwm_sel);
			PE_PWM_O20_Wr01(pe_pwm1_ctrl0, pwm1_width_falling_pos, gPwmInfo[pwmId].ctrl.pwm_width_falling_pos);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O20_Wr01(pe_pwm1_ctrl1, pwm1_free_width, gPwmInfo[pwmId].ctrl.pwm_free_width);
			}
			PE_PWM_O20_Wr01(pe_pwm1_ctrl2, pwm1_method, gPwmInfo[pwmId].ctrl.pwm_method);
			PE_PWM_O20_Wr01(pe_pwm1_ctrl2, pwm1_fc_h_disp, gPwmInfo[pwmId].ctrl.pwm_fc_h_disp);
			if(zeroDuty && (!gPwmInfo[pwmId].ctrl.pwm_fc_h_disp))
			{
				PE_PWM_O20_Wr01(pe_pwm1_ctrl2, pwm1_fc_l_disp, 1);
			}
			else
			{
				PE_PWM_O20_Wr01(pe_pwm1_ctrl2, pwm1_fc_l_disp, gPwmInfo[pwmId].ctrl.pwm_fc_l_disp);
			}
			PE_PWM_O20_WrFL(pe_pwm1_ctrl0);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O20_WrFL(pe_pwm1_ctrl1);
			}
			PE_PWM_O20_WrFL(pe_pwm1_ctrl2);
			break;
		case 2 :
			PE_PWM_O20_RdFL(pe_pwm2_ctrl0);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O20_RdFL(pe_pwm2_ctrl1);
			}
			PE_PWM_O20_RdFL(pe_pwm2_ctrl2);
			PE_PWM_O20_Wr01(pe_pwm2_ctrl0, pwm2_en, gPwmInfo[pwmId].ctrl.pwm_en);
			PE_PWM_O20_Wr01(pe_pwm2_ctrl0, pwm2_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
			PE_PWM_O20_Wr01(pe_pwm2_ctrl0, pwm2_resolution, gPwmInfo[pwmId].ctrl.pwm_resolution);
			PE_PWM_O20_Wr01(pe_pwm2_ctrl0, pwm2_inv, gPwmInfo[pwmId].ctrl.pwm_inv);
			PE_PWM_O20_Wr01(pe_pwm2_ctrl0, pwm2_sel, gPwmInfo[pwmId].ctrl.pwm_sel);
			PE_PWM_O20_Wr01(pe_pwm2_ctrl0, pwm2_width_falling_pos, gPwmInfo[pwmId].ctrl.pwm_width_falling_pos);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O20_Wr01(pe_pwm2_ctrl1, pwm2_free_width, gPwmInfo[pwmId].ctrl.pwm_free_width);
			}
			PE_PWM_O20_Wr01(pe_pwm2_ctrl2, pwm2_method, gPwmInfo[pwmId].ctrl.pwm_method);
			PE_PWM_O20_Wr01(pe_pwm2_ctrl2, pwm2_fc_h_disp, gPwmInfo[pwmId].ctrl.pwm_fc_h_disp);
			if(zeroDuty && (!gPwmInfo[pwmId].ctrl.pwm_fc_h_disp))
			{
				PE_PWM_O20_Wr01(pe_pwm2_ctrl2, pwm2_fc_l_disp, 1);
			}
			else
			{
				PE_PWM_O20_Wr01(pe_pwm2_ctrl2, pwm2_fc_l_disp, gPwmInfo[pwmId].ctrl.pwm_fc_l_disp);
			}
			PE_PWM_O20_WrFL(pe_pwm2_ctrl0);
			if (!gPwmInfo[pwmId].lowPowerModeEn) // W/A for M+ low power mode
			{
				PE_PWM_O20_WrFL(pe_pwm2_ctrl1);
			}
			PE_PWM_O20_WrFL(pe_pwm2_ctrl2);
			break;
		case 3 :
			PE_PWM_O20_RdFL(pe_pwm3_ctrl0);
			PE_PWM_O20_Wr01(pe_pwm3_ctrl0, pwm3_en, gPwmInfo[pwmId].ctrl.pwm_en);
			PE_PWM_O20_Wr01(pe_pwm3_ctrl0, pwm3_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
			PE_PWM_O20_WrFL(pe_pwm3_ctrl0);
			break;
		default :
			break;
	}
...
static int _REG_SetPwmTiming(UINT32 pwmId)
{
	...
	switch (pwmId) {
		case 0 :
			PE_PWM_O20_RdFL(pe_pwm_v_load_write);
			PE_PWM_O20_RdFL(pe_pwm0_v_r);
			PE_PWM_O20_RdFL(pe_pwm0_v_f);
			PE_PWM_O20_RdFL(pe_pwm0_v_sub_r);
			PE_PWM_O20_RdFL(pe_pwm0_v_sub_f);
			PE_PWM_O20_Wr01(pe_pwm_v_load_write, pwm0_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
			PE_PWM_O20_Wr01(pe_pwm0_v_r, pwm0_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
			PE_PWM_O20_Wr01(pe_pwm0_v_r, pwm0_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			PE_PWM_O20_Wr01(pe_pwm0_v_f, pwm0_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
			PE_PWM_O20_Wr01(pe_pwm0_v_f, pwm0_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			PE_PWM_O20_Wr01(pe_pwm_v_load_write, pwm0_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			PE_PWM_O20_Wr01(pe_pwm0_v_sub_r, pwm0_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
			PE_PWM_O20_Wr01(pe_pwm0_v_sub_r, pwm0_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			PE_PWM_O20_Wr01(pe_pwm_v_load_write, pwm0_v_sub_f_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			PE_PWM_O20_Wr01(pe_pwm0_v_sub_f, pwm0_v_sub_f, gPwmInfo[pwmId].t2nd.pwm_v_f);
			PE_PWM_O20_Wr01(pe_pwm0_v_sub_f, pwm0_v_sub_f_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			PE_PWM_O20_WrFL(pe_pwm_v_load_write);
			PE_PWM_O20_WrFL(pe_pwm0_v_r);
			PE_PWM_O20_WrFL(pe_pwm0_v_f);
			PE_PWM_O20_WrFL(pe_pwm0_v_sub_r);
			PE_PWM_O20_WrFL(pe_pwm0_v_sub_f);
			break;
		case 1 :
			PE_PWM_O20_RdFL(pe_pwm_v_load_write);
			PE_PWM_O20_RdFL(pe_pwm1_v_r);
			PE_PWM_O20_RdFL(pe_pwm1_v_f);
			PE_PWM_O20_RdFL(pe_pwm1_v_sub_r);
			PE_PWM_O20_RdFL(pe_pwm1_v_sub_f);
			PE_PWM_O20_Wr01(pe_pwm_v_load_write, pwm1_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
			PE_PWM_O20_Wr01(pe_pwm1_v_r, pwm1_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
			PE_PWM_O20_Wr01(pe_pwm1_v_r, pwm1_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			PE_PWM_O20_Wr01(pe_pwm1_v_f, pwm1_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
			PE_PWM_O20_Wr01(pe_pwm1_v_f, pwm1_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			PE_PWM_O20_Wr01(pe_pwm_v_load_write, pwm1_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			PE_PWM_O20_Wr01(pe_pwm1_v_sub_r, pwm1_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
			PE_PWM_O20_Wr01(pe_pwm1_v_sub_r, pwm1_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			PE_PWM_O20_Wr01(pe_pwm_v_load_write, pwm1_v_sub_f_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			PE_PWM_O20_Wr01(pe_pwm1_v_sub_f, pwm1_v_sub_f, gPwmInfo[pwmId].t2nd.pwm_v_f);
			PE_PWM_O20_Wr01(pe_pwm1_v_sub_f, pwm1_v_sub_f_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			PE_PWM_O20_WrFL(pe_pwm_v_load_write);
			PE_PWM_O20_WrFL(pe_pwm1_v_r);
			PE_PWM_O20_WrFL(pe_pwm1_v_f);
			PE_PWM_O20_WrFL(pe_pwm1_v_sub_r);
			PE_PWM_O20_WrFL(pe_pwm1_v_sub_f);
			break;
		case 2 :
			PE_PWM_O20_RdFL(pe_pwm_v_load_write);
			PE_PWM_O20_RdFL(pe_pwm2_v_r);
			PE_PWM_O20_RdFL(pe_pwm2_v_f);
			PE_PWM_O20_RdFL(pe_pwm2_v_sub_r);
			PE_PWM_O20_RdFL(pe_pwm2_v_sub_f);
			PE_PWM_O20_Wr01(pe_pwm_v_load_write, pwm2_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
			PE_PWM_O20_Wr01(pe_pwm2_v_r, pwm2_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
			PE_PWM_O20_Wr01(pe_pwm2_v_r, pwm2_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			PE_PWM_O20_Wr01(pe_pwm2_v_f, pwm2_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
			PE_PWM_O20_Wr01(pe_pwm2_v_f, pwm2_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
			PE_PWM_O20_Wr01(pe_pwm_v_load_write, pwm2_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			PE_PWM_O20_Wr01(pe_pwm2_v_sub_r, pwm2_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
			PE_PWM_O20_Wr01(pe_pwm2_v_sub_r, pwm2_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			PE_PWM_O20_Wr01(pe_pwm_v_load_write, pwm2_v_sub_f_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
			PE_PWM_O20_Wr01(pe_pwm2_v_sub_f, pwm2_v_sub_f, gPwmInfo[pwmId].t2nd.pwm_v_f);
			PE_PWM_O20_Wr01(pe_pwm2_v_sub_f, pwm2_v_sub_f_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
			PE_PWM_O20_WrFL(pe_pwm_v_load_write);
			PE_PWM_O20_WrFL(pe_pwm2_v_r);
			PE_PWM_O20_WrFL(pe_pwm2_v_f);
			PE_PWM_O20_WrFL(pe_pwm2_v_sub_r);
			PE_PWM_O20_WrFL(pe_pwm2_v_sub_f);
			break;
		default :
			break;
	}
...
static int _REG_GetPwmTiming(UINT32 freq_mode, UINT32 *pRising, UINT32 *pFall)
{
	...
	do {
		...
		{
			case 0 :
				PE_PWM_O20_RdFL(pe_pwm3_0_low);
				PE_PWM_O20_RdFL(pe_pwm3_0_high);
				PE_PWM_O20_Rd01(pe_pwm3_0_low, pwm3_0_low, *pRising);
				PE_PWM_O20_Rd01(pe_pwm3_0_high, pwm3_0_high, *pFall);
				break;
			case 1 :
				PE_PWM_O20_RdFL(pe_pwm3_1_low);
				PE_PWM_O20_RdFL(pe_pwm3_1_high);
				PE_PWM_O20_Rd01(pe_pwm3_1_low, pwm3_1_low, *pRising);
				PE_PWM_O20_Rd01(pe_pwm3_1_high, pwm3_1_high, *pFall);
				break;
			case 2 :
				PE_PWM_O20_RdFL(pe_pwm3_2_low);
				PE_PWM_O20_RdFL(pe_pwm3_2_high);
				PE_PWM_O20_Rd01(pe_pwm3_2_low, pwm3_2_low, *pRising);
				PE_PWM_O20_Rd01(pe_pwm3_2_high, pwm3_2_high, *pFall);
				break;
			case 3 :
				PE_PWM_O20_RdFL(pe_pwm3_3_low);
				PE_PWM_O20_RdFL(pe_pwm3_3_high);
				PE_PWM_O20_Rd01(pe_pwm3_3_low, pwm3_3_low, *pRising);
				PE_PWM_O20_Rd01(pe_pwm3_3_high, pwm3_3_high, *pFall);
				break;
			default :
