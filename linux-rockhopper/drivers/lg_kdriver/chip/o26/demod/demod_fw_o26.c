...
#include <O26_DEMOD_ALL_FW.h>
#define DEMOD_DDR_FW 	O26_DEMOD_ALL_FW
#define DEMOD_FW_NAME 	"O26_DEMOD_ALL_FW"

/* macro definition */
#define DEMOD_CHIP_INFO	0x26a0  //chungiii
 ...
 */
static int _DEMOD_O26_FW_Read_FilePath(const char *path, void *buf, size_t max_size)
{
	...
	size = kernel_read_file_from_path(path, 0, &buf, max_size, NULL,
					READING_FIRMWARE);
	if (size < 0) {
		DEMOD_ERROR("read failed %s, %ld", path, size);
		return size;
	}
#endif
	return size;
...
static int _DEMOD_O26_FW_Load(UINT8 * fw, int * fwlen) {
	...
	for (i = 0; i < path_num; i++) {
		if (_demod_fw_path_list[i]) {
			size = _DEMOD_O26_FW_Read_FilePath(_demod_fw_path_list[i], fw, LX_DEMOD_FW_MAXSIZE);
			if (size > 0) {
				break;
			}
		}
	...
	DEMOD_NOTI("demod fw find~!!!read %s (%d bytes)\n", _demod_fw_path_list[i], size);
	*fwlen = size;

	return NO_DEMOD_ERROR;
}

static void _DEMOD_O26_MCU_Reset (BOOL flag) {
#if 1 //chungiii 
	if(flag) {
		CTOP_CTRL_O26Ax_RdFL(SCRG_CTRL_DBB, crg_dbb003);
		CTOP_CTRL_O26Ax_Wr01(SCRG_CTRL_DBB, crg_dbb003, reg_soft_rst_mcu_n, 1);
		CTOP_CTRL_O26Ax_WrFL(SCRG_CTRL_DBB, crg_dbb003);
		DEMOD_PRINT("set CTOP DBB MCU reset\n");
	} else {
		CTOP_CTRL_O26Ax_RdFL(SCRG_CTRL_DBB, crg_dbb003);
		CTOP_CTRL_O26Ax_Wr01(SCRG_CTRL_DBB, crg_dbb003, reg_soft_rst_mcu_n, 0);
		CTOP_CTRL_O26Ax_WrFL(SCRG_CTRL_DBB, crg_dbb003);
...
static void _DEMOD_O26_REG_Init	(BOOL flag) {
#if 1 //chungiii 
	if(flag) {
		CTOP_CTRL_O26Ax_RdFL(IP_CTRL_DBB, ipw_dbb_004);
		CTOP_CTRL_O26Ax_Wr01(IP_CTRL_DBB, ipw_dbb_004, reg_dbb_reg_init, 1);
		CTOP_CTRL_O26Ax_WrFL(IP_CTRL_DBB, ipw_dbb_004);
		DEMOD_PRINT("set CTOP DBB REG init\n");
	} else {
		CTOP_CTRL_O26Ax_RdFL(IP_CTRL_DBB, ipw_dbb_004);
		CTOP_CTRL_O26Ax_Wr01(IP_CTRL_DBB, ipw_dbb_004, reg_dbb_reg_init, 0);
		CTOP_CTRL_O26Ax_WrFL(IP_CTRL_DBB, ipw_dbb_004);
...
static int _DEMOD_O26_ResetFirmware (LX_DEMOD_L2_Context * pFrontend) {
...
static int _DEMOD_O26_StopFirmware (LX_DEMOD_L2_Context * pFrontend) {
	int ret = NO_DEMOD_ERROR;
...
static int _DEMOD_O26_InitFirmware (LX_DEMOD_L2_Context * pFrontend, UINT32 fw_clk, UINT32 adc_clk) {
	int ret = NO_DEMOD_ERROR;
	...
	if (pFrontend->demod->chip->fw_type == DEMOD_FW_TYPE_INV) {
		_DEMOD_O26_MCU_Reset(MCU_RESET);		SPEND_TIME_CHECK("MCU_RESET");
		...
		_DEMOD_O26_MCU_Reset(MCU_RELEASE);		SPEND_TIME_CHECK("MCU_RELEASE");
		DEMOD_L0_system_wait(1);			SPEND_TIME_CHECK("wait(1)");

		_DEMOD_O26_REG_Init(REG_SET);			SPEND_TIME_CHECK("REG_SET");
		DEMOD_L0_system_wait(1);			SPEND_TIME_CHECK("wait(1)");
		_DEMOD_O26_REG_Init(REG_CLEAR);			SPEND_TIME_CHECK("REG_CLEAR");
	} else {
		/* MCU FW control register init */
...
#if 0
static int _DEMOD_O26_CheckFirmware (LX_DEMOD_L2_Context * pFrontend, LX_DEMOD_FW_STRUCT * fw_table) {
	UINT32 * fw_addr = NULL;
	int ret = RET_OK;
	...
	ret = DEMOD_O26_FW_Get_Info(data, NULL, NULL, NULL, NULL);
...
static int _DEMOD_O26_LoadFirmware_SOC (LX_DEMOD_L2_Context * pFrontend, UINT8 * data, UINT32 size) {
	UINT32 * fw_addr = NULL;
...
static int _DEMOD_O26_LoadFirmware_EXT (LX_DEMOD_L2_Context * pFrontend, UINT8 * data, UINT32 size) {
	int ret = RET_OK;
	int tryCount = 0;
...
static int _DEMOD_O26_LoadFirmware (LX_DEMOD_L2_Context * pFrontend, LX_DEMOD_FW_STRUCT * fw_table) {
	...
	ret = _DEMOD_O26_FW_Load(&_firmware_file.firmware_table[0], &_firmware_file.firmware_size);
...
#if (DEMOD_COMM == DEMOD_COMM_APB)
	ret = _DEMOD_O26_LoadFirmware_SOC(pFrontend, data, size);
	mem_base_addr = gMemCfgDEMOD_FW.memory_base;
#elif (DEMOD_COMM == DEMOD_COMM_I2C) || (DEMOD_COMM == DEMOD_COMM_FPGA)
	ret = _DEMOD_O26_LoadFirmware_EXT(pFrontend, data, size);
...
static int _DEMOD_O26_StartFirmware (LX_DEMOD_L2_Context * pFrontend) {
	int tryCount = 0;
	int ret = 0;
...
static int _DEMOD_O26_FW_FPAG_Reset(void) {
...
static int _DEMOD_O26_FW_Initialize(demod_fw_type fw_type) {
	...
	// o26_fpga, for fpga integrated test
	...
	if (pFrontend->demod->chip->fw_type == DEMOD_FW_TYPE_INV) {
		...
		if (NO_DEMOD_ERROR != _DEMOD_O26_StopFirmware(pFrontend)) {
			ret = ERROR_DEMOD_LOADING_FIRMWARE;
			goto return_with_api_unlock;
		}						SPEND_TIME_CHECK("StopFirmare");

		DEMOD_O26_IPC_Enable(FALSE);			SPEND_TIME_CHECK("IPC Disable");

		DEMOD_O26_IPC_Reset();				SPEND_TIME_CHECK("IPC_Reset");

		if (NO_DEMOD_ERROR != _DEMOD_O26_InitFirmware(pFrontend, fw_clk, adc_clk)) {
			ret = ERROR_DEMOD_LOADING_FIRMWARE;
			goto return_with_api_unlock;
		}						SPEND_TIME_CHECK("InitFirmware");

		if (pFrontend->demod->chip->fw_skip == 0) {
			if (NO_DEMOD_ERROR != _DEMOD_O26_LoadFirmware(pFrontend, fw_table)) {
				ret = ERROR_DEMOD_LOADING_FIRMWARE;
				goto return_with_api_unlock;
			}
		...
		if (NO_DEMOD_ERROR != DEMOD_L2_Set_SoftRst(pFrontend, 1, 0, 0, 0, 0, 0, 0)) {
			...
			wmb();
			return _DEMOD_O26_FW_Initialize(fw_type);
			//goto return_with_api_unlock;
		...
		if (NO_DEMOD_ERROR != _DEMOD_O26_ResetFirmware(pFrontend)) {
			rmb();
	...
	//DEMOD_O26_Set_UART(TRUE);

	if (NO_DEMOD_ERROR != _DEMOD_O26_StartFirmware(pFrontend)) {
		ret = ERROR_DEMOD_LOADING_FIRMWARE;
		goto return_with_api_unlock;
	}							SPEND_TIME_CHECK("StartFirmware");

	DEMOD_O26_IPC_Enable(TRUE);				SPEND_TIME_CHECK("IPC Enable");
...
int DEMOD_O26_FW_Initialize (void)
{
	...
	return _DEMOD_O26_FW_Initialize(DEMOD_FW_TYPE_TER);
}

int DEMOD_O26_SAT_FW_Initialize (void)
{
	DEMOD_NOTI("FW_TYPE_SAT\n");
	return _DEMOD_O26_FW_Initialize(DEMOD_FW_TYPE_SAT);
}
