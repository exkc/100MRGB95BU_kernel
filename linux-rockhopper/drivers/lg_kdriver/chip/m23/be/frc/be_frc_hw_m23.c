...
#undef BE_FRC_HW_M23_TEMP_USE_TYPE_BY_CTRL_SPATH
...
/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
//static BE_FRC_HW_M23_INFO_T _beFrcHwM23Info= {0,};
static BE_FRC_HW_M23_INFO_T _gBeFrcHwM23Info= {0,};
static BE_IPC_FRC_MEMC_MODE_T gPreMemcParam;
/*========================================================================================
	Implementation Group
========================================================================================*/
int BE_FRC_HW_M23_Initialize(void)
{
	int ret = RET_OK;

	do {
		memset(&_gBeFrcHwM23Info, 0, sizeof(BE_FRC_HW_M23_INFO_T));
...
int BE_FRC_HW_M23_MemcResume(void)
{
	int ret = RET_OK;
...
int BE_FRC_HW_M23_ControlMemc(LX_BE_FRC_MEMC_MODE_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		BE_TRACE1("%d, %d, %d -> %d, %d, %d\n", pstParams->u8MemcType, pstParams->u8Blur, pstParams->u8Judder,\
			_gBeFrcHwM23Info.u32MemcType, _gBeFrcHwM23Info.u32Blur, _gBeFrcHwM23Info.u32Judder);
		...
		stHwParams.uFRCUserLevel	= pstParams->u8MemcType == 0xFF ? 0xFFFFFFFF : pstParams->u8MemcType;
		 ...
		 if (stHwParams.uFRCBlur != 0xFFFFFFFF)
			_gBeFrcHwM23Info.u32Blur  = stHwParams.uFRCBlur;

  	        if (stHwParams.uFRCJudder != 0xFFFFFFFF)
			_gBeFrcHwM23Info.u32Judder        = stHwParams.uFRCJudder;

               if (stHwParams.uFRCUserLevel != 0xFFFFFFFF)
			_gBeFrcHwM23Info.u32MemcType  = stHwParams.uFRCUserLevel;

		BE_TRACE1("%d, %d, %d -> %d, %d, %d\n", pstParams->u8MemcType, pstParams->u8Blur, pstParams->u8Judder,\
		_gBeFrcHwM23Info.u32MemcType, _gBeFrcHwM23Info.u32Blur, _gBeFrcHwM23Info.u32Judder);

		BE_FRC_HW_M23_SetMemcMode(&_gBeFrcHwM23Info);
	} while (0);

	return ret;
}

int BE_FRC_HW_M23_SetMemcLowDelay(LX_BE_FRC_LOW_DELAY_T *pstParams)
{
	int ret = RET_OK;
	
	do {
 		BE_TRACE1("%d -> %d\n", pstParams->u32Lowdelay, _gBeFrcHwM23Info.u32LowDelay);
		
		if (pstParams->u32Lowdelay != 0xFFFFFFFF)
			_gBeFrcHwM23Info.u32LowDelay = pstParams->u32Lowdelay;
...
int BE_FRC_HW_M23_SetMemcMotionPro(LX_BE_FRC_MOTION_PRO_T *pstParams)
{
	int ret = RET_OK;

	do {
 		BE_TRACE1("%d -> %d\n", pstParams->u32MotionPro, _gBeFrcHwM23Info.u32MotionPro);
		
		 if (pstParams->u32MotionPro != 0xFFFFFFFF)
			_gBeFrcHwM23Info.u32MotionPro= pstParams->u32MotionPro;
...
int BE_FRC_HW_M23_SetMemcMode(BE_FRC_HW_M23_INFO_T *_gBeFrcHwM23Info)
{
	int ret = RET_OK;
	...
	do {
		...
		if (_gBeFrcHwM23Info->u32LowDelay == 1)
		{
			stHwParams.uFRCUserLevel= LX_BE_FRC_MEMC_DISABLE;
		}
		else
		{
			stHwParams.uFRCUserLevel= _gBeFrcHwM23Info->u32MemcType;
		}
		stHwParams.uFRCBlur	= _gBeFrcHwM23Info->u32Blur;
		stHwParams.uFRCJudder = _gBeFrcHwM23Info->u32Judder;
		stHwParams.uFRCMotionProEn = _gBeFrcHwM23Info->u32MotionPro;
		BE_TRACE1("%d,%d,%d/%d/%d -> MemcType(%d), Blur(%d), Judder(%d), MPro(%d)\n",\
			_gBeFrcHwM23Info->u32MemcType, _gBeFrcHwM23Info->u32Blur,_gBeFrcHwM23Info->u32Judder,\
			_gBeFrcHwM23Info->u32LowDelay, _gBeFrcHwM23Info->u32MotionPro, stHwParams.uFRCUserLevel,\
		...
		else if (_gBeFrcHwM23Info->u32MotionPro ==1)
		{
...
int BE_FRC_HW_M23_SetConfig(LX_BE_FRC_CFG_T *pstParams)
{
	int ret = RET_OK;
	do {
		/* check error */
		...
		{
			#ifdef BE_FRC_HW_M23_TEMP_USE_TYPE_BY_CTRL_SPATH
...
int BE_FRC_HW_M23_RunCommand(LX_BE_FRC_CMD_T *pstParams)
{
	int ret = RET_OK;
	UINT32 strLen;
	do {
...
int BE_FRC_HW_M23_Set_Demo_Mode(LX_BE_FRC_DEMO_MODE_T *pstParams)
{
	int ret = RET_OK;
...
int BE_FRC_HW_M23_Set_Debug_Box(LX_BE_FRC_ENABLE_DEBUG_BOX_T *pstParams)
{
	int ret = RET_OK;
...
int BE_FRC_HW_M23_Set_Bypass_Memc_Reg(LX_BE_FRC_MEMC_BYPASS_REGION_T *pstParams)
{
	int ret = RET_OK;
...
int BE_FRC_HW_M23_Set_YUV_444_Mode(LX_BE_FRC_COLOR_YUV444_T *pstParams)
{
	int ret = RET_OK;
...
int BE_FRC_HW_M23_Set_Low_Delay(LX_BE_FRC_LOW_DELAY_T *pstParams)
{
	int ret = RET_OK;
	unsigned int status = 0;
...
int BE_FRC_HW_M23_Get_Firmware_Ver(LX_BE_FRC_VERSION_T *pstParams)
{
	int ret = RET_OK;
...
int BE_FRC_HW_M23_Force_Frame_Rate(LX_BE_FRC_FRAME_RATE_T *pstParams)
{
	int ret = RET_OK;
...
int BE_FRC_HW_M23_Set_Freeze(LX_BE_FRC_FREEZE_T *pstParams)
{
	int ret = RET_OK;
	unsigned int status;
...
int BE_FRC_HW_M23_Update_FrameDelay(LX_BE_LOW_DELAY_T *pstParams)
{
	int ret = RET_OK;
...
int BE_FRC_HW_M23_ProcHandler(BE_FRC_PROC_ID_T proc_id, struct seq_file* m, void* data)
{
	switch(proc_id)
	{
