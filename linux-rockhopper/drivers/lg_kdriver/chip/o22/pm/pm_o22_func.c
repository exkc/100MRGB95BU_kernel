...
extern struct regulator *o22_regulator;
...
/*----------------------------------------------------------------------------------------
	Macro Definitions
----------------------------------------------------------------------------------------*/
static DEFINE_SPINLOCK(g_o22_pm_lock);
static DEFINE_SPINLOCK(g_o22_regul_lock);
//static DEFINE_SPINLOCK(g_o22_cpuregul_lock);
static DEFINE_SPINLOCK(g_o22_freq_lock);

static unsigned int o22_regul = 0;

module_param(o22_regul, uint, S_IRUGO);
MODULE_PARM_DESC(o22_gpuNP, "o22_regul");
...
void __iomem *o22_clk0_base = NULL;
void __iomem *o22_clk1_base = NULL;
void __iomem *o22_clk_pdb_base = NULL;
void __iomem *o22_clk_src_base = NULL;



#ifdef O22_CPUCTRL_I2C_SLT
static  pms_o22_regulator_t  _stsltcpuRegulator[] =
{
	/* vbase== 1030->1040 mV */
...
static  pms_o22_regulator_t  _stsltcoreRegulator[] =
{
	/* vbase== 900->910 mV */
...
#else
static	pms_o22_regulator_t  _stRegulator[] =
{
	/* vbase== 883 mV */
...
typedef struct
{
	UINT32 vout_ref;
	UINT32 vout_set;
}pms_o22_regulatorB_t;

/* TPS549B22 table */
static  pms_o22_regulatorB_t  _stRegulatorB[] =
{
	/*mv, set*/
	{1150,540},
	{1140,535},
...
/* MP8876 table */
static  pms_o22_regulatorB_t  _stRegulatorMPS[] =
{
	/*mv, set*/
	{1150,85},
	{1140,84},
...
static pms_o22_work_t g_o22_cpuregul;
#endif
static int g_o22_pm_task_disable = 0; // 이 변수는 CM3 RECOVERY용 변수임
static pms_o22_work_t g_o22_pms;
static pms_o22_work_t g_o22_regul;

static unsigned int g_o22_sampling_rate = 20; // 20ms
static unsigned int g_o22_core_vol = 0;

static unsigned int g_o22_err = 0;
...
#define O22_CORE_REGULATOR_IDX			2

#define O22_CORE_REGULATOR_DEVICE_ID_TPS549A20	0x3E
#define O22_CORE_REGULATOR_DEVICE_ID_TPS549B22	0x3A
#define O22_CORE_REGULATOR_DEVICE_ID_MPS	0xc0
...
int O22_CORE_REGUL_WRI2C( UINT8 ch, UINT8 subaddr,UINT8* data, UINT8 nbyte)
{
	...
	if (o22_regul == REGULATOR_TPS549A20)
	{
		rc = REG_WriteI2CL(ch, O22_CORE_REGULATOR_DEVICE_ID_TPS549A20,subaddr, data, nbyte);
	}
	else if (o22_regul == REGULATOR_TPS549B22)
	{
		rc = REG_WriteI2CL(ch, O22_CORE_REGULATOR_DEVICE_ID_TPS549B22,subaddr, data,nbyte);
	}
	else if (o22_regul == REGULATOR_MPS)
	{
		rc = REG_WriteI2CL(ch, O22_CORE_REGULATOR_DEVICE_ID_MPS,subaddr, data,nbyte);
	}
	else
	{
...
int O22_CORE_REGUL_RDI2C(UINT8 ch, UINT8 subaddr, UINT8* data, UINT8 nbyte)
{
	...
	if (o22_regul == REGULATOR_TPS549A20)
	{
		rc = REG_ReadI2CL(ch , O22_CORE_REGULATOR_DEVICE_ID_TPS549A20,subaddr, data,nbyte);
	}
	else if (o22_regul == REGULATOR_TPS549B22)
	{
		rc = REG_ReadI2CL(ch, O22_CORE_REGULATOR_DEVICE_ID_TPS549B22,subaddr, data,nbyte);
	}
	else if (o22_regul == REGULATOR_MPS)
	{
		rc = REG_ReadI2CL(ch, O22_CORE_REGULATOR_DEVICE_ID_MPS,subaddr, data,nbyte);
	}
	else
	{
		PM_NOTI("check DCDC vendor[%d]\n",o22_regul);
...
unsigned int o22_get_error_state(void)
{
	return g_o22_err;
}

int o22_set_error_state(unsigned int value)
{
	g_o22_err = value;
...
static void o22_set_work(pms_o22_work_t o22_work)
{
	unsigned long flags;

	spin_lock_irqsave(&g_o22_pm_lock, flags);
	g_o22_pms = o22_work;
	spin_unlock_irqrestore(&g_o22_pm_lock, flags);
}

static void o22_set_regul_work(pms_o22_work_t o22_work)
{
	unsigned long flags;

	spin_lock_irqsave(&g_o22_regul_lock, flags);
	g_o22_regul = o22_work;
	spin_unlock_irqrestore(&g_o22_regul_lock, flags);
}
#if 0
static void o22_set_cpuregul_work(pms_o22_work_t o22_work)
{
	unsigned long flags;

	spin_lock_irqsave(&g_o22_cpuregul_lock, flags);
	g_o22_cpuregul = o22_work;
	spin_unlock_irqrestore(&g_o22_cpuregul_lock, flags);
}
#endif

static int o22_GetCPUType(void)
{
	int ret = 0;
	pms_o22_avsinto_t info;

	pm_o22_avs = get_o22_pm_avs();
	ret = pm_o22_avs->get_cpu_avs_type(&info);

	return ret;
}

unsigned int o22_GetCPUOn(void)
{
...
unsigned int o22_cur_freq = 1400;
unsigned int o22_GetCPUFreq(void)
{
...
int o22_SetCPUFreq(unsigned int freq)
{
...
#else
	if(freq == o22_cur_freq)
		return 0;
	spin_lock_irqsave(&g_o22_freq_lock, flags);

	//(1) cpu core clock source change (AIOPLLSS_CPU ---> core533_clk, 533MHz)
	if(o22_clk_src_base != NULL)
		writel(0x00000001, o22_clk_src_base);
	//(2) AIOPLLS_CPU pdb(power-down bar) enable, power-off(reset)
	if(o22_clk_pdb_base != NULL)
		writel(0x000007BF, o22_clk_pdb_base);

	if( freq == 1400) /* dft mode freq */
	{
		//(3) AIOPLLSS_CPU setting change
		if(o22_clk0_base != NULL)
			writel(0x000D4404, o22_clk0_base);
		if(o22_clk1_base != NULL)
			writel(0x18000212, o22_clk1_base);
		o22_cur_freq = freq;
	}
	else if( freq == 1050) /* normal freq case */
	{
		//(3) AIOPLLSS_CPU setting change
		if(o22_clk0_base != NULL)
			writel(0x000E7505, o22_clk0_base);
		if(o22_clk1_base != NULL)
			writel(0x18000312, o22_clk1_base);
		o22_cur_freq = freq;
	}
	//(4) AIOPLLSS_CPU pdb disable
	if(o22_clk_pdb_base != NULL)
		writel(0x000007FF, o22_clk_pdb_base);
	...
	while ( (readl(o22_clk1_base) & 0x10) != 0x10) {  // PLL LOCK
		cur = sched_clock();
	...
	if(o22_clk_src_base != NULL)
		writel(0x00000000, o22_clk_src_base);
	spin_unlock_irqrestore(&g_o22_freq_lock, flags);
...
int o22_SetCoreSwitchingFreq(unsigned int freq)
{
	int ret = 0;
	...
	if (o22_regul == REGULATOR_TPS549A20)
	{
		...
		O22_CORE_REGUL_WRI2C(0, FREQUENCY_CONFIG, (UINT8 *)data ,1);
	...
	else if(o22_regul == REGULATOR_TPS549B22)
	{
		...
		O22_CORE_REGUL_WRI2C(0, MFR_SPECIFIC_03, (UINT8 *)data ,1);
		PM_NOTI("freq[%d]khz set[%d]\n",freq,freq_set);
	}
	else if(o22_regul == REGULATOR_MPS)
	{
		...
		O22_CORE_REGUL_WRI2C(0, SYSCTRL2, (UINT8 *)data ,1);
		PM_NOTI("freq[%d]khz set[%d]\n",freq,freq_set);
	}
	else
	{
...
int __O22_TPS549A20_set_volt(unsigned int volt, unsigned int ch)
{
	int ret = 0, i = 0;
	pms_o22_regulator_t * regul = NULL ;
	...
	if(ch == 0)
	{
		for(i=0; i < sizeof(_stsltcoreRegulator)/sizeof(pms_o22_regulator_t); i++)
		{
	...
	{
		for(i=0; i < sizeof(_stsltcpuRegulator)/sizeof(pms_o22_regulator_t); i++)
		{
...
#else
	for(i=0; i < sizeof(_stRegulator)/sizeof(pms_o22_regulator_t); i++)
	{
	...
	ret = O22_CORE_REGUL_WRI2C((UINT8)ch, OPERATION, (UINT8 *)data ,1);
	if(ret < 0 )
	{
		PM_NOTI("check tpsA\n");
		return -1;
	...
	O22_CORE_REGUL_WRI2C((UINT8)ch, ON_OFF_CONFIG, (UINT8 *)data ,1);
	...
	O22_CORE_REGUL_WRI2C((UINT8)ch, FREQUENCY_CONFIG, (UINT8 *)data ,1);

	data[0]= vout_margin_high << VOUT_MARGIN_VOMH_OFFSET | vout_margin_low << VOUT_MARGIN_VOML_OFFSET ;
	O22_CORE_REGUL_WRI2C((UINT8)ch, VOUT_MARGIN,(UINT8 *)data,1);
	//printf("vout_margin_high[%x] vout_margin_low[%x]\n",vout_margin_high,vout_margin_low);

	data[0]= vout_set;
	O22_CORE_REGUL_WRI2C((UINT8)ch, VOUT_ADJUSTMENT,(UINT8 *)data,1);
...
int __O22_TPS549B22_set_volt(unsigned int volt, unsigned int ch)
{
	int ret = 0;
	int i = 0;
	pms_o22_regulatorB_t * regul = NULL ;
	UINT32 vout_set  = 0;
	u8 data[5];
	...
	for(i=0; i < sizeof(_stRegulatorB)/sizeof(pms_o22_regulatorB_t); i++)
	{
	...
	O22_CORE_REGUL_WRI2C((UINT8)ch, MFR_SPECIFIC_03, (UINT8 *)data ,1);

	data[0]= vout_set % 256;
	data[1]= vout_set / 256;
	//PM_DEBUG( "VOUT_COMMAND [%x] [%x]\n",data[0],data[1]);
	O22_CORE_REGUL_WRI2C((UINT8)ch, VOUT_COMMAND,(UINT8 *)data,2);
	PM_NOTI("tiB volt[%d]mV set[0x%x:%d]\n",volt,vout_set,vout_set);
	return ret;

}

int __O22_MPS_set_volt(unsigned int volt, unsigned int ch)
{
	int ret = 0;
	int i = 0;
	UINT8 data[5];
	...
	for(i=0; i < sizeof(_stRegulatorMPS)/sizeof(pms_o22_regulatorB_t); i++)
	{
	...
	ret = O22_CORE_REGUL_WRI2C((UINT8)ch, SYSCTRL2, (UINT8 *)data,1);
	...
	O22_CORE_REGUL_WRI2C((UINT8)ch, SYSCTRL1, (UINT8 *)data,1);

	/* need to find valid vset table in mps */
	data[0] = vout_set << VSEL_OUTREF_OFFSET ;
	data[0] &= ~(1 << VSEL_VBOOT_OFFSET) ;
	O22_CORE_REGUL_WRI2C((UINT8)ch, VSEL, (UINT8 *)data ,1);
	PM_NOTI("mps volt[%d]mV vsel set[0x%x:%d] \n",volt,data[0],data[0]);

	while( done == 0)
	{
		O22_CORE_REGUL_RDI2C((UINT8)ch, SYSCTRL1, (UINT8 *)data ,1);
...
int o22_SetCoreVal(unsigned int volt, unsigned int ch)
{
	int ret = 0;
	...
	if (o22_regul == REGULATOR_TPS549A20)
	{
		ret =__O22_TPS549A20_set_volt(volt, ch);
	}
	else if (o22_regul == REGULATOR_TPS549B22)
	{
		ret = __O22_TPS549B22_set_volt(volt,ch);
	}
	else if (o22_regul == REGULATOR_MPS)
	{
		ret = __O22_MPS_set_volt(volt, ch);
	}
	else
	{
	...
	else if (o22_regul == REGULATOR_MPS)
	{
		...
		ret = O22_CORE_REGUL_WRI2C(VSEL, (UINT8 *)data ,1);
		if(ret < 0 )
		{
	...
	#endif
	#endif
	PM_DEBUG("o22_regul[%x] ret[%d]\n",o22_regul ,ret);


	return ret;
}
unsigned int o22_GetCoreVal(void)
{
	unsigned int mV = g_o22_core_vol;
	return mV;
}


static unsigned int o22_get_sampling_rate(void)
{
	return g_o22_sampling_rate;
}

static int o22_set_sampling_rate(unsigned int ms)
{
	g_o22_sampling_rate = ms;

	return 0;
}

static int o22_pm_task_dvfs(pms_o22_work_t *pms_o22_work)
{
	int ret = 0;
#ifdef USE_KER_CPUFREQ
	struct cpufreq_policy *policy;
	if ((pms_o22_work->do_dvfs == 1) || (pms_o22_work->do_dvfs == 2))
	{
		pms_o22_work->do_dvfs = 0;
		policy = cpufreq_cpu_get(0);
		if (policy != NULL)
			ret = cpufreq_driver_target(policy, pms_o22_work->target_freq*1000, CPUFREQ_RELATION_H);
		else
			ret = 1;

		if(ret != 0)
			o22_set_error_state(0xF << 28 | pms_o22_work->target_freq << 16 | (ERROR_MASK & ret));
	}
	else if (pms_o22_work->do_dvfs == 3)
#else
	if (pms_o22_work->do_dvfs == 1)
	{
		pms_o22_work->do_dvfs = 0;
		ret = o22_SetCPUFreq(pms_o22_work->target_freq);
		if(ret != 0)
			o22_set_error_state(0xF << 28 | pms_o22_work->target_freq << 16 | (ERROR_MASK & ret));
	}
#endif
...
static int o22_pm_task_hotplug(pms_o22_work_t *pms_o22_work)
{
	struct device * pCpudev = NULL;
	int ret = 0;

	if (pms_o22_work->do_hotplug && (o22_get_error_state() == 0))
	{
		pms_o22_work->do_hotplug = 0;

		PM_DEBUG_PRINT("HOTPLUG DO cpu%u:%u\n",pms_o22_work->core_n,pms_o22_work->core_onoff);

		if (pms_o22_work->core_onoff == 0)
		{
			if (cpu_online(pms_o22_work->core_n))
			{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6,12,0))
				ret = remove_cpu(pms_o22_work->core_n);
#else
				ret = cpu_down(pms_o22_work->core_n);
#endif

				if(ret != 0)
				{
					if (ret != -EBUSY)
						o22_set_error_state(0xD << 28 | pms_o22_work->core_n << 24 | pms_o22_work->core_onoff << 16 | (ERROR_MASK & ret));
				}
				else
				{
					pCpudev = get_cpu_device(pms_o22_work->core_n);
					device_lock(pCpudev);
					pCpudev->offline = 1;
					device_unlock(pCpudev);

					if (o22_GetCPUOn() & (1 << pms_o22_work->core_n))
					{
		...
		else
		{
			...
			{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6,12,0))
				ret = add_cpu(pms_o22_work->core_n);
#else
				ret = cpu_up(pms_o22_work->core_n);
#endif
				if(ret != 0)
				{
					if (ret != -EBUSY)
...
static int o22_pm_task_avs(pms_o22_work_t *pms_o22_work)
{
	int ret = 0;
	int prevMV = 0;
	unsigned int type = 0;

	if (pms_o22_work->do_avs == 1) /* i2c */
	{
		pms_o22_work->do_avs = 0;
		o22_SetCoreVal(pms_o22_work->target_corevol,pms_o22_work->ch);
	}
	else if (pms_o22_work->do_avs == 2) /* gpio */
	{
		pms_o22_work->do_avs = 0;
		type = pms_o22_work->target_vol;
		...
		prevMV = regulator_get_voltage(o22_regulator) ;

		switch(type){
		case CPU_AVS_S:
			//HV
			ret = regulator_set_voltage(o22_regulator, O22_CPU_AVS_S_UV,O22_CPU_AVS_S_UV);
			break;
		case CPU_AVS_N:
			//MV
			ret = regulator_set_voltage(o22_regulator, O22_CPU_AVS_N_UV,O22_CPU_AVS_N_UV);
			break;
		case CPU_AVS_F:
			//LV
			ret = regulator_set_voltage(o22_regulator, O22_CPU_AVS_F_UV,O22_CPU_AVS_F_UV);
			break;
		case CPU_AVS_FF:
			//VLV
			ret = regulator_set_voltage(o22_regulator, O22_CPU_AVS_FF_UV,O22_CPU_AVS_FF_UV);
		...
		PM_NOTI("CPU ret[%d] mV[%d] => mV[%d]\n",ret,prevMV,regulator_get_voltage(o22_regulator) );
		regulator_put(o22_regulator);
	}
	return ret;
}

#if 0
static void o22_cpuregul_task_call(pms_o22_work_t o22work)
{
	o22_set_cpuregul_work(o22work);
	complete(&cpu_regulator_completion);
}
#endif
void o22_taskdisable_reset(void)
{
	g_o22_pm_task_disable = 0;
}

static void o22_lgdtv_pm_task_reset(void)
{
	reinit_completion(&cpu_hotplug_completion);
}

static void o22_lgdtv_pm_task_call(pms_o22_work_t o22work)
{
	o22_set_work(o22work);
	complete(&cpu_hotplug_completion);
}

static void o22_lgdtv_regul_task_reset(void)
{
	reinit_completion(&regulator_completion);
}

static void o22_lgdtv_regul_task_call(pms_o22_work_t o22work)
{
	o22_set_regul_work(o22work);
	complete(&regulator_completion);
}


static int o22_lgdtv_pm_task(void *pParam)
{
	unsigned long flags;

	pms_o22_work_t pms_o22_work;

	PM_NOTI("o22 lgdtv_pm_task is created\n");

	do {
		/* Check stop condition when device is closed. */
		if (kthread_should_stop()) {
			PM_NOTI("o22 lgdtv_pm_task - exit!\n");
			break;
		}
		...
		if (g_o22_pm_task_disable == 1)
			continue;

		if (o22_get_error_state()== 0)
		{
			spin_lock_irqsave(&g_o22_pm_lock, flags);
			memcpy((void *)&pms_o22_work, (void *)&g_o22_pms, sizeof(pms_o22_work_t));
			spin_unlock_irqrestore(&g_o22_pm_lock, flags);

			if(pm_o22_gov->get_freq_enable())
				o22_pm_task_dvfs(&pms_o22_work);
			if(pm_o22_gov->get_mp_enable())
				o22_pm_task_hotplug(&pms_o22_work);
...
#if 0
static int o22_cpu_regulator_task(void *pParam)
{
	unsigned long flags;

	pms_o22_work_t pms_o22_work;

	PM_NOTI("o22_cpuregul_task is created\n");

	do {
		/* Check stop condition when device is closed. */
		if (kthread_should_stop()) {
			PM_NOTI("o22_cpuregul_task - exit!\n");
			break;
		}
		...
		spin_lock_irqsave(&g_o22_cpuregul_lock, flags);
		memcpy((void *)&pms_o22_work, (void *)&g_o22_cpuregul, sizeof(pms_o22_work_t));
		spin_unlock_irqrestore(&g_o22_cpuregul_lock, flags);
		o22_pm_task_avs(&pms_o22_work);
...
static int o22_regulator_pm_task(void *pParam)
{
	unsigned long flags;

	pms_o22_work_t pms_o22_work;

	PM_NOTI("o22_regulator_pm_task is created\n");

	do {
		/* Check stop condition when device is closed. */
		if (kthread_should_stop()) {
			PM_NOTI("o22_regulator_pm_task - exit!\n");
			break;
		}
		...
		spin_lock_irqsave(&g_o22_regul_lock, flags);
		memcpy((void *)&pms_o22_work, (void *)&g_o22_regul, sizeof(pms_o22_work_t));
		spin_unlock_irqrestore(&g_o22_regul_lock, flags);
		o22_pm_task_avs(&pms_o22_work);
...
void o22_pmtask_init(void)
{
	...
	if (cpu_hotplug_pm_task == NULL) {
		...
		cpu_hotplug_pm_task = kthread_create_on_node(o22_lgdtv_pm_task,
					NULL,
					cpu,
	...
	if (regulator_pm_task == NULL) {
		...
		regulator_pm_task = kthread_create_on_node(o22_regulator_pm_task,
					NULL,
					cpu,
	...
	if (cpu_regulator_task == NULL) {
		...
		cpu_regulator_task = kthread_create_on_node(o22_cpu_regulator_task,
					NULL,
					cpu,
...
static void o22_AllCoreOn(void)
{
...
#define CPU_ON				1
	...
	for(i=1; i<4; i++)
	{
		if (cpu_online(i) == 0)
		{
			...
			pms_work.do_dvfs = 0;
			o22_lgdtv_pm_task_call(pms_work);
			msleep(100);		/* max cpu on time */
		}
	}
}

static void o22_SetMaxPerformance(void)
{
	reinit_completion(&cpu_hotplug_completion);
	reinit_completion(&regulator_completion);
...
void o22_FuncInit(void)
{

	/* o22 DCDC regulater vendor is fixed as TI
		detailed info: http://clm.lge.com/issue/browse/SCDCR-1798 */

	if(o22_clk0_base == NULL)
		o22_clk0_base = ioremap(0xC6033010,0x4);
	if(o22_clk1_base == NULL)
		o22_clk1_base = ioremap(0xC6033018,0x4);
	if(o22_clk_pdb_base == NULL)
		o22_clk_pdb_base = ioremap(0xC603306C,0x4);
	if(o22_clk_src_base == NULL)
		o22_clk_src_base = ioremap(0xCAA10018,0x4);
...
static pms_o22_func_t o22_pm_func =
{
	.init = o22_FuncInit,
	.getfreq = o22_GetCPUFreq,
	.setfreq = o22_SetCPUFreq,
	.setcorevolt = o22_SetCoreVal,
	.setcore_switching_freq = o22_SetCoreSwitchingFreq,
	.getcorevolt = o22_GetCoreVal,
	.get_cpu_on = o22_GetCPUOn,
	.get_cpu_type = o22_GetCPUType,
	.set_max_performance = o22_SetMaxPerformance,
	.run = o22_lgdtv_pm_task_call,
	.run_regul = o22_lgdtv_regul_task_call,
	//.run_cpuregul = o22_cpuregul_task_call,
	.reset = o22_lgdtv_pm_task_reset,
	.reset_regul = o22_lgdtv_regul_task_reset,
	.taskdisable_reset = o22_taskdisable_reset,
	.get_sampling_rate = o22_get_sampling_rate,
	.set_sampling_rate = o22_set_sampling_rate,
	.get_error_state = o22_get_error_state,
	.set_error_state = o22_set_error_state,
};
