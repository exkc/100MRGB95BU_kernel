...
static OS_SEM_T			irb_mutex;

#define KDRV_IRB_LOCK_INIT()		OS_InitMutex(&irb_mutex, OS_SEM_ATTR_DEFAULT)
#define KDRV_IRB_LOCK()                      \
    do{                                             \
        if(OS_LockMutex(&irb_mutex) < 0)             \
        {                                           \
            KDRV_IRB_ERROR("Mutex Lock Fail\n");   \
            return -1;                              \
        }                                           \
    }while(0)

#define KDRV_IRB_UNLOCK()                    \
    do{                                             \
        OS_UnlockMutex(&irb_mutex);                  \
    }while(0)
...
----------------------------------------------------------------------------------------*/
int m23_irb_ioctl_main (unsigned int cmd, unsigned long arg);
...
/*----------------------------------------------------------------------------------------
	global Functions
---------------------------------------------------------------------------------------*/
static int m23_irbs_enable = 1;
static int m23_irb_inited = 0;

module_param(m23_irbs_enable, int, S_IRUGO);
MODULE_PARM_DESC(m23_irbs_enable, "m23 irbs disable");


static irbs_m23_func_t* irb_m23_func = NULL;
...
int m23_irb_ioctl_main (unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		case IRB_IOW_COMMAND_SET:
		{
			...
			kbuf = mbuf;
			IRB_COPY_FROM_USER(kbuf, (void __user *)ubuf, param.numMakeTimes*4, return -EFAULT);
			param.maketime = kbuf;

			KDRV_IRB_LOCK();
			ret = irb_m23_func->tx(&param);
			KDRV_IRB_UNLOCK();
			if(mbuf != NULL)
				kfree(mbuf);
...
static int m23_irb_ioctl (unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	...
	ret = m23_irb_ioctl_main(cmd, arg);

    return ret;
}

static int m23_irb_suspend(void)
{

	return 0;
}

static int m23_irb_resume(void)
{
	irb_m23_func = get_m23_irb_func();
	irb_m23_func->reinit();
...
static irbs_driver_t m23_irbs_driver =
{
	.init = m23_irb_init,
	.ioctl = m23_irb_ioctl,
	.suspend = m23_irb_suspend,
	.resume = m23_irb_resume,
};

irbs_driver_t* get_m23_irb_driver(void)
{
	return &m23_irbs_driver;
}
