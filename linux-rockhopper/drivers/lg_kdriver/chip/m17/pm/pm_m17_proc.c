...
static OS_PROC_DESC_TABLE_T	_g_pm_m17_device_proc_table[] =
{
...
static OS_PROC_DESC_TABLE_T	_g_pm_m17_gov_proc_table[] =
{
...
static OS_PROC_DESC_TABLE_T	_g_pm_m17_debug_proc_table[] =
{
...
========================================================================================*/
#define M16PC0_AVS  2
extern m16p_sidd_t gSidd_check_table[5];
extern char * cpu_volttype_c0[5];
extern char * core_volttype_c0[5];


static void _PM_PrintStatus(struct seq_file *m, void* data)
{
	pms_m17_avsinto_t info;
	memset (&info,0x0, sizeof(pms_m17_avsinto_t));

	seq_printf( m, "governor: %s\n", pm_m17_gov->get_gov_str(pm_m17_gov->get_gov()) );
	seq_printf( m, "cpu_num: %d\n", pm_m17_gov->get_cpunum());
	seq_printf( m, "load: %u %u %u %u\n", pm_m17_gov->get_load(0),pm_m17_gov->get_load(1),pm_m17_gov->get_load(2),pm_m17_gov->get_load(3)) ;
	seq_printf( m, "loadmax: %u \n", pm_m17_gov->get_load(4)) ;
	seq_printf( m, "loadavg: %u \n", pm_m17_gov->get_load(5)) ;
	seq_printf( m, "freq: %u Mhz\n", pm_m17_gov->get_max_freq()) ;
	seq_printf( m, "===========================\n"	);
	seq_printf( m, "coreavs_type: %s \n", pm_m17_avs->get_core_avs_type_str()) ;
	pm_m17_avs->get_core_avs_type(&info);
	seq_printf( m, "coreavs_current(mA): %u\n", info.avscurrent);
	pm_m17_avs->get_cpu_avs_type(&info);
	seq_printf( m, "cpuavs_type: %s \n", pm_m17_avs->get_cpu_avs_type_str()) ;
	seq_printf( m, "cpuavs_current(mA): %u\n", info.avscurrent);

	seq_printf( m, "===========================\n"	);
	seq_printf( m, "|  m16pc0 core avs table  |\n");
	seq_printf( m, "===========================\n"	);
	seq_printf( m, "|     < %3d |  %sV      |\n", gSidd_check_table[M16PC0_AVS].core_sidd_n_min,core_volttype_c0[1]);
	seq_printf( m, "| %3d ~ %3d |  %sV      |\n", gSidd_check_table[M16PC0_AVS].core_sidd_n_min,gSidd_check_table[M16PC0_AVS].core_sidd_f_min,core_volttype_c0[2] );
	seq_printf( m, "| %3d ~ %3d |  %sV      |\n", gSidd_check_table[M16PC0_AVS].core_sidd_f_min,gSidd_check_table[M16PC0_AVS].core_sidd_ff_min,core_volttype_c0[3] );
	seq_printf( m, "|     > %3d |  %sV      |\n", gSidd_check_table[M16PC0_AVS].core_sidd_ff_min,core_volttype_c0[4]);
	seq_printf( m, "===========================\n"	);
	seq_printf( m, "|  m16pc0  cpu avs table  |\n");
	seq_printf( m, "===========================\n"	);
	seq_printf( m, "|     < %3d |  %sV      |\n", gSidd_check_table[M16PC0_AVS].cpu_sidd_n_min,cpu_volttype_c0[1]);
	seq_printf( m, "| %3d ~ %3d |  %sV      |\n", gSidd_check_table[M16PC0_AVS].cpu_sidd_n_min,gSidd_check_table[M16PC0_AVS].cpu_sidd_f_min,cpu_volttype_c0[2] );
	seq_printf( m, "|     > %3d |  %sV      |\n", gSidd_check_table[M16PC0_AVS].cpu_sidd_ff_min,cpu_volttype_c0[3]);
	seq_printf( m, "===========================\n"	);
	seq_printf( m," M16++ CPU TEMP[%d]'C     \n",info.temperature);
	seq_printf( m, "===========================\n"	);
}
 ...
 *
*/
static int	m17_read_debug_proc(UINT32 procId, char* buffer )
{
	int ret = 0 ;
...
static int	m17_read_gov_proc(UINT32 procId, char* buffer )
{
	...
	switch( procId ) {
		case GOV_ID_MAX_FREQ:
			ret = sprintf( buffer, "%u\n", pm_m17_gov->get_max_freq() * 1000);
			break;
		case GOV_ID_MIN_FREQ:
			ret = sprintf( buffer, "%u\n", pm_m17_gov->get_min_freq() * 1000);
			break;
		case GOV_ID_HARD_UP_THRESHOLD:
			ret = sprintf( buffer, "%u\n", pm_m17_gov->get_hard_up_threshold() );
			break;
		case GOV_ID_EASY_UP_THRESHOLD:
			ret = sprintf( buffer, "%u\n", pm_m17_gov->get_easy_up_threshold() );
			break;
		case GOV_ID_EASY_COUNT:
			ret = sprintf( buffer, "%u\n", pm_m17_gov->get_easy_count());
			break;
		case GOV_ID_FREQDOWN_COUNT:
			ret = sprintf( buffer, "%u\n", pm_m17_gov->get_freqdown_count());
			break;
		case GOV_ID_MPD_COUNT:
			ret = sprintf( buffer, "%u\n", pm_m17_gov->get_mpd_count());
			break;
		default:
			ret = -1;
			break;
...
static int	m17_read_proc(UINT32 procId, char* buffer )
{
	int	ret = 0 ;
	pms_m17_avsinto_t info;
	...
	switch( procId ) {
		...
		case PROC_ID_MP_ENABLE:
			ret = sprintf( buffer, "%u\n", pm_m17_gov->get_mp_enable());
			break;
		case PROC_ID_TS_ENABLE:
			ret = sprintf( buffer, "%u\n", pm_m17_gov->get_ts_enable());
			break;
		case PROC_ID_TS_DEBUG_ENABLE:
			ret = sprintf( buffer, "%u\n", pm_m17_avs->get_ts_debug_enable());
			break;
		case PROC_ID_TS_CPU_APARAM:
			ret = sprintf( buffer, "%u\n", pm_m17_avs->get_cpu_ts_aparam());
			break;
		case PROC_ID_TS_CPU_BPARAM:
			ret = sprintf( buffer, "%u\n", pm_m17_avs->get_cpu_ts_bparam());
			break;
		case PROC_ID_TS_GPU_APARAM:
			ret = sprintf( buffer, "%u\n", pm_m17_avs->get_gpu_ts_aparam());
			break;
		case PROC_ID_TS_GPU_BPARAM:
			ret = sprintf( buffer, "%u\n", pm_m17_avs->get_gpu_ts_bparam());
			break;
		case PROC_ID_TEMPERATURE:
			ret = sprintf( buffer, "%u\n", pm_m17_avs->get_temperature());
			break;
		case PROC_ID_GET_CURRENT_LOAD:
			ret = sprintf( buffer, "%u\n", pm_m17_gov->get_load(LOAD_MAX));
			break;
		case PROC_ID_SAMPLING_RATE:
			ret = sprintf( buffer, "%u ms\n", pm_m17_func->get_sampling_rate());
			break;
		case PROC_ID_FREQ:
			ret = sprintf( buffer, "%u\n",pm_m17_func->getfreq() * 1000);
			break;
		case PROC_ID_CPU_AVS:
			pm_m17_avs->get_cpu_avs_type(&info);
			ret = snprintf( buffer,48,"cpuavs_type:%u %s cpuavs_current(mA):%u\n",info.avstype,pm_m17_avs->get_cpu_avs_type_str(),info.avscurrent);
			break;
		case PROC_ID_CORE_AVS:
			pm_m17_avs->get_core_avs_type(&info);
			ret = snprintf( buffer,48,"coreavs_type:%u %s coreavs_current(mA):%u\n",info.avstype,pm_m17_avs->get_core_avs_type_str(),info.avscurrent);
			break;
		default:
			ret = -1;
			break;
...
static int m17_write_debug_proc( UINT32 procId, char* command )
{
	int ret = 0;
...
static int m17_write_gov_proc( UINT32 procId, char* command )
{
	...
	switch( procId )
	{
		case GOV_ID_MAX_FREQ:
			...
			if( input_value < pm_m17_gov->get_min_freq()) {
				printk("input frequency is lower then min frequency\n");
				break;
			}
			pm_m17_gov->set_max_freq(input_value / 1000);
			break;
		case GOV_ID_MIN_FREQ:
			input_n = sscanf(command,"%u\n", &input_value);
			if( input_value < pm_m17_gov->get_max_freq()) {
				printk("input frequency is higher then max frequency\n");
				break;
			}
			pm_m17_gov->set_min_freq(input_value / 1000);
			break;
		case GOV_ID_HARD_UP_THRESHOLD:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m17_gov->set_hard_up_threshold(input_value);
			break;
		case GOV_ID_EASY_UP_THRESHOLD:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m17_gov->set_easy_up_threshold(input_value);
			break;
		case GOV_ID_EASY_COUNT:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m17_gov->set_easy_count(input_value);
			break;
		case GOV_ID_FREQDOWN_COUNT:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m17_gov->set_freqdown_count(input_value);
			break;
		case GOV_ID_MPD_COUNT:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m17_gov->set_mpd_count(input_value);
			break;
		default:
			ret = -1;
			break;
...
static int m17_write_proc( UINT32 procId, char* command )
{
	unsigned int input_value;
	unsigned int input_n;
	unsigned int temp;
	...
	switch( procId )
	{
		case PROC_ID_GOVERNOR:
			...
			if (input_value <= 10)
			{
				pm_m17_test->set_testmode(0);

				pm_m17_gov->set_gov(input_value);
		...
		case PROC_ID_MP_ENABLE:
			...
			if (input_value != 0)
				input_value = 1;
			pm_m17_gov->set_mp_enable(input_value);
			break;
		case PROC_ID_TS_ENABLE:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value != 0)
				input_value = 1;
			pm_m17_gov->set_ts_enable(input_value);
			break;
		case PROC_ID_TS_DEBUG_ENABLE:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value != 0)
				input_value = 1;
			pm_m17_avs->set_ts_debug_enable(input_value);
			break;
		case PROC_ID_TS_CPU_APARAM:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m17_avs->set_cpu_ts_aparam(input_value);
			break;
		case PROC_ID_TS_CPU_BPARAM:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m17_avs->set_cpu_ts_bparam(input_value);
			break;
		case PROC_ID_TS_GPU_APARAM:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m17_avs->set_gpu_ts_aparam(input_value);
			break;
		case PROC_ID_TS_GPU_BPARAM:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m17_avs->set_gpu_ts_bparam(input_value);
			break;
		case PROC_ID_TO_SUSPEND:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value == 1)
			{
				temp = pm_m17_gov->get_gov() | PMS_GOV_TOSUSPEND_FLAG;
				pm_m17_gov->set_gov(temp);
				pm_m17_gov->set_startup_timer(0);
				pm_m17_func->set_max_performance();
			}
			break;
		case PROC_ID_RESET_LOADMON:
			pm_m17_gov->reset_load_monitor();
			break;
		case PROC_ID_SAMPLING_RATE:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m17_func->set_sampling_rate(input_value);
			break;
		case PROC_ID_FREQ:
			input_n = sscanf(command,"%u\n", &input_value);
			if (input_value > (pm_m17_func->getfreq() * 1000))
			{
				pms_work.target_freq = input_value/1000;
				pms_work.do_dvfs = 1;
				pm_m17_func->run(pms_work);
			} else if (input_value < (pm_m17_func->getfreq() * 1000)) {
				pms_work.target_freq = input_value/1000;
				pms_work.do_dvfs = 2;
				pm_m17_func->run(pms_work);
			}
		...
		case PROC_ID_CPU_AVS:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m17_avs->set_avs_force(GPIO_AVS_CPU, input_value);
			break;
		case PROC_ID_CORE_AVS:
			input_n = sscanf(command,"%u\n", &input_value);
			pm_m17_avs->set_avs_force(GPIO_AVS_CORE, input_value);
			break;
		default:
			ret = -1;
			break;
...
void m17_proc_init (void)
{
	...
	OS_PROC_CreateEntryEx ( PM_MODULE, _g_pm_m17_device_proc_table,
										m17_read_proc,
										m17_write_proc );
	OS_PROC_CreateEntryEx ( PM_GOV_MODULE, _g_pm_m17_gov_proc_table,
										m17_read_gov_proc,
										m17_write_gov_proc );
	OS_PROC_CreateEntryEx ( PM_DEBUG_MODULE, _g_pm_m17_debug_proc_table,
										m17_read_debug_proc,
										m17_write_debug_proc );
}
...
static pms_proc_t m17_proc_func =
{
	.init = m17_proc_init,
	.cleanup = m17_proc_cleanup,
};

pms_proc_t* get_m17_pm_proc(void)
{
	return &m17_proc_func;
}
