...
int DEMOD_L1_IPC_Send_Sync(LX_DEMOD_L1_Context * core, UINT32 typ, UINT32 seq, char *ptr, int len) {
	...
	if (ret > 0) {
		DEMOD_INFO("send typ 0x%04X, seq %d, timeout, ret %d\n", seq, typ, ret);
...
extern int DEMOD_E60_FW_Error_Handler (int errcode);
...
int DEMOD_L1_IPC_Handler (LX_DEMOD_L1_Context * core) {
	...
	DEMOD_L0_Lock_CommDevice(core->chip);
	if(NO_DEMOD_ERROR != DEMOD_L0_ReadCommandBytes_NoLock(core->chip, REG_ERROR_FLAG, 4, &error_flag)) {
		DEMOD_ERROR("failed read REG_ERROR_FLAG\n");
	}
	if(NO_DEMOD_ERROR != DEMOD_L0_ReadCommandBytes_NoLock(core->chip, REG_RESET_FLAG, 4, &reset_flag)) {
		...
		if (error_flag != 0 && prv_error_flag != error_flag) {
			switch(error_flag) {
				case CMD_TYPE_ERROR_TIMEOUT:
					...
					DEMOD_E60_FW_Error_Handler(CMD_TYPE_ERROR_TIMEOUT);
					break;
				case CMD_TYPE_ERROR_FWRESET:
					DEMOD_MCU_ERROR("fw watchdog occurred! need fw reset\n");
					DEMOD_E60_FW_Error_Handler(CMD_TYPE_ERROR_FWRESET);
					break;
				default:
					break;
			}
	...
	while (DEMOD_L0_CBUF_Used(&core->ipc->rx) > 0) {
		...
		if (CMD_MAJOR_TYPE(bufh.typ) == CMD_MAJOR_TYPE_API) {
			...
			case CMD_TYPE_ERROR_TIMEOUT:
				{
					UINT32 timeout_epc1 = 0;
					UINT32 timeout_depc = 0;
					UINT32 timeout_ps = 0;
					if (bufh.len == 12) {
						memcpy(&timeout_epc1, &msg[0], 4);
						memcpy(&timeout_depc, &msg[4], 4);
						memcpy(&timeout_ps, &msg[8], 4);
						DEMOD_MCU_ERROR("MCU timeout: epc1 0x%08x, depc 0x%08x, ps 0x%08x\n", timeout_epc1, timeout_depc, timeout_ps);
					} else {
						DEMOD_MCU_ERROR("MCU timeout: %s\n", (bufh.len > 0 ? DEMOD_BYTE_TO_HEXSTR(msg, bufh.len) : "null"));
					}
				}
				break;
