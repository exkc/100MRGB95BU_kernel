 ...
 */
static void restore_ea_info(ntfs_inode *nai, const EA_INFORMATION *old_ea_info)
{
	s64 written;

	if (!old_ea_info) {
		if (ntfs_attr_rm(nai))
			ntfs_error(nai->vol->sb,
				   "Could not delete the EA_INFORMATION, possible inconsistency in inode %lld\n",
				   (long long)nai->mft_no);
		return;
	}

	written = ntfs_inode_attr_pwrite(VFS_I(nai), 0, sizeof(EA_INFORMATION),
					 (char *)old_ea_info, false);
	if (written != sizeof(EA_INFORMATION))
		ntfs_error(nai->vol->sb,
				"Could not restore the EA_INFORMATION, possible inconsistency in inode %lld\n",
				(long long)nai->mft_no);
}

static int ntfs_write_ea(ntfs_inode *ni, int type, char *value, s64 ea_off,
		s64 ea_size)
{
	struct inode *ea_vi;
	int err = 0;
		...
		int name_len, s64 *ea_offset, s64 *ea_size)
{
	...
	if (ea_buf_size < sizeof(EA_ATTR))
		goto out;

	offset = 0;
	do {
		p_ea = (const EA_ATTR *)&ea_buf[offset];
		...
		offset += next;
		 ...
		 sizeof(EA_ATTR) < (ea_buf_size - offset));
		...
		void *buffer, size_t size)
{
	...
	p_ea_info = ntfs_attr_readall(ni, AT_EA_INFORMATION, NULL, 0,
			&ea_info_size);
	if (!p_ea_info || ea_info_size != sizeof(EA_INFORMATION))
		return -ENODATA;
...
static inline int ea_packed_size(const EA_ATTR *p_ea)
{
	/*
 ...
 *
 * Returns 0 if successful a negative value if an error occurred.
 */
...
create_ea_info:
		p_ea_info = ntfs_malloc_nofs(sizeof(EA_INFORMATION));
		if (!p_ea_info)
			return -ENOMEM;

		ea_info_qsize = 0;
		err = ntfs_attr_add(ni, AT_EA_INFORMATION, AT_UNNAMED, 0,
				(char *)p_ea_info, sizeof(EA_INFORMATION));
		if (err)
			goto out;
 ...
 *
 * Returns 0, or error if there is a problem
 */
int ntfs_remove_ntfs_ea(ntfs_inode *ni)
{
	EA_INFORMATION *old_ea_info;
	struct inode *nai_vi, *na_vi;
	s64 old_ea_size;
	int err;
	 ...
	 */
	nai_vi = ntfs_attr_iget(VFS_I(ni), AT_EA_INFORMATION, AT_UNNAMED, 0);
	if (IS_ERR(nai_vi))
		return PTR_ERR(nai_vi);

	na_vi = ntfs_attr_iget(VFS_I(ni), AT_EA, AT_UNNAMED, 0);
	if (IS_ERR(na_vi)) {
		/* EA_INFORMATION present, but no EA */
		err = ntfs_attr_rm(NTFS_I(nai_vi));
	...
	old_ea_info = ntfs_attr_readall(ni, AT_EA_INFORMATION,
			(ntfschar *)NULL, 0, &old_ea_size);
	err = ntfs_attr_rm(NTFS_I(na_vi));
	NInoSetFileNameDirty(ni);
	if (!err) {
		err = ntfs_attr_rm(NTFS_I(nai_vi));
		if (err && old_ea_info) {
			 ...
			 */
			restore_ea_info(NTFS_I(nai_vi), old_ea_info);
		}
	} else {
...
int ntfs_ea_set_wsl_inode(struct inode *inode, dev_t rdev, __le16 *ea_size)
{
	...
	err = ntfs_set_ea(inode, "$LXUID", sizeof("$LXUID") - 1, &v, sizeof(v),
			0, ea_size);
	if (err)
		return err;
	...
	err = ntfs_set_ea(inode, "$LXGID", sizeof("$LXGID") - 1, &v, sizeof(v),
			0, ea_size);
	if (err)
		return err;
	...
	err = ntfs_set_ea(inode, "$LXMOD", sizeof("$LXMOD") - 1, &v, sizeof(v),
			0, ea_size);
	if (err)
		return err;
...
ssize_t ntfs_listxattr(struct dentry *dentry, char *buffer, size_t size)
{
	...
	ea_info = ntfs_attr_readall(ni, AT_EA_INFORMATION, NULL, 0,
			&ea_info_size);
	if (!ea_info || ea_info_size != sizeof(EA_INFORMATION))
		goto out;
	...
	do {
		...
		offset += ea_size;
		 ...
		 sizeof(EA_ATTR) < (ea_info_qsize - offset));
