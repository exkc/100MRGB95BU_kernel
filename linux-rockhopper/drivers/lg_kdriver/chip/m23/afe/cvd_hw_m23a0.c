...
#include "pe_hal.h"
#include "../pe/reg/pe_ddr_reg_m23.h"
#endif

#include "cvd_module.h"
#include "cvd_hw_m23a0.h"
#include "de_cvd_reg.h"
#include "cvd_hal.h"

#include "cvd_pe_m23a0/CVD_L_AV_NTSC_443_default_m23.h"
#include "cvd_pe_m23a0/CVD_L_AV_NTSC_M_default_m23.h"
#include "cvd_pe_m23a0/CVD_L_AV_PAL60_default_m23.h"
#include "cvd_pe_m23a0/CVD_L_AV_PALI_default_m23.h"
#include "cvd_pe_m23a0/CVD_L_AV_PALM_default_m23.h"
#include "cvd_pe_m23a0/CVD_L_AV_PALNc_default_m23.h"
#include "cvd_pe_m23a0/CVD_L_AV_SECAM_default_m23.h"
#include "cvd_pe_m23a0/CVD_L_RF_NTSC_M_default_m23.h"
#include "cvd_pe_m23a0/CVD_L_RF_PALI_default_m23.h"
#include "cvd_pe_m23a0/CVD_L_RF_PALM_default_m23.h"
#include "cvd_pe_m23a0/CVD_L_RF_PALNc_default_m23.h"
#include "cvd_pe_m23a0/CVD_L_RF_SECAM_default_m23.h"

#include "cvd_pe_m23a0/CVD_L_RF_NTSC_M_Black_Level_High.h"
#include "cvd_pe_m23a0/CVD_L_RF_NTSC_M_Black_Level_Low.h"
#include "cvd_pe_m23a0/CVD_L_AV_NTSC_M_Black_Level_High.h"
#include "cvd_pe_m23a0/CVD_L_AV_NTSC_M_Black_Level_Low.h"
/*----------------------------------------------------------------------------------------
...
/*----------------------------------------------------------------------------------------
  global Variables
  ----------------------------------------------------------------------------------------*/
UINT32 g_M23_CVD_HW_DRV_REV = 0x23062200;

CVD_COLOR_SYSTEM_ARRAY_3CS_T g_SetColorSystem_3CS_M23A0 =
{ 
	{
...
int CVD_M23Ax_Clamp_Current_Control(UINT8 value)
{
		...
		else
		{
			...
			*/
			...
			if (lx_chip_rev() <= LX_CHIP_REV(M16, A0))
				value = 3;
#endif
			CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc07);
			CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc07, reg_cvbsafe_cvbs_iclp, value);
			CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc07);
...
// TRUE : Power Down
// FALSE : Power Up
void CVD_M23Ax_Power_Down(LX_AFE_CVD_SELECT_T select_main_sub, BOOLEAN PowerOnOFF)
{
...
//			CTOP_CTRL_O22Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc05, reg_cvbsafe_cvbs_pdbm, 0x1);
		}
		else
		{
			//CVD off
...
void CVD_M23Ax_Reg_Init(LX_AFE_CVD_SELECT_T select_main_sub)
{

#ifdef M23_BRINGUP
	//110915 : release f54m, cvbs reset first.
	// CVD_H13_BRINGUP : soft_reset_5 to soft_reset_2
	MIP_REG_M23Ax_RdFL(crg_bmc02);

	// CVD_H13_BRINGUP :from h/w script
	MIP_REG_M23Ax_Wr01(crg_bmc02, swrst_vdac, 0);

	MIP_REG_M23Ax_Wr01(crg_bmc02, swrst_f54m, 0);
	MIP_REG_M23Ax_Wr01(crg_bmc02, swrst_cvbs, 0);
	MIP_REG_M23Ax_Wr01(crg_bmc02, swrst_f27m, 0);
	MIP_REG_M23Ax_WrFL(crg_bmc02);

	MIP_REG_M23Ax_RdFL(crg_bmc17);
	MIP_REG_M23Ax_Wr01(crg_bmc17, sel_inv_f54m_clock, 1);
	MIP_REG_M23Ax_WrFL(crg_bmc17);
#endif

	// SICDTV-10202 : 210805
	CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc07);
	CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc07, reg_cvbsafe_cvbs_regsel, 3); // 2 to 3
	CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc07);

	// SICDTV-10202 : 210825
	CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc06);
	CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc06, reg_cvbsafe_cvbs_cmlfsel, 4); // 3 to 4
	CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc06);

	//M16+ register change
	CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc07);
	CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc07, reg_cvbsafe_cvbs_cksel, 1);
	CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc07);

#ifdef M23_BRINGUP
	MIP_REG_M23Ax_RdFL(crg_bmc17);
	MIP_REG_M23Ax_Wr01(crg_bmc17, sel_inv_vdac_clock, 1);
	MIP_REG_M23Ax_WrFL(crg_bmc17);
#endif
	...
	CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc07);
	CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc07, reg_cvbsafe_cvbs_lpf, 1);
	CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc07);
	CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc06);
	CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc06, reg_cvbsafe_cvbs_bw, 0);
	CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc06, reg_cvbsafe_cvbs_dcatt, 1);
	CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc06);
...
//	CVD_Hal_Init_Level_Detector();
	...
	/* M16 cvbs_iclp2 : 0x3 */
	CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc06);
	CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc06, reg_cvbsafe_cvbs_iclp2, 0x3);	//160120 : iclp2
	CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc06);
	...
	CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc06);
	CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc06, reg_cvbsafe_cvbs_dcsel, 0x1);  //211005 : o22 PRODTESTH-1125 0x1(0.9V) //190522 O20 : 12n 0x3(1.0V)  //160919 : DC reference level 0.8V -> 1.0V
	CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc06);
...
int CVD_M23Ax_Set_Source_Type(LX_AFE_CVD_SET_INPUT_T	cvd_input_info)
{
		...
		*/
		...
		switch(cvd_input_info.cvbs_input_port)
		{
			case LX_AFE_CVBS_IN1:
				CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc07);
				CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc07, reg_cvbsafe_cvbs_insel, 0x0);
				CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc07, reg_cvbsafe_cvbs_shield_on, 0x1);
				CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc07);
				break;
			case LX_AFE_CVBS_IN2:
				CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc07);
				CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc07, reg_cvbsafe_cvbs_insel, 0x1);
				CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc07, reg_cvbsafe_cvbs_shield_on, 0x2);
				CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc07);
				break;
		...
		if (cvd_input_info.cvd_input_source_attribute != LX_CVD_INPUT_SOURCE_ATTRIBUTE_RF)
		{
			...
			CVD_M23Ax_Set_Internal_Demod_Mode(0);
			CVD_Hal_agc_half_en_control(0);	// to prevent random VLOCK!
#ifdef M16_CVBSAFE_BUG_WOKAROUND
			if (lx_chip_rev() <= LX_CHIP_REV(M16, A0))
			{
				//1 : Select IN2(SCART path)
				ACE_REG_M23A0_RdFL(videoafe18);
				ACE_REG_M23A0_Wr01(videoafe18, reg_cvbsafe_cvbs_insel, 0x1);
				ACE_REG_M23A0_WrFL(videoafe18);
		...
		else
		{
			...
			if(g_CVD_Status_3CS.use_internal_demod)	// if using internal demod
				CVD_M23Ax_Set_Internal_Demod_Mode(1);
			else
				CVD_M23Ax_Set_Internal_Demod_Mode(0);
...
int CVD_M23Ax_Get_Scart_FB_En(void)
{
	int ret;
	CVD_Reg_RdFL(fastblank_001);
...
int CVD_M23Ax_Channel_Power_Control(UINT32 on_off)
{
	...
	if(on_off)	// power ON
	{
		...
		CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc05);
		CTOP_CTRL_M23Ax_Rd01(MIP_ADC_CTRL_BMC, mip_adc05, reg_cvbsafe_cvbs_pdbm, cvbs_pdbm_status);
		CTOP_CTRL_M23Ax_Rd01(MIP_ADC_CTRL_BMC, mip_adc05, reg_cvbsafe_cvbs_pdb, cvbs_pdb_status);
		//ACE_REG_M17A0_Rd01(main_pll_4, dr3p_pdb, dr3p_pdb_status);
		CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc06);
		CTOP_CTRL_M23Ax_Rd01(MIP_ADC_CTRL_BMC, mip_adc06, reg_cvbsafe_cvbs_clp, cvbs_cp_status);
		...
		CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc05, reg_cvbsafe_cvbs_pdbm, 0x1);
		CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc05, reg_cvbsafe_cvbs_pdb, 0x1);
		CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc05, reg_cvbsafe_cvbs_reg_pdb, 0x1);
		CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc05);
		//ACE_REG_M17A0_Wr01(main_pll_4, dr3p_pdb, 0x1);

		//added 110622 : by tommy.lee to disable clamp control on power down condition
		CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc06, reg_cvbsafe_cvbs_clp, 1);
		CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc06);
...
/*
		...
		*/
		...
		CTOP_CTRL_M23Ax_RdFL(SCRG_CTRL_CVI, crg_cvi03);
		CTOP_CTRL_M23Ax_Wr01(SCRG_CTRL_CVI, crg_cvi03, reg_soft_rst_cvd27_n, 0x1);
		CTOP_CTRL_M23Ax_WrFL(SCRG_CTRL_CVI, crg_cvi03);

		CTOP_CTRL_M23Ax_RdFL(SCRG_CTRL_CVI, crg_cvi03);
		CTOP_CTRL_M23Ax_Wr01(SCRG_CTRL_CVI, crg_cvi03, reg_soft_rst_cvd27_n, 0x0);
		CTOP_CTRL_M23Ax_WrFL(SCRG_CTRL_CVI, crg_cvi03);
	}
	else	// power off
	{
...
//		// M16+ B1 : Component Noise when CVBS PDB is On : 161013
		CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc05);
		CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc05, reg_cvbsafe_cvbs_pdb, 0);
		CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc05, reg_cvbsafe_cvbs_reg_pdb, 0x0);
		CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc05);

		//added 110622 : by tommy.lee to disable clamp control on power down condition
		CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc06);
		CTOP_CTRL_M23Ax_Wr01(MIP_ADC_CTRL_BMC, mip_adc06, reg_cvbsafe_cvbs_clp, 0);
		CTOP_CTRL_M23Ax_WrFL(MIP_ADC_CTRL_BMC, mip_adc06);
...
int CVD_M23Ax_Bypass_Control(LX_AFE_CVD_BYPASS_CONTROL_T *cvd_bypass_control_t)
{
...
#ifdef M23_BRINGUP_TMP
			ACE_REG_M23A0_RdFL(abb_02);
			ACE_REG_M23A0_Wr01(abb_02, abb_mux_sel_vdac, 0);		// 0:DAC
			ACE_REG_M23A0_WrFL(abb_02);
...
//			ACE_REG_M17A0_Wr01(afe_vbuf_1, buf_pdb1, 0x0);	//SCART PDB OFF
			...
			ACE_REG_M23A0_RdFL(abb_02);
			ACE_REG_M23A0_Wr01(abb_02, abb_mux_sel_vdac, 1);	
			ACE_REG_M23A0_Wr01(abb_02, abb_mux_rcsel_vdac, 0);
			ACE_REG_M23A0_WrFL(abb_02);
...
//			ACE_REG_M17A0_Wr01(afe_vbuf_1, buf_pdb1, 0x0);	//SCART PDB OFF
			...
			ACE_REG_M23A0_RdFL(abb_02);
			ACE_REG_M23A0_Wr01(abb_02, abb_mux_rcsel_vdac, 1);
			ACE_REG_M23A0_WrFL(abb_02);
#endif
			break;
...
int CVD_M23Ax_OnOff_VDAC(BOOLEAN bonoff)
{
	...
	{
		/*
		CVD_M23Ax_VDAC_mute_control(1);
		OS_MsecSleep(5);
		CVD_M23Ax_VDAC_mute_control(0);
		*/
		MIP_REG_M23Ax_RdFL(crg_bmc02);
		MIP_REG_M23Ax_Wr01(crg_bmc02, swrst_vdac, 0x1);
		MIP_REG_M23Ax_WrFL(crg_bmc02);

		OS_MsecSleep(5);

		MIP_REG_M23Ax_RdFL(crg_bmc02);
		MIP_REG_M23Ax_Wr01(crg_bmc02, swrst_vdac, 0x0);
		MIP_REG_M23Ax_WrFL(crg_bmc02);
	}
	else	// power off
	{
...
int CVD_M23Ax_VDAC_mute_control(int enable)
{
#ifdef M23_BRINGUP
...
int CVD_M23Ax_Get_PE0_Motion_Value(int *p_tnr_x_avg_t, int *p_tnr_x_avg_s, int *p_tpd_s_status)
{
...
int CVD_M23Ax_Get_AFE_CVBS_status(UINT8 *cvbs_icon_value, UINT8 *cvbs_pdb_value, UINT8 *cvbs_cp_value, UINT8 *cvbs_insel_value, UINT8 *cvbs_pdbm_value)
{
	CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc07);
	CTOP_CTRL_M23Ax_Rd01(MIP_ADC_CTRL_BMC, mip_adc07, reg_cvbsafe_cvbs_iclp, *cvbs_icon_value);
	CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc05);
	CTOP_CTRL_M23Ax_Rd01(MIP_ADC_CTRL_BMC, mip_adc05, reg_cvbsafe_cvbs_pdb, *cvbs_pdb_value);
	CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc06);
	CTOP_CTRL_M23Ax_Rd01(MIP_ADC_CTRL_BMC, mip_adc06, reg_cvbsafe_cvbs_clp, *cvbs_cp_value);
	CTOP_CTRL_M23Ax_RdFL(MIP_ADC_CTRL_BMC, mip_adc07);
	CTOP_CTRL_M23Ax_Rd01(MIP_ADC_CTRL_BMC, mip_adc07, reg_cvbsafe_cvbs_insel, *cvbs_insel_value);

	return 0;
}

int CVD_M23Ax_Set_Internal_Demod_Mode(BOOLEAN bInternal_Demod)
{
...
#ifdef M23_BRINGUP_TMP
		ACE_REG_M23A0_RdFL(abb_02);
		ACE_REG_M23A0_Wr01(abb_02, abb_mux_sel_cvbs, 0);		// 0:ABB
		ACE_REG_M23A0_WrFL(abb_02);
#endif
	}
	else
...
#ifdef M23_BRINGUP_TMP
		ACE_REG_M23A0_RdFL(abb_02);
		ACE_REG_M23A0_Wr01(abb_02, abb_mux_sel_cvbs, 1);		// 1:Normal
		ACE_REG_M23A0_WrFL(abb_02);
#endif
	}
...
int CVD_M23Ax_OnOff_DMD_DAC(BOOLEAN bonoff)
{
...
#ifdef INCLUDE_KDRV_PE
#define CVD_M23Ax_CHK_DFLT_SIZE(_x)		\
	if (PE_DDR_M23_CVD_DFLT_SIZE != (sizeof(_x)/sizeof(CVD_REG_PARAM_T)))\
	{AFE_ERROR("%s size is wrong\n", #_x); ret=RET_ERROR;break;}

int CVD_M23Ax_SetPqModeToDdr(volatile UINT32 *p_ddr_reg)
{
	int ret = RET_OK;
	UINT32 i;
	UINT32 base_dflt[PE_DDR_M23_CVD_DFLT_ITEM_NUM];
	do{
		if (!p_ddr_reg)	{ AFE_ERROR("p_ddr_reg is Null\n"); ret=RET_ERROR;break; }
		CVD_M23Ax_CHK_DFLT_SIZE(cvd_av_ntsc_m_l_default_m23);
		CVD_M23Ax_CHK_DFLT_SIZE(cvd_rf_ntsc_m_l_default_m23);
		CVD_M23Ax_CHK_DFLT_SIZE(cvd_av_pali_l_default_m23);
		CVD_M23Ax_CHK_DFLT_SIZE(cvd_rf_pali_l_default_m23);
		CVD_M23Ax_CHK_DFLT_SIZE(cvd_av_secam_l_default_m23);
		CVD_M23Ax_CHK_DFLT_SIZE(cvd_rf_secam_l_default_m23);
		CVD_M23Ax_CHK_DFLT_SIZE(cvd_av_ntsc_443_l_default_m23);
		CVD_M23Ax_CHK_DFLT_SIZE(cvd_av_pal60_l_default_m23);
		CVD_M23Ax_CHK_DFLT_SIZE(cvd_av_palm_l_default_m23);
		CVD_M23Ax_CHK_DFLT_SIZE(cvd_rf_palm_l_default_m23);
		CVD_M23Ax_CHK_DFLT_SIZE(cvd_av_palnc_l_default_m23);
		CVD_M23Ax_CHK_DFLT_SIZE(cvd_rf_palnc_l_default_m23);
		base_dflt[0] = 0;
		for (i=1; i<PE_DDR_M23_CVD_DFLT_ITEM_NUM; i++)
		{
			base_dflt[i] = base_dflt[i-1]+PE_DDR_M23_CVD_DFLT_SIZE;
		}
		for (i=0; i<PE_DDR_M23_CVD_DFLT_SIZE; i++)
		{
			p_ddr_reg[base_dflt[0]+i] = cvd_av_ntsc_m_l_default_m23[i].cvd_pe_value;
			p_ddr_reg[base_dflt[1]+i] = cvd_rf_ntsc_m_l_default_m23[i].cvd_pe_value;
			p_ddr_reg[base_dflt[2]+i] = cvd_av_pali_l_default_m23[i].cvd_pe_value;
			p_ddr_reg[base_dflt[3]+i] = cvd_rf_pali_l_default_m23[i].cvd_pe_value;
			p_ddr_reg[base_dflt[4]+i] = cvd_av_secam_l_default_m23[i].cvd_pe_value;
			p_ddr_reg[base_dflt[5]+i] = cvd_rf_secam_l_default_m23[i].cvd_pe_value;
			p_ddr_reg[base_dflt[6]+i] = cvd_av_ntsc_443_l_default_m23[i].cvd_pe_value;
			p_ddr_reg[base_dflt[7]+i] = cvd_av_pal60_l_default_m23[i].cvd_pe_value;
			p_ddr_reg[base_dflt[8]+i]  = cvd_av_palm_l_default_m23[i].cvd_pe_value;
			p_ddr_reg[base_dflt[9]+i]  = cvd_rf_palm_l_default_m23[i].cvd_pe_value;
			p_ddr_reg[base_dflt[10]+i] = cvd_av_palnc_l_default_m23[i].cvd_pe_value;
			p_ddr_reg[base_dflt[11]+i] = cvd_rf_palnc_l_default_m23[i].cvd_pe_value;
		}
	}while (0);
	return ret;
}
...
int CVD_M23Ax_Set_PQ_Mode(LX_AFE_CVD_PQ_MODE_T cvd_pq_mode)
{
	...
	switch(cvd_pq_mode)
	{
		case LX_CVD_PQ_NTSC_M:		// AV NTSC
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_ntsc_m_l_default_m23;
			size	= sizeof(cvd_av_ntsc_m_l_default_m23) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_NTSC_M_RF:	// RF NTSC
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_ntsc_m_l_default_m23;
			size	= sizeof(cvd_rf_ntsc_m_l_default_m23) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL:			// AV PAL
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_pali_l_default_m23;
			size	= sizeof(cvd_av_pali_l_default_m23) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL_RF:		// RF PAL
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_pali_l_default_m23;
			size	= sizeof(cvd_rf_pali_l_default_m23) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_SECAM:		// AV SECAM
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_secam_l_default_m23;
			size	= sizeof(cvd_av_secam_l_default_m23) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_SECAM_RF:	// RF SECAM
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_secam_l_default_m23;
			size	= sizeof(cvd_rf_secam_l_default_m23) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_NTSC_443:		// AV NTSC 443
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_ntsc_443_l_default_m23;
			size	= sizeof(cvd_av_ntsc_443_l_default_m23) / sizeof(CVD_REG_PARAM_T) ;
			break;
		...
		case LX_CVD_PQ_PAL_60:			// AV PAL 60
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_pal60_l_default_m23;
			size	= sizeof(cvd_av_pal60_l_default_m23) / sizeof(CVD_REG_PARAM_T) ;
			break;
		...
		case LX_CVD_PQ_PAL_M:		// AV PAL M
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_palm_l_default_m23;
			size	= sizeof(cvd_av_palm_l_default_m23) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL_M_RF:	// RF PAL M
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_palm_l_default_m23;
			size	= sizeof(cvd_rf_palm_l_default_m23) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL_NC:		// AV PAL NC
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_palnc_l_default_m23;
			size	= sizeof(cvd_av_palnc_l_default_m23) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL_NC_RF:	// RF PAL NC
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_palnc_l_default_m23;
			size	= sizeof(cvd_rf_palnc_l_default_m23) / sizeof(CVD_REG_PARAM_T) ;
			break;
		default:
...
int CVD_M23Ax_Black_Level_Control(LX_AFE_CVD_BLACK_LEVEL_T black_level, LX_AFE_CVD_SOURCE_ATTRIBUTE_T rf_av_mode)
{
...
int CVD_M23Ax_Set_Buffer_Base_Addr(phys_addr_t cvd_paddr)
{
	if(cvd_paddr == 0)
		return -1;

	AFE_PRINT("### CVD Buffer Memory Base [0x%llx]\n", (unsigned long long)cvd_paddr);
	CVD_Reg_RdFL(iris_mif_gmau_007);
