...
extern struct file *hib_resume_bdev_file;
...
static int swsusp_rawdev_check(void)
{
	...
	hib_resume_bdev = file_bdev(hib_resume_bdev_file);
...
int preset_snapshot_header(struct snapshot_header *header)
{
	...
	header->crc = ~0;
	header->pa_resume_func = __pa(swsusp_arch_resume);
...
int lgsnap_write_magic(void)
{
	...
	hib_submit_io(REQ_OP_READ | REQ_SYNC, 0, info, NULL);
	header = GET_SNAP_HEADER(info);
