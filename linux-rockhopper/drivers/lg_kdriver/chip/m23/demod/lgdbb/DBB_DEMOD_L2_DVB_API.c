...
//	frontend->standard = DEMOD_MODULATION_DVBT;
	...
	)
{
	...
	switch (standard)
	{
		...
		{
			frontend->demod->prop->dd_auto_mode.auto_detect = DEMOD_PROP_AUTO_MODE_DET_DVBT_DVBT2_ENABLE;

			retc = DEMOD_L1_Set_Property(frontend->demod, DEMOD_PROP_AUTO_MODE_CODE);
			if(NO_DEMOD_ERROR != retc)
			{
				DEMOD_ERROR("FAIL, DEMOD_L1_Set_Property(DEMOD_PROP_AUTO_MODE_CODE) 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
				return retc;
			}

			if (1700 == ter_bandwidth_KHz)
			{
				frontend->demod->prop->dd_mode.bw              = DEMOD_PROP_MODE_BW_1D7MHZ;
			}

			if (DEMOD_MODULATION_DVBT == standard)
			{
				frontend->demod->prop->dd_dvbt_hp_sel.hierhpsel  	= dvbt_hpSel;
				retc = DEMOD_L1_Set_Property(frontend->demod, DEMOD_PROP_DVBT_HIERARCHY_CODE);
				if(NO_DEMOD_ERROR != retc)
				{
					DEMOD_ERROR("FAIL, DEMOD_L1_Set_Property(DEMOD_PROP_DVBT_HIERARCHY_CODE) 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
					return retc;
				}

				frontend->demod->prop->dd_mode.modulation  = DEMOD_MODULATION_DVBT;

				frontend->demod->prop->dd_dvbt2_lock_mode.lock_mode= T2_lock_mode;
				retc = DEMOD_L1_Set_Property(frontend->demod, DEMOD_PROP_DVBT2_MODE_CODE);
				if(NO_DEMOD_ERROR != retc)
				{
					DEMOD_ERROR("FAIL, DEMOD_L1_Set_Property(DEMOD_PROP_DVBT2_MODE_CODE) 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
					return retc;
				}

				if(DEMOD_PROP_MODE_TUNE_NORMAL != frontend->demod->prop->dd_mode.tuneMode) // autoscan
				{
					DEMOD_NOTI("DVB-T/T2 auto scan \n");
					...
					frontend->demod->cmd->dvbt2_plp_select.plp_id_sel_mode = DEMOD_CMD_DVBT2_PLP_SELECT_PLP_ID_SEL_MODE_MANUAL;
					...
					if(NO_DEMOD_ERROR != retc)
					{
						DEMOD_ERROR("FAIL, DEMOD_L1_SET_DD_DVBT2_PLP_Select 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
						return retc;
					}
				...
				{
					frontend->demod->prop->dd_auto_mode.auto_detect = DEMOD_PROP_AUTO_MODE_DET_DVBT_ONLY_ENABLE;
					retc = DEMOD_L1_Set_Property(frontend->demod, DEMOD_PROP_AUTO_MODE_CODE);
					if(NO_DEMOD_ERROR != retc)
					{
						DEMOD_ERROR("FAIL, DEMOD_L1_Set_Property(DEMOD_PROP_AUTO_MODE_CODE) 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
			...
			if (DEMOD_MODULATION_DVBT2 == standard)
			{
				frontend->demod->prop->dd_dvbt_hp_sel.hierhpsel  	= dvbt_hpSel;
				retc = DEMOD_L1_Set_Property(frontend->demod, DEMOD_PROP_DVBT_HIERARCHY_CODE);
				if(NO_DEMOD_ERROR != retc)
				...
				DEMOD_INFO("T2_lock_mode %d\n",T2_lock_mode);

				frontend->demod->prop->dd_mode.modulation  = DEMOD_MODULATION_DVBT2;
...
//					DEMOD_NOTI("frontend->demod->prop->dd_auto_mode.auto_detect = DEMOD_PROP_AUTO_MODE_DET_DVBT_DVBT2_ENABLE\n");
				}
				else
				{
					frontend->demod->prop->dd_mode.modulation  = DEMOD_MODULATION_DVBT2;
					frontend->demod->prop->dd_auto_mode.auto_detect = DEMOD_PROP_AUTO_MODE_DET_DVBT2_ONLY_ENABLE;
					retc = DEMOD_L1_Set_Property(frontend->demod, DEMOD_PROP_AUTO_MODE_CODE);
					if(NO_DEMOD_ERROR != retc)
					{
						DEMOD_ERROR("FAIL, DEMOD_L1_Set_Property(DEMOD_PROP_AUTO_MODE_CODE) 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
			...
			DEMOD_NOTI("DVB-T/T2 : BW %d KHz\n", ter_bandwidth_KHz);
		}
		break;
	case DEMOD_MODULATION_DVBC :
		{
			if (DEMOD_PROP_MODE_TUNE_FIXED_DATA == frontend->demod->prop->dd_mode.tuneMode)
			{
...
//				tempSrdCalc = tempSrdCalc - 4294967296; // 2^32 = 4294967296
				...
				frontend->demod->cmd->dvbc_sym_rate.auto_en	= DEMOD_CMD_DVBC_SRD_AUTO_EN_DISABLE;

				frontend->demod->prop->dd_constel.constellation = dvb_c_constellation;
				frontend->demod->prop->dd_constel.constel_autodet = DEMOD_PROP_CONSTEL_AUTO_DET_DISABLE;
				retc = DEMOD_L1_Set_Property(frontend->demod, DEMOD_PROP_CONSTEL_CODE);
				if(NO_DEMOD_ERROR != retc)
				{
					DEMOD_ERROR("FAIL, DEMOD_L1_Set_Property(DEMOD_PROP_CONSTEL_CODE) 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
					return retc;
				}
			}
			else
			{
				...
				frontend->demod->cmd->dvbc_sym_rate.auto_en	= DEMOD_CMD_DVBC_SRD_AUTO_EN_ENABLE;

				frontend->demod->prop->dd_constel.constellation = dvb_c_constellation;
				frontend->demod->prop->dd_constel.constel_autodet = DEMOD_PROP_CONSTEL_AUTO_DET_ENABLE;
				retc = DEMOD_L1_Set_Property(frontend->demod, DEMOD_PROP_CONSTEL_CODE);
				if(NO_DEMOD_ERROR != retc)
				{
					DEMOD_ERROR("FAIL, DEMOD_L1_Set_Property(DEMOD_PROP_CONSTEL_CODE) 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
			...
			DEMOD_NOTI("DVB-C : sr %d kbps, constel %d\n", symbol_rate_Kbps, dvb_c_constellation);
		}
		break;

	case DEMOD_MODULATION_DVBC2 :
		{
			DEMOD_NOTI("DVB-C2 : Not Supported\n");
		}
		break;

	default : /* ATV */
		{
			DEMOD_NOTI("'%d' standard (%s) is not managed by DEMOD_lock_to_carrier\n", standard, DEMOD_L1_API_Modulation_Text(standard));
			return 0;
		  }
		break;
	}
	DEMOD_NOTI("Setting BW is %d MHz\n", frontend->demod->prop->dd_mode.bw);
	...
	if (DEMOD_MODULATION_DVBC == standard)
	{
		retc = DEMOD_L1_SET_DD_SymbolRate_Detect(frontend->demod, frontend->demod->cmd->dvbc_sym_rate.auto_en, symbol_rate_Kbps);
		if(NO_DEMOD_ERROR != retc)
		{
			DEMOD_ERROR("FAIL, DEMOD_L1_SET_DD_SymbolRate_Detect() 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
			return retc;
		}
		DEMOD_INFO("tune post took %3d ms\n", DEMOD_L0_system_time() - checkTime);
	}
...
int DEMOD_L2_DVB_Get_PLP_ID   (LX_DEMOD_L2_Context *frontend, LX_DEMOD_L2_MULTI_PLP_ID_T *pPlpParams)
{
	...
	for (index = 0; index <	pPlpParams->plpTotalCount; index++)
	{
		...
		{
			DEMOD_ERROR("FAIL, DEMOD_L1_GET_DD_DVBT2_PLP_ID() 0x%02x: %s\n", retc, DEMOD_L1_API_Error_Text(retc) );
			return retc;
		}
	}

	DEMOD_NOTI("Valid num_plp = %d\n",PLPCount);
	DEMOD_NOTI("plpCount_Including_channInfo = %d(num_dplp = %d, num_cplp = %d)\n", pPlpParams->plpCount_Including_channInfo, num_dplp, num_cplp );
...
int DEMOD_L2_DVBT2_Set_BaseAddr_AXI(LX_DEMOD_L2_Context *frontend, UINT32 baseAddr1, UINT32 baseAddr2 )
{
	...
	// baseAddr1 : TDI
	// baseAddr2 : DJB
	retc = DEMOD_L1_SET_DD_DVBT2_BASEADDR_AXI(frontend->demod, baseAddr1, baseAddr2);

	if (retc != NO_DEMOD_ERROR)
	{
		DEMOD_ERROR("FAIL, DEMOD_L1_SET_DD_DVBT2_BASEADDR_AXI() BecauseOf %s\n", DEMOD_L1_API_Error_Text(retc));
...
int DEMOD_L2_DVB_Set_BaseAddr_AXI(LX_DEMOD_L2_Context *frontend, UINT32 baseAddr1, UINT32 baseAddr2, UINT32 baseAddr3 )
{
	...
	retc = DEMOD_L1_SET_DD_DVB_BASEADDR_AXI(frontend->demod, baseAddr1, baseAddr2, baseAddr3);

	if (retc != NO_DEMOD_ERROR)
	{
		DEMOD_ERROR("FAIL, DEMOD_L1_SET_DD_DVB_BASEADDR_AXI() BecauseOf %s\n", DEMOD_L1_API_Error_Text(retc));
		return -ERROR_DEMOD_WRITE;
	}
