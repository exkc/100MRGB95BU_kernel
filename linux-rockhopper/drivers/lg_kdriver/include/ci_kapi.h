...
#define	CI_IOR_READ_DATA				_IOWR(CI_IOC_MAGIC, 4, LX_CI_IOCTL_PARAM_T )
...
#define	CI_IOR_CAM_INIT					_IO(CI_IOC_MAGIC, 13)
#define	CI_IOR_PRINT_REG				_IOR(CI_IOC_MAGIC, 14, UINT32)
#define	CI_IOR_CAM_POWEROFF				_IO(CI_IOC_MAGIC, 15)
#define	CI_IOR_CAM_POWERONCOMPLETED		_IO(CI_IOC_MAGIC, 16)
#define	CI_IOR_GET_CIPLUS_VERSION		_IOR(CI_IOC_MAGIC, 17, LX_CI_IOCTL_PARAM_VERSION)
...
// Debug
#define CI_IOW_ENABLE_LOG				_IOW(CI_IOC_MAGIC, 25, UINT32)
...
#define CI_IOWR_GET_CIS_INFO			_IOWR(CI_IOC_MAGIC, 29, LX_CI_IOCTL_GET_CIS_INFO_T)

#define CI_IOC_MAXNR				30
/** @} */
...
typedef struct
{
	UINT8	tuplecode;	// tuplecode for CIS read
	UINT8	len;		// CIS length (VERS_1)
	UINT32	maxlen;
	union
	{
		UINT8*	buf;		// CIS str information
		UINT64	__compat_sizer_buf;
	};
}
LX_CI_IOCTL_GET_CIS_INFO_T;
