...
/*----------------------------------------------------------------------------------------
	global Variables
----------------------------------------------------------------------------------------*/
//static ucom_o24_gov_t* ucom_o24_gov = NULL;
...
static ucom_o24_func_t * ucom_func;
...
static OS_PROC_DESC_TABLE_T	_g_ucom_o24_device_proc_table[] =
{
...
int	o24_read_proc(UINT32 procId, char* buffer );
int o24_write_proc( UINT32 procId, char* command );
void o24_ucom_proc_init (void);
void o24_ucom_proc_cleanup (void);
...
int o24_write_proc( UINT32 procId, char* command )
{
	...
	switch( procId ) {
		...
		case PROC_ID_UCOM_SPI_W:
			...
			if(parse)
			{
				ret = o24_ucom_spi_write((UINT8 *)addr,(UINT8 *)value,count);
				if(ret)
		...
		case PROC_ID_UCOM_SPI_R:
		{
			...
			if(parse)
			{
				ret = o24_ucom_spi_read((UINT8 *)addr,(UINT8 *)value,count);
				if(!ret)
				{
					for( i = 0 ; i < count ;i++)
					{
						printk("%d Raddr[%8x] value[%8x]\n",parse, addr[i],value[i]);
					}
				}
				else
...
void o24_ucom_proc_init (void)
{
	ucom_func	= get_o24_ucom_func();
	ucom_drv =  get_o24_ucom_driver();


	OS_PROC_CreateEntryEx ( UCOM_MODULE, _g_ucom_o24_device_proc_table,
										o24_read_proc,
										o24_write_proc );
...
static ucom_proc_t o24_proc_func =
{
	.init = o24_ucom_proc_init,
	.cleanup = o24_ucom_proc_cleanup,
};

ucom_proc_t* get_o24_ucom_proc(void)
{
