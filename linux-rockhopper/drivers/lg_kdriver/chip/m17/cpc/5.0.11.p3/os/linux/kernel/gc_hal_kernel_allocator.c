    ...
    )
{
    ...
    {
#ifdef CONFIG_LG_EXT
        // see http://hlm.lge.com/issue/browse/TVPLAT-121741?focusedCommentId=4430006&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-4430006
        // chanho.min (2021/04/16)
        // e60은 o20과는 다르게 33bit 영역에서 device가 alloc 할수 있는데 DMA 로
        // 사용하는 메모리는 이를 방지 하기위해 GFP_DMA32 를 gfp flag를 주어야 합니다.
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
        p = alloc_page(GFP_KERNEL | GFP_DMA32 | /*__GFP_HIGHMEM |*/ gcdNOWARN);
#else
        p = alloc_page(GFP_KERNEL | GFP_DMA | /*__GFP_HIGHMEM |*/ gcdNOWARN);
#endif
#else
    ...
    )
{
    ...
    if (contiguous)
    {
        ...
        if (Mdl->u.contiguousPages == gcvNULL)
        {
            Mdl->u.contiguousPages =
#ifdef CONFIG_LG_EXT
                // http://hlm.lge.com/issue/browse/TVPLAT-121741?focusedCommentId=4430006&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-4430006
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0)
                alloc_pages(GFP_KERNEL | GFP_DMA32 | /*__GFP_HIGHMEM |*/ gcdNOWARN, order);
#else
                alloc_pages(GFP_KERNEL | GFP_DMA | /*__GFP_HIGHMEM |*/ gcdNOWARN, order);
#endif
#else
