...
struct de_test_util_o24_reg {
    ...
    struct {
     ...
     } hdr_pxl_rd_data;//REG_HDR10_E_HDR_PXL_RD_DATA_T
...
struct de_test_util_o24_pat_cvi {
    char *name;
...
----------------------------------------------------------------------------------------*/
static int _de_test_util_o24_map(void);
static int _de_test_util_o24_unmap(void);

/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
static struct de_test_util_o24_reg _g_de_test_util_o24_reg = {.is_map=0};

static struct de_test_util_o24_pat_cvi _g_de_test_util_o24_pat_cvi[] = {
...
========================================================================================*/
int de_test_util_o24_input_open(char *src_name,char *src_size,char *out_size)
{
    ...
    {
        /** 20250521, avoid dirty lut value on dce,dtm,dse, when linux6 clang kasan
        * (http://jira.lge.com/issue/browse/SDATMS-621) */
        extern int v4l2_vpq_cmn_set_bypass_block(int mask);
        v4l2_vpq_cmn_set_bypass_block(0x4);
        msleep(100);
    }
    #endif
...
int de_test_util_o24_input_close(void)
{
    struct v4l2_ext_vsc_connect_info input;
...
int de_test_util_o24_cvi_pat_on(char *name)
{
    struct de_test_util_o24_reg *p_reg = &_g_de_test_util_o24_reg;
    struct de_test_util_o24_pat_cvi *p_cvi = _g_de_test_util_o24_pat_cvi;
...
int de_test_util_o24_cvi_pat_off(void)
{
    struct de_test_util_o24_reg *p_reg = &_g_de_test_util_o24_reg;
    ...
    _de_test_util_o24_unmap();

    return 0;
}

int de_test_util_o24_check_cvi_i_pixel(char *name, UINT32 pos_x, UINT32 pos_y)
{
    int ret = 0;
    int i;
    ...
    while (p_cvi->name)
    {
        if (!strncasecmp(name,p_cvi->name,strlen(p_cvi->name)))
        {
            for (i=0; i<5; i++)
            {
                ...
                p_reg->hdr_pxl_rd_data.udata32 = *((UINT32*)p_reg->hdr_pxl_rd_data.phy);

                ret = (p_reg->hdr_pxl_rd_data.pxl_read_rcr >= p_cvi->min_r && p_reg->hdr_pxl_rd_data.pxl_read_rcr <= p_cvi->max_r && \
                    p_reg->hdr_pxl_rd_data.pxl_read_gyy >= p_cvi->min_g && p_reg->hdr_pxl_rd_data.pxl_read_gyy <= p_cvi->max_g && \
                    p_reg->hdr_pxl_rd_data.pxl_read_bcb >= p_cvi->min_b && p_reg->hdr_pxl_rd_data.pxl_read_bcb <= p_cvi->max_b)? (0):(-1);

                DE_NOTI("%-16s : r,g,b(10b) : (%4d ~ %4d) (%4d ~ %4d) (%4d ~ %4d)\n",name,p_cvi->min_r,p_cvi->max_r,p_cvi->min_g,p_cvi->max_g,p_cvi->min_b,p_cvi->max_b);
                DE_NOTI("%-16s : r,g,b(10b) : %4d, %4d, %4d -> %s (retry:%d)\n",name,p_reg->hdr_pxl_rd_data.pxl_read_rcr,p_reg->hdr_pxl_rd_data.pxl_read_gyy,p_reg->hdr_pxl_rd_data.pxl_read_bcb,(ret==0)? "ok":"ng",i);
            ...
            if (ret < 0)
            {
                DE_ERROR("%-16s : r,g,b(10b) : %4d, %4d, %4d -> %s (retry:%d)\n",name,p_reg->hdr_pxl_rd_data.pxl_read_rcr,p_reg->hdr_pxl_rd_data.pxl_read_gyy,p_reg->hdr_pxl_rd_data.pxl_read_bcb,"ng",i);
            }
            break;
        }
        p_cvi++;
...
int de_test_util_o24_check_cvi_o_pixel(char *name, UINT32 pos_x, UINT32 pos_y)
{
    int ret = 0;
    int i;
    struct de_test_util_o24_pat_cvi *p_cvi = _g_de_test_util_o24_pat_cvi;
...
static int _de_test_util_o24_map(void)
{
    ...
    p_reg->misc_ctrl.addr        = 0xCCCC0244;//REG_CVI_A_MISC_CTRL_T
    p_reg->hdr_pxl_rd_ctrl0.addr = 0xCCC10880;//REG_HDR10_E_HDR_PXL_RD_CTRL0_T
    p_reg->hdr_pxl_rd_ctrl1.addr = 0xCCC10884;//REG_HDR10_E_HDR_PXL_RD_CTRL1_T
    p_reg->hdr_pxl_rd_data.addr  = 0xCCC10888;//REG_HDR10_E_HDR_PXL_RD_DATA_T
    p_reg->ad_rp_hv_pos.addr     = 0xCC8E0540;//REG_LED_AD_RP_HV_POS_T
    p_reg->ad_rp_r_value.addr    = 0xCC8E0548;//REG_LED_AD_RP_R_VALUE_T
    p_reg->ad_rp_g_value.addr    = 0xCC8E054C;//REG_LED_AD_RP_G_VALUE_T
    p_reg->ad_rp_b_value.addr    = 0xCC8E0550;//REG_LED_AD_RP_B_VALUE_T
    ...
    p_reg->hdr_pxl_rd_data.phy = (volatile unsigned int *)ioremap(p_reg->hdr_pxl_rd_data.addr,sizeof(unsigned int));
    if (!p_reg->hdr_pxl_rd_data.phy)    {DE_ERROR("ioremap err (hdr_pxl_rd_data)\n");return -1;}
...
static int _de_test_util_o24_unmap(void)
{
    ...
    if (p_reg->hdr_pxl_rd_data.phy) iounmap(p_reg->hdr_pxl_rd_data.phy);
