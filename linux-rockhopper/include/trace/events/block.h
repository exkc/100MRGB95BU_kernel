...
TRACE_EVENT(block_dump_msg,

	TP_PROTO(char *rw, unsigned long long bi_sector,
		 char *bdevname, unsigned long count),

	TP_ARGS(rw, bi_sector, bdevname, count),

	TP_STRUCT__entry(
		__field( char*,			rw				)
		__field( unsigned long long,	bi_sector			)
	...
	TP_fast_assign(
		__entry->rw		= rw;
		__entry->bi_sector	= bi_sector;
		memcpy(__entry->bdevname, bdevname, BDEVNAME_SIZE);
		__entry->count		= count;
	),

	TP_printk("%s block %Lu on %s (%lu sectors)",
		__entry->rw, __entry->bi_sector, __entry->bdevname, __entry->count)
);
