...
**
**  Functions
**  Implemented:   int	DEMOD_O26_ResetHW
**                  int DEMOD_O26_SetI2C
**                  int DEMOD_O26_SetInclk
**                  int DEMOD_O26_AdcInit
**                  int DEMOD_O26_RepeaterEnable
**                  int DEMOD_O26_SoftwareReset
**                  int DEMOD_O26_Power_Save
**                  int DEMOD_O26_StdOperModeContrl
**                  int DEMOD_O26_NeverlockScan
**                  int DEMOD_O26_TPOutEnable
**                  int DEMOD_O26_Set_IF_Frq
**                  int DEMOD_O26_Set_AGCPolarity
**                  int DEMOD_O26_Get_Id
**                  int DEMOD_O26_Get_SpectrumStatus
**                  int DEMOD_O26_Get_IFAGC
**                  int DEMOD_O26_Get_OperMode
**                  int DEMOD_O26_Get_NeverLockStatus
**                  int DEMOD_O26_Get_CarrierFreqOffset
**                  int DEMOD_O26_Get_VABER
**                  int DEMOD_O26_Get_Packet_Error
**                  int DEMOD_O26_Get_BandWidthMode
**                  int DEMOD_O26_Get_QAMMode
**                  int DEMOD_O26_Get_MseInfo
**                  int DEMOD_O26_Get_Lock
**
**  References:
**
**  Exports:
...
static int _demod_pqe_pm_callback (int action);
...
//chungiii
	...
	// demod ip hold reset
	CTOP_CTRL_O26Ax_RdFL(IP_CTRL_DBB, ipw_dbb_004);
	CTOP_CTRL_O26Ax_Wr01(IP_CTRL_DBB, ipw_dbb_004, reg_dbb_reset, 1);
	CTOP_CTRL_O26Ax_WrFL(IP_CTRL_DBB, ipw_dbb_004);
	usleep_range(1000,10000);	// 1ms ~ 10ms

	CTOP_CTRL_O26Ax_RdFL(IP_CTRL_DBB, ipw_dbb_004);
	CTOP_CTRL_O26Ax_Wr01(IP_CTRL_DBB, ipw_dbb_004, reg_dbb_reset, 0);
	CTOP_CTRL_O26Ax_WrFL(IP_CTRL_DBB, ipw_dbb_004);
	usleep_range(1000,10000);	// 1ms ~ 10ms

	// for low power mode
	CTOP_CTRL_O26Ax_RdFL(MIP_CTRL_ADMD_MIP, mip_adc03);
	CTOP_CTRL_O26Ax_Wr02(MIP_CTRL_ADMD_MIP, mip_adc03, reg_admd_dpll_pdb,1,reg_admd_apll_pdb,1);
	CTOP_CTRL_O26Ax_WrFL(MIP_CTRL_ADMD_MIP, mip_adc03);
	usleep_range(1000,10000);	// 1ms ~ 10ms

	CTOP_CTRL_O26Ax_RdFL(MIP_CTRL_ADMD_MIP, mip_adc02);
	CTOP_CTRL_O26Ax_Wr01(MIP_CTRL_ADMD_MIP, mip_adc02, reg_admd_dpll_insel, 1);
	CTOP_CTRL_O26Ax_WrFL(MIP_CTRL_ADMD_MIP, mip_adc02);
	usleep_range(1000,10000);	// 1ms ~ 10ms

	CTOP_CTRL_O26Ax_RdFL(IP_CTRL_DBB, ipw_dbb_003);
	CTOP_CTRL_O26Ax_Wr01(IP_CTRL_DBB, ipw_dbb_003, pll_cgen, 1);
	CTOP_CTRL_O26Ax_WrFL(IP_CTRL_DBB, ipw_dbb_003);
	...
	{
		//volatile UINT32 * ctop_reg = (volatile UINT32 *)ioremap(0xC6881010, sizeof(UINT32)); // for O24
		volatile UINT32 * ctop_reg = (volatile UINT32 *)ioremap(0xC8A1100C, sizeof(UINT32)); //chungiii for O26 FPGA 
		if (NULL == ctop_reg) {
			DEMOD_ERROR("ioremap for ctop_reg pll cgen failed\n");
			return -1;
		}
		DEMOD_NOTI("ctop_reg pll cgen org 0x%08x\n", *ctop_reg);
		*ctop_reg = 0x1;
		DEMOD_NOTI("ctop_reg pll cgen chk 0x%08x\n", *ctop_reg);
		iounmap((void *)ctop_reg);
		ctop_reg = NULL;
		DEMOD_NOTI("WA: ctrl ctop reg topll cgen activation\n");


		volatile UINT32 * ctop_regg = (volatile UINT32 *)ioremap(0xC8A11010, sizeof(UINT32)); //chungiii for O26 FPGA 
		if (NULL == ctop_regg) {
			DEMOD_ERROR("ioremap for ctop_regg dbb reset failed\n");
			return -1;
		}
		DEMOD_NOTI("ctop_regg dbb reset org 0x%08x\n", *ctop_regg);
		*ctop_regg = 0x1;
		DEMOD_NOTI("ctop_regg dbb reset chk 0x%08x\n", *ctop_regg);
		iounmap((void *)ctop_regg);
		ctop_regg = NULL;
		DEMOD_NOTI("WA: ctrl ctop regg to dbb reset release \n");


	}

/*	
	DEMOD_NOTI("set mem slot @te as 0x%08x\n", gMemCfgDEMOD_FW.memory_base);
	{
		volatile UINT32 * te_reg = (volatile UINT32 *)ioremap(0xCEFE00C4, sizeof(UINT32));
		if (NULL == te_reg) {
			DEMOD_ERROR("ioremap for te_reg failed\n");
			return -1;
		}
		DEMOD_NOTI("te_reg org 0x%08x\n", *te_reg);
		*te_reg = gMemCfgDEMOD_FW.memory_base;
		iounmap((void *)te_reg);
		te_reg = NULL;
		DEMOD_NOTI("WA: ctrl te reg to using te 0xB0000000\n");
	}
*/	
#endif
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_AdcInit (void)
{
		
	DEMOD_PRINT("Start\n");
	#if 1//chungiii 
	CTOP_CTRL_O26Ax_RdFL(MIP_CTRL_ADMD_MIP, mip_adc03);
	CTOP_CTRL_O26Ax_Wr01(MIP_CTRL_ADMD_MIP, mip_adc03, reg_admd_dpll_pdb,1);
	CTOP_CTRL_O26Ax_WrFL(MIP_CTRL_ADMD_MIP, mip_adc03);
	#endif
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_RepeaterEnable (BOOLEAN bEnable)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_SoftwareReset (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int DEMOD_O26_APB_Access (BOOLEAN bEnable)
{
	LX_DEMOD_L2_Context *	pFrontend;
	UINT64	start_clock;
	UINT64	spend_clock;
...
int DEMOD_O26_Power_Save (BOOLEAN bEnable)
{
	LX_DEMOD_L2_Context *	pFrontend;
	pFrontend = DEMOD_Get_LGDBB_Context();

	bEnable = bEnable & 0x1;
	//DEMOD_INFO("%s power_save, pqepm id %d, statue %d\n", (bEnable ? "enter" : "leave"), _pqe_pm_id, _pqe_pm_state); //chungiii
	...
	DEMOD_PRINT("DEMOD_O26_Power_Save End\n");
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_StdOperModeContrl (LX_DEMOD_OPER_MODE_T operMode, LX_DEMOD_BWMODE_T bandWd, LX_DEMOD_SYMRDET_MODE_T symbr)
{
	LX_DEMOD_L2_Context *	pFrontend;
	...
	UINT8	reg_pad_diseqc_mux_sel = 0; //0: diseqco, 1: gp7_in[3] (gp7_out[3])
	//UINT8	reg_pad_diseqc_cmd_mux_sel = 0; //0: diseqco, 1: gp7_in[3] (gp7_out[3])
	...
	switch(operMode)
	{
		...
		case LX_DEMOD_DVBS2:
			...
			//reg_admd_adc_icon = 0x6;  //add 250530 
	...
	if (register_set_flag)
	{
		...
		reg_admd_adc_icon = 0x4;  //add 250530 
		
		CTOP_CTRL_O26Ax_RdFL(MIP_CTRL_ADMD_MIP, mip_adc00);
		CTOP_CTRL_O26Ax_Wr01(MIP_CTRL_ADMD_MIP, mip_adc00, reg_admd_i_pdb,reg_admd_i_pdb);
		CTOP_CTRL_O26Ax_WrFL(MIP_CTRL_ADMD_MIP, mip_adc00);

		CTOP_CTRL_O26Ax_RdFL(MIP_CTRL_ADMD_MIP, mip_adc01);
		CTOP_CTRL_O26Ax_Wr02(MIP_CTRL_ADMD_MIP, mip_adc01, reg_admd_dpll_odpre, reg_admd_dpll_odpre, reg_admd_adc_icon, reg_admd_adc_icon);
		CTOP_CTRL_O26Ax_Wr02(MIP_CTRL_ADMD_MIP, mip_adc01, reg_admd_q_pdb, reg_admd_q_pdb, reg_admd_t_pdb, reg_admd_t_pdb);
		CTOP_CTRL_O26Ax_WrFL(MIP_CTRL_ADMD_MIP, mip_adc01);

		CTOP_CTRL_O26Ax_RdFL(MIP_CTRL_ADMD_MIP, mip_adc02);
		CTOP_CTRL_O26Ax_Wr01(MIP_CTRL_ADMD_MIP, mip_adc02, reg_admd_dpll_cken, reg_admd_dpll_cken);
		CTOP_CTRL_O26Ax_Wr01(MIP_CTRL_ADMD_MIP, mip_adc02, reg_admd_bias_pdb, reg_admd_bias_pdb);
		CTOP_CTRL_O26Ax_WrFL(MIP_CTRL_ADMD_MIP, mip_adc02);

		CTOP_CTRL_O26Ax_RdFL(MIP_CTRL_ADMD_MIP, mip_adc03);
		CTOP_CTRL_O26Ax_Wr01(MIP_CTRL_ADMD_MIP, mip_adc03, reg_admd_dpll_odpost, reg_admd_dpll_odpost);
		CTOP_CTRL_O26Ax_Wr02(MIP_CTRL_ADMD_MIP, mip_adc03, reg_admd_dpll_divint,reg_admd_dpll_divint,reg_admd_dpll_icp,reg_admd_dpll_icp);
		CTOP_CTRL_O26Ax_Wr02(MIP_CTRL_ADMD_MIP, mip_adc03, reg_admd_apll_m, reg_admd_apll_m, reg_admd_adc_refsel, reg_admd_adc_refsel);
		CTOP_CTRL_O26Ax_WrFL(MIP_CTRL_ADMD_MIP, mip_adc03);

		CTOP_CTRL_O26Ax_RdFL(CPU_IOMUX_CTRL_WOC, pad_woc66);
		CTOP_CTRL_O26Ax_Wr02(CPU_IOMUX_CTRL_WOC, pad_woc66, reg_pad_ifagc_s_mux_sel, reg_pad_ifagc_s_mux_sel, reg_pad_ifagc_mux_sel, reg_pad_ifagc_mux_sel);
		...
		if (new_standard == DEMOD_MODULATION_DVBS)
		{
			...
			CTOP_CTRL_O26Ax_Wr01(CPU_IOMUX_CTRL_WOC, pad_woc65, reg_pad_diseqc_mux_sel, reg_pad_diseqc_mux_sel);
			CTOP_CTRL_O26Ax_WrFL(CPU_IOMUX_CTRL_WOC, pad_woc65);

			CTOP_CTRL_O26Ax_RdFL(MIP_CTRL_ADMD_MIP, mip_adc01);
			CTOP_CTRL_O26Ax_Wr01(MIP_CTRL_ADMD_MIP, mip_adc01, reg_admd_adc_icon, reg_admd_adc_icon);
			CTOP_CTRL_O26Ax_WrFL(MIP_CTRL_ADMD_MIP, mip_adc01);
 ...
 * @author 	Jeongpil.yun(jeongpil.yun@lge.com)
*/
int DEMOD_O26_NeverlockScan (DEMOD_CTX_T *pDemodInfo, LX_DEMOD_FLAG_T ResetOn, LX_DEMOD_FLAG_T *pScanProcessing, LX_DEMOD_LOCK_STATE_T *pScanLock)
{
	...
	if (RET_OK != DEMOD_O26_Get_NeverLockStatus(scanMode, &neverLockStatus))
	{
	...
	switch (neverLockStatus)
	{
		...
		case LX_DEMOD_DETECTED:
			*pScanProcessing	= LX_DEMOD_FLAG_FINISH;
			//_demodScanLock		= LX_DEMOD_TR_LOCK;  //chungiii check!!!
			_demodScanLock		= LX_DEMOD_LOCKED;
			DEMOD_NOTI("chungiii  LX_DEMOD_DETECTED\n");
			break;
		default:
			_demodScanLock		= LX_DEMOD_UNLOCKED;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_TPOutEnable (BOOLEAN bEnable)
{
	LX_DEMOD_L2_Context *	pFrontend;
	pFrontend = DEMOD_Get_LGDBB_Context();
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_GetTsClkRate (UINT32 *demodTsClkRate)
{
	LX_DEMOD_L2_Context *pFrontend;
	pFrontend = DEMOD_Get_LGDBB_Context();
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Set_IF_Frq (UINT32 ifFrq)
{
	LX_DEMOD_L2_Context *	pFrontend;
	pFrontend = DEMOD_Get_LGDBB_Context();
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Set_AGCPolarity (LX_DEMOD_AGC_POLAR_T agcPola)
{
	LX_DEMOD_L2_Context *	pFrontend;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_Id (UINT8 *pCellIDLength, UINT16 *pId)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O26_Get_SpectrumInvStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_SpectrumStatus (LX_DEMOD_LOCK_STATE_T *pSpectrumAuto, LX_DEMOD_LOCK_STATE_T *pSpectrumInv)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_IFAGC (UINT16 *pIfAgc)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_OperMode (LX_DEMOD_OPER_MODE_T *pOperMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_NeverLockStatus (LX_DEMOD_OPER_MODE_T scanMode, LX_DEMOD_LOCK_STATE_T *pNeverLockStatus)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
************************************************************************************/
/**
 * DEMOD_O26_Get_CarrierFreqOffset
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_CarrierFreqOffset (SINT32 *pFreqOff)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS 	signalStatus;
	SINT32			freqOffset;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_VABER (UINT32 *pVber)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS 	signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_Packet_Error (UINT32 *pError)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_BandWidthMode (LX_DEMOD_BWMODE_T *ChannelBW)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_QAMMode (LX_DEMOD_RF_MODE_T *pQamMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_MseInfo (LX_DEMOD_MSE_T *pSigMse)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS	signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_Lock (LX_DEMOD_LOCK_STATUS_T whatLock, LX_DEMOD_LOCK_STATE_T *lockState)
{
...
int DEMOD_O26_CountryGrpSetting (BOOLEAN bEnable)
{
	LX_DEMOD_L2_Context *	pFrontend;
	pFrontend		= DEMOD_Get_LGDBB_Context();
...
int DEMOD_O26_Get_I2C_Handle (UINT8 portI2C, UINT16 deviceID)
{
	LX_DEMOD_L2_Context *	pFrontend;
	UINT32			handle;
...
int DEMOD_O26_OperModeReset (LX_DEMOD_OPER_MODE_T operMode)
{
	UINT8	reg_admd_t_pdb = 0;
	...
	#if 1  //chungiii 
	CTOP_CTRL_O26Ax_RdFL(MIP_CTRL_ADMD_MIP, mip_adc01);
	CTOP_CTRL_O26Ax_Wr01(MIP_CTRL_ADMD_MIP, mip_adc01, reg_admd_t_pdb, reg_admd_t_pdb);
	CTOP_CTRL_O26Ax_WrFL(MIP_CTRL_ADMD_MIP, mip_adc01);
	#endif
...
int DEMOD_O26_FW_Configuration_Initialize (LX_DEMOD_OPER_MODE_T operMode)
{
	LX_DEMOD_L2_Context *	pFrontend;
	UINT32			retc;
...
int DEMOD_O26_Get_BW (DEMOD_CTX_T *pDemodInfo, UINT16 *pter_bandwidth_KHz)
{
...
int DEMOD_O26_Get_FW_PartInfo (UINT32 *pFW_Info)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int DEMOD_O26_Debug_ReadREGvalue (UINT16 address, UINT32 *pData)
{
	LX_DEMOD_L2_Context *	pFrontend;
	UINT8			pvalue[4];
...
int DEMOD_O26_Debug_ATSC_ReadREGvalue (UINT16 address, UINT32 *pData)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int DEMOD_O26_Debug_ATSC_WriteREGvalue (UINT16 address, UINT32 *pData)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int DEMOD_O26_ADC_Enable (void)
{
	...
	#if 1  //chungiii 
	CTOP_CTRL_O26Ax_RdFL(MIP_CTRL_ADMD_MIP, mip_adc02);
	CTOP_CTRL_O26Ax_Wr01(MIP_CTRL_ADMD_MIP, mip_adc02, reg_admd_bias_pdb, 1);
	CTOP_CTRL_O26Ax_WrFL(MIP_CTRL_ADMD_MIP, mip_adc02);
	#endif
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_O26_Get_Signal_Information
 * Get BER before RS on DVB
 *
 * @param *pVber 	[OUT] UINT32.
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_Signal_Information (LX_DEMOD_SIGNAL_TOTAL_T *pSignalStatus)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS 	signalStatus;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_O26_Get_CFG_Information (LX_DEMOD_SIGNAL_TUNE_CFG_T *pSignalStatus)
{
	LX_DEMOD_L2_Context *	pFrontend;
	LX_DEMOD_L2_SIG_STATUS 	signalStatus;
...
int DEMOD_O26_DVB_Set_MemSconfig (void)
{
	LX_DEMOD_L2_Context *	pFrontend;
...
int DEMOD_O26_DVBC_reset (void)
{
	...
	#if 1	//chungiii 
	CTOP_CTRL_O26Ax_RdFL(MIP_CTRL_ADMD_MIP, mip_adc02);
	CTOP_CTRL_O26Ax_Wr01(MIP_CTRL_ADMD_MIP, mip_adc02, reg_admd_bias_pdb, 0);
	CTOP_CTRL_O26Ax_WrFL(MIP_CTRL_ADMD_MIP, mip_adc02);
	#endif
